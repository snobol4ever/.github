...
**Gates:** `test_gate_template_medium_invisible` GREEN · `test_gate_emit_no_lang` GREEN (LANG-BLIND). ⭐ **RE-PROVED AFTER THE HANDOFF REBASE, and the number moved under me:** mid-session this read **3 sites / ceiling 3**; the rebase that carried my push pulled in seat6's `51b73ce9` (*ab-cell-hoist — the last three MEDIUM_ guards retire, ratchet 3 → 0*), so it now reads **0 sites / ceiling 0, GREEN**. Both readings were correct when taken; **the 0/0 is the one that describes the pushed tree**, and it is recorded here rather than the stale 3/3 precisely because RULES requires re-proving after a rebase. ⚠ Note for anyone spot-checking: a raw `grep -c MEDIUM_ src/templates/bb_*.cpp` still shows 9 hits (bb_define 3, bb_main 2, bb_call_proc_staged 2, bb_goto_dyn 1, bb_zdp_anchor 1) — **all of them inside comments**. The gate counts tokens surviving comment-strip, which is what the rule actually forbids; the raw grep is the very mis-measurement RULES.md's own ratchet history warns about. **Smokes:** Icon **14/14** both modes · Prolog 3/5 both modes (recorded watermark).
