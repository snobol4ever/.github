...
## ⛔⭐ LIVE CURSOR — s3 (2026-08-07, Sonnet s3 — static/initial deep-dive; context exhausted mid-fix)
ICN-FR-3 IN PROGRESS: SCRIP HEAD (working tree dirty — see below). f0+f1+fib still GREEN. SN4 byte-identical confirmed. Watermark 195 at session start.
**ROOT CAUSE OF STATIC/INITIAL NULL-READ — FULLY DIAGNOSED, FIX PARTIALLY LANDED:**
The `initial x := 10; write(x)` bug is a multi-layer issue in the zframe variable storage path:
1. **ZLS vslots stomp frame-local offsets.** `ir_drive_slot_assign` ran `zls_build`, which granted ZLS result-cell offsets (positive, e.g. 96) as vslots for variable names. At emit time `bb_varslot_peek("x")` returned 96 (a FORTH-spine offset, wrong for zframe). Fix: after zls_build, for zframe graphs reset `n_vslots=0` and repopulate params at `[rbp-(i+1)*16]` and locals at `[rbp-(np+j+1)*16]` matching `rt_icn_zframe_args_install`. ALSO scan all IR_VAR/IR_ASSIGN nodes for names not global and not yet vslotted (implicit locals like undeclared `x` in `initial` bodies) and grant each `[rbp-(np+nl+k+1)*16]`. **Fix landed in `src/contracts/scrip_ir.c` `ir_drive_slot_assign`.**
2. **TE-4 guards used `voff < 0` instead of `voff == -1`.** Frame-local offsets are negative (e.g. -16). The absent sentinel from `ir_varslot_of` is -1. Three guards in `emit.cpp` (IR_ASSIGN, IR_REV_ASSIGN, IR_REV_SWAP) aborted on any negative voff. **Fixed to `voff == -1`.**
3. **`bb_var.cpp` guard `op_sa >= 0` rejects negative frame-local offsets.** **Fixed to `op_sa != -1`.**
4. **`bb_assign_local.cpp` three IF guards use `op_sb >= 0`.** **Fixed to `op_sb != -1`.**
**REMAINING OPEN ISSUE — `n1_var_α` unresolved forward reference:** After all four fixes, build succeeds but mode-3 aborts with "bb_emit_end: 5 unresolved forward references to n1_var_α". The VAR template still hits the bomb path. Root cause: `drive_value_slot(nd)` for the VAR node returns `op_off < 0` — ZLS result slots (from `zls_off(nd)` / `nd_slot`) are DISTINCT from the vslots table, but clearing `n_vslots=0` in the zframe path may not affect them. Check `zls_result_off` for the VAR node at head of the chain — if ZLS granted it a result slot (it should, since `zls_build` ran), then `op_off` is valid. If ZLS did NOT grant a result slot for a VAR in a DISJUNCTION-body chain, that is the next bug to fix. The distinction: vslots (name→offset, used by `bb_varslot_peek` for variable cell access) vs result slots (node→offset, used by `drive_value_slot` / `nd_slot` for inter-node value passing via FRQ). Only vslots need the zframe override; result slots come from ZLS and should be intact.
**Working tree state:** 5 files modified, not committed (scrip_ir.c, emit.cpp, lower_icon.c, bb_var.cpp, bb_assign_local.cpp). `make -j4 scrip` succeeds. f0/f1/fib green; static1.icn aborts with n1_var_α unresolved.
**gen SEGV still FR-4 scope, γ-retain. Do not touch until FR-3 complete.**
**NEXT RUNG = ICN-FR-3 continued** — check `zls_result_off` / `nd_slot` for the VAR node in a DISJUNCTION chain; if ZLS result slot is absent, add a fallback in `drive_value_slot` for zframe graph VAR nodes; then commit all five files; then full-suite ratchet.
