...
## ⛔⭐ LIVE CURSOR — s12 (2026-08-08, Sonnet s12 — FR-5: no-rsp-unwind root-cause fix; rung03 cluster GREEN; SCRIP `70c2babe`)
**NEXT RUNG = ICN-FR-5 (continuing — SUSPEND/GEN-3 residual + jcon cluster).** Watermark: **m3 PASS≈225 FAIL≈38 XFAIL=30 TOTAL=293** (measured via 253-of-293 partial sweep at 180s timeout; rung03_suspend_{gen,gen_compose,gen_filter} + rung37_subscript_genproc + rung37_{neg_pos,scan_alt} all now PASS — net +6 from s11 baseline of 219). SN4 R-ICN-D: roman.sno compile md5 `573e9ecd` byte-identical with/without session patch (xa_flat.cpp icn_zframe_gen path never exercised by SN4). Repro quartet f0/f1/gen/fib ALL GREEN m3.

**ROOT CAUSE FOUND AND FIXED THIS SESSION (SCRIP `70c2babe`):**

**γ/ω EPILOGUE RSP UNWIND — THE FRAME-CLOBBER ROOT CAUSE.** The ICN-FR-4 epilogue did `lea rsp,[rbp+kt]` (unwind to entry_rsp = gen_rbp + frame_size) before PLT calls and before jmping the γ-wire. This placed rsp at the TOP of the generator's frame. Any C call from the L(3) landing (rt_proc_call_epilogue_γ, rt_call_arr, write(), PLT lazy-binding resolver, etc.) pushed frames downward from entry_rsp INTO [gen_rbp..gen_rbp+kt) — the live generator frame — clobbering params/locals (e.g. n-param at [gen_rbp+16] in upto()). This explained: (a) why `gen.icn` (no params, 112B frame) passed while `rung03_suspend_gen` (1 param, 272B frame) failed; (b) the PLT resolver's 2500-byte descent clobbering n-param on first γ-exit; (c) mode-3 failure (C call chain depth alone suffices without PLT resolver). **The anchor mechanic §5 γ-RETAIN** says: leave rsp at gen_rbp (the deep frontier after `add rsp,claim` in n6_suspend_α). Subsequent C calls from the caller then push BELOW gen_rbp — outside the frame entirely. **FIX:** Deleted `lea rsp,[rbp+kt]` from both the icn_zframe_gen γ and ω epilogues. The L(3) landing and the caller's fail path are entirely rbp-relative and don't need a specific rsp. PLT calls (rt_gen_get_gamma_wire, rt_gen_get_caller_rbp) now fire while rsp is at gen_rbp (safe); then rbp is overwritten to caller_rbp. Yield value read via callee-saved r14 (= gen_rbp) after the PLT calls.

**REMAINING SUSPEND/GEN-3 (of the original 8):** rung09_loops_until_gen · rung09_loops_repeat_break · rung35_block_body_every_gen_block. SINGLES still failing: rung11_bang_augconcat_bang_concat · rung23_table_table_key · rung37_cset_ops · rung37_proc_lookup. jcon cluster (15+): unchanged. **Next priority: MONITOR-FIRST on rung09_loops_until_gen (simpler shape, likely a different generator-control-flow issue).**
