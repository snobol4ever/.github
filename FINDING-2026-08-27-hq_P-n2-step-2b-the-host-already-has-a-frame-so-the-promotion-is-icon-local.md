...
Measured in an **isolated checkout outside the seat root**, which was the right call and remains so — the isolated tree gets its own objdir by construction, so these numbers are uncontended.

⛔⭐⭐ **AMENDED IN PLACE — THE REASON I GAVE FOR WORKING ISOLATED WAS WRONG, AND ITS SHAPE IS THIS FINDING'S OWN SUBJECT.** This paragraph originally read: *"`/home/claude_P/SCRIP` had up to **four concurrent `make pristine`/`make` runs** from other sessions throughout, and a binary built there segfaulted."* **False, and I escalated it to ceo as URGENT before checking.** ceo answered with the method and I verified it rather than take it: `readlink /proc/<pid>/cwd` puts every one of those builds in **`/home/claude14/SCRIP`, `/home/claude01/SCRIP`, `/home/claude05/SCRIP`** — worker seat roots. **Zero were in my checkout.** My tree was clean at `fcd981f4` throughout, which was already sitting in my own evidence disagreeing with my conclusion. The `sed -i` IR-rename was ceo's, in `/home/claude/SCRIP`, on Lon's order.

⭐ **`ps -eo args` LISTS EVERY PROCESS ON THE BOX AND SAYS NOTHING ABOUT WORKING DIRECTORY.** I read "is anything named `make` running on this machine" as "is anything building in my checkout". That is **exactly the `RULES.md:107` class this FINDING is about** — a correct instrument answering a broader question than the one it was read as answering, with no way to say so — committed by me, in the same session, one screen below where I wrote it up. The `command -v` oracle probe, the brief's `x86_main_prologue()` reading, and this are all one defect wearing three hats. ✅ **The rule: `ps` for existence, `/proc/<pid>/cwd` for locality — never `ps` for locality.**

⭐ **What this does NOT change:** every measurement above was taken in the isolated tree and stands. And the segfault on `rung03_suspend_gen` was never wreckage — it reproduces on a clean isolated build and `rung03_suspend_gen` is in the recorded icon-n2 FAIL-16 witness set, so it matches the baseline exactly.
