...
**⛔ RULING — STDERR IN THE MASTERS (ceo CEO-468, 2026-09-09 16:48 CDT, on hq_T's and hq_B's ask): RULED CEO-468: per-entry ALL.err blocks (hq_V design). stdout graded against ALL.ref and stderr against ALL.err separately; all 886 refs untouched; NO merged 2>&1 refs -- hq_P measured this morning that a combined capture pins the INTERLEAVING, which is a buffering artifact the two modes legitimately differ on (c1b9563d3), so a merged ref would grade buffering, not the program. ALL.err joins ALL.in/ALL.wantrc/ALL.trace as a per-entry side file of the master, which is the shape the standard already has. Half one is SCRIP `4a9433199` (the ladder captures stderr and reports the unasserted debt, gate `test_gate_ladder_asserts_stderr.sh` blocking); half two, the ALL.err assertion, is hq_T's.**
...
**2026-09-26 11:2x CDT (ceo, CEO-1281) — POINT 8 OF THE STANDARD: THE ATTRIBUTE ROW CARRIES THE TEST UNIT'S COMMAND LINE.** Lon, in-chat to the ceo, verbatim: *"We are meant to have stack and heap size parameters stored in the per-test attribute file for all the test suites."* · *"Basically the command-line arguments for compile time and run time (if needed) should be stored per-test unit."* Law: RULES.md, the hard-cap rule's clause 8 (f). The shape ruled: `heap_kb` and `stack_kb` stay (they are `-d`/`-s`); a `compile_args` and a `run_args` attribute join them in every `ALL.csv` (masters and packages) and in a standalone program's sidecars, read by the ONE reader (`corpus_suite_harness.py`, `lib_declared_arena.sh`) — the coo's row. Census at the ruling: masters and packages declare every row; the benchmark kernels of six languages declare nothing (only snocone's 16 do); the Prolog 23 declare since corpus `a91273cef`, found because the Prolog angles ran tak at the shipped 4 MB stack where it needs 64 MB and SKIPped it (SCRIP `3d66e3e09` carries the declarations as switches). Rows: the coo's reader row; one per HQ for snobol4, icon and raku kernels; pascal's and rebus's kernels by the ceo with their benchmark grids.
...
**2026-09-13 19:0x-20:xx (hq_T, MODE NONET — three asks against the shared bodies, all from other seats):**

⭐ **POINT 6, `lib_port_trace.sh` — THE SELF-PINS COULD NOT BE RE-CUT, AND THE FAILURE REPORTED PASS ON ITS OWN RUN.** hq_S cut the rebus refs, read `GATE PASS(0)` 86 of 86, re-ran the same gate on the same tree with no edit, and read `GATE FAIL(1)` 86 of 86 — **the green survived exactly one invocation.** hq_T's earlier cure (`9df862afc`) normalised the origin name out of the trace and was proven on a RELATIVE, SHORT witness; the body handed `$SCRIP` an ABSOLUTE path inside a mktemp dir, and the port-hook operand is the argv string VERBATIM, truncated to 48 chars by the `" %.48s"` stem at `src/templates/x86/x86_asm.h:1882`. Reproduced before curing: the witness at an absolute path prints `lit_string /tmp/tmp.UesRBcLuiF/ladder__rung01_arith_divide.` — volatile component intact, basename cut mid-name so the substitution keyed on it matched nothing — and leaks 2 trace lines; the same witness as `w.reb` prints `lit_string w.reb` and leaks 0. **CURE:** the witness materialises under a SHORT FIXED basename and every arm invokes it RELATIVE from `$W`, including the two m3 `--run` arms, which never `cd`'d and were where the path entered. hq_S's reasoning for rejecting a 48-char-prefix substitution is taken verbatim into the body: it would hard-code a truncation width living in `x86_asm.h` and fail silently the day it changes. **Exposed class, measured by hq_S for all seven, not inferred: rebus, snobol4, snocone leak; icon, pascal, prolog, raku do not** — which is why the raku end-to-end proof could not have surfaced it.

⛔⭐⭐ **AND THE PROCEDURAL ASK WAS TURNED INTO AN INSTRUMENT, BECAUSE IT IS A DUTY NO PROCEDURE KEEPS.** hq_S asked that a cure be graded by CUTTING AND THEN RE-RUNNING, never by the cut's own verdict. Right — and precisely the kind of rule that decays, because **a cut's own run is green by construction: it compares the traces it just wrote.** So `--cut` now REFUSES rc=2 if any normalised line still carries the run's throwaway workdir, on the reasoning that a fresh mktemp dir is by definition a string that cannot match next run. Proven to fail once (old shape re-introduced, guard retained → rc=2 naming origin, mode, count and first offending line, and NO ref written). Graded hq_S's way: rebus `--cut` PASS 86/86 then **two** re-runs PASS 86/86; new ref carries 0 temp-path lines and 172 `<SRC>` lines. **The rebus ref was REVERTED and nothing pushed to corpus — rebus is hq_S's lane and the re-cut is theirs to own.**

⭐ **POINT 1, `lib_ladder.sh` — THE DECLARED-CENSUS LAW WAS ENFORCED AT RUNG GRANULARITY ONLY.** hq_I reported from inside the shared body: a rung with AT LEAST ONE witness satisfies the check, and nothing compared the FORMS column against the origins that exist, so snocone rung23 (six forms declared, four built) graded `8/8` and printed the success shape over two gaps. hq_I did not patch it and asked for a census first, on the grounds that a refusal would red every language whose FORMS column is aspirational. **`scripts/util_ladder_form_census.py` is that census:** 94 of 1058 declared forms have no witness bearing their name, **but 77 sit in rungs with NO witness at all — the ladder DECLARED AHEAD, a roadmap, which is what a construct ladder is for.** Only **17** sit in rungs that HAVE witnesses and grade green over a gap: prolog r11/r12/r13/r18 (5), snobol4 r14/r19/r21 (10), snocone r23 (2). Four of seven languages pass today. **The refusal is NOT turned on:** those 17 are in the cto's, the cfo's and hq_I's lanes, and redding three lanes on a number they have not seen is an outage, not a gate. What landed is the half that harms nobody — the gap is PRINTED, non-fatal, verdict unchanged; snocone rung23 now names `stlimit_set` and `code_exit_value`, which hq_I had to state in words because the instrument would not.

⛔⭐⭐ **TWO INSTRUMENTS OVER ONE POPULATION, DISAGREEING, IS WHAT FOUND THE BUG — see `FINDING-2026-09-13-hq_T-two-instruments-over-one-population-found-a-tab-coalescing-bug-that-re-reading-either-one-would-have-confirmed.md`.** Two versions of one defect, one sitting. (1) The obvious census counts the `WITNESS_ORIGIN` column and reported **218 unbuilt forms for icon and 249 for snobol4** — both false, in the direction that manufactures a crisis. That column carries THREE conventions: literal names (snocone, prolog, raku, pascal), a human glob+count (icon, `ladder__rung01_paper_* (8)`), and an unpopulated dash (rebus, snobol4). Keyed on it, the census answers *"is this column populated in the style I assumed"*, never *"is the form built"*; the real census derives BUILT from `ALL.csv`. (2) The reporting arm then had its own version: it read the TSV with `IFS=$'\t' read`, and **tab is IFS *whitespace*, which bash coalesces**, so any row with an empty cell shifted every field left and it printed a confident false gap list (`pascal: rung6 … ladder__rung06_arrays` — an origin, not a form; `rebus: rung3 … -` — the empty-column dash). Python's `split('\t')` and `cut -f` do not coalesce. ⭐ **Nothing inside the runner could have caught it**: it was internally consistent and printed a clean success shape. The census said ZERO for pascal and rebus while the runner said four each, and **the disagreement was the entire signal** — re-running either one and reading its colour would have confirmed it. Same organism as `FINDING-2026-09-12-hq_T-moving-a-population-blinds-every-instrument-that-derives-it`, from the opposite direction: there an instrument went silently blind and stayed green, here one went silently wrong and stayed confident, and in both only a SECOND derivation of the same number, compared, could find it.

✅ **POINT 7 display seam CLOSED (cfo's ask, already cured at `746adb56a`):** `lib_ladder.sh` filtered the score writer's output through a grep listing only the SUCCESS shapes, so `util_score_row.py`'s ONE-RUNNER refusal — which correctly exits 0, because a bookkeeping refusal must never red a board — matched nothing and vanished for twelve of thirteen seats. Cured as THE CLASS on the cfo's own reasoning: an unmatched filter now falls back to PRINTING WHAT IT WAS GIVEN, so the next refusal wearing a fourth prefix is surfaced too.

⚠️ **RAKU LADDER STATE (hq_T's own standing duty, and the MODE line's "red from rung 2" no longer holds):** the declared ladder is EXHAUSTED at its top — `LADDER.tsv` declares rungs 0..18, all `BUILT`, and `test_raku_ladder.sh` reads **PASS 184/184** both modes with **zero** form-level gaps. The next duty in the lane is the **67 xfails** (all family `smoke`, clustering on class/method + say/replace: `class_method_say_replace` 15, `class_say_replace` 7, `say` 6, `say_replace` 4), each deleted as a faulty test with its reason or rowed as a defect on its rung under NONET's THERE IS NO XFAIL.
...
## ⛔⭐⭐ ABSORPTION LAW — A PROGRAM WHOSE OUTPUT ENCODES ITS OWN FILE NAME CANNOT BE ABSORBED INTO ANY MASTER (hq_V, measured 2026-09-10; routed to this standard by hq_T the same sitting)

**The mechanism, and it is structural rather than a bug anyone can fix in the builder:** absorption **renames** an entry (a loose family becomes a master entry under the builder's own naming), and the runner then writes that entry to disk **under the new name**. So any program whose *output* contains its own file name produces different bytes after absorption than before, and its `.ref` — cut correctly, from the oracle, before absorption — is wrong the moment the program moves. Measured on the cfo's trace pair: **86 differing lines**, with a control run proving that **the rename alone creates the red**, no other change involved.

⭐ **Why this belongs in the standard and not only in a FINDING:** point (2) says every language gets a regression master, and the obvious way to build one is to absorb everything absorbable. This names a class that **must stay a loose pair** — not because it is broken, but because absorption is a renaming operation and the program's correctness is defined partly by its name. The honest home for such a program is the suite's own `ALL.outside.tsv`-style record beside the master, **named, with its reason, out of the absorbed population** — the same treatment point (2)'s neighbours already give to what the oracle refuses. ⛔ **The failure mode if it is absorbed anyway is the worst-shaped one available:** a red that reads exactly like a corrupt master pair, so the next reader suspects the corpus.

⭐ **THE GENERAL FORM, which reaches past absorption:** **when a tool's job is to RENAME things, any property of the thing that depends on its name is silently outside that tool's contract** — and nothing in the tool can detect it, because from inside the rename every byte it touched moved correctly. Ask of any absorber, mover or re-indexer: *what does the thing know about where it lives?*
