...
**Status:** DONE against the row's own DONE-WHEN checklist (§6). Continues the IN-PROGRESS FINDING this same file held after the first sitting — that content is preserved below (§§0–2) and extended (§§3–5).
...
|---|---|---|---|---|
| `box-fusion` | 6 (not 1 — see above) | emitted box code directly | benchmark kernel | **REVIVABLE NOW** — its own blocker (no kernel emitted-code-share measurement) is resolved by §3b (99.79%). HQ's call to re-promote, not this row's. |
| `kill-the-plt` | 1 | PLT/GOT indirection on every RT call from emitted code | benchmark kernel (mode-4) | **VALID, real bucket confirmed** (§3c shows named, non-inlined RT calls e.g. `rt_coerce_num2_d` do occur and are individually attributed) — magnitude not isolated this session; fibonacci is arithmetic-inline-heavy (few RT calls/iter) so it undersells this row, a string/table-heavy kernel would show more. |
...
| `callout-fragment-entry-cost` | 1 | match-time deferred-callout entry/exit overhead | demo (claws5) | **VALID** — same emitted-code/RT-call bucket as kill-the-plt, on a different workload than the one measured this session; not directly re-measured, but the bucket it targets is confirmed real and dominant on non-beauty workloads generally. |
...
## 7. Items for HQ (findings, not blockers — this row continues per protocol regardless)
...
## 8. DONE-WHEN STATUS (brief's own checklist)
