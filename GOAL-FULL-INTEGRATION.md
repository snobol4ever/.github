...
source file
 ...
 ir/ir.h  — one EXPR_t struct, one EKind enum, ALL languages, ALL backends
    ...
    │
    ├──▶  --sm-run:   sm_lower()  → SM_Program  → sm_interp_run()   [x86 default]
    │
    └──▶  --jit-run:  sm_lower()  → SM_Program  → sm_codegen()  → sm_jit_run()   [x86 JIT]
```
...
8. SM layer missing 32 EKinds — --sm-run is SNOBOL4-only today; should cover all 6 languages
...
- [x] **FI-1B** — Wire Rebus into `scrip.c` `main()` and `polyglot_execute()`.
  ...
  at least one `.reb` file runs under `--ir-run` without crashing.
