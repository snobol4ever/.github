...
⛔⭐ **NAMED FORWARD DEPENDENCY (hq_P ruling, accepted the scoping above on this condition — write it down rather than let the next seat rediscover it):** an `@a`-only aliasing fix is correct ONLY because the family currently has one member. **The moment either parse gap above closes — hash-sigil parameters, or `[...]` indexing on a `$`-sigil variable — the aliasing fix MUST be re-examined in that SAME commit**, because at that instant the family acquires a second member and an array-only binding becomes exactly the per-op-filter shape RULES.md forbids. A scope that is correct only because of a missing feature has to say so out loud and stay attached to that feature's absence, not become permanent by default.
