...
## ⭐⭐⭐ M1: THE BEAUTY SIGSEGV IS CURED BY THE STACK PAIR, AND THE NEW WALL IS ONE STATEMENT
**Measured on the same tree, same board (`board_beauty_m1.sh --modes m3`), oracle verified alive (`x64/bin/sbl -bf`), the ONLY difference being the killswitch:**
| arm | rungs 1-20 | rungs 40 · 80 · 160 · 320 · 622 |
|---|---|---|
| **`SCRIP_WIRE_STACK=0` (r10/r11 register wires)** | PASS | **SEGV · SEGV · SEGV · SEGV · SEGV** |
| **default (caller PUSH-PUSH stack pair)** | PASS | **DIFF · DIFF · DIFF · DIFF · DIFF** |
⭐ **THE s194 DIAGNOSIS IS CONFIRMED AND THE CURE IS LANDED.** s194 root-caused the M1 SIGSEGV to the γ/ω wire contract and the RTCC bank BOTH claiming r10/r11, with `rtccb[32]` a flat global that does not nest, so an inner activation entered through C overwrote the outer's banked continuations. Moving the pair to the caller's stack removes the collision by construction, and the crash is **gone in every rung that carried it**. The ladder count is unchanged at **5/10, first red 40** — but the CHARACTER of the red has changed from an architectural crash to a wrong answer, and a wrong answer is dispatchable.
⛔ **THE LADDER NUMBER DID NOT MOVE AND IS NOT CLAIMED AS MOVEMENT.** 5/10 before, 5/10 after. What changed is the failure class, which is the thing that was blocking diagnosis.
### THE NEW WALL, LOCALIZED TO ONE STATEMENT
At rung 40 the oracle emits 40 lines; SCRIP emits 28 — a spurious `Parse Error`, then the line, then it stops. That output shape is `beauty.sno:617` `mainErr1` verbatim (`OUTPUT = 'Parse Error'` then `OUTPUT = Src` then `:(END)`), so the failing statement is **`beauty.sno:611`**:
```
main02            Src            POS(0) *Parse *Space RPOS(0)                    :F(mainErr1)
```
**A TWO-DEFER CHAIN UNDER POS/RPOS ANCHORS RETURNS FAILURE WHERE THE ORACLE SUCCEEDS.** ⭐ Note the trap in reading the output: because **beauty.sno is already its own fixed point**, the raw `Src` that `mainErr1` echoes is byte-identical to correctly pretty-printed output — so SCRIP's line 27 LOOKS like a correct format and is in fact the error path's echo. A reader who does not know `mainErr1`'s shape will mis-read this as "formatted fine, then stopped."
⛔ **AND THE CLASS 0-9 GRID DOES NOT COVER IT:** `ptc8*`/`ptc9*` are 82/82 green on this same build, so the passthru witness family does NOT reach this shape. The next M1 rung is to MINT the missing witness — a chained `*A *B` defer pair under `POS(0) … RPOS(0)` where A is a large grammar — rather than to debug beauty directly.
## RECEIPTS
