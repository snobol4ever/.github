...
## ⭐ ADDENDUM — the GENUINE version of this bug, found the same hour by seat03, and it is the opposite verdict

seat03 reported (`snocone-ast-drift-still-unruled`) that `tests-consolidate-snocone` auto-unparked to FREE — **and there the self-heal genuinely should not have fired.** `GOAL-SNOCONE-100.md`'s FRONT STATUS anchor is an **AND of two conditions**, but only one (`snocone-while-for-loop-second-iteration-error5`) ever had a task file to hang a `BLOCKED-ON:` off. That one went DONE, the self-heal fired correctly on the only half it could see, and the second condition (the TT_ASSIGN parser-fixture drift, `test_snocone_parser_fixtures.sh` still PASS=8 FAIL=59 at a fresh pull) stayed open and invisible to the dispatcher.

**`BLOCKED-ON:<topic>` holds exactly ONE topic.** A conjunction that lives in prose in a GOAL file and not in the queue's own state field cannot be evaluated by the picker, by construction. The self-heal was not wrong; **the interlock was under-expressed.**

seat03's remedy is the correct general pattern and is adopted as this class's answer: **reify the missing conjunct as its own row and block on that** — minted `snocone-parser-fixture-ast-drift-ruling` with a *direction-agnostic* DONE-WHEN (that gate's FAIL=0, whichever way TT_ASSIGN is ruled), then re-parked `tests-consolidate-snocone` behind it. Direction-agnosticism is what makes it honest: the row can close on either ruling without being rewritten to suit the outcome.

⭐ **The two reports arrived within an hour, look identical, and have opposite verdicts** — seat06's was a correctly-satisfied interlock that looked broken; seat03's was a genuinely-open interlock that looked satisfied. Both surfaced as "the self-heal un-parked my row." **The discriminating question is therefore not "did the self-heal fire?" but "does the queue's state field express the WHOLE blocking condition?"** Asked that way, seat06's case answers yes (one blocker, genuinely DONE) and seat03's answers no (two conditions, one expressible). Anyone auditing this subsystem should ask it in that form, because the symptom does not distinguish them and the fixes are opposites: one is *stop suppressing a correct signal*, the other is *make the missing half representable*.
