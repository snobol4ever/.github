...
| **M-G0-CORPUS-AUDIT** ✅ | **Plan** the migration of corpus source programs out of `one4all` and into `corpus`. No files move yet — this milestone produces the migration map only. See full inventory and open decisions below. | `doc/CORPUS_MIGRATION.md` committed `f4d351c` one4all. All three open decisions resolved G-7. Execution checklist in that file — open it first at every migration session. |
