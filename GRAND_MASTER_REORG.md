...
| **M-G-INV-SESSION-BASELINE** | **Gate: confirm full invariant suite runs to completion in the current Claude session environment before any emitter changes.** `CORPUS=/home/claude/corpus bash test/run_invariants.sh` must finish within session timeout and print `snobol4_x86 106/106`. If it times out, diagnose and fix (reduce JOBS, pre-build cache, split suite, or add `--serial` flag). No emitter code (M-G4 Icon/Prolog rows, Phase 3, Phase 5) may be touched until this milestone is green. Rationale: G-9 s12 invariant run timed out — baseline unconfirmed in new environment. | `snobol4_x86 106/106` printed to stdout within 150s |
