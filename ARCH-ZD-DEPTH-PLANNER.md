...
⛔⭐ **ZD-5B IS OPT-IN SINCE SCRIP `32620db58` (hq_snocone 2026-09-23): `SCRIP_ZD_5B=1` arms, the default leaves every ALT arm closure-only.** Measured unsound both ways, on the self-hosted parsers' tree dumper: an armed evaluated-arg leaf (`SPAN(v)`, `v` not foldable) keeps its 16-byte cell on γ while the continuation is planned at the ALT's own depth, so **unreleased** (an unbalanced ALT, blob mode) the continuation read the capture's saved cursor 16 bytes off — `rt_dcap_pump: CORRUPT CAPTURE ENTRY` and a wrong answer — and **released** on γ, backtracking re-entered the arm's β, which pops the cell a second time — SIGSEGV (`s = '12x'; POS(0) (SPAN(digits) | SPAN(alpha)) RPOS(0)`). The closure-only form grades correct on every witness and on ZD-5B's own original witness (`span_any_rpos_branch_2`, ex `alt_span_concat_segv`), whose `SPAN` argument now folds and no longer arms at all. The killswitch census over all seven masters moved 8 SNOBOL4 entries, each with the old configuration's verdict and rc in both modes. A correct re-arm needs the arm's γ to carry its cell into a continuation planned at the arm's depth, balanced across arms — not attempted. Gate: `test_gate_zd_a_back_edge_leaves_a_blob_run_and_alternation_arms_stay_unplanned.sh` (fail-once under `SCRIP_ZD_5B=1`).
...
⛔⭐ **THE BLOB RULE KNOWS BACK EDGES TOO (hq_snocone 2026-09-23, SCRIP `df6b55cb5`).** Inside one run, a port target is "in the run" (no release) by the run rule `k > r || gib` — an edge to an EARLIER member is an exit. But when the run's operand closure reaches outside it (`nblob > 0`: every match whose ALTERNATE lists unclaimed arm leaves in its operands), a second rule decided by closure membership alone, so a `while` loop's back edge inside such a run released nothing: the 11-line witness (an if/else on a match with an alternation, nested in an if that returns, recursing from a `while`) leaked 192 bytes a lap and SIGSEGVed on return, and so did every `parser_*.sc` printing its tree (`bootstrap/tdump.sc`). The blob rule now applies the run rule's direction test to members of this run (`claim == hi`, `rpos <= r`); killswitch `SCRIP_ZD_BLOBBACK=0`.
