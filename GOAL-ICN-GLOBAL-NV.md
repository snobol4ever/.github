...
- [ ] **GN-4 — Wire assign path: `IR_ASSIGN` with global lhs uses `NV_SET_fn` (NEW path, under the switch — KEEP the OLD slot assign). Gate: build + m2 smoke ≥ pre-rung baseline.**
  ...
  - **⚠ SWITCH RETROFIT — the GN-3 read dispatch is currently `nv`-ONLY and must be made switch-aware in GN-4 (so the OLD slot path stays reachable for globals).** GN-3 wired `emit_core.c` IR_VAR as `if (nd->state==1) bb_var_global(nd); else bb_var(nd);` — i.e. a global ALWAYS routes to the NEW `bb_var_global`. That is harmless today (globals EXCISE), but to honor KEEP-BOTH the read dispatch must become: `state==1 && switch==nv → bb_var_global`, else `bb_var` (which, for a `state==1` global under `slot`, emits the OLD frame-slot read — so `bb_var`'s descr-flat arm must accept a `state==1` global in `slot` mode, allocating its `bb_varslot` like any local-shaped global did before). Symmetrically, the `emit_bb.c` IR_VAR flat arm (GN-3 allocates `op_off` for `state==1`) must, under `slot`, take the `bb_varslot_peek` branch instead. **Net: BOTH the read (`IR_VAR`) and write (`IR_ASSIGN`) dispatches consult the switch; neither global path is hard-wired.**
