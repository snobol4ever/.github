...
```
one4all: ad250dce     (EC-UNI-8.1 SM_templates split: 5 family files → 57 per-opcode files mirroring BB_templates/ layout. 7 shared `static` helpers relocated to sm_template_common.h as `static inline`. Each split TU has identical prologue. Adding a new SM opcode now = create one new sm_<opcode>.c + add to sm_templates.h + 1 Makefile entry. Gates floor flag-off AND flag-on: 5/0·23/26·194/36/35. EC-UNI-3-beauty still owed.)
...
- [ ] **EC-UNI-8** — **Full unified template matrix.** Tighten EC-UNI-5/6/7 to the strictest form: exactly **one** C function per BB graph kind in `BB_templates/`, exactly **one** C function per SM opcode in `SM_templates/`, with **every backend × every mode-of-operation** handled as an arm inside the single function. No grouped-by-family multi-fn files. No silo helpers carrying backend-specific switches outside the template.
    - 8.1 ✅ **Granularity normalisation** (one4all `ad250dce`, 2026-05-19, Opus 4.7). Split SM_templates from 5 family files (sm_push_pop_lits, sm_arith, sm_compare, sm_control, sm_pat) into 57 per-opcode files mirroring BB_templates/ layout. Shared `static` helpers (jvm_ret_guard, net_ret_guard, jvm_pat_str_push, jvm_pat_long_push, jvm_pat_noarg_push, jvm_pat_pat_push, jvm_pat_2pat_push) relocated to `sm_template_common.h` as `static inline`. Each split TU: `#include "sm_template_common.h"`, `#include "emit_sm.h"`, single fn definition. Makefile: 5 source-list + 5 compile-rule entries → 57 each. `sm_templates.h` section comments updated. `scripts/ec_uni_8_1_split.py` kept for future opcode additions. Adding a new SM opcode now = create one new `sm_<opcode>.c` + add to sm_templates.h + 1 Makefile entry. Gates floor flag-off AND flag-on: 5/0·23/26·194/36/35.
