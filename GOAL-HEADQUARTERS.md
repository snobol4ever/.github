...
## Session State (2026-05-24i — TSX-DELETE re-scoped + DEL-0/DEL-1/DEL-2 DONE — GATE GREEN)

**one4all HEAD: `50a08f8b`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. Three-section audit GREEN. **This session — TSX-DELETE re-scoped into a propagation ladder + first three rungs done:** A dependency analysis proved single-shot deletion is impossible — the bulk of `insn_*` is reachable ONLY through relocation/PLT wrappers (`emit_seq_*`/`emit_call_sym_plt`/`emit_sym_lea_*`/`emit_prologue`) that are TSX-WIRE-deferred and STILL live in template arms (bb_lit/arbno/capture/pl_*). Deletion must propagate per template family. **TSX-DEL-0 ✅ `bf915876` (prereq):** converted the 5 text-arm `insn_*` sites (sm_defines:69, sm_returns:54, bb_lit:43/44/46) to `s_*asm`/`emit_2asm` string builders — zero `insn_*` remain in ANY template. **TSX-DEL-1 ✅ `bf915876` (orphan sweep):** deleted 9 now-zero-reference `insn_*` (6 rel32 jump encoders + mov_eax_i32 + mov_rdx_i64 + pop_rbp). **TSX-DEL-2 ✅ `50a08f8b` (orphan re-sweep):** deleted 3 dead JCC/JMP macro factories + the then-orphaned `bb_emit_patch_rel8` + 6 header ghost decls (`insn_mov_rax_i64`, `insn_{jmp,je,jne,jl,jge}_r8`). Layer fully drained — 25 `insn_*` defs remain, ALL reachable via reloc/PLT wrappers. **NEXT: TSX-DEL-W-LIT** — first per-family rewire (bb_lit binary arm → inline bytes; snapshot `--run` LEN bytes FIRST as the byte-diff oracle), then W-PORT (bb_arbno/capture port-call shape), W-PL (bb_pl_* PLT+lea), then DEL-FINAL. See the TSX-DELETE ladder in the rung block below.
