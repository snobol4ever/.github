...
**DONE: CPP-0 `bc83802c` (abort), CPP-2a `8db542bb` (fence), CPP-2b `90425d4a` (rem), CPP-2c `8a3b4b39` (len).**

 Two kinds of x86 arm. **INLINE-TEXT** (return-String pays off — concatenation of literal asm lines): `bb_pat_arb, bb_pat_len, bb_pat_pos, bb_pat_tab, bb_arbno, bb_capture, bb_lit` + done {abort, fence, rem}. **DELEGATING** (x86 arm calls a shared emitter — `bb_charset_emit` or `emit_flat_ir` + label relocation — so the whole arm stays imperative, returns ""; conversion is trivial but yields no concat): `bb_pat_alt, bb_pat_any, bb_pat_break, bb_pat_cat, bb_pat_notany, bb_pat_span`. NOTE classification is of the x86 arm only; JVM/NET arms may have inline text but are dominated by imperative jvm_*/net_* helpers (x86-only directive → those are stubs anyway). **→ Convert INLINE-TEXT files next (real Snocone-fold value); DELEGATING files are low-priority mechanical (.c→.cpp, all arms imperative).**
