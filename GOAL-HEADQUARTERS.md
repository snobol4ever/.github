...
```
one4all: 573bc63c     (EC-UNI-8.1 regroup + EC-UNI-8.3 matrix gate: 57-file split rolled back to 10 themed files (sm_push_pop_lits, sm_arith, sm_compare, sm_jumps, sm_halt, sm_returns, sm_pat_anchors, sm_pat_position, sm_pat_control, sm_pat_combine — 57 fns total). scripts/test_gate_em_template_matrix.sh + .py machine-check matrix coverage per fn; initial run flags 213 misses across 73 fns as diagnostic debt. Gates floor flag-off AND flag-on: 5/0·23/26·194/36/35. EC-UNI-3-beauty and 8.3-fixup still owed.)
...
- [ ] **EC-UNI-8** — **Full unified template matrix.** Tighten EC-UNI-5/6/7 to the strictest form: exactly **one** C function per BB graph kind in `BB_templates/`, exactly **one** C function per SM opcode in `SM_templates/`, with **every backend × every mode-of-operation** handled as an arm inside the single function. No grouped-by-family multi-fn files. No silo helpers carrying backend-specific switches outside the template.
    ...
    - 8.3 partial 🔄 **Matrix-completeness gate** (one4all `573bc63c`, 2026-05-19, Opus 4.7). `scripts/test_gate_em_template_matrix.sh` + `.py` machine-check every top-level fn in `{SM,BB}_templates/` for full backend × mode matrix coverage (10 cells per fn, with JS_BIN and BB-X86 auto-skipped). Walks fn bodies with string-literal-aware brace matching to handle multi-fn files. Initial run reports **213 misses across 73 fns** — real diagnostic debt: (a) sm_pat_anchors/combine/control/position one-liner fns still use bare `IS_JVM/IS_JS/IS_NET/IS_WASM` (escaped the EC-UNI-8.2 promoter; auto-fixup attempts had string-literal-collapse / inter-arm-prelude bugs); (b) BB_templates fns mostly lack `IS_WASM_*` (BB x86 was the original target); (c) `sm_jumps/sm_halt/sm_returns` missing `IS_X86_BIN` (EC-UNI-6 owed). Gate is diagnostic, not build-blocking. Future sessions can target one promotion at a time and rerun. **NEXT: 8.3-fixup** — hand-fix the 4 sm_pat_* files' one-liner promotions to drop the miss count.
