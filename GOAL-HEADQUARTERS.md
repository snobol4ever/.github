...
## Session State (2026-05-24f — TSX-INLINE SM sites DONE — GATE GREEN)

**one4all HEAD: `f9b87ef8`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. Audit GREEN. **TSX-INLINE-1 (`94e32073`) + TSX-INLINE-2 (`f9b87ef8`): all ten SM-template `capture_bytes(insn_*)` sites are now raw inline `std::string` byte data.** Pure string byte-builders `u8`/`u32le`/`u64le`/`bytes` added to `emit_str.{h,cpp}` (little-endian, byte-identical to `bb_emit_u32/u64`, no emitter state). Three insn primitives proven: `insn_call_plt(addr)`→`48 B8 <u64le> FF D0`, `insn_mov_edi_i32(v)`→`BF <u32le>`, `insn_mov_esi_i32(v)`→`BE <u32le>`. No template references `capture_bytes` anymore. **Remaining INLINE work: BB `insn_*` direct sites (the TM-rung binary arms — bb_pat_len/tab/etc) are still direct `insn_*` calls, the next INLINE batch. NEW STEP ADDED: TSX-CHARSET** (remove external helper `bb_charset_emit`, inline into the 4 charset callers — Invariant 10/11 fix; prereq for clean TSX-DELETE). **Resume: TSX-INLINE BB batch, then TSX-CHARSET, then TSX-DELETE → TSX-WIRE.**
...
- [ ] **TSX-CHARSET — remove `bb_charset_emit`; inline its body into all four callers (Invariant 10/11 fix).** `bb_charset_emit` (`bb_charset_helper.cpp`, declared `bb_templates.h`) is the last EXTERNAL emission helper — it violates Invariant 10/11 ("all emission stays in that TU; no external helpers") and was granted only a temporary audit exemption in TSX-B1. It is called twice each (rip-rel text path + brokered binary path) by `bb_pat_span`, `bb_pat_any`, `bb_pat_break`, `bb_pat_notany`. Each caller currently does the save/restore-`_.op_name*` dance around `bb_charset_emit(pBB)` in BOTH its MEDIUM_BINARY and MEDIUM_TEXT arms (returning `std::string()` — emission happens as a side effect inside the helper via `emit_*`/`emit_seq_port_call*`). **Step:** (1) move the helper body into each of the four templates as the actual content of their MEDIUM_TEXT and MEDIUM_BINARY arms, returning the built string instead of side-effecting through `emit_*`; the per-template `op_name2`/`op_kind`/rt-fn selector collapses to a constant per file (span→rt_bb_span, break→rt_bb_brk/brkx, any→rt_bb_any, notany→rt_bb_notany). (2) The text arm's `emit_seq_port_call_rip` + `.data` section build and the binary arm's `emit_seq_port_call` are NOT yet inlinable to raw bytes — they are the same PLT/rip-rel/strtab/`.data`-reservation relocation work flagged FLAGGED-EMPTY elsewhere; keep them as the still-TEXT path (text arm builds the asm-text exactly as today) and leave the binary arm FLAGGED-EMPTY (returns `std::string()` with a TSX-WIRE flag comment), matching how the other SM/BB binary arms already defer. The point of THIS step is removing the external-helper indirection, not resolving relocation. (3) Delete `bb_charset_helper.cpp` + `.c`, remove the `bb_charset_emit` decl from `bb_templates.h`, and remove the `bb_charset_helper` HELPER-EXEMPT entry from `util_three_section_audit.sh`. (4) Build clean; `util_three_section_audit.sh` GREEN with no exemption; GATE-PK 442/0/612 NEW=0 GONE=0 (text output byte-identical — same asm text, now built in-template; binary arms still unreached). Prereq for a clean TSX-DELETE (helper wraps `emit_seq_port_call*`, themselves on the delete list).
