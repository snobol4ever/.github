...
- [ ] **ER-1 — emit_str.h API complete.** Audit `emit_io.h` funnel API (emit_1asm .. emit_L3asm, emit_comment, emit_directive). Create matching `s_*` functions in `emit_str.h` returning `std::string`. Byte-validate each against baseline. Proof: `emit_L2asm(lbl, op, operand)` becomes `return s_L2asm(lbl, op, operand)` with identical output.
...
- [ ] **ER-3 — INLINE-TEXT templates to string.** Use baseline-shape method: `bb_pat_abort`, `bb_pat_fence`, `bb_pat_rem`, `bb_pat_len`, `bb_pat_arb`, `bb_pat_tab`, `bb_pat_rtab`. For each: read frozen baseline `.raw`, transcribe as concat of `s_*()` calls + data-injected variables (labels, node-ids, arity). Result: `return s_comment(...) + s_2asm(...) + ...;`. Empirically verify: byte-identical to baseline. GATE-PK green per file. **Status: CPP-2c proved the shape on bb_pat_len.**
- [ ] **ER-4 — DELEGATING templates to string.** `bb_pat_pos`, `bb_pat_rpos`, `bb_pat_any`, `bb_pat_notany`, `bb_pat_break`, `bb_pat_span`, `bb_pat_lit`, `bb_pat_alt`, `bb_pat_cat`, `bb_arbno`. These call relocation-aware helpers (emit_label_define, emit_jmp_label, insn_*) or bb_charset_emit. Convert callsites to concatenate: `return ... + bb_charset_emit(...) + emit_label_define(...) + ...;`. The helpers are STILL imperative internally (they patch relocation lists), but their outputs are strings. Each template arm returns ONE concat, or `std::string()` if it's pure-imperative (e.g., IS_BIN mode). GATE-PK green per file.
