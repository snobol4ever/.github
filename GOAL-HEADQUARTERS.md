...
- [ ] **EC-UNI-INLINE-ALL** ⚡ — **Lon directive, 2026-05-21 session #9: one and only one C template function per SM opcode and per BB kind, with all backend code emission explicit and inline.** Every IS_X86/IS_JVM/IS_NET/IS_JS/IS_WASM arm contains the literal `emit_text*`/`emit_byte*` calls for that backend — no `emit_sm_<op>_dispatch` shims, no `emit_sm_op(SM_*)` table-driver, no `emit_bb_x*` indirection. Helpers stay only at Layer 3 (`emit_text`, `emit_textf`, `emit_byte`, `emit_bytes` — the IO funnel) and at the truly-shared instruction primitives (`insn_*`, `bb_insn_*` in `emit_core.c`). Cost accepted: net LOC will rise, code duplication is the point. **Substeps:**
    - [ ] **INLINE-0** ⚡ — **Dead-code amputation (Lon directive, 2026-05-21 session #9, follow-up):** delete `emit_sm.c` / `emit_bb.c` code that emits without going through templates. Confirmed live path is `emit_walk_codegen → SM_templates/sm_<op>(void)` and `emit_bb_node → BB_templates/bb_<kind>(void)`; the residual `emit_sm_*` / `emit_bb_x*` fns are dead. **Empirical audit, session #9:**
        * **BB-side: 14 of 14 `emit_bb_x*` fns dead.** Zero external callers anywhere in the tree. The only intra-`emit_bb.c` use is `emit_bb_xeps` at line 920 (null-node fallback inside `emit_bb_node`) — delete the call too and replace with `if (!nd) { emit_textf("# BOX EPS()\n…"); return; }` or route through `bb_eps.c` template directly. Candidates for full deletion: `emit_bb_xarbn / xatp / xbrkx / xcallcap / xchr / xdsar / xeps / xfail / xfarb / xfnce / xfnme / xlnth / xnme / xposi / xrpsi / xrtb / xstar / xtb`. Headers (`emit_templates.h:84+`) lose their declarations in the same commit.
        * **SM-side: 71 of 103 `emit_sm_*` fns dead** (external-caller audit, excluding emit_sm.c/.h and emit_templates.h). Confirmed dead: `emit_sm_acomp / add / arith_op / bb_eval / bb_once_proc / bb_pump / bb_pump_case / bb_pump_sm / call_expression / call_fn / capture_fn / capture_fn_args / coerce_num / decr / define / div / exec_stmt / freturn / freturn_s / incr / jump / jump_f / jump_s / lbl / lblopt / macro_library / macro_library_to_path / neg / noop / nreturn_s …` — full list in audit. The 32 "live" fns are the `_dispatch` shims called from `SM_templates/` (those become INLINE-1/2 targets — inline first, then delete the shim along with the body).
        * **Stale "lift trail" comments inside `BB_templates/`** (`bb_atp_template.c`, `bb_dsar.c`, `bb_capture.c`, `bb_rem.c` et al.) reference the now-dead `emit_bb_x*` fns with phrases like "Live path remains emit_bb.c::emit_bb_xatp called from emit_bb.c's own walker." Those comments are themselves stale and contradict empirical truth — update or remove. Truthful replacement: "Inlined from former emit_bb.c::emit_bb_x* during EC-UNI-INLINE-0."
        * **Procedure:** Delete fns in dependency order — leafmost first (no incoming refs), parent last. Per-deletion: `make -j4 scrip` must succeed (catches missed callers); `bash scripts/test_per_kind_diff.sh` must show PASS=420 FAIL=0 (no behaviour change — dead code is dead). One commit per logical group (BB pat fns, BB capture fns, SM nullary corpses, SM arith corpses, …) so revert is fine-grained if a "dead" fn turns out to be reached via an indirect path missed by the static audit.
        * **Stop conditions:** if any "dead" fn's deletion breaks the build, restore it, audit again — there's a caller the static grep missed (e.g. via function-pointer table or macro). Do NOT force-delete.
        * **Exit gate:** `emit_sm.c` drops from 182KB to roughly its `master switch + state + the surviving `_dispatch` shims` — estimate ~30KB. `emit_bb.c` drops from 79KB to ~`emit_bb_node` + dispatcher trio + helpers — estimate ~15KB. GATE-PK PASS=420 FAIL=0 unchanged. Beauty gate suspended throughout (matches existing directive).
