...
## Session State (2026-05-24h — TSX-INLINE COMPLETE — GATE GREEN)

**one4all HEAD: `27b4af72`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. Three-section audit GREEN. **This session — TSX-INLINE BB-insn batch DONE:** inlined determinable binary bytes in `bb_pat_rem`/`bb_pat_len`/`bb_pat_pos`/`bb_pat_tab` (all `insn_*` removed from binary arms) + the one isolated site in `bb_lit` (`mov rdx,len`). Added `bb_sink_str()` helper (pushes inline bytes to live `bb_emit_byte` sink). Removed `bb_pat_pos.cpp.bak`. **TSX-INLINE is now COMPLETE** — zero `capture_bytes` and zero `insn_*` inside any `MEDIUM_BINARY` block across all templates. **Architectural finding:** BB binary arms are LIVE (forced-TEXT detour is SM-only), so BB bytes route through `bb_sink_str`→`bb_emit_buf`, not the returned string. **NEXT: TSX-DELETE** — but ⚠ BLOCKED by a prereq: text-arm `insn_*` calls (`sm_defines:69`, `sm_returns:54`, `bb_lit:43-46`) emit text mnemonics and must be converted to `s_*asm` string builders before the `insn_*` family can be deleted. Convert those first (ER/STRING-CONCAT), then TSX-DELETE → TSX-WIRE.
