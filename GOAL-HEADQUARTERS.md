...
- [ ] **EC-UNI-INLINE-ALL** ⚡ — **Lon directive (session #9, AMENDED session #N+2): grouped templates. Where N opcodes share emit shape, ONE template fn handles all of them via per-backend `switch(g_emit.instr->op)`.** No more one-fn-per-opcode duplication. No external helpers. All emission code stays inside the template TU. Locality 100% preserved; LOC reduction occurs naturally where shape is shared. **Three grouped templates landed (slices 13/14/15) as canonical examples.** **Substeps:**
    ...
    - [ ] **INLINE-4** ⚠ **RECIPE — Lon, 2026-05-21 session #N+3 (he repeated this multiple times, prior agents over-engineered it for hours).**
      ...
      **Procedure per template:**
      1. Grab the **exact emitted text** for this opcode from the huge generated-output reference file — **DO NOT hunt down a SNOBOL4 program to compile**. The reference dump for all (BB/SM × backend) cells already exists; it is what GATE-PK byte-diffs against. See `one4all/baselines/per_kind/` (frozen by `freeze_per_kind_baseline.sh`) — every cell's expected output is right there. The X86 emitted text for each opcode lives in those files.
      2. **Paste that text as a C string literal** inside the template's IS_X86 arm. Replace varying tokens with `%s`/`%lld`/`%d` and let globals (`g_emit.instr->a[*]`, `g_emit.i`, `g_emit.in_body`, etc.) fill them via `emit_textf`.
      3. **Set whatever globals the next template would have needed** (typically `g_pending_pc_label` consumption — if your template's text starts the line with a label column, consume it; the existing fn `emit_sm_consume_pc_label()` returns the pending one). Most templates need nothing.
      4. **Done.** Build → `test_per_kind_diff.sh` confirms byte-identity → commit → next opcode.
      Expected pace: **~10 minutes per template** (find text in baseline file, paste-with-holes, build, gate, commit, repeat). 16 templates remain. One session should sweep most.
      **What NOT to do (mistakes already made):** do NOT create new public headers exposing template machinery. Do NOT route templates through `bb3c_format`/`pat_arg_label`/`emit_sm_lblopt`/`sm_template_lookup` — those are the helpers being **deleted**. Do NOT try to construct a SNOBOL4 program that exercises the opcode just to capture its output — the per-kind audit dump already has every cell's text.
      **Reference location for emitted text:** `one4all/baselines/per_kind/` directory contains the canonical X86 (text + binary) + JVM + NET + JS + WASM output for every SM opcode and BB kind, 1059 cells frozen. Each .out file is the literal text the corresponding template prints. THIS is the source-of-truth to copy from.
      **Targets** (in suggested order — simplest first, most cells in baseline first):
