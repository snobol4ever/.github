...
- [x] **EC-UNI-14-PREREQ** `d6e5c8f1` 2026-05-20 — closed the divergence between `dispatch_one_x86` and the legacy switch.  Root cause was three NULL plumbing gaps: dispatch_one_x86 didn't set `g_emit.{prog,srclines}` per call; `emit_sm_stno_template` hardcoded NULL SrcLines; `emit_sm_return_template` hardcoded NULL prog and pc=0.  Symptoms: SIGSEGV at PC 1803 (SM_DEFINE_ENTRY deref); dropped source-line annotations on SM_STNO; generic RETURN_VARIANT in place of NRETURN_VAR with zeroed pc-arg.  Three-line plumb-through fix; the two shims now read from g_emit.  Beauty.s and beauty.o byte-identical under both flag settings (md5 `40df9e004c3e963c99af716c65f2c970` / `3adbb73f88edcc5416d38baade6faf97`).  EC-UNI-14 unblocked.

- [x] **SUSPEND_VALUE routing fix** `9630b549` 2026-05-20 — pre-existing bug surfaced during EC-UNI-14 recon.  `emit_wasm_from_sm` and `emit_jvm_one_instr` both routed `SM_SUSPEND_VALUE` into `sm_call_fn()` (same case-arm as SM_CALL_FN); now correctly routes to `sm_suspend_value()`.  JS and NET already correct.  Templates are distinct: `sm_call_fn()` emits a plain call (WASM: frame_push/save_var/PC-jump); `sm_suspend_value()` emits Icon-style suspend semantics (extra `sno_clear_var` + `sno_call_frame_close`).  Dormant today — no Icon-on-WASM or Icon-on-JVM smoke exercises SUSPEND_VALUE.  Gate floor unchanged (beauty md5 `40df9e00…`, all smoke counts identical).

- [x] **EC-UNI-14(a)** `66cf8506` 2026-05-20 — shared `emit_sm_dispatch(void)` introduced in `emit_core.c`; ~163 case-arms across WASM/JS/NET walkers collapsed into one canonical opcode→template mapping.  Returns 1 when the template emits its own PC transfer, 0 otherwise.  Companion `sm_op_is_dispatched(SM_op_t)` lets walkers distinguish dispatcher no-ops from truly unknown opcodes.  EC-UNI-0 scaffold (`emit_sm_dispatch(SM_sequence_t*, FILE*, mode)` returning -1) deleted — superseded by the parameterless g_emit-reading form.  Backend-only overrides retained where templates can't cover: WASM aliases SM_INCR/SM_DECR to ADD/SUB (rt fidelity gap); JS inlines SM_PUSH_EXPRESSION's `rt.push_null();`; NET inlines SM_LABEL frame setup (needs walker-local fn_params) and SM_EXEC_STMT.  Side-effect (safe): dispatcher's broader coverage adds SM_PUSH_LIT_CS and SM_PAT_FENCE1 arms to JS (legacy switch silently dropped both); no current JS-targeting frontend emits either.  Byte-identity sweep: all SNOBOL4 smoke programs × {jvm,js,wasm,net} + all Snocone smoke × x86 — zero drift.  Net LOC: +192 / −181 = +11 (dispatcher + helper slightly larger than the three switches).  Structural payoff: one canonical mapping for future template additions.

#### Open sub-rungs

- [ ] **EC-UNI-14(b) (NEXT)** — fold JVM and x86 silo walkers into `emit_sm_dispatch`.  JVM is structurally different (function-table method-split via `emit_jvm_one_instr` + `emit_jvm_sm_range`; no per-PC dispatch loop), so the first piece is whether to (1) keep the method-split wrapping and have it call `emit_sm_dispatch` per PC, or (2) reshape JVM to per-PC dispatch like WASM/JS/NET — open design call.  x86's `dispatch_one_x86` is already structurally aligned with `emit_sm_dispatch`; converging the two means tearing out the unified-dispatch flag entirely once x86's switch fully covers the legacy switch (currently `dispatch_one_x86` returns -1 for ~30 uncovered opcodes).

- [ ] **EC-UNI-14 (former monolith)** — single dispatcher in `emit_core.c` is now live (EC-UNI-14(a)); the deletion sub-step (delete `emit_walk_codegen`/`emit_jvm_one_instr`/`emit_js_from_sm`/`emit_net_from_sm`/`emit_wasm_from_sm`/`dispatch_one_x86` + ~30 `emit_sm_<op>_dispatch` helpers; flip flag default-ON then delete) blocks on EC-UNI-14(b) closing the JVM and x86 paths.  Expand `emit_bb_node(void)` to all 21 BB kinds (includes wiring the four `bb_pl_*` from 13(e)).  Net LOC at full close: −2500 to −3500.
