...
## Session State (2026-05-24n — TSX-DEADCODE-SWEEP: 28 dead emitter fns deleted (195 lines) — GATE GREEN)

**one4all HEAD: `87492642`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. Three-section audit GREEN. Prolog BB honest 124/0/0. smoke jit three-mode parity 184, `--run` 186/75. **This session — dead-code sweep (Lon directive): now that DEL-FINAL moved 100% of binary emission into BB/SM/XA templates + the bb_emit_* sink, systematically scanned the whole emitter for zero-caller functions and deleted them. 28 functions / 195 lines across 10 files. Byte-neutral (all had 0 callers). Each verified against the FULL tree (incl. headers/macros/dispatch) before deletion.**

- **Group A (8, 0-ref anywhere):** `emit_bb_box_banner`, `emit_bb_ptr_slot`, `emit_io_is_buffered`, `emit_macro_begin`, `emit_macro_end`, `emit_mov_rdi_imm64`, `emit_seq_noop_macro`, `emit_store_delta` (+ ghost `emit_load_delta` decl, defless).
- **Group B (emit_flat_* family):** 7 fns (`emit_flat_label`, `emit_flat_data_section/string/quad/long`, `emit_flat_text_section`, `emit_flat_intel_syntax`) + their 7 compat macros in emit_bb.h (`flat3c_label`/`flat_data_*`/etc., all 0-caller) + decls.
- **Group C (7 legacy void(FILE*) JVM/NET wrappers, superseded by _str siblings during ER wave):** `jvm_init_ms_int`, `jvm_init_ms_only`, `jvm_init_ms_str`, `jvm_val_helper`, `net_ms_length`, `net_ctor_none`, `net_spec_zw` + their decls in bb_template_common.h. (Templates call the `_str` versions; the plain `void` versions had 0 callers.)
- **Group D:** `s_L3asm` + unused `s_L3asm_c` wrapper.

**KEPT (verified LIVE, NOT dead — do not delete):** (1) `emit_seq_port_call_rip` — 6 template callers (bb_arbno/bb_capture mode-4 MEDIUM_TEXT); it is text-assembly emission, not a byte emitter. (2) The `emit_form_*` family (`emit_form_mem3/mem4/nullary1/reg32_imm32/reg_reg2/reg_reg3`) + the sigma/delta chain (`emit_load_sigma`, `emit_sigma_plus_delta`, `emit_mov_rdx_rax`, `emit_mov_eax_imm32`, `emit_pop_rbp`, `emit_xor_edx_edx`, `emit_add_delta_imm`, `emit_sub_delta_imm`, `emit_push_r10`/`emit_pop_r10`, `emit_test_eax_eax`) — all reached from the **live `emit_bb.c` flat-driver epilogue** (`bb_build_flat`/`bb_build_brokered`, ~lines 466-480: succ/fail labels, sigma+delta, ret). NOTE a zero-caller .c-only scan FALSE-POSITIVES `emit_form_*` because their callers are header-inline wrappers (`emit_mov_rax_rcxmem` etc. in emit_form.h) — confirm via header grep before ever flagging dead.

**⚠ ARCHITECTURAL NOTE for next session:** the `emit_bb.c` flat-driver epilogue (emit_sigma_plus_delta + emit_form_* + emit_pop_rbp + emit_ret at ~466-480, and the 4-byte brokered prologue inlined at bb_build_brokered) is binary/text emission **outside a template** — it is the BB box's α/β/γ/ω frame wrapper, not a per-opcode emitter. Per Invariant 15 the long-term goal is for this to also move into a template (an XA_* frame-wrapper opcode or the BB box's own prologue/epilogue template). That is TSX-WIRE / architectural work, deliberately out of scope for the dead-code sweep. The `emit_form_*`/sigma-delta chain lives or dies with that epilogue conversion.

**NEXT candidates (unchanged from 2026-05-24m + sweep follow-ups):** (a) trivial gate-neutral sweep of the pre-existing dead `emit_seq_frame_enter`/`frame_leave`/`mov_edx_i32` ghost decls + the 44-decl `bb_insn_*` legacy alias block in emit_core.h (defless declarations, never touched); (b) convert the emit_bb.c flat epilogue + brokered prologue into a template (XA frame-wrapper) — then the `emit_form_*`/sigma-delta chain orphans and can be deleted; (c) ER-8 relocation rethink for layout-dependent displacements (bb_lit memcmp-PLT / rip-rel-lea / rel32 still use `emit_call_sym_plt`/`emit_sym_lea_rcx`/`emit_jmp`). ⛔ Beauty gate SUSPENDED.
