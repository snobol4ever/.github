...
**TSX-DELETE — staged propagation ladder (re-scoped 2026-05-24i, Sonnet 4.6).** The single-shot "delete the whole `insn_*` family now" is **impossible as one atomic step**: a dependency analysis (this session) proved the bulk of `insn_*` is reachable ONLY through the relocation/PLT-dependent wrappers (`emit_seq_*`, `emit_call_sym_plt`, `emit_sym_lea_*`, `emit_prologue`) that are **explicitly deferred to TSX-WIRE** and are STILL CALLED from live template arms (bb_lit, bb_arbno, bb_capture, bb_pl_*). Chain: `template → emit_seq_*/emit_call_sym_plt/emit_sym_lea_* → insn_*`. The middle layer cannot die until each template family's binary arm is rewired to inline bytes (= TSX-WIRE work, per family). So deletion **propagates**: each rewired family frees a batch of `insn_*`, which is then swept. The ladder below does that. **GATE-PK 442/0/612 NEW=0 GONE=0 invariant after every sub-step.** Each TSX-DEL-W* step also requires the byte-snapshot oracle (capture `--run` JIT bytes for that family's sample BEFORE its insn_* go, compare after) — that snapshot is a TSX-WIRE deliverable, NOT needed for the zero-reference orphan sweeps (DEL-1/DEL-2) which delete unreachable code with no current byte output.
  ...
  - [ ] **TSX-DEL-W-LIT — rewire `bb_lit` binary arm to inline bytes, then free its wrappers.** This is the first TSX-WIRE family. `bb_lit` MEDIUM_BINARY still calls `emit_seq_bounds_len`, `emit_seq_sigma_delta_rdi`, `emit_seq_lea_rsi_sym`, `emit_call_sym_plt("memcmp")`, `emit_sym_lea_rcx`.
    **⚡ ORACLE FOUND (2026-05-24i, Sonnet 4.6) — no bespoke dump needed.** The existing per-kind gate cell **`baselines/per_kind/x86/binary/BB_PAT_LIT.bin` (125 bytes)** IS the byte-length oracle, regenerated identically by `./scrip --audit-per-kind <dir>` (driver: `src/tools/emit_per_kind_audit.c`, dispatched via `scrip.c:71`). The gate compares `x86/binary` cells by LENGTH (ASLR-tolerant, see `test_per_kind_diff.sh:60-79`) — EXACTLY the channel TSX-CHARSET used to validate its inlined charset binary arms. So W-LIT mirrors TSX-CHARSET: inline the 125-byte shape via `bytes()`/`u64le()`/`emit_fmt`, embedded addrs as live values, rel32 displacements stay 0 (relocation deferred). Verify by `wc -c` on the regenerated `BB_PAT_LIT.bin` == 125 and GATE-PK green.
    **⚠ REACHABILITY CAVEAT (2026-05-24i) — empirical probes:** instrumented `bb_build_flat`/`bb_build_brokered` entry + sealed-blob dump; ran literal-match programs (`line "world" . found`, compound `("wor" "ld")`, alternation `("foo"|"bar")`) under `--run` and `--interp --bb=wired`. **NEITHER builder was entered** — these programs lower literals to the `SM_PAT_LIT` opcode (→ `PAT_LIT` macro → `rt_pat_lit` runtime call, via `sm_pat_anchors.cpp`), NOT to a `bb_lit` BB box. So `bb_lit`'s binary arm appears **unreached by ordinary literal-match programs**; it is exercised only by `--audit-per-kind` (the harness drives every template arm directly) and possibly by complex patterns that force `SM_EXEC_BB` bridge boxes (not yet reproduced). **Implication:** like the SM arms, `bb_lit` binary may be effectively dormant in the live pipeline → its rewire could be gate-neutral (audit-cell length is the only consumer). CONFIRM the bridge-box path before assuming fully dead. Diagnostic patch was reverted (RULES: never commit diagnostics) — tree clean at `50a08f8b`.
    After `bb_lit` references none of those wrappers, check each wrapper for other callers; delete the now-orphaned ones + the `insn_*` they uniquely pulled in. GATE-PK green + `BB_PAT_LIT.bin` length unchanged.
