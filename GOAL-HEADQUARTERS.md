...
```
one4all: d6e5c8f1   (EC-UNI-14-PREREQ: dispatch_one_x86 plumbs prog+srclines into g_emit; sm_stno_template and sm_return_template shims read from g_emit instead of hardcoded NULL/0; beauty.s and beauty.o byte-identical under both SCRIP_UNIFIED_DISPATCH settings; prior 1544362a beauty artifacts cleanup; c01ac05f retargeted beauty.sno paths)
corpus:  5fc1427    (deleted feature-rich stray demo/beauty.sno; prior be6f478 renamed beauty/ → beauty_suite/)
.github: (this commit — PLAN row + watermark for EC-UNI-14-PREREQ closure; prior 0236e579 — GOAL-NET-BEAUTY-SELF note + 13(e) handoff)
...
unified-dispatch divergence: CLOSED 2026-05-20 (EC-UNI-14-PREREQ, one4all@d6e5c8f1). beauty.s under SCRIP_UNIFIED_DISPATCH=1 now byte-identical to legacy (md5 `40df9e004c3e963c99af716c65f2c970`, 882901 bytes); beauty.o assembles to md5 `3adbb73f88edcc5416d38baade6faf97` (494336 bytes) under both flag settings. Root cause was dispatch_one_x86 leaving `g_emit.{prog,srclines}` NULL per call combined with two public template shims (`emit_sm_stno_template`, `emit_sm_return_template`) hardcoding NULL/0 instead of reading from g_emit. EC-UNI-14 unblocked — flag can default-ON whenever EC-UNI ladder reaches it.
