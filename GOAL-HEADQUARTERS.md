...
## Session State (2026-05-24i — TSX-DELETE re-scoped + DEL-0/DEL-1 DONE — GATE GREEN)

**one4all HEAD: `bf915876`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. Three-section audit GREEN. **This session — TSX-DELETE re-scoped into a propagation ladder + first two rungs done:** A dependency analysis proved single-shot deletion is impossible — the bulk of `insn_*` is reachable ONLY through relocation/PLT wrappers (`emit_seq_*`/`emit_call_sym_plt`/`emit_sym_lea_*`/`emit_prologue`) that are TSX-WIRE-deferred and STILL live in template arms (bb_lit/arbno/capture/pl_*). Deletion must propagate per template family. **TSX-DEL-0 ✅ (prereq):** converted the 5 text-arm `insn_*` sites (sm_defines:69, sm_returns:54, bb_lit:43/44/46) to `s_*asm`/`emit_2asm` string builders — zero `insn_*` remain in ANY template. **TSX-DEL-1 ✅ (orphan sweep):** deleted the 9 now-zero-reference `insn_*` (6 rel32 jump encoders + mov_eax_i32 + mov_rdx_i64 + pop_rbp), 34→25 defs, build clean. **NEXT: TSX-DEL-2** (re-census for further orphans: `insn_*_r8` jumps, `insn_mov_rax_i64`, `bb_emit_patch_rel32`), then the per-family rewire+delete rungs **TSX-DEL-W-LIT → W-PORT → W-PL → DEL-FINAL** (each is incremental TSX-WIRE work + a byte-snapshot oracle). See the TSX-DELETE ladder in the rung block below.
...
**TSX-DELETE — staged propagation ladder (re-scoped 2026-05-24i, Sonnet 4.6).** The single-shot "delete the whole `insn_*` family now" is **impossible as one atomic step**: a dependency analysis (this session) proved the bulk of `insn_*` is reachable ONLY through the relocation/PLT-dependent wrappers (`emit_seq_*`, `emit_call_sym_plt`, `emit_sym_lea_*`, `emit_prologue`) that are **explicitly deferred to TSX-WIRE** and are STILL CALLED from live template arms (bb_lit, bb_arbno, bb_capture, bb_pl_*). Chain: `template → emit_seq_*/emit_call_sym_plt/emit_sym_lea_* → insn_*`. The middle layer cannot die until each template family's binary arm is rewired to inline bytes (= TSX-WIRE work, per family). So deletion **propagates**: each rewired family frees a batch of `insn_*`, which is then swept. The ladder below does that. **GATE-PK 442/0/612 NEW=0 GONE=0 invariant after every sub-step.** Each TSX-DEL-W* step also requires the byte-snapshot oracle (capture `--run` JIT bytes for that family's sample BEFORE its insn_* go, compare after) — that snapshot is a TSX-WIRE deliverable, NOT needed for the zero-reference orphan sweeps (DEL-1/DEL-2) which delete unreachable code with no current byte output.
  ...
  - [ ] **TSX-DEL-2 (orphan re-sweep) — after DEL-1, re-run the census and delete any FURTHER `insn_*`/helper now at zero refs.** Candidates to re-check: `insn_jmp_r8`/`je_r8`/`jne_r8`/`jl_r8`/`jge_r8` (kept in DEL-1 but may be pre-existing dead), `insn_mov_rax_i64`, `bb_emit_patch_rel32` (was only called by the deleted rel32 encoders — verify no other caller, then delete its sink machinery if orphaned). Census command: `for fn in $(grep -oP "void (insn_[a-z_0-9]+)" emit_core.c | grep -oP "insn_[a-z_0-9]+" | sort -u); do refs=$(grep -rn "\b$fn\b" src --include=*.c --include=*.cpp | grep -vP "void $fn" | grep -v emit_core.h); [ -z "$refs" ] && echo "ORPHAN: $fn"; done`. Delete only true zero-ref. GATE-PK green.
  - [ ] **TSX-DEL-W-LIT — rewire `bb_lit` binary arm to inline bytes, then free its wrappers.** This is the first TSX-WIRE family. `bb_lit` MEDIUM_BINARY still calls `emit_seq_bounds_len`, `emit_seq_sigma_delta_rdi`, `emit_seq_lea_rsi_sym`, `emit_call_sym_plt("memcmp")`, `emit_sym_lea_rcx`. ⚠ Snapshot the `--run` bytes for the LEN sample (`/tmp/len_sample.sno`) FIRST. Inline each sequence's bytes (per TSX-INLINE method; embedded addrs via `u64le`/`emit_fmt`, rel32 displacements stay 0 / flagged for the relocation pass). Once `bb_lit` references none of those wrappers, check each wrapper for other callers; delete the now-orphaned ones + the `insn_*` they uniquely pulled in (`insn_mov_eax_r10mem`, `insn_add_eax_i32`, `insn_cmp_eax_rcxmem` partially shared — census-gate each). Byte-diff vs snapshot. GATE-PK green.
  - [ ] **TSX-DEL-W-PORT — rewire `bb_arbno` + `bb_capture` binary arms (the `emit_seq_port_call`/`emit_seq_port_call_rip` 104-byte port-call shape, same as TSX-CHARSET inlined).** Reuse the charset port-call inline pattern (`bytes()`/`u64le()`, two 52-byte blocks, rel32=0). Snapshot both families' `--run` bytes first. After rewire, delete `emit_seq_port_call`/`_rip` and the `insn_*` they uniquely use (`insn_push_r10`/`pop_r10`/`sub_rsp_i8`/`add_rsp_i8`/`mov_rdi_i64`/`mov_esi_i32`/`call_plt`/`cmp_al_i8` — census-gate, several are shared with W-PL). GATE-PK green.
  - [ ] **TSX-DEL-W-PL — rewire all `bb_pl_*` binary arms (`emit_call_sym_plt`, `emit_sym_lea_rcx`).** Families: bb_pl_atom/var/seq/builtin/unify/arith. Snapshot a Prolog `--run` sample first (factorial or similar — see GOAL-PROLOG-BB). Inline the PLT-call + lea-sym byte shapes. After rewire, `emit_call_sym_plt` + `emit_sym_lea_rcx` should reach zero template callers → delete them + their internal `insn_*`. GATE-PK green + Prolog BB honest count unchanged.
  - [ ] **TSX-DEL-FINAL (the deletion the rung exists for) — with every template binary arm inline and every wrapper orphaned, delete the rest.** Remaining `insn_*` family, `emit_seq_cmp_delta_i`, `emit_sym_lea_r10`, `emit_prologue`'s insn_* body, `bb_emit_byte`/`bb_emit_u32`/`bb_emit_u64`/`bb_emit_i32`/`bb_emit_begin`/`bb_emit_end` + the `bb_emit_buf`/`bb_emit_pos`/`bb_emit_size` sink, the binary arms of `emit_jmp`/`emit_label_define`/`emit_store_delta`/`emit_add_delta_imm`, and `capture_bytes` itself. Remove `emit_str.cpp`'s `<functional>` include. Build clean with `-Werror=unused`/linker confirming nothing references them. GATE-PK green. **This completes TSX-DELETE.** (Note: the actual detour-opening so inline bytes become output is TSX-WIRE below; the W-* steps above each open one family's detour incrementally, and TSX-WIRE finalizes by deleting the `emit_sm.c` forced-TEXT + `emit_bb.c` binary driver.)
