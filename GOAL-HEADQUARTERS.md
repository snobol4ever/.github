...
## Session State (2026-05-23g)

**one4all HEAD: `1cc7ebea`** — CPP-2 continued. ALL genuine INLINE-TEXT BB leaves now converted: added bb_pat_arb (`7c0e6bfb`, IS_TEXT-guarded so binary x86 stays a stub — ARB had an `if (IS_TEXT)` guard the prior leaves lacked) and bb_pat_tab+rtab (`84fa42ca`, data-injected n + tab/rtab branch). Then began the DELEGATING set with bb_pat_pos+rpos (`c16eff2b`) and bb_pat_any (`1cc7ebea`) — mechanical .c→.cpp, all arms imperative (return std::string()). **Terrain correction (by inspection):** of the goal's nominal INLINE-TEXT list, only arb/len/tab are genuinely inline-text; pos/rpos, arbno, capture, lit are DELEGATING (call insn_*/emit_seq_port_call_rip/emit_label_define/g_cap_fixup_cb/bb_charset_emit + create runtime objects — no literal-asm concat). See corrected CPP-2 terrain note + mechanical recipe + two hazards below. GATE-PK 419/0/635 NEW=0 GONE=0 after every commit. ⛔ Beauty gate remains SUSPENDED.
...
- [x] **CPP-0 — build spike. ✅ COMPLETE 2026-05-23 (Sonnet, return-String variant).** Lon directive amended the rung: skip mechanical .c→.cpp, go straight to return-String. `emit_io.c`→`g++` not needed; instead added `CXX:=g++` + `CXXRT` (`-std=c++17`), converted `bb_pat_abort.c`→`.cpp`, added `emit_str.{h,cpp}` (C++ line-builders `s_1asm..s_L3asm`/`s_directive`/`s_comment`/`emit_fmt` — byte-identical to emit_io funnels but RETURN std::string), added `emit_text_n(const char*,size_t)` NUL-safe text sink so one std::string carries text+binary. Final link switched to `g++` (binary +140KB libstdc++). **The shape:** x86 text arm is now `return s_comment(...) + s_2asm(...) + s_L2asm(...);` — Snocone port = delete `+`. **Decisions taken:** (C) IS_BIN relocation arm + jvm_*/net_* layer-2 helpers stay IMPERATIVE (they mutate patch list / FILE*), return ""; only text arms return-String. **Header fixes (byte-neutral, but MERGE-GATED files — notify live sessions):** self-tagged `bb_label_t` (`emit_core.h`) + `BB_graph_t` (`BB.h`) so `struct X`==`X` in C++; renamed `typename` param→`type_name` (`snobol4.h`). **Gotchas for the bulk wave:** (1) `#include <string>` MUST be outside `extern "C"`; only project C headers inside. (2) add `#include "emit.h"` inside extern "C" for emit_directive/emit_comment. (3) UTF-8 in JS string literals → octal-escape (`\316\261`=α); UTF-8 *identifiers* (net_α_hdr) compile fine under g++13 default charset. (4) DESCR_t compound-literal macros (INTVAL/REALVAL/STRVAL) are INERT unless expanded — no problem for templates that don't expand them. GATE-PK 419/0/635 NEW=0 GONE=0. GATE-M 11 (pre-existing) / GATE-E gate1_beauty+gate4_icon (pre-existing) — no new regressions.
...
- [ ] **CPP-2 — bulk rename, leaves first.** Rename `BB_templates/*.c`, `SM_templates/*.c`, `XA_templates/*.c` → `.cpp` in waves (BB, then SM, then XA), updating the Makefile source list each wave. **DONE: CPP-2a `8db542bb` (bb_pat_fence), CPP-2b `90425d4a` (bb_pat_rem).** Plus CPP-0 `bc83802c` (bb_pat_abort).

**⚡ BASELINE-SHAPE METHOD (Lon directive 2026-05-23f — the FAST path, use this):** Don't preserve helper-call structure and convert helpers later. Instead read the frozen baseline `.raw` for the cell (`baselines/per_kind/<be>/text/BB_<KIND>.<ext>.raw`) — that IS the exact emitted output — and rebuild the arm as ONE returned concat of that text. The helpers (`jvm_*`/`net_*`/`bb_charset_emit`) get DELETED eventually because their *output* is already captured. **WHY IT WORKS THIS TIME (prior attempts stalled):** earlier passes tried to convert the live C helper call-graph in place, which fought the imperative FILE*-writing structure at every node. NOW the template is already C++ and purely concat-driven (return std::string), so the frozen baseline text drops in directly as string literals joined by `+` — there is no plumbing to reconcile, only literal text + a handful of re-injected variables. The generated baseline IS the source of truth; we are transcribing it, not re-deriving it. This is mechanical transcription, not engineering — fast and low-risk. Only re-inject the variable parts the normalizer (`scripts/normalize_per_kind_cell.py`) flags: (a) labels (`_.lbl_succ/fail/back` → `+ lbl +` or via `emit_fmt("jmp %s", lbl)`); (b) node-ids `_<sid>_<nid>` (`make_pat_%d_%d`); (c) data values baked into the asm — **THIS is the one judgment call**: e.g. LEN's `add eax, %d` (n=pBB->ival), ANY's `cmp al, 99` + `.string "abc"` (charset from arg), SPAN/BREAK charset, LIT string. Find the literal in the .raw, replace with the field. Everything else is fixed text → paste verbatim. **CPP-2c `8a3b4b39` (bb_pat_len) proved this on a data-computed template — byte-identical incl. injected n.** No second-guessing: the baseline is ground truth. Validate: `make scrip` + GATE-PK + optionally `diff <(./scrip --audit-per-kind /tmp/x; cat /tmp/x/<be>/text/BB_<KIND>...) baselines/...`.

**DONE: CPP-0 `bc83802c` (abort), CPP-2a `8db542bb` (fence), CPP-2b `90425d4a` (rem), CPP-2c `8a3b4b39` (len), CPP-2 `7c0e6bfb` (arb — IS_TEXT-guarded so binary x86 stays stub), CPP-2 `84fa42ca` (tab+rtab). ⇒ ALL genuine INLINE-TEXT BB leaves converted.**

**DELEGATING-FILE MECHANICAL RECIPE (.c→.cpp, all arms imperative) + TWO HAZARDS (learned on bb_pat_pos `c16eff2b`):** wrap includes (`#include <string>`+`emit_str.h` ABOVE `extern "C"`; project headers + `emit.h` inside); rename `void NAME(...)`→`static std::string NAME_str(...)`; convert C `return;`→`return std::string();`; append `extern "C" void NAME(...){ auto s=NAME_str(...); if(!s.empty()) emit_text_n(s.data(),s.size()); }`. ⚠ HAZARD 1: comment text must NOT contain `*/` — e.g. `insn_*/emit_text_jmp` closes the comment block early → reword to `insn_ / emit_text_jmp`. ⚠ HAZARD 2: a blanket `return;`→`return std::string();` ALSO clobbers literal `return;` substrings inside `emit_textf(...)` JS/WASM string arms (e.g. JS `{ self.fail.α(); return; }`) → after the bulk replace, grep `emit_textf.*return std::string` and restore those to literal `return;`. The only legitimate `emit_textf(...); return std::string();` is the trailing WASM arm's real C statement. DELEGATING remaining (larger, big NET arms): `bb_pat_notany 112, bb_pat_break 168, bb_pat_span 169, bb_lit 228, bb_pat_alt 233, bb_pat_cat 276, bb_arbno 414`. ✅ DONE delegating: `bb_pat_pos c16eff2b`, `bb_pat_any 1cc7ebea`. ⚠ PER-FILE QUIRK (charset users — any/notany/span/break): x86 arm calls `bb_charset_emit` which is implicitly declared in C but needs `#include "bb_templates.h"` inside the extern "C" block under C++.

 Two kinds of x86 arm. **INLINE-TEXT** (return-String pays off — concatenation of literal asm lines): `bb_pat_arb ✅, bb_pat_len ✅, bb_pat_tab ✅` + done {abort, fence, rem}. ⚠ CORRECTED 2026-05-23g (Sonnet, by inspection): `bb_pat_pos/rpos, bb_arbno, bb_capture, bb_lit` were mis-listed as INLINE-TEXT — they are in fact **DELEGATING**. pos/rpos calls `insn_*`/`emit_text_jmp`/`emit_sym_lea_rcx` (binary-reloc-aware); arbno+capture call `xa_dispatch`+`emit_seq_port_call_rip`+`emit_label_define`+`g_cap_fixup_cb` reloc callback + create runtime objects (`rt_bb_arbno_new`/`bb_cap_new`); lit has 12 delegating calls. None concatenate literal asm. **DELEGATING** (x86 arm calls a shared emitter — `bb_charset_emit` or `emit_flat_ir` + label relocation — so the whole arm stays imperative, returns ""; conversion is trivial but yields no concat): `bb_pat_alt, bb_pat_any, bb_pat_break, bb_pat_cat, bb_pat_notany, bb_pat_span` + (corrected) `bb_pat_pos/rpos, bb_arbno, bb_capture, bb_lit`. NOTE classification is of the x86 arm only; JVM/NET arms may have inline text but are dominated by imperative jvm_*/net_* helpers (x86-only directive → those are stubs anyway). **→ ALL INLINE-TEXT files now converted (arb/len/tab + abort/fence/rem). Remaining work is the DELEGATING set — low-priority mechanical .c→.cpp, all arms imperative, no concat value.**

**PER-FILE RECIPE (mechanical, proven on 3 files):** (i) move `#include <string>` + `emit_str.h` ABOVE `extern "C"`; (ii) inside extern "C": `bb_template_common.h` + `emit.h` + any `extern int g_*`; (iii) `static std::string NAME_str(args)` returns concat for text arms / `std::string()` for imperative arms; `extern "C" void NAME(args){ auto s=NAME_str(...); if(!s.empty()) emit_text_n(s.data(),s.size()); }`; (iv) `emit_Nasm`→`s_Nasm`, `emit_comment/directive`→`s_comment/s_directive`, `emit_textf`→`emit_fmt`; (v) `snprintf`-into-buf → `emit_fmt`. **Byte-safety verified:** `\u03a3`/`\u0394` escapes identical bytes C vs C++; UTF-8 in literals → octal-escape (`\316\261`=α `ce b1`, `\316\262`=β `ce b2`, `\317\211`=ω `cf 89`) OR keep literal UTF-8 (g++13 accepts both). **IMPERATIVE SUBSET (return ""):** emit_flat_ir / emit_label_define_bb / emit_jmp_label / emit_jmp / emit_label_define / insn_mov_* / emit_store_delta / bb_charset_emit; jvm_*/net_* FILE-writers. **Makefile:** edit BOTH the source-list line (~line 82-180) AND the compile line (~258-380) `.c`→`.cpp`, `$(CC) $(CRT)`→`$(CXX) $(CXXRT)`. **LOOP SHAPE ✅** proven (27B identical): `for(...) r += emit_fmt(...)` ≅ Snocone fold; lives in SM_templates (sm_calls/sm_returns/sm_pat_combine) + bb_pat_alt/cat. GATE-PK green per file.
