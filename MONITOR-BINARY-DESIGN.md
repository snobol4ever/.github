...
- **What it does not do yet:** lock-step against an oracle other than SPITBOL (layer 6 is per language and lands only where it earns its cost). SNOBOL4 is on the shared hooks since CEO-1187 (the section below); `--oracle` on a `.sno` holds SCRIP and SPITBOL in lock-step.
