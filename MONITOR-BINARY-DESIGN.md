...
- **`--oracle` today:** `.sno` holds SCRIP and the instrumented SPITBOL fork in lock-step; `.icn` holds SCRIP and the instrumented Arizona iconx (`icx`, since SCRIP `d85e6186d`); Prolog, Pascal and Raku follow as their oracles are instrumented in their own source (§ THE ORACLES ARE INSTRUMENTED IN THEIR OWN SOURCE). SNOBOL4 is on the shared hooks since CEO-1187.
...
- **SCRIP against SPITBOL AGREES on the witness, step 41 of 41,** after two cures the old wire could never have shown: a store to OUTPUT, TERMINAL or any output-associated variable is not a value event (`sno_name_is_output_assoc`), and the table fast path emits `<lval>`.
