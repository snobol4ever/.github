...
**SWI-Prolog, ROWED 2026-09-24 12:4x CDT on Lon's word (in-chat to the ceo, verbatim: *"To complete the IPC sync-step monitor for Prolog we must instrument SWIPL in the same manner as Gnu Prolog."*; CEO-1236; row `prolog-monitor-swi-prolog-is-instrumented-in-its-own-source-like-gnu-prolog-a-swipl-mon-fork-with-the-ipc-library-participant-swx-in-lock-step-with-scrip`, the cfo's, handed to hq_prolog after three witnesses agree):** `swipl-mon.patch` over `/home/resources/swipl-devel-master` (the drop; `/usr/bin/swipl` is 9.0.4), fire-points at the VM's port sites in `pl-wam.c` (call, exit, redo, fail — the ports `pl-trace.c`'s tracer sees, fired unconditionally from the source, no trace mode, no leash) with the goal's clause line, participant `swx` (`monitor_swx.c`), `build_swipl_mon.sh` with default prefix `/home/resources/swipl-mon` (Lon installs; `PROLOG_MON_ROOT` meanwhile), `monitor_run.sh` selecting the Prolog oracle participant by `MONITOR_PL_ORACLE=gprolog` (default) or `swipl`. The Prolog reading of the contract above binds both participants, so a `.pl` witness can be held in lock-step against either oracle and the two oracles against each other.
