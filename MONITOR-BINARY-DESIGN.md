...
- **BIND** (a VALUE, not a new kind) — right after every CALL, one VALUE per parameter of that routine, in declaration order, named by the parameter and carrying the value BOUND to it (typed as a store is; a parameter the oracle cannot type is UNKNOWN, and the controller grades it UNGRADED, never AGREE). ⛔ WHY (the coo 2026-09-23, row monitor-a-value-event-fires-per-parameter-at-call-argument-bind-time-...; hq_pascal's tbs_tb0207 at SCRIP a3dd15832): an argument-evaluation-order bug READ AGREE event-for-event, because both engines call the argument-producing function the same number of times with the same results in the same order and only the BINDING -- which result lands in which parameter slot -- differs, and binding fired nothing. The CALL record itself stays name-only on the wire (the text trace keeps its `name(args)` line). ⭐ Pascal is the first plug and `fpx` the first twin (`mon_ipc_param_nodes` beside `mon_ipc_call_node`, visible parameters only, a formal untyped one UNKNOWN); witness `sync_step_pascal_5.pas` (`show(next, next)`: right-to-left binds a=20 b=10). ⛔ THE TWO SIDES LAND IN ORDER, NEVER APART: a plug that fires BIND against an installed oracle that does not diverges at its first parameterized call, so the oracle fork is rebuilt and installed first and the plug follows -- STEP 1 LANDED at SCRIP `73e02c870` (the fpx source and the witness), STEP 2 (the Pascal plug, `pas_trace_wrap_proc` in `pascal.y`, trace-gated) waits on Lon's reinstall of `/home/resources/fpc-mon`. Measured with both sides built privately: the five Pascal witnesses AGREE with UNGRADED=0, and with `a3dd15832` reverted the old pair read AGREE at 19 over the bug while the new pair DIVERGES at step 17 on `VALUE a` (20 against 10). Icon, Prolog and Raku follow by their HQs on this contract, each with its oracle's twin in the same landing sequence.
...
- **OUTPUT** (the coo 2026-09-25, row `instruments-the-monitor-grades-no-output-event-so-a-wrong-value-written-without-an-assignment-reads-agree`, ceo CEO-1272 on hq_icon's measurement against iconx) — the bytes each write to STANDARD OUTPUT emits, as `MWK_OUTPUT` (kind 11, name_id `MW_NAME_ID_NONE`, type STRING, the bytes as the value), fired where the language's output routine hands them to stdout; before its FIRST statement a participant that speaks OUTPUT sends one hello, an `MWK_OUTPUT` with name_id `MW_OUTPUT_HELLO` (`0xfffffffe`) and no bytes. ⛔ WHY: every other kind grades a value only when it is STORED, so a wrong value written straight to output -- `write(f(x))`, `OUTPUT = F(X)`, `writeln(g)` -- was no event at all, and both sides agreed event for event over a wrong answer (`write(&version)` reads AGREE on the icx/scr pair while iconx prints `Icon Version 9.5.25a` and SCRIP `Jcon Version 2.2`). ⭐ OUTPUT IS A STREAM, NOT A PAIRED EVENT: two engines need not cut one `write` into the same pieces (icx sends `write("hello ", 3)` as `hello `, `3`, `\n`), so the controller absorbs OUTPUT records into one byte stream per participant and, at every compared step and at the end, requires the streams to agree up to the shorter one and wholly at END -- the first differing byte is a DIVERGE, `(OUTPUT)`, located after the last event both agreed on, with both streams named. Output is graded only when EVERY participant said hello; otherwise the VERDICT line says `OUTPUT=UNGRADED(<names>)` and `monitor_run.sh` says OUTPUT NOT GRADED beside its verdict, so no AGREE ever claims bytes nobody compared. THE TWO SIDES LAND IN ORDER: the oracle fork first, then the SCRIP plug. **Icon's fork, landed** (SCRIP, `icon-mon.patch`: `putstr` in `rsys.r` and the two newline `putc` sites of `fsys.r` call `icx_mon_output`, which fires only for `stdout`; `monitor_icx.c` sends the hello at the first statement; `monitor_ipc_lib.{c,h}` gain `mon_ipc_speak_output` and `mon_ipc_output`, so every fork built from the library has them). **SCRIP's plug is owed** (src/, not the coo's): SCRIP's Icon `write`/`writes` must send the hello before the first LABEL and every byte it puts on stdout through `mon_send_bin(MWK_OUTPUT, MW_NAME_ID_NONE, MWT_STRING, bytes, len)`; then `test_gate_monitor_grades_the_output_event.sh` arm 7 turns from rc 2 to green (`write(&version)` DIVERGEs at its OUTPUT, `write("same")` AGREEs with output graded) and loses its REPORTED dash in `make test`. Each other language follows on its own fork and plug as crawl work: the SPITBOL fork's own `monitor_ipc_runtime.c`, gpx, swx, fpx and rkx at their output routines.
- **THE PROLOG READING OF THE CONTRACT (ceo, 2026-09-23, from the GNU Prolog lock-step):** a body goal is a statement; `,` `;` `|` `->` `*->` are not statements and are walked at any nesting; every other goal is ONE statement with its own first-token line — `\+ G`, `call(G)`, `findall/3`, `catch/3` and every other goal with a goal ARGUMENT fire one statement and NOTHING inside it from the plug (the argument is data on the oracle side; the callee's own clauses still report), a variable goal is a statement, directive goals are silent; CALL fires ONCE at the predicate's entry (a retry into another clause on backtracking is no event); RETURN fires at EVERY clause success with the empty STRING as its value (a tail call is not sealed under the trace so the caller's clause reports too); builtins AND the compiler's own library predicates are never events (SCRIP's plug breaches this today: `member/2`, `append/3`, `length/2`, `$length_/3` report with the prelude's lines — hq_prolog's row); VALUE is no event for Prolog today (extend both plugs together). ⭐ **DYNAMIC AND MULTIFILE PREDICATES ARE NEVER EVENTS, ON EVERY SIDE (ceo CEO-1269, 2026-09-25, on the cfo's ask at the swx landing):** a predicate the program declares `dynamic` or `multifile`, or creates by `assert`, fires no CALL, no STMT inside its clauses and no RETURN — its clauses are the program's database, data it asserts and retracts, not code with a source line. Both participants already hold it by construction (gpx injects only into native-compiled user predicates, and gprolog runs dynamic clauses through its byte-code emulator; swx's loader marks no dynamic, multifile or system predicate), so SCRIP's plug is the one that breaches it: `sync_step_prolog_4` diverges at step 35 on both oracles because scr fires CALL for the dynamic `counter/1` — hq_prolog's plug row, beside the library-predicate breach. The caller's goal still fires its own STMT on all three sides, so a wrong answer from the database still surfaces at the next event: the rule costs the bracket nothing.
...
**SWI-Prolog, LANDED 2026-09-25 15:2x CDT at SCRIP `14cbc23c1` (ROWED 2026-09-24 12:4x on Lon's word, in-chat to the ceo, verbatim: *"To complete the IPC sync-step monitor for Prolog we must instrument SWIPL in the same manner as Gnu Prolog."*; CEO-1236; row `prolog-monitor-swi-prolog-is-instrumented-in-its-own-source-like-gnu-prolog-a-swipl-mon-fork-with-the-ipc-library-participant-swx-in-lock-step-with-scrip`, the cfo's, DONE by computed receipt):** `swipl-mon.patch` over `/home/resources/swipl-devel-master` (the 10.1.5 drop; `/usr/bin/swipl` is 9.0.4), six files, plus participant `swx` (`monitor_swx.c`) on the shared `monitor_ipc_lib.c` wire. CALL fires in the VM at the entry of every predicate the loader marked (`P_MON_CALL`, `FLAG64(40)`, in `pl-vmi.c` `depart_or_retry_continue`: every call, last-call depart and query entry passes it, a redo into another clause does not). STMT and RETURN are COMPILED IN, as the GNU Prolog fork's `pl2wam` does, because the VM keeps no per-goal line and `=/2`, `!`, `==/2` and `\+` compile inline with no port: `boot/init.pl`'s `'$store_clause'` stores every clause of a program file with `'$mon_stmt'(Line)` before each body goal and `'$mon_ret'(Name, Arity)` at its end, Line being the goal's first-token line from the reader's subterm positions (a character offset, looked up in the file's newline table by `'$mon_line'`, in Prolog, once per file); library and boot files, dynamic, multifile and system predicates and directives are never events, and the loader injects only while the READY pipe is named (or `SWIPL_MON=1`), so an untraced run is the pristine oracle's (the build script's control arm). ⛔ THE TRAP THE LOOKUP HIT: `freeStream()` in `pl-file.c` resets the loader's `source_file_name`/`source_line_no` when ANY closed stream names the file being loaded, so a second stream on the program file must restore `source_location/2` with `'$set_source_location'/2` after its close or `record_clause` asserts on line -1. `build_swipl_mon.sh <prefix>` builds pristine and fork and grades both (default prefix `/home/resources/swipl-mon`, Lon installs; `SWIPL_MON_ROOT` names a private one), and `monitor_run.sh <prog>.pl --oracle` runs `gpx scr` by default and `swx scr` under `MONITOR_PL_ORACLE=swipl`. MEASURED at landing: the row's three witnesses AGREE at steps 5, 24, 27; `sync_step_prolog`, `_2`, `_3` AGREE 9, 57, 16 exactly as gpx; `_4` DIVERGES at 35 on BOTH oracles (scr fires CALL for the dynamic `counter/1`, then prelude line L216 -- hq_prolog's). The Prolog reading of the contract above binds both participants, so a `.pl` witness can be held in lock-step against either oracle and the two oracles against each other.
