...
- [ ] **PAS-DISPLAY-0 — CHARACTERIZE (no code change).** Confirm the three regs are idle (grep proof above, recorded). Quantify the walk: dump M3+M4 `.s` for a 3-level nest probe (`main`→`outer`→`inner`, `inner` reads an `outer` local) and show the `FOR op_dval` dependent-load chain. Re-measure pcom + `nest*` nesting depth and FREEZE the level-mapping decision (rbx=0/r13=1/r14=2/r15=3/r12=current) and the depth-≥4 fallback choice here. Confirm `IR_VAR_FRAME`/`IR_ASSIGN_FRAME`/`*_REF` already carry the absolute level (today as `op_dval`=hops; decide whether codegen needs *absolute level* or can derive the display register from `current_level − hops`). Gate: documentation; mapping frozen.
