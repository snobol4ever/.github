...
## ▶ CURRENT STATE (Session 62, 2026-06-15)

**Landed this session (SCRIP `src/lower/lower_pascal.c` ONLY — zero emitter/template/x86/binary/runtime changes): the `marshal_varparam_addr: var-param arg is not a variable` BOMB (the prior handoff's named NEXT FRONTIER) is FIXED by a pure AST/lowering copy-in/copy-out rewrite for aggregate-element var-params.** ROOT CAUSE: a `var` (by-ref) parameter whose call-site argument is an aggregate ELEMENT — `genlabel(uprcptr^.pfname)` lowers to `arr_get(__pas_deref(uprcptr), idx)` (a `TT_IDX`, op IR_CALL=9) — has no stable addressable 16-byte cell, so the byref marshaller (which only handles `IR_VAR_FRAME`/`IR_VAR_FRAME_REF`/named `IR_VAR`) fell to `x86_bomb`. FIX (exactly the handoff's recommended design): in `lower_call`, look up the callee's `byref_mask` from `g_stage2.proc_table` (populated by `polyglot.c` before lowering); for any byref arg that is a `TT_IDX`, rewrite `genlabel(agg[i])` → `tmp := agg[i]; genlabel(tmp); agg[i] := tmp` by synthesizing a unique `__pas_vptmp_N` temp, building a `TT_SEQ_EXPR` of [copy-in assigns…, rewritten call, copy-out assigns…], and lowering THAT. The temp lowers to a named `IR_VAR` (marshaller-handled); the copy-out reuses the EXISTING write-back paths (`__pas_field_set` for `p^.fld`, `arr_set_pure` for `a[i]`). Plain-var byref args (`TT_VAR`) are NOT rewritten — the 99-arg working class is untouched. Three tiny local AST helpers added (`pas_lc_leaf`/`pas_lc_bin`/`pas_lc_clone`) to avoid depending on pascal.y internals. VALIDATED: Pascal **M3 125/0, M4 125/0** (was 123/0 ×2; +2 new probes, zero regressions); new probes `vpfld` (deref record-field var-param → `42`/`99`) and `vparr` (array-element var-param → `7`/`42`/`9`) pass BOTH M3 and M4; SNOBOL4 M4 crosscheck **166/87/8 byte-identical** to baseline (same fail list); SNOBOL4 DEFINE `DOUBLE(21)=42`; Icon/Prolog `hello` clean; template-medium-invisible `--strict` 0 violations; plain-var byref probes (varparam/swap/varframe/varmix/vartrans/recparam{,2,3}) all still pass. Diff **+38 −0, one file**. Full writeup: **HANDOFF-2026-06-15-PASCAL-BB-VARPARAM-AGGREGATE-COPYINOUT.md**.

**PCOM ADVANCE (the payoff):** `scrip --run pcom.pas < tiny.pas` (tiny=`program t(output); begin end.`) no longer bombs — pcom now **runs its real logic and emits the source listing** (`1   9 program t(output);` / `2   9 begin end.`, rc=0). pcom M4 **emits 234K asm lines, assembles cleanly, links, and runs** (the Session-60 `.Lcallfn` duplicate-label blocker #3 is no longer reproducing on the current tree).

**NEXT FRONTIER — pcom M3/M4 first-output divergence.** On tiny, **M3 emits the 2-line source listing but M4 emits NOTHING** (both rc=0). So pcom's first `write` (the source-listing line) reaches stdout in M3 but not M4 — a mode-specific divergence in early I/O, echoing the Session-59 observation. This is the live pcom frontier and is INDEPENDENT of the var-param fix. Suggested next: triangulate why the listing `write` is dropped in M4 (compare the emitted asm for the first `__pas_write`/listing path against a minimal `write`-in-loop probe that passes M4), since M3 demonstrably executes it. Note: there is no mode-2 `--interp` flag anymore (DE-INTERP removed the misnomer); the only modes are `--run` (M3) and `--compile` (M4).

## ▶ PRIOR Session 61 (2026-06-15)

**Landed (SCRIP `src/emitter/emit_bb.c` ONLY): the `bb_gvar_assign call-result: op_a_slot==-1` BOMB fixed by converting the two emitter side tables (`g_bb_slotmap`/`g_bb_varslot`) from fixed-size static arrays to dynamic `realloc`-grown arrays — NO hard caps remain.** ROOT CAUSE: `BB_SLOTMAP_MAX 512`; every `bb_slot_alloc*`/`bb_slot_register` guarded its append `if (g_bb_slotmap_n < BB_SLOTMAP_MAX)`, so past 512 entries the append SILENTLY no-op'd → a large proc's call-result node never got a slot → `bb_slot_get` returned −1 → `x86_bomb`. FIX: typed ptr + `_n` + `_max` grown by `bb_slotmap_push` (doubling idiom from `emit_label_alloc`); all 5 appenders + the inline `flat_drive_gen_scan` push route through it; `g_bb_varslot` likewise dynamic. Per-proc resets set only `_n=0`; buffers persist at `_max`, reused. `BB_SLOTMAP_MAX`/`BB_VARSLOT_MAX` GONE. VALIDATED Pascal M3 123/0 M4 123/0; canaries `charA N=90`/`charbig N=300` → `[S][A][B][E]` BOTH modes; SNOBOL4 M4 166/87/8 byte-identical. Diff +28 −11. Writeup: HANDOFF-2026-06-15-PASCAL-BB-DYNAMIC-SLOTMAP-AND-VARPARAM-AGGREGATE-FRONTIER.md.
