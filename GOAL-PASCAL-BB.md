...
- M3 (--run):    **PASS=122 FAIL=0 XFAIL=1** ✓ PARITY WITH M2
- M4 (--compile): **PASS=122 FAIL=0 SKIP=0 XFAIL=1** ✓ FULL PARITY — stable across repeated runs (proper gate: `scrip --compile` → `gcc -c` → link `out/libscrip_rt.so` (`-no-pie -lscrip_rt -lgc -lm -Wl,-rpath`) → run → cmp .ref)
