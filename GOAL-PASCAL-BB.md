...
- [ ] **PAS-GVA-2 — M3 ARENA SETUP (`--run`).** **Confirmed 2026-06-27:** Pascal M3 currently works (gate 127/0) but goes BY-NAME — `SCRIP_M3_GVA_TRACE=1 ./scrip --run g.pas` prints NO trace line (`g_gva_active=0`), whereas the same probe in Icon prints `[M3-GVA] m3 globals via rbx-arena: active=1`. So this step is for the OPTIMIZATION (hash→direct-memory, "ten times faster") and for M3/M4 storage parity, NOT for correctness. Teach the generic M3 path (`scrip.c:~3354`, the `[SBB]` run branch Pascal falls into) to set up the in-process arena, mirroring Icon's ICN-GVA-M3 block (`scrip.c:3210-3222`): `gva_collect_reset()`, collect the Pascal globals, allocate the `__gva` RX/BSS slab, `gva_register(names, arena, n)`, set `g_gva_active=1`, load `rbx`. Reference: `GOAL-ICN-GVA-M3.md` (M3-ARENA-1/2/3/4). **Caution:** `scrip.c` is shared — running ALL FOUR gates before commit is mandatory. Gate: M3 full gate stays 127/0; `SCRIP_M3_GVA_TRACE` now shows Pascal globals via rbx-arena.
