...
**GATE STATUS:**
- M2 (--interp): **PASS=121 FAIL=0 XFAIL=1** ✓ STABLE — must not regress
- M3 (--run):    **PASS=121 FAIL=0 XFAIL=1** ✓ PARITY WITH M2
- M4 (--compile): **PASS=121 FAIL=0 SKIP=0 XFAIL=1** ✓ FULL PARITY — stable across repeated runs (proper gate: `scrip --compile` → `gcc -c` → link `out/libscrip_rt.so` (`-no-pie -lscrip_rt -lgc -lm -Wl,-rpath`) → run → cmp .ref)
...
1. **Empty-procedure-call runtime bug — NOW THE LIVE pcom BLOCKER.** Calling a `begin end` proc breaks M3 chain continuation (pcom's `mark`/`release` ~line 300 are empty). With the nested-aggregate store closed (Session 58), pcom no longer aborts at emit time — fed a const-decl program it emits the source listing then stalls here. This is M3/M4-specific chain continuation: an empty proc body must thread γ→success without dropping the caller's statement sequence. Deserves a fresh context budget. Repro: a Pascal program calling a `procedure p; begin end;` and continuing afterward in M3.
...
3. **Deeper pcom porting past the empty-proc stall** — once #1 clears, re-drive pcom and find the next blocker. The nested-store sites (`gattr.cval.ival`, `gattr.cval.valp`, `values.ival`) now store correctly (verified by probes nestpv2/nestwith1).
...
bash /tmp/run_gate.sh    # must be 121/0
bash /tmp/run_gate_m3.sh # must be 121/0
bash /tmp/run_gate_m4.sh # must be 121/0 (assemble+link+run; -no-pie). NEVER trust an M4 watermark not produced by this recipe.
```
