...
## ▶ CURRENT STATE (Session 60, 2026-06-14)
...
**GATE STATUS:**
- M2 (--interp): **PASS=122 FAIL=0 XFAIL=1** ✓ STABLE — must not regress
- M3 (--run):    **PASS=119 FAIL=3 XFAIL=1** ⚠ REGRESSED by a CONCURRENT non-Pascal push (see flag below) — was 122/0
- M4 (--compile): **PASS=119 FAIL=0 SKIP=3 XFAIL=1** ⚠ REGRESSED (3 now SKIP) by the same push — was 122/0/SKIP=0 (proper gate: `scrip --compile` → `gcc -c` → link `out/libscrip_rt.so` (`-no-pie -lscrip_rt -lgc -lm -Wl,-rpath`) → run → cmp .ref)

**⚠ TOP-PRIORITY REGRESSION (NOT the frame fix — it is parity-preserving): commit `2fa5086` "IR_CALL split rung 5 (GVAR_USERPROC producer)" dropped the Pascal gate M3 122→119 (FAILING: `arr2dtype.pas boolfn.pas case2.pas`) and M4 to 119+3 SKIP.** Verified by gating `2fa5086` in isolation (without the frame commit) — identical 119/3, same three tests. That commit touched shared `bb_call.cpp` gvar build sites + `resolve_call_kinds_gvar` and, per its own message, exercised only "descr-only / 4 DEFINE probes" — i.e. it did NOT run the Pascal gate before pushing (the exact failure mode RULES warn about for shared `bb_call.cpp`/`emit_bb.c`/`scrip.c`). FIX THIS FIRST: bisect `2fa5086`'s `IR_CALL_GVAR_USERPROC` stamping/consumer-widening against those 3 probes (2D-array-type, boolean-fn, case) before any further pcom work. The frame commit `9149cb1` sits cleanly on top (its own base was 122/0 ×3; it adds zero gate delta).
...
1. **pcom's deeper M3 listing-output divergence — NOW THE LIVE pcom FRONTIER.** With the empty-proc call fixed (Session 59), M2 emits pcom's full source listing on a tiny `begin end` program but M3 emits NOTHING (rc=0, no abort; M4 compiles 234K-line asm clean). pcom's first output is the source-listing `write` in `endofline`/the main read loop; that `write` isn't reaching stdout in M3. Likely a chain-continuation or I/O-in-deeply-nested-context issue specific to pcom's size/shape (the 122 gated probes all pass M3, so it's a pcom-scale construct, not a primitive). pcom is NOT gated; deserves a fresh context budget. Repro: `scrip --run pcom.pas < tiny.pas` (tiny = `program t(output); begin end.`) vs `--interp`.
