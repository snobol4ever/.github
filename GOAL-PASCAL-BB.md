...
**CORPUS FOLLOW-UP:** 4 new probes (record-with-array-field, getbounds/forward-ptr/variant/with shape, anonymous + nested strarr rows) verified green but NOT imported — fpc raw output is NOT the integer-width oracle (fpc prints `5`, P4 prints width-10 `         5`); refs need the pcom→pint pipeline per provenance rules. Import with proper refs next session.
