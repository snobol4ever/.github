...
## ⛔⭐ ADDENDUM (same pass, caught before push settled) — THE RUNG15 "1/5 → 2/5" CLAIM ABOVE IS TRUE BUT INCOMPLETE: THE FIX MAKES TWO WITNESSES FLAKY, NOT CLEAN

**Retracting the clean framing, not the fix.** The rung15/control-arm measurements in the body above were each a single sample. Re-running `test_prolog_rung15.sh` five more times surfaced real run-to-run variance under the fix that a single sample hid — caught only because a stale-corpus re-verification (below) prompted a second look, not because it was checked for on purpose the first time. That is itself worth naming: **a control arm run once looks exactly like a control arm run enough times**, and this row already carries two FACT RULES about exactly that class of mistake.

**Controlled A/B, 5 runs each side, same binary each side (no rebuild between runs), file swapped via `git show <parent>:<path>` + `cp` rather than `git checkout` (a bare detached-commit checkout was refused by the harness's own permission classifier as a risky git operation — reasonable, and this route avoids it entirely without needing an exception):**

| witness | PRE-FIX (5/5 runs) | POST-FIX (5 runs) |
|---|---|---|
| `abolish_existing` | FAIL, FAIL, FAIL, FAIL, FAIL | PASS, PASS, PASS, **FAIL**, PASS |
| `abolish_nonexistent` | PASS ×5 | PASS ×5 |
| `abolish_one_of_two` | FAIL ×5 | FAIL ×5 |
| `abolish_then_query_fail` | FAIL ×5 | FAIL, FAIL, **PASS**, FAIL, FAIL |
| `abolish_then_reassert` | FAIL ×5 | FAIL ×5 |
| **PASS count** | **1,1,1,1,1** (zero variance) | **2,2,3,1,2** |

**Pre-fix is perfectly deterministic — zero variance across 5 runs.** Post-fix genuinely varies. This is not noise in the harness; it is the fix changing something real. The coherent reading: the omega-wire bug this pass fixed was a **stable, deterministic** wrong-jump (uninitialized `rdx` reliably held whatever the last intervening call left there, and on this witness that was reliably wrong) that crashed the same way every time — it happened to mask a **second, pre-existing, genuinely non-deterministic** defect further downstream (a different uninitialized read, ASLR/heap-layout-sensitive, in the `abolish`/backtrack continuation that only `abolish_existing` and `abolish_then_query_fail` happen to exercise) that was never reachable before because execution never got that far. Fixing the first defect exposed the second rather than closing it. **Not chased further this pass** — same discipline as the open thread already logged above, and consistent with the general shape this whole row keeps finding: fixing one fixed-slot/register assumption relocates the next one rather than closing the class.

**What still stands, unaffected by this correction:** the omega-wire mechanism itself (gdb-confirmed, SIGSEGV→resolved), the SNOBOL4/Icon inertness (both are exact-match control arms, not flaky — re-verify below), and `abolish_nonexistent`'s move from implicit to a clean, stable PASS both sides is real (it was already passing pre-fix in every sample taken, so no claim changes there either — the correction is scoped to `abolish_existing`/`abolish_then_query_fail` only). **What changes: do not read "rung15 1/5→2/5" as "one more test now cleanly passes."** Read it as "the fix is real, and its effect on two of five witnesses is to make them intermittently correct rather than reliably wrong" — worse than a clean fix, better than leaving the crash in place, and worth exactly this much confidence, no more.

## ADDENDUM 2 — the SIGILL witness narrowed further: it is downstream of a real GC collection, not another emitter wire mismatch

Single-instruction gdb trace from `fact$2F1_ω`'s landing forward (breakpoints at exact return addresses rather than blind `stepi`, since the first attempt burned its whole step budget inside glibc's one-time lazy PLT symbol resolution for `rt_proc_call_epilogue_ω` — expected, not a finding). Confirmed, in order:

1. `rt_proc_call_epilogue_ω()` returns `rax=0x68`(`DT_FAIL`)/`rdx=0` — correct, exactly `FAILDESCR`.
2. The NRETURN-consult at `.Lx63_2` correctly no-ops (`rt_g_ret_by_name=0`) and correctly branches to `n48_call_builtin_prolog_α` on `al==DT_FAIL` — `$disj0`'s own dispatch is behaving exactly as it should here.
3. `n48` calls `rt_pl_dop_unwind_nothrow` → `dop_call_nothrow` (`by_name_dispatch.c:1479`) → `rt_gc_point_arr` (asm veneer, `rt_asm_helpers.S`) → `rt_gc_point_arr_c` (`gc_heap.c:319`), which — per its own comment — is "called only through the `rt_gc_point_arr` asm veneer, which **parks all six callee-saved registers above `floor` so the seam scan repairs them and the pops restore the repaired values**," and which, when `g_gc_pending` or the heap line is crossed, calls a real **`gc_collect_ex(0)`** — a moving/compacting collection.

**This is a materially different bug class than the one this FINDING fixes, if it is a bug in this mechanism at all — not yet confirmed, only reached.** The wire-cross bug was an emitter-side register never loaded. This crash is now downstream of a call into GC machinery that deliberately walks and rewrites stack-parked registers looking for moved-object pointers to repair. Prolog graphs assign R15=CP (the live choice-point address) and R12=trail-top, per the register contract (`ARCH-PROLOG-DESCR-ZETAS-hq_C.md` §2) — both are among the "six callee-saved registers" the veneer parks. **A plausible, unconfirmed hypothesis worth the next session's first move:** if the seam scan's repair pass does not know these two registers hold CP/trail addresses rather than GC-heap object pointers, and either address happens to fall in a range the scanner treats as heap-shaped, a live continuation or choice-point address could be silently rewritten to a moved-object address — which is exactly the "a code address holds something else" texture of the crash observed (`rip`/`rax`/`rdx` all stack-address-shaped after this call chain, `rcx` a `libscrip_rt.so`-resident address). **Not verified this pass** — the next step is reading `gc_collect_ex`'s seam-repair logic directly (does it special-case R15/R12 for Prolog graphs, or does it scan them unconditionally?) rather than speculating further from the call trace alone.

**Same-pass housekeeping this addendum caught:** the corpus repo went through two more pull/rebase cycles while this was being written (other seats pushing concurrently, per RULES.md's commit-freely law) — one produced a real conflict on the same Prolog bench `.s` files (another seat's independent regen of the same 22 files), resolved by taking the incoming side and then **re-running `util_regen_prolog_bench_s_artifacts.sh` fresh against this seat's own current `scrip`** rather than hand-merging two compiler outputs (RULES.md: "sweep the compiler, never the artifacts"). Also surfaced: `test_corpus_snobol4.sh`'s denominator moved 365→267 mid-session from an unrelated, concurrent `crosscheck` consolidation commit (`0e75bfdbc`) — `FAIL=0`/`GATE OK` held throughout; per this file's own FACT RULE citations, a probe asserts `FAIL=0`/`SKIP=0`/its named witness and never a denominator, so this is not treated as a control-arm break, just recorded so the number isn't mistaken for a regression later.
