...
- **BENCHMARK = TEST (Lon, same ruling)**: *"the source files [are] an app you can run albeit RINKY that has a REF file which can be verified. So it is a benchmark and a test."* Every kernel is a runnable app with a `.ref`, and the `.ref` diff is verified on EVERY measured run (variable `iters:`/`ms:` lines stripped, per the existing timed-runner convention). A kernel without a verified `.ref` is ungraded and its number unpublishable.
  - ⛔⭐ **AND THE LOOP ITSELF IS PART OF THE TEST (hq_P 2026-09-02, measured on SCRIP `f4532dea`):** a loop-wrapped kernel's stdout must equal **N copies** of its `.ref`/`.expected` before its time is divided by N. The Prolog `between/3`+`fail` wrapper ran its body **exactly twice for every N** on m3 and m4 and exited 0, so both angle harnesses — gated on `exit=` alone — printed SCRIP at ~6,400x gprolog and AGREED with each other to 1%. `SCRIP/scripts/lib_prolog_bench.sh:loop_check` is the one implementation (angles 1 and 2 source it; a cell that did not loop N times reads `LOOP-OUTPUT-MISMATCH(lines=seen/wanted)`, never a rate); the SNOBOL4 angles verify their `.ref` per the line above and should adopt the same N-fold check when their loop is a program-level wrapper. FINDING: `FINDING-2026-09-02-hq_P-vanroy-loop-runs-its-body-exactly-twice-for-every-n-both-angles-divided-n-by-it-and-the-rivals-never-had-their-preludes.md`.
