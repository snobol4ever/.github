...
SNO_LIB=$BEAUTY /home/claude/x64/bin/sbl -b $BEAUTY/beauty_DRIVER_driver.sno 2>/dev/null \
    > /tmp/spitbol.out
SNO_LIB=$BEAUTY timeout 10 ./scrip --ir-run $BEAUTY/beauty_DRIVER_driver.sno 2>/dev/null \
    > /tmp/scrip.out
...
./scrip --ir-run corpus/.../subexpr/TEST.sno
...
- [ ] **S-5** — Apply dance to beauty self-host:
  ```bash
  SNO_LIB=$INC ./scrip --ir-run beauty_driver.sno
  ```
