...
- **It closes T1 and T2 together — but ONLY T1 WAS BROKEN, and the distinction is measured, not assumed.** Both resolvers gate on this one flag, so the line excludes both. **Corrected after the fact by revert-probe (same session):** a T2 witness (`&Word = SPAN(…)`, then `EVAL('DATATYPE(&Word)')` and `P = EVAL('&Word')` matched against a subject) reads `PATTERN` / `zebra` **identically with the fix and without it**. T2 was never observably broken here. The reason is REACHABILITY: T2 folds at `*&Name` defer sites in pattern position, and `EVAL('*&Word')` raises **Error 22 "Undefined function called"** — the deferred-expression construct is not in the fragment-compilable subset. A bare `EVAL('&Word')` is not a T2 site at all and is not a T1 site either (T1 admits only ILIT/FLIT/QLIT via `sno_const_scalar_tree`), so it goes by-name to the sealed cell and is correct on both arms. **The T2 half of this line is therefore PROPHYLACTIC and becomes load-bearing the moment the fragment subset widens to admit `*&Name`** — which is exactly what a fragment-subset rung would do without ever thinking about constants.
