...
**⌚ 2026-07-21 (Claude Opus 4.8 · SCRIP HEAD dc7a681d + ICN-BINWRITE-FIX uncommitted · corpus unchanged) — THE REAL SELF-HOST BLOCKER FOUND & FIXED: SCRIP could not write binary bytes 0x00/0x01/0x03, so EVERY JCON .class was corrupt ("bc_File ✓" was a false positive). Fixed byte-exact, zero regression. Unmasks the next jtran bug (unresolved ir_Label in the bytecode writer).**
Built the FULL run path this session (was never built before): jcon.zip Java runtime (333KB/382 classes, iTrampoline.java generated via SCRIP), SCRIP-jlink (linker.icn+bytecode.icn, 82K asm, 0 bombs), and the jcont-equivalent bundle+run sequence. Running SCRIP-jtran's hello_t.class on the JVM exposed `UnsupportedClassVersionError: version 28261.28271`. **ROOT CAUSE:** `out_write_str` (by_name_dispatch.c:3738) uses control bytes as an IN-BAND structured-value encoding (`\x03`=struct marker→any/all/one/none, `\x01`=separator, `\x04`=close); `write`/`writes` routed ALL string data through it, and the dispatcher's `char()` used STRVAL (slen=0, drops NUL). JVM .class files are saturated with 0x00/0x01/0x03 → total corruption, masked because nothing validated bytes until JVM load. **FIX (2 edits, by_name_dispatch.c):** (1) write/writes to a real file emits `slen` bytes verbatim via fwrite, bypassing the structure decoder (stdout/stderr keeps pattern display); (2) `char()` → BSTRVAL(buf,1) so char(0) is a real NUL byte. `DESCR_t.slen` (descr.h:30) made this tractable. **VERIFIED:** bintest.bin = `ca fe ba be 00 03 00 2d` exact; Icon smoke m3 14/14 + m4 14/14 zero regression. **NEXT BLOCKER (pre-existing, now unmasked):** bc_File → `Run-time error 500 / offending value: record(ir_Label)` — an unresolved ir_Label reaches a j_writer_u* integer writer (bytecode.icn ~1649-1664). Pointer: bytecode.icn:1548-1550 resolves j_label→int via `locations` table; a branch target never converted ir_Label→j_label→int is leaking. Diagnose in gen_bc.icn branch emission + bytecode.icn offset resolution (gdb/monitor). **SHELL SCRIPTS (per Lon directive — use these, do NOT re-derive switches):** `scripts/jcon_selfhost_build.sh` (bootstraps scrip -O0, libscrip_rt -O0 feature / PERF=1 for -O2, 17-module jtran, jlink, jcon.zip — generates do_ops/interface/iTrampoline via SCRIP+semicolonize) and `scripts/jcon_selfhost_run.sh <prog.icn>` (translate→jlink→bundle→java, canonical l$<base> naming). Full writeup: `FINDING-2026-07-21-CLAUDE-JCON-SELFHOST-BINARY-STRING-WRITE-FIX.md`. HANDOFF TODO: commit ICN-BINWRITE-FIX + 2 scripts; push (git user LCherryholmes/lcherryh@yahoo.com; code repos first, .github last via handoff_status.sh).
