...
## ⌚ 2026-07-19 (Claude Sonnet 4.6 · SCRIP `7e9e414f` · corpus `547d67d6`) — coexpr pipeline fixed; jtran runs; 4-way bench measured; LIVE CURSOR below
**LIVE CURSOR — NEXT SESSION START HERE.**
Four fixes landed (`769f3439` + `7e9e414f` on origin/main). Board: Icon 252/9/32 (32 flips vs pristine merged-HEAD's 220/41; zero regressions). Smokes icon 14/14×2, SN4 7/7, Prolog 5/5. Gates no_stack/one_reg/semicolon PASS. jtran 15-module merge: 0 bombs, links clean (456K lines), `preproc hello_t.icn : stdout` correct m4. 4-way bench queens(10)×3: iconx 233ms / JCON 804ms / SCRIP-m3 1849ms / SCRIP-m4 1832ms — gap = rt_num_arith + NV-dict on every set op (REGAIN territory). Arizona icont+iconx BUILT this sandbox (`/home/claude/icon-master`). JCON BUILT (`/home/claude/jcon-master`, needs `bash jcont`, run from source dir). **THREE OPEN ITEMS:** (A) m3 116M-line spin on full 15-module pipeline — retest with snapshot fix (likely cured, unconfirmed); (B) `bc_File` JVM-class stage hits Error 5 (loadfunc not in SCRIP runtime — WONTFIX candidate under single-unit model); (C) icode path `u_gen_File` ran but produced empty files — investigate arg plumbing (`-out:` from correct CWD). **NEXT: retest m3 full pipeline spin → if clear, self-host byte-compare vs iconx-jtran icode output → update bench with jtran-translation task timings.**
