...
**LIVE CURSOR (Claude Sonnet, 2026-08-12):** M34-1 (a/b/c) verified ALREADY COMPLETE this session — grep-confirmed zero live-code `mode-1`/`mode-2`/`mode_interp` outside attic, PLAN.md/REPO-SCRIP.md/ARCH-SCRIP.md/SCRIP/README.md all clean, `src/attic` does not exist (already physically deleted). Checklist was stale; deleted below per RULES.md handoff step 1.

**D1 IS LIKELY ALREADY FIXED, BY A DIFFERENT MECHANISM THAN THIS FILE DESCRIBES.** `src/driver/scrip.c` mode_run block (line ~1524, `if (is_icon || is_raku || is_sno_bb || is_prolog)`) sets up GVA via `rt_gva_island()` (a fixed pinned VA, `RT_GVA_VA`, mmap'd `MAP_FIXED_NOREPLACE` at `rt_pin_init`) + `gva_register()` + `g_gva_active=1` — for SNOBOL4-BB, Icon, Raku, AND Prolog together in ONE shared block, not the GC_MALLOC'd-arena-threaded-through-`rbx`-via-`m3_enter_with_rbx` mechanism this file's D1/M34-2a text describes. **`ARCH-ICON.md` (2026-07-18, "register truth for ALL BB codegen") confirms: `rbx` is no longer the GVA base — it's the WS/GC bump-frontier top; GVA globals now address via `ABSQ(RT_GVA_VA + k*16)`, no register base.** This file's Prereq-reads line ("`bb_regs.h`: rbx=GVA base") and M34-2a's exact mechanism are STALE. Needs a fresh empirical read (not prose) of whether D1 as originally measured still exists, and if so, what it now looks like under the new addressing scheme.

**FRESH CENSUS (Claude Sonnet, 2026-08-12s, `test_mode34_parity.sh`, crosscheck corpus — corpus has grown 262→318 programs since the 2026-06-25 baseline below, NOT apples-to-apples):**
`TOTAL=318 IDENTICAL=254 DIFFER=16 M3-MISS=46 M4-MISS=1 BOTH-FAIL=1`
- **Harness itself had a bug** (fixed this session, commit `94d283c` in SCRIP): `run_m4()`'s link step was missing `-no-pie`; this container's gcc defaults to PIE, so EVERY mode-4 link failed (`R_X86_64_32S` relocation against `g_rtcc_block`), making the raw census read `M4-MISS=271 BOTH-FAIL=47 IDENTICAL=0` before the fix — a false total-regression signal, not real.
- **DIFFER=16** (was 1 at baseline — `test_string` still present) — dominated by pattern/ARBNO-defer shapes: `066_capture_then_fenced_arbno`, `test_case`, `test_stack`, `test_string`, `064_replace_multi_arm`, `121_pat_calc_op_dispatch`, `141_pat_eval_double_fn_arbno`, `153_pat_operand_edge_matrix`, `154_pat_construction_time_hoist`, `156/157_pat_cap_*`, `165/180/181/182/183_pat_arbno_defer_*`. Per GATE PHILOSOPHY #4, DIFFER is the top priority (must hit 0 first).
- **M3-MISS=46** (was 63) — still dominated by native-pattern shapes (`word1-4`, `cross`, `expr_eval`, `*_pat_fence_*`, `*_pat_arbno_defer_*`, `*_pat_json_*`).
- **M4-MISS=1** (`1017_arg_local`), **BOTH-FAIL=1** (`coverage_sno_nodes`) — both shrunk from baseline.
- Full TSV: `/tmp/m34_census_fixed.tsv` (not yet committed anywhere durable — corpus repo has no obvious `docs/` census-drop convention checked yet).

**Next open step — unclear, needs Lon's call:** (a) re-verify D1 empirically against the NEW GVA mechanism (may already be closed, in which case M34-2 as written is obsolete and should be rewritten or deleted), or (b) triage the 16 DIFFER cases (gate-philosophy priority, but a new/different bug class than D1 — looks like ARBNO-defer/capture-alternation correctness, not a mode-3/4 divergence necessarily), or (c) something else Lon prioritizes. Build verified green (`make scrip` + `make libscrip_rt`, tri-probe smoke test m3==m4 on trivial program) before this cursor was written.
