...
**MODES 1 AND 2 DO NOT EXIST.** Mode 1 (the AST-walk `eval_node` evaluator) and mode 2 (`--interp`, the IR-graph software interpreter) are DELETED. The driver already exposes only `--run`/`--compile` (verified 2026-06-25 — no `--interp` flag remains). What survives is RESIDUE: stale strings/comments in live code, dead `src/attic/` files, and stale prose in authoritative docs. All references on LIVE/authoritative surfaces must be purged. **Scope guard:** archival HANDOFF/SESSION records are history, NOT live descriptions — they are NOT rewritten (rewriting them would falsify the record and violates the "don't touch unrelated goal files" rule). Target only surfaces a reader consults to understand SCRIP *today*.
...
- [ ] **M34-1a — live code (one site).** `src/runtime/runtime_eval.c:139` BOMB string says "(mode-1 era)" — reword to drop the dead-mode reference while keeping the message meaning ("AST-walk evaluator deleted; nothing interprets tree_t at runtime"). Grep-confirm zero live-code `mode-1`/`mode-2`/`--interp`/`IR_interp`/`mode_interp` outside `src/attic/`.
- [ ] **M34-1b — authoritative docs.** Update the mode tables/prose in `PLAN.md` (architecture paragraph "mode-2 INTERP (`interp/IR_interp.c`…)"), `REPO-SCRIP.md` (the `scrip modes` table lists `--interp` mode 2 — remove the row), `ARCH-SCRIP.md`, `SCRIP/README.md`, and the driver help/comments to describe ONLY modes 3 and 4. Do NOT touch HANDOFF-*/SESSION-* archival records.
