...
**LIVE CURSOR (Claude Sonnet, 2026-08-12, session end):** Full session detail in `FINDING-2026-08-12n-CLAUDE-SONNET5-MODE34-HARNESS-NO-PIE-BUG-AND-BYNAME-LABEL-COLLISION-PLUS-D1-LIKELY-ALREADY-SUPERSEDED.md` — read that first. Short version: M34-1 was already done (checklist deleted). Parity harness had a `-no-pie` bug (fixed, SCRIP `94d283c`) that was making the whole census read as a false total-regression. Fresh real census: `TOTAL=318 IDENTICAL=254 DIFFER=16 M3-MISS=46 M4-MISS=1 BOTH-FAIL=1`. D1 (GVA) looks ALREADY FIXED via a newer mechanism (`rt_gva_island` pinned VA) than this file describes — not empirically re-confirmed, needs a timing A/B. Two new defects cheaply isolated (5-line repros in the FINDING doc, not yet fixed): (5a) mode-4 TEXT emission collides `.Lbynamefn[zd]<nid>` labels when a statement calls the same by-name builtin 2+ times — `src/templates/bb_call.cpp:299`/`:323`, likely explains several of the 16 DIFFER; (5b) the `POS(0)(SPAN(ws)|'')REM.r` capture-inside-`DEFINE`d-function idiom is wrong in BOTH modes, differently — needs the full MONITOR/gdb hunt, not done this session. **Next session: start with 5a (highest leverage, cheapest to finish), then MONITOR-FIRST 5b.**
...
### M34-2 — Close D1: SNOBOL4 mode-3 GVA parity — ⛔ **PROBABLY ALREADY CLOSED; VERIFY BEFORE IMPLEMENTING ANYTHING**
⛔ **Do NOT start by writing code against M34-2a below — its mechanism is stale** (see LIVE CURSOR + FINDING-2026-08-12n §4). SNOBOL4 mode-3 already sets up GVA in a shared block with Icon/Raku/Prolog via `rt_gva_island()` (pinned VA `RT_GVA_VA`), and `rbx` is no longer the GVA base at all (`ARCH-ICON.md` 2026-07-18). **First action is a MEASUREMENT, not an edit:**
- [ ] **M34-2-verify — A/B the timing gap.** Run an `arith_loop`-equivalent (globals in a hot loop) in mode-3 vs mode-4, same box, same build, RT_OPT level labeled. D1's whole claim was a ~7-8× mode-3 penalty from by-name hashing. If the gap is gone → D1 is CLOSED: delete M34-2a/b and record the `rt_gva_island` mechanism as the answer. If the gap survives → the defect is real but has MOVED; re-characterize it against the pinned-VA scheme before touching the driver. Either way this is cheap and settles a question that is currently costing every reader of this file a wrong mental model.
- [ ] **M34-2a (STALE MECHANISM — rewrite or delete after the verify step)** — mirror the Prolog/Icon mode-3 GVA setup into the SNOBOL4 `--run` block. In `src/driver/scrip.c` SNOBOL4 mode-3 path: run `gva_collect_reset()` + `gva_collect_graph(sbbg)`; if `n_gva>0`, `GC_MALLOC` the cells arena + names array, `gva_register(...)`, `g_gva_active=1`, and enter via `m3_enter_with_rbx(fn, rt_frame(), 0, gva_arena)` instead of `fn(rt_frame(),0)`. Mirror exactly the Prolog block (~3306–3315) — same arena lifetime, same rbx contract (`bb_regs.h`: `rbx=GVA base`).
