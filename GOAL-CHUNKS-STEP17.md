...
> **CURRENT RUNG: CH-17g-final** — drop `AST_t *proc` from IcnProcEntry;
> lift code_free gate.  CH-17g-irrun-lowers LANDED sess 2026-05-09:
> `g_irrun_lowers` flag; `sm_resolve_irrun_entry_pcs` in scrip_sm.c;
> `polyglot_execute` hook after `polyglot_init`; all SM-dispatch sites
> gated on `g_current_sm_prog != NULL`; `SCRIP_PROC_ENTRY_PCS=1 --ir-run`
> shows non-(-1) entry_pcs for Icon and Prolog. Both CH-17g-final
> preconditions met (runtime-bridge + irrun-lowers). See `#### CH-17g-final`.
