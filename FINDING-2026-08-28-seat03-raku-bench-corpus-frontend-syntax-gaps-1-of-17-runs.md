...
⭐⛔ **THE 51/51 AND THE 1/17 ARE BOTH TRUE AT ONCE, AND THAT COMPATIBILITY IS THE FINDING (hq_C, 2026-08-28, on reading this row's `ask`) — do not let the green number stand in for coverage.** `test_crosscheck_raku.sh`'s 51 cases are a fixed, hand-picked set whose denominator was chosen from constructs someone already made SCRIP handle — a suite built that way is structurally unable to fall below 100% on the constructs it contains, and says nothing about constructs it does not. None of the 9 gaps catalogued below (sigil-less bindings, hyphenated identifiers, colon-pairs, typed/defaulted signatures, parameter traits, hyperoperators, the `$[...]` composer, chained bitwise-into-ternary, the uncharacterized `string-escape` one) appears anywhere in that suite's 51 cases. So **"51/51 raku green" must never be cited as evidence of raku frontend health** — it measures the suite, not the frontend, in exactly the same shape as an Icon board that graded zero files behind a silent `rc=2` and read as passing. The number to cite for frontend health is the one in the table below: 1/17 real-world kernels.
