...
0a. **THE PUSH-PAIR MOVE — THE way to move, almost everywhere (Lon: "the wonderful trick").** At a crossing, push γ and ω on the stack as RETURN LOCATIONS; the exit CHOOSES by a pop/ret combo depending on which way it goes. R10 and R11 stay **ALWAYS LIVE** so `jmp r10` / `jmp r11` always work — the pair rides registers for the jump and the stack for the return-choice; the two spellings are one value.
