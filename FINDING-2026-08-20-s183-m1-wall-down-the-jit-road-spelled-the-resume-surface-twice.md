...
- **Also owed here — seat1's "fenced RT publication", now PRESENT BY CONSTRUCTION BUT UNWITNESSED (HQ second self-correction, stated precisely):** seat1's cursor lists "the **fenced** RT publication this rung deliberately left alone (JIT road still publishes the stale first-allocated node)" as owed. The merge *does* change this — both roads call one function, so the JIT road now has the s182 fence arm **in code**. But **HQ first wrote this up as "CLOSED — verify, don't redo" and that was too strong**: measured at `ffbc1425`, **no witness on disk drives a *fenced* pattern down the JIT road at all**. Both fenced ladder rungs lower *statically* — `ptw_min_cw_fnfence` and `ptw_min_cw_fnrecfence` print only `[RTGRAPH] pat=PAT$0/PAT$1`, never `pat=RT$` (the pre-arg/MKPAT machinery takes `mk() FENCE(...)` down the static road). So the correct status is **present by construction, unproven by measurement**, and it needs a witness of its own: a runtime-composed concat that actually reaches `sno_pat_tree_graph_rt` **with `pfenced=1`**. Confirmed working the same way: the cure itself IS witnessed on the JIT road — `[RTGRAPH] pat=RT$ pfenced=0 brt=1 rn=1 body_root_op=72` on `ptw_min_fncat_arbno`, i.e. `IR_MATCH_LIT` where it published `116 = IR_SUCCEED` before.
