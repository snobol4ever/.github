...
⛔⭐⭐ **THE INSTRUCTION-COUNT ARGUMENT BEHIND THE WHOLE RANKING — THE POLICY STANDS, THE CEILING NUMBER IS RETRACTED, AND IT DIED TWICE (hq_P ruling 2026-08-27, row `perf-tooling-hardware-counters`; evidence seat06 STEP 1 + STEP 2).** This paragraph used to read, from FINDING s251 §2: *"SCRIP already beats SPITBOL on every microarchitectural axis — IPC 3.20 vs 2.33, mispredicts 0.14% vs 0.83%, frontend idle 4.93% vs 22.70%. It is instruction-count bound, so hand-written register-aware asm and box-level polish are capped near 1.4× and are the last mile."* ⛔ **Do not quote 3.20/2.33 or the 1.4× ceiling anywhere — not as a target, not as a bound, not "approximately".**

- **Death 1 — the inputs were wrong and were never oracle-invariant (seat06, measured twice).** Re-measured at fixed work on `beauty.sno < beauty.sno`, fixed point verified every rep: IPC is **2.19 (SCRIP) vs 1.60 (clean oracle `spitbol-bench-oracle/sbl`)**. Against the *instrumented* `x64/bin/sbl` the ranking is noise — ranges overlap and the sign flipped three times across three contention levels. A number whose sign depends on which SPITBOL you divide by is not a property of SCRIP. ⛔ That comparison is also forbidden outright by Lon s255 (`x64/bin/sbl` is the CORRECTNESS oracle only, ~2.2–3.5x handicapped by monitor hooks — re-confirmed here at **3.503x** at fixed work, agreeing with seat2's independent 3.53x).
- ⛔⛔ **Death 2 — the arithmetic never computed a ceiling, and this one would have survived a perfect re-measurement.** `1.4` was `3.20 / 2.33`: our IPC over the oracle's IPC. **That quotient bounds nothing.** SPITBOL's IPC is a fact about SPITBOL's instruction mix and places no limit on how well *our* stream can be scheduled. The real bound comes from the MACHINE — `cycles = instructions / IPC`, so polish that deletes no instructions can only raise IPC toward the core's issue width: on this Zen 4 part, from a measured 2.19 toward a sustainable ~4 is **~1.8x**, toward the ~5–6-wide limit ~2.5x. ⭐ **The ceiling landed near the right order of magnitude by luck, which is the most durable kind of wrong number** — every sanity check it met appeared to confirm it.

✅⭐ **THE QUALITATIVE CLAIM SURVIVES AND THE POLICY IS NOW BETTER FOUNDED, SO DO NOT RE-RANK OFF THIS RETRACTION.** SCRIP does beat the clean oracle on every microarchitectural axis — IPC 1.365x, branch-miss **0.41% vs 1.98%**, frontend-idle **15.57% vs 31.05%** — and it is instruction-count bound far harder than s251 said: at fixed work it issues **65.6x** the clean oracle's instructions (15,177,380,226 vs 231,323,437) and burns **48.1x** the cycles, i.e. **0.0208x**. ⛔ **So "SCRIP beats the clean oracle 1.36x" is true of the IPC row and false of the program** — always carry the denominator. Rank 1 is therefore populated by rows that delete work (table path, PLT indirection, un-inlined calls, fused boxes), not rows that shave cycles off work that stays: **65.6x is available from deleting work; at most ~1.8–2.5x from executing it perfectly.** ⭐ And the blind spot this row was chartered to open is **EMPTY** — there is no cache/branch/frontend win waiting on SNOBOL4; the machine already runs our code near-optimally. Full ruling: `FINDING-2026-08-27-hq_P-ipc-ceiling-retracted-microarchitecture-is-not-the-gap-scrip-is-0.0208x-on-cycles.md`.

---

## 8. ⛔ THE SPITBOL SIDE OF EVERY NUMBER ABOVE WAS MEASURED AGAINST AN INSTRUMENTED ORACLE (seat2, 2026-08-22)

Row `clean-oracle-build` built and verified a from-source, uninstrumented SPITBOL (`/home/resources/spitbol-clean/sbl`, authority in `scripts/lib_oracle_flags.sh`'s `sbl_clean_bin()`) and measured — callgrind Ir, matched `-bf` flags, cross-validated via an independent same-toolchain rebuild of the full instrumented fork tree agreeing to within 7 instructions of the checked-in binary — that the checked-in `x64/bin/sbl`'s monitor IPC bridge costs **~2.2-2.3x the instruction count** on synthetic call-dense/arithmetic-only witnesses, even with the monitor pipes unset.

⛔⛔ **Then ran the actual headline workload.** `beauty.sno < beauty.sno` (the exact input FINDING-2026-08-22-hq-scrip-spends-under-one-percent... already used, md5 `6f1671c0757729992ae01a6bdf16f081`) totals **806,084,475 Ir** on the checked-in oracle vs **228,144,314** on the clean build — **3.53x, 71.7% dead weight**, output still byte-identical to the fixed point on both. That FINDING's own "equalized comparison" (SCRIP 2.1x SPITBOL, "engine is close to competitive") stripped only a 23.47% pattern-match-hook subset of this; substituting a fully-clean SPITBOL denominator moves it to **~5.8x**, holding SCRIP's own reported number unchanged. This directly bears on §7's "instruction-count bound, 1.4x ceiling" reading of FINDING s251 — the SPITBOL side of that reading has never yet been measured clean. Full numbers and everything *not* re-derived (SCRIP's own ledger, IPC/branch-miss, any other workload): `FINDING-2026-08-22-seat2-clean-oracle-monitor-overhead.md` §4a. Flagging the question, not answering it.

## ⭐ PERF/CALLGRIND FIELD NOTES (2026-08-28, from the campaign's measured incidents — each line has a witness)
- ⛔ **Never invoke `/usr/bin/perf`** — it is a version-DISPATCHING shim that refuses on kernel 6.17 ("perf not found for kernel 6.17.0-1032") and reads as "no perf here." The working binary is **`PERF_BIN=/usr/lib/linux-tools-6.8.0-138/perf`** (or the repo wrapper `.tools/bin/perf`, which resolves it). Userspace-6.8 perf records and counts fine on the 6.17 kernel — measured all campaign. The refusal is the command-v-is-not-existence class: it answers "no tools for YOUR RUNNING KERNEL," not "no perf."
- ⛔ **callgrind needs `--main-stacksize=2000000000` on recursive pattern demos** (calculator-1-match, json-match) or it dies rc=139 "Stack overflow in thread #1" **while still writing a plausible partial profile** — a crashed run's profile does not announce itself. calculator-2-match survives the same sweep, so it presents as per-program flakiness, not a missing flag.
- ⛔ **callgrind `--branch-sim` is BLIND to return-stack effects BY CONSTRUCTION** — `ret` is not counted as an indirect branch (measured: a 15-million-return chain registers 287 indirect branches). It can neither confirm nor refute an RSB-mispredict attribution; use `perf stat -e branch-misses` for that axis (Instrument Law 7).
- ⚠️ **SCRIP_BBPROF is currently a dead instrument**: produces no output (bbprof_start bails on g_n==0) and `bbprof_report` is called only from the m3 driver — a mode-4 binary can never report BY CONSTRUCTION, and m4 is what the benchmarks measure. Row is with hq_C.
