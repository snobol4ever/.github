...
- ✅⛔ **The `catch_var_goal` clause could not pass at rung 10a — it was blocked on rung 9, which measured 0/6 at pickup. ⭐ RESOLVED THE SAME DAY, AND NOT BY ME: hq_C landed rung 9 mid-session (corpus `b37c32a36`), and on the re-measure (SCRIP `96df0a36`, pristine) the clause PASSES — the row went 5/7 → 6/7 without another line of call/N work.** The lesson survives its own fix, which is why it stays: a criterion can carry a clause that no amount of work on ITS OWN rung could ever close, and naming that at pickup beats discovering it at the landing. What follows is the state at pickup, kept as the record. `catch/3` still refuses at the ladder gate (`test_prolog_ladder.sh --only 9`: `PASS=0 FAIL=6`). The clause is correct to want, but it makes this row's DONE-WHEN un-closable until hq_C's rung 9 lands, independent of any call/N work. **Naming it beats discovering it at the landing.**

## 6. Gates

`ladder --only 10`: `ladder__rung10_dcg_phrase` **m3=PASS m4=PASS** (was FAIL/NOBUILD); `ladder__rung10_call_n` FAIL rc=2 → rc=1 (refusal → real failure, cause named in §4.2); `ladder__rung10_assert_retract_dynamic` is 10b, hq_C's, untouched. Row witnesses **4/7 → 5/7**, then **6/7** on the re-measure after hq_C's rung 9 landed. ⭐ **The only remaining witness is `multi_backtrack`, and it now fails `rc=134` on this finding's own named bomb** — so the whole rest of the row is the one half § 4 names: the PL-protocol re-drive box.

**Control arms** (shared node — `bb_call_value` is Icon's box). SNOBOL4 blocking board `m3 PASS=1679 FAIL=0 · m4 PASS=1679 FAIL=0 SKIP=0 · MISSING=0` ✅ GATE OK; `make test` ✅ PASS. ⛔ Icon master `377/377 of 381` against floor **379** — RED, **PRE-EXISTING, and controlled twice on two different trees**: on `fd3ec810`, 376 with the change stashed vs 376 applied; on `37ffc574`, 377 with the three files reverted to `HEAD~1` vs 377 applied. Byte-identical both times. Routed to ceo as its own item.

⛔ **TWO INSTRUMENT FAULTS OF MY OWN, named because each printed plausible output and neither would have announced itself.** (1) My *second* Icon control ran `git stash push -- src/` **after** the change was already committed, so it stashed nothing and measured the same tree twice — a convincing 377-vs-377 that proved nothing; caught only on the `No stash entries found` line. (2) I passed a rung LABEL to `update_icon_bench_asm.sh`, whose first argument is a **GLOB** — it matched zero files and reported `total=0 … unchanged=0`, exit 0, which reads exactly like "already current"; the handoff gate caught it by still showing 20 owed. ⭐ **Both are the vacuous-instrument class: a check that cannot fail prints the same string as one that passed. The gate that could not be fooled is the one that counted artifacts rather than trusting a script's exit code.**
