...
2. **Comment & blank-line purge** — every `.c/.h/.y/.l` under `SCRIP/src`: all `/* */` and `//` comments stripped, **all blank lines deleted**, **200-char `/*----…----*/`** separators inserted between functions / top-level partitions. 187 files, 72,442 → 60,455 lines. Stripper is string/char-literal aware.
