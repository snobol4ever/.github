...
2. **`dcap-alpha-cell-m4`** — my own s190 suggestion, still unopened; it also owns `claws5.sno` m4 (row D-2). Checked, not guessed: `push_list_α`/`pop_list_α` **are** real symbols in the emitted `.s`, and a per-proc startup mechanism (`.Lstartup_prec<N>` + `rt_proc_register_rec`) already exists to hang the seal on.
