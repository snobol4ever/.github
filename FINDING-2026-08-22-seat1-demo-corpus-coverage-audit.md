# FINDING — seat1: demo corpus coverage audit — 15 rows newly gated, 4 defects documented, 1 live regression flagged

**Date:** 2026-08-22 · **Seat:** seat1 (`/home/claude1`, Claude Sonnet 5) · **Topic:** demo-corpus-coverage-audit (THE LOOP queue, rank 0) · **Status:** coverage landed + pushed; 4 defects documented (not fixed, out of scope); 1 pre-existing regression escalated to HQ
...
## 5. ⛔⛔⛔ demo_treebank IS CURRENTLY BROKEN — LIKELY REGRESSION, NOT CAUSED BY THIS SESSION

`demo_treebank` was **already** a gated row before I touched anything (one of the original 4). Its `.ref` (`matched bytes=327`) is freshly reconfirmed oracle-correct: `x64/bin/sbl -bf treebank.sno < treebank.input` → exactly `matched bytes=327`, byte-identical to the committed `.ref` (git blame: last touched 2026-08-18, "treebank: fix under scrip"). But the current pristine-built `scrip`, on the exact same committed source and input, crashes:
...
in **both** m3 and m4. I did not modify `treebank.sno`, `treebank.ref`, or `treebank.input` — `git status` on the corpus repo confirms zero changes to any pre-existing file. This surfaced only because I did a `make pristine` + full gate run, which (per HQ-27's own rationale) is exactly the scenario a stale objdir can mask. The brief's cited baseline ("m3 339/341, m4 338/341+1 SKIP") already had 2 standing FAILs before my changes; whether `demo_treebank` was already one of those two (silently) or this is a fresh regression from a concurrent seat's commit since 2026-08-18, I can't tell from here — **I left the row exactly as committed rather than regenerating `.ref` to match the crash, which would hide a real defect the same way the brief warns against.** Escalating to HQ directly (§7) given the severity: a demo the board already counted as gated is not actually passing.
...
- ⛔ **Urgent:** `demo_treebank` regression (§5) needs ASM-DIFF-FIRST triage — likely candidate: whatever recently touched the TABLE/ARRAY subscript path or the deferred-match-action machinery, since `json.sno`'s hang and `calculator-2.sno`'s divergence are both in the same family of "big pattern match with deferred semantic actions building/indexing a data structure" programs. Not asserting a shared root cause — flagging the family resemblance as a lead.
