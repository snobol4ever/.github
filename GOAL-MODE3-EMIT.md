...
Carved 2026-05-11 (Claude latest, session ME-9c+ME-9d+ME-9e+ME-9f). Premier-goal declared by Lon. Architecture locked: one value stack, r12=TOS, r13=&SM_State, r10=BB data, rbp=fn frame, variant patterns stay dynamic. Full history in git log.

Closed rungs: ME-1 ✅ `cc3cd475` · ME-2 ✅ `babf76be` · ME-3 ✅ `aca47e6c` · ME-4 ✅ `06b8f503`+`ae7f325a` · ME-5 ✅ `880adc36` · ME-7 ✅ `3d88cee7` · ME-6 ✅ `accafb5f` · ME-9a ✅ `f087571e` · ME-9b ✅ `f087571e` · ME-9c ✅ `15fff315` · ME-9d ✅ `02bf4cd7` · ME-9e ✅ `38019ad1` · **ME-9f ✅ `497b3712`**.

Session 2026-05-11 (ME-9f): new `emit_me9_pat_binary_blob(fn, tramp)` — 63-byte template for binary-pattern combinators (pop right=TOS into rdx:rcx, pop left=TOS-1 into rdi:rsi, call fn via imm64, write result back at TOS-1, sub r12,16; net delta −1). Shape mirrors `emit_me4_concat_blob` exactly. `SM_PAT_CAT` and `SM_PAT_ALT` dispatched directly to `pat_cat`/`pat_alt` runtime entries — no me9_* helper indirection. **Both verified hot**: SM_PAT_CAT (op=47) fires 4× across feat/f04/f05/f07 plus many parser/ programs; SM_PAT_ALT (op=46) fires on parser/pat_alt_*, parser/defer_alt. All 21 feat/ programs byte-identical `--sm-run` vs `--jit-run`. parser/: 86 byte-identical, 2 pre-existing segfaults (defer_in_pat.sno, unary_not.sno) — both confirmed pre-existing pre-ME-9f via git-stash regression test; both belong to ME-13 (DT_P pat construction lifetime issue + unary `~` operator). Gates: smoke 7/7, broker 49/49, me6 reentry 3/3.

The 4-rung pattern ME-9c+d+e+f demonstrates two generic emit templates cover Groups C+D+E+F: `emit_me9_pat_charset_blob` (single-DESCR_t-arg, net delta 0; 54 bytes) and `emit_me9_pat_binary_blob` (two-DESCR_t-arg, net delta −1; 63 bytes). Any SM_PAT_* whose runtime entry matches `DESCR_t(*)(DESCR_t)` or `DESCR_t(*)(DESCR_t,DESCR_t)` — directly or via one-liner `me9_*` helper — plugs in with a single dispatch clause and zero new emit code.
