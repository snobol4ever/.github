...
Carved 2026-05-11 (Claude latest, session ME-9c+d+e+f+g — **Phase D closed**). Premier-goal declared by Lon. Architecture locked: one value stack, r12=TOS, r13=&SM_State, r10=BB data, rbp=fn frame, variant patterns stay dynamic. Full history in git log.

Closed rungs: ME-1 ✅ `cc3cd475` · ME-2 ✅ `babf76be` · ME-3 ✅ `aca47e6c` · ME-4 ✅ `06b8f503`+`ae7f325a` · ME-5 ✅ `880adc36` · ME-7 ✅ `3d88cee7` · ME-6 ✅ `accafb5f` · ME-9a ✅ `f087571e` · ME-9b ✅ `f087571e` · ME-9c ✅ `15fff315` · ME-9d ✅ `02bf4cd7` · ME-9e ✅ `38019ad1` · ME-9f ✅ `497b3712` · **ME-9g ✅ `12982b72`** · **ME-9 (parent) ✅ — all 7 sub-rungs closed**.

Session 2026-05-11 (ME-9g): added `me9_pat_deref(DESCR_t)` C helper (3-way DT_P/DT_S/else dispatch — pass-through / pat_lit / pat_ref(VARVAL_fn) with null-guard) and new `emit_me9_pat_refname_blob(name, tramp)` (52 bytes, same shape as `emit_me9_pat_lit_blob` with `pat_ref` imm64 instead of `pat_lit`).  `SM_PAT_DEREF` dispatches through the existing `emit_me9_pat_charset_blob` template with `me9_pat_deref` as helper; `SM_PAT_REFNAME` uses the new dedicated emit blob.  **Both verified hot**: DEREF fires 2× on `feat/f04`, 4× across parser/; REFNAME fires 3× across 3 parser/ programs. All 21 feat/ + 86 parser/ byte-identical; same 2 pre-existing segfaults as ME-9f (defer_in_pat, unary_not — both ME-13 territory). Gates: smoke 7/7, broker 49/49, me6 reentry 3/3.

**Phase D summary** (ME-9a..g): all `SM_PAT_*` constructor opcodes now have native blobs.  Three reusable emit templates cover everything: `emit_me9_pat_nullary_blob` (no args, push 1; 42B), `emit_me9_pat_lit_blob` / `emit_me9_pat_refname_blob` (imm64 string arg, push 1; 52B), `emit_me9_pat_charset_blob` (1 DESCR_t, push 1; 54B), `emit_me9_pat_binary_blob` (2 DESCR_t, push 1; 63B).  Five thin `me9_pat_*` C helpers handle coercion/type-discrimination where the runtime entry signature doesn't match directly (charset VARVAL coercion, integer-arg DT_I check, deref 3-way dispatch).  Frontend coverage of nullary `SM_PAT_*` opcodes (ME-9a) remains gated by the SNOBOL4 lexer's handling of bare keywords; SNOCONE/REBUS frontends may activate those paths in the future.  No new emit code expected for ME-10 / ME-12 — they reuse the same templates.
