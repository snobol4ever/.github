...
/*------------------------------------------------------------------------------------------------------------------------*/
static IR_t * emit(lcx_t * cx, IR_e op, IR_t * γ, IR_t * ω) { IR_t * nd = IR_node_alloc(cx->g, op); γ_to(nd, γ); ω_to(nd, ω); return nd; }
/*------------------------------------------------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------------------------------------------------*/
static IR_t * lower_nary(lcx_t * cx, const tree_t * t, IR_e op, IR_t * γ, IR_t * ω) { IR_t * nd = emit(cx, op, γ, ω); push_kids(cx, nd, t, 0); return nd; }
/*------------------------------------------------------------------------------------------------------------------------*/
static IR_t * lower_binop(lcx_t * cx, const tree_t * t, const char * opn, IR_t * γ, IR_t * ω) { IR_t * nd = emit(cx, IR_STRUCT, γ, ω); IR_LIT(nd).sval = opn; ir_operand_push(nd, lower(cx, t->c[0], NULL, NULL)); ir_operand_push(nd, lower(cx, t->c[1], NULL, NULL)); return nd; }
/*------------------------------------------------------------------------------------------------------------------------*/
static IR_t * lower_unop(lcx_t * cx, const tree_t * t, const char * opn, IR_t * γ, IR_t * ω) { IR_t * nd = emit(cx, IR_STRUCT, γ, ω); IR_LIT(nd).sval = opn; ir_operand_push(nd, lower(cx, t->c[0], NULL, NULL)); return nd; }
/*========================================================================================================================*/
static IR_t * lower(lcx_t * cx, const tree_t * t, IR_t * γ, IR_t * ω) {
    ...
    case TT_QLIT: { IR_t * nd = emit(cx, IR_ATOM, γ, ω); IR_LIT(nd).sval = t->v.sval; return nd; }
    case TT_NUL: return emit(cx, IR_LIT_NUL, γ, ω);
    case TT_VAR: { IR_t * nd = emit(cx, IR_LOGICVAR, γ, ω); IR_LIT(nd).sval = t->v.sval; return nd; }
    ...
    case TT_FNC: { IR_t * nd = emit(cx, IR_STRUCT, γ, ω); IR_LIT(nd).sval = (t->n > 0 && t->c[0]) ? t->c[0]->v.sval : t->v.sval; push_kids(cx, nd, t, 1); return nd; }
    ...
    case TT_CUT: return emit(cx, IR_CUT, γ, ω);
    case TT_IF: return lower_nary(cx, t, IR_ITE, γ, ω);
    case TT_ATTR: return emit(cx, IR_SUCCEED, γ, ω);
