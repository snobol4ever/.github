...
| `subscript_get2` | 41 stale / **52 real** (re-derived s217, sharded compiler sweep, 316 programs) | AGG | `OUT:ICON-RTX:s217` — step 0 complete: 0(d) **50,000→200,000 at N→4N, exactly 4×** · 0(g) **DT_S 100% synthetic / 97.2% corpus** (unguarded call, third confirmation of s216's sub-rule) · cost decomposition **exactly ONE `rt_str_alloc` per call, `rt_ws_alloc` FLAT at 185** ⇒ high portable fraction, the **inverse of refused `rt_make_list`**. ⛔ **CORPUS REACH IS ONLY 141 ARRIVALS across 303 programs — COMPLETENESS port, NO SPEED CLAIM.** Its sibling `subscript_set` is a phantom, see above |
