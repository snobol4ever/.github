...
0. **GREP THE FAMILY'S SYMBOLS IN `src/` AND CONFIRM A LIVE DEFINITION EXISTS** (added RTX-2, s163, after a whole rung was aimed at `blk_alloc`/`blk_free` — symbols with zero live call sites whose C sits unbuilt in `archive/backend/`). `grep -rn '<sym>' src/ --include=*.c --include=*.h --include=*.cpp` plus a live-artifact sweep. Two minutes; skipping it cost a rung.
