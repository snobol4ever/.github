...
⛔ **WHY THIS SECTION EXISTS: THE EXCEPTION WAS REAL AND UNWRITTEN, AND AN UNWRITTEN EXCEPTION IS HOW A LAW STOPS BINDING.** The no-runtime-dispatch rule reads as absolute everywhere it is stated, so every legitimate if-gate in the tree contradicted it on its face; a seat that has seen the rule contradicted once stops treating it as a rule. **THE MEASURED CONSEQUENCE (CEO-693/694):** `IR_MOVE_LABEL` — a JCON carry-over, deleted on Lon's word, *"there are no labels"* — existed as a **FREE-STANDING** op that wrote a gate slot, which made dynamic dispatch **AVAILABLE AS A GENERAL FACILITY**. Prolog's `catch/3` took it, at three sites (`lower_prolog.c:542-544`, `:564`), and `catch/3` is not an if-type construct. **AN EXCEPTION TO A LAW BECAME THE LAW'S ORDINARY CASE WITHOUT ANYBODY RULING THAT IT SHOULD.**
