...
## ⛔⭐⭐ STATIC WIRING ONLY, UNLESS IT IS AN IF-TYPE CONSTRUCT (Lon 2026-09-13, in-chat to ceo, verbatim: *"We use static wiring only, unless it is an if-type construct. So fix that."*; CEO-694)

**THE LAW:** ports are wired at compile time into straight-line jumps and the wiring IS the execution. **THE ONE EXCEPTION, NAMED: AN IF-TYPE CONSTRUCT MAY SELECT ITS CONTINUATION AT RUN TIME.** That is the Proebsting ifstmt.gate shape — a gate slot written at each arm's success, and an α that jumps through it (`frame_layout.c`'s `gate.stored resume target`, read by `IR_INDIRECT_GOTO`).

⛔ **WHY THIS SECTION EXISTS: THE EXCEPTION WAS REAL AND UNWRITTEN, AND AN UNWRITTEN EXCEPTION IS HOW A LAW STOPS BINDING.** The no-runtime-dispatch rule reads as absolute everywhere it is stated, so every legitimate if-gate in the tree contradicted it on its face; a seat that has seen the rule contradicted once stops treating it as a rule. **THE MEASURED CONSEQUENCE (CEO-693/694, WITNESS CORRECTED BY THE cto 2026-09-13, CEO-707):** `IR_MOVE_LABEL` — a JCON carry-over, deleted on Lon's word, *"there are no labels"* — existed as a **FREE-STANDING** op that wrote a gate slot, which made dynamic dispatch **AVAILABLE AS A GENERAL FACILITY**.

⭐⭐ **AND THE HONEST WITNESS IS STRONGER THAN THE ONE THIS SECTION FIRST CARRIED.** The ceo wrote that Prolog's `catch/3` had taken the facility at `lower_prolog.c:542-544`/`:564`. **THAT IS FALSE AND WAS CORRECTED ON THE SOURCE:** those sites are `pl_lower_softcut` — the ISO soft cut `(C *-> T ; E)`, whose `ml_e`/`ml_c`/`ml_t` are condition/then/else gate arms — and **`pl_lower_catch` BUILDS NO GATE AT ALL**, never did, being `IR_BOUND` + `IR_UNMARK` + a leaf. ⛔ **A SOFT CUT IS AN IF-TYPE CONSTRUCT — an if-then-else whose condition does not commit — SO ALL FIVE SITES FALL UNDER LON'S EXCEPTION AND NOTHING EVER ABUSED THE FACILITY.** **THE REAL WITNESS: THE FACILITY WAS REACHABLE BY EVERYTHING AND NOTHING ELSE TOOK IT — WHICH IS LUCK, NOT DESIGN, AND IS A STRONGER ARGUMENT FOR SCOPING THAN A VIOLATION WOULD HAVE BEEN.** A law justified by a violation lapses the day the violation is cured; a law justified by an unclaimed opening does not.

⭐ **THE STRUCTURAL RULE THAT FOLLOWS, AND IT IS THE POINT: THE GATE WRITE BELONGS TO THE IF CONSTRUCT'S OWN LOWERING, AS A NAMED PART OF IT — NEVER TO A GENERAL OP ANY CONSTRUCT CAN REACH FOR.** A facility reachable by everything is used by everything. Scoping the mechanism to the construct that is permitted to use it is what makes the exception enforceable rather than advisory; ⛔ and a new IR op that selects a continuation at run time, for anything that is not an if-type construct, is a defect and is refused at review whatever it is called.

⛔ **AND THE NAME IS NOT THE MECHANISM:** *there are no labels* is about the IR, not about the gate. An if-type construct **SELECTS A CONTINUATION**; it does not move a label. An op named for what JCON needed — a JVM target with no compile-time port wiring available to it — is **a defect wearing a provenance**, and `refs/jcon-master` is canonical for what Icon MEANS, never for how this machine should be built.

## ⛔⭐ THE MOD_OP TAG CEILING — 239 OF 255 SPENT, SIXTEEN LEFT (measured 2026-09-13, cto; re-measured by the ceo at SCRIP `5b17c350f`, CEO-676)

`MOD_OP` is an **8-BIT TRACE TAG**: the thunk macros pack `modop << 8` above a `uint8_t` `DESCR.v` and the ZSM reader masks it out, so the tag space is 0..255 and nothing wider fits without changing the packing. `src/ir/descr_tags.inc` stands at **239** (`MOD_OP_RT_PL_FLUSH_OUTPUT_SB`, the highest defined number). **SIXTEEN NUMBERS REMAIN.**

⛔ **THIS CONSTRAINS DESIGN IN THREE LANES AND NOBODY HAD LOOKED.** Any scheme that mints a tag PER RUNTIME ENTRY hits this wall: the original asm-thunk sweep proposal of 2026-09-13 would have wanted **seventy-four** of them and simply could not have been built. The cto refused that scope on evidence — auditing all 74 plain leaves to their `PL_CX_LEAF_HEAD` and `rt_pl_*_cell` and finding that **only `rt_pl_succ_plus_cell` writes a ball**, so the class was two names and not thirty — but the ceiling is what makes "mint a tag per entry" structurally unavailable rather than merely expensive.

⛔⭐ **ELEVEN OF THE SIXTEEN ARE ALREADY SPOKEN FOR (2026-09-13 14:4x, CEO-678):** the Prolog stream-leaf ball class is ELEVEN names, not the two first reported — `succ`, `plus`, and the nine INBOUND stream leaves (get/peek/unget of char/code/byte and their `_s` forms). **WHEN THAT ROW LANDS, FIVE NUMBERS REMAIN.** The rule below therefore binds harder and sooner than it did when this section was written an hour earlier, and the next design that wants a tag per entry should assume the space is effectively closed.

⭐ **THE RULE:** a seat proposing a design that mints `MOD_OP` tags states how many it will consume, against sixteen, IN THE ROW. A seat that budgets tags without knowing the ceiling discovers it the expensive way — at the point where the packing itself has to change, which is every thunk macro and the ZSM reader at once. This section exists so the number is greppable instead of re-derived; ⛔ RE-MEASURE IT (`tail -3 src/ir/descr_tags.inc`) BEFORE QUOTING IT — a number's tree is part of its label, and this one moves every time a runtime entry is added.
...
- ⛔ **FACT (Lon 2026-09-02, in-chat to ceo): `r10` = statement number, `r11` = BB node id** (`x86_asm.h` emits `mov r11, nid` at box entry, today under the diag-regs arm). **⛔ SUPERSEDED FOR r10 ON 2026-09-24 (Lon, in-chat to the ceo, verbatim: *"Is R10=statement number and R11=node-id number? If so one is derivative of the other. We should use only R11, and derive the statement number from the nonde id number."*; SCRIP `2a81a02db`, CEO-1243): `bb_statement.cpp` no longer loads r10; `r11` = the BB node id at every box α/β entry (the `SCRIP_DIAG_REGS` arm, default on, 306 loads in the arith_loop twin) is the ONE diagnostic register, and the statement number is DERIVED from it through the node table `--dump-bb` prints (a node belongs to exactly one statement). Nothing in the runtime read r10 (census 2026-09-24: zero readers). r10 is an RTX scratch register, caller-saved, and carries nothing across a call (CEO-551).** **And (same day): `r13`, `r14`, `r15` — the SNOBOL4/Icon string-scanning registers — are granted to Prolog for its internal transmission: ceo assigned `r13` = B (youngest choice frame), `r14` = ROOT (the ζ-STANDING record), `r15` = exception ball or 0, and — Lon's further grant the same hour, "R12 points to a C.A.S … it is a stack" — `r12` = TR, the Prolog trail as the Conditional-Assignment Stack (the role `rtx_abi.inc:16-21` records as r12's dead history — r12 is free in today's ABI, which is why Prolog may take it); saved/restored as a triple at polyglot language boundaries (`ARCH-PROLOG-BYRD-BOX-TRANSLATION.md`, top).** "Freed" above means freed from the continuation pair, not free for a new design to claim: any new wire (the Prolog B pointer, the exception ball, `ARCH-PROLOG-BYRD-BOX-TRANSLATION.md`) takes a register from the plane in `src/ir/zeta_choices.h` or a frame slot, never `r10`/`r11`.
