...
⚠️ **Status, stated so this section is not read as describing a finished world:** the generation-time check exists at the chokepoint as an **audit that reports and does not yet refuse** (`SCRIP_PORT_EXIT_AUDIT=1`; it emits zero bytes and is proven byte-identical to the pre-change compiler). Refusal needs emit-time `rax` provenance, i.e. new emitter state, which needs Lon's in-chat grant under NO-NEW-GLOBALS. The graded instrument is `scripts/test_gate_port_exit_value_contract.sh`, which is **RED at 9 sites across 3 languages** at the time of writing and is not wired into `make test`. ⛔ Its emitter-side coverage is **known incomplete and measured at 86%** — `g_emit.flat_lcl_proc` does not flag every promoting chain — so the gate cross-checks the emitter against a post-hoc TEXT scan as a *directional subset*, refusing only in the dangerous direction.
