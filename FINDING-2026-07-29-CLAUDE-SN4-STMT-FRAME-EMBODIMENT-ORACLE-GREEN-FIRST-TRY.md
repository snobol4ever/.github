...
## ENCODER SLICE (same session) — STATEMENT-FRAME FAMILY LANDED INERT, ALL GATES GREEN
`x86_asm.h` (beside the s20x push/cut family): `x86_stmt_enter/leave` (STATEMENT bracket), `x86_call_frame_enter(γ_ilbl, ω_ilbl)` (BB IR_CALL 32B header {pad, caller rbp, γ, ω}; body jmp + the `x86_alpha_carve` slot carve stay template business), `x86_return_floater`/`x86_freturn_floater` (roles 1/2: cut-from-any-depth, restore caller rbp, jmp wire). Composed ENTIRELY of pre-verified encoders (push/pop/mov/add/sub arms + named disp32 family + `x86_lea_rip_id` + `x86_jmp_reg`) — ZERO new byte encodings, keystone byte-verify discharged by construction; dead code until the classifier wires a consumer (CELL-0 landing discipline, verbatim). GATES: build clean · watermark m3 329/5 · m4 324/2 SKIP=8 IDENTICAL to session baseline with fail-set MEMBERSHIP diffed programmatically empty · witness compiler output byte-identical · all three handoff-step-4 regen sweeps run = zero artifact commits (byte-identity proven corpus-wide). SCRIP commit: see log ("statement-frame encoder family").
NEXT: the per-graph all-or-nothing regime — classifier (straight-line int-only statements first), env switch, first consumer wiring of the four encoders + per-BB carves at the emit choke point; watermark may only add passes.
