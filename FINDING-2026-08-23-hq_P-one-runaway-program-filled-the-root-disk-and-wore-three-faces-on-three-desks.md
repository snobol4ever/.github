...
## 4b. ⭐⭐ THE GATE FOUND A THIRD OFFENDER THAT TWO HAND-SWEEPS MISSED — AND ITS 1.8GB WAS SITTING IN THE EVIDENCE THE WHOLE TIME

All three cures landed (`e6faa54e`, `cf937987`, `dc2239ef`). The durable one is `scripts/test_gate_scratch_is_bounded.sh`: a **zero-assert** on the detonation-capable shape (untrapped + bare-`/tmp` scratch + a program's unbounded output redirected into it) plus a **computed ratchet** (23, measured never typed) on untrapped-but-*bounded* scratch, which leaks kilobytes rather than gigabytes.

⛔ **I reported the detonation-capable set as "exactly two files." It was three.** The gate failed on its **first run** and named `test_monitor_3way_sync_step_auto.sh` — the complete outage shape: `mktemp -d /tmp/monitor_auto_XXXXXX`, no trap, three participants' unbounded stdout+stderr redirected in, all under a `timeout`. My sweep's regex required a quoted `$WORK`; that script writes `$TMP`.

⭐ **And it had already detonated.** The s267 emergency cleanup listed a stale `monitor_auto` scratch dir holding **1.8GB — the fourth-largest consumer on the full disk.** That was this script's *past* eruption, sitting unattributed in the outage evidence while I was hand-grepping for the *next* one. Every byte of the outage is now attributed and every producer fenced or gated.

**The general point, and the artifact made it rather than the author:** two careful hand-sweeps missed a live instance that a computed check found in under a second. A sweep proves something about the moment it ran; a gate proves it every time anyone commits. *Two fixes are today; a gate is every day.*

**Negative-tested by injection, four cases, exit codes read UNPIPED:** untrapped `/tmp`+unbounded ⇒ **1** · same script *with* a trap ⇒ **0** (no false positive) · untrapped-but-bounded ⇒ the **ratchet** arm, not the zero-assert · empty scripts dir ⇒ **2 UNPROVEN**, not a pass. ⛔ Mid-verification I read `$?` through a `head` pipe and got `0` for a run that had genuinely failed — **RULES.md's own documented trap** (*"read the verdict line, not a pipeline's `$?`"*), hit while testing a gate built to stop exactly that family of false green. Re-checked unpiped rather than trusting the first reading.
