...
## 6. POST-REBASE ADDENDUM — the rule got a second witness, and neutrality got a stronger proof

Mid-session a parallel s22j pushed two `emit.cpp` codegen fixes (`ALT-NARY REJOIN` — every alternation in the corpus segvd — and `FIX 2`, restoring the graph slot region in the jmp-entry blob frame). ZD-K1/K2 rebased onto them with **zero conflicts**. My own artifact commits were dropped before rebasing rather than carried: they had been generated by a compiler lacking those fixes and were stale by construction.

**Neutrality re-proven at the merged base, and at a stronger level than §2.** A compiler built at `bc4a3467` (their HEAD, without ZD-K1/K2) and one built at mine were each run over the entire crosscheck corpus:

> **318 programs compiled by both · 318 byte-identical · 0 differing · 0 no-output**

Watermark at merged HEAD: **m3 276/41 · m4 276/40 · DIV=3** {`140_pat_eval_double_fn_trick`, `151_pat_arbno_inline_fence_backtrack`, `W04_arbno_basic`}. **That gain belongs to s22j's ALT fix, not to ZD-K1/K2.** Recording it any other way would be the attribution defect of §2 in the watermark column instead of the commit log.

**§2's rule took its second witness the same session.** The post-rebase regen produced 4 changed rung10 `func` artifacts, and the commit again initially claimed them for ZD-K1/K2. Compiling all four with both compilers gives identical md5s (`76417b6d`, `f20f8ef4`, `d0c99328`, `dfd40ed2`) — they are the parallel session's residual regen debt. Corrected before push.

⛔ **THE RULE, NOW TWICE-WITNESSED IN ONE SESSION:** a session that lands ANY codegen commit owes the three regens however it characterises itself; **and the session that later runs them must attribute the bytes BY MEASUREMENT — rebuild the earlier compiler and diff — never by assuming the bytes are its own.** The assumption is seductive precisely because the regen runs inside your session and commits under your name.

⚠ **CONSEQUENCE FOR THE NEXT RUNG:** the §3 break-set measurement (19 programs, A=14/B=5) was taken at `9df6c5d3`, *before* the ALT fix moved 45 programs. **RE-MEASURE the NOFC break set at merged HEAD before spending a rung on Family A** — the census may have shifted, and this file's own CENSUS SHELF LIFE law says re-run, never cite.
