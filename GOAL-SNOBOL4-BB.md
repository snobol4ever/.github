...
# ▶▶▶ NEXT SESSION — START HERE (session 27, 2026-06-25)

**AXS-1 LANDED (SCRIP `e3d0ff8`, session 26).** Inline array-subscript read + write, plus latent encoder fix. (a) `bb_idx_get.cpp`: inline DT_A+int fast path — guard `eax==DT_A`, guard index `r8d==DT_I`, bounds-check (`ecx = i - lo; js slow; cmp ecx,hi-lo; jg slow`), element load `[data+idx*16]` via ×8-scale twice (`add rcx,rcx; mov rax,[r11+rcx*8]; add r11,8; mov rdx,[r11+rcx*8]`); fall through to `subscript_get` on any miss. (b) `bb_idx_set.cpp`: same guards + bounds path, element store via REGDISP `[r11+0]`/`[r11+8]` (no MEMIDX8 store form exists); fall through to `subscript_set`. Index loaded once into `op_sa` scratch to prevent double-eval of side-effecting by-name forms (`ARR(INPUT)`). 32-bit `ecx` arithmetic matches slow-path `(int)to_int(idx)` truncation exactly — correct by construction without oracle. (c) `x86_asm.h` `x86_load_indexed8`: fixed latent binary-medium bug — hardcoded `REX=0x48` never set REX.B/X/R for r8–r15 operands; first exposed when AXS read path used base r11, causing mode-3 segfault while mode-4 (text) assembled correctly. Fix: compute rex from g/x/b register numbers. Gate: crosscheck PASS=171 FAIL=84, fail set byte-identical to pristine HEAD (zero regressions). Both-medium: mode-3 binary == mode-4 text on all AXS-exercising programs (`table_access`, `100_roman_numeral`). Pre-existing `1110`/`1112` mode-3 fault confirmed unrelated to AXS (lowercase `array()` = undefined in case-sensitive SCRIP). **Honesty note:** both benchmark targets cited for AXS (`table_access`, `mixed_workload`) use `TABLE(...)` not `ARRAY(...)` — AXS guards `DT_A` only, so those benchmarks still take the slow path. AXS wins on genuine array-indexed programs (`100_roman_numeral` ARRAY(13) lookup). Closing the TABLE gap needs a table-probe inline (separate rung). DCR-3 (3-way re-grounding) still pending (needs oracle token for `sbl -b`).

**▶ NEXT MOVE (session 27): DCR-3 ✅ DONE (re-grounded above). Remaining DCR open rung is DCR-2 (codegen-side by-name call drop). LON PIVOT (2026-06-25) → do the **OPT** rung next: IR/codegen optimizer passes (BOPT IR-level branch-chaining, then PEEP/CSE, then general CFOLD) per Proebsting's four-port paper — see `## ▶ PRIORITY RUNG (do FIRST, session 27): OPT` below. All per-primitive inlines (RPF relop-prefetch, SAB self-append, the DCR-2 codegen rung, table-probe) are HELD behind OPT. RPF WIP is git-stashed on `main` ("RPF (relop operand prefetch) WIP — on hold per Lon").**
...
**THREE-WAY COMPARISON — RE-GROUNDED post-DCR (session 27, 2026-06-25; DCR-3).** All 16 benchmarks run SCRIP mode-4 (AOT native) vs SPITBOL (`sbl -b`) vs CSNOBOL4 (`snobol4` 2.3.3). Reproducible: `scripts/test_3way_snobol4.sh` (wall-clock ratio, engine-neutral; both oracles auto-detected). **Correctness: 14/14 result-bearing benchmarks byte-identical across all three present engines** (`arith_loop` and `indirect_dispatch` emit no `result:` line, so they are correctness-`?` not graded). `indirect_dispatch` remains the one known DIVERGENCE (`R = $FN(X)`, `FN='ADD1'`): SPITBOL ERRORS ("undefined function called"), SCRIP returns blank `R`, CSNOBOL4 returns nothing — the divergence is in ERROR behavior, not a printed result, so no engine emits a comparable `result:` line; nothing gates it. **Performance (wall-ms ratio = SCRIP/SPITBOL): SCRIP is SLOWER than SPITBOL on 15/16, ≈parity or faster on 1.** Re-grounded ratios: arith_loop 2.90×, op_dispatch 2.29×, string_concat 10.01×, eval_fixed 8.16×, mixed_workload 12.11×, table_access 10.86×, roman 21.80×, **fibonacci 10.50× (was 23.1× — DCR HALVED it)**, string_pattern 10.35×, string_manip 17.22×, **func_call_overhead 6.62× (was 13.3×)**, **func_call 6.65× (was 14.2×)**, eval_dynamic 165.47×. var_access 1.05× (≈parity; was 0.77× faster — slipped to parity, measurement-sensitive at `-O0` gcc of the native `.s`). **Faster than SPITBOL on 1:** pattern_bt 0.77× (FZ path). **DCR EFFECT (the headline of this re-grounding):** the function-call cluster — the whole point of the DCR ladder — dropped from ~14–23× to ~6.6–10.5×: func_call 14.2×→6.65× and func_call_overhead 13.3×→6.62× (cell-cache + memoized cells, both ~2.6×), fibonacci 23.1×→10.50× (~2.2×). The A/B numbers (DCR-1 func_call 2.61×, fibonacci 2.07×) are confirmed at the whole-benchmark level. **roman barely moved (22.7×→21.80×)** — honest: roman's per-call cost is dominated by the pattern-match + `REPLACE`, not the call save/restore protocol DCR optimized, so the protocol win is a small slice of roman's work. vs CSNOBOL4 SCRIP now wins/ties more broadly (pattern_bt 0.71×, op_dispatch 0.78×, arith_loop 1.08×; func_call 2.63× vs SBL's 6.65×, i.e. SCRIP is ~2.6× SLOWER than CSNOBOL4 on calls but the DCR gap-narrowing applies vs both). **The "10×" headline is STILL inverted for general workloads — DCR narrowed the call cluster by ~2× but did not close it.** Root cause unchanged (assembly-traced): native control flow but a runtime PLT call per SNOBOL primitive that SPITBOL inlines/threads. eval_dynamic COMPLETES in ~72s (NOT OOM; the bench-script "CRASH" is its 30s timeout). The remaining PERF-CALL sub-rungs (AXS done; SAB, table-probe) inline the next primitives.
...
- [ ] **DCR-2 — codegen-side resolution (drop the by-name call entirely).** Replace the per-call `lea rdi,name; call rt_call_named_proc` with a baked proc index/cells resolved at preamble (GVA-analogous `__proc[]` table), so the call site does `mov rdi,[rip+__proc+k*8]; call rt_call_proc_direct` — eliminates even the cache lookup. `emit_bb.c` IR_CALL arm + `lower_snobol4.c` (mark statically-resolvable `IR_CALL`s; keep by-name fallback for `$FN`/`OPSYN`/runtime-`DEFINE`). **Gate:** crosscheck byte-identical; further `func_call` delta.
- [x] **DCR-3 — gate + re-ground.** ✅ DONE (session 27, 2026-06-25). `.s` confirmed byte-identical (DCR was runtime-only, idempotent regen, corpus tree clean). 3-way re-run via new committed `scripts/test_3way_snobol4.sh` (SCRIP/SPITBOL/CSNOBOL4 2.3.3, wall-clock ratio). Func-call rows updated in the THREE-WAY block above: func_call 14.2×→6.65×, func_call_overhead 13.3×→6.62×, fibonacci 23.1×→10.50× (DCR ~2.2× win confirmed at benchmark level); roman 22.7×→21.80× (unchanged — pattern/REPLACE-bound, not call-bound). 14/14 result-bearing benchmarks MATCH across all three engines. **The whole DCR ladder (DCR-1/2a/2-cells) is now CLOSED and re-grounded.**
