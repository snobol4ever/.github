**SESSION WATERMARK — 2026-06-13 · Sonnet 4.6 · SCRIP=722ca39. Three fixes landed this session: (1) ✅ `gvar_chain_prebuild_children_text` BFS fix (6e35735) — replaces flat `g->all[]` ARBNO/SCAN scan with reachability-filtered walk from `g->entry`; eliminates phantom `sno_flat_c0_wγ`/`_wω` undefined-symbol crash on Qize DEFINE probe. (2) ✅ Slot-collision fix (722ca39) — `x86_scratch_off` was always 0 (never assigned); now `walk_bb_flat` dispatch calls `bb_slot_claim(N)` and writes `g_emit.x86_scratch_off` before FILL for BREAK(4), BREAKX(8), ARB(8), SPAN(8), SPAN_VAR(16), ARBNO(8=saved+prev), FENCE(4). (3) ✅ `sub REG, FR32` encoder added to `x86_asm.h` (`x86_frame_sub_from_reg`; opcode 0x2B) — BREAK β was silently emitting nothing for `sub eax, dword ptr [r12+N]`. (4) ✅ Deferred-capture injection (722ca39) — `rt_dcap_begin/end_ok/end_fail` added to `pattern_match.c`; `flat_drive_scan_native` emits PLT calls wrapping `flat_drive_match` so COND captures buffer during match and commit/discard on success/fail. REMAINING GAP: Qize probe m4 output is `a.b.c` (should be `b.c`) despite all above — `rt_dcap_end_ok` flushes correctly, `rt_cap_assign_cursor` records correctly under `g_rt_dcap_active=1`. Root cause NOT yet identified; all the dcap wiring appears correct in the assembly (dcap_begin/end_ok/end_fail labels wired, Qize c0 slot at [r12+68]/[r12+72], CAPTURE SAVE at [r12+48], sub fix in BREAK β). UPSTREAM REGRESSION: smoke define test fails m4 (`.Lx2_0 already defined` — duplicate label) — introduced by upstream `14ae014` (FIXUP bb_unify.cpp), NOT by our commits; bisect needed. GATES at SCRIP=722ca39: smoke m4 6/7 (define FAIL — upstream regression) · pat-rung 19/19/19 no-SKIP · fence HARD. Probe: `DEFINE('Qize(S)part') :(e) Qize S ? ((BREAK('.') '.' ARBNO(NOTANY('.'))) . part) RPOS(0) :F(FRETURN) Qize=part :(RETURN) e OUTPUT=Qize('a.b.c')` — links and runs, gives `a.b.c`, should be `b.c`.**
