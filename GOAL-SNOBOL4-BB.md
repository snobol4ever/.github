**SESSION WATERMARK — 2026-06-13 · Sonnet 4.6 · SCRIP=d2d307c. This session (2 commits): (1) POWER_fn int-coercion — `**` was the lone arith op ignoring IS_INT&&IS_INT→INTVAL; int**non-neg-int now returns INTEGER (exact repeated-multiply, not pow-then-truncate), int**neg-int→REAL (2**-1=0.5), 0**0→err; mirrors add/sub (IS_NULL→0, coerce_numeric). All 3 modes route POWER_fn (M2 IR_BINOP→binop_apply, M3/M4 @PLT) so 027 fail/fail/fail→PASS/PASS/PASS. (2) SNOBOL4 keyword reads in M3/M4 — &LCASE/&UCASE/&ALPHABET/&STNO lower to IR_KEYWORD→bb_keyword, which only handled Icon kw (subject/pos/null/fail) gated on g_descr_flat_chain; SNOBOL4 sets g_gvar_flat_chain so FILL(IR_KEYWORD, emit_bb.c:2901) left op_off=-1→'no slot' bomb. Added rt_keyword_read(sval)=strip-&,kw_read,NV_GET-fallback (the exact 2 calls M2's IR_KEYWORD makes → parity by construction); FILL allocs slot+op_sval for g_gvar_flat_chain; bb_keyword guard accepts g_gvar_flat_chain + general arm = bb_var's both-medium lea-name/call/store idiom verbatim (NV_GET_fn→rt_keyword_read). Icon's other kw (date/clock/...) now resolve via kw_read too. GATES: SNO smoke 7/7/7 HARD · pat-rung 19/19/19 no-SKIP · fence HARD · ICON smoke M2 12/12 HARD (M3/M4 10/2 UNCHANGED — shared-box touch safe). CORPUS: M2=182(+1) M3=168(+9) M4=158(+9); M3↔M2 gap 22→14. Keyword fix alone +8/+8 (cleared 081/082/089/097/099/100 +more). NEXT (do-not-re-derive): (a) UNARY-MINUS M3 RUNAWAY — `OUTPUT=-1` prints -1 ~5M× in M3; IR clean (LIT_I→UNOP negate→ASSIGN) but walk_bb_node (emit_core.c:372) emits '; [kind=N unhandled]' for IR_BINOP(7)/IR_UNOP(8)/IR_LIT_F(2) appearing as flat-chain operands → malformed chain loops; 027 (single BINOP stmt) works, multi-stmt/unary-minus/real-lit don't — emitter rung. (b) 084 concat-with-CALL part (S=S bump(..)): gvar_seq_flatten can't flatten a call → 'no parts' bomb; needs the call eval'd to a slot then concat. (c) 091/092 array subscript A<i> assign+access (M2 ALSO fails — all-mode; only ARRAY() create emitted, subscripts dropped). (d) 093 table subscript, 094/095 DATA field-set (095 m3/m4 give 10|20|10 vs 10|20|99 — field write not persisting; 094 m4 compile-abort). (e) bare `OUTPUT = &LCASE` (cset→OUTPUT) aborts in M3 at cset-print — separate pre-existing cset-as-gvar-value gap, surfaced (not caused) by (2); no corpus test hits it. (f) kw_read lacks distinct 'alphabet'/'stno' rows — currently resolve via NV fallback; add exact-value rows if a test needs precise semantics.**
