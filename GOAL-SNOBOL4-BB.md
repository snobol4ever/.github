...
**Benchmark `.s` artifacts (Lon 2026-06-22):** `corpus/benchmarks/<name>.s` is the mode-4 (`--compile`) x86 output kept beside each `<name>.sno`. Any session that touches codegen MUST regenerate them at handoff: `bash scripts/util_regen_benchmark_s_artifacts.sh "<rung>"` — idempotent (deterministic `--compile`, commits only changed bytes); see RULES.md handoff step 4. The `.s` is honest current output: a still-unimplemented shape emits a loud bomb stub into the `.s` (it still assembles) and the file changes once the gap closes.
