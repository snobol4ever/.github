...
# ▶▶▶ NEXT SESSION — START HERE (handoff 2026-06-24, session 19)
...
**▶ NEXT MOVE: FZ-2** — wire `IR_REF_INVARIANT` so `string_pattern` folds to a native blob with no runtime builder call. Two ends: (a) lowerer produces `IR_REF_INVARIANT` (with matcher entry on aux channel) instead of `IR_DTP_ASSIGN`+`IR_PATTERN_CAT`; (b) main walk `case IR_REF_INVARIANT` calls `rt_gvar_assign_pat(name, head)` where head is the pre-pass-cached sealed blob (`child_cache_get` binary; `child_cache_get_lbl` text). The one new x86 helper needed: `x86("load_fn_or_lbl", …)` in `x86_asm.h` that emits `movabs rsi, <fn>` (binary) vs `lea rsi, [rip+<lbl>]` (text) without a `MEDIUM_*` branch in the template. Gate: `string_pattern` mode-3 AND mode-4 output `== .ref`; `bb_build_break_cap_lit_blob` absent from `string_pattern.s`. Prereq reads listed below the STEPS header.
