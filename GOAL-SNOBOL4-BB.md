...
1. ⭐ **ZLEAK-3 — SUBJ_CELL (150 leaks) and `fc_call_ok` (77 leaks). FULLY SPECIFIED AND DE-RISKED BELOW; one premise left to prove before coding.**
   ...
   **⛔ PROVE FIRST (the one blocking premise):** **plan-time entry ≡ emit-time entry, and one graph gets at most ONE choke decision.** `emit_chain` is called per graph EMISSION from several callers (driver loops, `runtime_eval.c`, `bb_pat_build.cpp`; emit.cpp ~2531), and the census shows ~592 choke decisions across 316 programs — so if any single graph is emitted more than once, or with a different `entry`, a single plan-time decision cannot match every emission and the fixpoint mis-decides (mixed regimes inside one graph = the s188 disease). Instrument the choke with a graph id and count decisions per graph BEFORE writing the fixpoint. If the premise fails, the fallback is a tagged registry (mark subject-registered nodes, gate only those at emit.cpp:889/1281) — but note that gating consumption alone risks rsp desync unless every pop/`fc_vwpop` touch point is gated with it.
