...
**CROSSCHECK WATERMARK (SCRIP `3cf0cc89`, committed; RE-DERIVE at session start — never trust this line):** `bash scripts/test_crosscheck_snobol4.sh` mode-3 **241/276 (35 FAIL)**, mode-4 **241/9/26 (9 FAIL, 26 SKIP)**, **DIVERGE=0**. Both former bracket-1 items are now LANDED (see below) — no patch is parked.
...
**BRACKET 1 — FRESH-FRAME + INPUT-EOF — ✅ CLOSED (Claude Sonnet 5, 2026-07-06).** Both halves landed and verified this session:
- **Fresh-frame half (SCRIP `920861ec`):** `bb_match_defer.cpp` now allocates a fresh 64KB ζ frame per deferred-pattern evaluation (`rt_zls_alloc`), released only on the failure branch (mirrors the established `rt_proc_call_gen`/`rt_proc_resume_gen` fresh-alloc + release-on-`IS_FAIL` convention already in `rt.c`) instead of sharing the single persistent `rt_frame()`. Recovers the recursive-grammar cluster (115/120/126/127/131/138/139/146/152, +9 both modes) whose nested `*var`/`*expr` re-entries were clobbering each other's capture/fail-exit slots.
- **INPUT-EOF half (SCRIP `3cf0cc89`) — the bug the fresh-frame patch unmasked, now independently root-caused and fixed:** `NV_GET_fn` (the one by-name getter for every global/keyword read) correctly returns `FAILDESCR` at EOF for the `INPUT` keyword (`core.c:2701 input_read`) and for any `INPUT(.VAR,chan,file)`-bound channel variable (`core.c:2098-2109`) — verified by direct inspection, not assumed. But `bb_var_global.cpp`'s `NV_GET_fn` call site never checked the returned tag, so a `FAILDESCR` was stored and treated as a successful string read; `LINE = INPUT :F(END)` therefore never took `:F` at EOF (a correctness bug against SPITBOL manual ch.5, not just a hang). Fix, two parts (verified the first alone is insufficient): (a) `cmp eax,(long)DT_FAIL; je ω` added right after `call NV_GET_fn`, mirroring the established `bb_idx_get.cpp` array/table OOB idiom; (b) `IR_VAR` added to the chain-BFS ω-discovery condition at all four mirrored sites in `emit.cpp` (`codegen_flat_chain_body` ×2, `emit_chain_operand_refs` ×2) — ω was already wired by the lowerer (`lc_build` threads the ambient fail-continuation for every expression) but never discovered/positioned by the emitter, the identical failure mode the ALT rung hit for the match family. GVA fast path deliberately untouched (INPUT is never GVA-promoted — audit separately if a future session finds an I/O-bound var riding GVA).
- **Verification:** independently reproduced the hang first (two independent minimal repros, both exit 124 pre-fix), installed gdb, confirmed root cause by inspection rather than guessing, fixed, rebuilt, both repros now exit 0 byte-identical to the SPITBOL x64 oracle in both modes. Crosscheck 230→**241/241**, DIVERGE 2→**0**. Zero regressions (precise set-diff both modes, both landings, against pre-session baseline). `.s` artifacts regenerated + committed (SCRIP `f6a550a1`+`0f42ce74`, corpus `5a2023b4`+`6657dc05`).
- **NEW FINDING (not part of this bracket, surfaced by the fix):** `word1`/`word3`/`cross`/`wordcount` no longer hang (all now terminate in ~10ms) but still FAIL for separate, distinct, unrelated reasons — a genuinely different next bracket, not gated on anything above: **word1** (`ARB . OUTPUT (...)`- pattern captures directly into the `OUTPUT` keyword; produces blank lines instead of matched text — capture-into-keyword-assignment is likely unwired, distinct from a plain-variable capture target); **word3** (3-capture chain `BREAK(...).WHEN (...) ARB.WHO (...) REM.WHAT` with the generator `ARB` in the *middle* capture, not the last — produces empty output; may be a residual capture-over-generator edge case beyond what SN4-PAT-CAPTURE-STACK covered, or the multi-capture-with-mid-chain-generator shape specifically); **cross** (wrong output, exit 1, not yet diagnosed past this); **wordcount** (unrelated lowerer FATAL: "expression form not in the landed subset: tree kind 31" — a different construct entirely, nothing to do with INPUT/capture). Each deserves its own static-first stab.

**NEXT BRACKETS (in order):**
1. **DT_P through user functions** (063-066 fence_fn cluster) — a DEFINE'd function returning `FENCE(...)`; DT_P must survive the call/return path and defer must accept call-result patterns.
2. **β-resume ACROSS the blob boundary** (074/W07 cursor cluster) — outer backtrack re-entering a returned blob's internal generator; the resumable-β protocol proper (β-dispatch prologue exists; needs per-invocation state survival — this now rides the LANDED fresh-frame, no longer gated).
3. **word1/word3/cross/wordcount** — the four distinct bugs surfaced above; word1 (capture-into-OUTPUT) and word3 (mid-chain-generator capture) look like the cleanest static-first stabs of the four.
4. **Remaining non-pattern stragglers** — 1011/1013/1015/1017, 082/213, test_case/stack/string (include-library).
