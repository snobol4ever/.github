...
- [/] **M3-NATIVE-4** — per-language bring-up + corpus parity. SNOBOL4 first.
  ...
  - [/] **SBL-M3-NATIVE-4 ARBNO tree-shape foundation 🟡** (Opus 4.7, 2026-05-28, one4all `debb8a4e`). Three files modified, baseline-neutral, ARBNO subtrees now routable through `patnd_to_bb_tree`. **(1) `src/include/BB.h`** — extended `bb_arbno_state_t` with `BB_t **kids; int nkids;` at the FRONT; layout-compatible with `bb_pat_kids_state_t` for its first two fields. `bb_exec.c` reads `inner/pos_stack/cap/saved_delta` by name, unaffected by offset shift. Legacy γ-chain XARBN leaves kids=NULL/nkids=0 (zero-init) — `bb_pat_nkids` returns 0 there, legacy consumers (mode-2 bb_exec, mode-3 brokered via `nd->α`) never read kids. **(2) `src/lower/lower_pat_dcg.c`** — new `case XARBN:` in `build_patnd_tree` recursively builds inner via tree path (so combinator inners like `ARBNO('a'\|'b')` in rung 054 get tree-shape), allocates one `bb_arbno_state_t` populating BOTH kid sidecar AND inner BB_graph_t for mode-2 bb_exec. **(3) `src/runtime/snobol4/stmt_exec.c`** — `patnd_tree_eligible` removes the blanket `patnd_contains_arbno → 0` rejection, adds explicit XARBN case recursing into inner's eligibility; `patnd_is_combinator_root` accepts XARBN as root. **Behavior-neutral on every baseline**: G1=13/13 default+native, G2=38, G3=162/82 (≡HEAD `10f97d29`, PLAN's 175 figure is stale), G4=237/43, native broad 157/280, rungs M2=19/M4=14 — verified via `git stash` round-trip against the same scripts. **052/054 no longer segfault under `SCRIP_M3_NATIVE=1` but now abort at `bb_emit_end: 1 unresolved forward reference(s): site=19 label='pat_brok_β'`.** **DIAGNOSIS COMPLETE (env-gated fprintf traces, all reverted):** the nested brokered build of the ARBNO node ("session 2") emits a 10-byte body matching exactly `bb_arbno.cpp` line 41's `!cfn` fallback (`bin = {{1,6},{γ_p,ω_p},{false,false}}`), NOT the 259-byte standard arm that would have `is_def=true` for β at site 186. The standard arm gates on `child_lbl` (line 18-19: `if (!child_lbl || !child_lbl[0])`) but `g_emit.bb_child_lbl` is set only inside the `MEDIUM_TEXT` branch of `bb_prepare_capture_arbno` (emit_bb.c:618) — in MEDIUM_BINARY it's stale/empty, so the early-return fires even when `bb_child_fn != NULL`. **Fix sketch for next session:** in `src/emitter/BB_templates/bb_arbno.cpp`, replace the `child_lbl` gate with a medium-aware check: `int have_child = MEDIUM_BINARY ? (g_emit.bb_child_fn != NULL) : (child_lbl && child_lbl[0]);`. Same pattern likely affects `bb_capture.cpp` (BB_PAT_ASSIGN_COND/IMM). After fix, 052 should yield `aaa` and 054 should yield `abba` natively; expect rung M4 14→16+ and native broad 157→up. Handoff `HANDOFF-2026-05-28-OPUS-SBL-M3-NATIVE-4-ARBNO-TREE-FOUNDATION.md`.
