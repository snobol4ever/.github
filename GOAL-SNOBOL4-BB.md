...
# ▶▶▶ NEXT SESSION — START HERE (session 26, 2026-06-25)

**DCR-2a + DCR-2-cells LANDED (SCRIP `3b30fbb` + `d9dc34b`, session 25).** Runtime-only proc-call optimization in two slices. (a) DCR-2a: name-ptr→index cache in `rt_proc_find` (direct-mapped 2048-slot, index not pointer — realloc-safe). O(1) lookup for multi-proc programs. (b) DCR-2-cells: new fields `pcells`/`rcell`/`cells_done` on `rt_proc_t`; `rt_proc_resolve_cells()` memoizes each param's and the return variable's GST `DESCR_t*` cell once on first call; `rt_name_save_push` gains optional `cells[]` so callers with precomputed cells skip `rt_cell_for` entirely. After warmup: per-call save/restore is pure `*cell` ops — zero hash lookups, zero `strcmp`, zero `rt_cell_for` cache probes. Both slices behavior-neutral. Gate: crosscheck PASS=171 FAIL=84, fail set byte-identical to pristine HEAD (zero regressions, both slices verified). A/B func_call (10M calls): DCR-2-cells ~6435ms vs DCR-2a ~6588ms (~2.5% further). Cumulative from pristine: 17174ms → 6435ms (~2.67×). Honest: DCR-1 took the big win (GST hash-walks → 2.6×); DCR-2 removes residual cache-hash overhead. Bigger wins remain: SAB (string_concat 8.8×, string_manip 11.3×), AXS (table_access 8.8×).

**▶ NEXT MOVE (session 26): DCR-3 (re-run 3-way comparison, update func-call ratios in watermark), then SAB or AXS — Lon decides.**
...
- [ ] **AXS-0** — detect array-with-int-index access in `lower_snobol4.c` (distinct from table).
- [ ] **AXS-1** — emit inline bounds-check + `[base+idx*16]` arm.
- [ ] **AXS-2** — gate: crosscheck byte-identical; **out-of-range semantics MUST match SPITBOL exactly** (FAIL vs error).
