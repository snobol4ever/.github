...
**Your first move = the last TWO edits of the LEN crux (the 3rd is already LANDED). Target: `PAT = LEN(2); S ? PAT` runs == oracle.** GATE AFTER EACH (build green · 6/6 PB-GREEN byte-identical · the rung == `sbl -b`):
- ✅ **(was step 3) DEFER SIDE LANDED — SCRIP `447892b`.** `rt_defer_match` now has a `DT_P` branch (`pattern_match.c`) that calls the built head as **`int head(const char *subj /*rdi*/, int cur /*esi*/, int len /*edx*/)` → new cursor | -1**, passing the live globals `Σ/cur_delta/Σlen`. **This FIXES the head ABI for G0** (sanctioned C-callable dispatched-BB form; subject via args, NOT live r13/r14/r15 — that sidesteps the register-liveness trap). The matcher you emit MUST be exactly this signature. Dormant until step 2 produces a `DT_P` head. (Later: unify to the `bb_box_fn(zeta,entry)→DESCR` flat form + capture; for G0 this int-ABI is the contract.)
1. **Emit a self-contained relocatable LEN matcher** = a sealed, labeled x86 blob with the C-callable ABI above. For `LEN(I)` the body is TINY and fully position-independent (no data, no external call — matches any I chars): `lea eax,[esi+I]` · `cmp eax,edx` · `jg .fail` · `ret` · `.fail: mov eax,-1` · `ret` (the −1 sentinel = fail; on success eax = cur+I = new cursor). I is the node's `ival`, baked as an immediate. Emit it behind a label, skipped at builder-exec time by a `jmp over` (so the bytes sit inline as copy-source but don't execute in place). Get its byte length L via `emitter_end()` delta (`bb_emit_pos`/`g_emit_pos`) bracketing the emit — or, since this sequence is fixed-size, a verified constant. BOTH-MEDIUM (binary + text) — emit via `x86()` so both arms come from the encoders.
2. **IR_PATTERN_LEN builder** — replace the `bb_pattern_unary_i` stub at `emit_core.c:370`: after emitting the sealed matcher (step 1) at label `M`, the builder (which is NOT relocated — runs in-image, normal addressing) emits `lea rdi,[rip+M]; mov esi,L; call pat_pool_emit` → head in rax; stash head (frame slot or straight into the DTP build). **IR_DTP_ASSIGN** (`emit_core.c:386`): build `DESCR_t{.v=DT_P,.p=head}`, `NV_SET_fn("PAT",descr)`. Now `PAT=LEN(2);S?PAT` → builder relocates a fresh LEN matcher into pat_pool every execution (variant ✓), stores DT_P; the landed defer branch calls it → cursor advances 2 → match. Gate; should == oracle.
Then, in order, GATING EACH: **BREAK** (`IR_PATTERN_BREAK` @ `emit_core.c:368` — first data-bearing box: seal the cset ADJACENT, reach `[rip+disp]`, `call strchr` absolute) → **CAPTURE** (`IR_PATTERN_CAPTURE` @ `:384`, makes r2 ⇒ `W=alpha`; this is where the int-ABI must grow to carry the matched span — likely widen the head ABI or add a capture sink) → **CAT** (`IR_PATTERN_CAT` @ `:382`, needs a pat_pool *stitch* primitive beside `pat_pool_emit` — patch one pool blob's port to another pool entry, REUSING the `flat_drive_match` port-patch shape, do not invent a second stitch). Then TR-4: roman/mixed_workload/string_pattern green at G0.
