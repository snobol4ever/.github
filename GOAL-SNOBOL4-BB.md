...
**SESSION 31 CLOSE (2026-07-03) — ▶ NEXT SESSION START AT RL-2.** Landed: emit_chain consolidation (FACT RULE above), RL-0/RL-1, unused-IR sweep (enum 60). RL-2 opening moves in order: (1) `scrip.c:609` — route SNOBOL4 through `ir_drive_slot_assign` (TMP-ERADICATE grants) instead of legacy `ir_tmp_slot_assign`; (2) mode-4 SNOBOL4 branch: replace the `gva_collect_graph` stub call with zero-collect (GST(NV)-first — n_gva=0, names ride NV) or a real collector — Lon's call; (3) re-lower assignment+OUTPUT in `lower_snobol4.c` onto `IR_ASSIGN`/NV (DT_N NAMEVAL association prints), replacing the IR_OP_COUNT placeholders. Gate: `hello/`, `output/`, `assign/` green both modes, zero Icon regression (209/44/36 anchor).
### Steps
- [x] **RL-0 — baseline recorded.** Numbers above; bomb sites + gutting mapped; one-emitter entry identified. (This session, pre-code.)
- [x] **RL-1 — driver reroute onto the one emitter + entry consolidation.** DONE (session 31, Lon directive 2026-07-03 — merged with the emit_chain consolidation, see FACT RULE above). Gate MET on mode-3: `hello.sno` now dies LOUD+NAMED inside the one emitter — `FATAL drive_value_slot: IR op=29 (IR_LIT_STRING) is a value-producer with no nd->tmp; add the grant in LOWER` — the GROUND-ZERO stub death is gone. ROOT CAUSE LOCATED for RL-2: `scrip.c:609` routes non-Icon through legacy `ir_tmp_slot_assign` while `emit_chain` (TMP-ERADICATE) requires `ir_drive_slot_assign` grants. Mode-4 dies one stub earlier in `gva_collect_graph` (driver GVA-preamble collection, NOT an emit entry; GST(NV)-first SNOBOL4 may legitimately collect ZERO and go by-name — RL-2 design call). Regression gates: Icon smoke 12/12 ×2; Icon full corpus PASS=209 FAIL=44 XFAIL=36 (matches documented baseline exactly); crosscheck counts unchanged (5/256, SKIP 261) as expected pre-RL-2. **RESIDUE — RESOLVED (Lon 2026-07-03, same session):** (a) the dormant-arm template FILES are KEPT — Lon: do NOT delete template files, they may be reused later; `g_gvar_flat_chain` (still never set — arms stay dormant) and `CALL_ROUTE_GVAR_USERPROC`/`bb_call_gvar_userproc_str` stay as those kept templates' compile dependencies. EMITTER PRINCIPLE recorded (Lon): emitter code is language-INDEPENDENT — per-language action belongs in LOWER (`lower_snobol4.c` beside `lower_icon.c`), so any future re-light happens by LOWERING onto live kinds, never by resurrecting per-language emitter paths. (b) the GVAR-named IR kind is DELETED with the unused-IR sweep (see Constraint above) — no rename tension remains.
- [ ] **RL-2 — re-lower assignment + OUTPUT.** Plain assignment → `IR_ASSIGN`/`IR_ASSIGN_VAR` NV arm; `OUTPUT = x` prints via the DT_N NAMEVAL association. Gate: `hello/`, `output/`, `assign/` green both modes.
- [ ] **RL-3 — concat + arith.** Value concat, `IR_BINOP`/`IR_UNOP`/`IR_BINOP_TEST`/`IR_UNOP_TEST`, `IR_LIT_*`. Gate: `concat/`, `arith/`, `arith_new/` green.
- [ ] **RL-4 — statement control flow.** `:S(L)/:F(L)/:(L)` → `IR_GOTO`/`IR_INDIRECT_GOTO`/`IR_MOVE_LABEL` + per-statement γ/ω wiring. Gate: `control/`, `control_new/` green.
