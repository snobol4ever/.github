...
**CROSSCHECK WATERMARK (SCRIP `6e6d6d18`, committed LOCAL/UNPUSHED — credential pending; RE-DERIVE at session start — never trust this line):** `bash scripts/test_crosscheck_snobol4.sh` mode-3 **250/276 (26 FAIL)**, mode-4 **250/9/17 (9 FAIL, 17 SKIP)**, **DIVERGE=0**.
...
1. **132_pat_fence_eps_recur_shallow** — the DEFINE-missing-entry-label bug just above; looks like the cleanest next stab (small, isolated, well-described).
2. **FENCE-in-ARBNO-body** (116/142/145/151) — real design work, see the finding above; do this as its own focused rung, verify 151 (the discriminator) still gives the SEALED/GOOD answer, not just that it compiles.
3. **062/063 (capture) — `SUBJECT PATTERN = REPL` splice.** Explicitly out of scope per the existing FATAL ("match-only for now"): stages 4-5 of the 5-stage model (build replacement + splice it into the subject) have no implementation at all yet. Real, scoped, well-understood new feature — not a wiring gap.
4. **word1/word3/cross/wordcount** — the four distinct bugs from the previous session's bracket-1 fix (still unfixed, still distinct: word1 = capture-into-OUTPUT-keyword; word3 = mid-chain-generator capture; cross = wrong output, exit 1, undiagnosed past that; wordcount = "expression form not in the landed subset: tree kind 31", an unrelated parse/lower gap).
5. **1011/1013/1015/1017/213/082** — `func_redefine`/`func_nreturn`/`opsyn`/`arg_local`/`indirect_name`/`keyword_stcount`; 082/213 are long-standing pre-existing baseline items per earlier sessions, not yet root-caused.
6. **124/143 ("regex")/140/141/132's-family (132 itself now #1 above)** — `EVAL`-adjacent and compound-pattern corpus probes, not yet individually triaged this session.
7. **test_case/test_stack/test_string** (corpus `lib/case.sno`/`stack.sno`/`string.sno`, include-library) — likely depend on some of the above (OPSYN, splice) landing first; re-check after 3 and 5.
