...
**Mandate (Lon 2026-06-19 PIVOT):** get every SNOBOL4 benchmark in `/home/claude/corpus/benchmarks/snobol4/*.sno` WORKING — runs to completion in mode-4 (`--compile`) AND result line matches the sbl oracle. This RUNG is FIRST; do its first incomplete STEP before any other rung in this file.
...
- [x] **PBG-2 — ✅ DONE (Claude 2026-06-22, SCRIP `e789f1e` + corpus `e9130815`). table_access GREEN → bench 5→6/16.** Foundation (by-name TABLE int/var-key read+write) plus clause D both landed. D1 BINOP-value write (`T<I>=I*2`): `lower_expr` gained an IR_IDX operand-box case; `lower_stmt_body` routes `T<k>=<arith>` to IR_IDX_SET with the value chained as a pre-walked slot operand; `bb_idx_set` reads `op_sc` as the BINOP result slot (DT_I-tagged). D2 mixed dynamic arith (`SUM=SUM+T<I>`): `bb_arith_is_dynamic` now fires when ≥1 operand is IR_IDX/CALL and the other is materializable; a dedicated gvar-arith branch pre-walks IR_IDX children to DESCR slots (`gvar_prewalk_idx_operand`) and the descr arith template materializes by-name VAR (NV_GET)/LIT_I operands into DESCR scratch before `rt_num_arith` — all DESCR-pure. Orphan-bail narrowed via `sno_arith_idx_lowerable`. Probes vs sbl: `T<I>=I*2;OUTPUT=T<I>`→6, `SUM=SUM+T<3>`→100, `T<3>+T<3>`→100, table_access→250500 (==ref). Gates non-decreasing: smoke M3 7/7 M4 7/7 · pat-rung M4 19/19 0-SKIP · fence T1=T2=0 · broad-corpus M4 170 (≥169) · prior-5 unregressed. `corpus/benchmarks/table_access.s` regenerated+gcc-verified+committed. FOLLOW-UP (do-not-re-derive): the one-dynamic-operand descr materialization now also covers `F(x)+1`/`N+F(x)` shapes (the long-standing PBG-1 follow-up) since `bb_arith_is_dynamic` + the materializing arm generalize beyond IDX — not in any remaining bench but available when next touched.
...
# ⛔ NEXT-PRIORITY RUNG (after PB-GREEN) — REC-COV · BROADEN COVERAGE TO COMMUNITY-RECOGNIZED CORPORA

**Mandate (Lon 2026-06-22):** the 16 `corpus/benchmarks/snobol4/*.sno` (namespaced under `snobol4/` by the 2026-06-22 reorg) are PROJECT-AUTHORED microbenchmarks (each carries a `* Bottleneck:` author note; `corpus/BENCHMARKS.md` frames them as a cross-engine self-comparison rig). They are a legitimate correctness/regression probe set but they are NOT a community-recognized SNOBOL4 benchmark. The SNOBOL4/SPITBOL community ships no standard *performance* suite — its gold standard is CORRECTNESS + self-host (`spitbol/x64` `./sanity-check` rebuilds SPITBOL 3×; `./test`; CSNOBOL4 tests). The recognized *program* corpora are Gimpel's *Algorithms in SNOBOL4*, the CSNOBOL4 test suite, Griswold's "Green Book" programs, and the SPITBOL-manual demos. This RUNG extends scrip's measured coverage into those corpora so the "10×" claim (PLAN.md, `bench/BENCHMARKS.md`) can stand on programs a SNOBOL person recognizes, not on self-authored microbenchmarks. **PB-GREEN stays session-first; do REC-COV after the PB-GREEN step list, or when Lon names it.**

**GROUND TRUTH — MEASURED 2026-06-22 (Claude; inventory only, pass-rates NOT yet measured — that is RC-0's job, do NOT fabricate a number before RC-0 runs):**
- `corpus/programs/gimpel/` — **145** `.sno` (Gimpel, *Algorithms in SNOBOL4*). No `.ref` shipped → oracle-generate.
- `corpus/programs/csnobol4-suite/` — **124** `.sno`, **each with a shipped `.ref` expected-output file** (verified: `100func.ref`, `8bit.ref`, `a.ref`, `alis.ref`, `alph.ref`, `alt1/2.ref`, `any.ref`, …) + `README.md`. This is a ready-made, oracle-verified community correctness suite — the MOST tractable target (refs already present, no oracle-gen needed).
- `corpus/programs/snobol4/demo/` — **18** `.sno` (SPITBOL-manual demos).
- Existing `test_mode4_only_corpus_snobol4.sh` targets `corpus/crosscheck` + `demo` + `beauty_suite` ONLY — it does NOT cover `gimpel/` or `csnobol4-suite/`. So both are genuinely UNGATED today.

**DEPENDENCY:** REC-COV is mostly a MEASURE-then-GATE layer riding on the PB-GREEN correctness fixes. The failure buckets the recognized corpora hit are expected to be the SAME ones PB-GREEN already enumerates (SCAN-NONLIT-M4 / EVAL-CHURN / TABLE-M4 / PERF) plus corpus-unique gaps surfaced at RC-0. Land the PB-GREEN fixes first; REC-COV closes residuals and locks the coverage in as a floor.

**GATE (this rung, HARD non-decreasing EVERY step):** the recognized-corpus oracle-match count strictly grows; all PB-GREEN floors hold unchanged — smoke M3/M4 7/7 · pat-rung M4 19/19 0-SKIP · fence TIER1=TIER2=0 · broad-corpus M4 ≥170 · the 16-bench genuine-green count never drops. Baseline for the recognized corpora = **UNMEASURED until RC-0** (establish it there, then it becomes the floor).

## STEPS — measurement first, then drive coverage, then gate + report. Each = ONE commit · floors green.

- [ ] **RC-0 — HONEST RECOGNIZED-CORPUS RUNNER + TRIAGE (no codegen; the PBG-0 analog).** Add a drop-in runner (mirror the PB-BENCH-0 truthful-runner pattern: hi-res wall, CRASH = nonzero-rc OR empty-stdout OR `BOMB` on stderr, result-line diff EXCLUDING any `ms:` line) that for `csnobol4-suite/` diffs scrip-M4 output against the **shipped `.ref`**, and for `gimpel/` + `snobol4/demo/` oracle-generates `.ref` from `sbl -b` first (commit the refs to corpus), then diffs. Emit a per-program triage table classifying each as green/bomb/wrong/timeout and mapping every failure to a CAUSE BUCKET (reuse PB-GREEN's: SCAN-NONLIT-M4 · EVAL-CHURN · TABLE-M4 · PERF · plus any new bucket found). GATE: runner exists, prints honest OK/CRASH/FAIL counts per corpus, exits non-zero on any crash; the three baselines (gimpel/csnobol4-suite/demo oracle-match counts) are RECORDED here and become the non-decreasing floor. ⚠️ Do NOT count run-to-completion as correct — diff against the ref (the PB-BENCH lesson: the old no-ref runner scored crashes as PASS).
- [ ] **RC-1 — csnobol4-suite GREEN-DRIVE (refs already present → start here).** From RC-0's triage, order the cause buckets by program-count and close them highest-frequency-first; most will already be cleared by the corresponding PB-GREEN fix (scan/table/eval), so this step is largely: rebuild, re-run, confirm the count climbed, fix residual suite-unique shapes. Probe each newly-green program m4≡its shipped `.ref`. GATE: floors hold; csnobol4-suite oracle-match count strictly up.
- [ ] **RC-2 — gimpel GREEN-DRIVE (*Algorithms in SNOBOL4*).** Same procedure against the oracle-generated refs from RC-0. Gimpel exercises heavier data-structure / recursion / sort idioms (STACK, HSORT, BSORT, TREEREAD, PERMS, POKER, …) — expect TABLE-M4 / DEFINE / DATA buckets to dominate; coordinate with PB-GREEN PBG-2-family and the S7 M4-DATA cluster. GATE: floors hold; gimpel oracle-match count strictly up.
- [ ] **RC-3 — demo GREEN-DRIVE (SPITBOL-manual programs).** Same against oracle refs. Smallest set (18); good final correctness sweep. GATE: floors hold; demo oracle-match count strictly up.
- [ ] **RC-4 — WIRE RECOGNIZED-CORPUS FLOORS INTO THE GATE SET.** Promote the three RC counts to HARD non-decreasing floors in the goal's Session-Setup gate block (alongside smoke/pat-rung/fence/broad-corpus), so coverage of recognized programs is a tracked, ratcheted metric — not a one-off measurement. GATE: gate block updated; all floors (old + new) green on a clean build.
- [ ] **RC-5 — RE-GROUND THE "10×" CLAIM ON RECOGNIZED PROGRAMS.** Run the green recognized programs (the ones that complete correctly) under both `sbl -b` and scrip M4; report honest per-program ratios in `bench/BENCHMARKS.md` as a SEPARATE, clearly-labeled "community-recognized corpus" table — distinct from the self-authored 16-microbenchmark table — so the speed claim is defensible to a SNOBOL audience. State the methodology (timed body only, startup excluded, same host). GATE: ratios recorded for every green recognized program; no correctness/coverage regression.

---

**SESSION WATERMARK — 2026-06-22 (benchmarks namespaced) · Claude · corpus + SCRIP scripts + `.github` (NO codegen, behavior-neutral; the 16 `.s` artifacts moved as-is, no regen). REORG: the flat SNOBOL4 benchmark set moved `corpus/benchmarks/*` → `corpus/benchmarks/snobol4/` (55 files: 16 `.sno`, 15 `.ref`, 16 `.s`, 1 `.xfail`, the `testpgms*` SPITBOL test programs, README) via `git mv`, so the benchmark root is now per-language (mirrors `corpus/programs/<lang>/`; future `benchmarks/icon/` etc.). Repointed 3 SCRIP scripts: `test_bench_snobol4_modes.sh` (BENCH_DIR default), `util_regen_benchmark_s_artifacts.sh` (BENCH default + comment), `test_crosscheck_jvm_artifact_check.sh` (ROMAN path) — verified each resolves to the new dir (16 `.sno` present). Updated active docs only (historical watermarks left intact): PB-GREEN mandate path, REC-COV mandate path, Session-Setup `.s`-artifact note, RULES.md handoff step 4 path, and benchmarks README (per-language layout + path note). ⚠️ CROSS-REPO FOLLOW-UP (out of this session's repos): the benchmarks README documents that sibling repos `snobol4jvm` and `snobol4dotnet` consume this corpus as a Git submodule and load `benchmarks/*.sno` directly — their benchmark runners need their load path updated to `benchmarks/snobol4/*.sno`. Gates NOT re-run live (scrip not built this session; mechanical move + path-only edits, verified by path resolution). NEXT: continue PB-GREEN (PBG-3 non-literal scan) on the 16, now at `corpus/benchmarks/snobol4/`.**

**SESSION WATERMARK — 2026-06-22 (REC-COV authored) · Claude · `.github` ONLY (doc-only; no SCRIP/corpus change, no codegen → no `.s` regen). Authored NEXT-PRIORITY RUNG REC-COV (steps RC-0..5) to broaden coverage from the 16 project-authored microbenchmarks into community-recognized corpora. INVENTORY MEASURED (not pass-rates): `corpus/programs/gimpel/` 145 `.sno` (no refs), `corpus/programs/csnobol4-suite/` 124 `.sno` WITH shipped `.ref` files (ready-made oracle-verified suite — RC-1 starts here), `corpus/programs/snobol4/demo/` 18. Confirmed `test_mode4_only_corpus_snobol4.sh` covers `crosscheck`+`demo`+`beauty_suite` only → gimpel + csnobol4-suite are UNGATED. REC-COV rides on the PB-GREEN correctness fixes (same SCAN/EVAL/TABLE/PERF buckets) and is a measure-then-gate-then-report layer; PB-GREEN remains session-first. NEXT: continue PB-GREEN (per Lon, the 16-bench set for now); pick up REC-COV at RC-0 (honest runner + triage, no codegen) when coverage breadth is the named goal. Pass-rates for the recognized corpora are deliberately UNMEASURED here — RC-0 establishes the baseline; do not fabricate one earlier.**

**SESSION WATERMARK — 2026-06-22 (later) · Claude · SCRIP `e789f1e` + corpus `e9130815` + `.github`. PBG-2 ✅ CLOSED — table_access GREEN, bench 5→6/16.** Clause D landed on top of the foundation. **D1** BINOP-value table write (`T<I>=I*2`): `lower_expr` gained an `IR_IDX` operand-box case (builds IR_IDX with base/key operands, returns it as a value box); `lower_stmt_body`'s write arm routes `T<k>=<arith>` to `IR_IDX_SET` with the value lowered via `lower_expr` and chained BEFORE the set so it deposits an int into its slot; `flat_drive_idx_set` pre-walks the BINOP value box to that slot; `bb_idx_set` gained a `bb_rk==IR_BINOP` arm reading `op_sc` as the raw-int result and re-tagging `{DT_I,val}`. **D2** mixed dynamic arith (`SUM=SUM+T<I>`): `bb_arith_is_dynamic` rewritten — fires when ≥1 operand is IR_IDX/CALL-kind AND the other is materializable (CALL/IDX/LIT_I/VAR-by-name), via new `bb_arith_dyn_kind`/`bb_arith_materializable` helpers; a dedicated gvar-arith dispatch branch (before the generic ARITH_SLOT arm) pre-walks IR_IDX children to DESCR slots (`gvar_prewalk_idx_operand`, mirrors `gvar_drive_call_arg_slots`), claims scratch for non-slot operands, sets `op_arith_descr=1`; the descr arith template (`bb_binop_gvar_arith_slot` first arm) now MATERIALIZES a by-name VAR (lea+NV_GET→slot) or LIT_I (movabs DT_I+imm→slot) operand into its scratch DESCR before `rt_num_arith` — all DESCR-pure. Orphan-bail for arith-with-idx narrowed via `sno_arith_idx_lowerable`+`sno_idx_operand_ok` so the supported `T<VAR|LIT_I>` shape lowers normally instead of bailing. **5 sites:** lower_snobol4.c (lower_expr IR_IDX case + write-arm ARITH value + sno_arith_idx_lowerable/sno_idx_operand_ok helpers + narrowed orphan-bail) · emit_bb.c (bb_arith_is_dynamic rewrite + gvar_prewalk_idx_operand + dynamic-arith dispatch branch) · bb_binop_gvar_arith_slot.cpp (descr-arm operand materialization) · bb_idx_set.cpp (BINOP-value arm) · flat_drive_idx_set (BINOP value pre-walk). **Probes vs `x64/bin/sbl -b`:** `T<I>=I*2;OUTPUT=T<I>`→6 · `SUM=SUM+T<3>`→100 · `T<3>+T<3>`→100 · foundation `T<3>=99;OUTPUT=T<3>`→99 · table_access→250500 (==`.ref`). **GATES (verified, non-decreasing):** bench table_access OK with real `ms:` · smoke M3 7/7 M4 7/7 · pat-rung M4 19/19 0-SKIP · fence T1=T2=0 · broad-corpus M4 **170** (was ≥169) · prior-5 unregressed. `corpus/benchmarks/table_access.s` regenerated via `util_regen_benchmark_s_artifacts.sh`, gcc-verified, committed (`e9130815`; only table_access.s + a determinism artifact mixed_workload.s changed). **FOLLOW-UP (do-not-re-derive):** the D2 generalization means one-dynamic-operand mixed arith (`F(x)+1`, `N+F(x)`) is now also covered (the long-standing PBG-1 follow-up) — not exercised by any remaining bench but live if next touched. **NEXT (recommended order):** PBG-4 eval_fixed — the uncommitted `runtime_eval.c` 1-entry source→chain cache (`CONVE_fn`) noted in the 2026-06-21 watermark may already be in a prior snapshot; check origin, then it greens eval_fixed (same `'X + 1'` 1M×) cleanly (+1). Then PBG-3 non-literal scan (roman/mixed_workload/string_pattern, +3, largest), PBG-5 perf (var_access/func_call/func_call_overhead/string_manip, folds OPSINGLE), PBG-6 indirect_dispatch xfail bookkeeping. **ENV:** build_scrip + `make libscrip_rt`; oracle `git clone …/x64 /home/claude/x64`, `sbl -b`; tri-probe = `scrip --compile p.sno | gcc -no-pie - -Lout -lscrip_rt -lgc -lm -Wl,-rpath,$PWD/out -o p && ./p` vs `sbl -b p.sno`.**

**SESSION WATERMARK — 2026-06-22 · Claude · SCRIP `25635f2` + corpus `f952718d` + `.github`. TWO THINGS LANDED & PUSHED.** (1) **PBG-2 FOUNDATION** — the 13-site by-name TABLE int/var-key read+write is wired; `T=TABLE(16);T<3>=99;OUTPUT=T<3>`→`99` (==sbl) in M4. Gates non-decreasing: smoke 7/7/7 · pat-rung M4 19/19 0-SKIP (M3 15) · fence 0/0 · 5 prior-green benches (arith_loop/op_dispatch/pattern_bt/string_concat/fibonacci) unregressed. Scope = base=VAR, key/value=VAR|LIT_I. THE design-correcting find (do-not-re-derive): by-name templates may read ONLY `walk_bb_node`-surviving fields (`bb_lk`/`bb_rk`/`bb_li`/`bb_ri`/`op_name1`/`op_name2`/`op_parts_lbl`/`op_parts_str[2]`/`op_sa`/`op_sb`/`op_sc`/`op_off`) because emit_core.c:327-339 resets op_node_kind/op_ival/op_sval/op_a_* from the node just before dispatch — the drafted bb_idx_get's use of op_node_kind for the KEY was the bug. Sinks = `subscript_get`/`subscript_set` (proven-correct; `rt_table_idx_*` dups stay retired). table_access NOT yet green — needs **clause D** (`T<I>=I*2` BINOP-value write + `SUM=SUM+T<I>` mixed arith); the foundation does not cover those, so bench count UNCHANGED at 5/16. NEXT = PBG-2 clause D (D1 BINOP value box→slot read by bb_idx_set; D2 extend `bb_arith_is_dynamic` to IR_IDX + materialize the gvar operand — the PBG-1 follow-up) → table_access→250500 (+1). (2) **BENCHMARK `.s` ARTIFACT PROCEDURE** (Lon directive) — new `scripts/util_regen_benchmark_s_artifacts.sh` emits `corpus/benchmarks/<name>.s` (mode-4 `--compile`) beside each `.sno`, gcc-`-c`-verifies, and commits to corpus ONLY changed bytes; idempotent (deterministic `--compile` → re-run = no commit, verified). All 16 artifacts committed (corpus `f952718d`). Wired into HQ: RULES.md handoff step 4 (regenerate on any codegen-touching session) + this goal's Session Setup note. Mirrors the existing `util_regen_demo_s_artifacts.sh` pattern.**
...
**Benchmark `.s` artifacts (Lon 2026-06-22):** `corpus/benchmarks/snobol4/<name>.s` is the mode-4 (`--compile`) x86 output kept beside each `<name>.sno`. Any session that touches codegen MUST regenerate them at handoff: `bash scripts/util_regen_benchmark_s_artifacts.sh "<rung>"` — idempotent (deterministic `--compile`, commits only changed bytes); see RULES.md handoff step 4. The `.s` is honest current output: a still-unimplemented shape emits a loud bomb stub into the `.s` (it still assembles) and the file changes once the gap closes.
