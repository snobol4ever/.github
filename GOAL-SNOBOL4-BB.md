...
- [~] **ED-6 — extract the remaining clean IR-query helpers (PARTIAL — 4 of N landed 2026-06-27, SCRIP `8633674f`).** ✅ LANDED this pass into `src/opt/ir_query.{c,h}`: `gz_node_bounded`, `descr_chain_terminal`, `ir_is_generator_kind` (its forward decl ~1992 deleted; header replaces it), `subchain_node_is_generator` (ordered after `ir_is_generator_kind` since it calls it). All re-verified pure (zero g_emit/file-static) + exclusively owned (no cross-file de-static collision). emit_bb.c 4189->4150 (-39; cumulative ED-5+6 = -77). Crosscheck byte-identical to baseline, benchmark .s `No changes`. **STILL OPEN (next pass):** `resolve_ite_entries_em` (emit_bb.c ~861), `gvar_chain_*` resolve/stno helpers, and `is_pat_chain_elem` (pure but has TWO forward decls + 11 call sites — its own sub-step, delete BOTH decls, let the header replace them; do NOT bundle). ⚠ RE-VERIFY each remaining candidate's purity (grep `g_emit`/file-static) before moving — emit_bb.c keeps shifting. Method = ED-1/5/6's exactly (dual-list Makefile — already done for ir_query, so remaining moves into the SAME files need NO new Makefile line; crosscheck byte-identical; .s byte-identical).
