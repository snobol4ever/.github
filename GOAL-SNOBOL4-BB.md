...
- [ ] **M3-NATIVE-1 — audit the native-SM emitter.** Map `codegen_sm_x86` BINARY-medium coverage: which SM opcodes have a working BINARY arm vs TEXT-only vs stub. Map `sm_image` API (alloc/seal/entry). Determine how far an SM program can be emitted to bytes + sealed today. Output: a coverage table (SM opcode × {TEXT,BINARY,stub}).
- [ ] **M3-NATIVE-2 — minimal native-SM runner.** Build `sm_run_native` (or revive `sm_jit_run`/`sm_run_linear` per ARCH naming): `codegen_sm_x86`(BINARY) → `sm_image` → `seg_seal` RX → call entry. Wire `scrip.c` mode_run to use it instead of `sm_interp_run`. Start with the SMALLEST program that has full BINARY opcode coverage (e.g. a single OUTPUT or assignment). Gate: that program produces identical output to mode-2.
