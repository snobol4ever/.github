...
**⭐ ZTOS-2 LANDED SAME SESSION (SCRIP `e064482e`) — ONE LINE, +37 BOTH ARMS.** The blocker named above was discharged immediately: `x86_frame_off`, THE one offset function, gave the rsp whole-graph arm (`x86_zop` arm 4) only `op_flat_disp` — LOWER's STATIC distance from rsp to the flat frame base. The box's own alpha carve moves rsp DOWN by K AFTER that distance was computed, so every such reference read K bytes too low. Adding `op_zdepth` beside `op_flat_disp` is the identical ZTOS law on the frame arm instead of the spine. **m3 214→251, m4 211→248, DIVERGE=3 flat.** Arms 1/3 (island r12, pinned rbp) are depth-IMMUNE and deliberately excluded — compensating a base that does not move is the FRQ/FRQB double-add class. This discharges the s21x-n cursor's NEXT(1) ("op_flat_disp must become the live authority as carves deepen the stack") and generalises the law: **a box compensates for exactly what IT carved, whichever base its references name.**

**⛔ THE SPINE ARMING HAS A SECOND BLOCKER (re-measured after ZTOS-2, still open).** With arm 4 live, `_spine = 0` was retried: **m3 251→192, m4 248→191** — much better than the pre-ZTOS-2 collapse to 108, so ZTOS-2 removed one of the two causes, but a second remains and is NOT yet identified. Reverted; the exclusion stands. ⚠ DO NOT re-run this experiment blind — it has now been measured twice with two different results, and the next session's job is to NAME the residual class (bisect with `SCRIP_BB_ONLY`/`SCRIP_BB_SKIP` nid csv, which is exactly what that instrument is for), not to re-observe that it regresses.

**NEXT, DEPENDENCY ORDER:** (1) ⭐ **Bisect the residual spine blocker** with `SCRIP_BB_ONLY`, one kind at a time, against the 251/248 baseline — the fail set IS the ladder and it is now ~1/3 the size it was this morning. (2) The remaining 11 templates spelling `"rsp"` directly (`grep -ln '"rsp"' src/templates/bb_*.cpp | grep -v glue`) — retire through ZTOS/x86_zop; `bb_match_capture`'s raw `rsp#` marker is load-bearing inside the SAVE/COND protocol and wants the splice ruling first. (3) `bb_binop_arith`'s own mid-body `add rsp,16` (pops its second operand cell) — traced, composes correctly with op_zdepth, still a raw rsp reference in a template. (4) **Wire `bb_glue_framed_*`** — the RBP half; customers closed by the s21x-c four constructs. ⛔ LAYOUT CONTRACT, not a spelling switch (s21x-m measured SEGV): whoever wires it wires the prologue's rbp save/seed in the same breath. (5) ARBNO onto the framed glue = the VARIABLE-EXTENT ANCHOR that retires the SUSPENDED-CELL law.
...
**DEFAULT = RSP ACTIVATED (Lon s21x-m, unchanged):** **m3 PASS=250 FAIL=66** *(flaps ±1 run-to-run — see negative result 2 above; m4 is authoritative)* · **m4 PASS=248 FAIL=32 SKIP=36** · **DIVERGE=3**. This is the LADDER.  *(s21x-o: 211/105 -> 250/66 and 208/72 -> 248/32 across ZTOS-1 + ZTOS-2, DIVERGE flat throughout.)*
