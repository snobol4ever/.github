...
- [ ] **W-1 · FRAME THE CONSTRUCTS (construction; SCRIP_WHOLESALE minted, default OFF)**
  ⚠ **STEP 1 WIP — SCRIP `2a120952`** (SN4 session, 2026-08-03). Parts 1-4 structural landed; gate-OFF byte-identical to W-0b. Gate-ON still SEGV. **ONE REMAINING BUG:** `x86_alpha()` fc-hook carves `op_fc_wbytes=24` (CAS-window sentinel slot) BEFORE `push rbp` in MECH-2; mech2_whack (`mov rsp,rbp; pop rbp`) does not release it; net 24B depth imbalance causes corrupted rip on exit. **NEXT ACTION (1 edit, ~5 min):** at MATCH_BEGIN dispatch, when `op_zw2`, zero `g_emit.op_fc_wbytes` and `g_emit.op_fc_base` — the fc-window slot is already handled by the explicit CAS sentinel push in the MECH-2 arm; `x86_alpha()` must skip the 24B sub. Then re-run gate-ON 044_pat_pos and full 318 crosscheck. Parts already landed in SCRIP `2a120952`: (1) `zd_plan` cm[] zzwr[k]=1 when zwr; (2) `op_mech2_kc` field+choke+compensation op_zdepth=8+Kc; (3) `bb_match_begin` sub rsp,Kc+CAS reorder+zread RDQ; (4) `bb_match_end` FRQ→RDQ restores; zd_plan zgpop/zwpop=0 for STMT_END/STMT when zwr; MATCH_BEGIN/END dispatch zgpop/wpop=0 when op_zw2.
  1. ~~Land the ratified ZW_RB fix~~ **ALMOST DONE — see WIP above**. Acceptance: `SCRIP_ZW_RB=1` → 044_pat_pos PASS + full 318 at-or-above W-0b baseline both modes; gate-OFF byte-identical.
