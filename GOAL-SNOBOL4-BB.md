**SESSION WATERMARK — 2026-06-12 · Sonnet 4.6 · INLINE-ALT M4 FIX LANDED (SCRIP `4097d52`). NEXT = D7-RB-3 (LEN value-assign lowering gap, or next open rung).** `flat_drive_capture` and `flat_drive_match` both silently dropped all but the first arm of γ/ω-threaded inline PAT_ALT chains (the shape lower_snobol4.c emits for inline scan alternation like `X ('cat' | 'dog') . V`). Root cause: fallback to `walk_bb_flat(ch,...)` on first arm only when `bb_operand_aux_get` returned na=0 (inline chains store arms via γ/ω port links, not operand_aux). Fix: `gather_inline_alt_arms()` collects ω-linked arms; both drivers emit each arm with cascading ω-labels and per-arm β labels (fresh alloca prevents duplicate-label assembler errors). Also fixed `flat_drive_match` abort-on-real-sibling to exempt detected inline alt chains. PROBES (m2==m3==m4==sbl): 050 `('cat'|'dog').V` PASS · 051 `('apple'|'banana'|'cherry').V` PASS · uncaptured `('yes'|'no')` PASS. NOTE: `P=LEN(3)` already works in all modes (was closed by D7-RB-2b, not a live gap). FLOOR: smoke 7/7/7 HARD · pat-rung M2 19/19 · M3 19/19 · M4 18/19 (054 ARBNO-alt pre-existing) · fence HARD.
---
