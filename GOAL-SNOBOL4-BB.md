...
# ▶▶▶ NEXT SESSION — START HERE (session 21, 2026-06-24)

**FZ-3 LANDED (SCRIP `973df9e`, corpus `5d30e2ed`).** `('aaa'|'bbb'|'ccc'|'ddd') SPAN('abcd') . W` stored pattern now constant-folds to a sealed `IR_REF_INVARIANT` blob — no runtime builder call. Gate MET: mode-3 AND mode-4 output `result: 500000` + `W: ccccddddaaaa` (hand-computed: first ALT match `bbb`@10, SPAN maximal `ccccddddaaaa`@13–24; oracle confirm pending x64 token). Added `sno_freeze_pat_ir()` — a general recursive **kids-channel** matcher-graph builder for fully-invariant stored patterns — replacing the narrow FZ-2 one-shape constructor. Key insight: `flat_drive_cat/alt` read arms from the **kids channel** (`bb_match_kids_state_t` on `IR_EXEC.counter`), not the γ/ω spine; `sno_freeze_pat_ir` builds in kids-channel form (ALT/CAT → `sno_freeze_kids_attach`; captures → `ir_operand_push` inner; leaves → IR_LIT). `sno_has_pat()` guard prevents value-concat expressions (e.g. `42 ' items'`) from entering the freeze path (both are kind==1 + non-buildable, but only one is a pattern). `pattern_bt.sno` + `.ref` updated (added `W` output, ref set to `ccccddddaaaa`). `string_pattern.s` byte-identical (FZ-2 path untouched). Feature .s artifacts (word1/3/4.s) updated. Bench OK=15 FAIL=0 CRASH=1 (eval_dynamic OOM pre-existing, unchanged). Crosscheck PASS=171 FAIL=84 vs pristine 168/87 — zero new failures, +3 fixed (`132/133/134_pat_fence_eps_recur`: also invariant SEQ patterns, now freezing via the general path).

**▶ NEXT MOVE: FZ-4** — retire dead fused builders. `sno_freeze_pat_ir()` handles the full invariant grammar; `sno_freeze_break_cap_lit_ir()` (narrow BREAK-cap-lit hand-roller) and the per-shape `bb_build_break_cap_lit_blob` / `bb_build_break_capture_blob` runtime builders in `src/runtime/rt/bb_pat_build.cpp` are now dead code. Gate: remove them; `test_crosscheck_snobol4.sh` byte-identical (zero regressions); bench unchanged. **Then FZ-5 (match-site direct reference — skip DEFER on once-assigned invariant patterns for the hot-loop win) or GVA-4 or OPSINGLE — Lon decides.**

**FZ-2 LANDED (SCRIP `c3b6a83`).** `string_pattern`'s stored `PAT = BREAK(',') . WORD ','` constant-folds to a sealed `IR_REF_INVARIANT` blob — NO runtime builder call. Gate MET: mode-3 AND mode-4 output byte-identical to `string_pattern.ref`; `bb_build_break_cap_lit_blob` count in `.s` = 0. Six pieces: `bb_ref_invariant.cpp`; decl in `bb_templates.h`; `case IR_REF_INVARIANT` in `walk_bb_node`; `flat_drive_ref_invariant` reads from aux channel; lowerer produces `IR_REF_INVARIANT` for narrow BREAK-cap-lit shape; Makefile registration.

**FZ-0 LANDED (SCRIP `a48c831`).** `sno_pat_kind` (3-valued classifier) + `sno_pat_islands` + `sno_island_walk` added. Behavior-neutral.

**FZ-1 LANDED (SCRIP `3b13020`).** Factored `sno_break_cap_lit_graph()` + freeze primitives. Behavior-neutral. `sno_freeze_break_cap_lit_bin/text` now dead (FZ-4 retires them).
