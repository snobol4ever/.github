...
## ⭐⭐⭐ LIVE CURSOR — 2026-08-07b (Fable — PATREF INVALIDATED; LADDER PB OPENED; docs only, zero code commits)

⭐ **LON RULING (verbatim): "5-stage STATEMENT EXECUTION: (1) build subject (2) build pattern from variant and invariant BB BLOBS, (3) use NEWLY built or TOTALLY INVARIANT PATTERN for EXECUTE PATTERN MATCH, (4) build replacement, then (5) perform splice subject replacement."** A pattern element is VARIANT (values fetched once at statement execution, at pattern build, left-to-right) or INVARIANT (pre-built). IR_MATCH_PATREF implements match-time by-name fetch — `*`/deferred timing for plain-ref syntax — which is NEITHER. **PATREF IS INVALID AND MUST BE DELETED.** Witness: roman.sno (Lon watching roman.s); gates: 141-probe + xc318 as always.

**SESSION FINDINGS (Fable, this session):**
1. **PATREF ≡ DEFER in implementation**: shares bb_match_defer template; roman.s n15 fetches T BY NAME at match time on every anchor-retry α entry. Semantic divergence class: immediate `$ X` mid-pattern followed by plain `X` — SPITBOL uses the BUILD-time snapshot, SCRIP re-fetches. Distinguishing witness to add: `X = 'Z'; 'AZB' ? 'A' $ X X` — oracle decides (snapshot ⇒ match 'AZ'; re-fetch ⇒ fail at pos 1).
2. **The 224B UCLAIM monolith on roman is CAUSED by PATREF**: ZW-13 graph-scope seal (`DEFER||PATREF`) seals BOTH roman statements (graph scope), forcing MATCH_BEGIN off the ZWS canonical frame onto the whole-statement carve. The seal is CORRECT for the implementation as it stands (by-name match-time fetch genuinely can re-enter) — deleting PATREF dissolves it for plain-ref programs; DEFER's seal stays (legitimately re-entrant, p.122).
3. **The replacement machinery ALREADY EXISTS**: lower_snobol4.c:1586-1599 lowers calls-in-pattern-position eagerly ("evaluate it once and use the result as a pattern") into IR_MATCH_VALUE with operand[0] = the value cell; bb_match_value.cpp already carries BOTH the ZD arm (ZOPQ(0,0), line ~29) and the DT_PATTERN invoke arm (dtp_fn_of, line 19). Plain names just need to route through the SAME idiom.
4. **Reverted this session (confession, per STALE-ORIENTATION honesty)**: two uncommitted default flips (SCRIP_ZD_MATCH ON, SCRIP_ZD_PATREF ON) were made then reverted — the 06l/06o cursor measurements (armed trails default by 18) contraindicate any default flip; it rides W-2's own gate. Tree verified clean at `8ab4c266`.

### ⛔⭐⭐⭐ LADDER PB — 5-STAGE STATEMENT EXECUTION / PATREF DELETION (Lon ruling this session; supersedes ZD-PATREF rungs and the s23i "static-shape patref arming population" rung — those armed the invalid box)

**⭐⭐ STITCH RULING (Lon, same session, verbatim example):** `P = '123'; Q = 'A' P 'Z'; R = Q Q Q` "keeps building larger and larger patterns at runtime" — each pattern ASSIGNMENT with plain refs SNAPSHOTS constituent values at ITS OWN execution and composes a NEW pattern value (structure sharing; reassigning P later leaves Q untouched). By contrast `Q = 'A' *P 'Z'; R = *Q *Q *Q` "are ALL totally invariant" — fixed structure (defer-by-name nodes), buildable ONCE at compile time; the indirection happens at match time by definition of `*`. **"So you must use the BB's that STITCH."** CONSEQUENCES: (a) **INVARIANCE CLASSIFIER**: a pattern expression is totally invariant iff every leaf is a literal, a primitive with invariant args, or `*name` — static blob, stage 2 is a NO-OP for it; ANY plain ref ⇒ variant ⇒ stage-2 runtime build. (b) **STITCH BBs**: variant pattern VALUES are runtime-composed heap structures (SEQ/ALT stitch nodes referencing invariant blob segments + snapshotted leaf DESCRs + defer-by-name nodes) — the bare-fn-pointer PAT$N representation with by-name interiors is the builder-level PATREF disease and cannot express R's snapshot of Q. (c) Stage 3 consumes the freshly stitched structure OR the static invariant blob — the ruling's "NEWLY built or TOTALLY INVARIANT" disjunction, verbatim. **THE 5-STAGE STATEMENT IS TOP PRIORITY (Lon).**

**MODEL MAPPING:** stage 1 = subject value-spine (EXISTS). Stage 2 = every plain ref in pattern position lowers to IR_VAR on the spine BEFORE MATCH_BEGIN, source order; invariant structure = the statically compiled element blob (EXISTS). Stage 3 = MATCH_BEGIN..MATCH_END consuming pre-built CELLS only; IR_MATCH_DEFER (`*expr`, lower:1251, star-ONLY by construction) is the SOLE match-time evaluation, by definition. Stage 4 = replacement spine (EXISTS). Stage 5 = MATCH_REPLACE (EXISTS). Polymorphic plain ref = IR_MATCH_VALUE: cell holds string → literal match; DT_PATTERN → dtp_fn_of(cell) → jmp blob via γ/ω wires. NEVER an NV by-name fetch inside the match.

- [x] **PB-0 RECON CLOSE-OUT (Fable, 2026-08-07b — MEASURED, zero code commits):**
  - **(a) ONE MINT SITE**: lower_snobol4.c:1257 `case TT_VAR:` — the bare plain-name arm inside `sno_pat_node`. `TT_DEFER` (`*X`) arm at :1251 is UNTOUCHED. No other lowerer mints PATREF.
  - **(b) PATREF COUNT IN ROMAN**: 18 references in roman.s — 1 node (n15), 17 jump-target occurrences. Two match statements in ROMAN proc bear PATREF (statement 1: `N RPOS(1) LEN(1) . T =`; statement 2: `'0,1I,..' ? T BREAK(',') . T`). Both statements fall to the 224B UCLAIM monolith because of these nodes alone.
  - **(c) CONSUMER CENSUS** — all files touching IR_MATCH_PATREF, grouped by action after deletion:
    - `src/contracts/IR.h:141` — enum declaration → DELETE entry, renumber none (enum value, not switch-critical)
    - `src/contracts/scrip_ir.c:105` — name string → DELETE row
    - `src/contracts/zeta_storage.c:141,256,295,345,361,403,431,596,609,981` — 10 sites, all `DEFER||PATREF` paired checks → DELETE `|| IR_MATCH_PATREF` from each; DEFER stays
    - `src/contracts/IR.h:225` — `pat_static` field on IR_t → stays (DEFER still reads it)
    - `src/optimizer/ir_query.c:16` — case in some query → DELETE or merge with DEFER case
    - `src/runtime/rt/bb_pat_build.cpp:77` — `DEFER||PATREF||VALUE` static-check → DELETE `|| IR_MATCH_PATREF`
    - `src/emitter/emit.cpp` — 12 sites: dispatch case :947 (shared with DEFER), deep-arrival :2878, emit_graph_zstatic :2927, zdyn veto :2013, span-shrink :2053, ZWS seal :2057, ZWR seal :2061, RPO push :2239, SPD-2 :2616, ARBNO _sq exclusion :951, op_sval :853, defer_op :857 → all `DEFER||PATREF` pairs, DELETE the PATREF half
    - `src/lower/lower_snobol4.c:1257` — the ONE mint site → REPLACE with 5-stage lowering (PB-1)
  - **(d) WHOLE-PATTERN `S ? P` ROUTING**: same `case TT_VAR:` at :1257. When P is a lone bare variable in pattern position, sno_pat_node routes to `TT_VAR` → PATREF. After PB-1 this routes to VAR-fetch on the pre-head spine + IR_MATCH_VALUE. NO separate code path for the whole-pattern case.
- [x] **PB-1 LOWER SWAP** — LANDED (SCRIP `9480c35e`). TT_VAR in pattern position → `IR_MATCH_VALUE(operand[0]=VAR cell)` via `sx_lower`. DEFER untouched. Killswitch `SCRIP_PAT_BUILD=0`. Gate M4 17/4 — **0 regressions**. PATREF nodes in roman.s: 18 → **0**. `sub rsp,224` remains (PB-3 seal shrink is next). Head-PIN bug (roman wrong output) is pre-existing, unrelated.
- [ ] **PB-1 LOWER SWAP** (killswitch `SCRIP_PAT_BUILD=0` restores 1257 PATREF mint byte-identical): lower_snobol4.c:1257's plain-name arm re-routed through the 1586/1599 eager idiom — mint the name's VAR fetch on the spine pre-head (source order among refs), feed IR_MATCH_VALUE(operand[0]=cell). `*` arm (1251) UNTOUCHED. Gates: roman m3+m4 == .ref; new timing witness (finding 1 above) vs SPITBOL oracle; 141-probe 0 regression; xc318 BY SET both modes.
- [ ] **PB-1s STITCH REPRESENTATION (design rung — Lon consult before landing)**: runtime pattern-value node set {INVARIANT-SEG (ref to static blob), VALUE-LEAF (snapshotted DESCR), SEQ-STITCH, ALT-STITCH, DEFER-BY-NAME}; pattern-ASSIGNMENT builders with plain refs emit stage-2 STITCH BBs (fetch constituents eagerly, source order, compose new structure); the invoke path walks the structure. Witnesses vs SPITBOL oracle: pb_stitch_snapshot.sno (`P = '123'; Q = 'A' P 'Z'; P = 'XXX'; 'A123Z' ? Q` — must SUCCEED on the snapshot) + the `*` twin (`Q = 'A' *P 'Z'` after reassign — must track new P). PB-0b census WIDENS to pattern-assignment builders: PAT$N interiors minting PATREF carry the same invalid timing (`R = Q Q Q` is the witness).
- [ ] **PB-2 VALUE INVOKE PARITY**: bb_match_value DT_PATTERN arm must carry n15's full invoke protocol (β resume = suspended blob continuation, wire quad) sourced from the CELL — diff against bb_match_defer's PATREF path, port anything missing. Gate: stored-pattern corpus class (`S ? P`, P holds pattern) green both modes.
- [ ] **PB-3 SEAL SHRINK**: drop IR_MATCH_PATREF from the ZW-13 graph seals (BOTH the zws line ~2057 AND zwr line ~2061) + the zdyn veto trio — gated on PB-1 census = 0 minted. DEFER stays sealed. Gate: SCRIP_ZWS_DIAG on roman shows both statement runs unsealed (decline reason must move off "seal").
- [ ] **PB-4 ROMAN ARM MEASURE** (env `SCRIP_ZD_MATCH=1`, NOT default): roman.s must show MATCH_BEGIN canonical frame + per-BB 16B carves, ZERO `sub rsp,224`-class claims; re-run 141-probe armed — classify remaining armed regressions (expect the FENCE-class 18, unrelated, MECH2/W-2 debt).
- [ ] **PB-5 DELETE**: IR_MATCH_PATREF enum + dispatch case + bb_match_defer routing + seal/veto mentions + PB-1 killswitch. Completion: `grep -rn IR_MATCH_PATREF src/` == 0. Regen ×3.
