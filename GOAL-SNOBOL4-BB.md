...
**▶ NEXT MOVE (session 27): DCR-3 ✅ DONE (re-grounded above). Remaining DCR open rung is DCR-2 (codegen-side by-name call drop). Next perf rung: SAB (string self-append, string_concat 10.0× / string_manip 17.2×) or table-probe inline (table_access 10.9×, where AXS's DT_A guard bypasses TABLE) — Lon decides.**
...
## ▶ PRIORITY RUNG (do FIRST, session 24): PERF-CALL — inline the per-primitive runtime calls
