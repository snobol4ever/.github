...
## ⭐⭐⭐ LIVE CURSOR — s23l (2026-08-02) — ZW-12 CANONICAL MATCH FRAME + ZW-3 R12 REVERSAL

**Directive (Lon):** *"Complete, once and for all, the NON-POPPING FORTH-style RSP ZETA stack with a C-style RBP used occasionally only when absolutely necessary. Get R12-R15 as SOLE pattern match state. Get proper whack brackets. Continue."* + "All your choices. I'm with you on this. Continue."

**LANDED (SCRIP `705c1c88` ZW-12, `aeb58762` ZW-3; kill-switch SCRIP_ZW_FRAME=0 restores byte-identity; gates: crosscheck-318 m4 BY SET IDENTICAL to s23k open bracket, bench 18/21 exact, killswitch A/B clean both rungs):**
1. ⭐⭐⭐ **RUNG ZW-12** (ZW-1+ZW-2 as one land): canonical MATCH frame `push rbp; mov rbp,rsp; sub rsp,56` for every run the ZD planner fully arms (Kc>0, nblob==0, no ABORT/FENCE1, no STF, END present, window-integrity). Head quintet `{Σ,δ,Δ,cas_base,anchor_snapshot,start_δ,cap_gen}` at `[rbp−8..−56]` raw depth-free. Subject read IN PLACE before `push rbp` (subjc pop violator deleted). &ANCHOR snapshotted once per SPITBOL §13678 ("obtained only at start of match"). Both tag-0 CAS marker scans deleted from MATCH_END — pump receives `cas_base` straight from frame. `mov rsp,rbp; pop rbp` = the whack — frees every member/producer cell in one move. zd_plan ±`ZW_FRAME_TOTAL`=64 between head and END from ONE named constant so template and model cannot drift. pin publication gated `&&!op_zw` (no pin under the frame). STF excluded (its own rbp bracket; two disciplines must not nest).
2. ⭐⭐⭐ **RUNG ZW-3** (R12-FREE-1 REVERSAL): under `op_zw=1`, capture COND writes 24B pend record AT r12 (`[r12+0/8/16]`), bumps r12 by 24 on γ, sub r12,24 on β — no cell reads/writes. Wholesale discard in BEGIN-ω and END-γ restores r12 from `[rbp−32]` (cas_base) then mirrors to the 0x70000000 cell for C-transit. Seeds re-added: `rt.c rt_outer_call` (`push r12; mov r12,[0x70000000]; call; pop r12`) and `scrip.c` m4 wrapper ×2. Old regime (`op_zw=0`) byte-identical by dual IF gates.
3. ⭐ **Why ZW fires on zero programs today (correct):** ZW verdict requires Kc>0, which requires the full ZD run to arm, which requires `zd_wl_kind` to approve all run members. Every real SNOBOL4 pattern program has blob members (IR_MATCH_LIT etc) not in the ZD whitelist — that is the r9/wire rung. Frame and r12 arm automatically when blob ZD arms land.
4. **WATERMARK (s23l):** session-open m3 281/26/10 · m4 266/39/10/2L (exact s23k hold). ZW-12 close m4 BY SET IDENTICAL; ZW-3 close m4 BY SET IDENTICAL to open. m3 flicker = pre-existing 127/152 placement pair (named s23i cap-slot class). Bench 18/21 exact s23k reds {roman,eval_fixed,eval_dynamic}. Regen ×3 (benchmark, feature, demo — all current, zero .s changes as expected since ZW fires on no current programs). Killswitch A/B byte-identical.

**NEXT:** **ZW-0 stage 2** (dead-arm deletion, 17 arms 16 files — safe standalone, no design work) → **ZW-4** (g_patstk_* deletion + g_anchor demotion, 1,112 mark sites die via regen) → **ZW-5 IR_STATEMENT** lower-side insertion per s23k design → **ZW-6** (glue-whack relocation, 1,264 sites). Then: **r9/wire** rung (retires roman + 127/152 + capture-bearing declines; ALSO unblocks blob ZD arming which fires the ZW frame for the first time in practice) → m4-EVAL rung · ZD-7c.
