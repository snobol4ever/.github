...
## ⛔⭐ LIVE CURSOR — s21x-n (2026-07-30, Claude: ZOP-1 ONE OPERAND-ADDRESS AUTHORITY + GLUE TEMPLATES — seam landed, two negative results measured).

**LON DIRECTIVES (s21x-n):** "Parameterize the access to operands via the FOUR modes" · "Do not put RSP references directly into the templates" · "Devise two glue codes, one flat jumps and the second the same plus setup of stack frame" · "Would those two glue snippets qualify for each having their own template" (yes — R6 is a template rule) · "You got this. Continue."

**LANDED (SCRIP `b2afdd54`):**

**ZOP-1 — `x86_zop()` is now the ONE operand-address authority** (`x86_asm.h`, `emit.h`, `emit.cpp`, `zeta_storage.c`). Before this rung a zeta operand address was the cartesian product of FIVE independent booleans across three near-duplicate accessors (`x86_fc_hit` / `x86_isle` / `x86_fb_pinned` / `flat_fb_refine`+`op_fb_rbp` / encoder-side `op_flat_disp`) — 32 representable states per node of which four are coherent. Those four outcomes are now named arms of one switch; `FR`/`FRQ`/`FRQB` are one-line width+bump wrappers. `x86_zstorage()` makes `ZC_STORAGE` load-bearing at the one place that matters: the selector had TWELVE references and not one was an operand-address decision; the mixed states its own design comment calls UNREPRESENTABLE stayed fully representable.

**GLUE-1/2 — `bb_glue_flat.cpp` and `bb_glue_framed.cpp`, one file each** (promoted from the four unwired `x86_glue_*` inline helpers in `x86_asm.h` that had ZERO callers). Deciding argument: R6 — four-mode variance wants the `IF()` combinator in one concatenation, which is a template rule. Changes on promotion: K read from `_` (`op_fc_bytes`, the ONE K authority) not a parameter; CELL_HEAP arm BOMBS rather than silently emitting nothing (the `x86_fc_hit` silent-fallback lesson); framed leave takes no K by design (`mov rsp,rbp` discards at whatever depth was actually reached). DECLARED, BUILT, NOT YET WIRED — allocation still rides `x86_port_hook`, so this rung moves no bytes.

**ZOP AUDIT** — `zop_audit_graph_close()` / `zop_audit_report()` in `zeta_storage.c`; bitmask `zop_seen` in `emit.h`; `SCRIP_ZOP_AUDIT=1` to report; `zop_audit_seen_clear()` makes the close idempotent.

**MEASURED, NOT ASSUMED:** 80 corpus programs, `--compile` output byte-diffed against the pre-ZOP compiler (rebuilt from stash for comparison), all equal. m4 PASS=208/FAIL=72/SKIP=36 and DIVERGE=3 reproduce EXACTLY.

**TWO NEGATIVE RESULTS — READ BEFORE ROUTING:**

1. ⛔ **THE MIXED-REGIME HYPOTHESIS IS FALSE.** The five-boolean cascade is real as code structure but is NOT producing mixed graphs. Corrected audit: **154 graphs over 116/120 programs, mixed=0.** Every graph resolves through exactly ONE whole-graph regime (rsp or rbp) with per-BB cell windows composing on top. The s21x-j..m failures are NOT regime mixing. ⚠ THE FIRST RUN OF THIS AUDIT SAID mixed=0 AND WAS VACUOUS: `zop_audit_graph_close` fires at the START of each graph, so every program's LAST graph went untallied and single-graph programs tallied nothing — 38 graphs across 120 programs, 15 programs responding. Fixed (atexit registered unconditionally + report closes the final graph) BEFORE the number was believed. IMPLICATION: the ladder failures are a **depth** problem, not a regime problem. `028_arith_unary_minus` prints `5` instead of `-5`; `SCRIP_BB_ALLOC=0` restores `-5` — same regime, wrong depth, because `op_flat_disp` is a LOWER-time static prefix sum and the universal carve made depth dynamic.

2. ⛔ **THE m3 WATERMARK IS NOT A STABLE NUMBER.** Measured 211 / 210 / 211 / 211 across four runs of an IDENTICAL binary with byte-identical `.s`. One program flaps between PASS and FAIL across runs. Any session reading 210-vs-211 as regression or progress is reading noise. **m4 and DIVERGE are the trustworthy arms.** This also resolves the s21x-m cursor's 210/106 vs this session's 211/105 — no divergence to chase.

**NEXT, DEPENDENCY ORDER:** (1) ⭐ **CRAWL THE LADDER** — the ~106 m3 / 72 m4 failures ARE the rungs, MONITOR-FIRST per RULES. Root cause is **depth displacement**: `op_flat_disp` (LOWER's running prefix sum) must become the live authority as carves deepen the stack. (2) **Wire the glue templates** — connect `bb_glue_flat_enter/leave` and `bb_glue_framed_enter/leave` through `x86_alpha()`/`x86_beta()`/`x86_gamma()`/`x86_omega()` so the allocation moves out of `x86_port_hook` and into the templates where the four-mode IF logic belongs. (3) **15 bb_*.cpp files still spell `"rsp"` directly** (measured: `grep -ln '"rsp"' src/templates/bb_*.cpp | grep -v bb_glue`) — retire them through `x86_zop`. (4) ARBNO onto `x86_glue_framed_*` = the VARIABLE-EXTENT ANCHOR that retires the SUSPENDED-CELL law. (5) `.s` artifacts remain stale from s21x-m; do not regen until a session can run all three regen scripts cleanly.

**⚠ CONCURRENCY:** watermark is SHARED STATE. `SCRIP_BB_ALLOC=0` reproduces the s21x-l baseline (m3 312/4) exactly and is the one-line answer for parallel sessions.

---

## ⛔⭐ WATERMARK OF RECORD (2026-07-30, s21x-n)

**DEFAULT = RSP ACTIVATED (Lon s21x-m, unchanged):** **m3 PASS=211 FAIL=105** *(flaps ±1 run-to-run — see negative result 2 above; m4 is authoritative)* · **m4 PASS=208 FAIL=72 SKIP=36** · **DIVERGE=3**. This is the LADDER.
