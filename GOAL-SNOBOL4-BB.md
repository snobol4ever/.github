...
- [ ] **W-1 · FRAME THE CONSTRUCTS (construction; SCRIP_WHOLESALE minted, default OFF)**
  ⚠ **STEP 1 WIP — SCRIP `9a06130d`** (SN4 sessions, 2026-08-03/04). Gate-OFF byte-identical confirmed. Gate-ON: match+capture produce correct output; 8-byte rsp imbalance at main_γ ret causes crash in rt_outer_call. **GATE NAME CORRECTION: `SCRIP_WHOLESALE` is goal-doc only — actual env var is `SCRIP_ZW_RB=1` (drives `zw_rb_on()` in emit.h:6). All gate-ON tests must use `SCRIP_ZW_RB=1`.** **EDITS (SCRIP `9a06130d`):** (A) emit.cpp:829: `op_zdepth=8` for mech-2 blob members (was `8+Kc`; blob FR now `[rsp+8+off]=[α-Kc_zwr+off]` in carved region). (B) emit.cpp after zwr: `Kc_zwr=Kc_cells+72` (64B fixed header `[rbp-8..rbp-64]` + 8B align pad; push_rbp(-8)+sub_rsp,Kc_zwr lands 0-mod-16 for C ABI). (C) bb_match_begin.cpp mech-2 arm: saves at fixed negative rbp offsets `[rbp-16]=outer_Σ,[rbp-24]=outer_δ,[rbp-32]=outer_Δ,[rbp-40]=cap_gen,[rbp-48]=start_δ,[rbp-56]=patstk,[rbp-64]=zls2`; sub rsp,Kc_zwr after saves before rt_match_enter; subject via rbp+8/16 (subjc) or rsp+Kc_zwr+8+op_sa (!subjc). (D) bb_match_end.cpp mech-2 arm: restores from same fixed offsets. **OPEN BUG:** main_γ ret has rsp 8 bytes too low → rt_outer_call ret reads `0x42e150` ("hello" string ptr) as return target → SEGV at `0x500000002`. The 8B shortfall is an unbalanced push/sub in the mech-2 activation. **NEXT ACTION:** gdb watchpoint at rt_outer_call entry on `[rsp-8]` (rt_outer_call's own return-addr slot); step through full mech-2 success path to main_γ ret; find the single push/sub not balanced. Candidate: `hfc()` arm inside zls2_mark save path — verify `hfc()=false` for mech-2 heads (`op_fc_wbytes=0` gated at emit.cpp:973 `!op_zw2`). Parent hashes: `.github`=`45edc7b0` · corpus=`8411e48f` · SCRIP=`4d902148`.
