...
## ⛔ STANDING POINTER (added 2026-07-07, Claude Sonnet 5) — read `GOAL-TEMPLATE-REVAMP-RULES-DRAFT.md` BEFORE writing or editing ANY `x86_asm.h` encoder or ANY `xa_*`/`bb_*` template code

This is a durable pointer, not a session narrative — see the addendum immediately below for what actually happened and why this pointer exists. In short: a new encoder for a new runtime-helper call is NOT exempt from that doc's R2/R7/R9/R10 just because it's new — it must be ONE `x86(...)` concatenation, consumed via `bb_emit_x86`, never a hand-written `IF(MEDIUM_TEXT,...)+IF(MEDIUM_BINARY,...)` pair duplicating one instruction twice (that doc's own named "forbidden shape"). This pointer is also now in `PLAN.md`'s BB-CODEGEN DESIGN SET checklist (session-start step 7) — a `GOAL-*-BB` rung doing template/encoder work assumes both are read.

---

## SESSION ADDENDUM — 2026-07-07 (Claude Sonnet 5, session 3 this day): graph-scale mark/release LANDED, both mediums, full crosscheck back to exact baseline — two additional real bugs found and fixed along the way; one design question still open, not yet investigated

**Starting point:** the previous same-day session's addendum (preserved below this one, do not delete it — it documents the alignment root-cause diagnosis this session built on) left the alignment bug diagnosed but unfixed, TEXT-mode only, BINARY mode untouched. This session picked up exactly at its "NEXT SESSION" pointer.

**What actually shipped, in the order the bugs were found (each confirmed via direct evidence — gdb register/backtrace inspection or hex dump of the actual emitted bytes — not inferred from reading the assembly alone):**

1. **Storage layer:** `zeta_storage.c`/`.h` grants a new graph-scope `zeta_mark_off` slot (mirrors `resume_off`'s existing pattern exactly — one quad, granted unconditionally for every graph, not gated on `g_frame_active`, because `ir_drive_slot_assign` runs for BOTH modes before `g_frame_active` is set for that compile — confirmed by reading `scrip.c`'s own call ordering, not assumed). Threaded through `IR_graph_t.zeta_mark_slot` (new field, `IR.h`) exactly like `resume_slot`, populated in `ir_drive_slot_assign` (`scrip_ir.c`) via a new `zls_g_zeta_mark(g)` accessor mirroring `zls_g_resume(g)`.

2. **First real bug, found and fixed:** an earlier version of `x86_zeta_mark_call`/`x86_zeta_release_to_call` (the new `x86_asm.h` encoders) fed pure `x86(...)`-composed output directly into `xa_flat.cpp`'s `xa_emit_one`, which does a literal raw-byte-copy (`bb_emit_byte` per character) with no awareness of `x86()`'s BINARY-mode output format — a tag-framed record stream (`'L'<len><bytes>` / `'J'<port>` / `'D'<port>` / etc., decoded only by `bb_emit_x86`, confirmed by reading `bb_emit_x86`'s own tag-loop in `x86_asm.h`). Confirmed via a raw hex dump of `xa_emit_one`'s own input (added a temporary debug print, removed after use): the `'L'` tag bytes (`0x4c`) were visibly corrupting the instruction stream mid-decode, reproducing the exact prior 108-test regression (`023_arith_add.sno` segfaulting on a plain, ARBNO-unrelated assignment). **Fix:** route the new calls through `bb_emit_x86` (the correct, ordinary template-dispatch consumer every other `bb_*.cpp` template already uses), never through `xa_flat.cpp`'s own bespoke `xa_emit_one` (which stays exactly as it always was, reserved for this file's own pre-existing raw-byte prologue/epilogue sequences — a real distinction, not an arbitrary one, since `xa_emit_one`'s single-patch-site design predates and is incompatible with the `x86()` record format).

3. **Correction mid-session (Lon directive):** an intermediate version of the two new encoders hand-wrote `if (MEDIUM_TEXT){...} if (MEDIUM_BINARY){...}` directly inside the encoder bodies — the exact anti-pattern `GOAL-TEMPLATE-REVAMP-RULES-DRAFT.md`'s R2/R10 name as the thing the whole template revamp exists to kill (see that doc's own "forbidden shape" worked example). **Fixed:** both encoders are now pure `x86(...)` concatenation, zero manual `MEDIUM_*` branching of their own — medium dispatch lives only inside `x86()`'s own internals, exactly like every other encoder in `x86_asm.h`. **`GOAL-TEMPLATE-REVAMP-RULES-DRAFT.md` is now referenced from this file's own top (this addendum) and from `PLAN.md`'s BB-CODEGEN DESIGN SET checklist — read it before touching `x86_asm.h` or any `xa_*`/`bb_*` template, unconditionally, not only for boxes already mid-conversion.**

4. **Second real bug, found and fixed:** after routing through `bb_emit_x86` correctly, a DIFFERENT crash appeared (`r12=0x3` at the fault site, confirmed via `gdb` register inspection, not guessed) — the mark call was sequenced BEFORE `push r12`/`mov r12, rdi` in the prologue, so its own `mov [r12+off], rax` wrote through a not-yet-valid `r12`. **Fix:** reordered the wrapper (`xa_flat_prologue`'s `extern "C"` function) to emit the real prologue first (which establishes `r12`), THEN the mark call — not the reverse. This is the same category of mistake as the earlier-diagnosed `esi`-must-be-preserved issue, just one instruction earlier in the sequence.

5. **Epilogue plumbing:** `xa_flat_epilogue_str`'s signature gained two additional output-parameter strings (`out_succ`/`out_fail`) so the wrapper can interleave `bb_emit_x86(release_to_call)` between the two logical halves (succeed-and-return vs. fail-write-FAILDESCR-and-return) independently — the function's own internal logic and every one of its return values are otherwise byte-identical to before this session (verified by inspection, not just build success). Both TEXT and BINARY mediums populate these two params identically; the non-frame-active and `flat_wired` paths leave them empty (correctly — `have_halves` gates on non-empty, so those paths fall through to the original untouched single-call `xa_emit_one`).

**CONFIRMED WORKING, this session's own fresh verification (re-derived, not inherited from the prior session's addendum):**
- Full clean rebuild (`rm -f scrip out/libscrip_rt.so` first, per this project's own caught "stale .so" lesson) → `bash scripts/test_crosscheck_snobol4.sh` → **`--run` 252/276 (24 FAIL) · `--compile` 251/9/16 (9 FAIL, 16 SKIP) · DIVERGE=1** — exactly the pre-session baseline, byte-for-byte same FAIL lists on both modes. The 108-test regression is fully closed, in BOTH mediums (unlike the prior session, which only ever tried TEXT).
- `scripts/test_gate_template_medium_invisible.sh` → unchanged at 34 hits (`bb_suspend.cpp`(2) + `xa_flat.cpp`(32)) — this session's new code added ZERO to this pre-existing debt count.
- `scripts/test_crosscheck_icon.sh` → 4/4 PASS; `scripts/test_crosscheck_prolog.sh` → 48/48 PASS (6 pre-existing ORACLE_MISS, unrelated) — confirms the shared files this session touched (`x86_asm.h`, `IR.h`, `scrip_ir.c`, `zeta_storage.c`/`.h`) didn't disturb the other BB languages riding the same spine.
- `test/snobol4/patterns/zb_act_arbno_in_define.sno` (the probe from two sessions ago, still in tree) re-verified in BOTH modes, correct output (`aaa`), with `SCRIP_ZLS_RELEASE_TRACE=1` showing the mark/release_to sequence firing (two nested `[ZLS-MARK]`s for `main`'s and `MATCHIT`'s own chains, one `[ZLS-RELEASE-TO]` correctly matching).

**NOT yet done / genuinely open — say this plainly rather than let the crosscheck pass stand in for it:**
- **Generator/resume interaction not checked.** The mark call is now placed after the ENTIRE prologue string returned by `xa_flat_prologue_str`, including — when `g_gen_proc_active` is true — a conditional `jne β` dispatch jump that a genuine RESUME call would take, skipping past the appended mark call entirely (which is very likely the semantically CORRECT behavior — a resume re-enters an already-marked graph and should not re-mark — but this reasoning has NOT been verified against a real generator/resume test case this session, only reasoned about from reading the prologue string's shape for the one non-generator `main` graph actually exercised). **Before this rung is called fully done, run this same wiring against a genuine SNOBOL4 or Icon generator with a real resume path and confirm mark/release counts stay correct across resume calls, not just across fresh entries.**
- **Failure-path release_to at THIS (graph-scale) boundary not separately stress-tested.** The full crosscheck passing is not itself proof the fail_half release_to fires correctly on every kind of graph-level failure exit — only that nothing in the existing corpus regressed. A purpose-built failing-match probe at the graph-exit boundary (distinct from the statement-scope failure-path gap the PRIOR same-day session already found and closed) has not been run.
- **`052_pat_arbno.sno` and the wider ARBNO/FENCE test cluster** — not specifically re-verified against THIS session's changes beyond the aggregate crosscheck pass; the prior session's own false-confidence lesson on this exact test (documented in the addendum this one replaces) should be taken seriously by whoever picks this up next.

**Crosscheck watermark at handoff (RE-DERIVE at next session start — do not trust this line blind, per this file's own standing doctrine): `--run` 252/276 (24 FAIL) · `--compile` 251/9/16 (9 FAIL, 16 SKIP) · DIVERGE=1 (`1017_arg_local`, pre-existing, see the CURRENT STATE section's bracket #2 below) — UNCHANGED from every watermark recorded elsewhere in this file; this session's landed changes do not move these numbers, confirmed via a genuinely clean rebuild immediately before writing this line, not carried forward from memory.**

---

## SESSION ADDENDUM — 2026-07-07 (Claude Sonnet 5, earlier same day): alignment bug diagnosed, fix NOT yet implemented at the time this was written — SUPERSEDED, see the addendum immediately above for the landed fix; preserved here for its diagnostic value, do not re-derive

**Context — what this session was asked to build:** Lon's directive this session, corrected across several turns: α always allocates (unchanged from the existing per-construct rule); **ω always frees its own α's block, unconditionally, at every site it appears** (this resolves the six-jmp-ω-sites ambiguity documented below in the TOP PRIORITY RUNG's own SESSION LOG — every ω site is free to free, since ω's definition doesn't depend on which control-flow role it plays); and **the construct's true leap-out — not tied to any internal γ, since no γ inside a repeatable construct is ever "the last one" from that node's own vantage point — is where the entire graph's own allocation footprint gets freed**, at the point control genuinely leaves the graph back to whatever called it. This is graph-scale, not construct-scale: it sits ABOVE the existing role-0/role-2 ARBNO alloc/free pair (§ZB-ACT-0, the earlier section of this same TOP PRIORITY RUNG), as a backstop for whatever a construct's own local free doesn't catch (e.g. an activation still mid-iteration when a later, unrelated part of the pattern succeeds or fails and the whole enclosing graph returns).

**FOUND: the exact runtime primitives this needs ALREADY EXIST, unused, in `zeta_alloc.c`/`zeta_alloc.h`.** `rt_zls_mark(void)` and `rt_zls_release_to(void *mark)` were already committed (by an earlier session, dated the same 2026-07-06 as the TOP PRIORITY RUNG's own SESSION LOG below — see that file's own header comment at `zeta_alloc.c` line 94-113, "BB-OWNED-ζ GRAPH-EXIT STEP"), fully implemented, but **never wired to any call site** — `bb_match_arbno.cpp` only used `rt_zls_alloc`/`rt_zls_release`/the ARBNO-specific carrier pair before this session. `rt_zls_mark()` snapshots `g_zls_cur` (the LIFO chain's current top); `rt_zls_release_to(mark)` walks the chain from wherever `g_zls_cur` is now back down to `mark`, releasing each block via the existing one-at-a-time `rt_zls_release`, and — this is a structural property, not a runtime check — **a block already freed by its own construct's local ω is no longer reachable from `g_zls_cur` at all** (already spliced out of the chain), so it can never be double-freed by this walk; only genuinely-still-live orphans are ever visited.

**CONFIRMED WORKING, via a real hand-built probe and direct execution trace (not just static reading) — the mechanism itself is sound:**
- New test `test/snobol4/patterns/zb_act_arbno_in_define.sno` (committed, keep it): a minimal `DEFINE`d procedure wrapping a plain ARBNO match, purpose-built because a bare top-level `X ARBNO('a') . V` statement turned out to ALSO be frame-active (see the corrected finding below — this was NOT the actual reason a separate probe was needed, but the probe itself is still the cleanest minimal DEFINE-wrapped case and is worth keeping regardless).
- Compiled via `--compile`, assembled with `as --64`, linked directly against `out/libscrip_rt.so` (bypassing the crosscheck harness for tight iteration), and run standalone with `SCRIP_ARBNO_STEP1_TRACE=1 SCRIP_ZLS_RELEASE_TRACE=1`: traced the complete, correct sequence — `[ZLS-MARK]` fires twice (once per nested chain — `main`'s own and `MATCHIT`'s own — confirming graphs-call-graphs nesting works as the zeta_alloc.c comment predicted), `STORE` fires once (role 0's α), `[ZLS-RELEASE-TO]` fires with the correct mark, and its internal loop correctly releases exactly the orphaned ARBNO carrier block (the one role 2's own `x86_arbno_role2_free()` never reached in this specific control-flow path) while correctly refusing to touch the mark's own boundary block. A second, independent, ADJACENT release (the same block address, released by `rt_call_proc_descr` at `rt.c:399` tearing down the call frame IT allocated for `MATCHIT`) initially looked like a double-free bug during tracing — it is NOT; it is two separate, correct mechanisms (mine, and the pre-existing call-frame lifecycle) each doing their own job on their own respective blocks, confirmed precisely with a per-call trace after real suspicion and real re-verification, not assumed clean on a first read. **Do not re-litigate this — it was checked hard and it holds.**

**ATTEMPTED AND REVERTED (this addendum's own session) — wiring this into `xa_flat.cpp`'s frame-active prologue/epilogue caused a REAL 108-test regression in mode-4 (`--compile`), confirmed via the full crosscheck, NOT a false alarm:**
- The wiring: `rt_zls_mark()` called once at the top of the frame-active TEXT-mode prologue (`xa_flat_prologue_str`, the `g_frame_active` branch only — BINARY mode was deliberately left untouched this session, a real asymmetry to fix later, not an oversight to forget), storing the result via a bare `push rax` immediately after the existing `push r12`/`mov r12, rdi`; `rt_zls_release_to()` called on both `succ_half` AND `fail_half` of the matching epilogue (`xa_flat_epilogue_str`), via `pop rdi` reading that same pushed value back, before the existing `dpop`/`pop r12`/`ret` unwind.
- Baseline crosscheck before this wiring (mode off, confirming nothing else drifted): `--run` 252/24, `--compile` 251/9/16, DIVERGE=1 — matches this file's own existing watermark exactly.
- **With the wiring active (`SCRIP_ZETA_SELFLOAD=4`): `--run` UNCHANGED at 252/24 (binary mode untouched, as expected) but `--compile` collapsed to 152/108/16 with DIVERGE=100.** `052_pat_arbno.sno` — the test used for initial hand-verification — was NOT among the 108 failures (a real, confirmed check, not assumed); the actual failures included trivially simple, ARBNO-unrelated programs, e.g. `023_arith_add.sno` (`OUTPUT = 1 + 2`, nothing else), which segfaulted (exit 139), not merely mismatched output.
- **Root cause, confirmed via gdb backtrace against a core dump (gdb was NOT installed in this container at session start — `apt-get install -y gdb` fixed this in under a minute; do this immediately in any future session needing real stack-level debugging rather than working around its absence with ad hoc prints, which cost significant time this session before the install was tried):** `#0` inside libc's `__vsnprintf_internal`/`__printf_buffer_init_end`, reached via `NV_SET_fn` → `output_val` → `VARVAL_fn` (`core.c:1821`) → `snprintf`, itself reached from the completely ordinary `IR_ASSIGN global` block emitted for `OUTPUT = 1 + 2`. **The bare, unbalanced `push rax` used to stash the mark shifts the stack by an unaccounted 8 bytes for the ENTIRE REMAINDER of the function body** — every subsequent `call` in the emitted chain that does NOT do its own defensive `and rsp, -16` (which is most of them; only calls I explicitly wrapped, plus a handful of pre-existing ones, actually realign) inherits a stack 8 bytes off System V's mandatory 16-byte call alignment. Most code tolerates this silently; `printf`-family internals (SSE register spill paths) do not, and fault hard, often several call-frames away from the actual site of the misalignment — exactly the "bug doesn't announce itself where it happens" trap this project's own rules warn about, now a confirmed instance of it, not just a hypothetical.
- **REVERTED (within that session). Superseded: the very next same-day session (see the addendum above this one) re-implemented this properly** — via a graph-scope FRAME SLOT (`[r12+off]`, the same storage every other per-box local already uses) instead of a raw stack push, which sidesteps the alignment question by construction rather than needing to get the padding arithmetic exactly right.

**STILL IN THE TREE, harmless, gated on env vars, zero behavior change when unset — keep these, they're genuinely useful:**
- `src/runtime/rt/zeta_alloc.c`: `[ZLS-MARK]`/`[ZLS-RELEASE]`/`[ZLS-RELEASE-TO]` prints in `rt_zls_mark`/`rt_zls_release`/`rt_zls_release_to`, gated on `getenv("SCRIP_ZLS_RELEASE_TRACE")`.
- `src/templates/bb_match_arbno.cpp`: `[ARBNO-PHASE]` print at the top of `bb_match_arbno()` reporting `op_phase`/`op_off`/`op_sa`/`op_sb`, gated on `getenv("SCRIP_ARBNO_PHASE_TRACE")`. (This trace is what disproved this session's own early, WRONG hypothesis that a specific test was hitting the v2/roles-3-4-5 collection-body path instead of v1/roles-0-1-2 — it wasn't; both phase-trace evidence and separately the `--dump-ir` output confirmed straight v1 for every case checked. Do not re-derive this; it's settled.)
- `test/snobol4/patterns/zb_act_arbno_in_define.sno`: new, minimal, correct, worth keeping regardless of what happens next.
...
## SESSION ADDENDUM — 2026-07-07 (Claude Sonnet 5, session 2 this day): BB-OWNED-ζ statement-scope pivot — mark/release_to wired at match-statement boundary, both exits; ZC_COL_ARENA arm for rt_zcol_push

**Pivot from the prior addendum's frame-active prologue/epilogue approach (which regressed and was reverted): instead of marking at the whole-graph prologue, mark once at the STATEMENT boundary — specifically at IR_MATCH_HEAD's own α, the one-time entry to a `SUBJ ? PAT` statement's scan — and release_to at BOTH of that statement's true exits (success and failure), not the function-level prologue/epilogue that caused the earlier alignment regression.** This sidesteps that regression's root cause entirely: no bare `push`/unbalanced stack adjustment anywhere in this session's code, only ordinary `call`s (mark/release_to both flow through normal C calling convention) and the pre-existing `push rbp; mov rbp,rsp; and rsp,-16; ...; mov rsp,rbp; pop rbp` alignment-safe scratch-frame idiom already used elsewhere in this codebase (`bb_match_arbno.cpp`, `bb_match_capture.cpp`, `bb_match_defer.cpp`, `bb_subject.cpp`).

**Register census performed before touching anything (worth keeping — saves re-deriving it):** confirmed by direct grep-and-read, not memory: R12 = zeta frame pointer (per-BB payload repoint in ARBNO v2 COLLECTION roles 3/4/5, AND the Prolog callee/query-frame save/restore in `bb_callee_frame.cpp`/`bb_query_frame.cpp` — same convention, two call sites, not two conventions). R13 = scanner cursor (`bb_match_head.cpp`) among other uses (13 files total). R14/R14d = heaviest-used general working register (120 combined uses). R15 = scanner limit. RBX = GVA (confirmed by Lon). RBP = the alignment-safe scratch-frame idiom (5 files). `bb_create.cpp`'s `contract_regs[6] = {r12,r13,r14,r15,rbx,rbp}` confirms all six callee-saved registers already have SOME meaning somewhere (Icon coexpression CREATE saves the whole set for later resume) — there was no genuinely free second register for a "ZETA top" the way first proposed; resolved by keeping the top in memory (the existing `g_zls_cur`/arena-header-chain mechanism in `zeta_alloc.c`, already proven correct in isolation the prior session) rather than commandeering a register.

### ZC_COL_ARENA — rt_zcol_push's new arena-backed grow arm (pattern_match.c) — LANDED, verified

Replaces `realloc` with `rt_zls_alloc` for ARBNO v2's per-iteration COLLECTION growth: allocates a fresh, larger block on the SAME LIFO arena `rt_zls_alloc`/`rt_zls_release`/`rt_zls_mark`/`rt_zls_release_to` already use, copies the live elements over, leaves the old (now-unreachable-via-`*ptr_cell`-but-still-chained-via-header) block for a future `rt_zls_release_to(mark)` to reclaim rather than freeing it explicitly. **No per-block `GC_add_roots`/`GC_remove_roots`** — confirmed by reading `rt_zls_alloc` itself, the arena's rooting is address-range-wide (widens to hiwater on every alloc), not per-block, so a block living inside the arena is already GC-visible regardless of tag.

**Verified, not assumed:** full crosscheck byte-identical to session-start baseline in both modes after landing. New purpose-built test `test/snobol4/patterns/zb_arena_collection_grow.sno` (kept in tree) forces two real grows (capacity 4→8→16 across 10 iterations via `s = 'abcdefghij'; s POS(0) ARBNO(LEN(1) | LEN(1)) RPOS(0)` — plain `ALT`, not `FENCE`, since FENCE is outside this build's landed subset and would have been an unrelated confound); confirmed against the real SPITBOL oracle in both `--run` and `--compile`; gdb-confirmed (breakpoint + hit-count, not a guess) the grow branch fires exactly twice at the correct indices (i=4→nc=8, i=8→nc=16) with `memcmp`=0 between old and new block contents at each grow; confirmed the return-value register (`rax`) flows straight from `rt_zcol_push`'s return into the caller's `lea r12` with nothing stale cached in between (checked via `finish` + register inspection at the actual call site, not the C++ template source, which does not itself execute at runtime — an early mistake this session, caught and corrected before drawing conclusions from it).

**Confirmed still correctly guarded:** `sno_pat_v2_ok` (lower_snobol4.c ~446) already refuses nested ARBNO inside a v2 body specifically because "an element's prev_rZ would point into a REALLOC-MOVABLE outer collection" — this arm's grow-and-copy is exactly as movable as the old realloc arm was (both relocate the block), so this pre-existing guard continues to correctly apply; read and confirmed, not assumed compatible.

### IR_MATCH_RELEASE — new node owning the match-statement's SUCCESS exit — LANDED, verified

**Why a new node was needed, not a hook on an existing generic port:** `bb_goto.cpp` (the template behind every plain `IR_GOTO`, including a statement's `sJ`/`fJ` targets) is used for every goto in the entire program — hooking release_to there centrally would have fired on every goto, not just a match statement's own two exits. `IR_MATCH_HEAD`'s own ω already **is** the failure exit (wired to `fJ` directly by `sno_lower_match`, unchanged architecture) — a genuine, structural asymmetry: failure has one fixed choke point (the scanner's own exhaustion, naturally owned by HEAD), success does not (it's whichever pattern element the match happens to end on, varies per pattern shape). `IR_MATCH_RELEASE` IS that fixed point for success, added for exactly this, tied into the existing α/β/γ/ω discipline (per Lon directive this session: "keep templates clean, tie all code to α/β/γ/ω") rather than as an ad hoc wrapper — single α, no β/ω (cannot itself fail), reads the saved mark from its operand[0] (the statement's own `IR_MATCH_HEAD` node) via the SAME operand[0]-owner convention `IR_MATCH_ARBNO`'s non-owner phases (1/2/4/5) already use to read role 0's slot.

**Full registration checklist** (determined by reading a genuinely-live dispatched node, `IR_MATCH_ARB`, as the model — `IR_MATCH_ADVANCE` was checked first and found to be an inert, not-yet-wired precedent, a useful negative example not to copy): enum entry (`IR.h`), name-array entry (`scrip_ir.c`), TWO emit.cpp dispatch sites (the drive/operand-setup switch AND the actual emit-call switch — easy to miss one), the template `.cpp` itself, forward declaration (`bb_templates.h`), and TWO Makefile lists (`RT_PIC_SRCS` for `libscrip_rt.so`, and the `scrip:` target's own explicit per-file compile-rule sequence — confirmed these are genuinely two separate enumerations, not one shared variable, by tracing which `make` variable each line belongs to before editing either).

**`IR_MATCH_HEAD` slot widened WITHOUT changing its slot count** (zeta_storage.c): confirmed by reading `zls_grant`'s caller (`k += zls_grant(...) ` at ~line 190, multiplying the return value by 16 bytes per unit) that the return value IS the allocation-spacing unit, not the `zls_field` size argument — so simply widening the declared field size would NOT have grown the actual reservation and would have silently invited the next node's slot to overlap. Instead: confirmed the existing grant already reserved a full 16B quad while the template only ever used 4 bytes of it (an int32 counter) — 12 bytes of genuinely unused padding already sat there. Added `head.zeta_mark` (8B, `ZK_PTR_GC` to match the ARBNO-COLLECTION-pointer tagging precedent — though confirmed `ZK_*` is currently write-only everywhere in the tree, read by nothing yet, so this has zero behavioral effect today; tagged correctly anyway for whichever future consumer reads it) at `off+8`, inside the SAME 16B quad, `return 1` unchanged — the safest possible way to add a field, verified by reading the consumer before touching the producer rather than assuming size and count were the same thing.

### Failure-side gap found, then closed same session

**Found via direct probe** (a hand-written `.sno` forcing a genuine match failure): after wiring mark-at-α and release_to-at-success only, `SCRIP_ZLS_RELEASE_TRACE=1` showed exactly one `[ZLS-MARK]` and ZERO `[ZLS-RELEASE-TO]` for a failing match statement — a real, confirmed leak on every failure path, not a hypothetical. **Closed same session:** redirected both of `IR_MATCH_HEAD`'s scanner-exhaustion conditional jumps (`jg`/`jne`, previously targeting `ω` directly) through one new local label that calls `rt_zls_release_to` (reading the SAME `op_off+8` slot the α just wrote) before falling through to the real `ω` — one call site, not duplicated at each conditional jump. **Re-verified after the fix:** the same failure probe now shows matching MARK/RELEASE-TO addresses; the success-path growth test (`zb_arena_collection_grow.sno`) still shows correct MARK/RELEASE-TO and still matches oracle; full crosscheck re-run, still byte-identical to the session-start baseline in both modes.

### Verification method notes worth keeping (mistakes made and corrected this session, not to repeat)

- **A breakpoint on a C++ template's OWN source line does nothing at runtime** — the template function runs at COMPILE time (emitting x86 text/bytes); the emitted x86 is what runs. Caught this when a `gdb break bb_match_arbno.cpp:154` breakpoint never fired during `--run` — corrected by breaking on the actual runtime call boundary (`rt_zcol_push` itself, then `finish` + register inspection) instead.
- **`make libscrip_rt` can silently report "Nothing to be done"** when it thinks the `.so` is current, even after a source change with `ZCFLAGS` — leaves `--compile` executing STALE code with NO error. Caught by an unexpectedly-clean crosscheck immediately after a `ZCFLAGS` rebuild that should have changed behavior; fixed by `rm -f out/libscrip_rt.so` before every `make libscrip_rt` this session, not trusting make's own staleness check.
- **A "pre-existing failure" claim needs an actual stash/rebuild A-B comparison against the pre-session commit, not a name-pattern-match against a remembered fail-list.** Did this for the 3 benchmark EMIT-FAILs (mixed_workload, roman, string_pattern) and spot-checked 1 of 17 feature EMIT-FAILs (1116_data_overlap) this way — all confirmed byte-identical fatal messages before vs. after this session's changes, all pre-existing guard clauses in `lower_snobol4.c` unrelated to pattern-match internals, none introduced by this session.

### Crosscheck watermark at handoff (re-derive at next session start per this file's own standing doctrine, do not trust this line blind)

`--run` 252/276 (24 FAIL) · `--compile` 251/9/16 (9 FAIL, 16 SKIP) · DIVERGE=1 — **UNCHANGED from the watermark already recorded above in this same file's TOP PRIORITY RUNG section**; this session's landed changes do not move these numbers (confirmed identical, byte-for-byte diff of the crosscheck script's own output, before vs. after every edit this session, not just at the end).

### Known gap for next session, stated plainly rather than left implicit

**ARBNO's own re-entry across statement re-execution within a DEFINE'd procedure body has NOT been separately stress-tested against this specific change.** The prior session's `test/snobol4/patterns/zb_act_arbno_in_define.sno` (a minimal DEFINE-wrapped ARBNO probe, kept in tree) exists for exactly this kind of check but was not re-run against this session's mark/release_to wiring specifically — the aggregate crosscheck passing does not by itself confirm this particular interaction, since it's a narrow, purpose-built case that may not be otherwise exercised by the broader corpus. Flagging explicitly so a future session checks it directly rather than inferring coverage from the aggregate pass.

**Extending this same mark/release_to discipline to procedure-call (graph-scale) boundaries — the ORIGINAL frame-active prologue/epilogue goal from the prior session's addendum — has not been attempted this session.** This session deliberately scoped to the STATEMENT level only (a smaller, more self-contained change than the whole-graph prologue/epilogue the prior session regressed on); whether the same alignment-safe approach (ordinary calls, no bare unbalanced push) generalizes cleanly to the graph/procedure-call boundary is open, not yet investigated.
