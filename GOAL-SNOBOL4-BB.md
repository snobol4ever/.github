...
## 🟢 CURRENT FRONTIER — **PB-RB-3 ✅ RESTORED + REG-0 ✅** (`bb_match`/`bb_subject` rebuilt on the ratified registers, SCRIP `8cd05c1`); **PB-RB-4 invariant ALT/CAT ✅ probe-proven** (`f166940`, NO STITCH machinery needed for the invariant case); **CONV-seam ALT aux-encoding fixed** (`c3d5547`) — pat-bb probes **6/6** — next: lowered-CAT γ-chain gather (driver-side) → m4 ALT/var-CAT TEXT path → REG-RO + REG-FENCE TIER2 (the flat chain is now gate-covered by the probe suite, lifting the "ungated path" deferral half)
