...
# ▶▶▶ NEXT SESSION — START HERE (handoff 2026-06-24, session 20)

**FZ-2 LANDED (SCRIP, local — push BLOCKED on credential).** `string_pattern`'s stored `PAT = BREAK(',') . WORD ','` now constant-folds to a sealed `IR_REF_INVARIANT` blob — NO runtime builder call. Gate MET: mode-3 (`--run`) AND mode-4 (`--compile`) output byte-identical to `string_pattern.ref`; `bb_build_break_cap_lit_blob` count in `string_pattern.s` = 0 (was 1); the `.s` shows the matcher graph sealed once under `flat_c0_α` + a store box `lea rsi,[rip+flat_c0_α]` → `call rt_gvar_assign_pat`. Bench OK=15 FAIL=0 CRASH=1 (the 1 = pre-existing eval_dynamic OOM) — zero new failures. Six pieces landed: (1) new template `bb_ref_invariant.cpp` (store box, β/ω via `x86_pair_loop`); (2) decl in `bb_templates.h`; (3) `case IR_REF_INVARIANT` in `walk_bb_node` (emit_core.c); (4) `flat_drive_ref_invariant` now reads the matcher entry from the **aux channel** (was `operands[0]`) to match the pre-pass cache key; (5) lowerer `lower_assign` produces `IR_REF_INVARIANT` (name in `IR_LIT.sval`, kids-channel matcher CAT via `sno_freeze_break_cap_lit_ir` on aux) for the narrow `BREAK(qlit) . var qlit` shape only; (6) Makefile registers the template in `RT_PIC_SRCS` + the `scrip:` target. KEY SIMPLIFICATION: the goal's speculative `x86("load_fn_or_lbl")` encoder was NOT needed — the existing RIPSEAL `lea` form `x86("lea",reg,"[rip + __]",ptr,lbl)` → `x86_load_ro` already does binary-baked-ptr vs text-`lea [rip+lbl]`. The FZ-1 freeze functions (`sno_freeze_break_cap_lit_bin/text`) are NOT on this path (pre-pass seals the aux child instead); they are now dead — FZ-4 may retire them.
...
**FZ-1 LANDED (SCRIP `3b13020`).** Factored `sno_break_cap_lit_graph()` (shared BREAK.cap.lit matcher-graph constructor) out of `bb_build_break_cap_lit_blob`; added `sno_freeze_break_cap_lit_bin` (mode-3 compile-time seal, guards `g_emit_cfg`/`g_frame_active`/`g_flat_node_id`) and `sno_freeze_break_cap_lit_text` (mode-4 text seal). Behavior-neutral: `string_pattern.s` byte-identical. State-independence proven: sealing with `g_flat_node_id=0` vs `4242` yields byte-identical 4096-byte blobs. `IR_REF_INVARIANT` wiring already exists in both pre-passes (`pre_build_children` binary + `pre_build_children_text`) — FZ-2 only needs to (a) produce `IR_REF_INVARIANT` from the lowerer and (b) add the main-walk `case IR_REF_INVARIANT` store box.
...
**▶ NEXT MOVE: FZ-3** — ALT-of-literals freeze for `pattern_bt` (`('aaa'|'bbb'|'ccc'|'ddd') SPAN('abcd').W`). Seal the 4-way ALT + SPAN + capture to one blob via the same `IR_REF_INVARIANT` path FZ-2 built (lowerer detects the ALT-SEQ shape, builds the kids-channel matcher graph, attaches to aux; pre-pass + store box already generic). **`pattern_bt.ref` is COUNT-ONLY (null PAT null-matches → 500000 regardless), so it CANNOT validate the match — add a result-sensitive `W` output and regenerate the ref from x64 `sbl -b` (this step DOES need the x64 token) or a hand-computed `W`.** FZ-2's `sno_freeze_break_cap_lit_ir` is the template to generalize; the store box + pre-pass need no change (they key off `IR_REF_INVARIANT` + aux, shape-agnostic).

**▶ FZ-2 wire `IR_REF_INVARIANT`** — ✅ DONE (session 20). `string_pattern` folds to a native blob with no runtime builder call.
...
### STEPS
- [x] **FZ-2 — `IR_REF_INVARIANT` store + DEFER fetch.** ✅ LANDED (session 20, local). `PAT = BREAK(',') . W ','` lowers to a sealed matcher blob + `IR_REF_INVARIANT` storing the head as DT_P (no `bb_build_break_cap_lit_blob`). Gate MET: mode-3 AND mode-4 output == `string_pattern.ref`; builder call absent from `string_pattern.s`. See START-HERE header for the six pieces.
...
- [ ] **FZ-4 — retire dead fused builders.** Remove now-unused per-shape builders in `bb_pat_build.cpp` (keep ONLY the structural-variance STITCH path). Gate: `test_crosscheck_snobol4.sh` byte-identical (zero regressions).
