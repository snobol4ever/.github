...
```
Then `bb_build_flat` emits the real `bb_match_len` body (`r14d += I`) into the blob. Verify: `IR_PAT_LEN` flat dispatch (emit_bb.c:2855 `case IR_PAT_LEN: FILL(...)`) and that it is in `is_pat_chain_elem` (emit_bb.c:2486 — it is) so slot/chain handling is consistent; check `bb_slot_alloc24`/`op_off` expectations for the leaf. Gate: `r1` ⇒ `W=CD` == oracle (tri-probe `/tmp/probe.sh`). The double-emit fix (`da8dfb7`) + this node swap should together make `r1` green.
