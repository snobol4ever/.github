...
## 🟢 CURRENT FRONTIER — **lowered-CAT γ-chain gather ✅** (SCRIP `6ffbae3`, pat-bb probes **7/7**: `gather_lowered_cat_arms` recovers the wire_seq arms from MATCH's entry — `wire_seq` sets NO sidecar and `lower2_match_entry` hands MATCH **entry[0]**, not the CAT — driving them through the arms-mode `flat_drive_cat_arms`, the exact `flat_drive_alt` aux-fallback shape) + **m4 var-subject scan ✅** (`54ab640`, rung **M4 0→1**, 038 flips: the 038-class bombs were a LOWERER omission — `v_scan`'s no-repl branch never set `sc->sval`, one-line fix, m2/m3 proven neutral) — next: **CAPTURE BOXES in the flat chain** (the REAL m4 rung unblocker: 17/18 remaining rungs carry `. V` captures, which still ride the brokered `child_cache_get` path in `walk_bb_flat` IR_PAT_ASSIGN_IMM/COND) → 057-only TEXT routing slice (var-subject `bb_subject` arm + synthesized SUBJECT/MATCH TEXT emission in `flat_drive_scan_stmt`; alone it flips only 057) → REG-RO + REG-FENCE TIER2. NOTE: the dense-nid duplicate-`bb%d_α` tail is ALREADY SOLVED structurally (`bb_fill_alpha` allocs fresh `bb%d_α` per FILL under `g_sno_m4_dense_nid`) — no work there.
