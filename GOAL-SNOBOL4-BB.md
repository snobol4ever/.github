...
- [ ] **MON-RE-5 — CALL/RETURN taps on function boundaries.** DONE (SCRIP `93990cc`, 2026-06-26). Added `mon_emit_call_bin(fname)` / `mon_emit_return_bin(fname, retval)` to `rt.c` (declared in `core.h`): these temporarily raise `kw_ftrace=1` to bypass `comm_call`/`comm_return`'s ftrace gate, emit the MWK_CALL/MWK_RETURN event, then restore `kw_ftrace`. Wired into `rt_call_named_proc`, `rt_call_proc_direct`, `rt_call_named_proc_sl` — all gated on `g_monitor_bin`. Also in this commit: (a) removed 4 redundant `mon_emit_value_bin` calls from `rt_gvar_assign_{str,int,var,descr}` — root cause of double VALUE emission: `NV_SET_fn → comm_var` already emits MWK_VALUE via monitor-fd path, explicit `mon_emit_value_bin` caused duplicate; (b) removed `SCRIP_TRACE=1`/`SCRIP_FTRACE=1` from scrip participant in `test_monitor_3way_sync_step_auto.sh` — these set `kw_trace=1` causing `NV_SET_fn` to call `comm_var` twice per assignment, doubling VALUE events. Gate: crosscheck `--run PASS=149 FAIL=112`, `--compile PASS=170 FAIL=85`, byte-identical to HEAD. Monitor on `/tmp/mdiv.sno` now: steps 1+2 AGREE (LABEL stno=1, LABEL stno=3), step 3 DIVERGES with `SPITBOL @3 CALL setit` vs `SCRIP LABEL stno=2`. **OPEN PROBLEM:** the CALL tap in `rt_call_proc_direct` fires correctly BUT the call to `setit("a","hello")` in the main body is not going through `rt_call_proc_direct` — the assembly shows the main flat body jumps directly into `setit_α_body` (the proc body native function) bypassing the call dispatch entirely. This is the SEMANTIC bug: the IR_CALL for the main program's invocation of `setit` is emitted inside the proc body itself rather than at the call site. The CALL tap therefore never fires before the proc body LABEL tap. This is the same root cause as `1010_func_recursion` failing — function call dispatch is broken in the main flat body.
...
# ▶▶▶ NEXT SESSION — START HERE (session 29, next date)

**DCR-2 LANDED (SCRIP `a5ed5be`, session 28, 2026-06-25).** Codegen-side direct proc resolution. `src/driver/scrip.c` only — activates dormant scaffolding: `proc_collect_reset/graph` over main+proc bodies sets `g_proc_direct_active=1`; emits `__proc_names`/`__proc` bss int64 table + `rt_proc_table_fill` preamble call. Call sites go from `lea rdi,name; call rt_call_named_proc@PLT` → `mov rdi,[rip+__proc+k*8]; call rt_call_proc_direct@PLT`. Fallback by-name kept for `$FN`/OPSYN/runtime-DEFINE. Gate: PASS=171 FAIL=84, fail set byte-identical, both-medium clean. 5 benchmark `.s` updated (fibonacci/func_call/func_call_overhead/mixed_workload/roman). A/B: func_call 5208→5138ms (~1.3%), fibonacci 2040→1987ms (~2.6%) — honest small win; DCR-1/2a/2-cells already made by-name O(1), so DCR-2 removes only the residual cache-lookup hash+compare. Remaining call-cluster cost is the save/restore protocol (rt_name_save_push/rt_name_restore), which is the genuine next performance lever.
