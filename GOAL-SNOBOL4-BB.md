...
- [ ] **M4-FENCE-P — FENCE(P) FUNCTION FORM DROPS ITS ARGUMENT (root cause found 2026-06-13; ALL-MODE bug, not m4-only).** The cited cluster splits into TWO root causes:
  - **(A) FENCE(P) function form is unimplemented — aliased to the keyword.** `lower_snobol4.c` `case TT_FENCE` (the pattern-subgraph builder, ~line 379-380, AND the statement-level builder ~line 130) allocates a nullary `IR_PAT_FENCE` and **ignores `t->c[0]` (the sub-pattern P)**. So `FENCE(LEN(1)|LEN(2))` builds the bare FENCE keyword (matches null, consumes nothing) and the inner pattern is discarded. Decisive proof: `X='AB'; X POS(0) FENCE(LEN(2)) RPOS(0)` → SCRIP M2 "cursor at 0" / sbl "cursor advanced 2". This breaks **061 ✅(passes by luck — seal+fail expects failure)**, 100/101/103 in ALL THREE MODES (M2 oracle included). The keyword `IR_PAT_FENCE` (interp `IR_interp.c:4270`; emit `bb_match_fence.cpp`) is correct and stays — it saves δ on α/γ, restores δ + →ω on β (backward re-entry = seal). SPITBOL Ch.19 p.207: `FENCE(P)` = P, except P's alternatives are visible only moving FORWARD; backtracking INTO P seals them. SPITBOL Ch.18 p.204: keyword FENCE = null match, fail on retreat.
    **RUNG PLAN (multi-commit; M2 oracle FIRST so it's the gate):** (1) new IR kind `IR_PAT_FENCE_P` (or `IR_PAT_FENCE` carrying a child subgraph head in a slot — pick per ONE-IR-ONE-LOGIC; a distinct kind is cleaner). (2) LOWER: lower P's subgraph normally (alternatives live); wire P.γ → fence-success; install a SEAL on P's backward-entry so that any later element backtracking into the fenced region jumps to the fence's ω WITHOUT re-examining P's alts (mirror keyword `bb_match_fence` δ-save/seal, but gating P's entry rather than a null node). (3) M2 interp arm in `IR_interp.c`. (4) M3/M4: extend `flat_drive_fence` (`emit_bb.c:366`) to splice a child subgraph (today it drives the nullary node); `IR_PAT_FENCE` is in the driver-owned-kinds set (`emit_bb.c:853`) so walk from JOIN, `ir_skip_alt_arms` first. (5) gate on 100/101/103 (all modes) + 061 hold + the via-var/nested siblings (105/107/132-134) + arbno-of-fence (116-119,142). Probe-first vs sbl each rung; m2==m3==m4 co-equal.
  - **(B) 062 — M4-only outer-alt-after-fence.** `X='bx'; X ('a'|'b') FENCE('x')` → M2/M3 "outer alt worked", **M4 "failed"**, sbl "outer alt worked". Independent of (A) (FENCE('x') has an invariant literal arg, but still hits the same drop-P path AND an m4 outer-backtrack wiring bug). Re-probe after (A) lands — may be subsumed or may be a separate β-wire fault in the m4 cat/alt outer arm.
