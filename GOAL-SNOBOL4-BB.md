...
## ⚠ OPEN SESSION HANDOFF (Claude, 2026-07-08 ~02:45 UTC) — α-DEF UNIFICATION (ONE-WAY-OUTPUT), WIP ON BRANCH `alpha-def-unification-wip`, SCRIP TREE THERE IS BROKEN BY DESIGN-GAP FOUND MID-FIX
Lon directive this session: the driver defining α's byte position directly (`emit_label_define_bb(lbls[i])` in `codegen_flat_chain_body`) is a second output path violating ONE-WAY-OUTPUT (`x86()` handles TEXT+BINARY internally; `x86("def","β")` already does this correctly). Landed on the branch: driver call removed; all live templates now emit their own `x86("def","α")` (149 mechanical conversions of the vestigial TEXT-only `x86("label",_.lbl_α)` + 7 hand conversions: goto/conjunction/enter_init/gen_scan/repalt-via-`bb_repalt_clear`; gather+mapgrep were already correct; cat/callee_frame/match_advance/match_retry/scan_splice_empty/case_arm/every/ite verified dead or not built — untouched per "live templates only"). **ROOT CAUSE FOUND, NOT YET FIXED — why the branch still aborts on `hello.sno` (`unresolved forward reference: xchain0_n1_α`):** `bb_fill_alpha` fills `_.lbl_α_p` (what `x86("def","α")` defines) from an 8-slot recycled ring buffer named `"bb%d_α"` — a **different `bb_label_t` object** than `lbls[i]` (`"xchain%d_n%d_α"`), which is what every inbound jump actually resolves against. β/γ/ω never had this bug because `emit_drive` receives them as parameters from the one real call site; α alone was never threaded through and got a decorative ring-buffer stand-in, papered over until now by the driver's (just-removed) direct define. Fix is fully scoped, small, and verified against the code: 5th `emit_drive` parameter carrying `lbls[i]`; `DRIVE_FILL`/`FILL` macros take it and set `lbl_α`/`lbl_α_p` directly (drop `bb_fill_alpha` call); 71 uniform call-site edits (70 in `emit_drive`'s switch + 1 in `flat_drive_repalt`, which already has `lbls` in scope). Full detail, environment notes, and cleanup list (a `CLAUDE_TMP_TRACE` diagnostic still sits in `bb_assign_global.cpp`) in `HANDOFF-2026-07-08-alpha-def.md` (this repo). Acceptance floor after the fix: watermark below, unchanged — the whole change must be behavior-neutral. **Why this rung exists at all:** a correctly-targeted template-level α def is the missing splice point the BB-OWNED-ζ self-load hook (§5h `ARCH-ZETA-LOCAL-STORAGE.md`, D13 in `GOAL-IR-IMMUTABLE-EMIT.md`) needs; main is untouched and stays at the watermark.
