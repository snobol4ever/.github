**SESSION WATERMARK — 2026-06-10 · Sonnet 4.6 · OLD SNOBOL4 LOWERER DELETED (SCRIP bdb18d7) · NL IS THE ONLY SNOBOL4 LOWERING PATH · D7-RB-1 still NEXT.** Lon directive executed: src/lower/lower_snobol4.c + lower_pattern.c + lower_snobol4_internal.h DELETED; live --dump-sno transpiler preserved verbatim as src/lower/tree_to_sno.c; lower.c stripped of ALL SNOBOL (SNO/SCO/REB dispatch, ROLE_PATTERN, TT_SCAN, 4 pattern entry wrappers) — its v_* ground-zero family is NOT SNOBOL and was left untouched per Lon (soon-to-be-dead code, not ours); prove_lower.c SNOBOL tests removed → new baseline 38P rc=0 (3 Prolog FAILs pre-exist at parent). FLOOR (honest, NL-only — SCRIP_NL=0 fallback no longer exists): smoke 7/7/7 HARD · pat-rung M4 16/19 (050/051/054 'no match' = pre-existing b11a963 regression, FIX-FORWARD ONLY) · M2/M3 18/19 (053) · fence HARD · --dump-sno OK. NEXT = execute D7-RB-1 exactly per HANDOFF-2026-06-10-SONNET46-SNOBOL4-BB-D7-RB1-DESIGN.md (all bytes proven there); it targets 053 and plausibly 050/051/054.
...
1. **D7-RB-1** — add the BINARY arm to `x86_reg_disp32_lea64` (+ the disp32 load/store siblings) in `x86_asm.h` (today TEXT-only at line ~366; copy the `MEDIUM_BINARY ? x86_Lrec(...)` pattern from the `[r13+rcx]` form at line 155); in binary mode pass a NUMERIC proto-length immediate (label diffs can't encode at emit time). Convert lit/alt/dtp_assign to the RT calls; add the 3 m2 interp arms. PROOF: 053/050/051 PASS m2==m3==m4 → `b`/`dog`/`banana`.
2. **D7-RB-2** — convert unary_i/unary_s/nullary/arb/cat identically (each its own commit, frag held at 3-field/24B) → `ins*` count in pattern builders drops to 0; re-probe REM/FAIL/SUCCEED/ARB/SPAN/ANY/LEN/CAT across ALL THREE modes (today m2/m3 empty).
