...
- [ ] **W-1 · FRAME THE CONSTRUCTS (construction; SCRIP_WHOLESALE minted, default OFF)**
  ⚠ **STEP 1 WIP — SCRIP `50588277`** (SN4 sessions, 2026-08-03). Parts 1-4 structural + 2 more edits this session; gate-OFF byte-identical confirmed (m3=280/26/11 · m4=274/32/10+1L). Gate-ON still SEGV. **OPEN BUG:** crash `rip=0x500000002` (= "hello" DESCR, DT_S slen=5) AFTER correct program output on any pattern with ≥2 blob members (e.g. `X POS(0) LEN(3) :S(YES)`). Single-member patterns pass. **EDITS THIS SESSION (SCRIP `50588277`):** (A) `IR_MATCH_BEGIN` dispatch (emit.cpp:973): gated fc-hook `op_fc_wbytes=24` on `!op_zw2` — mech-2 `push rbp` subsumes the CAS sentinel, no separate 24B sub. (B) per-node zdepth (emit.cpp:829): when `g_zd_uzwr && op_mech2_kc>0`, `op_zdepth = 8+Kc` so blob-closure FR reads address `[rsp+off+8+Kc]=[α_base+off]`; line 835 `!g_zd_uzwr||!op_mech2_kc` guard keeps non-mech2 nodes at `g_zd_k`. **NEXT ACTION:** MONITOR-FIRST on `X POS(0) LEN(3) :S(YES) YES OUTPUT="ok" END`. The crash is post-output → investigate `match_begin_β` give-up path's `mov rsp,[rsp+64]` (zls2_mark restore): under mech-2 with `sub rsp,Kc` active at β entry, `[rsp+64]` addresses inside the blob carve, NOT the pre-push rsp level. This restore may need to be gated behind `!op_zw2` since mech-2's `pop rbp` whack already restores rsp. Also check: does `rt_outer_call`'s `pop r12; ret` fire after `call exit@PLT` (exit shouldn't return), or is the crash inside an atexit/destructor reading a stale r12 or CAS entry.
