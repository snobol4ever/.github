**SESSION WATERMARK — 2026-06-11 · Sonnet 4.6 · D7-RB-2b LANDED (NL CAT ROUTING, SCRIP `906adfe`). NEXT = D7-RB-3 (LEN value-assign lowering gap).** Buildable TT_SEQ in `lower_assign` now routes to pairwise left-assoc `IR_PATTERN_CAT` chain + `DTP_ASSIGN` (4 helpers: `sno_leaf_buildable`, `sno_seq_buildable`, `sno_seq_has_pat_leaf`, `sno_build_leaf_ir`). Admitted leaves: QLIT, TT_VAR predefined-name (REM/ARB/FAIL/SUCCEED/FENCE/ABORT), unary-s(QLIT) (SPAN/ANY/NOTANY/BREAK/BREAKX), unary-i(ILIT) (LEN/POS/RPOS/TAB/RTAB). Guard `sno_seq_has_pat_leaf` requires ≥1 non-QLIT leaf — all-QLIT concat stays on ASSIGN_CONCAT path (smoke `concat` verified). PROBES (m2==m3==m4==sbl): LIT-LIT · SPAN-LIT · LEN-LIT · BREAKX-LIT · ANY-LEN · SPAN-LIT-SPAN · POS-LIT · CONCAT-nonreg · FAIL-lone all PASS. ⚠️ FINDING: `P='A' ARB 'C'` IR graph is correct (PATTERN_LIT/ARB/CAT chain produced, DTP_ASSIGN fires) but PAT_DEFER→rt_dtp_run is one-shot (B10 landmine) — ARB β-regen in built patterns awaits D7-RB-3 / B10 DEFER-IN-BUILD. FLOOR: smoke 7/7/7 HARD · pat-rung M2 19/19 · M3 19/19 · M4 16/19 (050/051/054 pre-existing b11a963) · corpus BYTE-IDENTICAL 172/156/144 both sides, FAIL-set diff EMPTY · fence HARD.
---
...
4. **D7-RB-3** ⬅ NEXT — close the `P=LEN(3)` value-assign lowering gap (unrouted in all modes).
