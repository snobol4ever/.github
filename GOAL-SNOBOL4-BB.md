...
- [ ] **ED-6 — extract the remaining clean IR-query helpers (optional follow-on to ED-5).** Same profile (pure, IR.h+libc only), into `src/opt/ir_query.{c,h}` or a sibling: `ir_is_generator_kind`, `gz_node_bounded`, `resolve_ite_entries_em`, `descr_chain_terminal`, `subchain_node_is_generator`, the `gvar_chain_*` resolve/stno helpers. ⚠ `is_pat_chain_elem` is ALSO pure but has TWO forward declarations (emit_bb.c ~267 + ~382) plus 11 call sites — extractable but the move must delete BOTH forward decls and let the header replace them; do it as its own sub-step, not bundled. ⚠ RE-VERIFY each candidate's purity (grep `g_emit`/file-static reads) before moving — the ED-5 scan was point-in-time; emit_bb.c shifts. Method = ED-1/ED-5's exactly (dual-list Makefile, crosscheck byte-identical, .s byte-identical). Maybe -120 more lines off emit_bb.c.
