...
1. ⭐ **FINISH THE LEAK CLASS — SUBJ_CELL (149 leaks) and `fc_call_ok` (77 leaks).** ⚠ **NOT the same easy shape as ZLEAK-1/2**: SEQ-CELL had zero armed customers so opt-in was free; these arm 44 and 58 graphs respectively and all differ, so a polarity flip would DELETE a live feature. The fix is the verdict reaching the planner — **the natural home is the PS-3 s153 LATE PASS `zls_build` already has** (invoked from `emit_chain` at each graph's emission entry, where a per-graph verdict IS available). Until then every measurement under those envs conflates regime with leak — the likeliest reason the D2 autopsy chased a weld that was never the fault.
