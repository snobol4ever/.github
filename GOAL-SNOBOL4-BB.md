...
**NEXT RUNGS (in order):**
1. **XFAIL.compile birth** — UNBLOCKED this session: `-o` full capture landed (SCRIP `5c7d09d9` — the 06f form sank only emit_textf; the emit_chain body writers still took stdout literals, so `-o` files got 27 preamble lines and the body spilled; fix = path-scope `_out` at both compile paths' four emit_chain sites; verified byte-identical to shell redirect on simple AND proc-arm programs, zero spill, `-o`-built binary REF-exact; `output_path`=NULL is byte-identical by construction, xc chunk-C proof, the lone m4 name 183 = 4/4-PASS isolated, the defer-class bistable flake).  Birth command, knobs verified in the harness header: `MODE=compile BASELINE=1 bash corpus/probe/bb/run_suite.sh`.
