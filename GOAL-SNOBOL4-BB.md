...
- [ ] **PB-0 RECON CLOSE-OUT**: (a) verify dynamic primitive args (BREAK(expr) class) producers sit PRE-HEAD (stage-2 compliant) — any inside the retry loop are the same disease, list them; (b) census PATREF-minting corpus programs (compile sweep, grep n\*_match_patref); (c) map what consumes the 1257 bookkeeping (sno_fz_mark_defer marks, seal 1/2 classes, pat_static) on PATREF nodes — what breaks if unminted; (d) whole-pattern lone-ref case (`S ? P`): confirm it routes through 1257 or find its site.
