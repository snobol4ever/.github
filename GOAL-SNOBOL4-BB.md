**SESSION WATERMARK — 2026-06-12 · Sonnet 4.6 · 027 POW WIP (partial — m2 fixed, m3/m4 path not yet executing). NEXT = debug 027 POW m3/m4: add `fprintf(stderr,"POW-BRANCH\n")` probe in `flat_drive_gvar_assign_binop` POW arm to confirm path, fix why branch not taken, then verify `OUTPUT = 2 ** 8` → 256 m2==m3==m4==sbl; run full gate battery; commit.** D7-RB-3 confirmed already done (P=LEN(3) works all 3 modes — note predated D7-RB-2 landing). Changes this session (NOT YET COMMITTED — gates green but 027 still failing m3/m4): (1) `lower_common.c` `binop_apply` BINOP_POW now calls `POWER_fn(lv,rv)` → m2 `OUTPUT=2**8` gives 256 not 256. (root cause: old code always returned DT_R). (2) `bb_binop_gvar_arith.cpp` new `bga_pow_ok()` arm: LIT+LIT POW → construct INTVAL DESCRs in rdi:rsi/rdx:rcx, call POWER_fn, push rdx, load name into rdi, mov rax→rsi, pop rdx, call rt_gvar_assign_descr, jmp γ/β/ω. (3) `emit_bb.c` `flat_drive_gvar_assign_binop`: new POW early-return branch before UNOP fold — detects `c0->op==IR_BINOP && ival==BINOP_POW && LIT_I+LIT_I && sval` → sets op_sa/op_sb/op_ival/op_sval/op_name1=0/op_name2=0/op_off=alloc → promotes c0 to IR_BINOP_GVAR_ARITH → EMIT_PAIR_FILL → bga_pow_ok() arm. DIAGNOSIS: the branch is coded and compiles clean but `;[walk_bb_node: kind=7 unhandled]` still appears in .s → branch not executing or bga_pow_ok() predicate failing. FLOOR: smoke 7/7/7 HARD · pat-rung 19/19/19 · fence HARD (unchanged — no regressions).
---
