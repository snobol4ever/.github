...
- [/] **M3-NATIVE-4** — per-language bring-up + corpus parity. SNOBOL4 first.
  ...
  - [x] **SBL-BOMB-STUB-ESCAPE-FIX ✅** (Opus 4.7, 2026-05-28, one4all this commit). Prophylactic cleanup of the 5 remaining `\\x` BOMB-stub sites flagged in the prior step's handoff. Each was a placeholder fallthrough (`jmp γ; jmp ω` 2-jmp or `jmp γ; lbl_β: jmp ω` 3-jmp pattern) emitting `5C 78 ?? ?? ?? ?? 5C 78 …` instead of `E9 ?? ?? ?? ?? E9 …`. None reached at runtime in any passing test — the 5 sites are: `bb_arbno.cpp:23` (no-child fallthrough), `bb_pl_alt.cpp:23` (n<=0 fallthrough), `bb_pl_call.cpp:41` (degenerate-call fallthrough), `bb_pl_choice.cpp:42` (n<=0 fallthrough), `bb_to.cpp:65` (dynamic-operand passthrough TODO). One-line `str_replace` each. `bb_binop_gen.cpp:120` was already cleaned by Sonnet 4.6's IBB-6 (`3aa200cd`) — that commit replaced the placeholder with a full odometer slab. Latent landmine eliminated: if any of these paths ever becomes reachable, it would have SIGSEGV the same way SPAN/ARB did. **Gates: G1=13/13 default+native, G2=39, G3=178/280, G4=251/280 (+1 marginal — likely the bb_to dynamic-passthrough corrupt stub now emits clean fallthrough for some default-mode pattern), native=195/280, rungs M2=19/M4=15, all five language smokes green, FACT=0, audit GATE OK. Zero regressions.** `grep -rE 'bytes\([0-9]+, ?"\\\\x' src/emitter/` now returns empty across all template directories — the double-backslash bug class is closed.
