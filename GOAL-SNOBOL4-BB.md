**SESSION WATERMARK — 2026-06-12 · Sonnet 4.6 · 027 POW DONE (m2==m3==m4==sbl; SCRIP 6da0aa2). NEXT = next open M4-BUILTIN or corpus rung per the bug ladder.** Root cause (3-layer): (1) flat chain BFS collected BINOP as standalone node; IR_BINOP walk_bb_node had no POW arm → rt_bomb. (2) flat_drive_gvar_assign_binop POW arm passed c0 (BINOP) to EMIT_PAIR_FILL; walk_bb_node overwrites g_emit.op_sval from IR_LIT(BINOP).sval=NULL. (3) g_emit.op_ival clobbered to ASSIGN.ival=0=BINOP_ADD → bga_ok() fired with bga_pow_ok() → duplicate labels. Fix: spoof ASSIGN node (not BINOP) to IR_BINOP_GVAR_ARITH in both POW arms; op_kind="POW" used as discriminant (not clobbered by walk_bb_node); bga_ok() excludes POW. GATES: smoke 7/7/7 HARD · pat-rung 19/19/19 no-SKIP · fence HARD.
---
