...
**SESSION WATERMARK — 2026-06-15 · Claude · SCRIP (LOWER operand single-channel, phase 1+2) + `.github`. Lon mandate: "ONE way — get rid of the AUX method, use ONLY operands[], then DELETE the aux variable." Phase 1 LANDED `d2fe4a0`: SNOBOL4 `lower_expr` binop arm now `ir_operand_push(op,lr/rr)` alongside aux. Phase 2 LANDED `6256c69`: SNOBOL4 `lower_expr` unop arm now `ir_operand_push(op,orr)` (was NEITHER aux nor operands — activated the dormant bb_child0 literal-fold arm at emit_bb.c:2966-2977; verified -A/-5/-A+2 → -5/-5/-3 both modes vs sbl). Gates green both commits: smoke M3/M4 7/7 · pat-rung M4 19/19 0-SKIP · fence TIER1=TIER2=0 (M2/M3 056/057 fails PRE-EXISTING, verified via stash). aux writes RETAINED on purpose — readers still depend on aux, so deletion is the staged rung OPSINGLE below, NOT done yet. **BENCHMARK INVESTIGATION (PB-BENCH-1, no codegen change — diagnosis only, committed `67ac25b4`):** attempted to green ONE benchmark; found NONE is close — all 16 gate on the single `LT(cond) value` concat-assign idiom (minimal repro `/tmp/pb/mini.sno` = `N=LT(N,5) N`, oracle `result: 5`). Post phase-1/2 the bomb MOVED from `concat: no parts` → `bb_gvar_assign int-binop op_a_slot==-1` (bb_gvar_assign.cpp:49), root-caused to emit_core.c:335 `bb_slot_get(op_a)`==−1 because the concat IR_BINOP is never slot-promoted in the gvar flat chain (no concat arm beside op_is_arith/pow/rel at emit_bb.c~1512). This CONFIRMS LOWER is now correct; remaining gap is PURELY the emit-side concat slot-producer = PB-BENCH-1 defect (A). Both native modes share templates → both bomb. fibonacci/eval_dynamic/string_manip "timeouts" are NOT near-wins (fibonacci errors at FIB(20)). Did NOT start the slot-producer — too large to land green in remaining context; a half-done one = red build. NEXT (priority order): (1) OPSINGLE = delete aux (fully pre-staged below), OR (2) PB-BENCH-1 emit concat slot-producer → first green benchmark. Either opens cold.**
