...
## 🟢 CURRENT FRONTIER — **CAPTURE BOXES in the flat chain ✅** (SCRIP this commit, pat-bb probes **8/8**: `flat_drive_capture` de-brokers IR_PAT_ASSIGN_COND/IMM to flat-inline emission — SAVE piece stores δ=r14d into a driver-claimed ζ-slot, child emitted INLINE via `walk_bb_flat(ch, cap_γ, lbl_ω, lbl_β)` so the construct's β IS the child's β (the lowered topology: child in `->α`, child-γ back-edge inert under label-driven walk), CAP piece at `cap_γ` calls `rt_cap_assign_cursor(varname, saved, δ, is_imm)` — the EXISTING pattern_match.c helper whose `g_rt_dcap_active` branch already gives COND its overall-success-deferred commit and both their per-retry re-record, SPITBOL ch.13/18; one `bb_pat_capture.cpp` x86() template, THREE sub-kinds on `op_ival` {0=SAVE,1=COND,2=IMM}, the bb_match multi-emission model; prebuild ASSIGN arms now recurse-through instead of `bb_build_brokered`, ARBNO/CALLOUT untouched; probe `probe_pb_rb_5_capture` proves COND-over-LIT **and** IMM-over-lowered-ALT assign the matched substring in the m3 JIT chain, disasm-verified stackless `[ζ+off]`) — next: **the m4 SCAN ROUTING slice** (the rungs still bomb BEFORE walk_bb_flat: `flat_drive_scan_stmt` must synthesize SUBJECT/MATCH TEXT emission + the var-subject `bb_subject` arm for capture-bearing patterns; with captures now inline this slice is what flips the 17 `. V` rungs) → REG-RO + REG-FENCE TIER2 (r10 20→**22**: the CAP piece's push/pop guard, same class as defer/break, dies with the family at REG-RO). NOTE: the dense-nid duplicate-`bb%d_α` tail is ALREADY SOLVED structurally (`bb_fill_alpha` allocs fresh `bb%d_α` per FILL under `g_sno_m4_dense_nid`) — no work there.
