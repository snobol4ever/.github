**SESSION WATERMARK — 2026-06-13 · Sonnet 4.6 · SCRIP=9f0d809. This session (1 commit): THREE M3/M4 PARITY FIXES. (1) gvar_seq_flatten IR_LIT_I — integer literals in concat (OUTPUT = 42 ' items') lowered op_parts_n=0 → BOMB; added slot-safe IR_LIT_I arm (snprintf ival→decimal in g_seq_int_bufs[slot], tag=0). (2) rt_call_arr APPLY_fn fallback — SNOBOL4 core builtins (REMDR, DATATYPE, DUPL, ARRAY, TABLE, CONVERT, DEFINE, etc.) registered via register_fn/DEFINE_fn in core.c were invisible to rt_call_arr (only consulted try_call_builtin_by_name); added APPLY_fn fallback. (3) M4 core_lib_init — compiled binaries called rt_proc_reset but not core_lib_init so APPLY_fn hash table was empty; emit call core_lib_init@PLT before rt_proc_reset@PLT in both sno_proc_startup and bare main. GATES: smoke 7/7/7 HARD · pat-rung m2/m3/m4 19/19/19 no-SKIP · fence HARD. CORPUS: M2=181(=) M3=159(+3 from 156) M4=149(+3 from 146). Next: continue closing M3/M4 gap to M2 — remaining M3 failures include: literals (unary minus,  + int coercion), triplet/concat-general-var, builtin clusters (081 DATATYPE, 084 DEFINE, 089, 091-096 ARRAY/TABLE/DATA), keyword cluster (082 &STNO, 097 &ALPHABET — bb_keyword bomb for SNOBOL4 keywords, pre-existing), and 027 exponent (outputs 256. real vs 256 int).**
