**SESSION WATERMARK — 2026-06-11 · Fable 5 · D7-RB-2 COMPLETE (step-5/5 ARB LANDED, SCRIP `fb0b825`) — pattern-builder `ins*` = 0. NEXT = D7-RB-2b (NL CAT ROUTING).** The last `ins*` builder `bb_pattern_arb` converted to marshal→`call rt_pattern_build`→4-ports (24 `ins*`→0): new `bb_arb_proto[104]` + `bb_arb_proto_desc {81,32,16,24,-1,-1,-1}` in `pattern_match.c`+`dtp.h` (bytes AS-ASSEMBLED from .s source, never hand-encoded; β increments counter BEFORE computing extension — fixes the old proto's redundant first-null-retry; SPITBOL Ch.6 shortest-first/extend-per-retry, manual-verified) · m2 `IR_PATTERN_ARB` arm mirrors REM · TT_VAR `ARB`/`arb` routed in `lower_snobol4.c` (joins REM/FAIL/SUCCEED/FENCE/ABORT) · ARB frag slot 32→24B · `descr_chain_arity` 0-arity leaves ARB+FENCE+ABORT (FENCE/ABORT = LATENT STEP-3 OMISSION — probed inert today because lower-time `ir_operand_push` survives the `-1` stack-reset, fixed for the hazard). VERIFIED: rt-level harness 6/6 (β-extend ×2 via CAT stitch, exhaust-fail, shortest-first, null-match, empty-subject, δ-restore in nested CAT — the ONLY exercise of CAT, see FINDING) · family probes 10/10 m2==m3==m4==sbl (ARB/REM/FAIL/SPAN/ANY/LEN/ALT/FENCE + nomatch arms). FLOOR (honest, NL-only): smoke 7/7/7 HARD · pat-rung M2 19/19 · M3 19/19 · M4 16/19 (050/051/054 = pre-existing b11a963, FIX-FORWARD ONLY) · corpus stash-baseline BYTE-IDENTICAL all three modes (172/156/144 both sides, FAIL-name-set diff EMPTY; the 144-vs-148 floor delta = container drift per this file's learned-fact — floor restated 144 for THIS container class) · beauty 3/17 (≥ 1/17 floor) · fence HARD. ⚠️ FINDING → D7-RB-2b ⬅ NEXT: NO NL lowering produces IR_PATTERN_CAT (buildable TT_SEQ → legacy orphan; `P='A' ARB 'C'` m4-divergent, legacy-SEQ class NOT a builder gap) — see ladder item 3. See `HANDOFF-2026-06-11-FABLE5-SNOBOL4-BB-D7-RB2-STEP5-ARB-LANDED.md`.
---
...
3. **D7-RB-2b NL CAT ROUTING** ⬅ NEXT — ⚠️ FINDING (2026-06-11): NO NL lowering produces `IR_PATTERN_CAT` — `bb_pattern_cat` + its m2 arm are converted but UNREACHED (CAT stitch exercised only via rt-harness); buildable TT_SEQ falls to the legacy orphan `sno_has_pat` arm (lower_snobol4.c:582), so `P = 'A' ARB 'C'` is m4-divergent (`nomatch` vs `matched` m2/m3/sbl — legacy-SEQ class, NOT a builder gap). FIX: in `lower_assign`, BEFORE the sno_has_pat orphan arm, recognize buildable TT_SEQ (flatten; every leaf a buildable pattern element or QLIT; reuse the TT_VAR-name + primitive-arg admissions) → recurse per kid, pairwise left-assoc `IR_PATTERN_CAT` (operands = the two frags, mirror the TT_ALT all-QLIT arm at :511), DTP_ASSIGN consumer. This transplants the B6 TT_SEQ recipe to NL and lets ARB's β-regen fire through real programs.
4. **D7-RB-3** — close the `P=LEN(3)` value-assign lowering gap (unrouted in all modes).
