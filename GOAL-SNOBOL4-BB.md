...
## SESSION ADDENDUM — 2026-07-07 (Claude Sonnet 5): graph-scale mark/release proven correct in isolation; frame-active prologue wiring attempted and REVERTED after a real 108-test regression, root cause found and precisely diagnosed, fix NOT yet implemented
...
**ATTEMPTED AND REVERTED — wiring this into `xa_flat.cpp`'s frame-active prologue/epilogue caused a REAL 108-test regression in mode-4 (`--compile`), confirmed via the full crosscheck, NOT a false alarm:**
...
- **REVERTED. Confirmed reverted TWICE this session** (once immediately after the crosscheck regression was found; a second time after the diff/diagnostic pass, which had re-applied the same change to compare against baseline and needed re-reverting before this handoff — check `git diff --stat` before trusting any "reverted" claim in a future session, including this one, rather than assuming a stated revert actually happened). Current tree state: ONLY the two harmless trace additions below remain; `xa_flat.cpp` is untouched (matches `origin`).

**STILL IN THE TREE, harmless, gated on env vars, zero behavior change when unset — keep these, they're genuinely useful for the next session:**
...
**NEXT SESSION, precise starting point — do NOT re-run the full diagnostic from scratch, the cause is known:**
1. Fix the alignment defect specifically: reserve the mark in an alignment-neutral way (e.g. `sub rsp, 16` for a padded slot, storing the mark at `[rsp]` and leaving the other 8 bytes as deliberate alignment padding) INSTEAD OF the bare `push rax`, so the net stack effect across the prologue is a multiple of 16 bytes, not 8. Re-verify with the SAME `023_arith_add.sno` case first (cheapest possible repro — a 3-line program, no ARBNO, no pattern matching at all) before touching anything ARBNO-specific again.
2. Re-run the FULL crosscheck (`CORPUS=/home/claude/corpus bash scripts/test_crosscheck_snobol4.sh`, both with `SCRIP_ZETA_SELFLOAD` unset AND `=4`) before believing any fix — a single hand-verified test passing is NOT sufficient evidence, as this session's own false confidence on `052_pat_arbno.sno` demonstrated concretely.
3. Extend the same fix to BINARY mode (`MEDIUM_BINARY` branch of `xa_flat_prologue_str`/`xa_flat_epilogue_str`) once TEXT mode is proven — this session deliberately scoped to TEXT-mode only and never touched BINARY at all; that asymmetry needs closing before this rung can be called done, per this project's own BOTH-MEDIUM MANDATORY rule.
4. Only after 1-3: revisit whether `x86_zeta_free_call()`'s central `op_omega_is_death` hook (currently deliberately disabled, `x86_asm.h` ~line 286-298) can be unified with this graph-scale mechanism, or whether they stay two separate, deliberately non-overlapping layers (construct-scale local free via the hook, graph-scale backstop via mark/release_to) — this was NOT investigated this session beyond confirming the hook stays correctly inert.

**Crosscheck watermark at handoff (RE-DERIVE at next session start — this line is not to be trusted blind, per this file's own existing doctrine): `--run` 252/276 (24 FAIL), `--compile` 251/9/16 (9 FAIL, 16 SKIP), DIVERGE=1 — UNCHANGED from the watermark already recorded below in this file's own TOP PRIORITY RUNG section; this session's surviving changes are trace-only and do not move these numbers.**
