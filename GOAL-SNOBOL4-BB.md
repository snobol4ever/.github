...
**Watermark.** SCRIP: `1eef20d` (base `88bfc4e`, rebased clean over a concurrent Icon GZ-3 + Prolog PLG-1 push) · .github: this handoff. **SNOBOL4 PATTERN EXECUTION — LOWER2-EXEC long pole (2026-05-31 Opus 4.8).** SNOBOL4 pattern-match statements now EXECUTE for the first time since SMX-4. The diagnosis: pattern LEAVES were topology-proven and `bb_exec.c` had 19 working `IR_PAT_*` oracle arms + a complete (but dead-since-PND-1) `bb_exec_pat` driver, but there was NO statement-level entry — `SUBJECT pat` parses to `(STMT :subj (TT_SCAN subj pat))` and VALUE-role `TT_SCAN` hit `lower_unhandled` (`[lower2] UNHANDLED role=0 kind=48`). Six surgical changes (1 commit `1eef20d`, +139/−14): **(1)** `lower.c` `v_scan` — VALUE-role `TT_SCAN`: subject lowered VALUE-role (γ→scan, value on AG ring); pattern lowered PATTERN-role into its OWN `IR_graph_t` (own PSUCC/PFAIL), pointer on `counter` (ARBNO idiom); optional 3rd child = REPLACEMENT (subject must be `TT_VAR` per SPITBOL Err #212, name on `sval`, `ival=1`, repl value threaded onto ring). **(2)** `cx.lang` seeded from `bbg->lang` in the 3 role-entry shims (additive, FACT-RULE: Icon scan routes loud, SNOBOL4 runs v_scan). **(3)** `bb_exec.c` `IR_SCAN` arm — peek subject (ring for plain / `NV_GET` for replace), set Σ/Σlen/Ω, anchored start-iteration (`kw_anchor`: 0=try-every-start, nonzero=start 0 — SPITBOL ch.6), `bb_exec_once(pat)`, `bb_dcap_flush` on match (BEFORE Σ restore), replacement splice + `NV_SET` write-back, save/restore subject registers for nesting. **(4)** `scrip_ir.c` `bb_reset` — preserve `IR_SCAN.counter` (pattern-graph ptr) across reset, like `IR_PAT_ARBNO` (this was the first bug: counter wiped before scan ran → early-return ω). **(5)** `lower_program.c` walker — match-and-replace synthesizes a 3-child `TT_SCAN` (was broken `TT_ASSIGN(TT_SCAN,repl)`); SNOBOL4 default control flow — a statement that succeeds OR fails (no goto) falls through to the NEXT statement (γ AND ω → next_α), not PFAIL (this was the second bug: a failed match killed the program). **(6)** `lower.c` `TT_VAR` pattern case — bare keywords ARB/REM/BAL/FAIL/SUCCEED/FENCE/ABORT (lex as `TT_VAR`; `pat_prim_kind` is consulted only for the parenthesized form) route to their primitive leaves (case-sensitive); non-keyword names still → `IR_PAT_DEFER`. **Verified byte-identical to SPITBOL oracle (`x64/bin/sbl`) on 13 cases:** LEN/literal/SPAN/BREAK/ALT/CAT+capture, the manual's `LEN(2).OUT 'A'`→`CD` backtracking example, match-and-replace (`'ADO'='FUSS'`/deletion/`'M'='B'`→BASH/`LEN(3)='XYZ'`/no-match-fall-through), bare ARB+REM, multi-capture `BREAK.A LEN.B REM.C`→abc/XYZ/def. **Gates (post-merge):** make scrip rc=0, make libscrip_rt rc=0, prove_lower2 35/35, concurrency invariants OK, sm_dead OK, TEMPLATE-PURITY 6 (byte-neutral), Icon m2 5/6 (HARD held — byte-neutral to Icon, verified via git stash; m3 3/6 from the merged GZ-3). All debug probes removed (verified). **NEXT (high→low):** **(A) CONCAT — the next SNOBOL4-corpus blocker:** value-role `TT_SEQ` (`OUTPUT = A B C`) routes to `v_conj`→`IR_CONJ` which yields only the LAST operand (Icon-conjunction semantics) — SNOBOL4 space-concatenation must CONCATENATE. Needs a `cx.lang==IR_LANG_SNO` branch in `v_conj` building an N-operand `BINOP_CONCAT` left-fold (the `binop_apply` CONCAT + ring-based `IR_BINOP` machinery already exists; single-node operands are the common case, multi-node-operand ring fragility is the edge to watch). **PRE-EXISTING, not a regression** (surfaced by test `S SPAN('a').A SPAN('b').B` then `OUTPUT=A '|' B`→`bbb`; captures themselves verified correct via per-line output). **(B) keyword-assignment `&ANCHOR = N`** — `TT_ASSIGN` with `TT_KEYWORD` lhs → unhandled (`v_assign` requires `TT_VAR`); needed for anchored-mode programs. **(C) explicit goto `:S(label)/:F(label)`** — only default fall-through is wired; gotos still ignored (needs label→node resolution in the walker). **(D) `IR_PAT_DEFER` runtime** — user-reassigned ARB/etc. + `*var` pattern-valued deref — Track B (the PND-1 dead-bridge abort still fires for these). Test files in `/tmp/sct/{t1-t7,r1-r5,p1-p6,cc}.sno`. — Prior watermark below. **SNOBOL4 PATTERN LEAF PROOFS + BAL (2026-05-31 Sonnet 4.6).** Two commits on `0fac566`: (1) `6c2277d` — `prove_lower2.c` kname extended (PLEN/PPOS/PTAB/PFNC/PABT/PARBN/PCAP/PCAPI/PATP/PDEF) + 17 SNOBOL4 dump_pat cases for the leaves wired prior session (LEN/POS/RPOS/TAB/RTAB/FENCE/FENCE(inner)/ABORT/SUCCEED/FAIL/ARBNO/CAPT_COND/ATP/DEFER ×2/VAR). (2) `cf6b7f6` — **TT_BAL** lowered: new `IR_PAT_BAL` kind appended at enum END (no Prolog/Icon shift, per the SBL-ATP precedent), bare-generator arm next to ARB/REM (β=self, grows on retry — SPITBOL ch.18 "shortest non-null paren-balanced"), `kind_is_resumable` + kname + proof case added. **prove_lower2.sh 33/33 PASS** (runtime outcome count is the authority; the prior handoff's "34" was an off-by-one). All L2-P lowering arms now topology-proven. **BAL's oracle/exec arm is NOT built** — `IR_PAT_BAL` has no `bb_exec.c` case, so it hits the loud default if executed (correct per FACT RULE "fall loud"); exec deferred to LOWER2-EXEC alongside every other pattern leaf. **L2-P remaining:** none — lowering COMPLETE. `TT_FNC` pattern-primitive folds are N/A (parser builds named primitives as dedicated `TT_*`, never `TT_FNC`; evidence in the L2-P ladder line). **Gates:** make scrip rc=0, make libscrip_rt rc=0, prove 33/33, FACT 6 (baseline, byte-neutral), sm_dead 1, concurrency invariants OK. **NEXT (the long pole for SNOBOL4 corpus): LOWER2-EXEC** — wire `lower2_pattern_entry` → `bb_exec` and build the pattern-engine oracle arms in `bb_exec.c` for every `IR_PAT_*` kind (LEN/POS/RPOS/TAB/RTAB/FENCE/ABORT/ARBNO/BAL/captures/ATP/DEFER + the foundation leaves), grounded in the SPITBOL pattern-match algorithm (ch.18) and the legacy `XBAL`/`pat_*` reps; only then are the lowered arms executable + value-provable. Also still open: R10 fork decision (only byte-affecting choice); byte-identical-x3 GOAL register FACT tables lag bb_regs.h (deferred lockstep, co-owned w/ dual-width session). — Prior watermark below.
