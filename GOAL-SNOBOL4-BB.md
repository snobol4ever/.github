...
- [ ] **ED-5 — extract the pure IR-shape classifier set (next safe move; scanned + verified 2026-06-27).** A grep+purity scan found a set of functions in emit_bb.c that are pure (zero `g_emit`, zero state) and depend ONLY on `IR.h` + libc (no local emit helpers) — i.e. provably-relocatable exactly like arith_fold. The CLEANEST subset to move into a new `src/opt/ir_query.{c,h}` (or `src/opt/scan_classify.{c,h}`): `scan_pat_is_single_lit` (2 calls), `scan_val_is_single_lit` (3 calls), `scan_pat_m3_native_safe` (2 calls) — three pattern-shape predicates, NO forward decls, NO interdependencies, trivial. Then optionally `ir_is_generator_kind`, `gz_node_bounded`, `resolve_ite_entries_em`, `descr_chain_terminal`, `subchain_node_is_generator`, the `gvar_chain_*` resolve/stno helpers (all same clean profile). ⚠ `is_pat_chain_elem` is ALSO pure but has TWO forward declarations (emit_bb.c ~267 + ~382) plus 11 call sites — extractable but the move must delete BOTH forward decls and let the header replace them; do it as its own sub-step, not bundled. Method = ED-1's exactly (dual-list Makefile, crosscheck byte-identical, .s byte-identical). Each classifier is ~3-15 lines; the whole set is maybe -120 more lines off emit_bb.c. Gate: build rc=0 both targets, crosscheck byte-identical to baseline, benchmark .s byte-identical.
