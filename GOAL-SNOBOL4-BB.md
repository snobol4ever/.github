...
**s23k LATE ADDENDA (same session, post-lock):**
- ⭐⭐⭐ **R12 RULING (Lon, verbatim essence):** *"since we are now successfully on the RSP stack… we will not need the ZETA FRAME on ISLAND STACK technique which was what was RESERVING the R12. So R12 all the way now for COND ASSIGN."* Resolves the FRAME_R12 collision = option (A).
- ⭐⭐ **ZW-0 STAGE 1 LANDED (SCRIP `b8ee3d6c`):** island retired at the ONE selector choke, 3 sites — CLI `--zeta-storage=frame-r12` loud rc=2 · `rt_zeta_storage_set` clamp (covers env name+digit) · `rt_zc_frame_live()` folds to `ZC_FRAME`. The 17 routed arms are DEAD CODE again. **GATE: 338/338 emitted `.s` BYTE-IDENTICAL pre→post** (`diff -rq` empty) + env-fallback proven by TELEM. Stage 2 = pure dead-arm deletion (16 files) — safe for a fresh session.
- ⭐⭐⭐ **IR_STATEMENT DESIGN OF RECORD (supersedes BB_END_STATEMENT end-only framing AND the α/β map):** Lon: *"it is the entire statement box… on statement start there is nothing special needed."* The named "something not right" = **the ω edges leave at DIFFERENT DEPTHS** (γ is single-valued; fails exit per-box with per-site live ζ — why the 5,923 pops fused into operators). Resolution: ONE `IR_STATEMENT` bracket box, labels `s<stno>_α/γ/ω` (`:stno` attr exists); **α = bare label, zero ceremony**; **γ = the one whack `add rsp, K_total`**; **ω = a per-distinct-arrival-depth stub ladder** (`add rsp, d_i; jmp :F`, usually 1–3 stubs, every d_i compile-time known from the ZD depth model) — pops homed IN the statement box, no frame, no anchor. Method-2 rbp frame survives ONLY for the jmp-entry/deep-arrival class (the roman/REPL-PIN population = existing `op_stmt_pin`, now scoped). MATCH keeps its own nested bracket; MATCH_END.ω exits at pre-match depth = one of the statement's stubs.
- ⭐⭐ **ZW-5 SLICE 0 LANDED (SCRIP `45e9a1b1`, DORMANT):** IR_STATEMENT minted — enum append-at-end, bb_op_name entry, K=0 in zd_k's one-authority line; lower does not mint yet; 338/338 byte-identical. Slice 1 = lower-side insertion per the design above.
- ⛔ **FINDING §6.1 CORRECTED same-session:** the CAS slab DOES carry cargo — it is the dcap stack (`rt_dcap_e{varname,saved_delta,len}`, γ-pushed by bb_match_capture, pumped by bb_match_end); patstk-cargo-free stands. ZW-3 resized to the R12-FREE-1 reversal; stacklets audit-first. Lesson: census a structure by every alias (`RT_CAS_TOP`/`g_dcap_top`/`1879048192` = one object, three names).
- ⭐ **GLUE REFINEMENT (Lon, handoff musing — NOT yet a rename directive):** two invocation flavors distinguished by PORT-COUNT SIGNATURE — **ONE-SHOT** (a result requested): target α + supplied γ/ω landing pads, **NO β** (a one-shot never resumes its callee); **PASS-THROUGH** (dynamic grafting, statically undeterminable wiring): target **α AND β** + supplied γ/ω = FOUR addresses (the graft participates in backtracking). Proposed names: **INVOKE** vs **BRIDGE**. Consistency: both remain pure transfer — the WHACK CONTRACT (glue never whacks) holds under either; the one-shot's record-restore is jump-back mechanics, not a ζ whack. ⛔ First audit question when formalizing: `bb_glue_pass_wires` today carries only γ/ω — by what mechanism does the β leg reach a grafted blob (BB_DEFER path)?
- **WATERMARK (s23k close):** benchmark board **18/21 EXACT HOLD** (session-open reprove, s23j parity: eval_fixed/eval_dynamic/roman the three reds); every code landing gated **338/338 `.s` byte-identity** (ZW-0 stage 1, slice 0) or **BY-SET rename-neutrality** (100_pat exact both modes; roman {cc208bbc,e9af39a8} both arms). Full crosscheck-318 deliberately not re-run — zero behavioral change shipped; owed at the first ZW behavioral rung.

**NEXT:** **ZW-1/2 UNBLOCKED** (MATCH_BEGIN canonical frame + MATCH_END frame-pop whack; the island arms in those templates are dead) → **ZW-0 stage 2** (dead-arm deletion) → **ZW-3** r12 CAS live → **ZW-5 IR_STATEMENT** per the design above → **ZW-4/6**. Then: m4-EVAL rung · ZD-7c · r9/wire. (Push state lives ONLY in `handoff_status.sh` — RULES rot-rule (a); the banner this line once carried is removed on that rule.)
