...
2. ⭐ **FLIP + DELETE (was NEXT 3), now evidence-backed by the 140/141 result above** — default the two-BB world, delete the legacy proc jmp-entry carve arm (Lon: "DELETE that stupidity").
...
4. Residual spine bisect — `W04_arbno_basic` is the sole surviving DIVERGE under CALL2BB and the one the killswitch removes ⇒ allocator-induced.
5. Framed-glue customers ARBNO/FUNCTION/FENCE1.
6. §3-of-s21x-q's unledgered `stfh` 48 — still the pre-identified landmine to fix in the same slice that admits the match family (path still dormant: 0/317 HKQ).
