**SESSION WATERMARK — 2026-06-10 · Opus 4.8 · D7-RB-1 LANDED — 053 PARITY m2==m3==m4 → `b`. NEXT = D7-RB-2.** The pattern-VARIABLE builders now go through the RT functions in ALL modes: `bb_pattern_lit`/`bb_pattern_alt`/`bb_dtp_assign` converted from `ins*` to marshal-regs→`call rt_pattern_build`/`rt_pattern_stitch_alt`/`rt_dtp_head_build` (0 `ins*` instruction passthroughs left in the three; `bb_pattern_lit` keeps only `raw .byte`/`raw .long` for the TEXT-mode inline proto+desc DATA). m2 gets the three direct-call interp arms (`IR_PATTERN_LIT`/`_ALT`/`IR_DTP_ASSIGN` in `IR_interp.c`) AND the consumer fix the design doc OMITTED: `IR_PAT_DEFER`'s `DT_P` branch was a hard `abort()` — now calls `rt_dtp_run` (mirrors `rt_defer_match`). New shared runtime data `bb_lit_proto[125]`+`bb_lit_proto_desc` and the new `rt_dtp_head_build` live in `pattern_match.c` (single definition the interp arm + binary template both reference). `emit_bb.c` IR_PATTERN_LIT now sets `op_sval` (was NULL). **ZERO `x86_asm.h` edits needed** — the design doc's encoder list was stale (`x86_and`/`mov32`/`XK_ROSLOT`/`ROQ`/binary-`call` already exist) or redundant (`rsi_proto_imm`=the existing RIPSEAL `lea`; `proto_len_edx` avoided via constant `mov32 edx,125`). FLOOR (honest, NL-only): smoke 7/7/7 HARD · pat-rung **M2 19/19 · M3 19/19** (both 18→19, +053) · **M4 16/19 (053 PASS; 050/051/054 'no match' = pre-existing b11a963 regression, FIX-FORWARD ONLY)** · fence HARD · --dump-sno OK · m4 corpus spot-check 12/12 clean. ⚠️ The D7-RB-1 DESIGN doc (`HANDOFF-2026-06-10-SONNET46-SNOBOL4-BB-D7-RB1-DESIGN.md`) is SUPERSEDED and materially inaccurate (3 ways above) — see `HANDOFF-2026-06-10-OPUS48-SNOBOL4-BB-D7-RB1-LANDED.md`. NEXT = D7-RB-2 (convert unary_i/unary_s/nullary/arb/cat identically).
...
2. **D7-RB-2** ⬅ NEXT — convert unary_i/unary_s/nullary/arb/cat identically (each its own commit, frag held at 3-field/24B) → `ins*` count in pattern builders drops to 0; re-probe REM/FAIL/SUCCEED/ARB/SPAN/ANY/LEN/CAT across ALL THREE modes (today m2/m3 empty). PATTERN (proven in D7-RB-1): marshal frame slots via `FRQ(off)`→`lea`, RO addresses (proto/desc/string) via RIPSEAL `lea "[rip + __]"` (text=local label / bin=movabs), constants via `mov32`, stack-align `push rbx;mov rbx,rsp;and rsp,-16;call;mov rsp,rbx;pop rbx`, `call SYM,fnaddr`; inline any per-shape proto+desc as `raw .byte`/`raw .long` in TEXT only; add the matching m2 `IR_interp.c` build arm storing the frag in `IR_EXEC(bb).counter`.
3. **D7-RB-3** — close the `P=LEN(3)` value-assign lowering gap (unrouted in all modes).
