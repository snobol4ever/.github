...
## ⛔ PENDING LON RULINGS
- **FOUR-modes confirmation** — `ZC_STORAGE_{FRAME_R12,FRAME_RSP,CELL_STACK,CELL_HEAP}`. ⛔ **NOW FORCED by the s23k r12-CAS collision (LADDER ZW-0): FRAME_R12 and r12-as-CAS-top cannot both hold r12.** Options: (A) retire FRAME_R12 (+FRAME_RSP archaeology) per Z4-9's own plan — CELL_STACK is the stated ultimate goal, and your s5 ruling already returns r12 to the capture/CAS role; ZW-0/1/2 unblock. (B) keep four configs — CAS top must then stay cell-resident (`[RT_CAS_TOP]`, the s5 arrangement) under island selection, r12-CAS only under CELL_STACK: config-dependent register roles, both maintained. Recommendation on record: (A).
...
- [ ] **ZW-0 ⛔ BLOCKED ON RULING — THE 17 ARMS ARE NOT DEAD (s23k discovery, zero edits made).** zeta_choices.h:288's guard text is STALE: Z4-7 slice 1 re-connected all former `ZC_FRAME` comparisons through `rt_zc_frame_live()/x86_zc_frame()` (`ZC_FRAME_ISLE`=0), so the arms are the LIVE machinery of `ZC_STORAGE_FRAME_R12` (island, 5/5 at Z4-7). **COLLISION: island uses r12 as ζ-frame base; s23k design uses r12 as CAS top.** Lon's own s5 Z4 ruling ("post-RTX the capture top RETURNS to r12; R11 serves ZETA") prefigures the CAS role; Z4's title plans "DELETE THE REST" at Z4-9; but FOUR-modes confirmation is PENDING LON — see PENDING RULINGS. On the ruling: retire FRAME_R12 (+FRAME_RSP archaeology per Z4-9) and this rung becomes that cut; ZW-1/2 unblock with it (bb_match_begin/end carry 5+8 routed island arms).
...
- [ ] **ZW-5 BB_END_STATEMENT** — COLLISION-FREE (no island arms touched); implementation map located s23k: FOUR-PORT FIT = **α is the success-entry, β is the fail-entry** (both statement edge families join through the one box), γ→:S target, ω→:F target, whack on BOTH exits (method 1 `add rsp,K` when depth static / method 2 frame otherwise). K entry = 0 (yields no cell) in `zd_k` emit.cpp:1908 — THE ONE AUTHORITY, one line. Dispatch shape = the IR_MATCH_BEGIN case at emit.cpp:979 (`bb_emit_x86(bb_end_statement())`). Lower insertion: build END first per statement, wire its γ/ω to the real :S/:F, then lower the body with succ:=END.α and the threaded `cx` fail continuation := END.β (`sno_ω_to` sites; fence-free-run shared fail targets at lower_snobol4.c:1444 route naturally since the run's one target becomes END.β). Retires the 5,923 fused pops; interacts with the 223 UCLAIM heads (ZD ladder).
