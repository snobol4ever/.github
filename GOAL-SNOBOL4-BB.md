...
## ⛔⭐ LIVE CURSOR — s21x-l (2026-07-30, Claude session: SEQ-CELL LANDED DORMANT — FALSIFIED TWICE, THE SUSPENDED-CELL LAW EXTRACTED). SCRIP `7839c64e`.

**LANDED (regime-keyed `SCRIP_STMT_FRAME=1`, killswitch `SCRIP_SEQ_CELL=0`; gate-off byte-identical by construction and proven):** fc_geom IR_MATCH_SEQUENCE 16B-cell arm (conjuncts `!fc_seq_active && !fc_arbno_member`) + the s21x-h 3-line grant preamble at the SEQ dispatch case (base = op_off; all 8 template FR refs are own-quad by inspection) + `fc_arbno_member` side table (fc_arm_member shape, zeta_storage.c) registered at the lowerer SEQ site (`sno_in_arbno > 0 || sno_tree_has_varext(S,0)`). zls_fc_cell net-out SKIPPED (16B flat double-count under gate-on = waste not wrongness; netting needs the s191 1:1 locals-quad proof).

**FALSIFIED TWICE, MEASURED:** (1) unfenced → armed 44→49 but m4 broke 066/141/164/165 (all ARBNO-family), DIVERGE 2→4; (2) body-residency fence (`sno_in_arbno` alone) did NOT cure. Subtree-extent fence cured all four; gate-on restored EXACTLY to baseline. **⛔ LAW — THE SUSPENDED CELL:** any rsp cell that stays pushed across γ ABOVE an ARBNO/DEFER extent shifts every depth-sensitive window slot below it (the s202 defer-window `mov [rsp+K],rsp` class) — body residency is irrelevant; the DEPTH is the disease.

**VERDICT — the grant is corpus-VACUOUS as fenced:** linear SEQs are chain-converted (fc_seq_active), variable-extent SEQs fenced, arm members declined; witness `X LEN(2) LEN(3)` is oracle-exact m3+m4+sbl but carves nothing (statement declines earlier on unregistered LEN). Census 44/316 unchanged; frontier unchanged (first-blockers: LIT_STRING 137 · SEQ 44 · LIT_INT 36 · VAR 14 · ALT 7 · LEN 5 · KEYWORD 5 · CALL 5 · COND 4 · PATREF/ANY 3 · UNOP/NOTANY/SAVE 2 · IMM 1). The mechanism + fence are the reusable decline surface every future cell grant needs; **THE UNLOCK IS THE VARIABLE-EXTENT ANCHOR** (NEXT 1).

**NEXT, DEPENDENCY ORDER:** (1) ⭐ **VARIABLE-EXTENT ANCHOR** — ARBNO/DEFER window state onto a depth-immune base: statement-bracket rbp slot or the s202 r9 park-address (named fix of record; emitter site never located). Lifts the SEQ fence, the 123-class, and the chain-vs-cell fork. MONITOR-FIRST. (2) **ONE-WALK vlit generalization** — consolidate the three registration loops (zeta_storage.c: IR_ASSIGN value-spine ~429 · SUBJECT ~445 · fc_call ~409) into ONE statement-spine walk registering lit/var producer chains in ALL consumer positions (replacement — needs the splice ruling — pattern-arg, keyword, call-arg); serves the 137+36+14 frontier. (3) **Window boxes** LEN/ANY/NOTANY/POS — refs are `op_sa+4/8` (shared base, not own-window); resolve op_sa's basis under the regime, then result-only cells per design law 1. (4) **ONE-GRANT consolidation** — the ~20 per-kind dispatch preambles into DRIVE_FILL (bases differ: op_off vs x86_scratch_off vs zls_off — per-kind base fn; byte-identity provable). (5) bb_assign_local / stub-body result-store audit (s21x-k disease class). (6) chained-assign flat-read latent. (7) crosscheck rc-checking arm (safe since the s21x-k cure). (8) ⛔ NEEDS LON: default-flip license; FOUR-modes confirmation (see rulings).

---

## ⛔⭐ WATERMARK OF RECORD (2026-07-30, re-proven s21x-l, both arms)

Gate OFF: **m3 PASS=311 FAIL=4** {test_case · 140_pat_eval_double_fn_trick · 141_pat_eval_double_fn_arbno · 160_pat_alt_inner_gen_resume} · **m4 PASS=311 FAIL=2 SKIP=2** {test_case · 160} · **DIVERGE=2** {140 · 141}. Gate ON (`SCRIP_STMT_FRAME=1 SCRIP_SUBJ_CELL=1`): **IDENTICAL**; armed census **44/316**. Runner: `scripts/test_crosscheck_snobol4.sh` (~30s/arm). ⛔ **Do NOT add `SCRIP_CALL2BB=1 SCRIP_CALL2BB_FC=1` for census work** — reproduces the s21x-i parked D2 defect (lib-composed 140/141 fail m4).
