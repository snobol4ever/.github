...
1. **FRESH-FRAME WIP** — bracket the word2/word4 m3-vs-m4 **DIVERGE=2** (`.github/WIP-sn4pat-defer-freshframe-2026-07-06.patch`: `rt_frame`→`rt_zls_alloc(64K)`/`rt_zls_release` per blob invocation, LIFO both continuations) BEFORE landing; it un-gates the recursive-grammar cluster (115/120/126/131/138/139, all PASS m3 under the patch). **STILL OPEN — session 2026-07-06 (Claude Sonnet 4.6) reframing, NOT a fix:** applied the patch and ran word2 both modes. m3 (`--run`) emitted all 4 CORRECT lines then hung (exit 124, timeout) — so the PATTERN MATCH is right under the patch; the hang is AFTER the last input line. Isolated with a pattern-free minimal repro (`LOOP LINE=INPUT :F(DONE) / OUTPUT="["LINE"]" :(LOOP) / DONE OUTPUT="REACHED-END"`, no defer/pattern code at all): fed 2 lines, printed `[alpha]`/`[beta]` correctly, then emitted **7,129,666 blank lines and hung (exit 124)** — `REACHED-END` never printed, so `INPUT` is NOT taking its `:F` branch at EOF. This means the DIVERGE=2 is very likely a pre-existing **`INPUT`-at-EOF failure-propagation bug**, unrelated to ζ-frame lifetimes, riding on top of the fresh-frame patch (m3 flushes per line so its correct output survives into the hang; the standalone m4 binary is presumably block-buffered to a pipe and loses unflushed output when `timeout` SIGTERMs it — inferred, not yet instrumented). Do NOT conclude the read primitive is at fault: `by_name_dispatch.c:2933` (`read()` builtin, stdin) and `:3443` (fp variant) both already do `if (!fgets(...)) { *out = FAILDESCR; return 1; }` — correct EOF→fail shape. The bug is that the `INPUT`-KEYWORD's own read path (grep for the literal string `INPUT` does NOT find it — it's behind an interned/lowercase keyword id, likely `kw_input` in `keywords.c`, actual read plausibly fires through the VARVAL/argval by-name value-fetch per ARCH-SNOBOL4) either skips that EOF check or its FAILDESCR isn't propagating to the statement's `:F` branch. NEXT SESSION: trace (not grep) that path — 2-way monitor or gdb on the minimal repro above — using the two known-good sites as the reference shape, then re-run crosscheck (expect DIVERGE→0, word2/word4 PASS) before landing this patch.
