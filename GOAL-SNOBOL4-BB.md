...
**▶ NEXT MOVE (session 27): DCR-3 (re-run 3-way comparison, update func-call ratios in watermark; needs oracle token), then SAB or table-probe inline — Lon decides.**
...
**THREE-WAY COMPARISON — re-grounds the "10×" claim (session 24).** All 16 benchmarks run SCRIP mode-4 (AOT native) vs SPITBOL (`sbl -b`) vs CSNOBOL4 (`snobol4`). **Correctness: 15/16 byte-identical across all three.** The exception is `indirect_dispatch` — a real SCRIP-vs-oracle DIVERGENCE (`R = $FN(X)`, `FN='ADD1'`): SPITBOL ERRORS ("undefined function called"), SCRIP returns blank `R`, CSNOBOL4 returns nothing. Not a clean pass; the bench script only marks it OK because nothing gates that benchmark's output. **Performance (wall-ms, ratio = SCRIP/SPITBOL): SCRIP is SLOWER than SPITBOL on 14/16** — arith_loop 2.5×, op_dispatch 1.6×, string_concat 8.8×, eval_fixed 6.2×, mixed_workload 12.9×, table_access 8.8×, roman 22.7×, fibonacci 23.1×, string_pattern 7.9×, string_manip 11.3×, func_call_overhead 13.3×, func_call 14.2×, eval_dynamic 176.1×. **Faster than SPITBOL on only 2:** pattern_bt 0.84× and var_access 0.77× — exactly the paths where FZ/GVA already landed. vs CSNOBOL4 SCRIP wins/ties 4 (op_dispatch 0.7×, pattern_bt 0.7×, var_access 0.4×, arith_loop 1.0×), loses 12. Root cause (assembly-traced): native control flow but a runtime PLT call PER SNOBOL PRIMITIVE; SPITBOL inlines/threads them. The "10×" headline is currently inverted for general workloads — true only on the two inlined paths. The PERF-CALL ladder below inlines the most-executed primitives, ranked by this measured cost. CORRECTION to prior watermark: `eval_dynamic` is NOT an OOM in this sandbox — it COMPLETES correctly in 87.7s (self-timer 1.5s; the 86s gap is mprotect-RX page-sealing/syscall time, 1M× per-EVAL recompile). It showed CRASH only because the bench script's 30s timeout killed it.
...
- [ ] **DCR-3 — gate + re-ground.** Side-by-side `.s` regen; re-run the 3-way comparison; update the func-call rows of the table in the START-HERE block with the new ratios.
