**SESSION WATERMARK — 2026-06-12 · Sonnet 4.6 · duplicate-label fixes landed (SCRIP 4cb8a0a). NEXT = fix `_wγ`/`_wω` undefined-reference link error (blocks omega_driver/Qize/XDump_driver beauty programs), then beauty-subsystems gate, corpus count, M4-BUILTIN/FENCE/CAPTURE-COND bugs.** Two bugs fixed this session: (1) `.Ldefspec`/`.Lprocfn`/`.Lbynamefn` rodata labels used `_.nid` (per-graph node index, resets to 0 per function) → duplicate "symbol already defined" for any program with DEFINE in multiple functions. Fix: `g_flat_node_id++` (global monotonic) in `bb_call.cpp`. (2) SNOBOL4 m4 proc emit loop emitted ALL proc_table entries including earlier duplicate-named ones (e.g. `DEFINE('TV...')` called twice → two BB graphs both named TV → `TV_c0_α` defined twice). Fix in `scrip.c`: skip any proc entry where a later entry in proc_table has the same name (last definition wins, matching runtime DEFINE semantics). Beauty compile+link: 20→30/34 pass. Remaining 4 failures (`omega_driver`, `Qize`, `Qize_driver`, `XDump_driver`) hit a new link error: undefined reference to `lwr_c0_wγ` / `lwr_c0_wω` — labels referenced from `lwr_c0_γ`/`lwr_c0_ω` jumps but never emitted. The `w`-suffix (wired) child-body labels are referenced when child sub-bodies set `text_externalise=1 wired=1` but the wired label pair is not always emitted. GATES: smoke 7/7/7 HARD · pat-rung 19/19/19 no-SKIP · fence HARD.
---
