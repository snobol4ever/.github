...
**NEXT (ZLS2 ladder):** (1) second consumer — extend the same save-slot pattern to another re-entrant match template (ARB's cursor-save is the natural sibling); (2) MALLOC/ASan-equivalent proving pass (ZLS2 has no malloc backing yet — add `ZC_ZLS2_MALLOC` diagnostic mode per the ZB-ACT-0 method); (3) flip default ON once a few consumers are proven; (4) trampoline retirement → rsp (the design's end state).
