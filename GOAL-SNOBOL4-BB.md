...
- **memcpy-safety is the ONE hard constraint on the relocatable matcher:** a blob copied by `pat_pool_emit` into pat_pool must use **ABSOLUTE addressing** — `mov rXX,imm64` for the literal/data addr and `mov rax,imm64; call rax` for external calls (memcmp/strchr) — and **NO `[rip+disp]`/rel32 to anything outside the blob**, because rel32 offsets break once the code is copied to a new address. (This is exactly why the DDS-0-deleted protos were absolute-addressed byte arrays. We now emit that same shape via `x86()` encoders, not hand-bytes — `x86_asm.h` must have/get `mov reg,imm64` + `call reg` forms; add the encoder if missing, never hand-encode in a template.)
