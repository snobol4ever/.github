...
- [ ] **PBG-2 — TABLE INT-KEY WRITE/READ in M4 → table_access green (+1).** `T<I> = v` (write) and `T<I>` in value position (read) for integer subscripts on a `TABLE(n)` (M4-DATA / S7 cluster). Probe m4≡sbl: `T=TABLE(16);T<3>=99;OUTPUT=T<3>`→`99` · fill+sum loop → `250500`. GATE: floors hold; table_access result == ref.
  **⚡ FOUNDATION LANDED (Claude 2026-06-22, SCRIP `25635f2`). The 13-site by-name read/write IS WIRED & probes green:** `T=TABLE(16);T<3>=99;OUTPUT=T<3>`→`99` (==sbl) in M4; gates non-decreasing (smoke 7/7/7, pat-rung M4 19/19 0-SKIP M3 15, fence 0/0; the 5 prior-green benches unregressed). Scope landed = base=VAR, key=VAR|LIT_I, value=VAR|LIT_I. KEY CORRECTION vs the PBG2-HANDOFF design (do-not-re-derive): the template must read ONLY fields that survive `walk_bb_node` (emit_core.c:327-339 resets op_node_kind/op_ival/op_sval/op_a_* from the node itself just before dispatch) — so the by-name templates use `bb_lk`/`bb_rk` (key/value kind), `bb_li`/`bb_ri` (immediates), `op_name1`/`op_name2`+`op_parts_lbl[0..1]` (base/key names), `op_parts_str[2]`+`op_parts_lbl[2]` (value name for IDX_SET), and `op_sa`/`op_sb`/`op_sc`/`op_off` (slots, all survive); the drafted bb_idx_get's use of op_node_kind/op_ival for the KEY was the one bug. The runtime sinks used are `subscript_get`/`subscript_set` (pattern_match.c DT_T branch — the proven-correct ones); the `rt_table_idx_*` dups in aggregates.c stay retired. **STILL OPEN for table_access GREEN = clause D below** (it is `T<I>=I*2` BINOP-value write + `SUM=SUM+T<I>` mixed arith — neither covered by the VAR|LIT_I foundation). `corpus/benchmarks/table_access.s` now emits & assembles but the BINOP-value write still bombs at the unimplemented shape; full-green lands with D.
