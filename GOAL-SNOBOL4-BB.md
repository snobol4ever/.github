...
## ⛔⭐ LIVE CURSOR — s21x-q (2026-07-30, Claude: ζ-CELL SPELLING SWEEP **CLOSED** (44→41, all 41 classified NOT-ζ, byte-identical 317/317) + ⭐⭐ THE D2 ROOT CAUSE IS RETIRED BY MEASUREMENT — the stfh match arm is DORMANT at HEAD (0/317 HKQ, 31/317 live and all arithmetic), so BOTH halves of s21x-p's diagnosis describe a path that never executes; the real STF defect is OPEN, in the ARMED arithmetic path).
...
**⛔⭐⭐ MEASURED LATE — THE WELD IS WITHDRAWN; THE stfh MATCH ARM IS DORMANT AT HEAD.** §3's mechanism is real as source reading and IRRELEVANT as a root cause, because **the path does not execute**: **0 / 317** programs emit the HKQ bracket map (`[rbp−48/40/32/24/16]`) under `SCRIP_STMT_FRAME=1`, while **31 / 317** have ≥1 graph going `live=1` — and those are **arithmetic** graphs (023–027 `arith_*`…), **not match graphs**. CONTROL PASSES (the null is NOT vacuous): `SCRIP_STF_DEBUG=1` prints `[STF] choke stf=1 …` so the env is read, and the gated `.s` differs by 220 lines on `085_define_two_args`. ⇒ `bb_match_head`'s `x86_zclaim(48)` and the whole HKQ bracket **never fire in the corpus**. **CONSEQUENCE: s21x-p's D2 root cause is about a DORMANT PATH on BOTH halves** — release's "missing arm" is not missing (§3) and the head carve it should match does not execute. **The `SCRIP_STMT_FRAME=1` heap-exhaustion originates in the ARMED (arithmetic/statement) path, not the match bracket — that is now genuinely OPEN.** Landing K=48 would have been a vacuous port of exactly the kind the RTX ladder keeps catching. §3 stays on file as the **pre-identified landmine** to fix in the same slice that admits the match family, not as a bug to hunt now.
**⚠ INSTRUMENT LAW (earned the hard way this session):** the first arming scan used `grep -cE "sub +rsp, *48"`, which matched **`sub rsp, 488`** and reported 101 false-armed programs. **An arming census keyed on an immediate operand MUST anchor the operand end** (`sub +rsp, +48$`); a bare digit prefix silently counts every wider carve. Cross-check with a second independent signature (here: HKQ refs) before believing either.

**NEXT, DEPENDENCY ORDER (re-ordered by the measurement):** (1) ⭐ **FIND THE REAL STF DEFECT IN THE ARMED PATH** — the armed set is arithmetic graphs, so bisect there, monitor-first (`test_monitor_2way_sync_step_bin.sh`); the 31-program live list is the population, `SCRIP_STF_DEBUG=1` `live=1` is the selector. The match-bracket theory is retired. (2) Retire the discriminator's `!flat_pat` conjunct (emit.cpp ~2535) — note `[STF] choke` shows graphs rejected with `pat=0` too (`jmp=1`, and a `floor`-bearing conjunct), so `!flat_pat` is NOT the only gate keeping match graphs out; name all of them from the debug line before cutting. (3) **FLIP + DELETE:** default the two-BB world, delete the legacy proc jmp-entry carve arm (Lon: "DELETE that stupidity"). (4) Residual spine bisect — **free hint:** `W04_arbno_basic` is the one DIVERGE the killswitch removes ⇒ allocator-induced, unlike 140/141. (5) Framed-glue customers ARBNO/FUNCTION/FENCE1. (6) When the match family IS admitted: fix §3's unledgered 48 in that same slice.
**⭐⭐ THE GATE'S ACTUAL DAMAGE — MEASURED FOR THE FIRST TIME (s21x-q close; nobody had this number, because the gate was "OFF-LIMITS" pending a weld that is now retired as vacuous):**
`SCRIP_STMT_FRAME=1` vs the 264/52 · 264/50/2 · DIV=3 baseline → **m3 232/84 (−32)** · **m4 223/91 SKIP=2 (−41)** · **DIVERGE 8 (+5)**.
**⛔ THE LEAD: THE DAMAGE EXCEEDS THE ENTIRE ARMED POPULATION.** Only **31 programs** have ≥1 `live=1` graph, yet **41 m4 programs regress**. So either (a) the gate damages programs in which NO graph arms — which would violate **ALL-OR-NOTHING PER GRAPH** and make the head/stub retarget machinery (emit.cpp ~1986/2061/2210/2308) the suspect rather than any box template — or (b) the live census undercounts (it counts *programs with ≥1 live graph*, not graphs). **FIRST STEP FOR NEXT 1 IS CHEAP AND DECISIVE: intersect the m4 regression set with the 31-program live set.** If the intersection is proper, (a) holds and the defect is outside the armed regime entirely. ⚠ NOT DONE THIS SESSION — the sets were never intersected; this is a measured discrepancy, not a diagnosis.
**BISECT POPULATION (the 31, all trivial — arith/assign/concat/goto/output, NO match programs):** 023-027 arith_{add,subtract,multiply,divide,exponent} · 029_arith_precedence · 009-013 assign_{string,integer,chain,null,overwrite} · 016_assign_to_output · 017-022 concat_* · 031_goto_unconditional · 036_goto_skip_to_end · 037_goto_nested_labels · empty_string · hello · multi · 001-005 output_* · 007_output_null_var · 008_output_double_quoted.

**⚠ RECONCILE:** this session measures **31/317 programs** with a live graph; the cursor's inherited "armed 44/316" counts armed **statements**. Different units — not a contradiction, but unreconciled. Do not treat either as the other.
