...
## ⛔⭐ LIVE CURSOR — s21x-u (2026-07-30, Claude: ⭐⭐ **STEP 1 IS FALSIFIED BY MEASUREMENT (−170 PROGRAMS) AND THE "ORDERING DEFECT" IS THE LOAD-BEARING DISCRIMINATOR.** Implemented s21x-t's NEXT(1) faithfully — `op_zdepth` from the FINAL `op_fc_bytes`, derived at read time — and measured m3 229 → **59**. REVERTED, tree clean at SCRIP `1c0124f1`, nothing landed. Also measured: **all 88 m4 reds compile CLEAN, zero bomb stubs** — they are addressing/semantic divergences, not mid-design shapes. Step 2 SIZED: **46 colliding templates + a 15-template safe pilot set.** WATERMARK m3 229/88 · m4 227/88/2 · DIV=1 (m3 oscillates 229/230 on the `test_string` ASLR flake — **compare m4**). Full text: `FINDING-2026-07-30-CLAUDE-SN4-ZRUN-2-THE-LINE-ORDER-IS-THE-DISCRIMINATOR-AND-STEP-2-IS-46-TEMPLATES-OF-PLUMBING.md`.)
...
**⭐ NEXT — STEP 2, AND IT IS REAL PLUMBING (s21x-t's "may need no new plumbing" is WRONG).** `_.op_sa` and `_.op_off` are distinct at the CALL SITE and erased at the accessor: `FR`/`FRQ`/`FRQB` take a bare `int` and cannot tell a cross-box read from an own-cell read. `bb_unop.cpp` mixes both through the same `FRQ`. RULE: `op_sa` is carve-inclusive ⇒ **must NOT add `op_zdepth`**; `op_off`/`x86_scratch_off` are 0-based ⇒ **MUST add**. SHAPE: add an `FRX`/`FRQX` sibling in `x86_asm.h` (adds `op_flat_disp`, not `op_zdepth`), then **convert the 15 `op_sa`-ONLY templates FIRST** — they carry no own-cell writes, so the measurement is clean — and only then the 46 colliders. ⛔ Do NOT start the 46 before the pilot is green: a colliding file half-converted holds both meanings unconverted-and-converted at once, which is strictly worse than today. ⛔ NOT a 108-template hand sweep (s21x-r proposed that twice, wrong twice).

**⭐ THE WITNESS IS THREE LINES AND NEEDS NO INSTRUMENT.** `028_arith_unary_minus` is `OUTPUT = -5`; SCRIP prints **`5`**. The negation is BYPASSED, not corrupted — the clean unnegated literal is the signature of a consumer reading the producer's INPUT cell, exactly one 16B cell off, corroborating s21x-t's `+16` table from observable behaviour alone.
