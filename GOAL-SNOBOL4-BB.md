...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  ▶▶▶▶▶  #1 PRIORITY (Lon 2026-06-27) — EMITTER CLEANUP — DO THIS FIRST  ◀◀◀◀◀  ║
# ╚═══════════════════════════════════════════════════════════════════════════╝

**This rung runs BEFORE the ladder-climb banner below.** It is behaviour-neutral structural work on `emit_bb.c` (the 4400-line god-file): lift pure analysis out to `src/opt/` so the emitter shrinks toward dispatch-only. Each step is a PROVABLE no-op (crosscheck byte-identical to baseline). ED-1/ED-2/ED-3 landed; see steps for live state. When EMIT-DECOUPLE is fully closed, the ladder-climb banner below becomes #1 again.
...
- [⛔] **ED-4 — wire branchopt — BLOCKED by a KNOWN crash; do NOT attempt naively.** `bopt_chain` (src/opt/branchopt.c) is compiled but never CALLED. ⚠⚠ The OPT rung already TRIED this (session 27) and it CRASHED: `flat_drive_assign FATAL` — **the γ-spine is overloaded as the operand-recovery structure, so rewiring γ edges shortens the walk the operand-recovery depends on.** `bopt_chain` rewrites exactly `nd->γ.node`/`nd->ω.node` (see branchopt.c) → it WILL hit that crash. Session 27 quantified the payoff as marginal (34/1686=2% collapsible branches corpus-wide, 0% in hot loops) and CLOSED the OPT rung as a perf lever, retaining branchopt.c as a dead-end record. **Therefore ED-4 is NOT free structural cleanup — it requires first DE-OVERLOADING the γ-spine from operand-recovery (a real redesign), then rechaining ONLY edges proven not to shorten an operand-recovery walk.** Until that redesign exists, ED-4 stays BLOCKED. (Cross-ref the OPT/BOPT findings + `docs/BOPT-DESIGN.md`.) This step is here for completeness; the LANDABLE EMIT-DECOUPLE value is ED-1..3 (done) plus any further PURE-helper extractions (ED-5+ below).
- [x] **ED-5 — extract the pure IR-shape classifier trio.** ✅ DONE (2026-06-27, SCRIP `65ac3f23`). `scan_pat_is_single_lit`/`scan_val_is_single_lit`/`scan_pat_m3_native_safe` moved verbatim+de-static to NEW `src/opt/ir_query.{c,h}`; `scan_pat_cat_concat` (uses GC_MALLOC, sits between them) left in place. 4 call sites unchanged (`flat_drive_scan_stmt` ~2478/2480/2481/2486). Dual-list applied (RT_PIC_SRCS ~250 + branchopt-style compile line ~553). emit_bb.c 4227->4189 (-38). Verified: build rc=0 both targets; `.so` `nm -u` clean (no undefined refs to the 3); crosscheck **byte-identical to baseline** (--run 159/102, --compile 179/76/6, DIVERGE 22; FAIL+DIVERGE program lists char-identical — only kernel PID noise in the "Aborted" shell lines differed); benchmark `.s` regen reported `No changes` (codegen output identical). Style: longest line 122, zero blank lines, one dash-separator per fn.
- [~] **ED-6 — extract the remaining clean IR-query helpers (PARTIAL — 5 of N landed 2026-06-27).** ✅ LANDED into `src/opt/ir_query.{c,h}`: `gz_node_bounded`, `descr_chain_terminal`, `ir_is_generator_kind` (fwd decl deleted), `subchain_node_is_generator` (SCRIP `8633674f`), and `resolve_ite_entries_em` (SCRIP `be23f739` — note: this one needed `ir_query.h` to `#include "box_state.h"` for `bb_ite_state_t`; benign — box_state.h is POD-over-IR_t, IR.h+gc.h only; no Makefile change since CBASE already carries `-I$(SRC)/emitter` on both build lines). All re-verified pure + exclusively owned. emit_bb.c 4227->4140 (cumulative ED-5+6 = -87). Each step crosscheck byte-identical to baseline, benchmark .s `No changes`. **STILL OPEN (next pass):** (1) the `gvar_chain_*` cluster (emit_bb.c ~3795-3895: resolve/resolve_stmt/skip_stno/collect_stnos/is_real/arity/prebuild_children/operand_refs) — a LARGER interdependent set with `γ.node`/`ω.node` walks, move as ONE unit, re-verify each is pure (some may touch g_emit — CHECK); (2) `is_pat_chain_elem` — pure but TWO forward decls + 11 call sites, its OWN sub-step, delete BOTH decls, header replaces them, do NOT bundle. ⚠ RE-VERIFY purity before moving — emit_bb.c keeps shifting. Method = ED-1/5/6's (moves into the SAME ir_query files need NO new Makefile line; crosscheck byte-identical; .s byte-identical).
...
- **1110_array_1d** — stops at **003: OOB-high should FAIL**. A bare subscript-read statement `a<4> :F(e003)` lowers to a bare IR_SUCCEED (`lower_snobol4.c:1171 case TT_IDX: return NULL`) → never takes :F. AND the read box `bb_idx_get` never branches ω on OOB — its slow `subscript_get` path always `jmp γ` (template ~line 66). So OOB array reference does not FAIL. **FIX = two parts:** (a) lower a bare TT_IDX subject to an IR_IDX whose ω=failure-target γ=success-target (supported subscript shape only); (b) make the read path SIGNAL FAILURE on OOB — `subscript_get` needs a found/OOB out-flag and the template an ω branch when the array bounds-check fails (mirror how the store fast-path `js`/`jg` detect OOB, but route to ω instead of the slow path for an ARRAY). SPITBOL semantics confirmed in manual (Ch.7): "the array reference will fail, and the failure may be detected in the Goto." (001/002/008/009 PASS now — null-init, real store, string store all work.)
...
- **1115_data_basic** — stops at **006: `val(a) = 'new'` mutate-and-reread**. DATA constructor + accessors + VALUE() all work through 005; only the field MUTATOR-then-reread (accessor on LHS of `=`, second time) fails. Likely a DATA field-set cache/aliasing issue.
- **094_data_define_access / 095_data_field_set** — DATA cluster (separate, not yet looked at).
