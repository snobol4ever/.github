...
- [ ] **M3-NATIVE-0 — BOMB THE STUBS FIRST (prerequisite; this is what should have been done two days ago).** A silent `return std::string()` (HQ Invariant 0 = STUB) is WHY two days were wasted: unimplemented arms emitted nothing, mode-3 fell through to the C oracle, and "verified via --run" was recorded as true while the path under test never ran. Current count: 96 silent stubs (37 SM_templates + 59 BB_templates). Replace each unimplemented BINARY arm's silent return with a LOUD BOMB: emit a trap/abort sequence (e.g. `ud2`, or a `call rt_bomb("<template> <medium> arm unimplemented")` that prints + aborts) so that ANY mode-3 execution reaching an unfilled arm CRASHES IMMEDIATELY AND VISIBLY rather than silently degrading. Rule going forward: **no arm may stub silently; an unimplemented arm bombs.** No "verified" claim is permitted for any arm until a non-bombing execution under `--run` is observed. Add a grep gate: zero `return std::string();` in `*_templates/*.cpp` (replaced by bomb emit or real bytes).
- [ ] **M3-NATIVE-1 — audit emitter coverage AGAINST the recovered engine.** Map `codegen_sm_x86` BINARY-medium coverage (which SM opcodes have a working BINARY arm vs TEXT-only vs BOMB) and the `sm_image` API (alloc/seal/entry). Cross-reference the deleted `h_*` handlers (`git show 2073c081~1:src/processor/sm_jit_interp.c`) — each `h_<op>` is the semantic spec for that opcode's BINARY arm. Output: coverage table (SM opcode × {TEXT, BINARY, BOMB}) annotated with the matching deleted `h_*`/`sl_*` reference.
