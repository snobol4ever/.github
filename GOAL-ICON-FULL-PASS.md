...
> **IN PROGRESS (NOT landed) — Icon native real-arithmetic path.** `rt_num_arith` (value-only DESCR-in/out) + `rt_jct_relop` real branch + `op_num_real` flag + `descr_binop_set_slots` + real arms in `bb_binop_arith`/`bb_binop_relop`. m3 PASSes `rung18` real_gt/mixed/real_eq/real_lt + `rung17` real_add; m4 PASSes real_add/real_lt. **RED — two regressions block landing:** (1) `rung19_pow_toby_pow_var` (gate `local_assign_rhs_ok_g` LIT_F relaxation admits real-POW graphs not real-routed → fix `return !graph_has_pow(g)`); (2) m4 dup-label `as` error on the root_node tree path for real_gt/mixed/real_eq (`flat_drive_binop_tree` re-walks chain operands → fix: skip walk when `bb_slot_get(child)>=0`). Full detail + patch: `HANDOFF-2026-06-15-CLAUDE-ICON-BB-REAL-ARITH-WIP.md` + `WIP-icon-real-arith-2026-06-15.patch`. After both fixes: expected m3/m4 +5 each, no regressions; `real_relop_goal` stays EXCISED (gen-alt track).
