...
- **NEXT (ordered) — GROUP B then GROUP A (the 15 remaining EXCISED: 9 in B, 5 in A, plus map/grep emit):**
  1. **GROUP B (9 EXCISED): `__rk_bool` dval==2.0 inline cmp+branch.** Tests: `jct_any/all/one/none/infix/str/nested`, `bool_truthiness`, `bool_compare_store`. IR shape (verified via `--dump-bb`): an `IR_CALL sval="__rk_bool" ival=1`, γ→truthy body, ω→falsy; the condition (`$x == any(...)`, a relop or junction-collapse) hangs off the sub-graph in `IR_EXEC.counter`. Path: in `bb_call.cpp`, detect `cblk->entry` is BINOP-relop or VAR/literal and emit inline cmp+branch (reuse the relop encoders already in `bb_binop_relop`); junction conditions call `rt_jct_relop`. GATE: `__rk_bool dval==2.0` is BLOCKED at three sites in `src/driver/scrip.c` (`icn_rhs_kind_ok` ~L208, `icn_graph_native_emittable_mode` ~L259, EXCISE pre-flight) which all `return 0` for `(__rk_bool||__rk_try)` dval==2.0. Flip `__rk_bool` (keep `__rk_try` excised) once the emit arm lands.
