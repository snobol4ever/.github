...
- [ ] **RK-GRAM-3 (THE SEAM) — native recursive-descent grammar engine on Σ/δ/Δ.** REPLACES `gram_expand`'s flatten-to-NFA (depth-16 stopgap). Per the X86-64 FACT table the subject triad is reserved for Raku INSIDE the grammar/regex slab: **R13=Σ subject base, R14=δ cursor, R15=Δ end bound**, δ CALLEE-SAVED so it stays ambient across subrule recursion (the called rule's box advances δ and the caller sees it on return — no arg marshaling, no "current position" frame field). Four ports keep their pattern-lang meaning: **α=fresh entry, β=resume/backtrack-retry, γ=match advanced δ, ω=fail**. Choice points save δ into the per-activation ζ=r12 frame (a δ-snapshot slot — NOT a value stack; the Icon scan "save δ on β, restore and fail" discipline). Captures record `(from=δ-at-α, pos=δ-at-γ)` → Rakudo `Match` `$!from`/`$!pos`. Grammar compiles ONCE to box code; each `.parse` loads Σ/δ/Δ and jmps to the TOP box; success = TOP reaches γ with δ==Δ (full) or any γ (subparse). **STANDING REQUIREMENT: fresh full-budget session; read `ARCH-x86.md` (Boxes-are-stackless + Flat-BB ABI) + `ARCH-SCRIP.md` + `ARCH-ICON.md` §"String scanning" + `src/emitter/bb_regs.h` FIRST.** Templates stay LANGUAGE-BLIND (FACT RULE) — these boxes dispatch on IR shape, not on `is_raku`; cross-language safety holds because only the Raku lowerer emits these IR kinds. STEPS (each both-modes-green, flatten kept as fallback for un-migrated shapes so the suite never regresses):
  - [ ] **RK-GRAM-3a — IR shape + lowering seam.** Add the grammar-box IR kinds (reuse `IR_ALT` for alternation; new `IR_GLIT` literal-match, `IR_GCC` char-class-match, `IR_GSUBRULE` subrule-call; sequence via existing chaining). Lower a rule/token body into a box graph instead of the flatten string, behind a per-rule gate so only migrated shapes take the new path. GATE: `rule TOP { "a" }` lowers to a box graph (dump shows `IR_GLIT`, no `nfa_build` call); all existing grammar smokes still green via fallback.
  - [ ] **RK-GRAM-3b — leaf boxes (literal + char-class), native, both modes.** `bb_rk_glit.cpp`: match fixed string at `[Σ+δ]` (RO bytes sealed `[rip+disp]`), `δ<Δ` bounds, advance δ by len on γ, ω on mismatch/EOS. `bb_rk_gcc.cpp`: match one char at `[Σ+δ]` against a sealed cset, advance δ by 1, γ/ω. δ=R14, Σ=R13, Δ=R15. GATE: `rule TOP { "abc" }` and `rule TOP { <digit> }` PASS both modes via the native boxes (dump: zero NFA on the `.parse` path for these).
  - [ ] **RK-GRAM-3c — sequence + the δ-save backtrack slot.** Chain leaf boxes (each γ → next α); claim the per-activation ζ-frame δ-snapshot slot at each choice point. GATE: `rule TOP { <digit> <alpha> }` PASS both modes natively.
...
**2026-07-14 (Claude Opus 4.8 · SCRIP LOCAL commit `788e6247` on top of `1ed0a3d1`; corpus untouched · .github edited · NOT pushed — `handoff_status.sh` is the push-truth, run it live; do NOT trust this line). RK-GRAM-3b PART 2 — `bb_rk_gcc.cpp` CHAR-CLASS LEAF, both media, inert+gated, proven neutral.** Char-class sibling of `bb_rk_glit`. Pure stackless flat leaf: bounds-check δ<Δ (`r14d`/`r15d`); load char at `[Σ+δ]`=`[r13+rcx]`; `strchr` against RO-sealed cset (the `bb_scan_any` membership idiom + `def L(0)`/`.quad LS(0)`/`.string` seal trailer); ω on non-member/EOS; on member `add r14,1` (advance δ by 1) then γ. NO ζ RW state, NO value slot — γ/ω ARE the result, exactly like `bb_rk_glit`. Language-blind (dispatches on IR shape, zero `is_raku`); ALL purity FACT RULES pass (0 `MEDIUM_*`, 0 raw-byte producers, 22 pure `x86()`/port calls — targeted grep confirmed). WIRED `IR_GCC` through BOTH emit dispatch levels (template switch + `emit_drive` prep, mirroring `IR_GLIT`); proto in `bb_templates.h`; BOTH Makefile paths. Lowerer: `rk_gram_class_members` (class name → member-char string for the strchr box) + `rk_gram_pure_charclass` (single-`<cc>` shape detector); `rk_lower_grammar_boxes` builds `IR_GCC` for `rule TOP { <digit> }` and siblings (digit/alpha/alnum/upper/lower/space/xdigit); literal path unchanged. **SUBSET NOTE (honest):** only SINGLE-char classes accepted — `<ws>` EXCLUDED (it is `[...]*` quantified; belongs to 3f, not a leaf). Same `RK_GRAM_NATIVE` env seam: default `--run`/`--compile` UNTOUCHED. GATES: `--dump-ir` on `rule TOP { <digit> }` shows `; proc gram__G__TOP` + a `GCC` node; `--compile` under the flag emits correct TEXT (`movzx esi,[r13+rcx]` / `strchr@PLT` / `add r14,1` / sealed `"0123456789"`) that assembles+links clean via `as` + `gcc -no-pie -lscrip_rt`. NEUTRALITY PROVEN (flag off): default emission box-ABSENT (0 occurrences in `.s`); Raku smoke 210/20 both modes, failure SET byte-identical to baseline (same 20 pre-existing ALIGN-INV OO/multi-dispatch fails: attr_mutate build_inherited_order class_method for_range_pointy_toplevel handles_delegate handles_two_attrs mi_two_parents_methods mro_tweak_order_3 multi_arity multi_colon_d_dispatch multi_method_arity multi_method_mro_inherited multi_method_type multi_two_typed_args multi_type_int_str multi_typed_beats_untyped op_overload_mixed_obj_int raku_str_gist_explicit role_two_roles_distinct tweak_inherited_order — zero added, zero fixed); Icon 12/12 both modes, SNOBOL4 7/7; clean `make scrip` + `make libscrip_rt`. TOUCHED: NEW `src/templates/bb_rk_gcc.cpp`; `src/templates/bb_templates.h` (+proto), `src/emitter/emit.cpp` (+2 dispatch cases), `src/lower/lower_raku.c` (+charclass helpers & IR_GCC lowering), `Makefile` (+2 entries).
**TRAMPOLINE DESIGN CAPTURED — `FINDING-2026-07-14-CLAUDE-RK-GRAM-3B-PARSE-TRAMPOLINE-DESIGN.md` (.github, this session).** Both leaf boxes (glit+gcc) now lower+emit but do NOT yet EXECUTE on the `.parse` path — the NFA fallback still runs (by design; boxes are inert/gated). The finding fully specifies the scan-entry trampoline that makes them execute, ready for mechanical execution by a fresh full-budget session. KEY FACT verified by dumping the emitted proc: `proc_gram__G__TOP` reads Σ/δ/Δ (r13/r14/r15) but its α-preamble NEVER SETS THEM — so entering via `rt_call_proc_descr` scans garbage; the proc DOES expose `.global` `_α/_β/_γ/_ω` ports and its γ/ω terminals `ret` a `spec_t` (rax=1 match / rax=99 fail) with δ live in r14. Trampoline = "`rt_call_proc_descr`, but load r13/r14/r15 from the subject first and read r14 back," intercepting `grammar_parse_core` (by_name_dispatch.c:381) with a native branch gated on `rt_proc_get_fn("gram__G__TOP")!=NULL` (auto-inert until the flag flips; NFA fallback kept for regex `~~` and un-migrated shapes). Recommended impl = Shape A (a naked-asm `rk_gram_enter_box` runtime shim reusing `rt_call_proc_descr`'s frame construction for zls balance) — one open question for Lon: whether that fixed runtime asm is inside TEMPLATE-ONLY EMISSION's spirit (it emits NO program instructions; it is plumbing). See the finding for the full C+asm, the frame caveat, the 5 both-mode smokes to add, and the 3c hand-off.
**NEXT (leverage order):** (1) **the `.parse` trampoline** per the finding — fresh full-budget session; read the finding + `ARCH-x86.md`/`ARCH-SCRIP.md`/`ARCH-ICON.md §String-scanning` (all read this session, distilled into the finding). Landing it flips `RK_GRAM_NATIVE` default-ON for the literal+char-class shapes and delivers RK-GRAM-3b's "zero `nfa_build` on the `.parse` path" clause. (2) Then 3c (sequence + the δ-save ζ slot) — builds directly on the trampoline (a sequence proc is the same entry over a ≥2-leaf box graph chained γ→α). (3) 3d alternation needs its own `IR_GALT` (IR_ALT is NOT in live IR.h; SNOBOL4 alt kinds are scan-specific — do not reuse). Parallel/independent: the 20 ALIGN-INV Raku OO regressions (staged-proc-call arg marshaling + concat-return path, rbp-free) remain open and are NOT grammar work.
