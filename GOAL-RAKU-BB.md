...
**STATE (2026-06-12 SESSION 5) — m3 16 PASS / 0 FAIL / 15 EXCISED; m4 16 PASS / 0 FAIL / 15 EXCISED. GROUP C FULLY CLOSED (m3 AND m4). Zero silent native FAIL — every native mode is PASS or LOUD [SMX] EXCISE. SCRIP HEAD `cf8006a`.**

- **Modes:** m2 **31/31** (HARD ✓). m3 **16 PASS / 0 FAIL / 15 EXCISED**, m4 **16 PASS / 0 FAIL / 15 EXCISED** — done-bar met (PASS-or-EXCISED, never silent FAIL). Peers INVARIANT: Icon m2 12/12 m3 10/12 m4 10/12, SNOBOL4 m2 7/7 m3 7/7 m4 7/7, NFA oracle **5/5 PASS** (verdicts + cset/anchor + captures), `g_vstack`=0.

- **FIXED THIS SESSION (commit `cf8006a`) — two root causes, both the prior-session NEXT items:**
  1. **m4 silent-fail (4 tests: `list_construct_read`, `array_sort`, `array_reverse`, `hash_sigil_delete`)** — `bb_call_fn_str` (the builtin `rt_call_arr` arm) built a `.Lrkfn%d` fn-name label then emitted the load via `x86("lea","rdi", std::string("[rip + ")+fl+"]")`. The `lea` dispatcher in `x86_asm.h` has NO plain-label-string form — it fell through to `return std::string()`, so the `lea rdi` was **silently dropped**. `rt_call_arr` then ran with garbage `rdi` (no fn name) → dispatch missed → binary exited 0 with no output. FIX: emit the load as `x86("directive", (" lea rdi, [rip + "+fl+"]").c_str())`, the exact proven idiom `bb_call_byname_str` already uses. (The `.string` directive WAS emitted; only the dependent `lea` was lost — that's why the asm looked plausible but produced nothing.)
  2. **`say_jct`/`say_list` (m3+m4, 2 tests)** — `rt_write_any_nl` (`io_format.c`) `fwrite`d the raw DESCR string, printing the SOH/ETX control bytes literally (`1^A2^A3`, `^Ca^A...`). The correct junction/array formatter already existed: `rk_write_str` in `by_name_dispatch.c` (ETX-junction → `any(1, 2, 3)`; bare-SOH array → space-join). FIX (NO-DUP rule — reuse the one formatter, don't clone it): un-`static` `rk_write_str`, forward-declare it in `io_format.c`, route `rt_write_any_nl`'s string branch through it.

- **NEXT (ordered) — GROUP B then GROUP A (the 15 remaining EXCISED: 9 in B, 5 in A, plus map/grep emit):**
  1. **GROUP B (9 EXCISED): `__rk_bool` dval==2.0 sub-graph condition → branch.** Tests: `jct_any/all/one/none/infix/str/nested`, `bool_truthiness`, `bool_compare_store`. IR shape (verified `--dump-bb` + `lower_raku.c` L217/L274): `IR_CALL sval="__rk_bool" ival=1 dval=2.0`; `bk->γ.node`=truthy target, `bk->ω.node`=falsy target; the condition is a **separate nested `IR_graph_t`** stored as `blks[0]` in `IR_EXEC(bk).counter` (an `IR_graph_t**`), built by `rk_arg_block(cx, cond_tree)` — NOT an inline operand. **m2 reference semantics (`IR_interp.c` L2335+):** `bb_reset(blks[0]); v = IR_interp_once(blks[0]);` then the relop/junction sub-graph itself returns FAIL-on-false / value-on-true, so `return IS_FAIL_fn(v) ? bk->ω.node : bk->γ.node`. **Native plan:** emit `blks[0]`'s boxes inline (its own γ/ω wire to the box's γ/ω), exactly the sub-graph-emission `bb_call_byname_str` already does for junction *value* args — but here the consumer is CONTROL FLOW (success→γ, fail→ω), not a value marshal. THREE sub-shapes to cover: (a) **plain relop** `(1>2)` → `bool_compare_store` (assign-relop, L217: γ→LIT_I 1, ω→LIT_I 0); reuse `bb_binop_relop` encoders. (b) **junction-collapse** `$x==any(...)` → `jct_*` (the cond sub-graph is a relop whose RHS is `__rk_jct_*`; collapse already works in m2 via `rt_jct_relop`/`junction_collapse` in by_name_dispatch.c). (c) **bare truthiness** `True`/`$x` → `bool_truthiness` (cond sub-graph is a LIT/VAR; reuse the EXISTING `bb_call_rk_bool.cpp` dval==0.0 slot-truthiness arm via `rt_rk_is_truthy`). Start with (a) — simplest, proves the sub-graph-control-flow mechanism — then (c), then (b). GATE: `__rk_bool dval==2.0` is BLOCKED at three sites in `src/driver/scrip.c` (`icn_rhs_kind_ok` ~L208, `icn_graph_native_emittable_mode` ~L259, EXCISE pre-flight ~same) which all `return 0` for `(__rk_bool||__rk_try)` dval==2.0. Flip `__rk_bool` (keep `__rk_try` excised) per-sub-shape as each emit arm lands — and add a per-sub-shape `icn_*` predicate so unbuilt shapes stay cleanly EXCISED, never abort.
