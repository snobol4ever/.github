...
Per Lon (2026-06-27) the grammar/regex work is **UN-PARKED** and is now the lead. The OO ladder is essentially complete (every rung `[x]`/`[~]`; the cheap wins are exhausted, remaining items are deferred tails with named costs). **RK-GRAM-3** — the recursive-descent grammar engine seam — is the first rung; mind its standing requirement (in the GRAMMAR/REGEX DIRECTION section below): it needs a **FULL context budget** and the `ARCH-x86.md`/`ARCH-SCRIP.md` reads done **first**, so it should open a fresh session rather than tail onto a spent one. RK-EMIT-MAP/GREP stays blocked on Icon GZ-7; RK-GRAM-4..6 sit on RK-GRAM-3. (Superseded prior banner: "RAKU OOP IS THE LEAD," Lon 2026-06-15 — the OO lead delivered its ladder.)
