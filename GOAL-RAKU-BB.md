...
**STATE (2026-06-12 SESSION 6) — m3 17 PASS / 0 FAIL / 14 EXCISED; m4 17 PASS / 0 FAIL / 14 EXCISED. GROUP C FULLY CLOSED. GROUP B sub-shape (a) [plain-relop assign-store] CLOSED (m3 AND m4). Zero silent native FAIL — every native mode is PASS or LOUD [SMX] EXCISE. SCRIP HEAD `5065b30`.**

- **Modes:** m2 **31/31** (HARD ✓). m3 **17 PASS / 0 FAIL / 14 EXCISED**, m4 **17 PASS / 0 FAIL / 14 EXCISED** — done-bar met (PASS-or-EXCISED, never silent FAIL). Peers INVARIANT: Icon m2 12/12 m3 10/12 m4 10/12, SNOBOL4 m2 7/7 m3 7/7 m4 7/7, NFA oracle **5/5 PASS** (verdicts + cset/anchor + captures), `g_vstack`=0.

- **FIXED THIS SESSION (commit `5065b30`) — GROUP B sub-shape (a): `__rk_bool` dval=2.0 plain-relop assign-store (`bool_compare_store`):**
  1. **Emit arm** — new `bb_call_rk_bool_cond_str` in `bb_call.cpp` (dispatched from `bb_call` for `g_descr_flat_chain && dval==2.0 && fn=="__rk_bool"`, before the LANGUAGE-BLIND bomb): walks `blks[0]->entry`'s γ-chain to the relop, emits `arith_opnd_a→rax`/store-scratch/`arith_opnd_b→rcx`/reload/`cmp rax,rcx` + `x86(relop_fail_mnem,"ω")` (false→ω) + `jmp "γ"` (true) + `def "β"` + `jmp "ω"`. Reuses the existing `arith_operands`/`arith_opnd_a`/`arith_opnd_b`/`relop_fail_mnem`/`arith_kind_ok` helpers (same idiom as `marshal_call_arg`'s boolean-relop path).
  2. **Gate** — new per-sub-shape predicate `icn_rk_bool_cond_emittable` in `scrip.c`: un-excises ONLY `__rk_bool` dval=2 whose cond sub-graph walks to a plain integer relop with `icn_rk_arith_operand_ok` operands (LIT_I or arith-binop). `icn_graph_native_emittable_mode` ~L274 flipped: `if (icn_rk_bool_cond_emittable(nd)) {} else return 0;`. `__rk_try` and the jct/bare-truthiness shapes stay cleanly EXCISED (operand whitelist deliberately excludes IR_CALL/VAR).
  3. **Phi-merge** (`emit_bb.c`) — root cause: the two `IR_LIT_I` arms (γ→1, ω→0) feeding one `IR_ASSIGN` wrote DIFFERENT per-box slots, but `op_a_slot = bb_slot_get(γ-lit)` reads only the γ-arm slot, so the ω/false path left it uninitialized (m3 printed empty for `(1>2)`). FIX: when the `__rk_bool` box is walked (BEFORE its lit children) and both γ/ω are `IR_LIT_I`, pre-allocate ONE shared 16-byte slot and register BOTH lit keys to it (new `bb_slot_register`); `IR_LIT_I` walk switched to `bb_slot_alloc16_or_get` so both arms reuse the shared slot. ASSIGN then reads the merged slot. Emit arm needs no merge logic — the lits do the value storage.

- **NEXT (ordered) — GROUP B then GROUP A (the 15 remaining EXCISED: 9 in B, 5 in A, plus map/grep emit):**
  1. **GROUP B (8 EXCISED remaining): `__rk_bool` dval==2.0 sub-graph condition → branch.** Sub-shape **(a) plain-relop assign-store DONE** (commit `5065b30`, `bool_compare_store` PASS m3+m4). Tests still EXCISED: `jct_any/all/one/none/infix/str/nested` (7), `bool_truthiness` (1). **CRITICAL CORRECTION discovered session 7:** `bool_truthiness` is actually `dval=0.0`, NOT `dval=2.0`. `rk_cond_wrap` in `lower_raku.c` L32 builds `__rk_bool dval=0.0` for bare `if(expr)` and pushes the condition as an inline operand (`ir_operand_push(bk, r)`). The `dval==2.0` gate at ~L277 does NOT apply; `dval=0.0` already passes `icn_rhs_kind_ok` at L207 (`IR_CALL dval==0.0 → 1`), and `bb_call_rk_bool.cpp` already handles `dval=0.0` truthiness via `rt_rk_is_truthy`. So `bool_truthiness` is EXCISED by something ELSE in `icn_graph_native_emittable_mode` — all opcodes in its IR dump are safe (`ASSIGN CALL CONJ FAIL LIT_I LIT_S SUCCEED`). Likely culprit: IR_VAR check at scrip.c ~L286: `if (nd->op == IR_VAR && sval[0] != '&' && state != 1 && !icn_graph_var_assigned_or_param(...)) return 0;` — the condition VAR `$x`/`$y` is used as a condition argument but may not pass `icn_graph_var_assigned_or_param` (which only checks for `IR_ASSIGN` with matching sval, not parameter scope). Fix plan: add a temp `fprintf(stderr,...)` before each `return 0` in `icn_graph_native_emittable_mode` to find the exact trigger, then broaden the gate (e.g., recognize that a VAR feeding a `__rk_bool dval=0.0` operand is safe). The existing dval=0.0 emit arm needs no changes — just the gate admission. Test file: `/tmp/bool_truth_smoke.raku` (4 cases: `if(True)/if(False)/if($x)/if($y)`, expected output `t f t f`). IR shape (verified `--dump-bb`): `__rk_bool dval=0.0` nodes at indices 8/17/24, γ/ω wired directly to `CONJ` statement-flow targets, condition in `operands[0]`. **Sub-shape (b) and (c) ordering re-stated correctly:** (c) = bool_truthiness (`dval=0.0`, gate-only fix, 1 test). (b) = jct collapse (`dval=2.0`, relop RHS is `__rk_jct_*` IR_CALL, 7 tests). Do (c) first. **Sub-shape (a) machinery (all reusable):** `bb_call_rk_bool_cond_str` + `rkbool_cond_relop` (bb_call.cpp L482/L489), `icn_rk_bool_cond_emittable`/`icn_rk_arith_operand_ok` (scrip.c L221/L227), `bb_slot_register` + `bb_slot_alloc16_or_get` (emit_bb.c L104/L80). dval=2.0 gate flip at scrip.c ~L277. Keep `__rk_try` excised.
  2. **GROUP A (5 EXCISED): GATHER/MAP/GREP IR nodes.** `gather_take`, `map_range`, `grep_range`, `map_over_gather`, `grep_over_gather`. Complex generator machinery (`bb_rk_gather.cpp` exists for one shape; `bb_rk_map`/`bb_rk_grep` blocked on Icon GZ-7 IR_ASSIGN ζ-slot store). Tackle after B.

- **Done (history):** RK-LOWER-0..5h, RK-NFA-ORACLE-FIX, RK-EMIT-1/2/3+GATHER, RK-HY-0..3, RK-NFA-1/2/3, RK-M34-1, while_loop fix, bb_call_fn MEDIUM arm, gate BUG1+BUG2, ONE-MEDIUM rk_bool, lbl_β double-colon, x86_uid dup-label, Group-C gate+emit (m3 AND m4 — fn-name lea drop + say array/jct format) (git log).
---
---
