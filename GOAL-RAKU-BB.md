...
**LIVE CURSOR — RK-GRAM-3c (sequence + the δ-save ζ slot). Fresh full-budget session. Read `ARCH-x86.md` + `ARCH-SCRIP.md` + `ARCH-ICON.md §String-scanning` + `x86_asm.h` first (rung requirement). A sequence proc is the same trampoline entry over a ≥2-leaf box graph chained γ→α, with the δ-snapshot slot claimed in the ζ frame at each choice point. GATE: `rule TOP { <digit> <alpha> }` PASS both modes natively.**
