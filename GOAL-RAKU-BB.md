...
**STATE (2026-06-12) — RK-LOWER: m2 25/25 FULL GREEN (was 21/25). Two fixes in lower_rv(): (1) TT_ARR_GET: replaced broken IR_IDX chain-wiring with lower_rcall("arr_get", 0, 1) — arr_get is already 0-based, correct Raku semantics; (2) TT_FNC: intercept nm=="hash_set"/"hash_delete" (TT_FNC function-call form), route to pure-variant writeback (IR_ASSIGN <- hash_set_pure/hash_delete_pure) matching the push/TT_HASH_SET/TT_HASH_DELETE precedent — was hitting try_call_builtin_by_name which has no "hash_set" entry. m3/m4: 1 PASS / 0 FAIL / 24 EXCISED (clean). Peers: Icon m2 12/12, SNOBOL4 m2 7/7, NFA oracle 5/5, g_vstack=0. SCRIP HEAD 6541335.**
