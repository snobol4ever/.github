...
**NEXT RUNG:** RK-GRAM-3d — alternation (`IR_ALT`) with δ-restore-on-β. Needs a NEW `IR_ALT` kind (does not exist; `IR_MATCH_ALTERNATE` is SNOBOL4's different mechanism), a new emitter path (try arm-1 → on ω restore saved δ from ζ frame → try arm-2), and the ζ δ-save slot placed in grammar procs' 256B scratch frame. **Fresh full-budget session required** (real new-IR-kind + new-emitter-path work). GATE: `rule TOP { <digit> | <alpha> }` PASS both modes; backtracking PROVEN by `rule TOP { "ab" | "ac" }` on "ac".
