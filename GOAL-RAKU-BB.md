...
**STATE (2026-06-13 SESSION 7 — ANALYSIS ONLY, NO COMMIT) — m3 17 PASS / 0 FAIL / 14 EXCISED; m4 17 PASS / 0 FAIL / 14 EXCISED. Full root-cause analysis of all 14 EXCISED complete. Fix plans documented below and in HANDOFF-2026-06-13-SONNET46-RAKU-BB-EXCISE-ANALYSIS.md. SCRIP HEAD `f766a9d` (unchanged).**
...
- **ANALYSIS SESSION 7 — root causes of all 14 EXCISED pinned:**

  **GROUP B-c (1 test) — `bool_truthiness`:** `TT_IF` lowers to `__rk_bool dval=2.0` with condition in sub-graph. For `if(True)/if(False)/if($x)/if($y)`, sub-graph entry is `LIT_S`/`LIT_I`/`IR_VAR` — not a BINOP relop. `icn_rk_bool_cond_emittable` rejects → EXCISED. **Fix:** new gate predicate `icn_rk_bool_truthy_emittable`; new emit arm `bb_call_rk_bool_truthy_cond_str` inlining tag+value for LIT_I (tag=6,ival), LIT_S (seal via `x86_ro_seal_str`, tag=1), VAR (`bb_varslot(name)`), calling `rt_rk_is_truthy(rdi,rsi)` → branch γ/ω. Update `bb_call_rk_bool_cond_str` to call it when `rkbool_cond_relop` returns NULL. NOTE: PRIOR WATERMARK ANALYSIS was WRONG — session 7 confirmed `TT_IF` always uses `dval=2.0` (not `dval=0.0`), so this is a `dval=2.0` gate extension, not a `dval=0.0` path issue.

  **GROUP B-b (7 tests) — `jct_any/all/one/none/infix/str/nested`:** Sub-graph for `if($x == any(1,2,3))` has `BINOP(EQ, VAR(x), CALL(__rk_jct_any, dval=0.0))`. `icn_rk_arith_operand_ok(rb)` where `rb=CALL dval=0.0 __rk_jct_*` → 0. **Fix:** extend `icn_rk_arith_operand_ok` to accept `IR_CALL dval==0.0 sval=__rk_jct_*`. New emit arm `bb_call_rk_bool_jct_cond_str`: marshal `ra=VAR` via `bb_varslot` → lhs_slot; marshal `rb=CALL(__rk_jct_any)` via `marshal_single_call(rb, rhs_slot, ...)` → rhs_slot; load slots into rdi+rsi / rdx+rcx; `r8d = binop_to_tt(relnd)`; call `rt_rk_jct_relop(DESCR_t lhs, DESCR_t rhs, int op)`. New runtime fn `rt_rk_jct_relop` in `by_name_dispatch.c`. Dispatch: `bb_call_rk_bool_cond_str` calls jct arm when `!arith_kind_ok(ra)||!arith_kind_ok(rb)`. **IMPORTANT:** `ra`/`rb` may be in `operand_aux` — `arith_operands` handles this fallback; verify non-NULL before marshalling. `jct_str` already PASSES (different sub-graph shape, not `__rk_bool dval=2`).

  **GROUP C (1 test) — `class_method`:** `ASSIGN` RHS is `IR_FIELD_GET` — `icn_rhs_kind_ok` rejects it. **Fix:** add `if (r->op == IR_FIELD_GET) return 1;` to `icn_rhs_kind_ok`; add to `icn_assign_safe_kind`. New template `bb_field_get.cpp`: load object VAR via `bb_varslot` → rdi+rsi; seal field name → rdx; call `rt_rk_field_get_v(DESCR_t obj, const char *field)` → DESCR_t in rax:rdx; store to own slot; `cmp eax,99; je ω; jmp γ; def β; jmp ω`. New runtime fn `rt_rk_field_get_v` wrapping `data_field_ptr`. Add dispatch to `emit_core.c`; declare in `bb_templates.h`; add to Makefile.

  **GROUP A (5 tests) — `gather_take/map_range/grep_range/map_over_gather/grep_over_gather`:** `IR_GATHER`/`IR_MAP`/`IR_GREP` as ASSIGN RHS, no `bb_rk_map`/`bb_rk_grep` templates. Blocked on Icon GZ-7. DO NOT ATTEMPT.

- **NEXT (ordered) — implement B-c, then B-b, then C, each with smoke-gate before commit:**
  1. B-c: `bool_truthiness` (gate + truthy emit arm). Expected result: 18 PASS m3/m4.
  2. B-b: `jct_any/all/one/none/infix/nested` (extend gate + jct emit + `rt_rk_jct_relop`). Expected: 25 PASS m3/m4. (`jct_str` already PASSES — don't count it as a gain.)
  3. C: `class_method` (FIELD_GET gate + template + `rt_rk_field_get_v`). Expected: 26 PASS m3/m4.
  4. A: DEFERRED (blocked on GZ-7).
