...
- [ ] **RENAME-C — de-language-name the NFA/regex engine** (`raku_nfa_*`/`raku_nfa_bb_*`/`Raku_nfa`/`Raku_match`/`g_raku_match`/`g_raku_subject`/`raku_re*` → `nfa_*`/`re_*`/`Match`/`g_match`/`g_subject`). Runtime-invoked from interp+runtime; ~6 files incl. `raku_re.h`; NOT asm-emitted (m3/m4 EXCISE). One commit; gate: `test_gate_raku_nfa_oracle.sh` 5/5 + all smokes invariant. See LANGUAGE-PREFIX PURGE in the watermark.
- [ ] **RENAME-SNO — de-language-name the SNOBOL4 m4 EMITTED labels** in `scrip.c` (`sno_proc_startup`/`sno_flat`/`.Lsno_*`/`sno_pidx_buf` → neutral). DO NOT touch `sno_parse_ast`/`sno_add_include_dir` (parser entry points, allowed). Audit `pl_*`/`pas_*` emitter labels too. Gate: SNOBOL4 m4 7/7 byte-stable.
- [ ] **RK-NFA-4a-SMOKE — lock the `~~` verdict in the harness.** Add a `smatch_verdict` test to `test_smoke_raku.sh` (`'abc123' ~~ /\d+/` truthy, `/^\d+$/` falsy) so the harness — not just the self-comparing oracle gate — guards the m2 verdict (m3/m4 EXCISE). (4a foundation already landed in working tree; see watermark.)
- [ ] **RK-NFA-4 — NATIVE NFA-on-Byrd-boxes, mode-3/4 (UN-SHELVED 2026-06-14 by directive).** Compile the regex/grammar NFA into native `IR_NFA_*` Byrd-boxes and emit them in m3/m4, so `~~` and grammar `.parse` run on BBs natively (not the runtime C matcher). **CENTRAL LAW — NO GLOBAL VALUE STACK.** The earlier NFA-in-BB attempt was shelved because native `SPLIT` backtracking reached for a value stack to hold thread/capture-save state; that is a FACT-RULE violation (lines 157-185). Backtracking MUST ride the four-port box-resumption protocol (γ = matched-advance, ω = fail/redo) — the SAME stackless mechanism already proven in `bb_alt.cpp` (Icon `IR_ALT` gen-alt), `bb_match_alt.cpp` / `bb_pattern_alt.cpp` (SNOBOL4 pattern alternation), and `bb_gen_scan.cpp`. `IR_NFA_SPLIT` is structurally a binary choice point identical to `IR_ALT`: model its native arm on `bb_match_alt.cpp`. The mode-2 oracle (`nfa_bt_ir_cap` in `raku_nfa_bb.c`) shows the reference semantics — C recursion + an n×(slen+1) `vis[]` memo grid to cut epsilon-loops, NO value stack; the native boxes reproduce that ordered leftmost walk via port resumption + a per-sequence `[ζ]` slot for the (node,pos) memo, never a global arena. **Current state:** no `bb_nfa_*.cpp` template exists yet; `~~` lowers to a single `IR_PAT_MATCH` box (runtime); `IR_NFA_*` boxes today live only inside `raku_nfa_bb_exec` (built on the fly, C-walked) and are oracle-validated by `test_gate_raku_nfa_oracle.sh` (RK-NFA-1/2/3, 5/5). **STAGED PLAN (each stage its own green commit; m2 stays the oracle; m3/m4 PASS-or-EXCISE never abort):** (4a) lowering: make `~~`/`.parse` emit the NFA-as-`IR_NFA_*`-graph (via `raku_nfa_to_bb`) instead of the opaque `IR_PAT_MATCH`/runtime call, behind the existing m2 walker so m2 stays green. (4b) native arms for the NON-branching boxes — `bb_nfa_char/any/class/bol/eol/eps/cap_open/cap_close/accept.cpp` (match-and-advance on γ, take ω on miss) + the leftmost start-offset sweep as a bounded box loop (NOT recursion); target linear patterns `/\d/`, `/abc/`, `/\w+ /`-free anchored forms in m3/m4. (4c) `bb_nfa_split.cpp` — the choice point: γ tries arm-1; on ω-resume, try arm-2; mirror `bb_match_alt.cpp`'s rel32 redo idiom; add the `(node,pos)` memo in a `[ζ]` slot to bound work + cut epsilon loops, exactly as `nfa_bt_ir_cap`'s `vis[]` does — stackless. Now quantifiers/alternation work in m3/m4. (4d) captures: `CAP_OPEN/CLOSE` boxes write group spans into per-sequence slots; converge `$<name>`/`$0` reads onto BB-emitted reads (this RETIRES the `re_named_capture` IR_CALL dval=2 path that ABORTS in m3/m4 — the current `$<name>` blocker). (4e) converge `~~` and grammar `.parse` onto the one BB matcher; delete the runtime-matcher escape for emitted code. **GUARD throughout:** `scripts/test_gate_no_vstack.sh` `g_vstack` line stays 0; no new push/pop arena under any name; `test_gate_raku_nfa_oracle.sh` stays 5/5 (the boxes must equal the oracle byte-for-byte).
- [ ] **RK-NFA-5 — greedy/longest-match + multi-capture correctness on the BB path.** The current matcher returns *a* valid split, not PCRE-greedy (`12abc` vs `<num>(\d+)\s*<word>(\w+)` → "1"/"2abc"). On the BB rebuild, make `SPLIT` arm-ordering encode greedy-first so leftmost-longest falls out of the ordered port walk; validate against the oracle with multi-capture probes. (Was folded into the shelved RK-NFA-4/5; now its own rung.)
- [ ] **RK-GRAM-3 (THE SEAM) — subrule `<name>` backtracking via the generator PUMP.** Resume-and-yield-next across subrule call boundary, Match-tree build. Routes through IR_* SUSPEND/ALT/PUMP (already exist). **PREREQUISITE LANDED 2026-06-14 (`f3b1837`): grammar `.parse` is now end-to-end in all 3 modes via the gram_expand inline-expansion path — grammars register (`rt_grammar_register`), `G.parse(s)` dispatches through `meth_call`→`grammar_parse_core`, and the bareword grammar name lowers to `IR_LIT_S` (survives + dodges the free-var EXCISE gate). RK-GRAM-3 now replaces the NFA call inside `grammar_parse_core` with the PUMP and builds a real Match object (then `$m<name>` named-capture access). See the 2026-06-14 watermark for the full wiring + the `scrip` static-runtime gotcha. **RECON (2026-06-14, in the handoff ADDENDUM): the Match-object/`$<name>` deliverable is coupled to two PRE-EXISTING, non-grammar blockers — (1) `$<name>` (`re_named_capture`, IR_CALL dval=2) ABORTS in m4 / empty in m3 (must be made emittable-or-cleanly-excised first; same failure via plain `~~`), and (2) NFA capture greediness gives wrong spans for adjacent quantified captures (`12abc`→"1"/"2abc"). The two cheap grammar edits (named-group emission in `gram_expand`; publishing `g_raku_match`/`g_raku_subject` from `grammar_parse_core`) are proven correct + non-regressing (35/35) and ready to re-apply once a cross-mode-testable `$<name>` consumer exists. Recommended order: fix `$<name>` m3/m4 → re-apply grammar edits + single-capture test → NFA greediness as its own item → PUMP cross-subrule backtracking last.**
...
- [ ] **RK-NFA-CONV — collapse `IR_NFA_CHAR/CLASS/ANY/BOL/EOL` ↔ SNOBOL4 `IR_PAT_*` into `IR_MATCH_*`** only where byte+semantics identical; SPLIT + captures stay separate. (UN-DEFERRED 2026-06-14; do AFTER RK-NFA-4 native arms exist so both languages share the converged boxes.)
...
**LANGUAGE-PREFIX PURGE (2026-06-14, by directive) — runtime/post-lower must be language-AGNOSTIC; name by CS terminology, never by the language that uses the technique.** Prefixes/suffixes `icn_`/`raku_`/`rk_`/`sno_`/`pl_`/`pas_` are ALLOWED ONLY in `src/parser/` and `src/lower/`. Every stage AFTER lower (`driver/ interp/ runtime/ emitter/ machine/ contracts/`) must carry zero language prefix on any VARIABLE, FUNCTION, or FILE name. NOT YET COMMITTED — landed in the working tree only.
  ...
  - **TODO — RENAME-C (NFA/regex engine):** `raku_nfa_*` / `raku_nfa_bb_*` / `Raku_nfa` / `Raku_match` / `g_raku_match` / `g_raku_subject` / `raku_re*` → CS-neutral (`nfa_*` / `re_*` / `Match` / `g_match` / `g_subject`). It is invoked at RUNTIME from `IR_interp.c` + `by_name_dispatch.c` but its `.c`/`.h` sit in `src/parser/raku/` (incidental filing — the arch doc calls it "the isolated regex matcher"). NOT emitted as asm labels (the path EXCISES in m3/m4), so this is a pure C-symbol rename across ~6 files incl. `raku_re.h`. Coherent unit — do in one commit, re-run `test_gate_raku_nfa_oracle.sh` (5/5).
  - **TODO — RENAME-SNO (emitted labels only):** in `src/driver/scrip.c` rename ONLY the codegen symbols `sno_proc_startup`→`proc_startup_sno?`/neutral, `sno_flat`, `.Lsno_*`, `sno_pidx_buf`. DO NOT touch `sno_parse_ast`/`sno_add_include_dir` — those are PARSER entry points (allowed). Also audit `pl_*`/`pas_*` emitter labels.
...
**STAGE-4a LANDED in working tree (NOT committed) — `IR_NFA_MATCH` foundation + verdict bug FIXED.** `~~ /literal/` "match" now lowers to a new `IR_NFA_MATCH` node that carries (a) the `Raku_nfa*` in `IR_LIT.ival` for the `RK_NFA_BB=0` oracle arm + group names, and (b) the pre-built `IR_graph_t*` NFA-as-BB graph (via `raku_nfa_to_bb`, the 4a deliverable) + the subject arg-block in a 2-entry `IR_EXEC.counter` block array. m2 arm in `IR_interp_node` honors `RK_NFA_BB` exactly like `re_match` (env 0 → `raku_nfa_exec`, env 1 → `raku_nfa_bb_graph_exec`). m3/m4 EXCISE via a new top-level guard in `icn_graph_native_emittable_mode` (now `graph_native_emittable_mode`): `if (nd->op == IR_NFA_MATCH) return 0;`. Non-literal/`match_global`/`subst` `~~` still falls through to the unchanged `re_match`. **CRITICAL FIX:** `bb_reset` (`scrip_ir.c`) was zeroing `IR_EXEC(counter)` for `IR_NFA_MATCH`, wiping the compile-time block-array pointer → match always FAILed (the `'abc123' ~~ /\d+/`→falsy bug, which the oracle gate could NOT see because it only self-compares the two arms). FIX: added `IR_NFA_MATCH` to the `bb_reset` counter-preservation exemption list (alongside IR_GATHER/IR_MAP/IR_GREP/IR_SUSPEND/IR_CALL-dval2-3). Factored `raku_nfa_bb_exec` → thin wrapper over new `raku_nfa_bb_graph_exec(IR_graph_t*, ngroups, subj, *result)` (oracle behavior byte-identical). Files: `IR.h` (+`IR_NFA_MATCH` enum), `scrip_ir.c` (name table + bb_reset), `raku_nfa_bb.c`+`raku_re.h` (graph-exec split), `lower_raku.c` (TT_SMATCH arm), `IR_interp.c` (m2 arm + raku_re.h include), `scrip.c` (EXCISE guard + `rhs_kind_ok`/`assign_safe_kind` admit IR_NFA_MATCH). VERIFIED: `~~` verdict correct both arms; oracle 5/5; Raku 35/35/35; peers invariant; `g_vstack`=0. NOTE the smoke harness still has NO `~~` test — add a `smatch_verdict` smoke (m2 PASS, m3/m4 EXCISE) so the harness (not just the self-comparing oracle gate) locks the verdict. **NEXT after the rename queue: stage 4b** (non-branching native `bb_nfa_*.cpp` arms) per the RK-NFA-4 rung below.

 The tier-seam "⏸ SHELVED `~~` stays on C matcher" decision is RESCINDED. RK-NFA-4/5 are now active rungs (see the ladder for the full staged, stackless plan); RK-GRAM-4..6 and RK-NFA-CONV un-deferred. The reason NFA-in-BB was shelved before — native `SPLIT` backtracking reaching for a value stack — is addressed head-on: backtracking rides the four-port box-resumption protocol modeled on the EXISTING stackless templates `bb_match_alt.cpp` / `bb_pattern_alt.cpp` / `bb_alt.cpp` / `bb_gen_scan.cpp`, never a side arena. `IR_NFA_SPLIT` ≡ `IR_ALT` (binary choice point). The mode-2 walker `nfa_bt_ir_cap` (C recursion + `vis[]` memo, no stack) is the reference semantics; the native boxes reproduce its ordered leftmost walk via port resumption + a per-sequence `[ζ]` memo slot. No code landed this turn (a ~90%-budget handoff turn) — this records the corrected direction + exact track so implementation starts next session with full budget. **FIRST STEP next session: stage 4a** (lower `~~`/`.parse` to the `IR_NFA_*` graph via `raku_nfa_to_bb`, m2 walker unchanged so m2 stays green), then 4b non-branching native arms. GUARD every commit: `test_gate_no_vstack.sh` `g_vstack`=0; `test_gate_raku_nfa_oracle.sh` 5/5 (boxes == oracle byte-identical).
