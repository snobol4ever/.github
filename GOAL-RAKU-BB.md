...
**STATE (2026-06-13 session 10 — LANDED + PUSHED, SCRIP origin/main `738b950`) — Raku m2 31/31 (HARD ✓); m3 26 PASS / 0 FAIL / 5 EXCISED; m4 26 PASS / 0 FAIL / 5 EXCISED. `class_method` (GROUP C) is now FULL PASS in all three modes IN THE BUILT TREE. The only remaining EXCISED = GROUP A (5: gather_take/map_range/grep_range/map_over_gather/grep_over_gather), hard-blocked on Icon GZ-7. Peers invariant: ICON m2/m3/m4 = 12/12/12, NFA oracle 5/5, g_vstack=0. SNOBOL4 m2 7/7 (HARD ✓); SNOBOL4 m3/m4 `define` is 6/1 — a PRE-EXISTING baseline FAIL (owned by the SNOBOL4 DEFINE-CARVE session, NOT this Raku work; verified identical on a stash-clean rebuild).**
...
**NEXT (ordered):**
  ...
  4. Robustness follow-ups (non-blocking): emit the m4 class-registration startup even when n_procs==0 (class-only programs); generalize the `gvar_chain` postfix loop (emit_bb.c ~3370) to N-ary for symmetry; generalize the same `rt_proc_has_native_fn` guard onto any other post-`ir_delete_all` dispatch that can reach `rk_ir_call_proc` (only `meth_call` does today).
