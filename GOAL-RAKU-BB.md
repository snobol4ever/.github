...
- **NEXT (ordered):**
  1. **C blocker first:** fix the nested-BINOP operand double-emission (chain walker enqueues a BINOP operand that `flat_drive_binop_tree` ALSO emits → duplicate node-id label). Start at `codegen_flat_chain_body` queue-build (emit_bb.c ~3318 enqueues `c->ω.node` for IR_BINOP) and `flat_drive_binop_tree`. Reproduce with a 3-line Raku `sub f($a){ return ($a + 1) * $a; }` under `--compile`. Then re-apply C parts (1)+(2) above and verify `class_method` EXCISED→PASS, m2 31/31, no peer regression, no silent m3/m4 FAIL.
  2. A: DEFERRED (GZ-7).
