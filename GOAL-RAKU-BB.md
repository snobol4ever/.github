...
**⚠ 2026-07-13 TIP BUILD-BLOCKER (P0 — NOT Raku; for the SCAN/dcap owner): the current SCRIP tip does NOT clean-build. `src/templates/bb_scan_alternate.cpp` (added by `eb9ac70e` SCAN-NARY-BOX) calls `rt_dcap_height`/`rt_dcap_restore_to`, which `32bd6106` (rbp-dcap) DELETED — rbp-dcap cleaned the sibling `bb_match_alternate.cpp` but missed this file. Link fails: `undefined reference to rt_dcap_restore_to` (bb_scan_alternate.cpp:51) + `rt_dcap_height` (:24). Unbuildable-from-clean since `eb9ac70e`; was masked by stale `/tmp/si_objs` objects until a clean rebuild exposed it. Likely fix per rbp-dcap's own theorem ("ALTERNATE NEEDS NOTHING; generator LIFO already restores rbp"): delete the obsolete dcap save (α, line 24) / restore (β, line 51) from bb_scan_alternate, mirroring the match-alternate cleanup — but the SCAN owner must confirm scan-alternate's state model matches the theorem before removing. Blocks `handoff_status.sh` (which builds) for everyone until fixed.**
