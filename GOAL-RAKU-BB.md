...
**[ζ TRACK] NEXT RUNG:** RK-ZETA-0 (see RK-ZETA LADDER below) — un-park Raku's ζ onto the ARCH-ZETA §13 two-flavor law (rsp FORTH spine for fixed-cell BBs; heap-promote family-A escapees). Foundation PROVEN this session: SNOBOL4 crosscheck byte-identical MALLOC vs BUMP_LIFO both modes; Icon green under BUMP_LIFO. Full record: `FINDING-2026-07-14-CLAUDE-RK-ZETA-RSP-MODE-INVARIANCE-PROVEN.md`. **Fresh full-budget session** (multi-file carve-out).
...
- [ ] **RK-ZETA-0 — baseline + un-park inventory.** Grep Raku's parked alloc sites (the compat shim). Classify each Raku BB: non-escapee fixed-cell (→ rsp spine) vs escapee/family-A (→ heap-promote). Capture the Raku fail-set under the current `MALLOC` default (`test_smoke_raku.sh` + `test_crosscheck_raku.sh`) as the mode-invariance baseline. GATE: inventory + baseline fail-set recorded in this file's watermark; no code change.
