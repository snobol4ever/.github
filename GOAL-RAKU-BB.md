...
**STATUS (after session 9): m3/m4 = 26 PASS / 0 FAIL / 5 EXCISED. `class_method` (GROUP C) LANDED — full m3+m4 PASS. The only remaining EXCISED is GROUP A (5 tests), hard-blocked on Icon GZ-7.** See the Watermark (bottom) for the authoritative live state.
...
**STATE (2026-06-13 OPUS48 session 9 — LANDED, pre-commit at write time) — Raku m2 31/31 (HARD ✓); m3 26 PASS / 0 FAIL / 5 EXCISED; m4 26 PASS / 0 FAIL / 5 EXCISED. `class_method` (GROUP C) now FULL PASS in all three modes (was EXCISED). The only remaining EXCISED = GROUP A (5: gather_take/map_range/grep_range/map_over_gather/grep_over_gather), hard-blocked on Icon GZ-7. Peers invariant: ICON m2/m3/m4 = 12/12/12, SNOBOL4 7/7/7, NFA oracle 5/5, g_vstack=0.**

**LANDED SESSION 9 (GROUP C — `class_method`). Six coordinated edits across SCRIP (all proven by m3+m4 class_method PASS, zero peer regression):**

1. **"Bug 2" ROOT-CAUSED + FIXED — it was NOT a double-emission; it was a MISSING postfix arity.** `descr_chain_arity` (`emit_bb.c`) had no `IR_FIELD_GET` case → returned `-1` → the postfix operand-hydration pre-pass `descr_chain_operand_refs` treats `-1` as "reset stack (`sp=0`), skip", so the inner `ADD` of `($!x+$!y)*$factor` never got its `operands[]` hydrated (`bb_child0/bb_child1` stayed NULL). FIX: `case IR_FIELD_GET: return 0;` — a leaf value-producer whose object operand is PRESERVED (arity-0 path never resets `n_operands`). The watermark's prior "duplicate node-id label" framing was wrong; with operands hydrated, `descr_binop_opnd_slot` resolves the FIELD_GET slot and the ADD reads it correctly. (`sub g($a){return ($a+1)*($a+2);}` always worked precisely because its operands are VAR/LIT leaves the pre-pass already handled.)

2. **obj_new N-ARY operands — generalized the same postfix loop.** `descr_chain_operand_refs`'s assignment loop only handled arity 1 and 2; a CALL with arity ≥3 (obj_new = `Point.new(x,y)` → arity 5; `P.new(x)` → arity 3) hit `else if (ar>=1){sp=0;}` and got NO operands → `ir_call_arg` returned NULL → `bb_call_fn` marshalled nothing → obj_new received garbage args → returned DT_FAIL → `je ω` → clean exit, no output. FIX: generalized to pop arbitrary N operands in order (`for k=ar..1: ir_operand_push(stk[sp-k])`), preserving the `IR_SCAN` arity-1 special case. (The `gvar_chain` twin loop at `emit_bb.c` ~3370 still caps at 2 — Raku does not use it; note for any future gvar N-ary call.)

3. **FIELD_GET template** — new `src/emitter/BB_templates/bb_field_get.cpp` (declared in `bb_templates.h`, wired in Makefile RT_PIC_SRCS + compile rule). Seals field name via `x86_ro_load_q("rdi",0)`/`x86_ro_seal_str(0,name)`, loads object DESCR from `_.op_a_slot` → `rsi:rdx`, `x86("call","dat_field_get",fptr)` (medium-complete), stores `rax:rdx` → `_.op_off`. Plus `walk_bb_node` (`emit_core.c`) `case IR_FIELD_GET` (descr-chain only): allocates `g_emit.op_off = bb_slot_alloc16(nd)`, calls `bb_field_get()`. (`op_a_slot`/`op_sval` auto-set by walk_bb_node's prologue; object VAR `self` gets its slot from `flat_drive_field_get`'s `walk_bb_flat(obj_box)`.) `dat_field_get(const char*, DESCR_t)` is `interp_data.c:80`, exported in libscrip_rt.so.

4. **GATE** (`scrip.c`): `icn_rhs_kind_ok` admits `IR_CALL dval==1.0` (obj_new/meth_call) and `IR_FIELD_GET`; `icn_assign_safe_kind` admits `IR_FIELD_GET`. (The only gate block for class_method was the ASSIGN rhs `my $p = Point.new(...)` = CALL obj_new dval=1.0.)

5. **m4 CLASS REGISTRATION** — the standalone m4 binary had an EMPTY dat registry (`rk_register_classes` runs at compile time IN the `scrip` process; m3 JITs in-process so it inherits it, but m4 is a separate binary). FIX: added 4 enumerators to `interp_data.c` (`dat_type_count`/`dat_type_name`/`dat_type_nfields`/`dat_type_field`) and a loop in `scrip.c`'s `icn_proc_startup` text emission that emits `record_register("Name(f1,f2,...)")` per registered class (via `record_register@PLT`) before main runs. (Currently emitted only inside the `if (n_procs>0)` startup block — fine for class_method which has methods; a class-only program with zero procs would not get the startup — note for later.)

6. **meth_call m4 DISPATCH** (`by_name_dispatch.c`) — `meth_call` looked up the method via `g_stage2.proc_table`, EMPTY in the m4 binary → FAIL. FIX: build `callargs` first, then if `pi >= proc_count` (m4), stage args into `g_call_args[]` and call `rt_call_proc_descr(procname, total)` — the SAME Raku-return-compatible path a dval=3.0 direct sub-call uses (`say g(3)` proved it: it dispatches via `rt_call_proc_descr@PLT`, which returns frame-slot-0 where Raku's bb_return Layer-B stores the value). NOT `rt_call_named_proc` (that returns via `NV_GET_fn(name)`, the SNOBOL convention — tried first, produced empty method returns). m2/m3 unchanged (they still hit the `g_stage2`/`rk_ir_call_proc` path first).

- **Modes:** m2 **31/31** (HARD ✓). m3 **26 PASS / 0 FAIL / 5 EXCISED**, m4 **26 PASS / 0 FAIL / 5 EXCISED** — done-bar met (PASS-or-EXCISED, never silent FAIL). The 5 EXCISED = GROUP A only.

- **Files touched (session 9):** `src/emitter/emit_bb.c` (descr_chain_arity + postfix loop), `src/emitter/emit_core.c` (walk_bb_node FIELD_GET case + extern), `src/emitter/BB_templates/bb_field_get.cpp` (NEW), `src/emitter/BB_templates/bb_templates.h` (decl), `Makefile` (source + rule), `src/driver/scrip.c` (gate + startup class-reg), `src/driver/interp_data.c` (4 enumerators), `src/runtime/by_name_dispatch.c` (meth_call m4 fallback).

- **NEXT (ordered):**
  1. **GROUP A (5) — gather_take/map_range/grep_range/map_over_gather/grep_over_gather — DEFERRED on Icon GZ-7 (IR_ASSIGN ζ-slot store). DO NOT ATTEMPT** until GZ-7 lands in GOAL-ICON-BB.
  ...
  4. Robustness follow-ups (non-blocking): emit the m4 class-registration startup even when n_procs==0 (class-only programs); generalize the `gvar_chain` postfix loop (emit_bb.c ~3370) to N-ary for symmetry.

- **Done (history):** RK-LOWER-0..5h, RK-NFA-ORACLE-FIX, RK-EMIT-1/2/3+GATHER, RK-HY-0..3, RK-NFA-1/2/3, RK-M34-1, while_loop fix, bb_call_fn MEDIUM arm, ONE-MEDIUM rk_bool, lbl_β double-colon, x86_uid dup-label, Bug 1 proc-double-emit (`33f7202`), user-sub CALL Layers A+B (`f6bbabb`), B-c bool_truthiness + B-b jct relops (`b9a2433`), **GROUP C class_method full m3/m4 (session 9)** (git log).
