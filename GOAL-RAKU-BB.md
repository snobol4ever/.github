...
**STATUS (after session 8): m3/m4 = 25 PASS / 0 FAIL / 6 EXCISED. See the Watermark (bottom) for the authoritative live state, the landed B-c/B-b work, and the corrected analysis.** Remaining EXCISED, in attack order:

**GROUP C (1 test) — `class_method`:** FIELD_GET + CALL-dval=1.0 ASSIGN + `walk_bb_node` `case IR_FIELD_GET`. **GATED on a pre-existing nested-BINOP operand double-emission bug** (`($!x+$!y)*$factor` emits the `$factor` operand twice → duplicate node-id label → m4 won't assemble, m3 silent-empty). Fix the double-emission FIRST, then the FIELD_GET template + gate. Full detail + repro in the Watermark.
...
**Done (session 8): GROUP B (jct_any/all/one/none/infix/nested + bool_truthiness) — native m3/m4 PASS. GROUP "C-array/hash/say" (list/array/hash/say_jct/say_list) — already PASS. `bool_compare_store` — already PASS.**
...
- **LANDED SESSION 8 (commit `b9a2433`) — B-c + B-b native; `scrip.c` gate + `bb_call.cpp` emit only (no runtime change, reused existing rt fns):**
  ...
  **GROUP B-c — `bool_truthiness` (PASS):** `TT_IF` → `__rk_bool dval=2.0`, cond in sub-graph `blks[0]`. **CORRECTION to session-7 analysis:** `True`/`False` lower to **`LIT_I 1` / `LIT_I 0`**, NOT `LIT_S "True"/"False"` (verified by SCRIP_DUMP_X). The truthy emit only needs `LIT_I` + `IR_VAR` for the smoke (LIT_S kept in the gate but unexercised — and the `LIT_S` route session-7 proposed would have been a BUG: `rt_rk_is_truthy("False")` returns TRUE because "False" is a non-empty non-"0" string). Gate `icn_rk_bool_truthy_emittable` (cond entry LIT_I/LIT_S/VAR-non-&). Emit `bb_call_rk_bool_truthy_cond_str`: LIT_I→(mov32 edi,6; movabs rsi,ival); VAR→(frame_load64 rdi,voff / rsi,voff+8); call `rt_rk_is_truthy`; test eax/je ω/jmp γ. `bb_call_rk_bool_cond_str` routes here when `rkbool_cond_relop`==NULL.

  **GROUP B-b — `jct_any/all/one/none/infix/nested` (PASS; `jct_str` already passed, not a gain):** cond sub-graph relop `BINOP(EQ, VAR(x), CALL(__rk_jct_any))`. **CORRECTION to session-7 analysis:** the in-cond jct CALL has **`dval=2.0`**, NOT `dval=0.0`. Gate: `icn_rk_bool_cond_emittable` returns 1 when one operand is a jct call (`icn_rk_is_jct_call`, sval prefix `__rk_jct_`) and both are `icn_rk_jct_marshallable` (LIT_*/VAR-non-&/CALL dval 2|3|5). Emit `bb_call_rk_bool_jct_cond_str`: marshal BOTH operands as full DESCR_t via `marshal_call_arg(op, cond, slot, NULL, idx)` into **anonymous `bb_slot_claim(16)`** scratch — NOT `bb_slot_alloc16(node)` (that registers the slot FOR the operand node, so `marshal_call_arg`'s "nested producer-box slot" branch fires and self-copies UNINITIALISED memory → wrong answer; this was a real bug found+fixed this session). Load lhs→rdi:rsi, rhs→rdx:rcx, `mov32 r8d, IR_LIT(relnd).ival` (**the BINOP code IS the op arg**), call **existing `rt_jct_relop(DESCR_t,DESCR_t,int)`** (by_name_dispatch.c:1894) — NO new `rt_rk_jct_relop`, NO `binop_to_tt`: rt_jct_relop already switches on `BINOP_*` and does junction-collapse + int/string fallback, so reusing it avoids the NO-DUPLICATED-LOGIC violation session-7's plan would have caused. Branch test eax/je ω/jmp γ.

  **ABI facts confirmed (reuse these):** `DESCR_t` = `{DTYPE_t v; uint32_t slen; union value;}`; first 8 bytes (offset 0) = `{v,slen}`, second 8 (offset 8) = value. One DESCR_t arg → rdi=[+0] rsi=[+8]; second DESCR_t → rdx,rcx; int → r8d. `x86("call",sym,fptr)` (→`x86_call_ro`, `@PLT` in text / movabs+call in binary) and `x86_movimm32` (REX.B 0x41 for r8d) are medium-complete. `g_descr_flat_chain`=1 / `g_gvar_flat_chain`=0 for Raku, so a plain local `IR_VAR` in `marshal_call_arg` falls to the `bb_varslot` frame-load tail (correct for `my $x`).

- **GROUP C — `class_method` — INVESTIGATED, NOT LANDED (stays cleanly EXCISED). SCOPE IS BIGGER THAN SESSION-7 CLAIMED — read this fully before re-attempting:** Session-7 said "ASSIGN RHS is FIELD_GET, add 1 template." That is INCOMPLETE. The program (`Point`/`Dog`, `.new`, methods `.sum/.scale/.greet`, `$p.x`, `$!x+$!y`) needs ALL of:
  1. **Gate:** admit `IR_CALL dval==1.0` as ASSIGN rhs — `my $p = Point.new(...)` lowers to CALL `obj_new` (dval=1.0, ival=5); methods are `meth_call` (dval=1.0). `icn_rhs_kind_ok` only allowed dval 0.0/2.0. (Plus `IR_FIELD_GET` in `icn_rhs_kind_ok` + `icn_assign_safe_kind` — necessary, but I REMOVED these this session because they were inert without the rest and were a latent abort trap.)
  2. **Emit:** Raku uses dispatcher **`walk_bb_node` (emit_core.c:372)**, reached from `walk_bb_flat` via the `FILL`/`EMIT_PAIR_FILL` macros (emit_bb.c:269/278). It has NO `case IR_FIELD_GET` → unhandled (node kind **123** = IR_FIELD_GET). Needs a `bb_field_get` template in emit-core descr-chain style: `_.op_a_slot` = object operand slot (walk_bb_node auto-sets it = `bb_slot_get(operands[0])`), `_.op_sval` = field name (auto-set), `_.op_off` = own slot (`g_emit.op_off = bb_slot_alloc16(nd)` in the case). Body: seal field name→rdi (`x86_ro_load_q("rdi",0)` + `x86_jmp_id(1)`/`x86_ro_seal_str(0,name)`/`x86_deflabel_id(1)`), obj→rsi:rdx, `call dat_field_get`, store rax:rdx→`_.op_off`, jmp γ. `dat_field_get(const char*field, DESCR_t obj)` exists (by_name_dispatch.c:3132). Wire: `case IR_FIELD_GET` in walk_bb_node + decl in bb_templates.h + Makefile source-list + compile-rule. (`obj_new`/`meth_call`/`write`/BINOP/RETURN are ALREADY handled by walk_bb_node — FIELD_GET is the only missing leaf. I built (1)+(2) this session and FIELD_GET emitted correctly with `op_a_slot` resolving fine.)
  3. **BLOCKER — pre-existing, EXPOSED not caused by C:** in method `scale` `return ($!x + $!y) * $factor`, the nested-BINOP operand (the `$factor` `IR_VAR`) is emitted **TWICE** — once as a chain node (`xchain11_n3_α`) and once as the `*`-BINOP lhs (`xbinop23_lhs_done`) — both using the same node-id label `bbNNNN_α` → DUPLICATE LABEL → m4 won't assemble (`Error: symbol bbNNNN_α already defined`), m3 silent-empty. This lives in the `flat_drive_binop_tree` × `codegen_flat_chain_body` chain-vs-tree interaction. **A FIELD_GET template + walk_bb_node case + dval=1.0 gate are NECESSARY BUT NOT SUFFICIENT — the double-emission must be fixed FIRST.** I reverted (1)+(2) when (3) surfaced, to keep the EXCISE honest (never ship a silent FAIL). `sum()`'s flat `$!x+$!y` is likely fine; the nested `scale` is what breaks.

  **GROUP A (5) — `gather_take/map_range/grep_range/map_over_gather/grep_over_gather` — DEFERRED on Icon GZ-7 (IR_ASSIGN ζ-slot store). DO NOT ATTEMPT.**

- **NEXT (ordered):**
  0. **BUG 1 FIRST — whole-proc DOUBLE-EMISSION (found 2026-06-13 OPUS48; diagnosis-only, NOT landed). SEPARATE from + PRIOR to the nested-BINOP "Bug 2" below.** EVERY named Raku `sub` / class method is registered TWICE in `proc_table` → m4 emits its `icn_proc_<name>_α:`/`_β:`/`_γ:`/`_ω:`/`_α_body:` labels TWICE → `as` rejects with `symbol 'icn_proc_<name>_α' already defined`. Blocks ANY sub-containing Raku m3/m4, not just `class_method`. **Repro (Bug-1 only, does NOT hit Bug 2):** `sub f($a){ return ($a+1)*$a; } say f(3);` → `./scrip --compile … | as` → duplicate-symbol errors; emitted text has two `icn_proc_f_α:` bodies + two `"f"` startup entries. **Root cause:** BOTH `polyglot_init` (polyglot.c:111 — the `|| proc->t == TT_SUB_DECL` clause; the line-110 `_is_raku_call` guard excludes only Raku `TT_FNC`, not the Raku-only `TT_SUB_DECL`) AND `rk_discover_procs`/`rk_register_proc` (lower_raku.c:398/433/443) register the same `TT_SUB_DECL`; `lower_raku_stage2`'s loop (lower_raku.c:499) lowers BOTH → two graphs → two emissions. m2 tolerates it (by-name graph dispatch, no label collision) — that is why m2 is 31/31 despite the dup. **Fix (recommended Option A):** extend polyglot.c:110 to `_is_raku_owned = (s_lang==LANG_RAKU && (proc->t==TT_FNC || proc->t==TT_SUB_DECL))` so `rk_discover_procs` is the sole Raku registrar (it alone handles class methods too). Verified safe: `module_registry.mods[].nprocs`/`.proc_start` are write-only (no reader outside polyglot.c); Icon has NO self-registrar so its polyglot registration is untouched. Option B (conservative): make `rk_register_proc` idempotent (skip if `name` already in `proc_table`). FULL detail + line numbers + safety proof in `HANDOFF-2026-06-13-OPUS48-RAKU-BB-PROC-DOUBLE-EMIT-DIAGNOSIS.md`.
  1. **Bug 2 (the nested-BINOP operand double-emission):** chain walker enqueues a BINOP operand that `flat_drive_binop_tree` ALSO emits → duplicate node-id label. Start at `codegen_flat_chain_body` queue-build (emit_bb.c:3057 enqueues `c->ω.node` for IR_BINOP) and `flat_drive_binop_tree` (emit_bb.c:1403). NOTE: the 3-line `sub f($a){ return ($a+1)*$a; }` does NOT exercise Bug 2 (it takes the `descr_flat_chain needs_walk==0` path) — it only triggers Bug 1; force `needs_walk==1` with operands lacking pre-assigned slots, e.g. `sub g($a){ return ($a+1)*($a+2); }`. Then re-apply C parts (1)+(2) and verify `class_method` EXCISED→PASS, m2 31/31, no peer regression, no silent m3/m4 FAIL.
  2. A: DEFERRED (GZ-7).

- **Done (history):** RK-LOWER-0..5h, RK-NFA-ORACLE-FIX, RK-EMIT-1/2/3+GATHER, RK-HY-0..3, RK-NFA-1/2/3, RK-M34-1, while_loop fix, bb_call_fn MEDIUM arm, gate BUG1+BUG2, ONE-MEDIUM rk_bool, lbl_β double-colon, x86_uid dup-label, Group-C gate+emit (earlier partial), **B-c bool_truthiness + B-b jct relops (session 8, commit `b9a2433`)** (git log).
---
---
