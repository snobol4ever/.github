...
- **NEXT (ordered) — GROUP B then GROUP A (the 15 remaining EXCISED: 9 in B, 5 in A, plus map/grep emit):**
  1. **GROUP B (8 EXCISED remaining): `__rk_bool` dval==2.0 sub-graph condition → branch.** Sub-shape **(a) plain-relop assign-store DONE** (commit `5065b30`, `bool_compare_store` PASS m3+m4). Tests still EXCISED: `jct_any/all/one/none/infix/str/nested` (7), `bool_truthiness` (1). IR shape (verified `--dump-bb` + `lower_raku.c` L217/L274): `IR_CALL sval="__rk_bool" ival=1 dval=2.0`; `bk->γ.node`=truthy target, `bk->ω.node`=falsy target; the condition is a **separate nested `IR_graph_t`** stored as `blks[0]` in `IR_EXEC(bk).counter` (an `IR_graph_t**`), built by `rk_arg_block(cx, cond_tree)` — NOT an inline operand. **m2 reference semantics (`IR_interp.c` L2335+):** `bb_reset(blks[0]); v = IR_interp_once(blks[0]);` then `return IS_FAIL_fn(v) ? bk->ω.node : bk->γ.node`. **Remaining sub-shapes (do (c) next, then (b)):** (c) **bare truthiness** `True`/`$x` → `bool_truthiness` (cond sub-graph is a LIT/VAR leaf, NOT a relop; reuse the EXISTING `bb_call_rk_bool.cpp` dval==0.0 slot-truthiness arm via `rt_rk_is_truthy`; NO phi-merge since γ/ω are statement blocks). Broaden the gate `icn_rk_bool_cond_emittable` to also accept a LIT/VAR cond entry, and branch in `bb_call_rk_bool_cond_str` (load value → truthiness test → γ truthy / ω falsy). (b) **junction-collapse** `$x==any(...)` → `jct_*` (cond sub-graph is a relop whose RHS is `__rk_jct_*` IR_CALL dval=2; collapse already works in m2 via `rt_jct_relop`/`junction_collapse` in by_name_dispatch.c). Will need: gate to accept the relop-with-jct-RHS operand shape (currently `icn_rk_arith_operand_ok` rejects IR_CALL on purpose to keep (b) excised), plus a jct emit path. **Sub-shape (a) machinery to reuse:** `bb_call_rk_bool_cond_str` + `rkbool_cond_relop` (bb_call.cpp), `icn_rk_bool_cond_emittable`/`icn_rk_arith_operand_ok` (scrip.c), shared-slot merge (`bb_slot_register` + `bb_slot_alloc16_or_get` in emit_bb.c). GATE sites in `src/driver/scrip.c`: `icn_rhs_kind_ok` ~L208 (unaffected — ASSIGN rhs is the LIT via γ-flow), `icn_graph_native_emittable_mode` ~L274 (the flip point). Flip per-sub-shape as each emit arm lands; keep `__rk_try` excised.
