...
**Carved:** 2026-05-28
**Author:** Lon Jones Cherryholmes · Jeffrey Cooper M.D. · Claude Opus 4.7
**Supersedes for new work:** `GOAL-ICON-BB.md` (frozen at PASS=194 mode-2 as legacy reference).
...
Mode 2 handler: `bb_exec_once(root)`, set last_ok, done. Modes 3/4: emit `lea r10, [rip + Δ_root]; jmp .Lroot_α; ... SM_HALT`. Boxes are CODE+DATA. Transitions are `jmp`. No SM dispatch in the body. No interpreter loop. No broker. No C walker in the mode-4 runtime binary.

Per `test_icon.c` (canonical sketch, 2026-03-10): every construct gets `_start` / `_resume` / `_succeed` / `_fail` labels. Wired by flat `goto`. Three-column form: `LABEL / ACTION / GOTO`.

---

## Banned in Icon SM streams (after GZ-7 flip)
...
These opcodes remain in `sm.h` for other languages until their own ground-zero rewrites.
...
- [x] Write `GOAL-ICON-GROUND-ZERO.md`.
- [x] Update `PLAN.md`: add ICON-GROUND-ZERO row, mark ICON-BB as legacy-frozen.
- [x] Commit + push (.github `3de7b774`).
...
- [x] Mode-3/4 stub in `src/emitter/emit_core.c` (real x86 deferred to GZ-5).
- [x] Build green; legacy smokes still 5/5 / 5/5 / 36.
- [ ] Write `src/lower/lower_icn_gz.c` with `BB_graph_t *lower_icn_ground_zero(CODE_t *prog)`. Walks AST: `STMT_t → TT_PROC_DECL → TT_PROGRAM body → lower_icn_expr_node` into one root graph; chain top-level statements via γ-edges; first statement's α is `bbg->entry`.
- [ ] Add header `src/lower/lower_icn_gz.h` exporting the function.
- [ ] Hook in `lower.c`: when `g_lang == LANG_ICN` AND env `SCRIP_ICN_GZ=1` set, call `lower_icn_ground_zero` instead of the legacy per-statement path, register root in `g_stage2.sm.bb_table[]`, emit ONLY `SM_BB_RUN_THE_DAMN_ICON root_idx` + `SM_HALT`. Otherwise fall through to legacy lower untouched.
- [ ] Gate: `SCRIP_ICN_GZ=1 ./scrip --interp /tmp/hello.icn` prints `hello\n`, exit 0.
- [ ] Gate: `SCRIP_ICN_GZ=1 ./scrip --dump-sm /tmp/hello.icn` shows exactly two opcodes: `SM_BB_RUN_THE_DAMN_ICON`, `SM_HALT`.
- [ ] Gate: legacy smokes still pass (unsetting SCRIP_ICN_GZ).
- [ ] Commit + push: `.github` (goal-file step marks) + `one4all` (code).

---

### GZ-2 — Boot shape decision

- [ ] After GZ-1 runs, decide: keep `SM_BB_RUN_THE_DAMN_ICON + SM_HALT` (two ops) or fold to single `SM_BB_RUN_THE_DAMN_ICON_AND_HALT`.
...
- [ ] `BB_ICN_CALL` for `write(int_expr)` — extend GZ-1's BB_CALL to accept a BB-typed integer arg.
- [ ] `lower_icn_gz.c` recognizes `TT_ADD` and integer literal cases.
- [ ] Gate: `SCRIP_ICN_GZ=1 ./scrip --interp /tmp/add.icn` prints `3`.
...
- [ ] `BB_ICN_EVERY` template. α: jmp body.α. body.γ wired to self.β (re-pump). body.ω wired to self.γ. Eight lines.
- [ ] `lower_icn_gz.c` recognizes `TT_EVERY` and `TT_TO`.
- [ ] Gate: `SCRIP_ICN_GZ=1 ./scrip --interp /tmp/every_to.icn` prints `1\n2\n3\n`.
...
- [ ] Gate: `SCRIP_ICN_GZ=1 ./scrip --compile /tmp/every_to.icn && ./a.out` prints `1\n2\n3\n`, byte-identical to mode 2.
...
- [ ] `lower_icn_gz.c` recognizes `TT_ALTERNATE`.
- [ ] Gate mode 2: `1\n2\n3\n`.
- [ ] Gate mode 4: `1\n2\n3\n`, byte-identical.

---

### GZ-7 — `every write(5 > ((1 to 2) * (3 to 4)))` mode 2 + mode 4
...
**Flip default after GZ-7:** drop `SCRIP_ICN_GZ` env-gate. `lower_icn_gz` becomes the default Icon lower path. Legacy `lower_icn_new_*` enters scheduled deletion. Legacy GOAL-ICON-BB rung gates retired.
...
### GZ-15 — Augmented assign

- [ ] `BB_ICN_AUGOP` template mode 2 + 4. Fixes the documented stack-underflow class from legacy ICON-BB.
...
### GZ-19 — Generator-in-subscript `L[1 to N]`

- [ ] Composition of GZ-17 + GZ-18 wiring; verify cross-product/β-chain behavior.
- [ ] Gate: `every write(["a","b","c"][1 to 3])` → `a\nb\nc\n`.
- [ ] Fixes the documented generator-in-subscript stack-underflow class from legacy ICON-BB.
...
### GZ-34 — Remaining JCON `ir_a_*` constructs
...
- [ ] All Icon constructs lower exclusively via `lower_icn_gz`.
- [ ] Legacy `lower_icn_new_*` functions deleted.
- [ ] Legacy `bb_exec.c` Icon arms shrink to one dispatch table OR are deleted entirely (mode-2 itself running flat x86 via templates).
- [ ] `SM_BB_INVOKE` / `SM_BB_PUMP_PROC` / `SM_BB_ONCE_PROC` removed from Icon path (still present for other langs until their own GZ).
- [ ] `GOAL-ICON-BB.md` deleted from repo (history preserved in git).
- [ ] This file's watermark: "all programs PASS mode 2 + mode 4, byte-identical, FACT 0."
...
# Functional gate
SCRIP_ICN_GZ=1 ./scrip --interp   /tmp/rung_NN.icn  > out_m2.txt
SCRIP_ICN_GZ=1 ./scrip --compile  /tmp/rung_NN.icn ; ./a.out  > out_m4.txt
diff out_m2.txt out_m4.txt    # must be empty

# SM-shape gate (Icon programs)
SCRIP_ICN_GZ=1 ./scrip --dump-sm /tmp/rung_NN.icn   # only SM_BB_RUN_THE_DAMN_ICON + SM_HALT
...
# Legacy smoke gates (must hold throughout ground-zero work, run WITHOUT SCRIP_ICN_GZ)
bash scripts/test_smoke_icon.sh                # PASS=5
bash scripts/test_smoke_prolog.sh              # PASS=5
bash scripts/test_smoke_unified_broker.sh      # PASS>=35
bash scripts/test_icon_all_rungs.sh            # PASS=194 (legacy watermark — DO NOT extend)
```
...
Not "rungs PASS." **Programs PASS, both modes, byte-identical.**

| State | Programs PASS (mode 2 + mode 4 identical) | Notes |
|-------|-------------------------------------------|-------|
| GZ-0 ✅ | 0 | carve complete |
| GZ-1 | 0 → 1 (`hello`) | mode 2 only at this rung |
| GZ-3 | → 2 | mode 2 only |
| GZ-4 | → 3 | mode 2 only |
| GZ-5 | 3 (mode 2 + mode 4) | first dual-mode rung |
| GZ-6 | 4 (dual) | |
| GZ-7 | 5 (dual) — full test_icon.c | **flip default** |
| GZ-22 | ~15 (dual) | user procs working |
| GZ-34 | full Icon | legacy deleted |
...
- Extend `GOAL-ICON-BB.md` watermark. That goal is frozen.
- Use `SM_BB_INVOKE` for Icon programs going through `lower_icn_gz`.
...
## In-progress notes for the next session (state at end of 2026-05-28 session)

- GZ-0 closed, pushed to `.github` `3de7b774`.
- GZ-1 steps 1–5 done in `one4all` (uncommitted at session end):
...
- **Next session picks up at GZ-1 step 6:** write `src/lower/lower_icn_gz.c` with `lower_icn_ground_zero(CODE_t*) → BB_graph_t*`. Then step 7 (header), step 8 (hook in `lower.c` gated by `SCRIP_ICN_GZ=1`), then steps 9–11 (gates).
- Pointers for next session:
  - `BB_alloc(N, BB_LANG_ICN)` creates a graph; set `bbg->entry`.
  - `lower_icn_expr_node(bbg, tree)` lowers an expression node into the graph.
  ...
  - For GZ-1's hello: AST shape is `STMT.subject = TT_PROC_DECL(name, params, body)` where `body = TT_PROGRAM(TT_FNC(write, "hello"))`. The TT_FNC lowers via existing `lower_icn_new_Call` to `BB_CALL` which `bb_exec.c case BB_CALL` already routes to the `write` builtin.
