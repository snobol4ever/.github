...
**Read `GOAL-SNOBOL4-BB-ALPHA.md`'s s23q cursor instead of the paragraph below.** s23a's CARVE-DATA-ERAD had already deleted the whole-graph jmp-entry veto; the gate line is now `!_zd || x86_port_mode() != ZC_PORT_FORTH || n <= 0` and the long comment beside it is history. The measured discriminator was the **function-scoped `zdyn` dynamic-box veto** (one PATREF declining every match statement in the graph), narrowed to run scope s23q behind `SCRIP_ZD_DYNSCOPE`. ⭐ **THE TRANSFERABLE LESSON, which is why this section stays instead of being deleted: `SCRIP_ZD_GAP=1` reports ONLY `zd_wl_kind`'s per-node CAPABILITY verdict — it is BLIND to runs declined by the quartet veto / `nops` / `opnd`, and it prints "every node admits" for all three. Reading a decline off ZD-GAP is what produced this section's wrong diagnosis. Attribute declines with `SCRIP_ZD_DIAG=1` (`[ZD] run ... DECLINED at i=N (why)` + the s23q `[ZD-DYN]` line), always.** ⛔ Also: adding ANY env var to an A/B arm shifts initial rsp and flips the s23i placement class (127/152) — hold env LENGTH equal across arms.
