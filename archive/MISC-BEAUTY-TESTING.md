...
Drivers live in `SCRIP/test/beauty/` alongside `.ref` oracle files.
...
Each driver lives in `SCRIP/test/beauty/<subsystem>/`:
...
Both scripts live in `SCRIP/test/beauty/` on the `asm-backend` branch.
...
# 6. Fire milestone — commit SCRIP, update PLAN.md + TINY.md, push .github
```
