...
## ⛔ THE BOARD DID NOT MOVE, AND THE DEPTH FLOOR IS NECESSARY BUT NOT SUFFICIENT
`board_beauty_m1.sh` at HEAD: **M3 3/10 green, FIRST RED AT LINE 10** — m4 identical (`rc1` from rung 10 on). **Unchanged by this session**; no fix landed for class A, so this rung closes on the brief's *investigation-only, mechanism named* branch, not on a board improvement.
HQ's later `defer-depth-floor` message (row rank 0) proposes that a **two-level `*defer` floor** subsumes this row. The floor itself **verifies at HEAD** — `ptw_min_defer2_floor` nomatch vs oracle match; `L0`/`L1` PASS, `L2`/`L3` FAIL; `defer1_arbno_ctl` PASS vs `defer2_arbno` FAIL. **But two of its supporting claims do not survive measurement here:**
1. The subsumption argument rests on *"`*Stmt nl` SEGVs and `*Comment nl` is clean — because `Stmt = *Label` is TWO defer levels"*. At HEAD **`*Stmt nl` is rc=0, 5/5, in all three arms**, so it is no longer evidence for the depth law.
2. ⭐ **DEPTH 2 IS NECESSARY BUT NOT SUFFICIENT.** In the reduced witness **both** arms are `Parse → Stmt → Label`, i.e. **exactly depth 2**: `Stmt = *Label` is GREEN 5/5 and `Stmt = *Label *Label` is RED 5/5. Same depth, **one extra composed element**, opposite verdicts.
Which is exactly what the RSP reading predicts — *one spine cell per composed element*. The floor and this wild jump are plausibly **one depth-accounting authority seen from two sides** (the floor loses the match, this jumps wild), with seat7's finding a third face. Whoever fixes the floor should re-run `util_beauty_m1_reduce.sh` as the blast-radius check: it is a one-token pair and answers immediately whether class A evaporates.
