...
- [ ] **L-4 · 061 — VARIABLE-ARG PATTERN-PRIMITIVE CLASS.** ⛔ **s45: ROOT CAUSE FOUND AND GDB-CONFIRMED — NOT YET FIXED.** `emit.cpp:2702` (the ONE authority staging `g_zd_read[k]` ⇒ `op_zread[k]` ⇒ `ZOPQ`) computes a naive `zd_out[consumer] - zd_out[producer]`, which is blind to `IR_MATCH_BEGIN`'s RUNTIME carve. Producer-before-match + consumer-inside-match ⇒ read short by MATCH_BEGIN's carve plus every un-popped intervening node's K (measured 48 = 32+16 on this witness). The site's own comment already names this class for sibling `IR_MATCH_LEN` and gives a formula citing `zd_ud`/`zdh` — **fields that do not exist in the codebase; documented, never implemented.** Witness: `corpus/crosscheck/capture/061_capture_in_arbno.sno`. Full derivation + proposed fix in the s45 LIVE CURSOR.
...
## ⛔ CLOSED — DO NOT RE-DERIVE, DO NOT RE-SPEND (carried forward from deleted rungs L-0/L-3/L-3b)
...
---
## ⭐ LIVE CURSOR — 2026-08-12 s43 (Claude Opus 5). **L-3b CLOSED — l3 board 13/13 GREEN.** SCRIP `93fbe1e2` + artifact commits; corpus `5da19d40`. *(The "PUSH BLOCKED" note this cursor carried is STALE — s44 confirmed both commits are on `origin`.)*

**TWO defects, not one; the second was never on anyone's list.** Every cursor since s35 called the carving class (TAB/RTAB/POS/RPOS) a separate unstarted defect. It was one line in LOWER. Board 8/13 → 11/13 (defect #1) → **13/13** (defect #2). Both mechanisms are recorded in **⛔ CLOSED — DO NOT RE-DERIVE** above; not restated here.

| instrument | result |
|---|---|
| l3 board m3 | **13/13 PASS**, re-run at final HEAD via the script |
| broad SNOBOL4 corpus (336) | m3 **264**, m4 **257** (from 261/255); FAIL+SKIP sets diffed byte-for-byte against a stashed-and-rebuilt baseline **in-session**; zero new failures either direction |
| `probe/bb/run_suite.sh` | 159 pass / 1 xfail / 0 XPASS / 5 regression — `D12 D13 H31 X01 X10`, the identical set s39 recorded and proved pre-existing. Unmoved. |

⛔⭐ **PROVENANCE — THE ONE INVARIANT VIOLATED, 5th recorded (s34, s35, s38b, s40, now s43), and this is the one session where both halves are nameable.** The concurrent seat's cursor described *my* session's work — my mechanism, my numbers, my commit `3d223c12` — and stated its author never issued that commit. **I issued it.** Conversely the two diagnostics that made defect #1 findable (`SCRIP_ARB_DIAG`, `SCRIP_ZOP_DIAG`) were already in my working tree, uncommitted, at my session start, and I recorded assuming they came from "a prior session" — they came from the concurrent seat, live. **Both seats independently traced the same root cause; both write-ups are honest; neither is plagiarism; the duplicated spend is real** and is exactly the cost s40 predicted in writing. Both diagnostics are retained in-tree deliberately (env-gated, additive-only) — they earned it.

---
*History below s43 pruned s45 per STALE-ORIENTATION (c) ("newest at top; prune below the last ~3"). Deleted records: s33/s33b, s34–s42 cursors, the resolved TWO-SEATS alarm, and completed rungs L-0/L-3/L-3b. Everything load-bearing from them was lifted into the ⛔ CLOSED, LAND MINES, BOARDS, and STANDING ASKS sections above, and s43's unstarted `fc_walk_range` audit was promoted to rung L-6 rather than dropped. Full history: `git log GOAL-SN4-HOME-LOWER.md`.*
