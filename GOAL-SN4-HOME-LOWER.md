...
- [ ] **L-4 · 061 — VARIABLE-ARG PATTERN-PRIMITIVE CLASS.** The dynamic arm reads a cell the arg never landed in (whole class, not POS-specific). ⛔ s44: RE-MEASURED, STILL BROKEN. s43's "same as defect #2 / ASSIGN_SAVE gap" hypothesis is FALSIFIED (SCRIP_CAP_DIAG shows that arm sound for this witness). Localized via monitor to `POS(N)`'s own dynamic-arg read (`bb_match_pos.cpp` → `FRQ(op_sa+8)`, regime-3/4, not the already-fixed regime-2 path). Root mechanism NOT YET CONFIRMED — s44's own in-transcript offset arithmetic was retracted as premature. Next: gdb on `x86_frame_off()`/the `FRQ(op_sa+8)` call site, hit-count isolated, not hand-derived from `.s` text. See s44 LIVE CURSOR for full trace. Separately: `l4_span_varg_hang.sno` (SPAN(V) variant) now passes m3 clean but SEGFAULTS m4 — new, unstarted, possibly-unrelated finding, own witness.
...
## ⭐ LIVE CURSOR — 2026-08-12 s44 (Claude Sonnet 5). **L-3b RE-CONFIRMED FRESH (14/14 incl. control). L-4/061 MEASURED, NOT FIXED. s43's defect-#2 hypothesis for L-4 FALSIFIED. NEW mode-3/mode-4 divergence found, uninvestigated. NO CODE CHANGED THIS SESSION.**
