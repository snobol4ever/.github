...
**O-PB-2b FALSIFIED — umin CLAMP (−7m3 −10m4, 11 regressions in arbno/fence/defer):** The hypothesis that stmt2's `umin` is pulled low by foreign stmt1 closure members is WRONG. The blob-closure members below `run_umin` are genuine accesses — PATREF's indirection reaches stmt1's IR_MATCH_ASSIGN_SAVE/LIT_STRING/ASSIGN_IMM nodes at their actual zls offsets, and those ARE the right coordinates. The 224B residual is not contamination; it reflects the actual span of the cross-statement pattern closure. **O-PB-2b is ALPHA terrain.** The fix belongs in `zvo_resolve` / `zd_plan`'s owner-table base scoping. Coordinate with ALPHA before next attempt.

**O-PB-3 FALSIFIED — naive op_zres arm suppresses slow arm wrongly:** Built: `IF(_.op_zres, x86_omega("jz"))` fast-arm null-guard + `IF(!_.op_zres, slow-arm)` + `IF(_.op_zres, x86_beta_trampoline())` β stub. Immediate failure: 053_pat_alt_commit (`P = ('a'|'b'|'c')`) fails because the slow arm's `rt_defer_open` pump loop IS the backtrack mechanism for patterns with internal alternatives. `pat_static=1` means the pattern NAME is static — NOT that the pattern value has no alternations. Suppressing the slow arm kills alternation retry. **O-PB-3 requires GLUE #2 first:** the slow arm must be rewritten as an emitted `push rbp; mov rbp,rsp` frame wrapping the pump loop, not suppressed. GLUE #2 = mechanism-2 (O-PB-4) applied to `bb_match_defer.cpp`'s slow path. Sequence: O-PB-4 design → apply to slow arm → then O-PB-3 gate flip.

**WHAT DOES NOT NEED BUILDING (confirmed this session):** 066 passes under DYNARM=7 (O-PB-1 confirmed). DYNARM=7 BY SET is zero P→F at HEAD. The `zdyn` veto is doubly irrelevant (PATREF gate OFF, veto evidence expired).

**NEXT (ordered, dependency-real):** (1) O-PB-4 ARBNO/FENCE1 own frames — design the nested RBP frame (`push rbp; mov rbp,rsp` at iteration/commit entry, `mov rsp,rbp; pop rbp` at success exit); (2) apply same frame to `bb_match_defer.cpp` slow arm — this IS O-PB-3 GLUE #2; (3) once slow arm is emitted-frame-clean: add `op_zres` arm redirecting the now-emitted pump loop on ZD-armed path, then flip `SCRIP_ZD_PATREF` default ON; (4) O-PB-2b when ALPHA delivers closure scoping fix; (5) O-PB-5 reconciliation.
