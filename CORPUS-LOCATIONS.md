...
| **BB reference embodiments** | `corpus/probe/bb/` | `test_sno_*.c` · `test_icon*.c` · `test_sno_cell_*.s` · etc. (the 11 C reference embodiments + `bb_witness_ladder.sh`, self-contained) | one home, confirmed present |
| **BB SNOBOL4 probe suite** ⬅ moved 2026-08-28, probe-consolidate-bb | `corpus/tests/snobol4/probe/` | `bb.{sno,ref}` (14 top-level witnesses) · `bb_probes.{sno,ref}` (166-probe XFAIL/XPASS suite, formerly `probe/bb/probes/`) · `bb_x01_witness.{sno,ref}` · `bb_wreg_witness.{sno,ref}` — all four suite pairs, no more loose `.sno` under `probe/bb/` | one home, confirmed present |
...
| **csnobol4_suite** (Phil Budne's CSNOBOL4 test suite) | `corpus/snobol4/csnobol4_suite/` | ⛔ **CORRECTED same-session (hq_C, 2026-08-24): there is no duplicate.** An earlier draft of this row flagged a second copy at `corpus/programs/csnobol4-suite/` (hyphen) as unresolved; that path is `.gitignore`d local cruft left over from the s269 flatten (`git ls-files` finds 0 files there, confirmed on two independent checkouts) — not repo content. `corpus/snobol4/csnobol4_suite/` (underscore) is the one real tree. Its native oracle is **csnobol4, NOT SPITBOL** — see RULES.md's FACT RULE on grading it against `sbl -bf` (30/120 false reds). | 251 files |
