...
## Addendum, same day — the class-B exemplar, and the build flag the whole conservative design rests on

**Class B was landed as an exemplar rather than a sweep**, 19 sites in one function (SCRIP `a5c11b7b3`), with three more converted and then HELD BACK, to make the class concrete for the ceo's cut: `by_name_dispatch.c:rt_str_method` (19 sites) and `arr_set_pure` (3). Every conversion is `rt_pinned_alloc(N)` → `rt_str_alloc(N - 1)`, byte-for-byte the same allocation, since `rt_str_alloc` adds the terminator itself; each size expression was checked one by one against its original rather than pattern-matched, after a first attempt whose regex over-ran the function bounds and mangled an unrelated line — the compiler caught it, but a `git checkout` and an explicit line range was the fix, not a cleverer regex.

Evidence, both directions, because the by-name dispatcher is a SHARED node: a Raku string-method witness reads byte-identical to `/usr/bin/raku` at `SCRIP_GC_STRESS=1` — **a collection at every single allocation** — and a Pascal global-array witness of 4000 stores into a 128-entry table of ten-character mnemonics reads byte-identical to `fpc -Miso` at stress 1 and 50 — that Pascal arm is a NON-REGRESSION arm and not coverage of the converted path, which it provably never enters. ⛔ **The three held-back sites are `arr_set_pure`, and they are held because nothing I can run reaches them**: a gdb breakpoint on that fallback never fires under a 4000-store Pascal global-array witness, and the one known reproducer does not execute — `scripts/test_pascal_p4_selfhost.sh` line 29 is a bash syntax error on origin, a case pattern list opening with a bare `|`. Converting what cannot be exercised is not a landing. `arr_set_pure` is the site the coo measured from the other end: their 179 blocks of ~3 MB each, and their stress run dying *on a pinned allocation*, are that fallback rebuilding the whole backing string per element store into an unreclaimable block. The garbage is now reclaimable. **The quadratic is untouched and is still there** — the fallback still rebuilds the entire string on every store — and it waits on the coo's frontend half, because a path they are about to make unreachable is not a path worth optimising.

⛔ **The fact worth keeping: the conservative scan's correctness is load-bearing on `-O0`.** Class C — scratch held only in a C local across an allocation that can collect — is fixed up by the stack scan only when the local has a stack home. At `-O0` every local does, which is why class C has no failing witness today. `RT_OPT` is `-O0` by Lon's s262 FACT RULE and `test_gate_no_o2_arm_in_scripts.sh` polices it across 620 scripts, so the guarantee is real and enforced rather than incidental. But it means **a hypothetical future runtime built with register allocation would corrupt class C silently**, and no gate we own would see it: the failure depends on what the compiler decided about register pressure, not on the program. That is an argument for `A_TRANS` on its own merits, independent of the rung, and it is the reason class C should not be converted the way class B just was.
