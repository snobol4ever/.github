...
## s89 ADDENDUM — RULINGS + SLICE 3 (SCRIP `740292a4`)
**Lon rulings (verbatim substance):** co-expressions have their own stack; generator procedures are just like every other BB. ⇒ escapee class = COEXPR ONLY; gen procs LIFO on the calling spine (the ZS-2/s85 suspension-record shape, per the s59 EVAL=DEFER generalization); the s16 dual-storage clause and s69's "generator procedures get their own mmap stack" clause are AMENDED accordingly. The tree had already voted: `ZC_ZETA` defaults ZLS2, its own comment recording det-accumulation as ZH's failure mode (fib/tak/queens never reclaim under --zeta=zh).
**Slice 3 landed:** coexpr pthread stacks are carved FROM the collected span — `rt_gcheap_alloc(HB_ZBLK, stksize+3pg)` per create, page-aligned interior, low guard page PROT_NONE (setstack stacks get no libc guard), `pthread_attr_setstack`, window+guard recorded in `scrip_coctx_t`; `scrip_coexpr_destroy` restores the guard RW and retitles HB_FILL ⇒ gc SLIDE reclaims dead coexpr stacks. Behind `ZC_COEXPR_STACK_GCHEAP=1` (anonymous-stack arm at 0). Thread-safety: the coswitch semaphore handshake serializes to one runner, so the lockless gc allocator is never raced. Gates: sno smokes 7/7×2 · icon 12/2+10/4 unmoved · crosscheck EXACT m3 303/4 · m4 293/13/1 · DIVERGE=10 identical sets.
**Rungs opened, not executed:** ZH-RETIRE (ZH + rt_proc_call_gen_h/rt_proc_resume_frame_h slab path superseded by the rulings — deletion sweep needs its own session, NCB-3b intersects); refs/icon-master canonical confirmation of core-Icon LIFO resume order still owed before gen-proc emit work. GC-U-7 ROOTS is NEXT (un-pin HB_WS/HB_ZBLK; ctx/pkg conservative roots per §6b finding ii). Ruling (C) rc=134 remains open.
