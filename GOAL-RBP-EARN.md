...
## ⛔⭐⭐⭐ LIVE CURSOR — 2026-08-12 s43d (Claude Sonnet 5) — **SECOND FIX ATTEMPT DELIBERATELY NOT MADE — TWO MORE AVENUES CHECKED AND RULED OUT, SESSION STOPS HERE WITH TREE CLEAN RATHER THAN RISK A THIRD GUESS LATE IN CONTEXT BUDGET.**

**Two candidate fix shapes examined and ruled out before writing any code, so the next seat does not re-walk them:**

**(1) A dedicated frame slot (`bb_slot_claim`/`FRQ`-style) instead of a spine push/pop.** `bb_slot_claim` — named as "precedent" in this very template's own comment (line 32) — has ZERO other call sites anywhere in `src/` (`grep -rn bb_slot_claim src/` finds only that one comment). Whatever mechanism the comment is pointing at either never landed in this form or lives under a different name this seat did not find in the time available. `FRQ`/`FR`'s underlying offset function (`x86_frame_off` in `x86_asm.h`) carries a multi-paragraph history of subtle depth-compensation bugs (PIN-REBASE, ZTOS-1/2, UCLAIM, FLATDISP-8 — each fixing a prior mis-add of the SAME kind of offset). Using it correctly for a brand-new slot in this node without deep additional reading risks reproducing exactly that bug class. Deferred, not attempted.

**(2) Reading the true wires from an existing durable record instead of re-deriving a private copy.** Checked whether `rt_proc_call_open`'s pcall record (the mechanism `.Lx4_0`/`.Lx55_0`'s COLD path already uses, visible in the `.s` as `rt_proc_open_fn`/`rt_proc_call_epilogue_γ/ω`) could supply a stable home for the caller's wires that survives a callee's own resume-record push. **It cannot help on the path that actually hangs**: the bug lives on the FAST path (`n2_match_defer_α`'s GVA-cache hit, `.Lx4_10`'s `jmp rax` straight into the resolved blob) — which is fast *specifically because* it bypasses the pcall record entirely ("Under SCRIP_WREG=1 the wires ride r10/r11 and the blob needs ZERO receiving code," `bb_match_defer.cpp`'s own comment). Adding a record on this path would undo the exact optimization the fast arm exists for.

**(3) A different register pair for this node's private continuation, leaving r10/r11 untouched for the callee.** Ruled out: r10/r11 are a PRODUCT-WIDE hardcoded contract, not a per-site convention — `proc_PAT$0_γ`/`_ω` (the callee) read r10/r11 UNCONDITIONALLY by construction (confirmed in the compiled `.s`, no register parameterization anywhere in the template or its callee). Changing the wire registers for one call site would break the callee's contract and requires a product-wide convention change, far outside a targeted bugfix's blast radius.

**⇒ The correct fix needs infrastructure this session did not have time to build safely: either (a) design and land the missing frame-slot mechanism `bb_slot_claim` was a stub/precedent for, verified against `x86_frame_off`'s known bug history, or (b) something this seat did not think of. Recorded as ruled-out rather than silently re-discoverable, per this file's own STANDING INSTRUMENT / re-derivation-avoidance law.**

**FINAL STATE THIS SESSION: SCRIP tree clean (`git status` empty, matches origin), `earn0_stored_varref.sno` and the minimal repro both hang exactly as at s42's handoff (rc=124) — no regression, no partial fix, nothing broken. `.github` has three cursor entries (s43a root-cause / s43b falsified-fix-with-mechanism / s43c classification-question-closed / s43d this entry) recording the full investigation so a fresh seat can start at "design the frame-slot fix" rather than at "find out why it hangs."**

**PUSH BLOCKED ON CREDENTIAL — asked Lon in-chat per RULES.md 6b, not yet supplied. All `.github` commits this session are LOCAL, rebased clean onto origin, ready to push the moment a credential is available. Per RULES.md's FACT RULE, this session's state is NOT "handoff complete" until that push succeeds — stated plainly here rather than left implicit.**

## ⛔⭐⭐⭐ LIVE CURSOR — 2026-08-12 s43c (Claude Sonnet 5) — **CLASSIFICATION QUESTION CLOSED: `Q`'s BARE (non-`*`) REFERENCE TO `P` CORRECTLY PRODUCES `IR_MATCH_DEFER` BY DESIGN (PB-1s SNAPSHOT, Lon's 5-stage ruling 2026-08-07) — NOT A LOWERER MISCLASSIFICATION. THE BUG IS SOLELY THE EMITTER GLUE'S WIRE LIFETIME, CONFIRMED s43a/s43b. NO CODE CHANGED THIS PASS.**

**Why this needed checking:** before designing a second fix attempt, verified whether the REAL defect might be one level up — that `Q = POS(0) P LEN(2) RPOS(0)` (bare `P`, no `*`) should never have compiled to `IR_MATCH_DEFER` at all, since manual p.122 says pattern construction is BY VALUE and only `*` defers (this file's own EARN-0 OPAQUE-hazard classification depends on exactly that cut). If bare-`P` were misrouted into the star-only DEFER path, the fix would be in the LOWERER (stop emitting DEFER for the bare case), not the emitter glue.

**Answer, read directly from `lower_snobol4.c:1330-1341` and its own comments:** this is NOT a misclassification. `*X` (TT_DEFER) is documented as "the ONLY arm the manual lets recurse (p.122) — IR_MATCH_DEFER is star-ONLY BY CONSTRUCTION" for THAT specific producer — but there is a SECOND, deliberate producer at line 1341: the **PB-1s SNAPSHOT** ruling routes a PLAIN (non-`*`) pattern-variable reference through `IR_MATCH_DEFER` too, on a hidden `PATV$`-prefixed frozen-snapshot global (`VAR(live name) -> ASSIGN(hidden global)` in a pre-chain, `IR_MATCH_DEFER` fetches the frozen value "at scan time by name"). This matches the compiled `.s` exactly: `n2_match_defer_α` reads `PAT$1$V0`, a `PATV$`-style hidden global, through the write-once GVA-cache fast path (`bb_match_defer.cpp` lines 48-72, the `ci >= 0` arm) — i.e. the SAME template, a DIFFERENT entry ramp, but the SAME `bb_glue_pass_wires_blob(4,5)` wire-clobber exit that s43a/s43b already root-caused. **So the classification is correct by design; the bug is exactly and only what s43a found.** This closes off the most plausible alternate theory cleanly rather than leaving it for a future seat to re-derive.

**Also worth flagging for whoever designs the real fix:** since bare-pattern-var reference and `*`-deferred reference share ONE template and ONE wire-clobber bug, a correct fix should clear BOTH `earn0_stored_varref` (bare, this session's repro) AND presumably any pure `*P`-style hang sharing the same shape — worth checking `earn0_varref_blob_hang` (s39 probe table, "same class") against whichever entry ramp it exercises once a real fix lands, rather than assuming.

**STATUS UNCHANGED from s43b: SCRIP tree clean, zero net src bytes, hang reproduces exactly as before (rc=124).** The open work is still: design a fix that survives the callee's own resume-record push (s43b's falsification), most plausibly via a durable per-node frame slot (EARN-3-shaped) rather than a bare spine push/pop — NOT YET ATTEMPTED A SECOND TIME this session; ran out of confidence to land it safely within this session's remaining budget rather than risk a second wrong guess.

## ⛔⭐⭐⭐ LIVE CURSOR — 2026-08-12 s43b (Claude Sonnet 5) — **NAIVE push/pop WIRE-SAVE FIX ATTEMPTED, BUILT, TESTED, AND REVERTED: IT TRADES THE HANG FOR A SEGV BECAUSE `proc_PAT$0_γ` (THE DEFERRED TARGET'S OWN SUSPEND PROTOCOL) PUSHES ITS OWN {res,r10,r11,pad} RECORD ON TOP OF MINE BEFORE JUMPING TO MY L(4)/L(5) CONTINUATION. THE ROOT CAUSE BELOW (s43a) STANDS; THE FIX SHAPE IS HARDER THAN A BRACKETING PUSH/POP. TREE IS CLEAN — src/templates/bb_match_defer.cpp REVERTED, ZERO NET SRC BYTES.**

**What happened:** implemented the fix the s43a cursor (below) proposed — `push r11; push r10` immediately before `bb_glue_pass_wires_blob(4,5)` in `bb_match_defer.cpp`, `pop r10; pop r11` right after each of `def L(4)` and `def L(5)` (two independent, mutually-exclusive entry points confirmed by grep — neither is reached any other way, and `x86_jmp` never falls through, so the two pop-sites don't double-pop a single push). Read `ARCH-ICON.md` + `GOAL-TEMPLATE-REVAMP-RULES-DRAFT.md` first (mandatory, per the goal file's own instruction for any template touch) — confirmed r10/r11 aren't part of ARCH-ICON's named register contract (that names R12–R15/RBX; r10/r11 are the SNOBOL4-specific WREG wire convention documented only in this goal file's LADDER WREG section) and confirmed `push`/`pop` are sanctioned R9/vocabulary primitives already used elsewhere (`x86_glue_framed_enter` et al.) — this was NOT a violation of the template rules as constructed.

**Build clean, both repro paths tested: the infinite hang is GONE (rc 124→139) but replaced by a SIGSEGV.** gdb on the crash: `rip` lands at an address matching a live STACK location (`0x7fffb1608660`, in `[rsp,rbp]` territory, not the `0x7f...` JIT-code region) — i.e. `jmp r11` jumped through a value that was a stack address, not a code pointer. Root cause of the SEGV, confirmed by re-reading `proc_PAT$0`'s own template output (`proc_PAT$0_γ`, lines 27-33 of the compiled `.s`): **the deferred target's own SUCCESS port does `sub rsp,8; push r11; push r10; push <proc_PAT$0_res return addr>; jmp r10`** — i.e. `P`'s blob (any blob entered this way, per its own resume/backtrack protocol, LADDER RBP-EARN's own "heap-frame activations… the frame OUTLIVES THE SPINE" crossing) pushes its OWN {res, r10-copy, r11-copy, pad} 32-byte resume record on top of the stack **before** jumping to my `L(4)` continuation. So by the time `L(4)`'s `pop r10; pop r11` runs, it pops `proc_PAT$0_γ`'s pushed copies (harmless in value — they happen to be correct — but WRONG IN PRINCIPLE) while MY originally-pushed values are now buried permanently 32 bytes deeper on the stack, never popped, silently leaking two words per invocation and leaving the stack 16 bytes net-taller than the caller expects on every path through this node. That leak is what produced the SEGV a few frames later (not immediately at `L(4)` itself, since the popped values happened to still be usable that one hop) — **this is measured, not guessed: the leaked-16-bytes theory is consistent with the observed rip/rdx values landing on the real stack near rsp/rbp, but was NOT independently reconfirmed with a second gdb pass isolating the exact leak site before this seat ran low on budget and reverted rather than risk landing something broken.**

**⛔ REVERTED. `git diff --stat` on SCRIP is clean.** `earn0_stored_varref.sno` and the minimal repro are back to the ORIGINAL hang (rc=124), matching s42/s43a's baseline exactly — no regression, no partial fix left in the tree.

**What this means for the real fix:** a bare push-before/pop-after bracketing the blob transfer is NOT sufficient, because the transfer target is not a simple call — `bb_glue_pass_wires_blob`'s contract explicitly allows (and this specific case exercises) the callee pushing its OWN suspend record before jumping to the given continuation label, and that record's presence on the stack is exactly what lets the callee be RE-ENTERED later (backtracked into) via `proc_PAT$0_β`'s `jmp proc_PAT$0_ω` / the outer `n2_match_defer_β: jmp qword ptr [rsp]` mechanism already visible in the `.s`. **Any wire-save fix must account for that record's presence — either by saving r10/r11 in a location the callee's own push sequence can't shadow (a fixed FR/FRQ frame slot rather than the bare spine, if this node is judged to EARN a frame per the goal's own law — see EARN Rule 1, which literally names "the operator who has a `*P` DEFER as an operand" as a frame owner), or by having L(4)/L(5) read the ORIGINAL wires from wherever `n2_match_defer_α`'s CALLER (not this node) already keeps them durably (if such a location exists product-wide) instead of re-deriving a private copy at all.** ⭐ **This is very plausibly the EARN-3/EARN-10 territory this goal already names**, not a standalone glue bug: EARN Rule 1's OWNERSHIP clause plus THE ANCHOR rule (fixed per-frame slot, frame-enter copies parent's, one indirection, no register claim) is close to exactly the shape needed here — a fixed slot that survives across the callee's own resume-record pushes, rather than a spine push that the callee's OWN pushes can bury.

**NEXT SEAT:** (a) do NOT re-attempt the bare push/pop shape — it is falsified, reasoning above; (b) read `bb_match_capture.cpp`'s save/restore convention if any (EARN's ruling (a) on capture pendings referenced spine-vs-heap tradeoffs for a structurally similar problem — same shape, worth checking for a precedent) before designing a second attempt; (c) consider whether this node (`IR_MATCH_DEFER` referencing a stored pattern var, non-`*`-deferred) should classify as EARN-owning under the goal's own predicate rather than staying frameless — if EARN-1's `frame_need_of()` existed and covered this case, the fix might already be "give this node a frame" rather than "patch the glue"; (d) `earn0_varref_blob_hang` (s39 probe-ownership table, "same class, shares MATCH_DEFER transfer") was NOT re-tested this session — do not assume the eventual fix here clears it without checking.

**WATERMARK: NOT RE-MEASURED, zero NET src bytes (attempted-then-reverted).** s39/s42's stands per staleness law.

## ⛔⭐⭐⭐ LIVE CURSOR — 2026-08-12 s43a (Claude Sonnet 5) — **EARN-0b's OPEN QUESTION ANSWERED: THE `earn0_stored_varref` HANG IS NEITHER IN `rt_defer_step` NOR A GENERATOR-TERMINATION ISSUE. IT IS A CLOBBERED `r11` WIRE IN `bb_match_defer`'s BLOB-ENTRY GLUE. ROOT-CAUSED LIVE UNDER gdb. THE NAIVE FIX IS FALSIFIED — SEE s43b ABOVE, READ THAT FIRST.**
