...
- ⚠️ **s274's armed-m4 intermittency REPRODUCES and stands unretracted.** Armed `suspend_single` read m4 `crash 1/10` with `m3⛔≠m4` on the post-rebase tree, having read `0/10` on the pre-rebase tree in this same session. Across sessions the rate now reads **2/10 (s274) · 0/10 (s275) · 0/10 then 1/10 (s276)**. ⛔ It **predates this change** — s274 measured it before the change existed — so it is not attributable here; and a 10-rep sample cannot characterize a ~10% rate in either direction. ⛔⭐ **A 50-rep characterization was attempted and produced NOTHING — and the way it failed is worth more than the number would have been.** It was launched as `REPS=50 … | grep -E '…'` under `timeout 900s`; it was still running at 900 s and was killed (rc=143), and because the output was **piped through `grep`, which block-buffers when its stdout is not a terminal, the kill discarded every line** — not a partial result, not the reps already done, nothing. ⭐ **Two separate defects, both already named in this project's own rules and both walked into anyway:** (a) `timeout 900s` was a bound guessed from a 10-rep run rather than measured — the very "a bound within ~2x of the real duration is not tight, it is flaky" class `CLAUDE.md` warns about, here applied to a job whose duration was unknown by construction; (b) **a long measurement must never be piped into a filter**, because buffering converts "killed late" into "no data at all". ✅ **Corrected recipe for whoever characterizes this: write raw output straight to a file (`> out.txt 2>&1`, no pipe, or `stdbuf -oL`), give it `timeout 3600s`, and filter the FILE afterwards.** Until then the rate stands as the three sessions measured it and **must not be quoted as characterized**.
