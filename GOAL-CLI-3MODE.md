...
- [x] **CLI-3M-9** — `interp_exec.c` deleted (`188475d7`). `interp_eval()` AST walker deleted from `interp_eval.c` (`9aeede7d`). All 25+ call sites in `interp_call.c`, `interp_ref.c`, `interp_eval.c` replaced with `FAILDESCR`. Mode 1 is genuinely removed from the binary. `interp_eval.c` retained (live builtin dispatcher, kw runtime). `interp_ast_stubs.c` provides `g_exec_prog=NULL`, `execute_program_steps` and `interp_eval` stubs. 2026-05-18.
