...
- [ ] **CLI-3M-9** — Delete `interp_eval.c` / `interp_exec.c` / `interp_call.c` / `interp_ref.c` / `interp_label.c` / `interp_data.c` / `interp_hooks.c` / `interp_globals.c` / `interp_private.h` / `interp.h`. Possibly `stmt_ast.c`. Big rip — link failures surface remaining hidden dependencies. **Scout tier: see GOAL-ICON-BB-JCON DAI-7.**
