...
- [ ] **M4SN-4c** — Target: mode-4 broad corpus PASS ≥ sm-run PASS. No regression vs sm-run. Next candidates: check `emit_bb_xatp` (XATP, `@var` cursor capture) for the same binary-mode emit-gap as XSTAR — could yield +2 (`074_pat_star_var_cursor`, `W07_capt_cur`). Then DATA-type accessor cluster (+6 potential): `ShiftReduce_driver`, `Gen_driver`, `TDump_driver`, `ReadWrite_driver`, `counter_driver`, `stack_driver` all pass ir-run, fail sm-run on user-defined DATA records with field accessors.
...
1. **DATA-type accessor family** (`ShiftReduce_driver`, `Gen_driver`, `TDump_driver`, `ReadWrite_driver`, `counter_driver`, `stack_driver`): all use user-defined `DATA('Node(t,v,c,n)')` with field accessors. Pass in `--ir-run`, fail in `--sm-run`. Probably one bug — DATA-record runtime lookup not wired for SM path. Could unblock 6 tests.

2. **`*Fn(args)` deferred-call captures** (`expr_eval`, `140_pat_eval_double_fn_trick`, `141_pat_eval_double_fn_arbno`): per prior watermark, `pat_assign_callcap` / `XCALLCAP` BBs. 140/141 already fail in `--ir-run` too — deeper than mode-4. SL-13c in `eval_code.c` was a partial fix; XCALLCAP native emission may need similar treatment.
...
5. **Recursive-pattern segfault** (`139_pat_calc_paren_deep`, `075_pat_arbno_star_backtrack`): segfault in `--ir-run` too. Deep pattern-engine.
...
7. **Grammar drivers** (`word1/2/3`, `cross`): fail in `--ir-run` too — frontend/lower issue.
