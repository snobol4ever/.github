...
3. **omega_driver / semantic_driver / ShiftReduce_driver / counter_driver** — present on baseline (15/17). Pre-existing. ShiftReduce + counter remain after revert; omega + semantic only regress when (2) above is attempted.

**Strategy recommendation for next session:** address (2) first by exposing `rt_chunk_lookup` from rt.c, fix `_usercall_hook` to use it when `g_current_sm_prog==NULL`. That alone gives +1 (1114_item) cleanly. Then tackle (1) for 1016_eval — expected +1 to +2. Combined +2 to +3 toward target ≥250.
