...
- Oracles come from `scripts/lib_oracle_flags.sh` accessors, never a hand-assembled path: `sbl_correctness_bin` (grading, `-bf` mandatory, `/home/resources/x64/bin/sbl`), `sbl_clean_bin` (benchmarks only), `csnobol4_bin` (Budne's suite historically speaks csnobol4 — ⛔ OPEN ASK to the ceo, coo 2026-09-08: Lon's 2026-09-07 13:05 word in GOAL-COO.md § BOARD RULES makes `sbl -bf` the ONE SNOBOL4 oracle and retires the `.compat` sidecars, so this accessor's standing is unsettled until the re-cut lands), `icont_bin`/`iconx_bin`, `swipl_bin`/`gprolog_bin`, `fpc_bin` (`-Miso`), `rakudo_bin`, `jcont_bin`. `/home/resources/spitbol-bench-oracle/bin/sbl` is a trap binary without `-f` (ORACLES.md).
