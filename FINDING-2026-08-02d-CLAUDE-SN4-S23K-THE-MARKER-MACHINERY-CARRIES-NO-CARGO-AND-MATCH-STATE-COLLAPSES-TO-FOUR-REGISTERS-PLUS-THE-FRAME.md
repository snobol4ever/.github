...
1. ⭐⭐⭐ **THE PATSTK CARRIES NO CARGO — ⛔ CORRECTED same-session: THE CAS SLAB DOES.** g_patstk: all **1,112** emitted references corpus-wide are mark/restore (`lea rcx,[rip+g_patstk_sp]`); zero pushers anywhere, ever — patstk deletion stands, fully measured. ⛔ **The `0x70000000` slab claim was an emitted-grep blind spot** (the writers live C-side under ANOTHER NAME — `#define g_dcap_top (*(const char **)RT_CAS_TOP)`, pattern_match.c:645): the slab is the **dcap stack** — real deferred-capture records `rt_dcap_e{varname, saved_delta, len}` (24B, :642) **pushed by bb_match_capture, applied and scan-popped by bb_match_end's pump**, plus the tag-0 match markers (tag-0 works BECAUSE varname ptrs are never 0). ⭐ CONSEQUENCE, the good kind: **Lon's push-at-γ/pop-backward law is already half-built here** — ZW-3 shrinks from "capture rewrite" to the R12-FREE-1 REVERSAL (s5's own site list). ⭐ LESSON (s200-class, generalized): census a STRUCTURE by every alias of its address, not by one spelling — `RT_CAS_TOP`, `g_dcap_top`, and `1879048192` are one object wearing three names.
...
**DELETIONS (all measured safe or made safe by the above):** `g_patstk_sp` + island + lazy-init + both mark slots + the scan (cargo-free, §6.1) · `g_cap_gen`/`g_cap_gen_next` where the r12+frame discipline provably covers their job (⛔ per-slot `rt_cap_stk_t` stacklets serve a DIFFERENT axis — ARBNO slot-index reuse, pattern_match.c:1313 — AUDIT before deleting, do not bundle) · `g_anchor` DEMOTED to keywords.c-private static (the &ANCHOR feature keeps its storage; the match architecture stops referencing it — snapshot at BEGIN) · the `0x70000000` marker records (slab becomes the r12 CAS arena; `RT_PIN_BASE` seed = r12 init) · `old_rbp` slot ceremony · the dual marks.
...
- **ZW-3** R12 CAS live = **the R12-FREE-1 REVERSAL** (s5 site list: 6 emitted sites + 2 m4 wrapper seeds + m3 `rt_outer_call` thunk) + `rtx_match.S` reads r12 direct. Coherence for free: r12 is callee-saved, so nested emitted matches through C transit see the live top with ZERO cell sync; the cell survives only as the lazy-init bootstrap seed. γ-push already exists (bb_match_capture); fail-discard becomes `r12←cas_base` once ZW-1/2's frame lands. cap_gen deletion rides the frame; stacklets AUDIT-first (§6.1 correction).
