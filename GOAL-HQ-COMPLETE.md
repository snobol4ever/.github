...
|---|---|
| **SNOBOL4 #1** | ✅ **M1 fixed point HOLDS in both media** (C-0 closed above). ⭐ **CORPUS BASELINE, RE-MEASURED BY hq_C s270 (2026-08-24), pristine `-O0`, SCRIP `ad56bb88`: `m3 PASS=363 FAIL=1` · `m4 PASS=363 FAIL=1` · `SKIP=0` (364 total)** — and it held UNMOVED across strip waves 1, 2, 3a, 3b, 4a. This independently equals hq_P's pinned board, so two seats measuring separately agree. ⛔ **EVERY EARLIER FIGURE ON THIS ROW WAS STALE AND IS NOW DELETED** — `357/359`, `339/341`, `320/321`: the denominator is **364**, not 359/341/321. **The single red is `demo_treebank`** — ⛔ **NOT "deliberate"**, that word inverted the fact (Lon's correction, s269): it is the OPEN `(A , B)` selection-expression defect, row `vlist-expr-alternation`, **rank 0 and assigned to hq_C** per Lon (*"We want SNOBOL4 to be 364 out of 364"*). 363/364 is one open bug away from clean, never clean-with-an-intentional-exception. The old `132_pat_fence_eps_recur_shallow` and `demo_porter` SKIPs are **CURED and gone**. Receipt: `bash scripts/test_corpus_snobol4.sh`. |
