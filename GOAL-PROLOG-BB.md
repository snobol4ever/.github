...
**<= LIVE CURSOR (2026-07-24, s141): PERF TRACK — DIAGNOSIS SESSION (no code landed; tree == origin, rung suite 164/164 x3 green). Full write-up: `FINDING-2026-07-24-CLAUDE-PL-NREV-PERF-FOUR-DISPROVEN-SUSPECTS-AND-DET-DELIVERY-IS-24PCT.md`. 🔑 BIGGEST RESULT — HALF THE DEFICIT IS AN `-O0` MEASUREMENT ARTIFACT + MODE-3 IS CAPPED AT `-O0`: measured at the sanctioned `-O1` perf level, mode-4 nrev x5000 is **0.727s vs `-O0` 1.367s = −47%, ZERO code change** (`~15×`→`~8×` gprolog). Prior cursors' headlines (s139 "38.4×", s140 "33×") are ~DOUBLE the real `-O1` figure — they timed an `-O0` runtime. ⚠ MODE-3 CANNOT SEE THIS: `scrip --run` does NOT link the `.so`; its hot runtime (by_name_dispatch.c deref/unify, gc_heap.c alloc, unification.c) is compiled INTO `scrip` at `-O0` (`CBASE:=-O0`, `CRT:=$(CBASE)`, Makefile 513/514/544; `ldd scrip` has no libscrip_rt). So an `-O1` `.so` helps ONLY mode-4; **mode-3 leaves ~47% on the table, recoverable purely by BUILD CONFIG — the single biggest lever, NOT a code change** (proof needs no scrip rebuild: mode-4 runs identical runtime source at `-O1` and is 47% faster). RECOMMENDED NEXT (biggest+safest): add a perf build variant compiling scrip's RUNTIME TUs at `-O1` (keep parser/emitter `-O0` for dev-loop speed), gated behind an explicit flag so O0-DEV default is untouched; re-measure mode-3, expect ~47%. NOT landed this session (build-system change touching the FACT-RULE-protected `-O0` default; scrip rebuild ~5min; half-tested Makefile edit worse than a measured recommendation). DET-DELIVERY (first-arg indexing, PL-SPEED-5 lever 2) STACKS ON `-O1`: mode-4 `-O1` plain 0.727s → +base-clause-cut 0.526s = −29% more; ROADMAP `-O0` 1.367→`-O1` 0.727→`-O1`+DET 0.526 = **2.6× total, ~15×→~5.8× gprolog**; residual ~5.8× is the boxed-cell-vs-WAM structural gap (RSP-F-4 register residency / deferred HB). ── ⚠ METHODOLOGY TRAP (why prior sessions re-chased ghosts): gdb sampling of a MODE-3 run mis-attributes leaf frames — the JIT blob has NO symbols, so `??` frames (18/40 leaf, 421/1200 all-frame) get attributed to whatever C sink the PC is near. A mode-3 sample showed `rt_proc_hash_lookup`/`rt_proc_fnv` ~12% leaf; a call-COUNT instrument proved it runs <300k times in nrev x20000 (~10M proc-calls) = NOT a per-call path (confirms s140's red-herring note; the pointer cache works). RELIABLE METHOD = profile the MODE-4 binary (`scrip --compile` → `as` → `gcc -no-pie … libscrip_rt.so` → gdb `-p PID -batch -ex "bt 8"` loop); real symbols, `??` drops 421→9. FOUR SUSPECTS DISPROVEN by direct measurement (not sampling): (1) proc-resolution — <300k calls, cache hits. (2) per-alloc INIT CONSTANT (s140's (A)) — consolidated all 5 per-call init/env guards in `rt_gcheap_alloc`/`_carve` into one boot ctor + cached flags + fast-carve-first, semantics-neutral, env features verified: **1.319 vs 1.308s = ZERO (noise)**; REVERTED. (3) GC — env-gated collect counter = **0 collects** for nrev x5000 (118MB fits the 512MB heap). (4) cons ZEROING — env-gated skip of the 79MB HB_PLJ memset: **1.340 vs 1.316s = no gain** (32B sequential memset is free; also UNSAFE — 2 of 6 rt_plj_alloc callers leave a cell unwritten on the ar==0 edge). Alloc VOLUME real+inherent (SCRIP_ALLOC_HIST: 2.475M type-209 conses, 79.2MB, ~495/iter, O(n²)) but every PER-ALLOC lever is noise-or-unsafe. EMITTED EVIDENCE for the CP tax: `proc_app$2F3_α` arms the CP UNCONDITIONALLY at entry (clause-2 resume `[rbp+1632]` + `rt_pl_dop_trail_mark` box n0) BEFORE arg0 discrimination; slice-A ix guard (`dop_ix_g`, s113) fast-fails clause-2 redo but the CP is already armed + trail marked, so failure-driven backtrack redoes every app CP + unwinds its trail — the measured `pl_trail_unwind` ~9% leaf is the signature. NEXT after the `-O1` build variant: **PL-SPEED-5 lever 2 DET-delivery** (~29% at `-O1`), per s139's shape (c) runtime CP-collapse when the arg0 ix-guard proves a sole candidate; CORRECTNESS-CRITICAL (collapsing a CP with a live alternative = SILENT WRONG ANSWERS — gate nondet-app-UNBOUND-arg0 hard, must NOT collapse); READ THE BB-CODEGEN DESIGN SET (PLAN step 6) first (touches shared list-unify emission). WATERMARK: SCRIP `<unchanged — tree == origin; out/ gitignored so the -O1 .so is not a tree change>` / .github `<local — this cursor + the s141 FINDING; push pending credential>` / corpus `<unchanged>`. CONTAINER OPS: gdb+gprolog+nasm via `apt-get update` then `apt-get install --no-install-recommends` (plain install 404s on an unrelated libpoppler recommend); `-O1` `.so` build ~4.5min detached via `setsid` (survived tool-boundary; `rm -f out/rt_pic/*.o out/libscrip_rt.so` FIRST per the FACT-RULE timestamp trap — a bare `make RT_OPT=` change relinks stale objects). BANKED (carried from s140, none resolved here): **NO LAST-CALL OPTIMIZATION** — plain `loop(N):-...,loop(N1)` deep recursion SEGFAULTS ~N=50–100 (grows the machine stack per tail call; failure-driven loops fine); own rung. (a) NESTED-\\-> BINDING LEAK (MONITOR-HUNT). (b) retractall/1 existence_errors (needs retract/1 generator fix). (c) compiled-path IR_CALL_PROC_STAGED silent-fail on undefined user predicates.**
