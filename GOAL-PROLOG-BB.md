...
**STATE (2026-07-05, this session): rung suite `PASS=7/115` m2≡m3≡m4 (was 5/115). rung03 (`=/2` unification) landed this session on the DESCR/by-name spine with ZERO new IR opcodes and ZERO new templates; rung42_floatunify_bind came along free (the `$unify` primitive is type-general). Icon unregressed (223/30/36 both modes); SNOBOL4 smoke 6/1 both modes (the 1 FAIL is pre-existing, stash-proven). `bench 22` NOT re-attempted at scale.**
...
**NEXT:** rung04 (`arith` — `is/2` and arithmetic comparison `</2`,`>/2`,`=:=`, etc). The current `is_builtin_exec`/`is_builtin_argw` arms build bare `IR_OP_COUNT` placeholders; `is/2` needs a value-producing arith-eval (candidate: an `IR_CALL_BUILTIN "$is"` producing a bound value, or reuse `IR_BINOP` arith on deref'd operands) and the comparisons need the same boolean-call-to-ω shape `$unify` just proved. Read `lower_icon.c`'s `IR_BINOP`/`bb_binop_arith` handling and the existing `$unify` primitive before touching anything; ask for the menu before using any template.
