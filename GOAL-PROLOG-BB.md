...
m2/m3 **115/115** byte-identical · m4 **101/0/14** · siblings Icon m2 12 · SNOBOL4 m2 7. SCRIP HEAD `f44c20c` (PT-3 catch/throw native landed; prior PT-1b meta resolver + PT-2b conj `2cfd1bb`, PT-2b-SIMPLE `ea9e5ea` + literal-int-LHS is `c8063ec`, PT-0/1a/2a `62426a6`).
...
- [ ] **PT-1** — **PT-1a LANDED `62426a6`** (rt_call_term + rt_redo_meta in unification.c; true/fail inline; single-level meta). **PT-1b CONJ/DISJ LANDED `2cfd1bb` (2026-06-04 Opus 4.8):** term-level frame-tree resolver in unification.c transcribing gprolog `BipsPl/call.pl` `'$call_internal_with_cut'` — MK_CONJ forward/backward index driver, MK_DISJ with per-branch CP+trail marks (`meta_disj_mark/unwind` = the host-CP analog of gprolog's two `'$call_internal_or'` clauses; needed because deterministic bindings in a failed branch sit under no CP), MK_PRED transcribes the bb_goal.cpp α/β protocol PER-FRAME (`g_pl_meta_redo` single static DELETED — redo target + CP mark live in each frame; the frame tree IS the nested meta-call stack, reentrant via new exports `rt_meta_solve(goal, &root)`/`rt_meta_redo(root)`; old rt_call_term/rt_redo_meta kept as compat shims over one static root), MK_BUILTIN term-level set {`is`, `=:=`, `=\\=`, `<`, `>`, `=<`, `>=`, `=`, `\\=`} (`is/2` = `Load_Math_Expression`-law recursive term eval + GENERAL UNIFY; int core = one `rt_arith(IR_LIT_I,…)` call — NO-DUP, exact m2 int parity; float arms for ±*/ min max abs; pi/e), `call/1` recurses. STILL OPEN: `!`/`->`/`\\+`/catch/throw inside meta-calls rejected LOUDLY (stderr) — `rt_call_term(Term *goal)`: deref → atom/compound → (name, arity, args); `true`/`fail` inline;
      ...
      single-level meta (one live redo target); PT-1b `,`/`;` DONE (above). PT-1b REMAINDER = `->`/`\+`/`!`:
      **Cut inside a meta-called goal is LOCAL to the call** (gprolog
      hidden `A(arity)` cut register) — design jointly with WAM-CP-9 ITE-commit (same local-barrier mechanism); resolver rejects these LOUDLY until then.
- [ ] **PT-2** — **PT-2a LANDED `62426a6`** (rung43 `[]` m2==m3==m4; one old EXCISED rung also unlocked → m4 91). **PT-2b-SIMPLE LANDED `ea9e5ea` + `c8063ec` (2026-06-03 Opus 4.8) — m4 91→95:** findall over nondeterministic SIMPLE goals (admission = goal graph contains EXACTLY ONE control node — `pl_findall_goal_graph_simple`; rejects conj/disj whose graph ENTRY is the first goal's α masquerading as a simple registered goal, the wrong-answer trap this session hit — + callee in compile-time pred registry, suffix-stripped `"name/arity"` match, + args/tmpl/result term-buildable). `emit_build_compound_term` gained the IR_GOAL arm (arity-0 → atom for rt_call_term's TERM_ATOM path); `rt_findall_term` now saves/restores outer env+cut state (redo-exhaustion left g_resolve_env on the callee env → `write(Xs)` printed `_`). Companion landing `c8063ec`: literal-int-LHS `is/2` (`0 is X mod 2`; gprolog `arith_inl.wam` is/2 = math_load_value + get_value = general unify, type-sensitive, NOT =:=; `rt_is_lint` + TEXT arm + RICH-gate-only admission — flat gate untouched per the float-unify m3-in-process precedent). New passes: findall_{basic,template,filter} + dcg_generate (phrase → single DCG-translated IR_GOAL). **PT-2b-CONJ LANDED `2cfd1bb` (2026-06-04 Opus 4.8) — m4 95→96 (findall_arith de-EXCISED, `[1,4,9]`):** admission widened with `pl_findall_goal_conj_admissible` (goal graph = EXACTLY ONE `IR_GCONJ` + its `bb_conj_state_t.goals` members and nothing else control-flavored; each conjunct = registry `IR_GOAL` via `pl_findall_goal_admissible` OR `IR_BUILTIN` ∈ {is,=:=,=\\=,<,>,=<,>=} with α/β term-buildable). `emit_build_compound_term` gained the `IR_GCONJ` arm (right-nested `','(g0, ','(g1, …))` via `emit_build_conj_chain`) + the `IR_BUILTIN` arm (admitted set as `functor(α,β)`, IR_ARITH-arm shape). Findall TEXT arm now serializes the GCONJ CONTAINER when the goal graph holds one (`fs->goal_node` is only the first conjunct's entry — the masquerade trap from last session, now routed around instead of rejected). DISJ resolver arm exists term-level (dormant-but-canonical); DISJ ADMISSION deferred until a rung demands it. OPEN (was): **PT-2b control-construct goals** — DONE for `(A,B)`; `g_pl_meta_redo` single-static replaced by PT-1b's frame tree as planned. Original plan text: findall onto the rail: `bb_findall_state_t` gains `goal_node` (additive; set in lower.c
...
## 📊 Gate table (2026-06-04 session end — PT-1B META RESOLVER + PT-2B CONJ ADMISSION `2cfd1bb` LANDED: findall over conjunction goals (`findall(Y, (num(X), Y is X*X), Ys)` → `[1,4,9]`) emits + runs natively in mode-4. unification.c rebuilt around a TERM-level frame-tree resolver transcribing gprolog `BipsPl/call.pl` `'$call_internal_with_cut'`: MK_CONJ forward/backward driver; MK_DISJ with per-branch CP+trail marks (`meta_disj_mark/unwind` — the host-CP analog of gprolog's two `'$call_internal_or'` clauses; deterministic bindings in a failed branch sit under no CP, so the resolver owns the unwind); MK_PRED transcribes the bb_goal.cpp α/β protocol per-frame (`g_pl_meta_redo` single static DELETED; the frame tree IS the nested meta-call stack — reentrant `rt_meta_solve/rt_meta_redo`, old pair = compat shims); MK_BUILTIN term-level {is,=:=,=\\=,<,>,=<,>=,=,\\=} with `is/2` = `Load_Math_Expression`-law recursive eval + GENERAL UNIFY, int core = one `rt_arith(IR_LIT_I,…)` call (NO-DUP, exact m2 int parity). `!`/`->`/`\\+`/catch/throw in meta-calls rejected LOUDLY pending the WAM-CP-9 joint local-barrier design (Lon sign-off). Emitter: `emit_build_compound_term` + IR_GCONJ arm (right-nested `','` chain) + IR_BUILTIN arm; findall TEXT arm serializes the GCONJ CONTAINER (goal_node = first conjunct's entry, the masquerade trap, now routed around). Admission: `pl_findall_goal_conj_admissible` (one GCONJ + member conjuncts only; conjuncts = registry IR_GOAL or admitted arith IR_BUILTIN with buildable α/β). m4 **95→96**. Gates re-verified post-rebase onto ICN-SCAN-3 `d629a36` (shared scrip.c/emit_bb.c/x86_asm.h touches — clean merge, all green at merged HEAD). NOTE: corpus .s box labels are generation-NONDETERMINISTIC (address-derived `bbNNNNN_α`) — verified same-compiler back-to-back runs differ; .s byte-diff churn across sessions is expected, the suite set-diff is the invariant; a deterministic label counter is a future hygiene item. Prior: PT-2b-SIMPLE FINDALL `ea9e5ea` + LITERAL-INT-LHS IS `c8063ec` (m4 91→95; findall basic/template/filter + dcg_generate; rt_findall_term env-restore fix; rt_is_lint = rt_arith eval + general unify), SCALAR FLOAT-UNIFY `873792f` (m4 87→89), ITE-NESTED-NORMALIZE `bfabff3` (86→87), PT-0/1a/2a `62426a6` (89→91), PL-HY-1a/1c COMPOUND-UNIFY `374c2ff` (75→86), PL-HY-FENCE `1a0127e`, WAM-CP-7c `5dff1a8`.)
