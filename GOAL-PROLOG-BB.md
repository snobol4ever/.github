...
**Next:** PL-AREAS-3b — the E-area deterministic-POP. Prerequisites now met: `cs->det` marks bounded calls at the call site; `bb_callee_frame` op_sa==1 emits the γ/ω return already. The live switch is: (i) `rt_enter` for bounded calls bumps from E-area (drop the if(!*slot) cache — the documented #1 corruption trap); (ii) `bb_cell_call` caller saves E-mark at α and resets at γ; (iii) non-bounded frames stay on GC_malloc. MONITOR-FIRST — frame-lifetime bugs are the silent-corruption class the 2-way sync-step monitor exists for.
