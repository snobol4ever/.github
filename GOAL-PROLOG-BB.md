...
## ▶ STATE (2026-06-24 — bench suite 7 GREEN / 4 frontier / broken=0; rung suite 115/115 m3+m4; m3≡m4)

**THIS SESSION (Claude, 2026-06-24 PM) — PB-ITE-CALLEE landed (`c30a975`): IF-THEN-ELSE now admitted INSIDE callee clauses.** Driver-only widening, NO emitter change (the emitter's `gz_emit_ite`/`gz_emit_chain` already consume the ITE state — cond/then/else heads + `gate_slot` — shape-agnostically; the main-clause path already had ITE via `pl_gz_build_goal`). Three balanced touch points in `scrip.c`: (1) ADMIT — `pl_gz_rule_clause` stops hard-rejecting `IR_ITE`, validates cond/then/else roots via new `pl_gz_rule_body_root_ok` (GCONJ-aware), and excludes ITE-branch GCONJs from the single-body-GCONJ search (mirrors `pl_gz_admit`'s `claimed[]`); (2) COUNT — `pl_gz_nsynth_chain`/`pl_gz_clause_nsynth` refactored onto a shared `pl_gz_nsynth_goal`/`pl_gz_nsynth_root` (also removes pre-existing duplication per NO-DUP rule), ITE counts +1 gate + cond/then/else sub-counts; (3) BUILD — `pl_gz_callee_body_node` `IR_ITE` arm + new `pl_gz_callee_build_root` (lbase-remapping analogue of `pl_gz_build_root`), gate slot from `synth_next` so callee frames (`arity+nlocals+nchild`) auto-size. Tested m3≡m4 on bare/nested-arith/3-arg-digit ITE-callee shapes. **Floor held: rung 115/115 m3 AND m4; bench green=7 frontier=4 broken=0 (regen changed=0, byte-neutral).** **This sequencing matters: ITE-in-callee was the highest-leverage single SENDMORE prerequisite that is driver-only/emitter-free. SENDMORE still ALSO needs arity-8 (emitter ABI: args 0-3 in regs, 4+ through callee frame cells — `bb_cell_call.cpp` `bcc_sh`/`bcc_ar` + the `ar>4` admit gates) AND gated nbodies-16 (the arith-head fence below). No frontier bench is one-widening from green; the next green requires the arity-8 emitter change, which a fresh-context session should take.**

**PRIOR SESSION (Claude, 2026-06-24 AM) — PB-BENCH-3 `zebra` CLOSED GREEN; pre-existing arity-4 bug FIXED; nested-arith + `//` capability landed. Bench `green=7 frontier=4 broken=0`.** Three tested, floor-safe widenings (the full PL-BB ladder rungs for the remaining four stay open):
...
- **meta_qsort**: full meta-interpreter — `clause/2` + `call/N` meta rail (PL-BB-5).
