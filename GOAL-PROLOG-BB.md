...
**<= LIVE CURSOR (2026-07-23, s132): TWO STRANDED-SENTINEL ISO PREDS FIXED — `\=`/2 (rung81) + `writeln`/1,/2 (rung82). Rung suite 161->163/163 x3 modes. SCRIP <LOCAL, PUSH BLOCKED — no credential> / corpus <LOCAL, PUSH BLOCKED>. ROOT CAUSE (shared): `op=122` is `IR_OP_COUNT`, the enum terminator, NOT a real opcode. TWO places in lower_prolog.c build stranded `IR_OP_COUNT` placeholder nodes that reach emit_drive and abort: (i) the old hand-rolled `\=` arm (~line 356), and (ii) the `is_builtin_exec` fallthrough arm (~line 599) which builds `IR_OP_COUNT` for ANY whitelisted builtin lacking a specific lowering arm. Both were never-worked paths (not regressions); zero corpus coverage hid them (161/161 was green while `\=` — a CORE ISO §8.2.2 predicate — always crashed, even bare `a \= b`). FIX PATTERN (both, pure LOWER reuse, no new globals / no template / no runtime change): route the broken predicate through an EXISTING working arm. (1) `\=`/2 -> `lower_ite( =(X,Y), fail, true )` — mirrors gprolog unify.pl:56 `X \= Y :- \+ X = Y`; `\+` already lowers via lower_ite. 13-/1+. (2) `writeln`/1 -> `goal( (write(X), nl) )` and `writeln`/2 -> `goal( (write(S,X), nl(S)) )` — reuses the working $write/$write2/$nl0/$nl1 arms; the `,`-conjunction-synth-then-`goal()` idiom is already used by forall/setup_call_cleanup/with_output_to. ORACLES: `\=` vs gprolog 1.4.5 (m2==m3==m4 byte-exact, 7 cases incl. binding purity); `writeln` vs swipl 9.0.4 (gprolog 1.4.5 has NO writeln — existence_error; writeln==write+nl UNQUOTED confirmed). corpus rung81_neq_unify + rung82_writeln (ground-only .expected, engine-stable). GATES: rung suite 163/163 x3; smoke 5/5 x3; no_value_stack PASS; no_new_global unchanged (ONLY pre-existing g_pl_disj_ctr FAIL, floor 14 — git diff shows ZERO globals touched). BANKED (each its own rung): (a) **NESTED-`->` BINDING LEAK (pre-existing, NOT \=-specific, affects `\+` identically)**: when a `\+`/`\=` whose body is an `->` is used as the CONDITION of an OUTER `->`, the inner condition's bindings leak past the outer soft-cut — `( (X \= a) -> true ; true )` gives scrip `bound(a)` vs gprolog `unbound`; `\+` leaks the same. General lower_ite/soft-cut trail-unwind defect; MONITOR-HUNT candidate. rung81 uses only the correct disjunction form so it is a faithful non-flaky lock. (b) **`retractall`/1 CRASHES (op=122)** — EXPLORED + REVERTED this session: the gprolog-shaped reuse `retractall(H) ≡ (retract(H),fail;true)` (matches gprolog assert.pl:92-96 exactly) compiles and is correct for single-clause + nonexistent, BUT surfaces a DEEPER pre-existing bug: **`retract/1` only retracts the FIRST match under backtracking** — isolated `(retract(d(X)),write(X),fail;true)` over d(1),d(2),d(3) prints only `1`, leaves `[2,3]` (swipl: `1 2 3`, `[]`). rung14 retract corpus passes only by sidestepping via explicit recursion. Fix retract/1's generator (MONITOR hunt) → the reuse arm becomes correct for free; OR register a synthetic recursive `$retractall` helper. Not shipped (byte-exact bar). (c) THE STRANDING ARM ITSELF is architecturally wrong: line ~599 crashes on any whitelisted-but-unhandled name/arity instead of a clean unknown-predicate error — a sweep found ~1 more genuine valid-arity crasher (retractall) amid wrong-arity noise (arg/1, functor/1, sort/1 etc. correctly shouldn't exist but crash-instead-of-fail). NEXT: retractall/1 (new handler, swipl+gprolog oracle); OR the nested-`->` MONITOR hunt; OR make the stranding arm fail-clean instead of crash. FINDING-2026-07-23-CLAUDE-PL-NEQ-UNIFY-STRANDED-ITE-SENTINEL-FIXED-AND-NESTED-ARROW-LEAK.md has full detail. rung8[0-9] glob already covers rung81/82.**
