...
**<= LIVE CURSOR (2026-07-21, s117): char_io LANDED (get_char/peek_char/get_code/peek_code /1+/2, put_code /1+/2; EOF -> end_of_file atom / -1 code; ISO type/instantiation errors on put_code). Input via fh_cur_in_fp (/1) and pl_resolve_stream_arg want_output=0 (/2 aliases user_input/user_output). BOARD 144->145/145 x3 MODES, m3==m4, byte-exact vs gprolog 1.4.5 + swipl 9.0.4. 3 edits in by_name_dispatch.c (det-gate ~L200 + handler block before $write2 + name/arity table ~L3413), ZERO lower_prolog.c change (char_code pattern), no-new-global PASS floor 14, no regressions. corpus rung64_char_io. NOTE: open/3 NOT implemented -> file-stream /2 forms blocked on it (separate feature, now the #1 stream gap). ALSO THIS SESSION: GOAL FILE PRUNED (511->355 lines, 161KB->51KB; landed rungs + old cursors cut, git holds them; FACT-RULE blocks byte-verified verbatim). NOT PUSHED (awaiting credential). PRIOR (s116): const_io LANDED (write_to_atom/2, format_to_atom/3, read_from_atom/2, with_output_to/2 atom/string/codes/chars sinks, format/3 atom-sink). BOARD 144/144 x3 MODES, verified byte-exact vs gprolog 1.4.5 + swipl 9.0.4. Mechanism: open_memstream capture (fh_capture_begin/end in driver_globals.c); with_output_to lowered as a setup_call_cleanup bracket; LIFO nesting via pl_wot_stk. SCRIP 9b701d6b, corpus dfbc1f35. STILL OPEN (priority order): (1) open/3 + close/1 file streams (unblocks file-stream char/term I/O; get_char/2 etc. already accept a $stream(N)/DT_FH via pl_resolve_stream_arg, just no way to MAKE a file stream yet); (2) write_canonical LIST dialect split - SCRIP prints [a,b], GNU/ISO wants '.'(a,'.'(b,[])), SWI keeps [a,b] - gate on a dialect flag; (3) current_stream/1 + stream_property/2 (GEN rail).**
...
- [~] **PL-ISO-7 STREAM I/O.** read/1 + read_term/2 (7a), stream core + term-writer reroute + stream-form writers + const_io (s115/s116) LANDED. OPEN: char_io.pl (get_char/peek_char/put_code/get_code /1+/2 via fh_cur_in_fp; needs temp-file stdin); current_stream/1; stream_property/2.
