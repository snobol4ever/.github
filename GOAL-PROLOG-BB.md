...
**STATE (2026-07-05, this session): rung suite `PASS=5/115` m3≡m4 (was 1/115 — rung01 only, itself just-landed this session). `bench 22`/smoke are NOT yet re-attempted at scale — only the 5-case smoke run, `2/5` m2≡m3≡m4.**
...
- **Incidental unlock (not directly targeted):** `rung13_assertz_*` (×3) now pass too — a side effect of the fact-alternation + reduced-IR plumbing, not yet root-caused; flagging for whoever picks up rung03+ so it isn't mistaken for new work.
- **Also fixed in passing:** `ast_print.c`'s `flat_length`/`print_node` assumed every `tree_t.v.sval` was a real pointer; Prolog's `tr_assign_slots` legitimately stores a small integer slot index in that SAME union field for `TT_VAR` nodes (post-slot-assignment), which crashed `--dump-ast`/`--dump-ir` (`strlen` on a small integer cast to `char*`). Guarded on `(uintptr_t)v.sval >= 4096` before treating it as a string, excluded `TT_CLAUSE` (its `v.dval` holds arity, not a string) from the same print path, and print `TT_VAR` nodes as `#<slot>` when the union holds an integer. This is shared/general (`ast_print.c`, not Prolog-owned), but the fix is narrowly scoped to the pre-existing guard conditions and doesn't touch unrelated kinds.

**NEXT:** rung03 (`f(X,a)=f(b,Y)` — head/compound unification via `=/2`; the existing `goal()` `"="` arm builds a bare `IR_OP_COUNT` placeholder pair and needs the same reduced-IR reconstruction rung01/02 got). Read `lower_icon.c`'s binop/unify-adjacent handling for the transplant pattern before touching templates; ask for the menu before using any.
