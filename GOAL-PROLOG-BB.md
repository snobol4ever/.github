...
- [ ] **PL-AREAS-0 — DE-RISK on fib FIRST (one throwaway, do NOT skip).** Before touching the ABI: temporarily make `rt_enter` bump-allocate frames from a static slab instead of `GC_malloc` (fib is deterministic → no reclamation needed for the measurement). Rebuild runtime only; measure (a) fib `GC_malloc` count (baseline 21,891) → target ~0, and (b) fib wall-clock vs gprolog 11ms. **Decision gate:** if fib frame-mallocs collapse AND wall drops toward parity, the area-register campaign is justified — proceed. If it doesn't move, the bottleneck is call/return wiring not allocation — STOP and re-plan. REVERT the throwaway after measuring (it breaks reclamation; not production). Record the number in STATE.
