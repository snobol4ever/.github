...
m2/m3 **115/115** byte-identical · m4 **95/0/20** · siblings Icon m2 12 · SNOBOL4 m2 7. SCRIP HEAD `c8063ec` (PT-2b-SIMPLE `ea9e5ea` + literal-int-LHS is `c8063ec` landed; prior PT-0/1a/2a `62426a6`).
...
## 📊 Gate table (2026-06-03 session end — PT-2b-SIMPLE FINDALL `ea9e5ea` + LITERAL-INT-LHS IS `c8063ec` LANDED: findall over nondeterministic SIMPLE goals (single registered predicate call) emits + runs natively in mode-4 through the PT meta rail. `emit_build_compound_term` gains the IR_GOAL arm (goal Term from `bb_goal_state_t`; arity-0 → atom for `rt_call_term`'s TERM_ATOM path). Admission (RICH gate only): goal graph contains EXACTLY ONE control node (`pl_findall_goal_graph_simple` — rejects conj/disj whose graph ENTRY = first goal's α masquerading as a simple goal, the wrong-answer FAIL this session caught and re-excised) + callee in the compile-time pred registry (suffix-stripped `name/arity` match) + goal-args/tmpl/result term-buildable. BUG FIX `rt_findall_term`: redo-exhaustion left `g_resolve_env` on the CALLEE env (`rt_redo_meta` installs `cp->env`; the final failing redo never restores) → `write(Xs)` printed `_`; now saves/restores outer env + cut flag/barrier, mirroring `rt_findall`. Companion `c8063ec`: literal-int-LHS `is/2` (`0 is X mod 2`) — gprolog `arith_inl.wam` is/2 = `math_load_value` + `get_value` (general unify; LIT_I LHS = type-sensitive unify, NOT `=:=`); `rt_is_lint` = one `rt_arith` eval + one `unify` (NO-DUP); TEXT-only arm sibling of the V-2 binary arm; flat gate untouched per the float-unify m3-in-process precedent (m4-native, m3-interpreted). m4 **91→95**; new passes findall_{basic,template,filter} + dcg_generate (phrase → single DCG-translated IR_GOAL). m2/m3 byte-identity held; siblings Icon (`991a26b` re-baseline verbatim) + SNOBOL4 (stash-A/B) zero delta. Prior: SCALAR FLOAT-UNIFY `873792f` (m4 87→89), ITE-NESTED-NORMALIZE `bfabff3` (86→87), PT-0/1a/2a `62426a6` (89→91), PL-HY-1a/1c COMPOUND-UNIFY `374c2ff` (75→86), PL-HY-FENCE `1a0127e`, WAM-CP-7c `5dff1a8`.)

| Gate | Mode-2 | Mode-3 | Mode-4 | Notes |
|---|---|---|---|---|
| GATE-1 smoke | 5/5 ✅ | 4/5 | 5/5 | m3 unify = smoke-harness artifact; rung suite covers it |
| GATE-3 rung suite | **115/115** ✅ | **115/115** ✅ | **95 / 0 / 20** | byte-identical m2/m3. m4 91→95 (findall basic/template/filter + dcg_generate). EXCISED 20 = findall_arith (conjunction goal → PT-1b) · 5 retract · 5 abolish (PT-4) · 4 aggregate/nb (PT-4) · 5 catch/throw (PT-3) — the whole cluster sits on the PT ladder |
...
| siblings (HARD m2) | Prolog 115 ✅ · Icon 12 ✅ · SNOBOL4 7 ✅ | — | — | PT-2b touches are Prolog-territory only (bb_builtin*/scrip.c Prolog gates/rt_findall_term+rt_is_lint) → Icon suite = 991a26b re-baseline verbatim; SNOBOL4 pat suite stash-A/B identical |
