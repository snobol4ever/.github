...
## ▶ STATE (2026-06-13 — PIVOT: m3/m4 PARITY PRIORITY)

**⛔ PIVOT — GZ-ONLY (Lon, 2026-06-13, this commit). "DELETE the old way and take the hit. Leave only our future."** Both Prolog dispatches (m3 `--run`, m4 `--compile`) are now **GZ-ONLY**: `pl_gz_admit` or a loud `[PL-GZ FENCE]` abort. The flat tier and the rich/heap-env tier are no longer reachable from Prolog. **This SUPERSEDES the C-FRAME B-1/B-2/B-3 ladder** (we did NOT fix the rich path's m4 GZ-TEXT compound-term emit — we deleted the path instead). DELETED THIS COMMIT (build green, GATE-1 5/5/5 HARD intact): scrip.c `pl_flat_body_root`, `pl_rich_node_emittable`, `pl_rich_graph_ok`, `pl_rich_body_root` (~135 lines); emit_bb.c heap-env codegen `codegen_graph_block`, `codegen_callee_block`, `pl_catch_collect_graph`, `codegen_pl_catch_blocks`, `codegen_clause_dispatch`, `codegen_pl_pred_table` (~132 lines); the m3+m4 dispatch flat/rich fallback blocks. KEPT (shared, other languages): `bb_build_flat`/`codegen_flat_build`/`gvar_flat_chain_build` (SNOBOL4/Icon flat tier), and a 4-line `pl_catch_block_index` stub so the (now-unreachable) `bb_catch.cpp` template still links. `g_gz_no_struct_ptr=1` LEFT AS-IS in m4 (lifting it segfaults — m4 GZ-TEXT compound build is broken — rather than passing; a clean abort is better than a segfault). **REMAINING OLD-WAY DEMOLITION (next session, all currently dead-but-compiling, harmless):** `resolution.c` control engine + `resolve_bb_env_*` (entangled with the KEEP file `unification.c` + `rt.c` — needs care); the control-coupled templates `bb_goal.cpp`/`bb_choice.cpp`/`bb_catch.cpp` (GUT-list "control-coupled bb_goal/bb_choice/bb_catch") → stub to loud bombs, then the `resolve_bb_env_*` users collapse. Watermark: SCRIP `(A6 landed, this session)`. **m2 114/115 · m3 93/115 · m4 49/115 — ratchet floors m3 93, m4 49 (pivot RESET to m3 89/m4 48 was the accepted hit; A6 then +4 m3 / +1 m4 — nb_setval/getval + aggregate_all count/sum/max/min; m2 unchanged; GATE-1 HARD 5/5/5 intact).** Prior C-FRAME/C-LABEL forensics retained below as history. C-FRAME RE-ROOT-CAUSED 2026-06-13 (see GROUP C below): m3≠m4 is an ADMISSION divergence (`g_gz_no_struct_ptr=1` in m4 only), NOT the bare-path r12 theory recorded previously — that theory is disproven and superseded. Next concrete work = C-FRAME B-1 (fix m4 GZ-path compound-term TEXT emit; the segfault exposed by flipping `g_gz_no_struct_ptr=0`, which else drops m4 87→58). C-LABEL CLOSED (`3eb48b9`): IR_GOAL bb_prepare copied callee name verbatim into bb_ls_buf instead of routing it through `bb_intern_into` (which converts a name to its `.S<k>` strtab label — correct for bb_unify's RIP-string use, wrong for a call target; the `.`→`_` sanitize produced `.Lplpred__S1_2`). Compounding bug: `blbl_lea` in bb_resolve.cpp + one site in bb_aggregate_nb.cpp spelled the operand `"[rip+__]"` (no spaces) but the x86() classifier strcmp-matches `"[rip + __]"` (spaces) → fell through every dispatch arm → returned EMPTY → every compound-term-arg `lea` silently vanished, segfaulting rt_node_to_term(sval=0x1). Fixing both unblocked +26 (not the estimated +4) because the silent-lea corrupted compound-term construction for nearly every goal. bb_choice (prior `27b9fe7`): rt_cp_{trail_unwind,inc_cursor,get_cursor} helpers; dup-β removed; β_nosol label added.
Gates: GATE-1 m2 5/5 HARD · m3 5/5 · m4 5/5. GATE-3 m2 **114**/115 · m3 **93**/22-FAIL (floor=93, GZ-only) · m4 **49**/66-FAIL (floor=49, GZ-only; was 87 pre-pivot — the drop is the deliberate "take the hit" from deleting the flat+rich tiers).
...
Current m3: **91**/115. Ratchet: never regress. Full audit 2026-06-12 (Sonnet 4.6); scores re-measured 2026-06-13 (Opus 4.8) at HEAD `758d7b1`.

**22 m3 failures (floor=93 pass), 18 m4 failures + 10 excised (floor=49 pass).** All m3 failures hit PL-GZ FENCE ("not admitted"). Groups:
...
- [ ] **C-FRAME — m3≠m4 is an ADMISSION DIVERGENCE, not a bare-path bug (RE-ROOT-CAUSED 2026-06-13 Opus 4.8, supersedes prior entry; blocks rung06 + every struct-carrying program in m4).** SYMPTOM unchanged: `cnt([_|T],N) :- cnt(T,N1), N is N1+1` and rung06/len pass m2+m3, fail m4. **PRIOR DIAGNOSIS WAS WRONG** — it claimed "two emission paths, length lands on the bare path, m3 passes by luck on the bare path." DISPROVEN by measurement: m3 runs these on the GZ cell path (the clause shapes are ALREADY admitted by the validators; m3 is the proof), and m4 forces them to the rich/heap-env path. m3 passes because it is on the GOOD path, not by luck.
