...
Watermark: SCRIP `9f56a73` (battery green on merged main). **PL-GZ-0..4, 5a–5c, 6, 6b, 7a, 7b, 8, 8b, 9a LANDED** — details: git history of this file + handoff docs.
Gates: GATE-1 m2 5/5 HARD · m3 4/0/1-EXC (`recursion`) · m4 5/5. GATE-3 m2 **115/115 HARD** · m3 **22**/0/93-EXC (+1 rung03) · m4 105/0/10-EXC. gz2..7+gz8+gz8b gates PASS, corrupt-proven. Coupling ceilings: choice 19 · goal 10 · others 0 · rung05 .s 39; new-path boxes (incl. CELL_ITE, DET_IS, DET_CMP) emit ZERO control calls.
...
**PL-GZ-9a LANDED (9f56a73)**: compound-term unification (`IR_STRUCT`) in GZ query body. `rung03_unify` (f(X,a)=f(b,Y)) native m3 +1. Three-part landing: (1) `bb_cell_unify.cpp` new STRUCT arm — `rdi=r12` (GZ frame), `rsi/rdx=sealed IR_t* trees [rip+disp]`, calls `rt_pl_unify_struct_gz` which builds `Term*`s from frame cells recursively then unifies; (2) new `rt_pl_unify_struct_gz` in `unification.c` — GZ-frame-aware `IR_t*→Term*` builder using `cells[slot]` for `IR_LOGICVAR`, recursively building compound for `IR_STRUCT`; (3) `pl_gz_admit`: STRUCT nodes allowed when inside `IR_UNIFY` operand chain; `g_gz_no_struct_ptr` flag blocks STRUCT sealing in m4 TEXT path (pointer is in-process only — m4 falls through to flat/rich emitter which already handles rung03 correctly).
**Next opener: PL-GZ-9b** — multi-clause predicates with compound/list heads (rung05/06/08). Key blocker identified: STRUCT heads in callee clause bodies require a new admission tier. The existing `pl_gz_choice_inline` → `bb_cell_choice` path only handles atom/int constant matching; STRUCT head patterns (`[X|_]`, `[_|T]`) need `rt_pl_unify_struct_gz`-based matching routed through the `pl_gz_rule_callee_body` path instead. Attempted during this session — caused infinite recursion due to recursive predicates without depth guard. Design: for multi-clause predicates with ANY STRUCT head arg, bypass `pl_gz_choice_inline` entirely and use `pl_gz_callee_get_choice` → `pl_gz_rule_callee_body` which emits `CELL_UNIFY(slot_i, STRUCT_node)` chains. The `bb_cell_unify` STRUCT arm already handles `CELL_UNIFY(LOGICVAR{i}, STRUCT{...})` correctly via `rt_pl_unify_struct_gz(frame, lv_node, struct_node)`. The missing piece is (a) adding recursion depth/visit guard to `pl_gz_choice_rule_clauses` and (b) ensuring the `pl_gz_call_args_ok` + `pl_gz_fact_clause_units` relaxations are scoped correctly to avoid admitting programs the GZ callee frame can't handle (e.g., unbounded mutual recursion).
**Next opener: PL-GZ-9** — corpus reconquest: all 115 rungs onto the new path. findall = drive the NEW boxes (no meta rail); catch/throw = PT-3 CP-truncate + ball-copy LAW re-landed; aggregate/nb likewise; dynamic DB = B-full.
