...
m2/m3 **112/112** byte-identical · m4 **87/0/25** · siblings Icon m2 12 · SNOBOL4 m2 7. SCRIP HEAD `bfabff3`.
...
- DONE: bb_cut/arith/conj/ite/disj/catch/unify (PL-RV-1..5; **unify now covers compound (list/struct) operands too — PL-HY-1a/1c LANDED `374c2ff`; only float-unify `IR_LIT_F` still deferred to CAT-D**) · bb_builtin 28 BINARY arms + bb_fail (PL-RV-6).
...
## 📊 Gate table (2026-06-03 session end — ITE-NESTED-NORMALIZE LANDED `bfabff3`: recursive `pl_rewrite_control` in `prolog_lower.c` rewrites `( ->; )` and standalone `->` to `TT_IF` everywhere (nested in conj/disj, bare no-else), not just at clause-body top level. Fixes the 2 independent WAM-CP-9 bugs (m4 nested-ITE link gap + m2 ITE-bare-call no-output) — both ONE parse-tree root cause. Grounded in gprolog `Pl2Wam/syn_sugar.pl` `normalize_cuts1` + swipl `pl-comp.c` `compileBody`. m4 **86→87**; new corpus rung41 (m2/m3/m4 = `1 1 e c`). SCRIP HEAD `bfabff3` (rebased over peer SNOBOL4 concat `047dded`). Prolog-parser-only → m2/m3 byte-identity held, siblings Icon 12/12, SNOBOL4 7/7. The m2-oracle-WRONG ITE-commit *semantics* (WAM-CP-9, needs Lon) is untouched. Prior: PL-HY-1a/1c COMPOUND-UNIFY `374c2ff` (m4 75→86), PL-HY-FENCE `1a0127e`, WAM-CP-7c `5dff1a8`.)

| Gate | Mode-2 | Mode-3 | Mode-4 | Notes |
|---|---|---|---|---|
| GATE-1 smoke | 5/5 ✅ | 4/5 | 5/5 | m3 unify = smoke-harness artifact; rung suite covers it |
| GATE-3 rung suite | **112/112** ✅ | **112/112** ✅ | **87 / 0 / 25** | byte-identical m2/m3. m4 86→87 (rung41 nested-ITE). EXCISED 25 = findall, retract/abolish/assertz, aggregate/nb_setval, catch/throw, dcg_generate, float-unify (IR_LIT_F) — all need a runtime substrate (PLG-9g) |
| no_bb_bin_t | 0 ✅ | — | — | compiler-enforced |
| medium-invisible | **343** | — | — | bb_unify.cpp = 0 (x86()-pure); the 343 all in bb_builtin_* family files; informational |
| siblings (HARD m2) | Prolog 112 ✅ · Icon 12 ✅ · SNOBOL4 7 ✅ | — | — | ITE fix is `prolog_lower.c`-only → siblings structurally untouched |
