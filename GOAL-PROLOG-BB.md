...
- [ ] **PL-AREAS-2 — TR → bump area, TOP in R14.** Convert the trail from the realloc'd `g_pl_trail` array to a contiguous bump area whose top is R14; preamble inits R14 + the area base/limit; add the cross-language BB-jump save/restore STUB (the deferred trio swap). Convert `rt_trail_mark`/`rt_trail_unwind`/inline `pl_trail_push` to register-relative. Mechanical; proves the register-wiring + preamble path. Floor-neutral (same behavior, fewer C calls).
