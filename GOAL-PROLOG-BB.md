...
## ▶ STATE (2026-06-25 — bench suite 16 GREEN / 0 frontier / broken=0; rung suite 115/115 m3+m4; m3≡m4)

**THIS SESSION (Claude, 2026-06-25 #3) — PL-DESCR-2 sub-flip 2 made TURNKEY (no code flip this session; SCRIP UNCHANGED at `82d4c4d`, recovery point pristine).** Full survey of the atomic Term*→inline-cell flip: every touch site classified across 28 templates (74 `GZ_CELL_OFF` sites) + the runtime. Two new `.github` artifacts: `PL-DESCR-2-SUBFLIP2-MAP.md` (`5eb2b9ef`) — the classified plan (4 call-site patterns, 4 risk seams, helper-internal native/bridge split, edit order, fail-fast validate), and `PL-DESCR-2-SUBFLIP2-READY.md` (`ab5dec06`) — paste-ready C bodies for the DETERMINISTIC parts (trail global `g_pl_trail`, the 5 hot is/cmp helpers, `rt_unify_terms`/`unify_cell_const/float`, write/type-test bridge, `rt_pl_gz_init`/`rt_pl_cells_init` `pl_init_var`, new `rt_pl_compound_cell`, gutted dead env-readers) + the template edit list. **Plus a test-first artifact (SCRIP `6544586`):** `src/parser/prolog/pl_descr2_hot_test.c` — isolated, validates the READY doc's hot is/cmp/unify-const bodies (cell-native arith/compare/unify + partial-fail trail-clean) **20/20 GREEN** before they are pasted (mirrors sub-flip-1's `pl_cell_test` 33/33 + `pl_cell_conv_test` 11/11). Additive (not in the live build; scrip builds + smoke 5/5 unchanged), so SCRIP `6544586` preserves the `82d4c4d` green floor. **Key findings:** (a) the live GZ helpers already take `void*cell` (no signature churn — only `mov→lea` at call sites + internal rewrites); (b) `g_resolve_env` is NOT read by any live GZ helper (only legacy `rt_arith`/`bb_unify` non-cell boxes) so its deletion is gutting dead readers + the init-sync writes; (c) NO compileable-and-correct partial commit exists (half-flip compiles, segfaults all) — confirmed indivisible. **NEXT SESSION: execute the flip from the READY doc in one batch, build, fail-fast on fib (arg-copy seam), then zebra/qsort (compound builder), queens_8 (trail), term-ops (deep bridge); 115/115 floor + bench-16 + doomed 15→14.** Reason it was not flipped this session: the atomic ~30-file surgery + native-segfault debugging exceeded the remaining context budget with margin; the turnkey package is the de-risking deliverable that makes the next session's flip mechanical and fail-fast.
