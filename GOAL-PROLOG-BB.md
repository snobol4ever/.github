...
## ▶ STATE (2026-06-15 — m3≡m4 @ 113; PL-GZ dynamic DYNITER rail B2 LANDED; 6 of 8 dynamic reds closed)

**THIS SESSION (Claude) — built the runtime-clause DYNITER rail: dynamic predicates are now store-backed and enumerable. m3/m4 107→113 byte-identical (+6).** GATE-1 m3/m4 5/5; GATE-3 m3 **113** / m4 **113**, parity intact; NO-NEW-GLOBAL ratchet 15/15 (the store rides the existing SANCTIONED `g_pl_dyn_pred_table` — NO new global; marked-set + seeds are FIELDS of `g_stage2`, not new `g_*`); cross-lang stable (Icon run 122; IR enum additions append-only before IR_OP_COUNT, no value shift). Closed: **retract_basic, retract_unify** (rung14) + **abolish ×4** (rung15: existing, one_of_two, then_query_fail, then_reassert — incl. runtime `assertz` reassert). Remaining **2** m3≡m4 fails: `rung14_retract_retract_mixed` (FENCED both modes — m4 integer-literal-arg struct-build bug, see below) + `rung14_retract_retract_all` (callee-body `retract`, the B3 work below).
- **THE RAIL (how a dynamic pred becomes a Byrd box).** Detection → suppression → dyniter callee → population. (1) **Marking** (`prolog_lower.c` `pld_mark_scan`, run at top of `prolog_lower` BEFORE the directive fold): a functor/arity is dynamic iff it is a target of `retract`/`retractall`/`abolish` (anywhere) or `:- dynamic(F/A)` or **runtime** `assertz`/`asserta` (in a RULE body, gated by `mark_assertz = (cl->head != NULL)`); **directive** `:- assertz` alone does NOT mark (stays static-folded — this is the precise rule, and getting it wrong regressed rung13). Marks live in `g_stage2.pl_dyn_{name,arity,n}` (compile-time const metadata, same tier as `g_pl_nl_arith`; queried via extern `pl_dyn_is_marked`). (2) **Suppression + dyniter callee** (`lower_prolog.c` `lower_pl_register_all_preds`): a marked pred SKIPS static clause/choice lowering; instead `lower_pl_dyniter_graph` builds a 1-node graph whose entry is `IR_CELL_DYNITER` (state `pl_gz_dyniter_state_t{functor_atom, arity, cursor_slot=arity, mark_slot=arity+1}`, `nslots=arity+2`). (3) **Inline-path declines** (`scrip.c`): `pl_gz_fact_inline`/`pl_gz_choice_inline` return early for marked preds so the call falls through to the real CELL_CALL→dyniter callee instead of inlining baked static clauses; `pl_gz_rule_inline_check` + `pl_gz_rule_body_goal_ok` accept a DYNITER-entry callee graph; NEW `pl_gz_callee_get_dyniter` (nclauses=1, nlocals=2, body_head=the dyniter node). (4) **Population** (`prolog_lower.c`): directive-`assertz` clauses for MARKED preds are captured (`pld_seed[]`, a copy of the `assertz(arg)` goal tree) and PREPENDED to `main`'s clause body so they run as the first goals — seeding the store at startup. (5) **The box** (`bb_cell_dyniter.cpp`, NONDET four-port): α = `rt_trail_mark`→mark cell, `rt_pl_dyn_iter_begin(functor,arity)`→cursor cell, `rt_pl_dyn_iter_step`; β = `rt_trail_unwind(mark)` THEN `rt_pl_dyn_iter_step`. **The trail-mark/unwind on β is load-bearing** — without it the previous solution's bindings persist and the next `iter_step`'s head-unify fails against the still-bound arg (the bug that made retract_basic emit only the first solution). (6) **Runtime** (`unification.c`, all Term-only, rides `g_pl_dyn_pred_table`): `rt_pl_dyn_assertz_cell(clause_cell, prepend)` (copy_term_deep head+body, append/prepend), `rt_pl_dyn_iter_begin`→heap cursor held in a frame cell (NOT a global stack — same shape as the findall acc), `rt_pl_dyn_iter_step` (head-unify under per-step trail mark, advance/unwind). (7) **`IR_DET_ASSERTZ`** + `bb_det_assertz.cpp` (det recipe; `op_parts_ival[0]`=clause slot, `[1]`=prepend) — serves BOTH population AND main-level runtime `assertz`/`asserta` (admitted in `pl_gz_count_synth_goal` + `pl_gz_build_goal`, mirror of retract); `IR_DET_RETRACT`/`IR_DET_ABOLISH` (A0) now operate on the POPULATED store.
- **⚠ REMAINING 1 — `retract_mixed` m4 bug (FENCED, not fixed). SHARPENED DIAGNOSIS this session (investigated, not solved):** the bug is NOT in the integer term construction and NOT in retract itself. PROVEN by isolation in m4: (a) integer term build for the retract head is **byte-identical asm** to the seed-`assertz` head build (both `rt_node_to_term(int)`→`rt_compound_build_n(.S0,1,[n])`→unify-into-slot — verified by diffing `--compile` output); (b) `retract(fact(2))` SUCCEEDS in m4 (`( retract(fact(2)) -> write(removed) ; ... )` prints `removed`); (c) integer enumeration via the dyniter WORKS in m4 standalone (`assertz fact(1/2); fact(X),write,fail` → 2 solutions). **The bug is the INTERACTION: a SUCCESSFUL `retract` followed by dyniter enumeration yields EMPTY in m4 — but only for integer args; the IDENTICAL shape with ATOM args works in m4** (`retract(c(b)), c(X),write,fail` → a,d ✓; `retract(fact(2)), fact(X),write,fail` → empty ✗; both fine in m3). So: m4-only, integer-arg-only, requires a preceding SUCCESSFUL retract. `rt_pl_dyn_retract_cell` returns on a successful match WITHOUT unwinding its trail mark (intentional — `retract(age(bob,X))` must keep X bound, and that case passes in m4); for a GROUND head (`fact(1)`) the unify makes no bindings, so the trail-leak theory does not explain the ground case. NEXT: gdb the m4 binary on `/tmp/ts.pl` (`assertz fact(1/2); retract(fact(1)); fact(X),write(found),fail` → m3 `found`, m4 empty) — inspect the trail + the dyniter cursor/store row state right after the ground integer retract; compare the heap `Term` for the stored vs copied integer (suspect: an m4 integer-`Term` identity/`copy_term_deep`/`term_deref` divergence that only bites under the post-retract trail state). CURRENT MITIGATION (committed): integer-literal retract HEADS fenced in BOTH modes (`pl_gz_build_goal` retract arm: `if h0 is IR_STRUCT with any IR_LIT_I operand return 0`) → m3≡m4, retract_mixed back to its pre-session failing state (no regression vs the 107 floor). DELETE the fence after the fix → +1 both modes.
- **▶ B3 HANDOFF — `retract_all` (callee-body retract), the last of the original 8.** `retract(item(_))` lives in the BODY of a CALLED predicate (`retract_loop :- retract(item(_)), retract_loop`), not at main level → FENCED (both modes) because callee-body admission has no retract arm. Three sites (mirror the A0 main-level retract + the rung28 catch-in-callee-body extraction): (a) `pl_gz_rule_body_goal_ok` — add an `IR_BUILTIN "retract"/1` admission arm (head is LOGICVAR or STRUCT/ATOM, same shape test as the main-level arm; for `item(_)` the head is a STRUCT with a fresh var arg); (b) `pl_gz_rule_clause` whitelist — `continue` for `retract`/1; (c) `pl_gz_callee_body_node` — emit `IR_DET_RETRACT` with slot-mapping (`pl_gz_struct_slot_map` the head if STRUCT, synth a CELL_UNIFY to a slot like the main-level path, count the synth slot in `pl_gz_clause_nsynth`). retract_all's `retract_loop` is a 2-clause pred (the recursive clause + the base `retract_loop.`), already CHOICE-admissible; only the `retract` goal inside clause 1's body is unadmitted. Expected: +1 both modes → 114/114. (Note: retract_all uses `retract(item(_))` with a VAR arg, so the integer-struct fence does NOT block it.)



**THIS SESSION (Claude, SCRIP `c602da9` work base → rebased onto sibling Pascal tip `2b82494` → pushed `3db2ca5`; .github docs) — Lon DELEGATED the two open dynamic-rail forks to Claude ("Your choice. Continue."). Decisions made + increment A0 landed: det `retract`/`abolish` on a new SANCTIONED clause store, m3/m4 105→107 byte-identical.** GATE-3 live modes m3 **107** / m4 **107**, parity intact; NO-NEW-GLOBAL ratchet 15/15 (3 NEW globals added to the allowlist as SANCTIONED, not doomed); `bb_one_box` unchanged at 56; smoke m3/m4 5/5. Remaining **8** m3≡m4 fails = retract ×4 (basic/mixed/all/unify) + abolish ×4 (existing/one_of_two/then_query_fail/then_reassert) — all need the DISPATCH + POPULATION of increment B (below).
- **FORK DECISIONS (Lon delegated; recorded):** (1) **Fork 1** — minimal Prolog-only dynamic rail, store designed so Fork 2 (the shared EVAL/CODE/pattern rail that also unblocks the SNOBOL4 B-ladder via re-entrant `pl_gz_build`) is a clean later increment riding the SAME store. (2) **Global = `g_pl_dyn_pred_table`/`_n`/`_cap`**, SANCTIONED tier — the mutable sibling of the static `g_pl_pred_table`; a heap clause DB keyed by (functor-name, arity) holding `Term*` clause copies, NOT a control/value stack (per-activation control stays in box frame cells + the trail). (3) **Dynamic detection** = infer from `retract`/`abolish`/runtime-`assert` targets (covers all 10) AND honor an explicit `:- dynamic`; directive-`assertz` on a NON-targeted pred stays static-folded (no regression to green rungs).
- **A0 LANDED (`3db2ca5`, +2): det `retract`/`abolish`, MAIN-level only, on the (empty for A0) store.** Lands `rung14_retract_nonexistent` + `rung15_abolish_nonexistent`. The det-builtin recipe end to end: `IR_DET_RETRACT`/`IR_DET_ABOLISH` (IR.h + scrip_ir.c name table); `bb_det_retract.cpp` (1 cell arg) / `bb_det_abolish.cpp` (2 cell args), TEMPLATE-ONLY `x86()`; `emit_core` dispatch + `emit_bb` `bb_prepare` slot population + Makefile (RT_PIC_SRCS + compile rules); runtime `rt_pl_dyn_retract_cell`/`rt_pl_dyn_abolish_cell` in unification.c (mirrors the self-contained nb_setval store — `copy_term_deep` the stored head, `unify` under a trail mark, unlink+succeed on first match else 0; abolish always succeeds, clears the row). scrip.c **site 3** (`pl_gz_count_synth_goal`: +1 retract head, +2 abolish) + **site 4** (`pl_gz_build_goal`: named arms BEFORE the generic arity-1 arm; abolish reads the `F/A` spec as **`IR_ARITH`**/`IR_STRUCT` `operands[0]`=ATOM, `[1]`=LIT_I — verified via `--dump-ir`). **det suffices for ALL 5 retract tests** — none backtracks into a single retract; iteration is fail-driven. Sites 1+2 (callee-body admission) INTENTIONALLY deferred → `retract_all` (retract inside `retract_loop`'s body) stays correctly FENCED, no half-state.
- **▶ INCREMENT B HANDOFF (the remaining 8 → 115): the nondet store-iterator DISPATCH + clause POPULATION + runtime assertz.** Pieces:
  1. **Store-iterator dispatch box `bb_cell_dyniter` (nondet, HAS β)** — the synthetic DEFINITION for a CALLED dynamic pred. α calls `rt_pl_dyn_iter_first(functor, arity, &arg_cell0, &cursor_cell)`, β calls `rt_pl_dyn_iter_next(&cursor_cell, &arg_cell0)`; head-unify against a `copy_term_deep` of each stored clause under a trail mark (unwind on β re-drive); cursor lives in a FRAME cell (no global stack). Mirror `bb_cell_findall`'s four-port shape. functor/arity are compile-time constants (bake as immediates or a synth atom/int cell). The pred's formal args are frame slots 0..arity-1 (pass `&cell[0]` + arity; cells are contiguous by slot — verify `GZ_CELL_OFF(i)` stride).
  2. **`IR_DET_ASSERTZ` → `rt_pl_dyn_assertz_cell`** (deep-copy clause into the store, append; grows `g_pl_dyn_pred_cap` — currently unused, that's why it's there). Used by BOTH runtime `assertz` (abolish_then_reassert) AND directive-assertz population.
  3. **Lower-time dynamic pass** (lower_prolog.c / prolog_lower.c): mark dynamic preds (detection rule above); for them, SUPPRESS the static fold of directive-assertz (prolog_lower.c ~610–660) and instead synthesize (a) a `bb_cell_dyniter` DEFINITION for each CALLED dynamic pred [so a call to color/1 has an α-label whose body iterates the store — call sites unchanged], and (b) store-population. POPULATION HOOK: the GZ path runs ONLY `main`'s first clause (lower_prolog.c ~516–553; the `main.` fallback is dropped, directives are folded) — so prepend the dynamic facts as `assertz` goals to the entry conjunction (each an `IR_DET_ASSERTZ` in main's GCONJ — admitted exactly like the nb_setval det builtin), OR synthesize a boot entry that conjoins (assertz×N) ++ (call real entry).
  4. **scrip.c sites 1+2** (`pl_gz_rule_body_goal_ok` + `pl_gz_rule_clause` whitelist) for retract in a CALLEE body (`retract_all`), + `pl_gz_rule_callee_body`/`pl_gz_callee_body_node` codegen for `IR_DET_RETRACT` with slot-mapping (mirror the rung28 catch-in-callee-body extraction).
  - Detection MUST stay precise: only retract/abolish/runtime-assert targets become store-backed; directive-assertz alone stays static-folded (else green rungs regress). Test expected outputs are in the test files (`corpus/programs/prolog/rung1[45]_*.expected`).



**THIS SESSION (Claude, SCRIP `2c38d15` (work base) → `023fb43` (rebased onto Icon tip `9354db7`); .github docs) — landed rung28 `rethrow` (catch/3 inside a CALLED predicate body), m3/m4 104→105 byte-identical, AND found the tree had advanced past this file's `81b63f1` watermark to `2c38d15` with mode-2 (`--interp`) PHYSICALLY DELETED.** GATE-3 live modes m3 **105** / m4 **105**, parity intact; NO-NEW-GLOBAL ratchet 15/15 (no new global); no new box, no new IR kind, no runtime change. Remaining 10 m3≡m4 fails = **retract ×5** (rung14) + **abolish ×5** (rung15), still parked on Lon's two dynamic-rail forks (unchanged). See HANDOFF-2026-06-14-CLAUDE-PROLOG-BB-RETHROW-M2-DELETED.md.
- **rung28 `rethrow` LANDED (+1 each mode, `023fb43`).** Pure build-side fix in `src/driver/scrip.c`; emit + runtime UNTOUCHED. The REAL gate was the FIRST-PASS structural filter in `pl_gz_rule_clause` — it rejected `IR_CATCH` alongside CHOICE/DISJ/ITE *before* the semantic check ever ran, so the missing admission arm was only half the story. Four sites: (1) `pl_gz_rule_clause` stop-rejecting IR_CATCH (kept CHOICE/DISJ/ITE rejected; safe because catch `goal_g`/`rec_g` are separate `IR_alloc` graphs — only the catch node + catcher live in the parent `all[]`, so the single-GCONJ assumption at the GCONJ-uniqueness check holds); (2) `pl_gz_rule_body_goal_ok` IR_CATCH admission arm (recurse both sub-chains + validate catcher shape ATOM/LIT/LOGICVAR/STRUCT); (3) `pl_gz_callee_body_node` — NEW, extracted the per-goal builder out of `pl_gz_rule_callee_body` so a catch's γ-linked sub-chains reuse the identical slot-mapping builder — plus an IR_CATCH case building the `IR_CELL_CATCH` box with slot-mapped sub-chains + catcher, mark slot from `synth_next`; (4) `pl_gz_nsynth_chain` (NEW) + IR_CATCH case in `pl_gz_clause_nsynth` reserving the catch mark slot + sub-chain temporaries in the callee frame. The catch mark lives in a frame cell; propagation rides the pre-existing single-slot `g_pl_throw_ball` register (NOT a value stack — `rt_pl_throw_match` leaves a non-matching ball pending, unification.c:766). `gz_emit_cell`→`gz_emit_catch` + `gz_collect_callees` sub-chain recursion already handled catch inside callee bodies, so emit needed nothing. Follows the documented Admission-recipe (this file → Architecture reference).
- **⚠️ STALE-MAP — MODE-2 (`--interp`) IS DELETED AT HEAD.** `a2440f4` "DELETE the IR-graph interpreter — only BBs run, no interpreter accessible" (ancestor of HEAD `2c38d15`, NEWER than this file's `81b63f1` watermark) physically removed the `--interp` CLI arm; `94c94f4` then trimmed the CLI to `--run/--compile/--target/--dump-ast/--dump-ir/--transpile/--bench`. `./scrip --interp X` now errors `cannot open '--interp'`. CONSEQUENCES, each unhandled: (a) **GATE-1 smoke + GATE-3 suite still invoke `--interp`** (`test_smoke_prolog.sh`, `test_prolog_rung_suite.sh:51`) → m2 reports 0/5 + 0/115 = SILENT FALSE FAIL (NOT a regression); the smoke gate's "exit 0 iff m2 all-PASS" condition now ALWAYS exits non-zero. (b) The **"m2 114/115 HARD GATE"** premise across this file + handoffs is VOID. (c) The **catch-residue demolition gate** ("do NOT delete `g_resolve_catch_*`/`rt_catch_native` — m2 rung28 routes through them") and the **NO-NEW-GLOBAL doomed-floor 15** rationale ("each doomed `g_*` still reached by m2") are now STALE — several doomed globals may be genuinely dead and droppable. (d) The MANDATORY-READ + Architecture-reference sections below still describe m2 `--interp` as the oracle. **LIVE TRUTH: only m3 `--run` + m4 `--compile x86` exist; m3 105 ≡ m4 105.** DECISION FOR LON (not done this session — gate-harness re-baselining + demolition re-audit were not authorized, only flagged): re-baseline the gate scripts (drop/replace the m2 path; reset the smoke pass-condition) and re-audit catch-residue + doomed-floor now that m2 is gone.
- **Remaining 10 (retract ×5 + abolish ×5) UNCHANGED — still on Lon's forks:** (1) minimal Prolog-only dynamic rail vs the shared EVAL/CODE/pattern rail; (2) the new SANCTIONED global's name/home (`g_pl_dyn_pred_*`). The ⭐ KEY FINDING (`assert*` IS the Prolog `CODE()`/`EVAL()`; the runtime BB-compiler `pl_gz_build` already exists; building this rail also unblocks the SNOBOL4 B-ladder) is in the PRIOR STATE block below. NOTHING decided — no global added, no FACT RULE written, no allowlist edit.

## ▶ PRIOR STATE (2026-06-15 — m3≡m4 @ 107; PL-GZ dynamic rail A0 LANDED; Fork 1 chosen; SCRIP `3db2ca5`)

**A0 (det `retract`/`abolish` on the new SANCTIONED `g_pl_dyn_pred_table`, m3/m4 105→107) was the predecessor to this session's DYNITER rail. Lon delegated the two dynamic-rail forks to Claude; Fork 1 (minimal Prolog-only dynamic rail) was chosen. The full A0 narrative is in git history (`3db2ca5`) and HANDOFF-2026-06-15-CLAUDE-PROLOG-BB-DYNAMIC-CLAUSE-STORE-WIP.md.** This session's STATE block (top) supersedes A0: the store A0 created is now POPULATED + ENUMERABLE via the DYNITER rail.

## ▶ PRIOR STATE (2026-06-14 — m3≡m4 @ 104; ORIENTATION + DYNAMIC-BB-BUILDING INVESTIGATION, NO code change)

**THIS SESSION (Claude, NO code commit; SCRIP at HEAD `81b63f1`) — orientation + an architecture investigation at Lon's direction. The prior STATE block below was STALE (said 98); the tree had already advanced.** GATE-1 5/5/5 HARD; GATE-3 m2 114 / m3 **104** / m4 **104** (re-verified as found); NO-NEW-GLOBAL floor 15. m3≡m4. See HANDOFF-2026-06-14-CLAUDE-PROLOG-BB-DYNAMIC-BB-BUILDING-INVESTIGATION.md.
- **STALE-MAP CORRECTION.** catch/throw LANDED in `5abf275`→`00d7b72`→`cf2e1d5` (+4 each, 100→104) by a PRIOR un-handed-off session (NOT this session's work; that session left the STATE at 98). `findall_arith`+`dcg_generate` are already GREEN (the `lower_prolog.c` `pl_ll_*` lambda-lift closed them). The REAL current 11 m3≡m4 fails: **retract ×5** (rung14), **abolish ×5** (rung15), **rung28 rethrow ×1**.
- **rung28 `rethrow` ✅ LANDED `023fb43` (see top STATE block) — diagnosis below was correct.** Nested catch across a call boundary: `inner :- catch(throw(mine),other,…)`. `pl_gz_rule_body_goal_ok` (scrip.c:524, callee-clause-BODY admission) has NO `IR_CATCH` arm → the predicate `inner` is rejected → `[PL-GZ FENCE]`. The 4 passing rung28 cases all catch at MAIN level. Fix: add the `IR_CATCH` admission arm (mirror `pl_gz_admit` scrip.c:1242) + extend callee-body codegen (`pl_gz_rule_callee_body`) to emit the catch box inside a rule body with slot-mapping. Still GATED by the m2-catch-residue probe (do NOT delete `g_resolve_catch_*` yet).
- **retract + abolish ARE NOT SIMPLE BOXES — they need a RUNTIME DYNAMIC-CLAUSE STORE that does not exist on the GZ path.** Directive `:- assertz(fact(a))` is folded AT COMPILE TIME (`prolog_lower.c` ~634–690) into the static predicate, which is why directive-assertz works in all modes. But **`assertz` inside `main` FENCES** (GZ not admitted), and the legacy `pl_rt_assertz` builds IR at runtime (violates NO-IR-AT-RUNTIME, GZ-incompatible — dead end). So `abolish_then_reassert` + all 5 retract need a runtime store; only the 4 non-reassert abolish cases are doable with a throwaway "abolished" gate.
- **⭐ KEY FINDING (Lon's insight, verified): `assert*` IS the Prolog `CODE()`/`EVAL()`. A Prolog clause is CODE (a Byrd box), like a SNOBOL4 pattern is code. So `assert*`/`retract`/`CODE`/pattern-construction belong on the EVAL/CODE exception rail as ONE DYNAMIC-BB-BUILDING system.** The runtime BB-compiler ALREADY EXISTS — `pl_gz_build` (emit_bb.c:3827): `bb_alloc` RW page → `walk_bb_flat` emits BB machine code → `bb_seal` RW→RX → returns a callable `bb_box_fn`; the m3 driver already does `gzfn=pl_gz_build(); ir_delete_all(); gzfn(...)` (compile→free IR→jump in). `bb_pool.c`/`pat_pool.c` are general runtime code arenas. The EVAL/CODE rail (`exec_stmt`, `interp_eval_pat`) AND all runtime `pat_*` construction are currently BOMBED stubs (the SNOBOL4 "B-ladder") — the SAME missing piece. Building the dynamic rail for `assertz` ALSO unblocks the SNOBOL4 B-ladder. PREREQS: a runtime predicate-fn table (NEW sanctioned `g_*` clause-DB heap — proposed `g_pl_dyn_pred_*`, must be added to the no-new-global allowlist); `pl_gz_build` re-entrancy (file-static codegen state needs save/restore); a runtime-table fallback in `IR_CELL_CALL`/`bb_cell_choice` for callees with no compile-time α-label.
- **OPEN FORKS PUT TO LON (UNANSWERED — he said "perform hand off"):** (1) minimal Prolog-only dynamic rail vs the shared EVAL/CODE/pattern rail from the start; (2) confirm the new global's name/home (`g_pl_dyn_pred_*`, SANCTIONED tier). NOTHING decided — no global added, no FACT RULE written, no allowlist edit.
- **NEXT (recommended):** (i) rung28 rethrow (+1, isolated, no dynamic rail needed) — cleanest first commit; (ii) the dynamic-BB-building rail (foundational — unblocks retract ×5 + abolish_then_reassert + the SNOBOL4 B-ladder), pending Lon's fork answers; (iii) the 4 non-reassert abolish cases (do ON the new table, not a throwaway gate).

## ▶ PRIOR STATE (2026-06-14 — m3≡m4 PARITY @ 98; findall_filter + DCG struct-unify landed; DEMOLITION doomed floor 15)

**THIS SESSION (Claude, SCRIP `7561e59`→`89e8dd0`, rebased onto upstream Icon/SNOBOL4/Raku/Pascal commits; .github docs) — rung11 findall_filter + rung30 DCG (4/5), m3/m4 93→98 (+5 each, byte-identical).** GATE-1 5/5/5 HARD; GATE-3 m2 114 / m3 98 / m4 98; rung27 5/5, rung30 4/5; NO-NEW-GLOBAL floor 15 (no new global, no new box, no new IR kind); cross-lang green (SNOBOL4 m4 7/7, Icon 12/12/12, Raku 35/35/35). `bb_one_box` unchanged at 56 (A/B-proven pre-existing).
- **(1) `ae2dd38` — rung11 findall_filter (+1).** TWO coupled fixes. (a) LITERAL-LHS `is/2`: `even(X):-num(X), 0 is X mod 2` — `pl_gz_rule_body_goal_ok` now admits `IR_LIT_I`/`IR_LIT_F` as the `is` LHS; `pl_gz_rule_callee_body` synthesizes a fresh logicvar slot, binds it to the literal via `IR_CELL_UNIFY`, drives `IR_DET_IS` against it (runtime already `unify()`s LHS cell with the computed RHS — NO runtime change); `pl_gz_clause_nsynth` counts the new synth slot. (b) CHOICE CLAUSE CAP 4→8: the real test's `num/1` has 5 facts; `pl_gz_callee_t.clause_head[4]`+`pl_gz_choice_state_t.consts[4][8]` (IR_interp_state.h), the four `gz_emit_callee` stack arrays (nb/cladv/redo/cbase), `zsk[4]`/`lbase[4]` in `pl_gz_callee_get_choice`, and the three `>4` caps (pl_gz_choice_inline, pl_gz_choice_rule_clauses, bb_cell_choice) all widened to 8. The emit loops were already dynamic over nclauses; only sizes+caps were the ceiling.
- **(2) `89e8dd0` — rung30 DCG 4/5 (+4).** DCG `-->` rules expand (in the parser) to clauses threading a difference list: `ab-->[a],[b]` becomes `ab(S0,S):-S0=[a|S1],S1=[b|S]`; the body unifies a logicvar to a LIST (cons struct). Two GZ gaps: (a) ADMISSION `pl_gz_rule_body_goal_ok` IR_UNIFY arm only allowed scalar operands — extended to allow a STRUCT operand either side (mirrors `pl_gz_admit`'s own top-level struct-unify allowance); (b) CODEGEN `pl_gz_rule_callee_body` IR_UNIFY arm passed a non-logicvar operand VERBATIM, so a list's nested logicvars kept graph-relative indices — now routes a STRUCT operand through `pl_gz_struct_slot_map` (already used by the call-arg/findall paths). Closes basic_terminals/nonterminals/phrase3/pushback_rest in both modes.
- **DEMOLITION STALLED BY DESIGN (probed this session):** the 15 doomed `g_*` all serve the **m2** meta-rail / catch / resolution engine, which "forget m2" means LEAVE RUNNING, not delete. Measured caller graphs: `g_meta_compat`/`g_meta_builtins` feed `rt_call_term`/`rt_meta_solve` called only from `IR_interp.c` (m2 `call/N`+`once`); `g_resolve_active` read by `interp_hooks.c` (m2 dispatch); `g_resolve_exception` is m2's catch ball; `g_resolve_env`(125)/`g_resolve_cut_*`(43+34) deeply entangled with resolution.c. Deleting any would break the m2 HARD gate (114). Floor 15 cannot drop further until m2's `call/N`/`once`/`catch` are themselves migrated off the meta-rail OR Lon retires those m2 paths. (`g_resolve_nb_store`/`g_resolve_nb_count` already at 0 refs — the floor-vs-list gap.)
- **NEXT (pre-traced):** 17 m3/m4 fails remain, all parity. (i) FINDALL-COMPOUND-GOAL family closes BOTH `findall_arith` (`findall(Y,(num(X),Y is X*X),Ys)`) AND `dcg_generate` (`findall(X,phrase(item(X),[a]),As)` — callee takes a non-logicvar list arg) — lambda-lift the conjunction/compound findall goal into a synthetic helper pred at lower time, then the existing single-callee drive works (+2). The findall arm at `scrip.c:~1503` keys on `groot->op==IR_GOAL` (sees only the conj's first goal) and `:~1541` requires every callee arg be a logicvar. (ii) retract ×5 / abolish ×5 — each a NEW box (DB-cursor generator / bounded bulk-remove). (iii) catch ×5 — NEW control box, GATED by the m2-catch-residue probe (do NOT delete `g_resolve_catch_*` until it lands). See HANDOFF-2026-06-14-CLAUDE-PROLOG-BB-FINDALL-FILTER-DCG.md.
...
- [~] **PL-BB-6 — DYNAMIC DB + CATCH/THROW + DCG. [DYNITER rail LANDED for retract/abolish; catch/DCG already green; B3 + m4-int remain]** The tail.
  - **DYNAMIC DB — DONE via the DYNITER rail (this session, m3/m4 107→113).** NOT a `bb_cell_retract` cursor over the static DB as originally sketched — instead dynamic preds are store-backed: a runtime clause store (`g_pl_dyn_pred_table`), a synthetic `IR_CELL_DYNITER` callee per marked pred (enumerates the store, β re-drives the scan with trail-mark/unwind), `IR_DET_ASSERTZ`/`IR_DET_RETRACT`/`IR_DET_ABOLISH` det builtins, and startup population by prepending directive-`assertz` seeds to main. See top STATE block for the full rail. retract is realized as a det first-match remove (NONDET cursor-retract not needed for these rungs); abolish + the dyniter handle the enumeration nondeterminism. **2 reds remain:** retract_mixed (m4 integer-struct bug — FENCED; un-fence after fix) + retract_all (callee-body retract — B3, sites in top STATE block).
  ...
  - GATE: corpus near-full (113/115); resolution.c meta-rail + δ/ε demolition continues independently.
...
Current m3: **113**/115 (== m4 113). Ratchet: never regress (floors m3 113 / m4 113). Re-measured 2026-06-15 (Claude) post DYNITER rail. The 2 remaining: `rung14_retract_retract_mixed` (m4 integer-struct bug, FENCED both modes — un-fence after the m4 fix) + `rung14_retract_retract_all` (callee-body retract, PL-BB-6 B3 below).
...
- [ ] **B2 — rung28 catch/throw (+5 m3):** catch/throw already admitted in `pl_findall_conj_member_admissible` (lines 1660-1687). Need `pl_gz_rule_body_goal_ok` arm for IR_CATCH and `throw` IR_BUILTIN. Admission: IR_CATCH with `pl_findall_term_buildable(catcher)`. ⚠ GATED by the m2-catch-residue probe — do NOT delete `g_resolve_catch_*`/`rt_catch_native` until this box lands and a probe confirms the m3/m4 catch box does not route through them (m2 rung28 still passes through them).
...
[x] **B5 — rung30 DCG/phrase 4/5 (+4 m3/m4) LANDED 89e8dd0 (2026-06-14):** DCG `-->` expands in the parser to clauses unifying a logicvar with a LIST. Fixed in admission+codegen (NO new box): `pl_gz_rule_body_goal_ok` IR_UNIFY arm now allows a STRUCT operand either side (mirrors `pl_gz_admit`); `pl_gz_rule_callee_body` IR_UNIFY arm routes a STRUCT operand through `pl_gz_struct_slot_map` instead of verbatim. Closes basic_terminals/nonterminals/phrase3/pushback_rest in both modes. `dcg_generate` remains — it is `findall(X,phrase(item(X),[a]),As)`, a findall-compound-goal (callee takes a non-logicvar list arg), see B6.
- [ ] **B6 — findall-compound-goal (+2 m3/m4: findall_arith + dcg_generate):** `findall(Y,(num(X),Y is X*X),Ys)` — the findall GOAL is a CONJUNCTION; `pl_gz_build_goal`'s findall arm (scrip.c ~1498) keys on `groot->op==IR_GOAL` (sees only the conj's first goal `num(X)`) and silently drops the tail; and arm ~1539-1542 rejects any callee arg that is not IR_LOGICVAR (so `dcg_generate`'s `[a]` list-arg fences). FIX (closes both): lambda-lift the findall goal into a synthetic helper pred `'$fa0'(Shared):-<goal>.` at lower time in `lower_prolog.c`, then the existing single-callee drive handles the (now ordinary) helper body. Shared vars = vars in both template and goal → helper head args.
