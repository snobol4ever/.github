...
- OPEN: bb_goal(13)+bb_choice(6) real `x86()` conversion only when a backtracking program emits them natively in m4. PL-HY-1a unblocks bb_unify's deferred arm + restores m4 toward 86.
  - **DIAGNOSED 2026-06-03 (Opus 4.8) — bb_unify's deferred compound arm is SHOVEL-READY (no new runtime; gprolog-grounded; gate-safe by construction). Awaiting Lon's one-word sign-off; see `HANDOFF-2026-06-03-OPUS48-PROLOG-BB-COMPOUND-UNIFY-DIAGNOSIS.md`.** m4 EXCISED rung03/05/06 share ONE cause: compound (list/struct) `IR_UNIFY` deferred (gate `scrip.c:227-228` + `bb_unify.cpp` `u_deferred()` bomb). The substrate already exists: `emit_build_compound_term` (the sanctioned mode-4 serialized encoder bb_goal uses — bakes literal immediates + `[rip+strlabel]`, zero runtime pointers) + `rt_unify_terms`. ONE open design fork (why it awaits Lon): bb_unify is `(void)`/pBB-free, so it must get its operand nodes via either (a) a `pBB` arg (grows the 13-site pBB debt) or **(b) a `g_emit` node-ptr sidecar (the bb_arith mechanism) — RECOMMENDED**. Gate-safe: m2/m3 route compound-unify through the interpreter so 111/111 byte-identity cannot move; rung03/05/06 are EXCISED so zero regression surface. Payoff m4 **75→78**. 3-touch edit (emit_bb `bb_prepare` IR_UNIFY arm promotes 2 node ptrs · bb_unify general arm swaps `u_build`→`emit_build_compound_term`, drops the STRUCT half of `u_deferred` · scrip.c admits `IR_STRUCT`). The earlier "bake the node ptr as `movabs`" idea is STRUCK (forbidden + vacuous — mode-3 uses the interpreter).
