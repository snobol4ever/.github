...
**NOTE — corpus mismatch, unresolved.** This session's own handoff notes claimed `PASS=52/136 (was 33)`; this environment's rung suite total is `115`, not `136`, and the pre-patch floor measured `12`, not `33`. Most likely explanation: the `snobol4ever/corpus` clone here is missing additional `rung11`/`rung14`–`rung40`-range `.pl`+`.expected` test files that existed in the prior session's own sandbox but were not included in the uploaded fix bundle (which contained only the 3 changed `.c` files + a patch, no corpus additions). Not verified against the actual corpus repo's remote history — worth checking `git log` on `snobol4ever/corpus` for anything unpushed from that session, if reachable.

**NEXT:** (a) read `scrip.c`'s four admission-recipe sites, retry the `write/1` → `IR_CALL_BUILTIN_PROLOG` fix with that wiring included, to close the 5-rung parity gap; (b) track down the missing corpus rung test files (11/14–40 range) so the suite total matches what this session's fixes were actually built against; (c) multi-clause user-predicate dispatch (PL-BB-2) remains the standing larger NEXT item — unaffected by this session's work, still open.
