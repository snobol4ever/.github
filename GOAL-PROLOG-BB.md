...
- [x] **A2 — rung22 writeq/write_canonical (+3 m3, partial) LANDED 84ae655:** `IR_DET_WRITE` extended with write-mode ival (0=write, 1=writeq, 2=write_canonical); `bb_det_write.cpp` dispatches `rt_pl_writeq_cell`/`rt_pl_write_canonical_cell` already in `unification.c`. Admission: LOGICVAR/ATOM/STRUCT args only; IR_ARITH (`write_canonical(1+2)`) excluded (m4 IR-pointer-embedding issue, same as rung17). Remaining: wc_ops_as_functor (+1 m3, needs m4-safe ARITH→term path).
- [x] **A3 — rung24 string_ops (+6 m3 +6 m4) LANDED c718f58:** `atom_string/2`, `number_string/2`, `string_length/2`, `string_concat/3`, `string_upper/2`, `string_lower/2` — extend `rt_pl_atom_op_cell` in `unification.c` with these six names. Four scrip.c sites updated (is_atom2 extended, whitelist, count_synth, build_goal). string_concat arity-3 path mirrors atom_concat. m3: 71→77, m4: 56→62.
- [x] **A4 — rung25 term_string/term_to_atom (+3 m3) LANDED c718f58 (m3 only):** `IR_DET_TERM_STRING`; `rt_pl_term_string_cell` captures `pl_writeq` via `dup2`/pipe (works across m3 in-process and m4 separate-process); `bb_det_term_string.cpp`; Makefile/bb_templates/emit_bb/emit_core/scrip_ir wired; four scrip.c sites (IR_ARITH via pl_gz_arith_to_struct for `term_to_atom(1+2,A)`). m3: 77→80 (+3). m4: 0/3 — struct-literal args embed IR_STRUCT pointer in `.quad` (same root cause as 17 excised m4 cases; not a regression).
- [ ] **A5 — rung26 copy_term/concat_atom (+5 m3):** `IR_DET_COPY_TERM` added (IR.h, scrip_ir.c, bb_det_copy_term.cpp, emit_bb.c, emit_core.c, bb_templates.h, Makefile, rt_pl_copy_term_cell with deep var-map copy). `atomic_list_concat`/`concat_atom` arms added to `rt_pl_atom_op_cell`. scrip.c sites 1+2 done. **OPEN: scrip.c sites 3+4 (count_synth + build_goal arms for atomic_list_concat arity-2/3 and copy_term).** Also: `string_to_atom` already passing (A3). Next session: finish sites 3+4, build, test rung26 5/5 m3.
