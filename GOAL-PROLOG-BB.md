...
## ▶ STATE (2026-06-07-S46)

Watermark: SCRIP `7ca12b0` (battery green). **PL-GZ-0..4, 5a–5c, 6, 6b, 7a, 7b, 8, 8b, 9a, 9b LANDED** — details: git history + handoff docs.
Gates: GATE-1 m2 5/5 HARD · m3 5/5 · m4 5/5. GATE-3 m2 **114/115** (rung23_arith_ext_power: `27^3` returns `27.0` not `27` — int-power float promotion bug, pre-existing) · m3 **26**/90-FAIL (ratchet floor = 26) · m4 **51**/55-FAIL (ratchet floor = 51).
...
- [ ] **PL-GZ-9** — corpus reconquest: all 115 rungs onto the new path, m3/m4 verdicts byte-identical. Strategy: admission-gate expansion in `pl_flat_goal_is_simple` / `pl_gz_rule_body_goal_ok` / `pl_gz_build_goal` / `pl_gz_admit` for each IR shape encountered; `gz_fill_goal` IR_BUILTIN pass-through fix (`op_sval`/`op_ival` not propagated → rung16/19/40 next). findall = drive the NEW boxes (no meta rail); catch/throw = PT-3 CP-truncate + ball-copy LAW re-landed; aggregate/nb likewise; dynamic DB = **B-full** (runtime assert = lower + MEDIUM_BINARY emit into the RX slab; m3 ≡ m4 by construction). Current m3: **26**/90-FAIL (26 = 2026-06-08 baseline after M34-4). Ratchet: never regress.
**STRUCT-ARG DIAGNOSIS (2026-06-08, S47):** rung05 (`member/2`, `[H|T]` patterns) — THREE admission-gate changes needed and implemented in scrip.c but REVERTED due to runtime semantic failure: (1) `pl_gz_rule_clause` line 537 — remove `IR_STRUCT` from blanket node-type rejection; (2) `pl_gz_rule_clause` lines 558–561 — allow `IR_STRUCT` as head slot RHS (for `member(X,[H|T])` head patterns); (3) `pl_gz_call_args_ok` line 412 — allow `IR_STRUCT` call args (they get synthesized into `IR_CELL_UNIFY` nodes at `gz_fill_clause`); (4) `pl_gz_admit` lines 1020–1027 — extend `parent_ok` check to also accept structs that are call args inside `bb_goal_state_t` (not just UNIFY children). After all four changes: admission succeeds, `rt_pl_unify_struct_gz` fires for head unification — but **backtracking loops infinitely** (prints `a` then `_G0` forever). Root cause: when `member(X,[a,b,c])` backtracks to clause 2 (`member(X,[_|T]) :- member(X,T)`), `T`'s binding (`[b,c]`) is not propagated into the recursive call's struct arg synthesis — `gz_fill_clause` synthesizes a fresh `IR_CELL_UNIFY` at emit time using the STATIC IR_STRUCT pointer from the original clause, not the RUNTIME-bound tail. Fix direction: struct call args need their tail variable to be read from the caller's frame cell (like `IR_LOGICVAR` args), not from a static `IR_STRUCT` pointer. The synthesis in `gz_fill_clause` lines 656–664 must generate a runtime-dynamic unification that reads the bound tail from the choice frame. This is a LOWER/gz_build rework, not a simple gate change.
