...
Watermark: SCRIP `dd6b5b7` (battery green). **PL-GZ-0..4, 5a–5c, 6, 6b, 7a, 7b, 8, 8b, 9a, 9b LANDED** — details: git history + handoff docs.
Gates: GATE-1 m2 5/5 HARD · m3 4/5 · m4 4/5. GATE-3 m2 **114/115** (rung23_arith_ext_power: `27^3` returns `27.0` not `27` — int-power float promotion bug) · m3 **23**/92-FAIL · m4 **45**/60-FAIL.
**M34-1 LANDED (this session):** Deleted `g_resolve_env = GC_MALLOC(...)` from m3 Prolog driver block in `scrip.c`. Honest m3 baseline: 15 (was 29; 14 rungs were silently env-coupled). Gate: zero live assignments in m3 block confirmed.
**M34-2 PARTIAL (this session):** `rt_is_f` → `rt_is_cell` in `IR_interp.c` + `bb_common.h`. BINARY + TEXT arms of `bb_is_cmp.cpp` updated to marshal frame-cell pointers (`icm_bin_is_cell` helper, `icm_lea_disp` disp8/disp32 encoder, `icm_arg_load_lit` for TEXT). `bb_io.cpp` BINARY + TEXT LOGICVAR write arms updated to `rt_pl_write_cell` via frame-cell load. `pl_write` TERM_FLOAT fixed to full round-trip precision. All `rhs->α/β` accesses updated to `ir_pair_arg(rhs,0/1)` for post-IRD operand compatibility. rung29 (float constants/conversion/math/parts) and rung23 (power/max_min/bitwise) now m3≡m4 PASS. m3: 15→23 (+8). m4 dropped 91→45 because old `rt_is@PLT`/`rt_is_lint@PLT` TEXT arms now reachable for integer-arith shapes but still read `g_resolve_env` (same disease; next session fix). **NEXT OPENER — M34-2 continuation:** `rt_is` and `rt_is_lint` also read `g_resolve_env` — replace with frame-cell equivalents matching the `rt_is_cell` pattern. Then restore m4 to ≥91 and proceed to M34-3.
...
- [ ] **M34-2 — REPLACE `rt_is_f` with `rt_is_cell` (CONTINUATION).** `rt_is_f` fully replaced; rung29 + rung23 floaty family pass m3≡m4. REMAINING: `rt_is(dst_slot,op,lk,li,rk,ri)` and `rt_is_lint(lval,op,lk,li,rk,ri)` still read `g_resolve_env[dst_slot]` / `g_resolve_env[li]` / `g_resolve_env[ri]`. Replace both with frame-cell equivalents: `rt_is_cell2(void*dst,const char*op,int lk,void*la,int rk,void*ra)` (integer-only, no xmm args) and update `bb_is_cmp.cpp` TEXT arms that call `rt_is@PLT` / `rt_is_lint@PLT` to use `ir_pair_arg(rhs,0/1)` for operand access + pass `[r12+GZ_CELL_OFF(slot)]` for LOGICVAR args. Restore m4 to ≥91 before proceeding. Completion: all `rt_is*` variants env-free; `grep -rn 'g_resolve_env' src/interp/IR_interp.c | grep rt_is` == 0 (already); `grep -rn 'g_resolve_env' src/runtime/ | grep rt_is` == 0.
