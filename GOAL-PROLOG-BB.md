...
**<= LIVE CURSOR (2026-07-24, s140): PERF TRACK — PROFILE-FIRST REFRAME + LEAF HOT-PATH HOISTS (gated green, semantics-neutral). Continued s139's nrev/qsort perf push, but PROFILED before touching the queued PL-SPEED-5 rung — and the profile CHANGED THE PLAN. GDB steady-state sampling + direct instrumentation of nrev (failure-driven x-loop, `/tmp/abwork/nrevfd.pl`) shows the ~33x-slower deficit is DOMINATED BY per-append WORK, not the nondet choice-point tax the rung assumed: hottest symbolized frames are `plw_cell_deref`/`_slow` (#1), `dop_unify_lst` (list head-unify), and cons-cell ALLOCATION (`rt_gcheap_carve`/`rt_plj_alloc`/`plw_mkc_kids` — nrev allocates ~496 conses PER ITERATION, alloc-type 209, O(n^2)); `rt_pl_dop_trail_mark` (the CP/trail machinery) is a MINOR cost. ⚠ RED HERRING DISPROVEN: `rt_proc_hash_lookup` looked hot in a cold-start sample, but instrumenting the strcmp path proved it runs <4096 times TOTAL for nrev x2000 — the pointer cache (`g_proc_idx_key`, rt.c:1484) already works; proc-name resolution is NOT the steady-state cost. LANDED (3 hoists, all zero-behavior-change, targeting real `-O0` call overhead on the hottest paths): (1) `plw_poison_trap` debug flag → file-scope `g_plw_poison` + `__attribute__((constructor)) plw_poison_init`; `plw_cell_deref` hot path now reads the cached int instead of calling the fn every deref. (2) `plw_entry`/`plw_unbound_tag`/`plw_cell_deref` → `static inline __attribute__((always_inline))` (forces inlining at `-O0`, where plain `inline` does nothing). (3) `rt_alloc_hist_on` env-flag → cached `g_ah_on` read in `rt_gcheap_alloc` hot path + `__attribute__((constructor)) rt_alloc_hist_init`; both debug flags still FUNCTION when set (verified: SCRIP_ALLOC_HIST=1 still emits [AH] lines). Files: `src/runtime/by_name_dispatch.c` (+9-6), `src/runtime/rt/gc_heap.c` (+1, +1 hot-path swap). MEASURED (best-of-N, `-O0`, gprolog 1.4.5 oracle): nrev x5000 1576ms→1485ms = ~6% on the worst cell (scrip/gp 33.53→33.00). HONEST: modest, near the `-O0` ~2-3% noise floor — leaf hoists remove real overhead but the ~33x gap is STRUCTURAL (allocation volume + genuine unify/deref work), not micro-optimizable at the leaf. GATES: rung suite 164/164 x3 (interp/run/compile) after EACH rebuild; cross-lang smokes green (hello-matrix 6/6, icon 14/14 x2, all-langs-compile 6/6); the 1 Raku "division float" smoke FAIL is pre-existing + independent (float path, untouched by these Prolog-cell/alloc-flag edits). WATERMARK: SCRIP `<local — push pending credential; 3-hoist diff uncommitted>` / .github `<local — this cursor>` / corpus `<unchanged this session>`. NEXT (where the real win is, per the profile): attack the STRUCTURAL cost, in payoff order — (A) cons-cell allocation volume/overhead (the alloc fast path `rt_gcheap_alloc` still does per-call arena/atexit/stress checks; the ~496-conses-per-nrev O(n^2) is inherent but the per-alloc constant is fat), (B) the original PL-SPEED-5 first-arg-indexing / DET-delivery rung (choice-point avoidance) — still valid, just NOT the dominant cost the s139 cursor implied. ⚠ NEW BANKED (found this session): **NO LAST-CALL OPTIMIZATION** — a plain `loop(N):-...,loop(N1)` deep recursion SEGFAULTS between N=50 and N=100 (SCRIP grows the machine stack per tail call; failure-driven loops are fine). Separate significant issue — correctness/robustness, not just speed; its own rung. CONTAINER OPS: gdb + gprolog were NOT installed (apt-get both; gprolog 1.4.5 matches oracle); background clones/builds die when their subshell exits (lost corpus clone once, re-cloned foreground; `setsid` detach survived for the compiler build). BANKED (carried from s139, none resolved here): (a) NESTED-\\-> BINDING LEAK (MONITOR-HUNT). (b) retractall/1 existence_errors (needs retract/1 generator fix). (c) compiled-path IR_CALL_PROC_STAGED silent-fail on undefined user predicates.**
