...
Watermark: SCRIP `3dfdc7c` (battery green on merged main). **PL-GZ-0..4, 5a–5c, 6, 6b, 7a, 7b, 8, 8b LANDED** — details: git history of this file + handoff docs.
Gates: GATE-1 m2 5/5 HARD · m3 4/0/1-EXC (`recursion`) · m4 5/5. GATE-3 m2 **115/115 HARD** · m3 **21**/0/94-EXC · m4 105/0/10-EXC. gz2..7+gz8+gz8b gates PASS, corrupt-proven. Coupling ceilings: choice 19 · goal 10 · others 0 · rung05 .s 39; new-path boxes (incl. CELL_ITE, DET_IS, DET_CMP) emit ZERO control calls.
...
**PL-GZ-8 LANDED (ad6372f)**: is/2 + LIT_I/LIT_I arith-cmp const-fold on GZ substrate. IR_DET_IS (gz_arith_const_eval + rt_pl_is_cell_int) + IR_DET_CMP (const-fold jmp). m3 +1 rung04. gzarith_cmp/is/ite GZ-asserted in test_gate_pl_gz7.
**PL-GZ-8b LANDED (d1ac369)**: LOGICVAR operands for DET_CMP and IS with Y-op-C rhs. New runtime helpers rt_pl_arith_cmp_cell_val (cell+ival ABI, no IR_t* at runtime) and rt_pl_is_cell_arith (Y op C shape). Admission relaxed: CMP accepts LIT_I|LOGICVAR on both sides; IS rhs accepts LOGICVAR or ARITH(LOGICVAR, LIT_I). Bug fixed: test32 had no TEXT encoder → stale ZF → wrong branch; replaced with x86("test","eax","eax"). Gate: gzvarith_cmp (reentry) + gzvarith_is (Y is X-3) GZ-asserted. `recursion` smoke still EXCISED m3 (requires full multi-clause + CUT resolution, beyond GZ-8b scope).
**lower_prolog.c STANDALONE (3dfdc7c)**: fully self-contained — own `plcx_t`, own static helpers (`pl_nalloc`, `pl_set_succ_fail`, `pl_ret`, `pl_emit_leaf`, `pl_lower_unhandled`, `pl_tm`, `pl_tm_g`, `pl_wire_seq`, `pl_wire_alt`), `pl_lower_goal` as internal recursive entry. Zero calls to shared `lower.c` functions at runtime. Three public symbols: `lower_goal(lcx_t)` shim for `lower.c` dispatch, `lower2_goal_entry`, `lower2_clause_body_entry`. All gates hold.
