...
## ▶ STATE (2026-06-15 — m3≡m4 @ 114; PL-GZ retract_all (B3) LANDED; 7 of 8 dynamic reds closed; SCRIP `f7f37db`)

**THIS SESSION (Claude) — landed `rung14_retract_retract_all` (callee-body retract) + the empty-clause sentinel. m3/m4 113→114 byte-identical (+1). Then RE-DIAGNOSED R1 (`retract_mixed`); the prior "integer-materialization" theory is DISPROVEN.** GATE-1 m3/m4 5/5; GATE-3 m3 **114** / m4 **114**, parity intact (re-verified post-rebase + clean rebuild); NO-NEW-GLOBAL ratchet 15/15 (no new global, no new box, no new IR kind); cross-lang unaffected (change is `pl_gz_*` Prolog-only; Icon m3 spot-verified). Remaining **1** m3≡m4 fail: `rung14_retract_retract_mixed` (R1, FENCED both modes — see corrected diagnosis below). All 5 abolish + the other 4 retract green. Full session writeup: HANDOFF-2026-06-15-CLAUDE-PROLOG-BB-RETRACT-ALL-B3-LANDED-R1-REDIAGNOSED.md.
- **THE DYNITER RAIL (how a dynamic pred becomes a Byrd box) — unchanged from B2.** Detection → suppression → dyniter callee → population → box → runtime. (1) **Marking** (`prolog_lower.c` `pld_mark_scan`, top of `prolog_lower` before the directive fold): dynamic iff target of `retract`/`retractall`/`abolish` (anywhere), `:- dynamic(F/A)`, or **runtime** `assertz`/`asserta` (RULE body, gated `mark_assertz = (cl->head != NULL)`); **directive** `:- assertz` alone does NOT mark (stays static-folded — load-bearing, getting it wrong regressed rung13). Marks: `g_stage2.pl_dyn_{name,arity,n}` (compile-time const, `g_pl_nl_arith` tier; extern `pl_dyn_is_marked`). (2) **Suppression + dyniter callee** (`lower_prolog.c` `lower_pl_register_all_preds`): marked pred SKIPS static lowering; `lower_pl_dyniter_graph` builds a 1-node graph, entry `IR_CELL_DYNITER` (`pl_gz_dyniter_state_t{functor_atom, arity, cursor_slot=arity, mark_slot=arity+1}`, `nslots=arity+2`). (3) **Inline declines** (`scrip.c`): `pl_gz_fact_inline`/`pl_gz_choice_inline` return early for marked preds; `pl_gz_rule_inline_check` + `pl_gz_rule_body_goal_ok` accept a DYNITER-entry callee; `pl_gz_callee_get_dyniter` (nclauses=1, nlocals=2). (4) **Population** (`prolog_lower.c:771-787`): directive-`assertz` for MARKED preds captured (`pld_seed[]`) and PREPENDED to `main`'s clause body → run as the first goals (`IR_DET_ASSERTZ`), seeding the store at startup. (5) **The box** (`bb_cell_dyniter.cpp`, NONDET four-port): α = `rt_trail_mark`→mark cell, `rt_pl_dyn_iter_begin`→cursor cell, `rt_pl_dyn_iter_step`; β = `rt_trail_unwind(mark)` THEN `rt_pl_dyn_iter_step` (the β unwind is load-bearing). (6) **Runtime** (`unification.c`, Term-only, rides `g_pl_dyn_pred_table`): `rt_pl_dyn_assertz_cell`, `rt_pl_dyn_iter_begin`→heap cursor in a frame cell, `rt_pl_dyn_iter_step`. (7) **`IR_DET_ASSERTZ`** + `bb_det_assertz.cpp`; `IR_DET_RETRACT`/`IR_DET_ABOLISH` operate on the populated store.
- **B3 LANDED (`f7f37db`, +1 both modes). The handoff's 3 retract sites were necessary but NOT the wall.** `retract(item(_))` in the body of a CALLED pred (`retract_loop :- retract(item(_)), retract_loop`). Built the 3 sites (admit/whitelist/codegen for `retract`/1 in `pl_gz_rule_body_goal_ok` / `pl_gz_rule_clause` / `pl_gz_callee_body_node`, + synth-count in `pl_gz_{nsynth_chain,clause_nsynth}`, integer fence preserved in the admission arm) — but the program was STILL fenced. **The real blocker was a pre-existing guard in the SHARED multi-clause callee-frame path:** `pl_gz_rule_callee_body` ended `if (!head && ce->nclauses > 1) return 0;`. `retract_loop`'s 2nd clause is the empty-body base fact `retract_loop.` (arity 0 → `head==NULL`) → rejected. Never fired before because all-fact preds take the inline path; only a RULE+empty-FACT mix that is CALLED reaches here. **FIX:** synthesize ONE `IR_SUCCEED` sentinel (`bb_succeed` = `α: jmp γ` / `def β: jmp ω`) so the empty clause has a real entry node — satisfies the build guard AND `gz_emit_callee`'s clause-advance label indexing. (#5 touches shared codegen → gated against the full suite both modes + GATE-1 + no-new-global + Icon m3 before commit; no regression.)
- **⚠ REMAINING 1 — R1 `retract_mixed` (FENCED, not fixed). RE-DIAGNOSED this session; prior theory DISPROVEN.** Reproduced via `/tmp/tret.pl` (`:- assertz(fact(1/2/3)). main :- retract(fact(2)), fact(X), write(X), nl, fail. main.`), chased by static asm diff THEN runtime trace. (a) **The emitted code is byte-identical** — the `fact(2)` retract-head build (`CELL_UNIFY`→`rt_node_to_term`→`rt_compound_build_n`→`rt_unify_terms`) matches the working seed-`assertz` build modulo the frame slot. So "integer operand not materialized in TEXT medium" is WRONG. (b) **The real divergence is the STORE-POPULATION PATH differing between m3 and m4.** Runtime trace: **m4 runs the seed `assertz`es at runtime (3 `rt_pl_dyn_assertz_cell` calls, building `[1,2,3]`); m3 calls `rt_pl_dyn_assertz_cell` ZERO times yet its store is still populated + enumerable.** After the SAME `retract(fact(2))`: m3 store = `[1,3]` (cursor non-nil → prints 1,3 ✓); m4 store = EMPTY (cursor nil ✗). The retract C code is the same `.so`; the inputs differ. (c) **NEXT (cheapest first):** why does m3 `--run` NOT execute the injected seed goals (`prolog_lower.c:771-787`) while m4 does? `--dump-ir` is identical for both, so the divergence is at EMIT or RUNTIME-INIT, not lowering — compare what each mode's emitted entry actually calls at startup. Then gdb the m4 binary: break `rt_pl_dyn_retract_cell` return, inspect `row->head`/`prev`/`c` for the fact(2) middle-removal (the m4 assertz trace shows correct `[1,2,3]` pre-retract, so corruption is at/after retract). Atom args work in BOTH modes — integer-only + m4-only + needs the runtime-assertz seed path. **MITIGATION (committed):** integer-literal retract HEADS fenced in BOTH the admission arm (`pl_gz_rule_body_goal_ok` ~482) and the build arm (`pl_gz_build_goal` ~1736): `if h0 is IR_STRUCT with any IR_LIT_I operand return 0`. Delete BOTH fences after the fix → +1 both modes → 115/115.
