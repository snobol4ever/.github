...
- [ ] **PL-AREAS-1 — area infra + choice-point (H,TR,E)-mark save/restore IN MEMORY (globals, parallel to the existing trail).** Define the three area structs (`{base, top, limit}`) + bump/reset/overflow-grow primitives in a new `pl_area.h`. Extend the choice box to save (H,TR,E) marks at α and restore at β/retry — but with the areas as MEMORY globals (no registers yet). Prove the 3-mark save/restore discipline is behavior-NEUTRAL (floor 115/115, bench 22, `.s` may change but observable output identical). This de-risks the discipline without the register wiring.
