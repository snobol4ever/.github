...
- **THE DYNITER RAIL (how a dynamic pred becomes a Byrd box).** Detection → suppression → dyniter callee → population → box → runtime. (1) **Marking** (`prolog_lower.c` `pld_mark_scan`, before the directive fold): dynamic iff target of `retract`/`retractall`/`abolish`, `:- dynamic(F/A)`, or **runtime** `assertz`/`asserta` (RULE body, gated `mark_assertz=(cl->head!=NULL)`); **directive `:- assertz` alone does NOT mark** (load-bearing — marking it regressed rung13). Marks in `g_stage2.pl_dyn_*` (extern `pl_dyn_is_marked`). (2) **Suppression** (`lower_prolog.c` `lower_pl_register_all_preds`): marked pred skips static lowering; `lower_pl_dyniter_graph` builds a 1-node `IR_CELL_DYNITER` graph (`pl_gz_dyniter_state_t{functor_atom,arity,cursor_slot=arity,mark_slot=arity+1}`). (3) **Inline declines** (`scrip.c`): `pl_gz_{fact,choice}_inline` return early for marked preds; `pl_gz_callee_get_dyniter` (nclauses=1, nlocals=2). (4) **Population** (`prolog_lower.c:771-787`): directive-`assertz` for marked preds captured (`pld_seed[]`) + PREPENDED to `main` as `IR_DET_ASSERTZ` goals. (5) **The box** (`bb_cell_dyniter.cpp`, NONDET four-port): α=`rt_trail_mark`+`rt_pl_dyn_iter_begin`+step; β=`rt_trail_unwind` THEN step (β unwind load-bearing). (6) **Runtime** (`unification.c`, Term-only, rides `g_pl_dyn_pred_table`): `rt_pl_dyn_assertz_cell`, `rt_pl_dyn_iter_begin`→cursor in frame cell, `rt_pl_dyn_iter_step`. (7) `IR_DET_ASSERTZ`+`bb_det_assertz.cpp`; `IR_DET_RETRACT`/`IR_DET_ABOLISH` operate on the populated store.
...
- **⚠ REMAINING 1 — R1 `retract_mixed` (FENCED, not fixed). RE-DIAGNOSED; prior "integer-materialization" theory DISPROVEN.** Repro `/tmp/tret.pl` (`:- assertz(fact(1/2/3)). main :- retract(fact(2)), fact(X), write(X), nl, fail. main.`). The `fact(2)` term build is byte-identical m3/m4. Real divergence: **m4 runs the seed `assertz`es at runtime; m3 calls `rt_pl_dyn_assertz_cell` ZERO times yet its store is populated.** After the same retract m3 store=`[1,3]` ✓ / m4 store=EMPTY ✗ (same `.so`, different inputs). NEXT (cheapest first): IR is identical both modes → divergence is EMIT/runtime-init, not lowering; compare what each mode's emitted entry calls at startup, then gdb m4 at `rt_pl_dyn_retract_cell` return. Atom args work both modes (integer-only + m4-only). MITIGATION (committed): integer-literal retract heads fenced in `pl_gz_rule_body_goal_ok` (~482) + `pl_gz_build_goal` (~1736); delete both fences after fix → 115/115. Full detail: HANDOFF-2026-06-15-…-R1-REDIAGNOSED.md.
...
m3/m4 **114**/115 (parity). Ratchet: never regress (floor 114 both modes). The 1 remaining: `rung14_retract_retract_mixed` (R1, FENCED both modes — un-fence after the m4 store-population fix; see STATE).
