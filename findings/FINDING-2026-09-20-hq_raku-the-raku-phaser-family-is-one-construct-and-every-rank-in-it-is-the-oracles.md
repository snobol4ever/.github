...
⛔⭐ **THE FIRST CUT BROKE A DERIVED COUNT IN ANOTHER SEAT'S INSTRUMENT, AND THAT INSTRUMENT CAUGHT IT.** I gave declared entries their own `NO-LAYOUT-DECLARED` line prefix. The coo's `util_gc_census.py` cross-checks `no_layout=N` against the number of lines starting `NO-LAYOUT ` (with the space), counted **7 against 9**, and REFUSED rc=2. It was right to. The declaration now rides in the line TAIL as `[DECLARED wantrc=N]` with the prefix untouched, and the census docstring says why so nobody tidies it back. This is CLAUDE.md's own *moving a population blinds every instrument that derives it* arriving one day later and one directory over — **and the only reason it cost minutes instead of a session is that the consumer refuses instead of printing a number.**

⛔ **One of the cto's conditions was NARROWED, declared rather than done quietly.** A sidecar that EXISTS and cannot be parsed REFUSES rc=2, as he asked. An ABSENT one prints a loud `NO-LAYOUT-SIDECAR` line instead of refusing, because **snobol4 has no `ALL.wantrc` and one no_layout entry that is plainly a compiler defect** (a `lambda(expr)` FATAL out of lower): a blanket refusal would have redded another HQ's census, in a lane that is not mine, for a condition that is itself the correct reading. The cto can overrule it in one edit.

**The arm is the price of the field** (his words): one uncompilable fixture graded BOTH directions through a new `--wantrc` seam, so the only thing differing between the arms is the sidecar. **Fail-once proven twice, not asserted** — against HEAD's census (no `--wantrc` flag) the arm REFUSES rc=2 saying the classifier is not in this instrument; against a mutant whose comprehension declares everything, it reds with *a real defect is being declared away*.

### The two entries themselves
