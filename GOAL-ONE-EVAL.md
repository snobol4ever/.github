...
- [ ] **OE-9** — Make `sm_lower` language-aware.
  ...
  Gate: `make scrip` clean; `--sm-run` on a single-lang SNO file still passes.
...
- [ ] **OE-10** — Implement `SM_BB_PUMP` handler in `sm_interp.c`.
  ...
  Gate: `make scrip` clean; at least one Icon `--sm-run` test produces correct output.
