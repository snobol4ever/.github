...
4. **The dynamic face generalises beyond SPAN.** s170's item 3 already flags that `ARB`/`BAL` are refused by `leaf_frame_candidate()` and have **no cure on either arm**; §3 shows the coordinate need not be large to be fatal, so an `ARB`/`BAL` ALT arm under a long-running ARBNO is the next witness worth minting and it has no killswitch behind it.
