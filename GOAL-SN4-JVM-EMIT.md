...
- [x] **REFNAME fix** (sess 2026-05-16) — SnoPat.java REFNAME case was treating `*VarName` deferred patterns as string literals. Fixed: now checks if variable holds a SnoPat object and dispatches to it. Enables `*Parse` (complex recursive pattern) to work. Smoke 13/13 PASS.

- [ ] **Remaining gaps for beauty "Parse Error"** — (1) `call_returning()` stub in SnoRt.j: needed for `*Shift(...)`, `*Reduce(...)` deferred fn calls in pattern position (USERCALL_ARGS case in SnoPat). Requires Java reflection to dispatch to `Prog.sno_fn_*()`. (2) `call_external()` stub: pattern capture fn callbacks. (3) NullPointerException on empty INPUT. Next: implement `call_returning()` via reflection.
