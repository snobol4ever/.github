...
- ✅ **SECOND LANDING, SAME SESSION — `rtcc-veneer-drops-the-r9-gva-reload-when-the-gva-table-is-empty` (hq_B's design, ruled a row by ceo, mine after the cut) at SCRIP `5a49419b`:** the RTCC veneer's `r9` reload is dropped per COMPILE when `gva_count()==0` (6 lines in `x86_asm.h`). hello.pl 7 → 0, probe_loose 4 → 0, hello.sno 1 → 0, each losing exactly its reload lines; every program WITH globals byte-identical (arith_loop 17, string_pattern 24, five Icon programs); Icon all-rungs identical failure set pre/post on this tree (`PASS=263 FAIL=6 BADEXIT=1`); make test IN FLIGHT at Lon's wrap-up order (21:1x, box clock): the row's computed `done` (DONE-WHEN = reload counts + gates + Icon watermark + SNOBOL4 board) was launched detached on the final rebased tree at 21:14Z and writes its verdict to QUEUE.tsv and to hq_P's scratch done_r9.log; every fast arm on that tree passed before the push (hello 0, probe 0, arith_loop 17, ipxref 363, quad gate PASS, ladder 2/2). Pushed on Lon's 'wrap it up now' under ceo's empty-intersection landing rule; if the detached done reads red, the next session reads done_r9.log first. Not landed: the consumer-less `[rip+sym]` encoders and the Icon-only xa_flat r12 re-homes (parked, PROLOG-ONLY). The row's minted DONE-WHEN was vacuous (trailing `|| true`) — rewritten. Receipt: `FINDING-2026-09-02-hq_P-rtcc-veneer-drops-the-dead-r9-reload-per-compile-when-the-gva-table-is-empty-hello-7-to-0-globals-byte-identical.md`.
