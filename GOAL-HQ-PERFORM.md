...
**LANDED THIS SITTING** (SCRIP `e98ab24ac`, corpus `dae88001b`, .github `1e7eec1a`, all pushed; `handoff_status.sh` rc=0):

1. ⭐ **`HOST(2,i)` / `HOST(3)` — row `snobol4-host-argv-not-staged-for-zero-param-entry`.** The row located the cause as the driver's `nparams >= 1` gates on `rt_main_args_stage`. Those gates **are** wrong, but they are **not** why the witness failed: `_HOST_` had **no selector-2 arm at all** and returned a hardcoded `INTVAL(0)` for selector 3, so nothing downstream ever read what staging staged. ⛔ **One control arm refutes the whole diagnosis — the same witness with a `DEFINE('M(X)')` entry printed `argc=0` too.** Contract **measured on both oracles** (they agree; matches the manual at `spitbol-manual-v3.7.txt:6926`): `HOST(2,i)` indexes the **whole command line** with `i=0` the binary, **FAILS past the end** (`genc.sno:37` is `:F(NOFILE)`), and `HOST(3)` is an **index**, not a count. Cured with **NO NEW GLOBAL**: `/proc/self/cmdline` streamed, plus `rt_main_args_count()` as a pure function over the staged list's own `frame_size`. `HOST(3) = cmdline entries − staged user args`, true by construction in m3 and m4 with no `--` sniffing. `genc.sno` goes from **0 bytes** to output whose first 47 bytes are **byte-identical to the csnobol4 oracle**.

2. ⭐ **SPITBOL DREAL literals — row `snobol4-dreal-literal-1-0d2-unlexed-...`, CLOSED (`done`, computed).** `EXP [eE]` → `[eEdD]`, **plus** a normalizer turning `d`/`D` into `e` **before `atof`** — the half a lexer-only fix misses, since `atof("1.0D2")` stops at the `D` and returns `1.0`: the rule alone would have made it **parse and then silently compute the wrong number**. Oracle grammar **probed, not assumed** (`sbl -bf` takes `1D2`, `1.0d2`, `1.0D+2`, `3D0`). Mirror hazard proven clean (`D2`, `DOUBLE` still identifiers). `.l` + `.lex.c` committed together and **that pairing proven**: reverting only the generated file brings the parse error straight back.

3. **`SCRIP_M4_HEADROOM` reserved before the argv pushes**, not between them and the read — latent while staging was gated, reachable by every m4 program once it wasn't. `host3=3` → `host3=1`; ⛔ it **did not crash**, it answered wrongly.

4. **Two instrument cures found in passing:** `util_build_package_suite.py:228` had a bare `csv.writer` (default terminator `\r\n`) and writes the **package** suites — my own lane — so it was a live CRLF re-infection source; and `corpus/tests/raku/ALL.csv` was an origin-inherited CRLF file **reddening `make test` for every seat**.

⛔⭐ **THE LESSON, AND IT IS THE SAME ONE TWICE: A ROOT CAUSE FOUND BY READING IS A HYPOTHESIS, NOT A MEASUREMENT.** Both batons named a real, true-sounding defect in the right file; neither had been given the one cheap experiment that could have falsified it. Same family as `CSN_NO_SEGV_HANDLER` (a correct procedure with a false explanation) and `command -v` for an oracle (an instrument answering a narrower question than intended). **Run the arm the diagnosis predicts will PASS.**

⛔⭐ **AND A DONE-WHEN CAN BE READABLE, RUNNABLE, AND GRADE THE WRONG QUESTION.** The HOST row's original DONE-WHEN demanded `HOST(3)` be a **count** over a user-args-only vector — semantics **neither oracle implements**, contradicted by the row's own GOAL prose, and under which `genc` still dead-ends at `NOFILE`. **A GREEN DONE-WHEN WOULD HAVE BEEN THE FAILURE.** Re-pinned to the measured contract on the ceo's ruling ("a DONE-WHEN that cannot say YES is the mirror of a gate that cannot say NO"), old criterion recorded on the ledger, and the new one **negative-tested both ways** — it passes on the cured binary and reds on a pre-cure baseline worktree build.

**CONTROL ARM (unchanged across both cures, same two names not merely the same counts):** SNOBOL4 master m3 PASS=1720 FAIL=2 · m4 PASS=1720 FAIL=1 SKIP=1 over 1760. Both reds **proven inherited** by building origin `cfde5756f` in a separate worktree: `simple_output_276` (pre-existing GZ#5 lowerer refusal) and `user_function_len_defer_branch_6` (**hq_B's**, bisected to `04d1b9cd2`; Lon 18:37 *"SPITBOL's answer is the ref"*).

**csnobol4 BOARD, post-oracle-swap (x64 `c0dc231`):** total=118 m3 PASS=56 FAIL=26 REJECT=35 CRASH=1, m4 identical. ⭐ **The useful cut is not the total but the split:** of 62 red names, **30 are REGEN-CANDIDATE** (live csnobol4 disagrees with the pinned `.ref` too — the pin may be stale, not SCRIP) and **32 are genuine** (live csnobol4 agrees with the `.ref`). If most of the thirty are stale pins the lane's real red count is far below what the board shows.

**⛔ FOR THE NEXT SEAT — testpgms is EIGHT programs, not four.** `testpgms.spt` (1412 lines) contains **8** `-TITLE SPITBOL TEST PROGRAM` boundaries (1, 424, 685, 745, 863, 965, 1075, 1280); only **4** were ever split into files (27792 of 44736 bytes). #5 TREESORT4, #6 TOPOLOGICAL SORT, #7 SYMBOL TABLE GENERATOR, #8 BRIDGE DEALER are named by **zero** runners — **not failing, ABSENT**, the one state a board cannot show you. That is the measured answer to Lon's 17:57 question, and the umbrella row's own title and denominator are wrong. Second, independent: `testpgms-test2/3/4.spt` each begin with a stray `./*` split artifact that truncates the **oracle** at line 1 (ERROR 214) — removing it takes test3 18→47 lines, test4 18→29. ⛔ **So part of the "oracle crashes on test 2/3/4" story is our own vendoring, not the oracle.**

**ROWS MINTED AND ASSIGNED (every DONE-WHEN negative-tested; the 32-class one also proven able to PASS on a synthetic census, so it is not unsatisfiable):** seat07 `snobol4-csnobol4-thirty-regen-candidate-refs-stale-pin-or-real-defect` · seat08 `snobol4-csnobol4-thirtytwo-genuine-reds-censused-by-construct-class` · seat09 `snobol4-testpgms-only-four-of-eight-programs-were-ever-split-out` · seat10 `snobol4-testpgms-test1-unresolved-forward-reference-return-after-dreal-parses` (test1 now **parses**; the next blocker is codegen `unresolved forward reference` naming `RETURN`).

**NEXT:** seat11 still needs a row; the 32-name census will mint the class cures that are hq_P's own. FINDING: `FINDING-2026-09-04-hq_P-host-argv-the-located-root-cause-was-not-the-defect-host-2-was-never-implemented.md`.

### ⭐⭐⭐ LIVE CURSOR (SUPERSEDED) — 2026-09-04 late (hq_P, **MODE `FLEET-16`**, computed from `/home/resources/postoffice/MODE`). **THE STARTUP-TOUCH A/B INSTRUMENT EXISTS AND THE FOUR-SESSION CAMPAIGN HAS ITS ANSWER: THE LINK-ORDERING LEVER IS A SMALL REAL GAIN (m3 minflt `1.022x`, maxrss `1.107x`) AND THE ORDERING DOES IT, NOT THE FLAG — PLUS ASLR TURNS OUT TO BE A UNIT NOBODY NAMED**
...
- ⛔ **LON OVERRIDE, routed to ceo the same session** (`override-lon-init-digest-refresh-plus-carve-release-hypothesis`, THE LOOP step 6(b)): under ceo's finish-or-park order Lon ran `/init` in-chat on this seat. Lon wins; executed. **`/home/claude_P/CLAUDE.md` only** — not in git, backup `CLAUDE.md.bak-20260904-init`.
- ✅ **`test_gate_digest_matches_rules.sh` re-run after the edit: this root CLEAN, 0 hits.** The single `GATE FAIL(1)` of 38 roots is `/home/claude01/CLAUDE.md:217` (SEGV-HANDLER-ATTRIBUTION), **pre-existing — 1-of-38 both before and after my edit**, and not this root's to cure.
- ⭐ **What the digest had wrong, all verified by execution:** `.github` 629→**1189** entries · `RULES.md` 183→**357** lines · `FINDING-*` 395→**1004** · `GOAL-*` 100→**59** · `ARCH-*` 29→**17** · scripts 485→**642** `.sh` · `test_gate_*` 110→**193**. ⛔ **HALF THE COUNTS DRIFTED *DOWN*** (consolidation retires files), so *"a stale count can only be too low"* is not a safe reading — that assumption is now written into the digest.
- ⭐ **The structural ones:** `make test` is **~15 arms and minutes**, not the 3-arm/`~32 s` recipe the digest advertised · the board denominator is **1736 master entries** (floor 1576), not 365 · `corpus/{crosscheck,probe,demo,suites}` **ALL GONE** (conversion complete; `demos/` is plural) and `SUITES=` is retired from the runner · `src/templates/` split into `bb/ x86/ xa/`, so the flat `*.cpp` glob **and** the `x86_asm.h` path were both dead · `SCORE.md` was entirely undocumented and now has a section.
- ⛔⭐ **THE SHARPEST FIND, AND IT IS AN INSTRUMENT LESSON, NOT A DOC FIX: the digest promised the ζ flags `EXIT rc=2 WITH A REFUSAL`. THE REFUSAL WAS REMOVED.** `./scrip --zeta-storage=cell-stack` now exits **`rc=1`, `cannot open '--zeta-storage=cell-stack'`** — it falls through to the source-file path and reports a **missing file**. ⭐ **A retirement moved an error path from LOUD-AND-CORRECT to LOUD-AND-MISLEADING**, and every harness testing for `rc=2` silently changed meaning without failing. Related: `SCRIP_ZETA*` is no longer *"zero occurrences"* (**5 survive**, all telemetry/trace, no selector) — the back door is still shut, but **that grep is no longer the proof of it**.
- ⚠️ **ROUTED, NOT MINE — seat02's shared-node SIGSEGV (`snocone-carve-release-omega-asymmetry`). ANALYSIS ONLY, NOT MEASURED, NO BUILD TAKEN.** In `src/templates/x86/x86_asm.h` every omega RELEASE is keyed `site==X86H_JMP` (ZLS2 `:2026`, `bb_glue_flat_leave` `:2038`, `add rsp,_.op_wpop` `:2041`); the jcc path reaches them only through `x86_fc_jcc_omega()` `:612`, gated at `x86_jcc()` `:429` on `x86_fc_on() || _.op_wpop != 0 || x86_pl_trace_on()` — which names `op_wpop` but names **neither** `(op_zls2_ops & ZLS2_RELEASE)` **nor** the flat-glue case. **HYPOTHESIS:** a box carving via ZLS2 or flat glue with `op_wpop==0` and fc off takes the plain jcc path and never releases; `k16=(op_zls2_bytes+15)&~15` `:2022` fits seat02's fixed 16 bytes and explains why `je`/`jne` polarity was the only compiled difference. **Confirmable in one run with an EXISTING hook and no build:** `SCRIP_ZETA_OMEGA_TRACE=1` (`:2017`, fires only on the JMP path), control arm `SCRIP_PORT_EXIT_AUDIT=1` (`:2015`, fires on both). ⛔ **SHARED-NODE VERDICT SCOPE applies — this is the encoder, so any cure is graded on every frontend, not just Snocone.**
- ⭐ **PARKED FOR THE FLIP, wording agreed with hq_B (I take first authorship, hq_B co-signs):** *A MARKER THAT NAMES A CLASS MUST NAME ITS OWNER* — a knowndiff/xfail/SKIP/TODO reads to every later reader as COVERED while owning nothing; it is the gate-with-no-runner shape in a different costume, and invisible to any census that counts markers rather than resolving them. My two witnesses carry its second half: a DONE-WHEN pinning a population count, and an I26 re-cut that passed all seven languages **because mawk does not match `/^#{2,3} /`** — a gate that could not fail, in the same message that forbade one.

### ⭐⭐⭐ LIVE CURSOR (SUPERSEDED) — s285 (hq_P, **TRIO**, `MODE` file computed). **THE LADDER RUNNER GRADES ONE RUNG ALONE (`--only N`, ceo rank-0 brief) — AND `--only 6` REFUSES, BECAUSE THE RUNG-6 WITNESSES DO NOT EXIST YET; PLUS `pascal-bubble-m3` CLOSED ON A CURE THAT LANDED A DAY BEFORE ITS BATON COULD SEE IT**

**SCRIP `b8cbe837` (parent `7d0ed0f8`, rung 1; `src/` untouched this session) · corpus `607bcf92a` · pristine `-O0` (HQ-27).**

- ✅ **`test_prolog_ladder.sh --only N` IS ON ORIGIN (`b8cbe837`), scripts-only** — ceo's rank-0 brief (Lon 16:55–17:05: *"Let's get Prolog to 100%"*, *"just start climbing the rungs"*). `--to N` stays cumulative and behaviour-unchanged (control arm: `--to 1` PASS 4/4). `--only N` grades rung N alone in both modes, so the ceo can land rung-6 code while hq_C's rungs 2–5 are still red. **Refuses rather than guessing:** `--to` + `--only` together name two different populations → rc=2; non-numeric → rc=2; **a rung with no witnesses → rc=2** (*"a rung with no witness is UNMEASURED, never a pass"*). Every board and verdict line names the selector actually used, so an `--only` run can never be misread as cumulative. **Failed once, passed once, refused once:** `--only 0` 2/2 · `--only 1` 2/2 · `--only 2` RED 2/2 (hq_C's rung, not landed) · `--only 99` rc=2 · `--only 2 --to 2` rc=2 · `--only foo` rc=2 · `S4E_HOME=/nonexistent` rc=2.
- ⛔⭐ **MEASURED AND TELEGRAMMED TO ceo (`ladder-only-n-on-origin`) — THE INSTRUMENT IS READY, THE WITNESSES ARE NOT.** `--only 6` **REFUSES rc=2 today**: the master carries `ladder__rung*` witnesses for **rungs 0–5 ONLY** — 12 origins (r0 1 · r1 1 · r2 1 · r3 1 · r4 3 · r5 5). Landing rung-6 code does not turn `--only 6` green; it stays a refusal until hq_B cuts `ladder__rung06_*` into `ALL.pl`/`ALL.csv`/`ALL.ref`. **A landing gate that refuses is not a landing gate yet.**
- ✅ **Row `pascal-bubble-m3-segv-and-devnull-masks-it` DONE on its own DONE-WHEN** (`M3-CLEAN-AND-MATCHES-REF`, rc=0; correct at reps 2/5/20). The cure is **`f9a90958`** arm 2 (in-run NORMALIZE ARRIVALS, 2026-09-01, row `calling-convention-depth-tracked`), whose own message names bubble as *Shape B* with the 288 = 0x120 over-pop. ⭐ **Proven by its killswitch on today's tree, not by a bisect build:** `SCRIP_ZD_NORMALIZE_INRUN=0` → **rc=139, 3 of 3**; default → correct. Eight seats had released this row unworked with the diagnosis already complete.
- ⛔⭐ **THE PROCESS LESSON, worth more than the row:** seat11 parked it on its TRUE blocker; two later re-parks **REPLACED** that blocker instead of adding one, so when the real blocker landed the row could not self-clear and surfaced a day late describing a crash that no longer existed. **A `BLOCKED-ON` park is a claim about CAUSALITY — a re-park should ADD a blocker, never REPLACE one.** And `calling-convention-depth-tracked` is no longer in `QUEUE.tsv` at all: a park keyed on a topic that leaves the index can never self-clear by construction.
- ⚠️ **ROUTED, NOT MINE — the Pascal m3 gate is RED `rc=1` on this tree**, one entry: `program_procedure_nested_1` bombs `bb_var_frame: PAS-DISPLAY L>=4 fallback unimplemented` (SIGABRT, honest stub from `3579f7ef`; witness entered at corpus `066a680bb`). Pre-existing, no `src/` touched. Nearest row `pascal-uplevel-nested-proc-hang` (rank 4, FREE) says *hang* where the symptom is a *bomb*.
- ⭐ **MY OWN INSTRUMENT ERROR, KEPT ON THE RECORD:** I first read both Pascal gates as `rc=0` and wrote them down green — I had piped them through `tail`, so `$?` was the pager's. Unpiped, m3 is `rc=1`. The digest carries this warning for `handoff_status.sh`; it applies to **every** gate, and it hides well because the failing gate still prints its numbers while its exit code is replaced. **A verdict read through a pipe is not a verdict.**
- ✅ **SECOND ROW CLOSED, AND IT WAS ALREADY FINISHED — `snobol4-floor-cutover-to-the-one-flat-suite-board-equality-first`.** The picker served it as rank-1 FREE work; its baton's current block reads *"CLOSED BY THE NAMED LANE OWNER"* (hq_C, 2026-08-30) with all three deliverables published — exclusions classified 30/30, named-set equality proved (and an instrument defect found in the proof itself), failures and XPASS named. Only the **QUEUE column** still read FREE. I re-proved the criterion on today's tree rather than redoing a day of work, and closed the column: DONE-WHEN rc=0, `S4E_DONE_TIMEOUT=3600` per the baton's own warning that `done`'s default 900s sits beside a ~500s board. ⭐ **Same class as the bubble row above, twice in one session: a finished row whose column disagrees with its baton is re-served as fresh work, and the honest cost is a whole session of re-measurement.** The cheap defence is the one both closures used — **read the baton before re-deriving the work.**
- ⭐⭐ **ARCHITECTURE, FROM THE PASCAL RED, AND IT CONSTRAINS THE PROLOG REBUILD — `bb_var_frame.cpp:11`: `frame_display_reg(lvl)` is `1→r13 · 2→r14 · 3→r15 · NULL beyond`.** ⛔ **Those are exactly the quad registers** (`r12` TR · `r13` B · `r14` ROOT · `r15` BALL, § A.1, my gate). Pascal's display and Prolog's quad are the same four callee-saved registers. **So the obvious cure for the Pascal bomb — add `r12` as a fourth level — would put a Pascal write on Prolog's TRAIL**, and the boxes are shared. ✅ The cure is a **frame-resident display** (ζ-ACTIVATION-FRAME, indexed by level), which removes the ceiling instead of moving it 3→4. ⚠️ Not live today (the quad gate sweeps Prolog witnesses only; no Pascal graph is reachable from a Prolog one) — live the moment either changes. **Bracketed by construction:** 2/3/4 nested procedures rc=0 correct · 5/6/7 bomb rc=134, break exactly at L=4, no intermediate degradation. Routed to ceo (`pascal-display-pins-r13-r14-r15-the-prolog-quad-registers`); row `pascal-uplevel-nested-proc-hang` re-written with the bracket, the source cause, the constraint and a STEP 1 — ⚠️ its **title says *hang* where the symptom is a *bomb***, so symptom-matching will not find it, and at rank 4 it undersells being the one red in the Pascal m3 gate.
- ✅ **THIRD LANDING — `corpus_suite_harness.py list` GAINS `--lang`, AND REFUSES INSTEAD OF ACCUSING THE DATA (SCRIP `abbbe9d3`, scripts-only; reported by hq_B from the rung-6 promotion).** The subcommand **could not read ANY non-SNOBOL4 master**: `read_suite()` hardcodes SNOBOL4's `*` banner marker, so a Prolog master (`%` comments) had no line recognised as a banner, every line fell to the one-line branch, and the committed pair died with `family.ref is shorter than family.sno at seq 1107`. `cmd_run` has dispatched on `--lang` since 2026-08-29; `cmd_list` was never given the same dispatch. **No new grammar — one authority, two callers.**
- ⭐⭐ **THE EXPENSIVE HALF WAS THE MESSAGE, AND IT IS THE PART WORTH CARRYING FORWARD: it names the REF FILE as short, so a wrong-READER fault presents as a corpus-DATA fault.** hq_B had to re-check the committed baseline to prove their own edit had not broken it — a bisection they did not owe. So the no-`--lang` path now **refuses up front**, naming the reader, the suffix and the flag, *before* the grammar can produce a message that blames the data. ⛔ **It had also made a documented procedure unfollowable** — `lib_master_extract.sh`'s INTERIM PROMOTION PROTOCOL tells a promoter to prove with `list`, which for `.pl` could not be done at all: the *"a prerequisite nobody can satisfy is either a blocker or a dead letter"* shape, live. **Proved with a control arm:** `--lang prolog` rc=0 / 389 entries · no-`--lang` rc=3 refusal · SNOBOL4 control rc=0 / 1726 and **byte-identical** before/after · all six `ladder__rung06_*` origins resolve to listed entry names · `make test` rc=0. Receipt: `FINDING-2026-09-02-hq_P-the-suite-harness-list-subcommand-could-not-read-any-non-snobol4-master-and-its-error-blamed-the-ref-file.md`.
- ✅ **THE RUNG-6 GATE IS ARMED AND CORRECTLY RED.** hq_B landed the population at corpus `6936651c5`, closing the gap I telegrammed. Re-measured on my own instrument: **`--only 6` prints `witnesses=6 modes=2 graded=12 PASS=0 FAIL=12`, rc=1**, where it refused rc=2 before. Ladder origins by rung: `r0 1 · r1 1 · r2 1 · r3 1 · r4 3 · r5 5 · r6 6` = 18. ⭐ **That red is the correct reading** — every witness refuses with the ladder's own *"builtin … is not on the ladder yet"*. A gate red before its rung lands is a gate; one that refuses is not armed; one green before the work is the real alarm. ⚠️ From hq_B, not cured here: `ALL.trace` carries no rung-6 blocks, because a trace ref can only be cut from a witness that runs — that is rung 6's landing to do.
- ✅ **RUNGS 7-12 ARE ARMED TOO — hq_B landed the whole remaining population at corpus `ab53ffc6c` within the hour.** The ladder is now **29 witnesses, rungs 0-12**. Measured on my instrument at SCRIP `abbbe9d3`: `--only 7` grades 4 RED · `--only 8` grades 4 RED · `--only 11` grades 2 RED · `--only 12` grades 2 RED — all four refused rc=2 before. **Rungs 7, 8, 11 and 12 are this seat's** (§ E rows 227/228/230/231), and every one now has a real fail-once baseline.
- ⛔⭐⭐ **MY INSTRUMENT DEBT, AND hq_B WAS RIGHT TO FLAG IT RATHER THAN LET ME INHERIT IT: RUNGS 11 AND 12 HAVE A *CORRECTNESS* WITNESS ONLY.** Their verbatim warning: *"`--only 11` going green will mean 'it still computes the right answer', NOT 'LCO happened'."* ⭐ **This is the vacuous-gate class in its subtlest form — the instrument CAN fail, just never on the dimension it is named for.** A stdout `.ref` cannot carry a *resource* fact (rss flat across N) or a *shape* fact (zero β labels for an indexed predicate); both are exactly what rungs 11 and 12 claim. § E already names the real instruments and they are **mine to build before those rungs land**, the same way the quad gate was built ahead of rung 0: **rung 11** — `nreverse` on 10⁵ and `count(N)` to 10⁶ under `ulimit -s 512`, rss flat across N (`tools/bench_rusage`); **rung 12** — on `fact/2` with distinct constant first args, `--compile` emits **ZERO β labels** for the predicate, and the trace shows `Exit` not `*Exit`. ⛔ **Until those exist, a green `--only 11` must never be quoted as evidence that last-call optimization happened.**
- ⚠️ **Also outstanding for rung 6 (ceo's landing, not mine to fix):** `ALL.trace` carries no rung-6 blocks, since a trace ref can only be cut from a witness that runs and `--cut` is the one sanctioned writer; and all six rung-6 witnesses carry XFAIL in both banners and the CSV column, so promoting those markers is part of the landing.
- ⛔⭐⭐ **RUNG-6 REVIEW DISCHARGED BEFORE THE LANDING (my review obligation; ceo telegrammed `rung6-landing-criterion-is-vacuous-and-here-is-the-real-number`): THE § E RUNG-6 CRITERION IS GREEN TODAY, BEFORE THE WORK, SO IT CANNOT CERTIFY THE LANDING.** It reads `grep -c _rkfn corpus/benchmarks/prolog/*.s corpus/demos/prolog/*.s` = 0. Three measured defects on corpus `ab53ffc6c`: **(1) the glob examines almost nothing** — `benchmarks/prolog/*.s` matches **zero** files (they live one level down in `vanroy/`), and `demos/prolog/*.s` reaches 3 of 4. **(2) It is already satisfied** — `_rkfn` is 0 across **all 108** Prolog `.s`, and 0 on a freshly compiled *landed*-rung witness. It passes before, during and after, so it can never fail once. The label is not dead (1176 occurrences in `benchmarks/**`, 414 in `demos/**` for the other languages) — it is simply not the symbol Prolog's by-name path uses. **(3) It watches the wrong symbol** — § B.12 names the one P6 deletes: `rt_call_arr_bl` (`bb_call_fn.cpp:583`).
- ⭐ **THE REAL NUMBER, AND IT IS A CLEAN FAIL-ONCE TARGET:** in the two artifact trees Lon's s269 ruling keeps regenerated, `rt_call_arr_bl` is **benchmarks/prolog 0 · demos/prolog 214**. **Rung 6 should take demos/prolog 214 → 0.** The other 1236 occurrences sit in `packages/prolog` (93 files, abolished-artifact category, not regenerated) and are stale snapshots, not current output. ⛔ **And it must be measured on FRESH compilation, never on checked-in `.s`:** the six rung-6 witnesses exit `rc=2` from `--compile` today (*"builtin … is not on the ladder yet"*) and emit **no `.s` at all**, so an artifact grep would score them 0 and read as a pass while nothing compiled. **Proposed gate: fresh `--compile` of all six (rc=0 is itself today's fail-once arm) with `_rkfn`=0 AND `rt_call_arr_bl`=0, plus demos/prolog regenerated to 0.** I re-run exactly this on the landing.
- **Next for this seat (ceo's brief):** rung 6 is now the **ceo's** to climb (Lon's order) — I review the landing as perf owner (`dop_*` direct, no `_rkfn`, no by-name path). **Rungs 7–8 start the moment hq_C's rung 2 is on origin**; the van Roy two-number pin is taken at rung 8, on the cured instrument. The quad allow-list grows by routine NAME per rung, never by label.

### ⭐⭐⭐ LIVE CURSOR — s284 (hq_P, **TRIO**, `MODE` file computed). **THE QUAD GATE IS GREEN AT RUNG 0 AND IN `make test` — BY SITE AND SHAPE, SUPERSEDING s283's BORN-RED LABEL ALLOW-LIST; AND THE SLICE-CAPTURE ROW IS CLOSED ON ITS DONE-WHEN**

Prolog #1 lane, instrument lane of `ARCH-PROLOG-BYRD-BOX-TRANSLATION.md` § E (*"hq_P mints the quad gate"*). **SCRIP `9623ff55` (parent `db299d41`, rung 0; `src/` untouched) · corpus `9c6489879` · pristine `-O0`.** Receipt: `FINDING-2026-09-02-hq_P-quad-gate-lands-green-at-rung-0-by-site-and-shape-superseding-the-born-red-label-allow-list.md`. Row `prolog-quad-gate-no-r12-r15-write-outside-tr-b-root-ball` (minted this session, rank 0) DONE on its DONE-WHEN.
- ✅ `scripts/test_gate_pl_quad_regs.sh` + `scripts/pl_quad_regs_scan.py`: every r12–r15 write in emitted mode-4 TEXT attributed to its site (driver / root seed / box+port / fn) and matched against an explicit **(site, register, shape)** table; the rtx routines the program reaches (transitive `call`/`jmp`) swept the same way, TR helpers admitted **by routine name** (`TR_HELPER`, env `QUAD_HELPER_RX` — the rung-1 brief's word still holds), push/pop-bracketed scratch printed as preserved. Three canaries turn it red on EVERY run before the real run is trusted. **Measured on the rung-0 tree:** ladder=12 probe_plz=9 corpus=145 → compiled=1 (hello) refused(not-on-the-ladder-yet)=163 other=2 (a `det.pl` parse error, twice); writes=5, all five ROOT seeds of § A.1; rtx 47 defined / 3 reachable / 0 writes. `make test`: rc=0 in 713 s (m3 PASS=1679 FAIL=0 · m4 PASS=1679 FAIL=0 SKIP=0 · MISSING=0; the quad gate last, PASS).
...
- ⛔ **It supersedes s283's gate at `4253dd88`** (label allow-list, born red, opt-in `--self-test`, not in `make test`): measured RED at HEAD on the five ROOT seeds, and its only way to green was naming `main`/`main_α` — which admits ANY write under those labels. Its ASCII-only awk also missed 2 of the 3 reachable rtx routines (`rt_gen_spine_pass_γ/ω`). **Lesson: an allow-list keyed on a label is a per-site exception wearing a regex — admit by WHAT (register, shape) at WHERE (site), never by where alone.** The s283 cursor below describes the old design; read it as history.
- ✅ **`slice-capture-aliasing-breaks-beauty-selfhost` CLOSED** — cured by ceo at `caffe0d4` on 2026-08-30 and declared done then, but never closed in the picker, so it kept surfacing at rank 1. Its DONE-WHEN run verbatim on `db299d41`: rc=0 in 3 s (beauty m4 self-host byte-identical with slices ON, START witness clean, `rt_cap_slice_on` present). No code change.
- ✅ **SECOND LANDING, SAME SESSION — `rtcc-veneer-drops-the-r9-gva-reload-when-the-gva-table-is-empty` (hq_B's design, ruled a row by ceo, mine after the cut) at SCRIP `5a49419b`:** the RTCC veneer's `r9` reload is dropped per COMPILE when `gva_count()==0` (6 lines in `x86_asm.h`). hello.pl 7 → 0, probe_loose 4 → 0, hello.sno 1 → 0, each losing exactly its reload lines; every program WITH globals byte-identical (arith_loop 17, string_pattern 24, five Icon programs); Icon all-rungs identical failure set pre/post on this tree (`PASS=263 FAIL=6 BADEXIT=1`); make test the row's computed `done` (DONE-WHEN = reload counts + gates + Icon watermark FAIL<=6/BADEXIT<=1 + SNOBOL4 board GATE OK) ran detached on the final pushed tree (SCRIP 5a49419b) and EXITED 0 -- row DONE, computed, after the push (the push went first on Lon's 'wrap it up now'; the fast arms had passed on that tree before it). Not landed: the consumer-less `[rip+sym]` encoders and the Icon-only xa_flat r12 re-homes (parked, PROLOG-ONLY). The row's minted DONE-WHEN was vacuous (trailing `|| true`) — rewritten. Receipt: `FINDING-2026-09-02-hq_P-rtcc-veneer-drops-the-dead-r9-reload-per-compile-when-the-gva-table-is-empty-hello-7-to-0-globals-byte-identical.md`.
- **Next for this seat (per the MODE file):** rungs 6–8 wait behind hq_C's rungs 1–5; the van Roy pin stays parked until rung 8. Until then: enrolments as rungs land (the baton's NEXT names the recipe), and the r9-reload datum on the pin tree when a Prolog graph compiles beyond hello.

### ⭐⭐⭐ LIVE CURSOR — s283 (hq_P, **TRIO**, `MODE` file computed). **THE VAN ROY INSTRUMENT WAS FABRICATING ITS SCRIP COLUMN — CURED ON `scripts/` ONLY; THE PIN STAYS PARKED (ceo 13:40–13:50: until rung 8 lands); NO BASELINE NUMBER EXISTS OR IS QUOTED**

Prolog #1 lane, ceo standup `prolog-instruments-and-baseline-standup`. **SCRIP `4253dd88` + `48b09e04` · corpus `d0351a546` · pristine `-O0`.** Receipt: `FINDING-2026-09-02-hq_P-vanroy-loop-runs-its-body-exactly-twice-for-every-n-both-angles-divided-n-by-it-and-the-rivals-never-had-their-preludes.md`. Two defects, both measured on the pristine pin tree `f4532dea` before any number was taken: (A) angle 1/2 (`test_bench_prolog_timed.sh`, `bench_prolog_fixed_iter.sh`) never consulted the rival preludes, so the 10 self-timed kernels failed the rival correctness gate on `wall_us/1` and the whole triangulation read UNPROVEN for gnu AND swi (the 08-27 TSV was the last informative one for that reason); (B) the `between/3`+`fail` loop wrapper runs its body **exactly twice for every N** on m3 and m4 (N=1 → 2 answers, N=64 → 2, `nrev` → 0) and exits 0, so both angles divided N by two iterations' CPU and printed `fib` m3 at **3,102,442 iter/s vs gprolog 485** — and AGREED with each other to 1%. ✅ Cure: `scripts/lib_prolog_bench.sh` (`loop_check`: stdout must equal N × `.expected`, all four engines alike, else `LOOP-OUTPUT-MISMATCH(lines=seen/wanted)` and no rate; `gnu_filter` the one banner filter), both harnesses source it and consult the preludes, REFUSE rc=2 on either missing; proven both directions on `fib`/`nrev` and by the refusal arms. ⛔ hq_C's `test_gate_vanroy_prolog_acceptance.sh` CLEAN admits `fib` (2/64 answers) and `nrev` (0/65536) — routed to hq_C, their lane, `loop_check` is the one-line cure.  ✅ **THE QUAD GATE IS MINTED — `scripts/test_gate_pl_quad_regs.sh` (ceo rung-0 brief, instrument lane):** no `r12`–`r15` write reachable from a Prolog graph outside the TR/B/ROOT/BALL helpers, ARM 1 over every Prolog witness compiled fresh (35 today: probe_plz 9 + bench 23 + demos 3 + ladder 0), ARM 2 over the rtx functions reachable from them (transitive, balanced push/pop = callee-saved-clean, unreached = INFO). **Born red on `f4532dea`: 5303 writes / 5303 violations** (4606 `lea r12,[rip+g_pl_*]` sink scratch · 417 `mov r12` · 245 `pop r12` dcα continuations · 35 `xor r14d` m4 prologues; `r13`/`r15` zero), rtx 47 functions / 2 reachable / 0 writes; `QUAD_HELPER_RX='.'` → PASS over the same 5303 (can say YES); `--self-test` proves NO and YES on both arms on fixtures. `QUAD_HELPER_RX` is the one allow-list — hq_C names the helpers there. ⛔ Third harness defect, same sitting: swipl runs through `timeout`'s SIGTERM (648 s on `vanroy/queens.pl` under `timeout 60`); every angle-harness `timeout` now carries `-k 5`. **Triangulation on the cured instrument:** run 1 exposed Defect D (angle-2 drivers carried pre-bracket bodies: 8 kernels DISAGREE 1.2x–2.75x), run 2 on refreshed drivers (corpus `d0351a546`, TSV `triangulation-20260902T190902Z.tsv`) reads cal/derive/divide10/ops8 AGREE on both rivals (bucket MEASURED=4), four more AGREE on one rival, log10/tak DISAGREE on both at 0.83–0.90x (calibration, declared with a discharge condition), SCRIP cells UNPROVEN everywhere by the loop check; Defect E, the coverage gate counted 23 not 21 and had been red since 09-01 — cured SCRIP `48b09e04`, now `total=21 covered=21 rc=0`; `--two-number` bucket plumbing `MEASURED=4 DECLARED=5 REFUSE=12` rc=0, multiples unquoted. **Acceptance gate on the pin tree, worst-of-3:** `CLEAN=3 (floor 3) · REFUSE=3 · CRASH=13 · other=2`, 11 of 21 flip, `GATE FAIL(1)`; the three CLEAN (`fib nrev queens_8`) include the 2-of-64 and 0-of-65536 programs. **r9 datum, re-measured here, command named:** `./scrip --compile --target=x86 bench/<k>.pl | grep -c 'rtccb+48'` → `nrev` 225, `nreverse` 168 (ceo's 244 was `2748100d`, a different tree). Single-shot at `f4532dea`: gnu 21/21, swi 21/21, m3 10/21 (the ten self-timed kernels), 11 crash/fail. ⛔ `src/` untouched (ceo: hold until "rung-0-landed"). ⭐ **Second sitting, served by the picker as the rank-0 blocker: `pascal-quick-m4-wrong-checksum-crash-masked` — CURE PROVEN, PARKED WITH ITS PATCH (ceo 15:35: no `src/` landings before rung 0).** The lowerer already writes an exit-guard literal for every Pascal procedure and `dead_pure.c` deletes it as unreferenced, so the relop's raw `rax` reached `qsort_γ` (seat09's mechanism, now with its missing link: `SCRIP_OPT_STATS` `dead=1` → `dead=0`); one statement in `lower_pascal.c` gives a procedure the function's exit shape (RETURN of the guard): m4 15505 both modes, Pascal gates identical before/after on a scratch-worktree control arm, `pascal-quick-m4-5x 5/5`. Patch at `/home/resources/postoffice/tasks/pascal-quick-m4-wrong-checksum-crash-masked.patch`; row parked `BLOCKED-ON:prolog-rung-0-…`; receipt `FINDING-2026-09-02-hq_P-pascal-quick-m4-wrong-answer-cured-the-lowerer-wrote-the-exit-guard-and-the-dead-pure-pass-deleted-it.md`. Landed the same sitting, `scripts/` only (SCRIP `862c51cb`): five gates no longer default their seat root to `/home/claude_P` (hq_C measured the p7 gate grading MY checkout from THEIR seat); the zd-omega acceptance gate's Prolog floor re-pinned to the 12 stems both binaries read at `48b09e04` (`rung15_abolish_abolish_one_of_two` left, `rung22_write_canonical_write_canonical_list` entered — hq_C's). **NEXT:** ceo's rung-0 brief — (1) the quad-clean gate (no r12–r15 write reachable from a Prolog graph outside the TR/B/ROOT/BALL helpers, emitted `.s` + rtx asm), (2) rungs 6–8 once hq_C's rung 0–2 frame is on origin, (3) the two-number pin after rung 8, on THIS instrument.

### LIVE CURSOR — s282 (hq_P, TRIO) — BX-0 the port-trace instrument landed at SCRIP `f4532dea` (`SCRIP_PL_TRACE=1/2`, gate `test_gate_pl_port_trace.sh`, refs `corpus/tests/prolog/ALL.trace`); r15 was a driver mmap address at every ω; the pin was parked on Lon's wrap-up. Folded into s283 above; receipt `FINDING-2026-09-02-hq_P-bx0-port-trace-lands-r15-is-a-driver-mmap-address-at-every-omega-and-the-baseline-pin-is-parked.md`.
...
Icon: a fresh emit carries ZERO `.loc` — only SNOBOL4's lowering populates the line table (prolog bench regen
`changed=0`). ⛔ **NOT an Icon all-clear:** the verifier reports 15 Icon bench `.s` owed + 3 CERR on this tree —
re-measured at 6.5k–7.6k differing lines each, carrying no `.loc`, i.e. the standing s272 Icon backlog, not this row. Prolog smoke **4/5**: `clause` fails identically in both arms **and in m2**, which this change cannot
...
✅ **s275c — THE TWO-ASPECT BOARD IS LIVE, AND THE TWO ASPECTS DISAGREE IN DIRECTION** (`bench-rep-loop-demo-harness`, SCRIP `f93ba644`, board corpus `1a4b2e7c1`). All 10 tier-1 twins × sbl/m3/m4. **m4 vs the clean oracle — ASPECT 1** (whole process, compile included): we lose **every** row, 3–6x behind on the small demos. **ASPECT 2** (in-program bracket, match only): **claws5-match 1.686x AHEAD**, **treebank-match-fence 1.194x AHEAD**, calculator-1-match-fence 0.987x (par), calculator-1-match 0.350x, calculator-2-match 0.376x, **json-match 0.086x**. ⭐ **The reading: on the small demos our loss is dominated by STARTUP+COMPILE, not by matching — and the match engine is already ahead on two tier-1 demos today.** ⛔⛔ **json-match's MATCH PHASE ALONE IS 11.6x BEHIND**, its fence twin 0.106x, and aspect 1 puts json-match at 0.21x — **the whole-process number understates the match gap by a factor of five** and would have aimed a cure at the wrong half. That number did not exist before this session.
⛔⛔ **THE CALIBRATION FINDING, WHICH WOULD HAVE POISONED THE BOARD HAD I BAKED A REP COUNT: THE FIRST MATCH PAYS LAZY PATTERN COMPILATION, so a rep loop with too few reps measures COMPILE AGAIN WEARING A BRACKET.** treebank-match m3 vs oracle: reps=20 → **0.085x**, reps=200 → 0.863x, reps=2000 → **1.017x**, reps=20000 → 1.128x. **The low-rep reading is wrong by 13x and looks perfectly healthy.** ⭐ So reps is committed **nowhere**: the harness ramps ×4 and **refuses** until two successive steps agree within 8% at a real window. Convergence is **per-demo** — calculator-1-match is stable by reps=5 (per-match work dwarfs the one-time compile), treebank needs ≥2000. ⚠️ Two arms (treebank-match/sbl, claws5-match-fence/m4) did not converge by 4,000,000 reps while their neighbours did; **printed as refusals, not dropped**, and the script exits non-zero rather than showing a smaller green board.
⭐ **THREE SILENT-WRONG-NUMBER BUGS IN MY OWN INSTRUMENT, ALL FOUND BEFORE I QUOTED IT:** a fixture detector keyed on `?` found **zero** fixtures in calculator-2 and json (they use the implicit juxtaposition form `src  C  :F(error)`) and would have bracketed the wrong statement had it guessed; the m4 arm cached its binary under a stem without the rep count, so it would have built once at reps=1 and reused it while dividing by the current `r`; and aspect 1 on `/usr/bin/time -f %e` reported treebank/claws5/json as a flat **0.0** because that clock ticks at 10 ms while these demos run in single-digit ms — **a real measurement rendered as a zero that reads like "too fast to matter" when it means "below my tick."** Now on `tools/bench_rusage` (ns). ⛔ **Every one of those would have produced a plausible board.**
⚠️ **Also routed to `hq_C` (a wrong ANSWER, not mine to cure):** `OUTPUT(.V, ch, '[-f2]')` **silently discards every write** in both m3 and m4 — rc=0, no diagnostic, while the oracle writes correctly; file association works in both, so it is the `-fn` form specifically. → `FINDING-2026-08-28-hq_P-scrip-output-association-to-fd2-stderr-silently-discards-writes.md`. ⭐ Bonus calibration in that FINDING: **`TIME()` is NANOSECONDS in both engines** (measured against an external clock, not taken from the manual's millisecond convention), which is what makes a cross-engine bracket legitimate at all.

✅ **s275d — RE-MEASURED POST-SLICE-(b): FOUR TIER-1 DEMOS NOW BEAT SPITBOL ON THE MATCH PHASE, AND ceo's json HYPOTHESIS IS NOT SUPPORTED** (board corpus `36c43855a`, harness SCRIP `a56e98f8`, tree SCRIP `69178c73`). m4 vs clean oracle, aspect 2: **claws5-match 1.735x · claws5-match-fence 1.670x · treebank-match-fence 1.103x · treebank-match 1.096x AHEAD**. ⚠️ Two of those (treebank-match, claws5-match-fence) are **first readings, not improvements** — their arms refused to converge the previous run and converged this one; flagged so four green rows do not read as four wins.
⛔⛔ **ceo asked me to re-measure json's match gap on the slice-(b) tree "since json is capture-heavy". TESTED, NOT SUPPORTED: json-match-fence 0.106x → 0.110x, essentially unchanged.** Slice (b) did not touch it, so capture-copy cost is **not** the explanation and a cure aimed there would have missed. json's match phase remains ~9x behind and the cause is now **unattributed** — which is a better starting point than a wrong attribution.
⭐ **The other half is unchanged and is the campaign's real target:** on aspect 1 we lose **every** row (m4 3–4x behind on the small demos) while winning four on aspect 2. **On these demos our loss is STARTUP+COMPILE, not matching** — which is where the new m3-may-diverge freedom should be spent, not on the half already winning.
✅ **ceo's three rulings discharged:** bracket moved to **stderr** and labelled **`TIME_ns`** (not cpu/elapsed — TIME() is measured nanoseconds in both engines, but wall-vs-cpu is *not* settled, every calibration run being compute-bound); hq_C's fd2 cure verified in **both modes on my own build** before switching rather than taken on trust (their cure is wider than the row I routed — `-f1` was discarded identically, `_io_parse_opts` having exactly one caller, `_INPUT_`); and the artifact-regen ruling discharged by **running the sanctioned chain**, which is a **no-op** — benchmarks 20/20 and demos 21/21 already byte-identical, because I regenerated after `9688e110` (corpus `986b44e23`). ⚠️ The PT-3 promotion changes **no** benchmark artifact *by construction* (gated on `vslot >= 0`; no benchmark has a `$V` site) — that looks like a missed regen and is not.
⚠️ **THE REFUSALS MOVE BETWEEN RUNS, AND THAT IS ITSELF THE FINDING:** previous run refused treebank-match/sbl and claws5-match-fence/m4; this run refuses json-match/sbl. All three are the **fastest arms** (~400–1900 ns per match), where an 8% tolerance sits close to timer and scheduler noise — so it is a property of **the ramp at the fast end**, not of those demos. ⛔ **Tolerance deliberately NOT loosened: a tolerance widened until it stops refusing is a tolerance chosen to produce answers.**

⛔⛔ **s275e — I CAUGHT MY OWN INSTRUMENT TAXING THE PATH IT MEASURED, AND IT DID NOT CANCEL** (cure SCRIP `8c5514f5`, clean board corpus `2d22dceec` superseding the 0430Z and 0600Z TSVs). The aspect-2 bracket opened its `OUTPUT` association **before** the timed loop — and in SNOBOL4 an open association makes `NV_SET_fn` consult the channel table on **every store**, so it taxed exactly the path a capture-bearing match uses. ⭐ **Caught by a DIFFERENTIAL PROFILE, not by any failure — every run was green:** claws5-match (we win) profiled 100% emitted match boxes; json-match came back led by `_io_chan_find_by_var` **18.35%** with **zero match boxes in its top ten**. **A match benchmark with no match boxes in its profile is a broken instrument** — that absurdity was the only signal, and it is now the law text's transferable detection. ⛔ **The tax scales with how much a program ASSIGNS inside its match, so it did NOT divide out: json-match 0.086x → 0.168x (m4 arm inflated 81%), claws5-match 1.736x → 1.742x (0.3%).** It hit hardest exactly the row being used to choose the next cure. Corrected to ceo within minutes; RULES.md **clause 6** landed with this witness — *participant-not-instrument; disjoint-or-report-the-perturbation*. → `FINDING-2026-08-28-hq_P-my-own-bracket-taxed-the-path-it-measured-and-did-not-cancel.md`
⭐⭐ **s275f — json-match LOSES ON CALLOUTS, NOT ON MATCHING, AND ONE MECHANISM EXPLAINS BOTH THE WIN AND THE LOSS** (SCRIP `c06a21a8`). Only **15.6%** of json's match phase runs in emitted match boxes; **~31.6% is procedure-entry ceremony** per pattern callout (`rt_name_save_push` 5.78 · `rt_proc_call_prologue` 5.52 · `rt_proc_find` 4.90 · arg `memmove` 3.85 · `rt_call_proc_descr` 2.93 · `rt_proc_call_open` 2.52 · `rt_proc_enter` 2.02 · epilogue 1.05). **claws5-match, which we WIN at 1.681x, is 100% emitted match boxes with zero ceremony.** claws5's grammar is pure pattern primitives; json's drives the *same* engine through **procedure callouts**. ⭐ **Where we spend time in the pattern engine we win; where a demo reaches the engine through callouts we lose, and the loss is the callout.** Corroborates ceo's whole-process dig from an **independent** instrument (their 6.7/5.1/4.1/3.4/5.4 vs my match-isolated 5.78/4.90/5.52/2.93/3.85) and **kills** their capture-cost hypothesis (json unmoved across slice-(b): 0.106x → 0.110x). → `FINDING-2026-08-28-hq_P-json-match-loses-on-callouts-not-on-matching.md`
✅ **Cure landed:** `rt_call_fastpath_ok` → `always_inline` (one global read; at `-O0` a plain `static` is a real call, and the annotation put it at **9.20% of `rt_name_save_push`'s own self time**) — **json's match phase 2341.1 → 2054.9 ns, −12.2%**. ⛔ **A guess I checked and abandoned, recorded because the method is the point:** I expected `rt_name_save_grow` (called once per parameter, a real call at `-O0`) and had the hoist half designed; the annotation does not show it among that function's hot instructions at all. **Measuring before curing cost one command and saved a pointless commit.**
⚠️ **Scope stated, not implied: ~31.6% of ceremony does NOT explain a 5x gap.** Killing all of it yields ~1.5x, and the emitted boxes are only 15.6% — **the arithmetic does not close and I am not claiming it does.** The remainder is unattributed; the design-scale lever (frame-based DEFINE locals — the ζ discipline applied to procedures) is GOAL-scale, not improvisable.
⛔ **PROCESS: I detected TWO `make pristine` runs racing in this checkout** — the hazard CLAUDE.md documents — killed both and restarted a single build rather than trust a raced verdict. **The failure mode is not a red board; it is a plausible GREEN one off a half-linked tree.** Gate numbers of record: pristine, single build, **m3 892/892 · m4 892/892 SKIP=0 MISSING=0**, both gates rc=0, icon 14/14, snocone 5/5, rebus 4/4.

⭐⭐ **s275g — ASPECT 1 IS A FIXED STARTUP FLOOR, NOT PER-PROGRAM WORK; AND THE OBVIOUS RELOCATION LEVER IS MEASURED DEAD** (FINDING `7d45dcbb`; measurement + a negative result, **no cure landed**).
⚠️ **Split corrected: ~55/45, not ~80/20.** treebank-match best-of-5 CPU — oracle user 0.00 / sys 0.88 / cpu **0.88 ms**; m4 user 1.16 / sys 2.31 / cpu **3.47 ms**. The 2.59 ms gap is **sys 55% / user 45%**: `ceo`'s direction was right, but both halves are load-bearing and funding one stalls at about half. (`sys` also carries exec/mmap/dynamic-loading, not only faults.)
⛔⛔ **NEGATIVE RESULT, THE MOST USEFUL THING HERE: DYNAMIC RELOCATION IS NOT THE LEVER.** The RT `.so` carries `.rela.dyn` **3.19 MB = 132,389 `R_X86_64_RELATIVE`** applied every process start, against a 272 KB oracle binary — an obvious suspect. Relinked the same objects with `-Wl,-z,pack-relative-relocs`: relocs **132,389 → 1,014**, `.so` 34.4 → 31.3 MB, output byte-identical. ⭐ **Measured with `LD_DEBUG=statistics`, not wall clock** (the wall A/B sat at 2–3% and was readable either way): loader startup **694,927 → 664,966 cycles**, relocation **458,769 → 427,759**. **Packing 132,389 relocations saved 31,010 cycles ≈ 0.01 ms ≈ 0.4% of the gap** — the RELATIVE loop runs at ~3 relocs/cycle and the whole loader startup is only ~0.23 ms. **Flag NOT landed**: 0.4% does not buy a link-line divergence. ⛔ **No amount of reading section sizes would have told us this.**
⭐ **What it actually is:** `tiny` (whole body `OUTPUT = 1`) already costs **maxrss 9,044 kB / 602 minor faults**; treebank-match m4 12,712 kB / 844; the **clean oracle's entire real run 1,948 kB / 170**. **A SCRIP program that does nothing pays 9 MB and 602 faults**; treebank's real work adds only 3.7 MB and 242 on top. **We are not losing on the work, we are losing on the entry fee** — which is also why every small demo loses by a similar factor regardless of what it does. The floor is the `-O0` RT `.so` paging in (`.text` 5.19 MB · `.rodata` 1.38 MB · `.data.rel.ro` 1.12 MB, fully written during relocation so guaranteed touched). ⛔ **`.bss` (53.5 MB declared) is NOT the cost** — zero-fill-on-demand; I checked the largest symbol, `dat_types` at **17.5 MB**, before blaming it, precisely because it sat suspiciously close to the 18 MB resident figure. It is counter-indexed and only used entries are touched.
⛔ **Consequence, a priority change rather than a new row: the floor cannot be optimised away in C** — `RT_OPT` is `-O0` by Lon's standing FACT RULE, so `.text` is unoptimised **by mandate**. Shrinking it is structurally the **RTX** program's job, which this moves from "long-term" to **the larger half of the tier-1 aspect-1 loss, sized**. ✅ `ceo`'s user-side lever (the `x86()` encoder string path, ~25% of user time → string-free BINARY emission, TEXT keeps strings, helpers-first) stands unchallenged and addresses the 45% half.

✅ **s275h — THE ASPECT-1 FLOOR IS ISOLATED TO LIBRARY LOAD; THREE LEVERS MEASURED DEAD, ONE WORKS AND HAS A PRICE** (FINDING addendum `f4f6292c`; **nothing landed** — the working lever needs a ruling).
⭐ **Isolated by control:** C `main(){return 0;}` with the RT **not** loaded 1,252 kB / 67 faults / 0.34 ms · same program with the RT **loaded** 8,672 kB / **497** / **2.15 ms** · m4 `OUTPUT = 1` 9,044 kB / 531 / 2.22 ms. **Merely loading `libscrip_rt.so` costs 430 faults, 7.4 MB and ~1.8 ms; the m4 program's own init adds 34 faults.** The oracle's entire real run is 170.
⛔⛔ **A CONTROL-ARM TRAP THAT NEARLY PRODUCED A PUBLISHED REVERSAL OF MY OWN HOUR-OLD FINDING.** My first control linked `-lscrip_rt` and read **0.34 ms** — saying the library was free and the attribution wrong; I had the correction half-written. **The control was wrong: `--as-needed` is the default, the control referenced no symbol, and the linker dropped the `DT_NEEDED` entry entirely** (`ldd` = 0 refs). Rebuilt `--no-as-needed`: 2.15 ms, original attribution stands. ⭐ **A control that silently does not exercise the thing under test agrees with every hypothesis** — belongs beside clause 6.
⛔ **Three levers, measured dead — two of which look compelling on paper:** `-z pack-relative-relocs` (132,389 relocs → 1,014) = **0.4%**; `-Bsymbolic-functions` (loader −19.6%) = **~1.3%, end-to-end inside noise**; `strip --strip-debug` (`.so` −64%) = **no effect**, debug is never mapped. **The loader's total startup is only ~0.20 ms of 2.15 ms, so every link-level lever is bounded at ~9% before it starts.** Nobody should spend another hour on link flags.
✅ **Static linking works:** `OUTPUT = 1` 2.22 → **1.08 ms** (faults 531 → 194, **−51%**); treebank-match 2.82 → **2.01 ms** (770 → 418, **−29%**), moving its aspect-1 multiple **0.17x → 0.24x**; answers byte-identical. ⛔ **NOT LANDED — the price is a Lon/ceo call, not a perf decision: 27,404 kB per binary against 32 kB shared**, and it cuts against `out/libscrip_rt.so` being the canonical path 73 scripts reference by name, so it could only be an *additional* link mode. ⚠️ It also does not close the gap — 0.24x is still ~4x behind; the floor is **code-size-shaped** and `-O0` is mandated, so the structural cure remains **RTX**. Static linking is worth about a third of the aspect-1 loss for a price someone else must agree to pay.

**NEXT (unstarted, named so the successor inherits a list and not a search):** (0) **json-match's match phase at 0.086x is now the single worst tier-1 number and the obvious next target** — profile the match phase specifically, not the whole process; (1) `--calibrate` should **REFUSE a row whose match share it cannot raise above a floor** rather than silently measuring startup — a scale nobody can justify from a measurement is the same class as a guard keyed on a coincidence; `treebank`/`json`/`claws5` `-match` twins are pattern-COMPILE dominated at their committed inputs (the whole `treebank-match` run is 9.3M instructions; 60x input reaches 13.4M), which is exactly how a 1.41x cure sat dark with every board green. (2) slice **(c)** `rt_dcap_pump` → rtx ASM: ceiling computed **before** writing any asm, per that file's own law — self time **24.64%** of `string_pattern`, diffuse `-O0` frame traffic with no instruction above 2.70%, so the ceiling is real; ⛔ but Lon's *"delete the C twin"* ruling (via `hq_C`) means no new `c_` twin, so the ASM must cover the pump's full surface including the user-code-calling `*` arm — categorically bigger than the delegate-to-C slices that precede it, and it is capture semantics, which is `hq_C`'s custody. **Confirm the reading with `ceo` before building it.** (3) table-path re-measure (gates claws5/treebank/json) with `seat01`'s contamination caveat: re-take the 7x before anyone cures against it.
