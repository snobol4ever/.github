...
## ⭐⭐ INSTRUMENT CHANGE INCOMING (Lon s265, relayed via hq_C) — THREE BENCHMARK MODES, AND WALL-CLOCK IS PERMITTED AGAIN
*"now he can run the app and time wall clock and perf values, and enjoy a TIME based or an ITERATION based benchmark harness. Yeah baby. We should have three options I would think."* The 33 benchmark programs become STANDALONE applications with real `.ref`s (time them from outside with `/usr/bin/time -v`, `perf stat`, callgrind, and they are graded while timed), plus `scripts/bench_wrap.sh <prog>.sno` for TIME mode and `--mode=iter --n=N` for ITERATION mode (`--bud=<ms>` overrides the budget).
⛔ **NOT ON ORIGIN AS OF THIS HANDOFF — VERIFY BEFORE RELYING ON IT.** Checked at handoff: `scripts/bench_wrap.sh` does not exist, and `corpus/benchmarks/snobol4/demo/claws5.sno` still ends in `-INCLUDE '../harness.inc'`. hq_C said "landing now"; pull and confirm before writing a runner against it.
⭐ **THIS SEAT'S PUBLISHED NUMBERS STAY VALID AND NEED NO RE-DERIVATION.** hq_C's note that iteration mode was UNREACHABLE for the data-driven demos is correct and this seat hit it independently: the old gate was `fixed_n = INPUT`, a stdin read, and claws5/json read their corpus from stdin. The workaround used for every number in this file was to build a vehicle with the harness `-INCLUDE` replaced by `OUTPUT = 'check: ' ZBODY(<literal>)` + `END` — i.e. the iteration count BAKED IN, exactly what `bench_wrap.sh --mode=iter` now does officially. ⭐ Still re-baseline on `bench_wrap.sh` before mixing its numbers with these (same-tree, same-vehicle rule).
⛔ **WALL-CLOCK IS PERMITTED BUT THIS BOX IS NOT QUIET** — 4 fleet workers + hq_C were building throughout s264, and s256 measured 2x swings under that load. Ir at fixed work stays the quotable instrument for a RATIO; use wall-clock/perf for what Ir cannot see (the `_tbl_grow` cache-miss conflict below is exactly that case, and is now finally answerable).
