...
| C31 | prolog-master-red-class-ast-dump-refs-134-entries | seat | the 133 clause-only entries whose `.ref` is `--dump-ast` output: grade them with the instrument that wrote the ref (or re-cut the refs to `--run` output) so the master board stops counting a right compiler as 133 fails; was BLOCKED-ON P1 by proximity, unblocked by ceo 2026-09-01 ~20:15 on hq_C's per-entry evidence (FINDING `d2b1391f`) |
| ⭐C32 | prolog-findall-directive-replace-5-returns-empty-list-silently | seat | the ONE genuine silent wrong answer on the post-C1 board (`findall_directive_replace_5` prints `[]`, wants `[20,30]`, rc=0, no stderr); minted by hq_C 2026-09-01 ~20:15 at rank 0 on the C2 per-entry sweep (FINDING `d2b1391f`), placed by ceo the same sitting |
