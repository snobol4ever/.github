...
- [x] **FIX-2** — `bb_assign_frame` + `bb_assign_frame_ref`: operand fusion (`pBB->α->ival/dval`) → `_.op_*` plumbing (driver prepares; template goes blind). TIER S, design pinned: execute when the cursor arrives. ✅ 2026-06-06 chain 5ca0a90 (op_a_dval field + emit_core.c driver prep) → 2c38a14 (bb_assign_frame blind, eb 4→0) → 03222bc (bb_assign_frame_ref blind, eb 4→0). _.op_a_ival_sg already carried α->ival; only α->dval needed plumbing. Asm-diff EMPTY each rung; prove_lower2 PASS; all gates at floors.
...
bash scripts/test_snobol4_pat_rung_suite.sh          # M2>=18 M4>=18 (M4 raised by f44f4df 2026-06-06)
bash scripts/prove_lower2.sh                         # PASS (mandatory for any TIER S rung)
bash scripts/util_template_purity_audit.sh           # floor: 2 pre-existing sites (bb_call_write_slot, bb_every — FIX-3 family, pre-date FIX-2 close); criterion = NO GROWTH
...
**Carved 2026-06-04, ATTENDED-CURSOR model. FIX-0 ✅ d4e3a45. FIX-2 ✅ 2026-06-06 (5ca0a90/2c38a14/03222bc). LAP 1 stops 1–11 ✅ (see HANDOFF-2026-06-06-OPUS48-BB-FIXUP-LAP1-STOPS-9-11.md). FOURTH attended run 2026-06-06 (Opus 4.8): stops 12–14 + the BUILTIN RENAME. Stop 12 bb_findall (5e4141c, 13→6+1) · stop 13 bb_io (d7ea5f3, 51→30) · stop 14 bb_is_cmp (10b83cf, 121→64, icm_* helpers, asm-diff EMPTY incl LIVE is/2 + >= corpus arms; eb/nw rose 28/2→34/8 = de-aliasing made grandkid reads counter-visible, honest [S]). ⛔ BUG CAUGHT MID-RUN: stops 12–13 had swapped BINARY-arm ports to _.lbl_* strings — those parse XK_SYM and emit NOTHING in MEDIUM_BINARY jmp/def (silent mode-3 jump loss; every gate stayed green because corpus only fires these templates mode-4). Repaired 7f3384d: literal Greek glyph strings ("γ" "β" "ω") are the ONLY sanctioned pe-fix in BINARY arms — they parse XK_PORT, byte-identical to PORT_* macros (bb_arith precedent). TEXT arms keep _.lbl_*. THE LESSON IS THE GOAL'S OWN THESIS: gates verify behavior on exercised paths only; the wrongness was confident and green. Verify the x86() parse path before any port-arg transform. BUILTIN RENAME (Lon directive 2026-06-06: every BB is builtin — the word means nothing; goes until _builtin_ is never seen): 75662d3 = 11 arms bb_builtin_X.cpp→bb_X.cpp + _str symbols + bb_builtin_common.h→bb_common.h + dispatcher bb_builtin.cpp→bb_resolve.cpp (extern bb_resolve, the by-name RESOLVE box; emitted comment RESOLVE_BUILTIN→RESOLVE) + Makefile + 4 gate scripts; b633af1 = bb_call_builtin.cpp→bb_call_fn.cpp (bare strip collides with bb_call.cpp; Icon canon: builtins are FUNCTIONS, pairs with bb_call_userproc). BB layer now 100% builtin-free. RESIDUE FOR LON: rt_call_builtin/rt_builtin_is_known + src/runtime/builtins/ + by_name_dispatch (runtime), IR_BUILTIN (contracts), driver/interp hooks, parser .tab.c, backends/js, is_builtin local in HOT bb_call.cpp — runtime/contracts-layer naming = RUNTIME RENAME / LI-CORE territory, NOT swept unilaterally. Tracker ring kept DOCUMENT ORDER through the rename (re-sort mid-lap would scramble the lap; re-sort at lap end on Lon's word). NOTE: new bb_pat_breakx.cpp landed via SNO-HY-1 (7a24653) — joins the ring next lap; rank script globs it already. CONCURRENT LANDINGS merged green ×3: ba7622c REG-RO, 3ec6470/1ad0019 PASCAL PB-10b/c, 7a24653 SNO-HY-1. Gates at floors: smoke 19/0, pat-rung M2=18/M4=18 (053 M2-fail pre-existing), prove_lower2 67/67 PASS, purity 2-site floor, bb_bin_t 0, sno_pat_reg TIER1+TIER2 HARD, vstack 3, one_box PASS. Lap metric 1640→1586, 93 files (breakx joined) / 83 dirty / 10 clean; emit-blind 219 (de-alias visibility, see stop 14). `# CURSOR: bb_list.cpp` (eb=8 nw=6 rb=39 ef=22 pe=2 lv=23 per 06-04 snapshot — re-audit on arrival, renamed file). Session ended ~72% context per law 7. Next: resume LAP 1 at cursor; FIX-3 (bb_call family — STILL HOT, profile changed again under PB-10b + ICN-HY-7c, re-audit) remains next pinned-pending TIER S, design NOT pinned, flag on arrival.**
