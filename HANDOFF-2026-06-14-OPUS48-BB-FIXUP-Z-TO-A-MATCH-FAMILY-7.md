...
- **Live boxes** (`tab`, `span`, `rtab`, `pos`, `notany`, `len`): direct firing, no vehicle. e.g. `S SPAN("0123456789").X` (2 boxes), `S POS(3) LEN(2).X / S RPOS(2) LEN(2).Y` (both arms), `NOTANY("X")` + `NOTANY("xyz")` (both dispatch arms). m2 (`--run`), m3 (`--run`, native in-proc), m4 (`--compile`) **all raw byte-identical**, m4 identical sha256 (no BB-label normalization needed — pure inlining left emission order + sizes unchanged; the one exception, notany's first attempt, is the bug above, fixed and re-proven identical).
