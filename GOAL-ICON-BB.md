...
- **✅ ICN-CDIFF-FIX (2026-07-21, Claude Sonnet 4.6) — SCRIP `(see commit)`. Icon 240/21/32 (environmental math drift — rung17/rung37 real-formatting `2.` vs `2.0` pre-existing; A/B confirmed: rung37 fails identically before and after the fix; same recurring environmental delta as prior cursor notes). ONE FIX advancing the JCON self-host pipeline past bc_File. ROOT CAUSE: `rt_num_arith_impl` (`src/runtime/arithmetic.c:223`) called `to_real(a)`/`to_int(a)` eagerly and unconditionally for ALL ops including cset ops (CUNION/CDIFF/CINTER). When a cset operand arrives as `DT_C`, `to_real()` hits its default branch and calls `core_runtime_error(1, NULL)` — before the CDIFF branch (line 232), which handles csets correctly via string extraction, is ever reached. Bug was LATENT in m3 (interpreter): `rt_num_arith`'s setjmp wrapper at line 211 caught Error 1 and returned FAILDESCR, masking it. In m4/jtran (compiled path, different error-recovery context), the abort was fatal. DIAGNOSED via gdb conditional breakpoint on `core_runtime_error(code==1)`: backtrace = `to_real(v)←rt_num_arith_impl(op=20=BINOP_CDIFF)`. TWO-LINE FIX: `double ld = csop ? 0.0 : to_real(a), rd = csop ? 0.0 : to_real(b);` / `int64_t li = csop ? 0 : to_int(a), ri = csop ? 0 : to_int(b);` (`csop` already defined two lines above). VERIFIED: `'abcdef'--'cd'`→`'abef'`, `'abc'++'cde'`→`'abcde'`, `'abcdef'**'cdef'`→`'cdef'` all match canonical icont oracle. **JCON SELF-HOST — bc_File ADVANCE: pipeline now produces `.class` output.** With jtran relinked against updated libscrip_rt.so: `SCRIP_BETA_ELIDE_OFF=1 ./jtran preproc hello_t.icn : yylex : parse : ast2ir : bc_File -O -class:lhello_t -dir:./tmp/` → rc=0, empty stderr, `tmp/lhello_t.class` + `tmp/p_lhello_t$main.class`. Pipeline status: preproc✓ yylex✓ parse✓ ast2ir✓ bc_File✓ → `.class` output. **SECONDARY BLOCKER DIAGNOSED (non-blocking for bc_File): `symbolic_File` — Error 5 "Undefined function or operation" from `APPLY_fn("name",…)` — SCRIP has no `name()` builtin. `dump_get_names` calls `name(!record)` to fetch field names. Deeper issue: canonical `name(underef v)` needs lvalue semantics; SCRIP's `!record` delivers dereferenced value (`DT_I`) not a VCELL. Oracle: `name(!pt(3,4))` → `pt.x`, `pt.y`. Non-blocking: `bc_File` does not use `name()`. ALSO: `icont`+`iconx` oracle built this session at `/home/claude/work/icon-master/icon-master/bin/`. NEXT SESSION:** (A) `jlink`: run `jlink`/`jcon.zip` against `tmp/lhello_t.class` to get a runnable JCON bundle and confirm `java -jar jcon.zip lhello_t` prints `hello, world`; (B) byte-compare SCRIP-jtran `.class` vs canonical icont-jtran `.class` for hello_t.icn; (C) three-way benchmark (iconx / JCON-JVM / SCRIP-jtran) on `irgen.icn`; (D) rung17/rung37 real-formatting fix — `write(sqrt(4.0))` prints `2.` vs expected `2.0` — check `real_str` / `%g` format in `rt_real_to_str`; (E) `name()` builtin + underef protocol for `symbolic_File` if needed. **WORKDIR STATE:** `/home/claude/jt/` has all 17 modules, `jtran.s` (505K lines), `jtran` binary (5MB), `hello_t.icn`, `tmp/lhello_t.class`, `tmp/p_lhello_t$main.class`. Oracle at `/home/claude/work/icon-master/icon-master/bin/icont+iconx`. See `FINDING-2026-07-21-CLAUDE-JCON-SELFHOST-CDIFF-FIX-AND-BC-FILE-ADVANCE.md`.**
