...
- **HEAD rung:** ZB-ICN-FC — FORTH RSP ζ spine for Icon scan boxes. **Three constructs landed (`IR_SCAN_TAB`, `IR_SCAN_MOVE`, `IR_SCAN_MATCH`). Full scan-function family classified (see watermark below). Next: widen to other non-scan Icon constructs (`IR_TO`/`IR_LIMIT`/`IR_ITERATE`) OR tackle the deferred `{*}` generator-scan family (needs scan re-pump on the spine first).**
- **RESUME-THROUGH-SCAN (prior open rung — still open, unchanged):** `every write("98765" ? upto(&digits))` → `1` then stops ❌. The 6-site fix design + narrowing guard spec is in the WATERMARK 2026-07-14 block below — still accurate. Do NOT re-derive; start mechanical implementation when this rung is next targeted.
- **LAST SESSION:** 2026-07-14 (Claude Sonnet 4.6) — ZB-ICN-FC-1/2/3: `IR_SCAN_TAB`, `IR_SCAN_MOVE`, `IR_SCAN_MATCH` granted fixed 16B rsp cells under `ZC_PORT_FORTH`. Default byte-identical to HEAD; FORTH m3+m4 oracle-exact; cells verified active in `.s`; ladders 239/15/35 both ports == baseline; all smokes/gates green. Classified full `fscan.r`/`fstranl.r` family: ANY/MANY/POS zero-cell by law; UPTO/FIND/BAL deferred (`{*}` generator-in-scan re-pump breaks LIFO — measured regression, reverted, documented in `fc_geom` NOT-YET comment). Key lesson: fc grant set at **emission dispatch** (`walk_bb_node` ~line 812), NOT promotion site (DRIVE_FILL resets `op_fc_bytes` before template runs). `op_fc_base = op_off + 16` for non-shifted Icon scan boxes (result at op_off+0, scratch at op_off+16 — unlike SNOBOL4's shifted layout).
- **WATERMARK:** SCRIP `c9300158` · Icon ladder **239 PASS / 15 FAIL / 35 XFAIL** · smoke **14/14** m3+m4 · gates all green · Fail set: `rung36_jcon_{args,coerce,endetab,fncs1,htprep,kwds,mffsol,mindfa,prepro,recogn,scan,scan1,scan2,string,var}`
- **DECISIVE REPRO (still failing):** `every write("98765" ? upto(&digits))` → `1` then stops ❌ (want `1 2 3 4 5`). Explicit-subject `every write(upto(&digits, "98765"))` → `1 2 3 4 5` ✅ unchanged. Bug: the scan leave-success wrapper does not re-pump its body generator on backtrack from an outer `every`.
- **CORRECTED FIX DESIGN (this session — the prior cursor's "FIX SHAPE" was underspecified in three ways):**
  - **SCRIP uses push/pop env stack, NOT canonical's swap.** `rt_scan_enter` pushes, `rt_scan_leave` pops. The prior cursor's "restore oldpos/oldsubject = canonical ScanSwap" does not apply; re-entry must re-push the inner env via a new runtime primitive `rt_scan_reenter()`, which pops from a parallel `scan_saved[]` save-stack that `rt_scan_leave` populates before popping. This is the **4th site** the prior cursor missed.
  - **`IR_t` has no β field.** α/β targeting is the `.sz` tag on `IR_ref_t`, and the template's β arm emits a label+code, not a stored edge. The body-β label is delivered to the template via the `g_emit.lbl_t0` channel — the **proven `IR_LIMIT` idiom** (lines 1770-1773 of `emit.cpp`): for an `IR_SCAN` leave node, resolve `operands[1]` (the body value node) in the node list and grab `betas[k]`. Then `x86_jmp_tgt(X86T_TGT0)` in the leave β arm jumps there. **5th site: a block in the emit BFS loop, mirroring the LIMIT block.**
  - **Making `IR_SCAN` generator-kind flips internal body→leave edges to β.** The body's γ→leave_succ was auto-stamped β (since leave_succ is now generator-kind), breaking fresh entry. Fix: interpose α-forcing `IR_GOTO` trampolines between the body and the leave nodes (the `lower_proc_body` lines 983-986 idiom), built with `lc_γ_to`/`lc_ω_to` (not `build()`, which would auto-stamp β again).
- **THE HANG-REGRESSION (why the fix was reverted):** `every write("abcde" ? while move(2))` — **pristine `ed95a693`: rc=0, one empty line (terminates cleanly). With the 6-site fix: hangs (rc=124), infinite empty lines.** The ladder did NOT catch this — no passing rung exercises `every ... ? while ...`; it lives only inside the still-failing `scan` rung. Root: for `while move(2)` as the scan body, `operands[1]` (bv, the body value node) resolves to a **GOTO trampoline**, not the actual body generator (`SCAN_MOVE`). So `g_scan_body_beta` points to the GOTO's β, which has no meaningful resume semantics — re-pumping it cycles. **The narrowing guard:** in site-4a (emit BFS loop), only install `g_scan_body_beta` when `operands[1]->op` is `ir_is_generator_kind` (a leaf scan generator or other clean generator). Compound-body scans (`while`/loops-in-scan) fall back to `lbl_t0=NULL` → the template β arm's `IF(_.lbl_t0 != 0, ...)` guard is false → β falls through to ω (old behavior, no re-pump, terminates). This makes the fix safe for simple-generator bodies and neutral for compound bodies. One line added to site-4a.
- **6-SITE FIX MAP (for next session):** (1) `ir_query.c`: `IR_SCAN` → `ir_is_generator_kind`. (2) `lower_icon.c` TT_SCAN: build `succ_tramp`/`fail_tramp` = α-forced IR_GOTO nodes; lower body to trampolines; trampolines forward to leaves at α via `lc_γ_to`/`lc_ω_to`; set `cx->beta = leave_succ` unconditionally. (3) `gen_runtime.h`: declare `rt_scan_reenter(void) → ScanSubjRegs`. (4) `gen_runtime.c`: add `scan_saved[SCAN_STACK_MAX]`/`scan_saved_depth`; `rt_scan_leave` saves inner `scan_subj`/`scan_pos` to `scan_saved` before popping; `rt_scan_reenter` pops from `scan_saved`, re-pushes current outer onto `scan_stack`, restores inner subj/pos, returns inner ptr/len. (5) `emit.cpp`: declare/reset `g_scan_body_beta`; in BFS loop after LIMIT block, for `IR_SCAN` node with `n_operands>1` AND `ir_is_generator_kind(operands[1]->op)`: resolve `betas[k]` → `g_scan_body_beta`; in IR_SCAN drive arm: set `g_emit.lbl_t0`/`lbl_t0_p = g_scan_body_beta`. (6) `bb_gen_scan.cpp`: extern `rt_scan_reenter`/`rt_scan_sync_in`; leave β arm: `IF(_.lbl_t0 != 0, call rt_scan_reenter; mov r13,rax; mov r15,rdx; call rt_scan_sync_in; mov r14,rax; jmp_tgt(X86T_TGT0))`.
- **VALIDATION PROTOCOL (next session):** after implementing: (a) `every write("98765" ? upto(&digits))` must give `1 2 3 4 5` m3+m4; (b) `every write("abcde" ? while move(2))` must terminate cleanly (rc=0) — the hang-regression test; (c) full ladder ≥239 with byte-identical fail set; (d) smoke 14/14 both modes; (e) four Icon gates green.
- **WATERMARK:** SCRIP `ed95a693` on origin (unchanged — fix reverted). Icon ladder **239 PASS / 15 FAIL / 35 XFAIL**; smoke **14/14** m3+m4; gates all green. Fail set: `rung36_jcon_{args,coerce,endetab,fncs1,htprep,kwds,mffsol,mindfa,prepro,recogn,scan,scan1,scan2,string,var}`.
- **LAST SESSION:** 2026-07-14 (Claude Sonnet 4.6) — orientation + baseline validation + RESUME-THROUGH-SCAN design archaeology. Discovered the prior cursor's FIX SHAPE had three wrong premises (swap vs push/pop; no β field in IR_t; internal α/β stamp conflict). Built the corrected 6-site fix; confirmed it fixes the target bug and the full ladder stays 239/15/35 — but a dig found `every ... ? while ...` hangs (pre-existing-pass flipped to infinite loop, hidden by the still-failing scan rung). Reverted to `ed95a693`. Cursor now carries the corrected design + the narrowing guard spec for the next session to implement.
...
## ⌚ WATERMARK 2026-07-14 s2 (Claude Sonnet 4.6 · SCRIP `c9300158` · corpus unchanged) — ZB-ICN-FC-1/2/3: TAB + MOVE + MATCH on the FORTH rsp spine; generator-scan hazard mapped and documented; 239/15/35 both ports

**Session scope:** fresh clone (ICON+JCON zips → `refs/` symlinks); full orientation per PLAN.md/RULES.md/ARCH-ICON/GOAL-TEMPLATE-REVAMP; baseline validated (239/15/35, smoke 14/14, 4 gates green). Implemented ZB-ICN-FC-1/2/3: granted fixed 16B rsp cells under `ZC_PORT_FORTH` to `IR_SCAN_TAB`, `IR_SCAN_MOVE`, `IR_SCAN_MATCH`. Discovered, measured, and reverted a regression on `IR_SCAN_UPTO` (scan-subject `every` produced empty instead of `6` — `{*}` generator-in-scan breaks fixed-cell LIFO). Classified the full `fscan.r`/`fstranl.r` scan-function family (granted / zero-cell by law / deferred), documented in `fc_geom` NOT-YET comment. Key integration insight: fc grant must be set at the **emission dispatch** in `walk_bb_node` (post-DRIVE_FILL), not at the promotion site.

**Validation:** default `--compile` byte-identical to HEAD for all three constructs; FORTH m3+m4 oracle-exact; cells verified active in emitted `.s` (sub/add rsp,16 + `[rsp+0]` rebase); ladders 239/15/35 both ports == baseline; scan-subject `every`/`while` patterns identical both ports; Icon smoke 14/14, Prolog 5/5, SNOBOL4 7/7 both ports; all four Icon gates green.

**Authors:** Lon Jones Cherryholmes · Jeffrey Cooper M.D. · Claude Sonnet 4.6

## ⌚ WATERMARK 2026-07-14 (Claude Sonnet 4.6 · SCRIP `ed95a693` unchanged · corpus unchanged) — RESUME-THROUGH-SCAN: full 6-site fix designed, built, tested — hung on `every ... ? while ...` (hang-regression hidden by failing scan rung); REVERTED; corrected design + narrowing guard spec in LIVE CURSOR above

**Session scope:** fresh clone (ICON+JCON zips → `refs/` symlinks); full orientation; baseline validated (239/15/35, smoke 14/14, 4 gates green). Designed and implemented RESUME-THROUGH-SCAN (6 files, 37 insertions). Fix correctly resolved `every write("98765" ? upto(&digits))` → `1 2 3 4 5` m3+m4. Full ladder held 239/15/35 byte-identical. BUT: dig found `every write("abcde" ? while move(2))` hangs (pristine: terminates, fix: infinite loop, rc=124). Hang hidden from the ladder because the construct lives only inside the still-failing `rung36_jcon_scan`. Reverted all 6 files to `ed95a693`. Three wrong premises in the prior cursor's FIX SHAPE corrected in the LIVE CURSOR: (1) SCRIP uses push/pop, not swap — needs `rt_scan_reenter`; (2) no β field in `IR_t`, body-β delivered via `g_emit.lbl_t0` channel (LIMIT idiom); (3) making `IR_SCAN` generator-kind flips internal body→leave edges — needs α-forcing trampolines. Narrowing guard identified: site-4a should only install `g_scan_body_beta` when `operands[1]->op` is `ir_is_generator_kind` (leaf generators only, not compound/while bodies). Next session: implement the 6-site fix WITH the narrowing guard; validate with the hang-regression test case as a required gate.

**Authors:** Lon Jones Cherryholmes · Jeffrey Cooper M.D. · Claude Sonnet 4.6

## ⌚ WATERMARK 2026-07-13 s2 (Claude Opus 4.8 · SCRIP `d4dd11f7` local, `eb9ac70e` on origin · corpus unchanged) — REPAIRED the non-building committed tree (SCAN-NARY link break); 239/15/35 re-confirmed; tab/move slice-value bug in the landed SCAN box DIAGNOSED (fix = item 1); PUSH of the build fix PENDING credential

**Session scope:** fresh clone (ICON+JCON zips → `refs/` symlinks); orientation from GOAL-ICON-BB.md + RULES.md + ARCH-ICON/x86 + JCON `irgen.icn` + ICON `*.r` + `bench/test_icon.c`. Built scrip — **it did not link.** Repaired, re-baselined, then pursued the Lon directive (kill IR_GOTO/IR_MOVE_LABEL, state-in-box) and found the landed box's value contract is wrong for cursor-movers.

**LANDED — `d4dd11f7` SCAN-NARY-BOX BUILD FIX (the prior commit `eb9ac70e` was committed NON-BUILDING).** `eb9ac70e` added `bb_scan_alternate.cpp` as a near-verbatim copy of `bb_match_alternate.cpp`, but that copy PREDATED `32bd6106` (rbp-dcap), which deleted `rt_dcap_height`/`rt_dcap_restore_to` from the runtime. The committed scan template still called them → `undefined reference` at link. The prior watermark's claim "both templates already build" was FALSE (and its "PUSH BLOCKED / local commit pending" banner was ALSO stale — `eb9ac70e` IS on origin; verified `git branch -r --contains`). Fix mirrors the s46 match-alternate deletion: Icon scanning produces a VALUE (the `rt_substr` slice) and never appends to the SNOBOL4 capture/assignment (dcap) stack, so there is no height to save; both C-call windows deleted, slot +20 kept as dead pad (no offset shift), `zeta_storage` comment updated. scrip + libscrip_rt now link clean.

**RE-BASELINE (fresh, this session, on the repaired tree):** Icon `test_icon_all_rungs.sh` (m3) **239 PASS / 15 FAIL / 35 XFAIL** — byte-identical FAIL set to the prior watermark (`rung36_jcon_{args,coerce,endetab,fncs1,htprep,kwds,mffsol,mindfa,prepro,recogn,scan,scan1,scan2,string,var}`). Icon smoke **12/12 m3 AND m4**; prolog smoke 5/5 all modes; gates `icn_no_stack` 0 / `icn_one_reg_frame` 0 / `icn_semicolon_required` PRISON / `icn_local_no_nv` PASS / `emit_no_lang` PASS.

**⛔ DIAGNOSED, NOT FIXED — the landed `IR_SCAN_SEQUENCE`/`IR_SCAN_ALTERNATE` box's γ slice is WRONG for cursor-mover arms (`tab`/`move`).** The box's success-glue (na_s) computes `rt_substr(Σ, saved_δ, δ−saved_δ)` — the subject text spanned by the whole box. This equals the concatenation ONLY when every arm is a RELATIVE-advance matcher (`=s`/`match`, which does `r14 += len`). `tab`/`move` set r14 to an ABSOLUTE position (`bb_scan_tab.cpp:36 mov r14, rdx`), and MEASURED the box then yields the wrong slice: `"hello" ? (tab(3)||tab(5))` → `""` (should be `"hell"`), `"abcdef" ? (move(2)||move(2))` → `""` (should be `"abcd"`), while `"ab" ? (="a"||="b")` → `"ab"` correctly. **All three route through the committed box** (`--dump-ir` shows `SCAN_SEQUENCE`). It stayed invisible because NO passing rung uses a tab/move concat — the watermark's "proven correct + regression-free" was true only for the `=`/match arms it was tested on. **THE FIX IS THE WATERMARK'S OWN ITEM 1** (below): the box must concatenate each arm's OWN value slot (via `str_concat_d`, the `bb_binop_concat_slot.cpp` idiom) instead of slicing the subject span. Threading arm value-slots into the box is the real work — the `2N=(entry,resume)` operand array is the SHARED control-flow contract consumed by `flat_drive_match_alt` (also SNOBOL4's `IR_MATCH_*`), so the value slots need a SEPARATE channel; do NOT overload the operand pairs.

**ATTEMPTED + REVERTED — `icn_scan_alt_nary` (cursor-mover alternation → `IR_SCAN_ALTERNATE`).** To retire the `IR_DISJUNCTION`+per-arm `IR_MOVE_LABEL` goto-web for `s ? (tab|tab|move)`, wrote `icn_scan_alt_nary` (near-verbatim mirror of `icn_scan_seq_nary`) + an `icn_arg_is_cursor_mover` predicate + a `lower_alt` gate firing only when EVERY arm is `tab`/`move` (the only arms whose value equals the slice). It built and correctly routed to the box (`--dump-ir` SCAN_ALTERNATE, goto-web gone) — but produced the SAME wrong slices as the sequence bug above (`tab(3)|tab(5)` → `""`), i.e. it would REGRESS the currently-correct disjunction path (verified old path: `tab(3)|tab(5)` → `"he"`). Reverted in full (kept only the build fix). **Re-land it as the SECOND half of item 1, after the box's γ uses per-arm values** — the lowering itself is correct and ready (near-identical to `icn_scan_seq_nary`; keep the tight all-cursor-mover-or-all-scan-fn gate).

**NEXT (leverage order):** (1) **item 1 — box γ concatenates per-arm VALUE slots, not the subject slice** (fixes the latent tab/move concat bug AND unblocks alternation; thread value-slots via a non-operand channel; then widen the seq gate + re-land `icn_scan_alt_nary`). Verify with a fresh iconx oracle on `tab||tab` / `move||move` / mixed `tab||"y"` — none is in a passing rung, so add them as probes. (2) after item 1, the remaining `IR_DISJUNCTION`/`IR_MOVE_LABEL` general-alternation cases (`a|b|c` with arbitrary arms) become boxable by having the box copy the live arm's value slot (not a slice) — the general state-in-box alternation. (3) the 15-open FAIL set is unchanged and orthogonal (scan cluster needs generator-in-scan; `kwds`/`var` need builtins; `coerce` real-to..by loop).

**⚠ STALE-PROSE NOTE for the next reader (RULES.md rule):** the prior watermark (below) still reads "SCRIP local commit pending · PUSH BLOCKED (credential needed)" — that is FALSE, `eb9ac70e` is on origin. Trust `git log origin/main` + `handoff_status.sh`, never a committed status banner.
