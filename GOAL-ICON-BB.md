...
- **🔴 ICN-CASE-ALT-SELECTOR (2026-07-21 SESSION 2, Claude Opus 4.8) — clone HEAD `65023c05` (BASELINE CLEAN, FIX NOT LANDED). Icon 241/20/32. jtran hello self-host GREEN at baseline (verified this session with a from-scratch `jtran_base` build).** ⚠ NOTE: clone HEAD is `65023c05`, NOT the prior cursor's `00aee19c` — parallel-session drift; `ICN-CASE-ALT-BODY` (`a9d6fec8`) and the 5 precedence fixes (`3c96859c`) are already landed. Selector fix is the only piece left.
  **OPTION B WAS TRIED AND FALSIFIED THIS SESSION — do NOT retry it as specified.** Implemented the dedicated `IR_GOTO` α-entry trampoline in TT_CASE (`if (ke && ke->op == IR_DISJUNCTION) { KENT=build(IR_GOTO); lc_γ_to_α(KENT,ke); lc_ω_to_α(KENT,ke); }`, chain routed through KENT). Result: corpus repro `case cur of {1|2|3:}` FIXED, both modes match oracle, **corpus 241/20/32 zero regression** — but jtran hello self-host **SEGVs (rc=139)**. Even after narrowing the guard from `ir_is_generator_kind` to `IR_DISJUNCTION`-only (to spare scan selectors), the SEGV persisted.
  **WHY OPTION B CANNOT WORK (mechanism, gdb-proven):** the emitter's edge-tag resolver (`emit.cpp` ~1912-1913, mirrored ~1679-1680, ~2022) **follows through `IR_GOTO` nodes transparently** — `while (_g->op == IR_GOTO) { OR-accumulate β/σ/φ tags; _g = _g->γ.node; }`. So a trampoline `IR_GOTO` is NOT opaque to the emitter: it walks straight through KENT to the disjunction and reads the disjunction's own state anyway. The trampoline does not decouple the forced entry from the adjacent scan box, which was the entire premise of Option B. gdb backtrace on the crash: `xchainNNNN_n76_α → rt_call_arr_gen("upto") → VARVAL_fn → rt_ws_strdup_c → rt_gcheap_alloc` faults — the emitted α-entry re-runs the preprocessor's `upto` scan with a corrupted workspace/frame pointer. The offending jtran construct is `preprocessor.icn:298` `"\""|"'":` — a **value-alternation selector living inside the `while tab(upto(...)) do case move(1) of {...}` scan environment** (δ=R14/Σ=R13/Δ=R15 live). Force-α re-enters the disjunction's α code, which reads `&subject[&pos-1]`, after the scan frame has been perturbed. Note the emitter GOTO-follow checks β/σ/φ but NOT `α!` (0xce 0xb1 0x21), so `α!` does not even propagate outward through the trampoline — another reason the node is the wrong lever.
  **REAL FIX LOCUS = the emitter, not the lowerer (prior cursor's Option A).** The forced-fresh entry into a value-alternation selector must NOT reset the scan registers when the selector is nested in a live scan environment. That distinction (scan-live vs not) is only visible in `emit.cpp`'s chain walk (`scan_live` flag already exists ~2047, `SCRIP_SCAN_OFF` hatch ~1711), NOT at AST-classification time — which is exactly why every AST/lowerer gate (this session's Option B + last session's 3 narrowings) fails. Next session: work `emit.cpp` where α-entry codegen for an `IR_DISJUNCTION` selector can consult `scan_live` and preserve R13/R14/R15 across the fresh entry (save/restore the scan-env triple, same shape as `bb_gen_scan.cpp`'s scan-env swap), OR emit a scan-preserving α-entry variant for disjunction selectors reached inside a scan chain.
  **ARTIFACTS THIS SESSION (fresh container — prior session's /home/claude/rep*.icn and icon-master/bin are GONE):** Option B WIP saved `/home/claude/ICN-CASE-ALT-SELECTOR-OPTB-WIP.patch` (+ `.c` copy). Repro `/tmp/rep_alt.icn`. Oracle rebuilt from the uploaded icon-master source at `/home/claude/work/icon-master/icon-master/bin/{icont,iconx}` (v9.5.25a). Full self-host toolchain built at `/home/claude/jt/` (patched `jtran` SEGVs, `jtran_base` = baseline clean, both 5081992 bytes; `jlink`, `jcon.zip` all present). Gate: 241/20/32; `rung37_math_hello` FAIL is the known env float-format drift (Cluster A), not code.
