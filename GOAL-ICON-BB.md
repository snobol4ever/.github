...
- **🟢 JTRAN-ASTIR-ILLEGALTYPE CLOSED — TWO SET-VALUE BUGS FIXED (2026-07-22, Claude Sonnet 4.6) — SCRIP commit pending push. Icon 241/20/32 (zero regression). NEW BLOCKER: JTRAN-PARSE-LISTLIT.**
  **BUG 1 — `copy(set)` returned table:** `by_name_dispatch.c` copy() DT_T branch copied `dflt`/`init`/`inc` but NOT `is_set`. `ir_copy_inuse(inuse)` (called in ir_a_If) produced tables from set-valued ir_stacks fields; `ir_union_inuse` then tried `set ++:= table` → Error 1. **Fix:** `nt->is_set = src.tbl->is_set;` (one line).
  **BUG 2 — `++:=/--:=/**:=` on field lvalues lowered to integer add:** `augop_code()` in `lower_icon.c` had no cases for `AUGOP_CSET_UNION/DIFF/INTER` → `default: return 0` → BINOP_ADD. Fast-path (TT_VAR lhs) used `icn_augop_binop_tt()` which DID have the cases, so plain-variable `a ++:= b` worked; field lvalues (`x.tmp ++:= y.tmp`) took the general path → Error 1. **Fix:** three cases: `AUGOP_CSET_UNION→19, _DIFF→20, _INTER→21` matching BinopKind enum in `gen.h`.
  ...
  **🔴 NEW BLOCKER — JTRAN-PARSE-LISTLIT:** SCRIP-jtran's `parse` stage fails on multi-element list literals. `keyword.icn` (17 lines, `set(["features","allocated","collections","regions","storage"])`) → `Expecting ]` with garbage line number (3572). Minimal: `["a","b","c"]` → `Expecting ]`; `["a"]` → `invalid character "t"`. Segfault on parse stage. Coord/state corruption in compiled-jtran's lexer→parser coexpression handoff. yylex stage is clean on all these inputs.
  **NEXT SESSION START:** (1) `make scrip && make libscrip_rt` (both `-O0`). (2) Oracle: `icon-master/bin/icont` (already built if container persists; else `make Configure name=linux && make`). Oracle jtran: `cd /home/claude/ojt && make jtran`. (3) SCRIP-jtran: `bash scripts/jcon_selfhost_build.sh` steps 5–6 (or reuse `/home/claude/jt/jtran` if container persists). (4) Attack **JTRAN-PARSE-LISTLIT**: MONITOR-FIRST on `preproc lst.icn : yylex : parse` where `lst.icn` = `procedure main(); local t; t := ["a","b","c"]; write(*t); end`. Bracket where oracle-jtran parse and SCRIP-jtran parse diverge on this token stream.
