...
- **🟡 JTRAN-SELF-HOST IN PROGRESS — FOUR SCAN FIXES APPLIED (2026-07-22 session 3, Claude Sonnet 4.6) — SCRIP commit pending push. Icon 241/20/32 (zero regression). preproc stage NOW BYTE-IDENTICAL to oracle on irgen.icn (56660 bytes). Pipeline advances to yylex but fails on `\\ident.field` backtrack bug.**
  ...
  **BUG E — conjunction backtrack bypasses cursor-restore β for `str <- ="."` (`lower_icon.c`):** `="."` desugars to `tab(match("."))` (icon_parse.c:294-297). In `str <- ="." & str ||:= tab(many(&digits))`, when `tab(many(&digits))` fails, the conjunction's fail-jump routes to the `if`-statement ω, bypassing `tab(match("."))`'s β that would restore r14. **Fix:** `icn_tree_is_cursor_mover()` predicate marks conjunction operands that move the scan cursor (direct `tab`/`move` calls, `TT_REVASSIGN` whose RHS is a cursor-mover); the conjunction wiring routes a later operand's failure back through these operands' β. **Status: fix in place but yylex still reports `invalid character: "c"` at irgen.icn line 27 — the conjunction node being patched may not be the one actually executed for this case. NEXT SESSION must verify this by dumping the emitted asm for lex_yylex0's dot-conjunction and checking whether the fail-path now routes through n104_β (REVASSIGN restore) and then n81_β (tab restore).**
  **BUG E ALTERNATIVE APPROACH (if conjunction fix insufficient):** Make `bb_scan_tab`'s β path also call `rt_scan_sync_out(r14)` to publish the restored r14 into `scan_pos`. This is unconditional and covers any β-reachable tab restore regardless of conjunction wiring. File: `src/templates/bb_scan_tab.cpp` line ~41 (`x86("mov", "r14", FRQ(_.op_off+16))` → add sync_out after restore). Same for `bb_scan_move.cpp`.
  **CURRENT STATE:** SCRIP-jtran built at `/home/claude/jt/jtran_scrip`. Pipeline: preproc ✅ BYTE-IDENTICAL; yylex ❌ `invalid character: "c"` at irgen.icn line 27 (`c := (\p.coord | ir_coordinate("", 0, 0))`). Oracle emits ~100 `.class` files to `tmp_oracle/`; scrip emits 0.
  **NEXT SESSION START:** (1) `cd /home/claude/SCRIP && make -j$(nproc) scrip` (130s, -O0). (2) Recompile jtran: `cd /home/claude/jt && env SCRIP_BETA_ELIDE_OFF=1 LD_LIBRARY_PATH=/home/claude/SCRIP/out /home/claude/SCRIP/scrip --compile dump.sc.icn do_ops.sc.icn preprocessor.sc.icn lexer.sc.icn ast.sc.icn parse.sc.icn ir.sc.icn keyword.sc.icn irgen.sc.icn gen_bc.sc.icn gen_symbolic.sc.icn gen_dot.sc.icn gen_ucode.sc.icn optimize.sc.icn interface.sc.icn bytecode.sc.icn jtran_main.sc.icn > jtran.s && gcc -no-pie jtran.s -L/home/claude/SCRIP/out -lscrip_rt -Wl,-rpath,/home/claude/SCRIP/out -o jtran_scrip`. (3) Verify preproc still PASS: `./jtran_scrip preproc /home/claude/jcon-master/tran/irgen.icn : stdout | wc -c` should be 56660. (4) **Attack Bug E (lexer \\ident.field backtrack):** In `jtran.s`, find `proc_lex_yylex0_α` (~line 47683). Grep for the `"."` single-char string literal and trace the conjunction node — verify `jn[i]` for the `tab(many(&digits))` operand now routes to the REVASSIGN β (which routes to the tab(match(".")) β that restores r14). If not: apply the bb_scan_tab β sync_out fix (see BUG E ALTERNATIVE above), rebuild scrip+jtran, test `./jtran_scrip preproc /home/claude/jcon-master/tran/irgen.icn : yylex : devnull` for clean exit. (5) Once yylex clean, run full pipeline: `./jtran_scrip preproc irgen.icn : yylex : parse : ast2ir : bc_File -O -class:lirgen -dir:./tmp_scrip/` and byte-compare vs oracle `tmp_oracle/` (already populated from this session).
