...
- [ ] **GZ-11+ — corpus features rebuilt stackless** (lists, tables, records, scanning, csets,
  ...
  - [~] **`bb_to` STAGED-DORMANT (Sonnet 2026-06-03, uncommitted working tree):** `bb_to.cpp` stackless int-range
    generator + `flat_drive_to` + `walk_bb_flat`/`walk_bb_node` dispatch + Makefile, all landed and compiling; box pumps
    correctly in isolation. BLOCKED on the bodyless-`every` re-pump back-edge (`flat_drive_every` ival==0 arm must thread
    body-success → generator-β, per canonical `ir_a_Every`); `IR_TO`/`IR_TO_BY` kept in `icn_kind_native_stub` (honest
    EXCISE) until that lands — see the NEXT block in the Watermark for the full diagnosis + fix.
...
**HEAD (SCRIP) = `8570c97` — ICN-HY-4: stackless `bb_to` int-range generator, STAGED-DORMANT.** Wrote the stackless `bb_to` generator template (`src/emitter/BB_templates/bb_to.cpp`) on the `test_icon.c` `to1` shape: α seeds cursor from the lo operand slot, internal-label loop `x86("def"/"jmp", L(0))` tests `cursor > hi → ω` else yields `cursor → γ`, β steps (`inc` for `to`, `add` for positive-int `to_by`) and loops; reads operands `FRQ(op_sa+8)`/`FRQ(op_sb+8)`, result DESCR + cursor in ONE 24-byte frame block at `[r12+op_off]` (cursor at `op_off+16`). Pure `x86()` (template-purity gate clean); ZERO new encoders. Wired: `flat_drive_to` (emit_bb.c — resolves lo=`bb_slot_get(α)`, hi=`bb_slot_get(β)`, result+cursor via `bb_slot_alloc16`+`bb_slot_claim(8)`), `walk_bb_flat`/`walk_bb_node` `IR_TO`/`IR_TO_BY` dispatch → `bb_to`, both Makefile lists. **DORMANT BY DESIGN — `IR_TO`/`IR_TO_BY` KEPT in `icn_kind_native_stub`:** the box pumps correctly in isolation but bodyless `every E` lacks the `body.success → expr.resume` re-pump back-edge (canonical `ir_a_Every`, irgen.icn 325-330); un-stubbing made `every write(1 to 3)` print only `1` (SILENT MISCOMPILE, 22 corpus EXCISED→FAIL), so it stays EXCISED until `flat_drive_every`'s `ival==0` arm threads the generator-β re-pump (full diagnosis + fix in the NEXT block below). **ZERO REGRESSION vs `e09dcc2`:** m2 corpus **130 (HARD)**, Icon smoke m2 **12/12 (HARD)**, Prolog smoke m2 **5/5 (HARD)**, m3/m4 PASS **6** EXCISED **59** (baseline, symmetric m3==m4). Icon-lane gates green: bb_bin_t=0 · no-handencoded=0 (`--strict`) · g_vstack=0 · no-stack 10≤127 · one-reg-frame 0≤21 · prove_lower2 PASS. (`scrip.c` unchanged from `e09dcc2`; the `medium-invisible --strict` RED at `bb_builtin.cpp` is the unrelated Prolog-lane WIP, untouched.)
