...
- *Frame plumbing* (`xa_flat.cpp`, `g_icn_frame_active`): prologue `push r15; mov r15, rdi` (frame base ζ = zeta arg); epilogue `pop r15` (`41 5F`). **Register is r15 today; ratified target is r12** — do the R15→r12 move as an isolated byte-neutral step FIRST (frame is currently loaded-but-unused, so all current smokes must stay green; verify SNOBOL4 pattern templates' r12 DATA-ptr use does not collide on the Icon path).
...
| **SMX-4** abort/delete SM lowering | NEXT | Delete the ~589 `SM_emit*` sites in lower.c (non-Icon SM lowering — already dead at runtime via SMX-1). Then delete the SM machinery files: `sm_interp.c`, `sm_native.c`, `sm_codegen.c`, `sm_image.c`, `emit_sm.c`, `SM_templates/*`, and finally the `SM_t`/`SM_sequence_t` structs in SM.h + the `sm` field in stage2_t. Keep LOW-LEVEL reusable fns (emit_core byte/label/patch primitives, rt_* runtime helpers) per Lon. Build green + Icon 6/6 after each cut. |
| **SMX terminal** delete | folds into SMX-4 | Once `SM_t`/`SM_sequence_t` and all SM-traversing code are gone, death ratchet → 0. "ALMOST" → "completely". |
