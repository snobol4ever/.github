...
- **BENCH STATUS (fresh run this session — trust this over GOAL-ICON-FULL-PASS.md which is stale):**
  - deal/ipxref/queens/rsg: **byte-identical both modes** · m4 vs JCON: ipxref 0.23×, rsg 0.34×, deal 0.38×, queens 0.48× (all faster than JCON)
  - concord/tgrlink: compile+run clean now (segv fixed), but **output suppressed both modes** (generator-in-call-arg pre-existing divergence — not new)
  - geddump: **ABORT both modes** — `Too many GC root sets` (Boehm GC limit, shared runtime, pre-existing)
  - MODE34-IDENTICAL: **holds on all 7** (m3 output ≡ m4 output on every benchmark)
- **DECISIVE REPRO (still failing):** `every write("98765" ? upto(&digits))` → `1` then stops ❌ (want `1 2 3 4 5`). Explicit-subject `every write(upto(&digits, "98765"))` → `1 2 3 4 5` ✅ unchanged.
- **CORRECTED FIX DESIGN (from 2026-07-14 s3 — still accurate, not re-derived):**
  - **SCRIP uses push/pop env stack, NOT canonical's swap.** `rt_scan_enter` pushes, `rt_scan_leave` pops. Re-entry must re-push the inner env via a new runtime primitive `rt_scan_reenter()`, which pops from a parallel `scan_saved[]` save-stack that `rt_scan_leave` populates before popping. This is the **4th site** the prior cursor missed.
  - **`IR_t` has no β field.** α/β targeting is the `.sz` tag on `IR_ref_t`. The body-β label is delivered via the `g_emit.lbl_t0` channel — the **proven `IR_LIMIT` idiom** (lines 1770-1773 of `emit.cpp`): for an `IR_SCAN` leave node, resolve `operands[1]` (the body value node) in the node list and grab `betas[k]`. Then `x86_jmp_tgt(X86T_TGT0)` in the leave β arm jumps there. **5th site: a block in the emit BFS loop, mirroring the LIMIT block.**
  - **Making `IR_SCAN` generator-kind flips internal body→leave edges to β.** Fix: interpose α-forcing `IR_GOTO` trampolines between the body and the leave nodes (the `lower_proc_body` lines 983-986 idiom), built with `lc_γ_to`/`lc_ω_to` (not `build()`, which would auto-stamp β again).
- **THE HANG-REGRESSION (narrowing guard required):** `every write("abcde" ? while move(2))` hangs with the 6-site fix without the guard. **The narrowing guard:** in site-4a (emit BFS loop), only install `g_scan_body_beta` when `operands[1]->op` is `ir_is_generator_kind`. Compound-body scans fall back to `lbl_t0=NULL` → β falls through to ω (no re-pump, terminates). One line added to site-4a.
- **6-SITE FIX MAP (for next session):** (1) `ir_query.c`: `IR_SCAN` → `ir_is_generator_kind`. (2) `lower_icon.c` TT_SCAN: build `succ_tramp`/`fail_tramp` = α-forced IR_GOTO nodes; lower body to trampolines; set `cx->beta = leave_succ`. (3) `gen_runtime.h`: declare `rt_scan_reenter(void) → ScanSubjRegs`. (4) `gen_runtime.c`: add `scan_saved[SCAN_STACK_MAX]`/`scan_saved_depth`; `rt_scan_leave` saves inner env to `scan_saved` before popping; `rt_scan_reenter` pops from `scan_saved`, re-pushes outer, restores inner. (5) `emit.cpp`: declare/reset `g_scan_body_beta`; in BFS loop for `IR_SCAN` with `ir_is_generator_kind(operands[1]->op)`: resolve `betas[k]` → `g_scan_body_beta`; in IR_SCAN drive arm: set `g_emit.lbl_t0/lbl_t0_p = g_scan_body_beta`. (6) `bb_gen_scan.cpp`: extern `rt_scan_reenter`/`rt_scan_sync_in`; leave β arm: `IF(_.lbl_t0 != 0, call rt_scan_reenter; sync_in; jmp_tgt(X86T_TGT0))`.
