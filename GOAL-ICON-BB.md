...
- **HEAD rung:** RESUME-THROUGH-SCAN / generator-in-scan. **SCRIP tree stays at `ed95a693` — this session's fix was REVERTED after uncovering a hang-regression (see below).** Design is now fully specified and the regression is precisely bracketed; next session implements the corrected narrowed guard.
