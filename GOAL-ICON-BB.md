...
- **✅✅ JCON-SELFHOST-SET-OPS + WHILE-ALT (2026-07-21, Claude Sonnet 4.6) — SCRIP `(see commit)`. Icon 241/20/32 (zero regression vs baseline — FAIL set byte-identical before/after both fixes). TWO FIXES landing; ONE BLOCKER newly diagnosed. See `FINDING-2026-07-21-CLAUDE-JCON-SELFHOST-SET-OPS-AND-WHILE-ALT.md` for full detail. SELF-HOST BASE CASE PROVEN: `hello, world` runs end-to-end — SCRIP-built jtran→jlink→JVM matches iconx oracle and canonical JCON-JVM. Smoke 14/14 ×2. Gates no_stack/one_reg/semicolon PASS.** **FIX 1 (ICON SET OPS):** `++`/`--`/`**` on SET operands (`DT_T with is_set`) were routing through the cset string path — `set -- set` returned 0 (oracle 2), `set ++ set` returned 10 (oracle 3), etc. Root: `BINOP_CUNION/CDIFF/CINTER` are overloaded in Icon (cset op when operands are csets/strings, SET op when both are sets — canonical `oset.r`). SCRIP only implemented the cset arm. Added `set_union`/`set_diff`/`set_inter` helpers in `src/runtime/aggregates.c` (declared in `core/core.h`); both call sites patched: `src/runtime/arithmetic.c` and `src/runtime/by_name_dispatch.c`. This is what made the JVM NPE (`<local18> is null`) disappear — the bytecode preamble (`getstatic v$write$ / NewLocal("write") / astore`) is now emitted correctly because `bc_createvars`' `undeclared = bc_var_set -- params -- locals -- statics` set-difference now works. **FIX 2 (WHILE/UNTIL ALTERNATION):** `while A | B do` / `until A | B do` entered the body ONCE then exited (the condition's disjunction was RESUMED on loop-back instead of RESTARTED from α). Root: `lower_while`/`lower_until` in `src/lower/lower_icon.c` wired the body's back-edge directly to `centry` (the condition), which β-promoted when `centry` is a disjunction/generator. Fix: added α-force trampoline `CENT = build(IR_GOTO)` with `lc_γ_to_α(CENT,centry)` / `lc_ω_to_α(CENT,centry)`, lowered body targeting `CENT`. Mirrors `lower_every`'s mark→body α-force pattern exactly. **BLOCKER (DIAGNOSED, NOT FIXED): `===` (BINOP_EQV) as an alternation ARM.** `if a===5 | a===6 then` → SCRIP "N", oracle "Y". `==` and `=` in identical alt position both work; single `===` works; 3-arm `===` works; 2-arm fails. Root is a malformed IR operand graph in lower_alt × binop composition for EQV — `--dump-ir-verbose` shows the two `BINOP_TEST` nodes cross-wiring into each other and the DISJUNCTION node itself (node ordering / operand push sequence differs for EQV vs SEQ). Line 308 `opfail` ω-override tightening had no effect (reverted). The fault is in how binop+alternation compose for BINOP_EQV specifically; next session must trace the node-construction path for the 2-arm EQV case in `lower_alt` × the binop site (lines 898-908) to find the cross-wire source. **Why it matters:** `parse.icn`'s `parse_locals` is `while parse_tok_rec === lex_LOCAL | ... === lex_STATIC do` — so any Icon program with a `local`/`static` declaration fails to compile through SCRIP-jtran ("Expecting end"). hello_t (no locals) is why the base case works. **WORKDIR STATE:** `/home/claude/jt/` has 17-module jtran (505K asm, 5MB binary, clean parse.icn), jlink, jcon.zip, hello_t.icn proof. Oracle at `work/icon-master/icon-master/bin/icont+iconx` (v9.5.25a) and `work/jcon-master/jcon-master/bin/`. refs/ symlinked. **NEXT SESSION:** Fix `===`-in-alternation lowering bug (lower_alt × binop, 2-arm EQV cross-wire), rebuild jtran, confirm `local`-declaration programs compile and run through self-host, widen proof to set-exercising and loop-heavy programs, byte-compare vs oracle, run rung suite. The `===` fix is the single gate blocking self-host on all non-trivial Icon programs.**
- **✅ ICN-LOCAL-SHADOW (2026-07-20, Claude Sonnet 4.6) — SCRIP `37c45277`. Icon 241/20/32 (zero regression — stash/rebuild/diff proves FAIL set byte-identical, same 20 names). ONE-LINE FIX in `lower_icon_resolve_call_kinds()` (`src/lower/lower_icon.c`): added `if (graph_has_local(g, vn)) continue;` before the `IR_VAR→IR_PROC_VALUE` rewrite. ROOT CAUSE (corrects the prior finding's mis-diagnosis): the rewrite pass checked params + assignment targets for local-shadowing but omitted declared locals (`lnames`). A record field like `a` (from `record u_cset(a)` in `gen_ucode.icn`) makes `rt_builtin_is_known("a")` return true via `rt_dat_field_of_any()` — so a declared-but-unassigned local `a` was rewritten to the field-accessor proc-value → `type(a)="function"`, `\a` wrongly succeeding, clobbering parse values. Fix matches canonical Icon scoping: a declared `local`/param always shadows any global/builtin/field-accessor (verified against JCON `irgen.icn` ir_a_Ident). NOTE: prior finding's stated mechanism (`is_global("a")` true → emit gate fires) was WRONG — instrumentation showed `is_global("a")==0` and the IR_VAR slot-grant was never reached for `a`; the rewrite is in the LOWER pass, not the emitter. VERIFIED: repro `type(a)=null`, `image(a)=&null`, `\a` fails, multi-field and assigned-local cases correct; m3==m4 byte-identical. Icon smoke 14/14 both modes (libscrip_rt.so built). Gates no_stack/one_reg/semicolon PASS. **JCON SELF-HOST: ast2ir blocker CLEARED. NEXT SESSION:** (A) Build the 17-module jtran in `/home/claude/jt` (recipe: corpus/programs/icon/jcon-compiler/ modules + semicolonize + `SCRIP_BETA_ELIDE_OFF=1`); (B) Run pipeline through ast2ir — confirm `ir_Function` emitted for hello_t.icn; (C) Advance bc_File → .u1/.u2 → jlink → run; (D) Byte-compare SCRIP-jtran ucode vs icont-jtran; (E) Three-way benchmark (iconx / JCON-JVM / SCRIP-jtran) on `irgen.icn`. ALSO OPEN (from prior cursor): `type()`/`image()` for record constructors return `"function"`/`"function <n>"` instead of `"procedure"`/`"record constructor <n>"` — `by_name_dispatch.c` type() DT_E branch defaults "function", only upgrades named user procs; needs upgrade for constructor+builtin values. Pre-pinned regressions still open: geddump/tgrlink → `git revert 7aade169`; ipxref → LOWER-side lower_alt arm-interior BFS ordering fix.
- **✅✅✅✅ FOUR RESUME/VALUE FIXES FOR JCON SELF-HOST (2026-07-19, Claude Sonnet 4.6) — SCRIP `abce8b1d`. Icon 241/20/32 (environmental math drift vs prior cursor, same as baseline at HEAD — no regression). FOUR FIXES landing at once:** (1) **generator in non-last call arg** (`lower_call` `chain_live` gate) — `every f(!L,k)` now fully iterates; (2) **`\!gen` null-filter re-pump** (`IR_UNOP_TEST` ω→uβ) — `\!keys` continues past nulls; (3) **`image()` string→function upgrade removed** — `image("pos")`==`"pos"` not `function pos`; (4) **TT_NUL list-literal arm** — `[a,b,c,]` yields 4-elem list, not program death. All four ZERO regressions (stash/rebuild/diff). Together make SCRIP-run `interfacegen.icn` **BYTE-IDENTICAL** to icont oracle (415 lines, empty diff). **JCON SELF-HOST STATUS:** 17-module jtran (workdir `/home/claude/jt`) — pipeline `preproc:yylex:parse` all ✓; `ast2ir` BLOCKED by LOCAL-vs-RECORD-FIELD name collision: `parse_expr3`'s local `a` resolves to field-accessor `a` from `gen_ucode.icn record u_cset(a)` — `is_global("a")` true (field), `graph_has_local` returning false → `bb_var_global()` → accessor value. RENAME `a`→`zz_alt` PROVEN to unblock ast2ir. RELATED: `type()`/`image()` return `"function"`/`function a` for record constructors (should be `"procedure"` per canonical Icon). Both in FINDING-2026-07-19-CLAUDE-JCON-SELFHOST-FOUR-RESUME-FIXES-AND-LOCAL-VS-FIELD-COLLISION-BLOCKER.md. **CANONICAL ORACLES BUILT this session:** `icon-master/bin/icont+iconx` (v9.5.25a, oracle+perf#1) + `jcon-master/bin/jtran+jlink+jcon.zip` (oracle+perf#2) — all verified. **NEXT SESSION:** (A) Fix local-vs-field-accessor in emit IR_VAR / graph_has_local / slot-grant gate; (B) Fix type()/image() for constructors; (C) Advance ast2ir → bc_File → .u1/.u2 → jlink → run; (D) Three-way benchmark (iconx / JCON-JVM / SCRIP-jtran) on Makefile bmark (compiles irgen.icn).
