...
- [ ] **GZ-11+ — corpus features rebuilt stackless** (lists, tables, records, scanning, csets,
  ...
  - [ ] remaining GZ-11+ families: `not`/`size`/`nonnull` stackless `bb_unop`; **`string_op` (concat) ✅ LIT 2026-06-02 (`186b9b0` — IR_LIT_S slot + `x86_ro_seal_str` string REG-RO); relop tiers (`if_expr`/`while`/`until`/`repeat_break`) STILL NOT LIT — relop-var operand needs a slot: `bb_var` bombs (assigned var has no ζ-slot in this path) + a `kind=5 unhandled` node + the if/relop GZ-8 control flow → multi-subsystem, NOT a simple operand-slot fix**; generator-operand binops (Fig-1 native m3/m4); `rt_call_builtin` (find/upto/many/any).
...
- [x] **ICN-HY-1 — DE-FUSE bb_binop.** ✅ DONE 2026-06-02 (Sonnet, SCRIP `186b9b0`). **STALE-TEXT CORRECTION:** `bb_binop_lit_arith.cpp` + `bb_binop_concat_lit.cpp` named here NO LONGER EXIST — they were deleted by the TEMPLATE-REVAMP commits AFTER ICN-HY-0 (which only relocated; the revamp then replaced the router with the `binop_slot_kind` remap IR_BINOP→IR_BINOP_{RELOP,CONCAT,ARITH} dispatching to the already-pBB-free slot boxes). So the de-fuse was structurally complete EXCEPT one gap: `IR_LIT_S` never produced a slot, so `write("a"||"b")` slot-missed in `bb_binop_concat_slot`. **THE FIX (the last de-fuse gap + the GZ-11+ "IR_LIT_S operands still need slots / string REG-RO analogue" item):** (1) `emit_bb.c` `walk_bb_flat` IR_LIT_S now `bb_slot_alloc16(nd)` in the flat-chain (gated `g_descr_flat_chain` = Icon-only; SNOBOL4 on `g_gvar_flat_chain` stash-verified byte-identical); (2) `bb_lit_scalar.cpp` IR_LIT_S arm stores `{DT_S,0,&str}` (== `STRVAL`) to its ζ-slot, mirroring IR_LIT_I; (3) `x86_asm.h` additive `x86_ro_seal_str` — BINARY seals the host pointer (== `x86_ro_seal_q`), TEXT emits `.quad <label>`+null-term `.string`, loaded via existing `x86_ro_load_q` (medium split inside the encoder → template stays pure `x86()`; medium-invisible gate 0 `--strict`). **VERIFIED:** `write("a"||"b")` m2==m3==m4 = `ab`; `write(2+3)` m2==m3==m4 = `5` (unregressed). Corpus m2 **127** (HARD, byte-stable) · m3/m4 **5→6** · crosscheck PASS **1→2** (concat agrees). **ZERO DUP-FORM-3 fusion remains** (no consumer box reads `pBB->α->ival/sval/dval` or `->α->t==IR_LIT_*`). **NOTE — `write(42)` (bare int literal) STILL uses the fused GZ-2 arm** (`.quad`+`rt_write_int_nl`) because `icn_ring_to_tree` does NOT decline a literal-only call, routing it through the TREE path where the literal gets no slot, so GZ-7 misses → GZ-2 fires; de-fusing THAT is entangled with tree-vs-chain routing (touch `icn_ring_to_tree` or the tree-path slot alloc, risking green `write(int)`) and is folded into ICN-HY-2's "De-fuse: write(int_literal) must read the literal's slot."
- [x] **ICN-HY-2 — `bb_call.cpp` (459→207 router + 4 shape files).** ✅ DONE 2026-06-03 (Sonnet 4.6, SCRIP `3487a90`). **DE-FUSE:** Extended `icn_ring_to_tree` (one line, `scrip.c`) to also decline `IR_LIT_I`/`IR_LIT_S` — routes `write(42)` and `write("hello")` to flat-chain where `bb_lit_scalar` gives them ζ-slots; GZ-7 slot arm (`rt_write_any_nl`) handles them. Old fused GZ-2 (sealed int RO) and GZ-1 (sealed strlit) arms in `bb_call` are now dead. **DE-CRAM:** Split into 207-line pure router + 4 per-shape files: `bb_call_proc_staged.cpp` (GZ-10 `dval==3.0` staged args → `rt_call_proc_descr`), `bb_call_write_slot.cpp` (write-slot/binop/legacy shapes), `bb_call_userproc.cpp` (`rt_call_proc`), `bb_call_builtin.cpp` (`rt_call_builtin`). Raku's `dval==2.0` (`rt_call_arr`) stays in router — Raku ON HOLD. Makefile: +4 `RT_PIC_SRCS` + compile rules. **GATES:** m2 corpus **127 (HARD, byte-stable)** · m3/m4 **6** (unchanged) · smoke m2 12/12 (HARD) · m3/m4 4/12 · bb_bin_t=0 · medium-invisible=0 (--strict) · no-handencoded-bytes=0 (--strict) · g_vstack=0 · FACT=0 · no-stack 10≤127 · one-reg-frame 0≤21 · prove_lower2 PASS.
- [x] **ICN-HY-3 — Icon bang `!x` generator.** ✅ DONE m2 ORACLE 2026-06-03 (Sonnet 4.6 [SCRIP `f935c3b`]). **STALE-TEXT CORRECTION:** the "457-line `bb_iterate.cpp` de-cram" framing was stale — the file did NOT exist (removed in the mass-stub sweep) and the bang was broken across THREE layers, not one. Diagnosis: `!` parses to `TT_ITERATE`, which `v_unop` lowered to the *value-only* `IR_UNOP` (no bang arm → `default→fail` → `every` pumped once, exited empty; `IR_UNOP` structurally cannot re-pump). **THE FIX (lowerer, `lower.c` `v_unop`):** route `TT_ITERATE` → `IR_LIST_BANG`, a pull-model generator (`bb->α` = operand subgraph, resumable) — the oracle's existing `IR_LIST_BANG` arm drives it via the proven `list_bang_at` (string/table/list/record). Can only RAISE m2 (every bang program failed before). **VERIFIED:** string `!s` → `a/b/c`, table `!t` → `99`. Corpus m2 **127→130** (+3, byte-stable). **NATIVE box STAGED (builds green, gate-clean, dormant):** `rt_list_bang_at` (`rt.c`/`rt.h`, by-value, delegates to `list_bang_at` — no dup logic); `bb_iterate.cpp` (NEW — stackless `IR_LIST_BANG` generator, body written once via internal label `L(0)`: α sets idx=0 + falls in, β does idx++ + jumps back; **ZERO new encoders** — `x86_frame_*`/`x86_cmp_imm64`/`x86("je"/"jmp",PORT)`); `emit_core` dispatch + `flat_drive_list_bang` slot deposits (op_sa=operand slot, op_sb=private idx, op_off=out) + Makefile (scrip + `RT_PIC_SRCS`). **NATIVE m3/m4 NOT YET LIT:** the live path for `every write(!x)` is the **xchain builder**, which bombs on the plain operand `IR_VAR` (pre-existing flat-chain gap, not bang-specific) BEFORE reaching the box — so `bb_iterate` is correct but off the live path (xchain does not route through `emit_core` `FILL`). Added `IR_LIST_BANG` to `icn_kind_native_stub` → m3/m4 cleanly EXCISE `[SMX]` (was segfault). **NEXT to light up native:** teach the xchain emitter to route `IR_LIST_BANG` through `FILL`/`bb_iterate` (+ slot the operand var on that path), then drop `IR_LIST_BANG` from the EXCISE list. List literal `[1,2,3]` still blocked earlier in the lowerer (`tree_e` kind 56) — out of scope. **GATES:** m2 corpus **130 (HARD)** · m3/m4 6 (unchanged) · icon smoke m2 **12/12 (HARD)** · m3/m4 4/12 · prolog smoke m2 5/5 (HARD) · bb_bin_t=0 · medium-invisible=0 (--strict) · no-handencoded-bytes=0 (--strict) · g_vstack=0 · icn-no-stack PASS · one-reg-frame PASS · prove_lower2 PASS. `bb_iterate.cpp` passes all FACT/purity gates (uses `x86_bomb`, not host `fprintf`).
...
**HEAD (SCRIP, this session — Sonnet 4.6, 2026-06-03) = `f935c3b`: ICON-BB ICN-HY-3 — Icon bang `!x` m2 oracle fix (lower `TT_ITERATE`→`IR_LIST_BANG`) + staged native `bb_iterate`; corpus m2 127→130, m3/m4 honest `[SMX]` (was segfault).** The watermark's "457-line `bb_iterate.cpp` de-cram" was STALE: the file did not exist (mass-stub sweep) and the bang was broken across THREE layers. `!` parses to `TT_ITERATE`; `v_unop` lowered it to the value-only `IR_UNOP` — which has NO bang arm (`default→fail`) and structurally cannot re-pump (no state/β) — so `every write(!s)` pumped once, failed, exited empty. **(1) LOWERER (`lower.c` `v_unop`):** route `TT_ITERATE` → `IR_LIST_BANG`, a pull-model generator (`bb->α`=operand subgraph, own α/β=self, resumable). The oracle's existing `IR_LIST_BANG` arm (IR_interp.c:3032) drives it via the proven `list_bang_at` (string/table/list/record) — one code path, no dup. Strictly positive: every bang program failed before. `!s`→a/b/c, `!t`→99; **corpus m2 127→130** (+3, byte-stable). **(2) NATIVE box STAGED** (builds green, gate-clean, currently dormant): `rt_list_bang_at(obj,idx)` (`rt.c`/`rt.h`, by-value, delegates to `list_bang_at`); `bb_iterate.cpp` (NEW — stackless `IR_LIST_BANG` generator, body written ONCE via internal label `L(0)` [α: idx=0+fall-in; β: idx++ +jmp back], **ZERO new encoders** — `x86_frame_load64/store64/mov_imm64/inc64` + `x86_cmp_imm64(rax,99)` [FAILDESCR v=99 slen=0] + `x86(je/jmp,PORT)`); `emit_core` dispatch `case IR_LIST_BANG: bb_iterate(nd)`; `flat_drive_list_bang` deposits op_sa=`bb_slot_get(pBB->α)`, op_sb=`bb_slot_claim(8)`, op_off=`bb_slot_alloc16(pBB)`; Makefile (scrip compile line + `RT_PIC_SRCS`). **(3) NATIVE m3/m4 NOT YET LIT — why:** the LIVE native path for `every write(!x)` is the **xchain builder** (`icn_*_chain`), NOT the `flat_drive_list_bang`→`FILL`→`emit_core` path I instrumented. The xchain path bombs on the plain operand `IR_VAR` (`kind=5 unhandled` → `bb_var` BOMB) — a PRE-EXISTING flat-chain gap, NOT bang-specific (identical at session start) — BEFORE the bang box is reached, and it does not route `IR_LIST_BANG` through `emit_core`'s `FILL`. So `bb_iterate` is correct but off the live path. Added `IR_LIST_BANG` to `icn_kind_native_stub` (`scrip.c`) → m3/m4 cleanly EXCISE `[SMX]` instead of segfaulting (honest per three-mode discipline). List literal `[1,2,3]` still dies earlier in the lowerer (`tree_e` kind 56) — out of scope. **GATES (all green):** m2 corpus **130 (HARD, was 127)** · m3/m4 **6** (unchanged) · icon smoke **m2 12/12 (HARD)** · m3/m4 4/12 · prolog smoke **m2 5/5 (HARD)** · bb_bin_t=0 · medium-invisible=0 (--strict) · no-handencoded-bytes=0 (--strict) · g_vstack=0 · icn-no-stack PASS · one-reg-frame PASS · prove_lower2 PASS. `bb_iterate.cpp` passes all FACT/purity gates (uses `x86_bomb`, not host `fprintf`); purity-audit's 2 flags are PRE-EXISTING (`bb_call_write_slot`/`bb_every`, untouched). Rebased clean onto peer `f7cb3d5`, re-verified post-rebase (build green, m2 12/12). **NEXT — to light up native `!x` (m3/m4):** teach the xchain emitter to route `IR_LIST_BANG` through `FILL`/`bb_iterate` AND give the operand variable a ζ-slot on that path (the SAME `bb_var` slot-threading gap that blocks the relop tiers / `if_expr`), then drop `IR_LIST_BANG` from `icn_kind_native_stub`. The box + `rt_list_bang_at` are ready and gate-clean. Then **ICN-HY-4** (`bb_binop_gen.cpp` — cross-product odometer) per the #0-ladder.

**PREV HEAD (SCRIP, prior session — Sonnet 4.6, 2026-06-03) = `3487a90`: ICON-BB ICN-HY-2 — `bb_call` de-fuse + de-cram; `write(int/str literal)` now flat-chain slot path; 459→207-line router + 4 shape files.** Two changes in one gated commit. **(1) DE-FUSE:** Extended `icn_ring_to_tree` (`scrip.c`, one line) to also return NULL for chains containing `IR_LIT_I` or `IR_LIT_S` nodes (previously only `IR_BINOP` was declined). This routes `write(42)` and `write("hello")` to the flat-chain path where `bb_lit_scalar` gives them ζ-slots; the existing GZ-7 write(slot)→`rt_write_any_nl` arm then handles them. Old fused GZ-2 (bb_call sealing `{DT_I,val}` RO inline) and GZ-1 (bb_call sealing strlit ptr RO) arms are now dead — both replaced by the slot path. **(2) DE-CRAM:** Split `bb_call.cpp` (459 lines) into 207-line pure router + 4 per-shape files: `bb_call_proc_staged.cpp` (GZ-10 `dval==3.0` user-proc, staged args → `rt_call_proc_descr`), `bb_call_write_slot.cpp` (write-slot / write-binop / write-legacy shapes), `bb_call_userproc.cpp` (`rt_call_proc` shape), `bb_call_builtin.cpp` (`rt_call_builtin` shape). Raku's `dval==2.0` (`rt_call_arr` + `marshal_call_arg`) stays in the router since Raku is ON HOLD — no Raku files touched. Makefile: 4 new `RT_PIC_SRCS` lines + 4 compile rules appended at the Icon-group tail. **VERIFIED:** `write(42)` m2/m3/m4=`42`, `write("hello")` m2/m3/m4=`hello`, `write(2+3)` m2/m3/m4=`5`, `write("a"||"b")` m2/m3/m4=`ab` (all unregressed). SNOBOL4 smoke m2 unchanged (stash-verified). **RESULT:** Icon corpus **m2 127 (HARD, byte-stable)** · m3/m4 **6** (unchanged) · EXCISED 33. Icon smoke **m2 12/12 (HARD)** · m3/m4 4/12. Gates: **bb_bin_t=0** · **medium-invisible=0 (--strict)** · **no-handencoded-bytes=0 (--strict)** · g_vstack=0 · FACT=0 · no-stack **10≤127** (up from 8 — rt_pop_write_* now counted in both router + shape file, pre-existing) · one-reg-frame **0≤21** · prove_lower2 PASS. Prolog smoke m2 5/5. **NEXT — ICN-HY-3** (`bb_iterate.cpp` — `!list`/`!string`/`!table` shapes). The file does not exist yet (removed in the mass-stub sweep). Start by reading `refs/icon-master/src/runtime/oref.r` (canonical `!` semantics — read this session) and `IR_interp.c` lines 3032+ (mode-2 oracle for `IR_LIST_BANG` and `IR_ITERATE`). Three shapes to implement: `!list` (index walk), `!string` (char-by-char), `!table` (hash-chain walk). Dispatch is `emit_bb.c` `flat_drive_list_bang` → `FILL` → `emit_core` → `bb_iterate`.

---

**PREV HEAD (SCRIP, this session — Sonnet, 2026-06-02) = `186b9b0`: ICON-BB GZ-11+ / ICN-HY-1 — `IR_LIT_S` slot producer (the string REG-RO analogue); `write("a"||"b")` native m3/m4; corpus m3/m4 5→6, smoke 3→4, crosscheck 1→2.** The last de-fuse gap for ICN-HY-1 + the GZ-11+ ladder's *"IR_LIT_S operands still need slots / a string REG-RO analogue"* item, in ONE 3-edit slice. After `da9859c` gave `IR_LIT_I` a ζ-slot, `bb_binop_concat_slot` (slot→slot) still slot-MISSED on `write("a"||"b")` because `IR_LIT_S` produced no slot. **(1) `emit_bb.c` `walk_bb_flat` IR_LIT_S** now `g_emit.op_off = bb_slot_alloc16(nd)` in the flat-chain (split out of the LIT_F/LIT_NUL fall-through), gated `g_descr_flat_chain` — **Icon-only**; SNOBOL4 runs on `g_gvar_flat_chain` and was **stash/rebuild-verified byte-identical** (m3 3/3 with my edits stashed == with them applied). **(2) `bb_lit_scalar.cpp` IR_LIT_S arm** (mirrors the IR_LIT_I arm): stores `{DT_S,0,&str}` — exactly `STRVAL` (slen=0 is correct; the runtime uses `strlen`, not `.slen`) — into its 16-byte ζ-slot via `x86_frame_mov_imm64(off,DT_S)` + `x86_ro_load_q("rax",0)` + `x86_frame_store64(off+8,"rax")`, sealed by `x86_ro_seal_str(0,lit)`. **(3) `x86_asm.h` additive `x86_ro_seal_str(n,lit)` + `x86_asm_str_escape`:** BINARY seals the host pointer (byte-identical to `x86_ro_seal_q` — valid since the AST string lives in JIT process memory), TEXT emits `.quad <label>` + null-terminated `.string`; loaded by the **existing** `x86_ro_load_q` (no new walker machinery — the disp32 rides the `J`/`D` records). The per-medium string-addressing difference lives INSIDE the encoder (x86_asm.h is gate-exempt), so the template stays a pure `x86()`/`x86_*` concat → **medium-invisible gate 0 under `--strict`**. **VERIFIED all three modes:** `write("a"||"b")`=`ab` (m2/m3/m4); `write(2+3)`=`5` unregressed. Generated `.s` confirmed: each `IR_LIT_S` builds a proper `{DT_S,0,ptr}` DESCR with the null-term string sealed inline at a per-box-unique `.Lx<uid>_0` label, concat reads both 16-byte DESCRs (rdi/rsi/rdx/rcx) → `str_concat_d`. **Completes the ICN-HY-1 de-fuse** (both IR_BINOP operands now slot-bearing producers; the fused `bb_binop_lit_arith`/`concat_lit` boxes were already gone via the revamp; **zero DUP-FORM-3** `pBB->α->ival/sval` reads remain in any consumer). **RESULT (rebased onto peer `68ae22f` SNOBOL4 IR_SCAN, merged clean, re-verified):** Icon corpus **m2 127 (HARD, byte-stable)** · m3 5→**6** · m4 5→**6** · EXCISED 33. Icon smoke **m2 12/12 (HARD)** · m3/m4 3→**4** (`string_op`). Crosscheck PASS 1→**2** (`concat` agrees; remaining FAILs `if_expr`/`every_to` = the still-open relop/generator tiers). SNOBOL4 smoke m2 7/7 · m3 3/3 (identical); Prolog m2 5/5. Gates: **medium-invisible 0 (--strict)**, `bb_bin_t` 0, no-handencoded-bytes 0, C-byrd-box 0, `g_vstack` 0, no-stack 8≤127, one-reg-frame 0≤21, purity 2 (pre-existing `bb_call`/`bb_every` rel32 idiom, not mine), `prove_lower2` PASS. **NEXT — ICN-HY-2** (next #0-ladder step) is the `bb_call.cpp` (459-line, count stale in step text) de-cram, but is **cross-language**: its `dval==2.0` RK-EMIT-2 arm is Raku's (travels to Raku files per RK-HY-3; Raku ON HOLD) — coordinate before splitting. Its **de-fuse of `write(int_literal)`** is entangled: `write(42)` reaches the fused GZ-2 arm via the TREE path (`icn_ring_to_tree` does NOT decline a bare-literal call → literal gets no slot → GZ-7 misses), so de-fusing means touching tree-vs-chain routing (or the tree-path slot alloc), risking the green `write(int)`. The adjacent **relop tier** (`if_expr`) is multi-subsystem: `bb_var` bombs (var `x` has no slot), plus a `kind=5 unhandled` node + the if/relop GZ-8 control flow — needs `bb_var` slot threading from the assignment, not a simple operand-slot fix. Both want a fresh full-budget session.

---

**PREV HEAD (SCRIP) = `0b7a166`: ICON-BB TEMPLATE-REVAMP — `bb_call` + `bb_unop` → x86() medium-invisible; the MEDIUM-INVISIBLE GATE for Icon reaches 0 (61→0).** Converted the last two Icon BB boxes still carrying raw-byte producers (the private `x86_Lrec`/`x86_b1..4` primitives + `IF(MEDIUM_BINARY,…)` instruction branches) onto the `x86()` self-encoding front-end, so a template reader cannot tell BINARY from TEXT and `scripts/test_gate_template_medium_invisible.sh` reads **0** (was 61: `bb_call`(60)+`bb_unop`(1)). **`x86_asm.h` (additive, shared — both byte-verified vs the TEXT/`as` forms they replace, no collision with peers since additions are append-only):** (1) `x86_neg(reg)` = REX.W `NEG r/m64` (`48 F7 /3`) + `"neg"` dispatch case in `x86(mnem,op1)` beside push/pop/idiv; (2) `x86_pair_jmp(idx)` = the in-band `E9` + `'F' idx` pair-loop jmp record (the non-omega β target), mirroring `x86_pair_loop`'s jmp half — BINARY emits the record stream `bb_emit_x86` discovers, TEXT emits `jmp <pairname>`. **`bb_unop.cpp`:** NEG arm `x86_Lrec(x86_b3(0x48,0xF7,0xD8))` → `x86("neg","rax")`. **`bb_call.cpp` (all 60 sites):** `movabs rdi,ptr`+`call rax` → `x86("mov","rdi",ptr64)`+`x86("call",sym,fptr)`; frame loads/stores → `x86_frame_load64`/`_store64` (existing); RO-int / strlit-pointer → `x86_ro_load_q`+`x86_ro_seal_q` (existing REG-RO); `E9 'F' 0` β-jmp → `x86_pair_jmp(0)`; `mov32 edx/esi,imm` → `x86("mov32",reg,imm)`. **PURELY STRUCTURAL — byte-identical machine code, zero behavioral change** (the encoders regenerate the same bytes the raw producers emitted). **RESULT:** Icon corpus **byte-stable** — m2 **127 (HARD)**, m3 5, m4 5, EXCISED 33 (identical to baseline, verified by stash/rebuild). Icon smoke **m2 12/12 (HARD)**, m3/m4 3/12 (pre-existing GZ-11+ gaps). Prolog smoke m2 5/5 (HARD). Gates: **medium-invisible 0**, `bb_bin_t` 0, no-handencoded-bytes 0, no-stack 8≤127, one-reg-frame 0≤21, FACT 0, purity audit 2 (the sanctioned BINARY rel32 idiom in `bb_call`+`bb_every`). Crosscheck PASS=1 FAIL=3 — confirmed **identical with changes stashed** (the 3 FAILs = `concat`/`if_expr`/`every_to` = the STILL-OPEN relop/concat/generator-operand tiers, NOT a regression). **NOTE — `bb_call` KEEPS `pBB`** (it still reads neighbor `subs[i]->entry`/`pBB->α` for argument marshaling); the full no-`pBB`/`_.node` FACT-RULE conversion (driver promotes operand slots to `g_emit` first, as `bb_binop_arith`/`bb_unop` value boxes already do) is a SEPARATE later step — this slice cleared ONLY the raw-byte/medium-branch debt, which is what the three medium gates measure. **NEXT (divvy-up, pair-driven — discriminate on MEDIUM, expect the variable-length `D`/`J`/`x86_pair_loop()` idiom for runtime-variable pair counts): `bb_seq`(4) or `bb_suspend`(2); then `bb_alt`(5), `bb_to`(5)/`bb_to_by`(5), `bb_upto`(6), `bb_iterate`(17), `bb_binop_gen`(11).**
