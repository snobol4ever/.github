...
- **NEXT LEVER (diagnosis complete — full writeup in HANDOFF-2026-06-27-CLAUDE-ICON-BB-IS-RESUMABLE-BINOP-GEN-PREREQ.md):** lower a generator-wrapping binop as `IR_BINOP_GEN` (already a generator-kind everywhere; currently produced by NO lowerer) so the chain walker routes its loop-back to the inner generator's β; the `bb_binop` β arm must jump to the operand-generator's β (model: `to_inner_gen_operand_k`, emit_bb.c:2377) instead of recomputing α. This is the same "binop operand node identity" frontier from 12f0656/61f8836 (the `descr_binop_set_slots` slot=-1 duplicate-clone tell). Closes rung13_alt_alt_filter + cross_arg_sideeffect + contributes to several rung36_jcon_* image(GEN)|"none" cases. Real lower+emit work, not a one-liner.
