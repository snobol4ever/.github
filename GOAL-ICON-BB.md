...
- **HEAD rung:** RESUME-THROUGH-SCAN / generator-in-scan. **Item 1 (the SCAN-NARY value contract) is DONE — LANDED `ed95a693`, HEAD on origin** (verified this session: commit ancestry + `bb_scan_sequence.cpp` on disk uses `scanseq_concat_chain()` → `str_concat_d` over each arm's own value slot `FRQ(zls_off(operand[2j+1]))`, NOT the `rt_substr` subject slice). The box is now correct for ALL arm configs, not just `=`/match: verified `tab(3)||tab(5)=hell`, `tab(4)||tab(2)=helel` (backward — the exact case the old slice got wrong), `move(2)||move(2)=abcd`, 3-arm `tab(2)||tab(4)||tab(6)=abcde`, all m3==m4. Same commit also fixed a MODE34-IDENTICAL `movsxd` frame-source divergence (new `x86_movsxd_frame` encoder). `IR_SCAN_ALTERNATE` box exists and its movsxd site is fixed by construction, but it is UNREACHABLE — no frontend constructor emits it; scan alternation still lowers to the generator-disjunction path. **NEXT rung = RESUME-THROUGH-SCAN — DIAGNOSED + BRACKETED this session (fix not started; needs fresh budget, land all parts together):** the first wall in the scan cluster is a generator inside a scan not re-pumping on backtrack. DECISIVE repro ladder: `every write(upto(&digits, "98765"))` (explicit-subject arg) → `1 2 3 4 5` ✅ but the SAME generator `every write("98765" ? upto(&digits))` → prints only `1` then stops ❌. So the bug is NOT in `upto`/`every`/`repeat`/`break` — it is the scan wrapper failing to resume its BODY generator. ROOT (mechanically located): `lower_icon.c` `TT_SCAN` (case at ~line 575): the body is lowered first (sets `cx->beta` to the body generator's resumable self), but the very next line does `cx->beta = ω`, DISCARDING the body's resume edge, and the scan's final `cx->beta` (~line 593) is taken from the SUBJECT's beta only — never the body's — so a consumer's β never reaches the body generator through the scan. CANONICAL cross-check: `refs/jcon-master/tran/irgen.icn` `ir_a_Scan` (the `/bounded & ... p.ir.resume` chunk, ~line 74-77) does `ir_ScanSwap(oldsubject,oldpos)` then `goto p.body.ir.resume` — exactly the body-resume edge SCRIP drops. Also: `IR_SCAN` (the leave/wrapper box) is NOT in `ir_is_generator_kind` (`ir_query.c` — only the LEAF `IR_SCAN_UPTO/FIND/MANY/BAL` are), so the routing helpers can't treat the wrapper as resumable either. FIX SHAPE: make the `IR_SCAN` leave-success wrapper resumable when its body is a generator — on β, swap the scan env back in (the `oldpos`/`oldsubject` restore = canonical `ScanSwap`) then jump to the body's β; expose THAT (not the subject's beta) as the scan's `cx->beta`. Likely needs the `bb_scan` leave TEMPLATE to grow a real β arm, not a lowering-only change. Land lowering + template + registry TOGETHER — partial (e.g. registry-only) historically regresses/loops. Then the rest of the leverage list: (2) `htprep`+`prepro` parse errors (`;` inside function-call context); (3) `bb_assign_local`/`bb_assign_global` flat-chain BOMB (geddump/ipxref, `TE-4`); (4) `:=:`/`<->` swap; (5) proc-generator-as-argument BENCH-F3. Fragile per prior boards — monitor-first per RULES.md.
- **WATERMARK:** SCRIP `ed95a693` on origin (HEAD; = `.github` `84aaa118`). Freshly re-baselined this session: Icon ladder **239 PASS / 15 FAIL / 35 XFAIL**; icon smoke **14/14** m3+m4; prolog smoke 5/5 all modes; gates `icn_no_stack` 0 / `icn_one_reg_frame` 0 / `icn_local_no_nv` PASS / `icn_semicolon_required` PRISON (3 locks). Fail set: `rung36_jcon_{args,coerce,endetab,fncs1,htprep,kwds,mffsol,mindfa,prepro,recogn,scan,scan1,scan2,string,var}`.
- **LAST SESSION:** 2026-07-13 — the `ed95a693` session landed the movsxd fix + item-1 value-slot concat (byte-identical fail set, box dormant in all corpus programs so no rung flipped). Then a re-orientation session (Claude Opus 4.8) verified all of the above against git + the live tree and CORRECTED THIS BLOCK: the prior s2 cursor still read "item 1 = NEXT rung, tab/move produce WRONG values," which was already false — item 1 landed hours later at `ed95a693` but was recorded ONLY as a watermark appended at the BOTTOM of this file (line ~590), never promoted to this cursor. Textbook STALE-ORIENTATION (RULES.md): newest state belongs at the top; that watermark is still misfiled below.
