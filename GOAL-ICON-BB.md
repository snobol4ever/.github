...
| LFJ-15b | AG-pure consolidation: fold the AG-pure intercept branches from `lower_icn_expr_threaded_b` (Families 3/5/6/7/8.1/8.2) INTO their corresponding `lower_icn_new_<KIND>` functions so each new function returns an AG-pure-shaped node directly. After LFJ-15b: `lower_icn_expr_threaded_b` keeps only its leaf-seeding (`icn_leaf`) and Family-1/Family-2 ATOMIC ASSIGN/CALL deep-thread paths; the AG-pure scrub-α/β + chain-via-γ + sval/ival-marker work moves to where the node is built. True "one traversal" — the chain walker's first visit sees AG-pure shape with no reshaping pass needed. Method: per family (one at a time, gates green each commit), pass `γ_in`/`ω_in` through to the new function for that kind, perform the scrub-and-wire there, retire the corresponding `if (e->t == ... && nd->t == ...) { ... }` block from `lower_icn_expr_threaded_b`. Acceptance: `lower_icn_expr_threaded_b` body has no `e->t == TT_*` switches outside the Family-1/2 paths and the leaf fallback; sval markers `"ag"`/`"ai"`/`"ar"` and `nd->ival = 1` are set by the new functions, not the wrapper. | rungs 198. |
...
**WATERMARK:** one4all `cde72b79`. Gates: smoke_icon 5/5 · broker 30 · rungs 198 · smoke_prolog 5/5 · FACT RULE 0. LFJ-15 complete (14 of 15 LFJ rungs · 93%). Next: LFJ-15b (AG-pure consolidation — fold `_threaded_b` intercepts into the new functions).
