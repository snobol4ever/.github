...
### ▶ NEXT SESSION — `rung36_jcon_lists`: FULLY DIAGNOSED, READY TO IMPLEMENT (runtime + lowering + one template arm)

**Symptom (2-line diff only):** with x = `[1,2,3,4,5,6,7,8,9]`, `x[-3+:6]` and `x[3-:6]` (both source-commented `# should fail`) must FAIL so the `| write("...wraparound failed")` alternative fires; SCRIP wrongly produces `[4] 3 4 5 6` for both. Everything else in lists already matches byte-for-byte.

**Canonical rule (definitive) — `refs/jcon-master/jcon/vDescriptor.java:282-305` SectPlus/SectMinus:**
- `x[i+:j]`: `end = i + j`; **wraparound iff `(-i ^ -end) < 0`** → FAIL; else `Section(i, end)` (= plain `x[i:end]`, which does cvpos on both).
- `x[i-:j]`: `end = i - j`; same check; else `Section(i, end)`.
- Why: Icon positions are two kinds — from-start `1,2,3…` vs from-end `0,-1,-2…`; wraparound = `i` and `end` are opposite kind, detected by sign-bit XOR of their negations. Equivalent boolean: exactly one of `i, end` is `≥ 1`.
- Verifies: `x[-3+:6]`→i=-3,end=3→`(3 ^ -3)<0`→FAIL ✓; `x[3-:6]`→i=3,end=-3→same→FAIL ✓. Normal forward slice `deck[k+:hs]` (k≥1,hs≥0)→both ≥1→no wrap→`Section(k,k+hs)` ✓ — **must NOT break deal; verify deal stays byte-identical.**

**Why SCRIP fails now:** `lower_icon.c` at BOTH section sites (~L177 lvalue path, ~L485 `case TT_SECTION*`) builds `IR_SUBSCRIPT` with `ival=0` and, for `sec_variant∈{1,2}`, inserts an `IR_BINOP` (`BINOP_ADD` for plus / `BINOP_SUB` for minus) computing `end = i ± len`, then pushes operands (base, i, end). So it ALREADY emits `Section(x, i, i±len)` — the ONLY missing piece is the wraparound check. `IR_SUBSCRIPT`(3-operand)→`bb_section()` (dispatch `emit.cpp:768`), whose rval arm calls `subscript_get2(arr,i,end)` (`pattern_match.c:268`) and **explicitly `x86_bomb`s if `ival != 0`** ("s[i+:n]/s[i-:n] pending"). CRITICAL: plain `x[-3:3]` and extended `x[-3+:6]` produce the IDENTICAL `subscript_get2(x,-3,3)` call — the runtime cannot distinguish them without a flag, so the variant flag is REQUIRED (plain must reorder+succeed, extended must wraparound-fail).

**Three-change plan (canonical-faithful, minimal):**
1. **`lower_icon.c` (both sites):** set `IR_LIT(sec).ival = sec_variant` (was `0`). `lower_icon.c` is NOT a codegen-regen trigger (only `lower_snobol4.c` is). AT-HANDOFF-UNFINISHED CHECK: verify no other pass keys on `IR_SUBSCRIPT.ival==0` for 3-operand sections — inspect `emit.cpp:1106` (`case IR_SUBSCRIPT` slot/operand pass) and confirm `lower_snobol4.c:169` builds only 2-operand subscripts (→ `bb_subscript`, never 3-operand `bb_section`), so the new `ival!=0` arm stays Icon-only.
2. **runtime `pattern_match.c` (near `subscript_get2`):** add `DESCR_t subscript_get2_ext(DESCR_t arr, DESCR_t i, DESCR_t end) { long ii=(long)to_int(i), ee=(long)to_int(end); if (((-ii)^(-ee))<0) return FAILDESCR; return subscript_get2(arr,i,end); }`. (This IS the canonical SectPlus/SectMinus; the canonical's pre-fail `Section()` "provoke error" call is a no-op for lists — cvpos FAILS, does not error — so returning FAILDESCR matches observable behavior.)
3. **`bb_section.cpp` template:** replace `if (_.op_ival != 0) return x86_bomb(...)` with a real arm identical to the plain rval arm but calling `subscript_get2_ext` and routing FAIL→ω (`cmp eax, DT_FAIL; je "ω"` — the SAME pattern the lvalue string arm already uses in this file). Add `extern DESCR_t subscript_get2_ext(...)` to the `extern "C"` block.

**Blast radius / MANDATORY verification (template + runtime touched):** icon suite fail-set diff (expect exactly `{rung36_jcon_lists}` removed); SNOBOL4 crosscheck **stash-round-trip** (should be byte-identical — `bb_section ival!=0` is Icon-only, `subscript_get2_ext` is a new fn only Icon calls); Raku/Rebus/Snocone/Prolog; icon+smoke; 3 gates; **all 4 regen scripts** (feature `.s` should show ~0 change since SNOBOL4 has no `+:`/`-:`; run `update_icon_bench_asm.sh` and **CONFIRM deal `.s` still byte-identical**). Then commit source + the standard regen commits; the staleness note above will not recur.

**After lists, next contained candidates (leverage order):** `string1` (wrong output, 54L, likely runtime-only); the keyword arms `kwds`/`keywords`/`neg_pos` (`&subject` assign + `:=:`/`<->` swap — lower+template, more involved); then the rc=139 segfault cluster (`args`/`endetab`/`fncs1`/`substring`/`table` — need minimal-repro bisection). Highest architectural leverage remains **RESUME-THROUGH-SCAN** (unblocks `scan_alt`+`parse`, prereq for scan-context generators) but it is the risky rung multiple sessions reverted — attempt only with fresh budget.
