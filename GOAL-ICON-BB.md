...
**▶▶ SCAN-VALUE CARRY-OUT — PRECISELY DIAGNOSED, NOT IMPLEMENTED. This is the 5-test cluster (scan, scan1, scan2, scan_alt, parse) and the next session's highest-leverage rung.** All five BOMB with either `bb_call marshal: IR_VAR arg names a local with no LOWER-granted varslot (TE-4)` or the paired `[TE-4]` assert. **The BOMB message is a red herring — the real gap is that a scan expression's value cannot be consumed by an outer box.** Minimal repro (no alternation needed): `write("abc" ? move(1))` BOMBs. Trace:
...
**LEVERAGE BOARD (supersedes the item-1..10 board in the same-day earlier watermark; #1 D-CORE now DONE):**
1. **SCAN-VALUE CARRY-OUT** (above) — unblocks scan/scan1/scan2/scan_alt/parse (5 tests). Highest leverage; full diagnosis + fix shape in hand.
