...
**HEAD (SCRIP) = `96d35539` (PUSHED)** — m3/m4 **PASS=211 FAIL=41 EXCISED=1** (289 total). icon smoke 12/12 m3+m4 · prolog 5/5 · snobol4 m4 7/7 · no-stack 0 · one-reg 0 · semicolon prison green · LVA gate PASS. FAIL sets byte-identical m3=m4.

**2026-06-27 (Claude Opus 4.8 — `seq()` generator implemented; rung30 GREEN both modes):**
- **GAIN:** `rung30_builtins_misc_seq` now PASSES both modes — `every write(seq(1) \ 3)` → `1 2 3`. Suite **PASS=210→211 FAIL=42→41 EXCISED=1**, byte-identical m3=m4. Rigorous stash/rebuild/FAIL-set diff: exactly one fix (`rung30` removed), zero regressions, zero swaps.
- **Root cause:** `seq` was unimplemented — fell through `lower_call` to a plain `IR_CALL` and aborted with "Undefined function or operation."
- **Fix (`src/lower/lower_icon.c`, +13 lines, ONE file):** new `lower_seq()` intercepted at the top of `lower_call`. `seq(from, by)` lowers to the existing `IR_TO_BY` generator with a SYNTHESIZED upper bound (`INT64_MAX` for `by>0`, `INT64_MIN` for `by<0`) — exactly canonical semantics (`refs/icon-master/src/runtime/fmisc.r:523` itself bounds `seq` at `MaxLong`). Reuses the proven `to`-generator + `\`-limit machinery: NO new emitter box, NO new dispatch, NO new resume wiring. Default `from=1`/`by=1` honored; `by==0` and variable (non-const) step safely RETURN NULL → fall through to the existing (erroring) call path, never emitting garbage. Verified standalone: `seq(5)→5,6,7`, `seq(2,3)→2,5,8,11`, `seq(10,-2)→10,8,6` (negative step), `seq()→1,2,3`.
- **Rebased** cleanly onto the concurrent `cf8e44b8` (idx_set base-slot fix); revalidated full suite + smokes + gates on the rebased tree (`96d35539`).
- **NOTE — pre-existing bench-asm drift (NOT this change):** `corpus/benchmarks/icon/version.s` is stale vs HEAD codegen (`bb00001_α:`→`bb1_α:` label format from the prior dense-nid commit `5806b7eb`); `version.icn` does NOT use `seq`. Left untouched (focused-commit hygiene); needs a dedicated artifact-refresh pass to catch up the label-format drift across the corpus.
- **NEXT LEVER:** No further single-line wins remain (10+ FAILs inspected this session, all confirmed substantial). The next real progress requires committing to one of: the resume-spine redesign (≈6 rungs — find-gen β-vs-α routing, binop/augop-over-generator — the documented two-attempt minefield, needs a real plan not patches), the cset/math/integer keyword subsystem (rung37_keywords), positional-section `s[i+:n]`/`s[i-:n]` + the rest of the string-ops batch (rung36_jcon_string1, integration program — section alone flips nothing), or file-I/O builtins (`open`/`remove`).

**2026-06-27 (Claude Opus 4.8 — cross-arg non-generator call value-drop + over-eval FIXED; real Icon built as oracle; expected-file corrected):**
- **GAIN:** `rung13_alt_alt_cross_arg_sideeffect` now PASSES both modes — `every write(1|2, noisy(), 3|4)` → `[eval]/1X3/1X4/[eval]/2X3/2X4`. Suite **PASS=209→210 FAIL=42 EXCISED=1**, byte-identical m3=m4, zero regression (rigorous stash/rebuild/diff: exactly one gain, nothing moved).
- **GROUND TRUTH:** built real Icon 9.5.25a in `refs/icon-master/bin/` (NoGraphics) and ran the exact program. The committed `.expected` was WRONG (`[eval]` once); canonical `ir_a_Call` (`refs/jcon-master/tran/irgen.icn:360`, wiring `L[i].success → L[i+1].start`) re-enters every rightward arg FRESH when a leftward generator advances, so a non-generator arg runs once per value of any generator to its left → `[eval]` TWICE. Corrected the corpus `.expected` to match real Icon (canonical wins; rule beats benchmark).
- **Root cause (both bugs, one site):** `marshal_call_arg` (`bb_call.cpp:454`) caught EVERY `ir_is_call_kind` arg and routed it to `marshal_single_call` (inline RE-INVOKE) before reaching the slot-read at line 471 that generator args use. So the chained `write`'s middle non-generator call `noisy()` — already emitted as a spine producer box (`xchain5_n1_α`, result in `[r12+24]`) — was BOTH re-invoked at marshal time (over-eval: 2 spine + 4 marshal = 6 `[eval]`s) AND its spine result dropped (value `13` not `1X3`). The literal-middle sibling passes because a literal marshals from its RO constant.
- **Fix (`bb_call.cpp`, +1 line expanded):** when the call arg is the consumer's own (`owner == _.node`), is a STAGED USER-PROC kind (`IR_CALL_PROC_STAGED`/`USERPROC`/`GVAR_USERPROC`/`PROC_GEN` — NOT builtin/byname), and has a live spine slot (`bb_slot_get(lf) >= 0`), READ that slot instead of re-invoking. The user-proc gate is the principled separator: staged user procs are emitted as standalone spine producers (execute before the consumer's marshal); builtins (`char`/`ord` via `rt_call_arr`) are marshaled inline and MUST still re-invoke. The looser `bb_slot_get >= 0`-only gate regressed `rung28_builtins_str_char_ord` + `rung36_jcon_trim` (builtin call args with allocated-but-unpopulated slots) — tightening to user-proc kinds resolved both.
- **CHANGE SET (commit-ready, PUSH BLOCKED pending credential):** SCRIP `src/emitter/BB_templates/bb_call.cpp`; corpus `programs/icon/rung13_alt_alt_cross_arg_sideeffect.expected`. Icon bench `.s` regen: updated=0 (11 pre-existing compile-errs unchanged). No `.s` artifact drift.
- **NEXT LEVER:** 42 FAILs remain. Real Icon is now available in-sandbox as a true oracle for the whole ladder — candidate to wire into the suite (decision deferred to Lon). Nearest related frontier stays the alt-as-call-arg / richer-alternation family.

**2026-06-27 (Claude Sonnet 4.6 — is_resumable binop-over-generator PREREQUISITE; resume-spine frontier pinned):**
- Suite re-tallied at `fc64cac` (two commits past the `61f8836` watermark above): **m3/m4 PASS=208 FAIL=44 EXCISED=1**.
- Landed a behavior-neutral PREREQUISITE in `src/lower/lower_icon.c` (2 lines): `is_resumable()` now recognizes a binop/relop over a generator operand (canonical `ir_binary` re-drives `p.right.ir.resume`) and `x:=GEN` as resumable. Zero regression, **zero new pass** (identical FAIL set), byte-identical bench `.s` (the 11 bench compile-errs are pre-existing — confirmed by stashing + re-running on baseline). icon smoke 12/12 · prolog 5/5 · snobol4 7/7 · all 4 icn gates green.
- **WHY zero new pass:** the change makes the canonical backtrack ω-edge appear in the IR (`write.ω→relop`, `relop.ω→ALT`, verified), but those edges target FRESH (α) entries, not RESUME (β) entries. The chain walker (`emit_bb.c codegen_flat_chain_body`, ~line 3665) routes a loop-back to a target's β ONLY when the target `ir_is_generator_kind` (emit_bb.c:2366); a relop is plain `IR_BINOP` (not a generator kind), so `write.ω→relop` lands on the relop's α → recomputes `x>2` with the same x → ALT never resumed for 4,5. Repro `every (x:=(1|2|3|4|5))>2 & write(x)` → emits `3`, wants `3 4 5`; `every x:=(1|2|3) do write(x)` → `1 2 3` (plain alt-in-every is fine — only relop-over-gen drops results).

**2026-06-27 (Claude Sonnet 4.6 — every (A & B) conjunction-filter generators; SCRIP 654f720):**
- **GAIN:** `rung13_alt_alt_filter` now passes — `every (x:=(1|2|3|4|5))>2 & write(x)` → `3 4 5` (was `3` only). Suite **PASS=209 FAIL=43 EXCISED=1**, both modes, FAIL sets byte-identical. Baseline diff: exactly one gain, zero regressions, no swaps (rigorous stash/rebuild/diff confirmed).
- **Root cause:** `lower_every` no-BODY path had `gen_node` collapse to `E` when GEN was a monolithic `IR_CONJ` (`every A & B` parses as `every (A&B)`), because the conjunction reported `cx->beta == E` (rightmost element `write` is non-resumable). The buried generator (ALT via relop's ω) was never re-driven.
- **Fix 1 — `lower_icon.c` (+10/−9):** additive `icx_t.conj_resumable` field. Conjunction lowering records the β of the rightmost resumable element (the relop, which threads to the ALT) without touching `cx->beta`. `lower_every` uses `conj_resumable` as fallback `loop_target` when `gen_node` collapses to `E`. Enables 2-element conjunction filters.
- **Fix 2 — `emit_bb.c` (+1):** chain-walker ω-routing followthrough. A consumer whose `ω→IR_BINOP`(non-gen) whose own `ω→generator-kind` now routes to the generator's β instead of the binop's α. Enables 3+-element conjunctions (`every A & B & C`) and resolves the m3≠m4 asymmetry those exposed.
- **NEXT LEVER:** `rung13_alt_alt_cross_arg_sideeffect` — `every write(1|2, noisy(), 3|4)` — call-argument cartesian-product with once-only evaluation of non-generator args. IR mapped: two `IR_ALT` nodes as `write`'s args, `noisy()` in the middle with `ω→ALT1`. The failure is two-fold: the single-shot middle arg's value is dropped (shows `13` not `1X3`), AND `noisy()` is re-evaluated each resume. This is call-argument-evaluation machinery, not conjunction lowering — a different, harder subsystem. Suggested entry: trace how the m4 call-arg slot-map handles generator-kind args vs once-only args in `bb_call.cpp`.
...
**2026-06-27 (Claude Sonnet 4.6 — --interp eradication + reversible subscript assign):** (1) **--interp eradicated** — replaced every occurrence of `--run` everywhere across all three repos (SCRIP, .github, corpus): 126 HQ MD files, 155 shell scripts, Makefile, test files. Only `--run` and `--compile` survive. (2) **Reversible subscript assign `arr[i] <- v`** (canonical `oasgn.r` semantics — `GeneralAsgn(x,v); suspend x; on resume restore; fail`): rewrote `TT_REVASSIGN` lowering in `lower_icon.c` (subscript lvalue builds base/key/value as direct operands of `IR_RASGN` with `ival=1` discriminator, mirroring `IR_IDX_SET`); extended `flat_drive_rasgn` in `emit_bb.c` to detect subscript mode via `ival` and read each operand's owned result slot; new `bb_rasgn_subs()` arm in `bb_rasgn.cpp` calls `subscript_get`/`subscript_set` runtime helpers. Verified: backtracking generator `every (0=arr[r:=1 to 3]) & arr[r]<-1 do write(r)` yields `1 2 3`. (3) **Suite harness `m4run` stdin bug fixed** — `</dev/null` was hardcoded, starving compiled binaries of their stdin file; fixed. This revealed the true baseline: **PASS=212 FAIL=41** (was reading 208; the 4 `rung27_read_*` programs were always correct). (4) All 4 Icon discipline gates green, both template gates green, Prolog smoke 5/5 both modes, zero regression. SCRIP commit `a2d0385b`. **Remaining 41 FAILs are all dense JCON corpus programs requiring multiple advanced features each; no single-feature lever flips a whole program. The largest coherent next lever is the computed/indirect-invocation subsystem (`fn='?'/'o'/'p'/'goal'` — ~6 programs, `invoke.r` is the canonical reference). `rung15_real_swap_lconcat` is a bad test (`|||` on strings contradicts canonical `ocat.r`; should be XFAIL).**
