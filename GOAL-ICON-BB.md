...
- [x] **GZ-0 … GZ-SCAN — DONE (m2 oracle).** HELLO→scan ladder all landed: GZ-0 scaffold/gates; GZ-1 `write("hello")`,
  GZ-2 `write(42)`, GZ-3 `write(1+2)`, GZ-4 `every write(1 to 3)`, GZ-7 `x:=42;write(x)`, GZ-8 if/relop, GZ-9
  while/until/repeat+break/next, GZ-10 user proc (m2/m3/m4); GZ-5 alt, GZ-6 nested-gen Fig-1, GZ-11 suspend, GZ-SCAN
  `subj ? body` (m2 oracle only — native m3/m4 of alt/suspend/scan LOUDLY EXCISE via `icn_kind_native_stub`).
  Stackless throughout — RO `[rip+disp]`, RW `[ζ=r12+off]`, no value stack. corpus m2 reached 127 at GZ-SCAN. (Full
  per-rung detail in git log + `HANDOFF-*.md`.)
...
**HEAD (SCRIP) = `f935c3b` — ICON-BB ICN-HY-3: Icon bang `!x` m2 oracle fix + staged native `bb_iterate`.** `!` parses to `TT_ITERATE`; `v_unop` had lowered it to the value-only `IR_UNOP` (no bang arm, cannot re-pump) so `every write(!s)` exited empty. **FIX (lowerer `lower.c` `v_unop`):** route `TT_ITERATE` → `IR_LIST_BANG`, a pull-model generator (`bb->α`=operand subgraph, own α/β=self); the oracle's existing `IR_LIST_BANG` arm drives it via `list_bang_at` (string/table/list/record) — one code path, no dup. `!s`→a/b/c, `!t`→99; **corpus m2 127→130** (byte-stable). **NATIVE box STAGED but DORMANT:** `rt_list_bang_at` (`rt.c`/`rt.h`, delegates to `list_bang_at`) + `bb_iterate.cpp` (stackless `IR_LIST_BANG` generator, body once via internal label `L(0)`; ZERO new encoders) + `emit_core` dispatch + `flat_drive_list_bang` slot deposits + Makefile. **NOT YET LIT (m3/m4):** the live native path for `every write(!x)` is the **xchain builder**, which bombs on the plain operand `IR_VAR` (pre-existing flat-chain gap, not bang-specific) BEFORE the box, and does not route `IR_LIST_BANG` through `emit_core`'s `FILL`. Added `IR_LIST_BANG` to `icn_kind_native_stub` → m3/m4 cleanly EXCISE `[SMX]` (was segfault).

**GATES (all green):** Icon corpus **m2 130 (HARD)** · m3/m4 **6** · EXCISED 33 · Icon smoke **m2 12/12 (HARD)** · m3/m4 4/12 · Prolog smoke **m2 5/5 (HARD)** · bb_bin_t=0 · medium-invisible=0 (`--strict`) · no-handencoded-bytes=0 (`--strict`) · g_vstack=0 · C-byrd-box=0 · icn-no-stack PASS · one-reg-frame PASS · prove_lower2 PASS.

**NEXT — to light up native `!x` (m3/m4):** teach the xchain emitter to route `IR_LIST_BANG` through `FILL`/`bb_iterate` AND give the operand variable a ζ-slot on that path (the SAME `bb_var` slot-threading gap that blocks the relop tiers / `if_expr`), then drop `IR_LIST_BANG` from `icn_kind_native_stub`. Then **ICN-HY-4** (`bb_binop_gen.cpp` cross-product odometer) per the #0-ladder. Other open native targets: relop tiers (`if_expr`/`while`/`until`/`repeat_break` — need `bb_var` operand slots), generator-operand binops (Proebsting Fig-1 native, the `every write((1 to 3)*(1 to 2))` cluster — `icn_ring_to_tree` must decline `IR_BINOP_GEN` so they take the flat-chain path), `rt_call_builtin` (find/upto/many/any). GZ-DEFER (EVAL/CODE/`*P`) remains open.

*Full session history in `git log` + the `HANDOFF-*.md` files. Recent landed commits (newest first): `f935c3b` ICN-HY-3 bang · `3487a90` ICN-HY-2 bb_call de-fuse/de-cram · `186b9b0` ICN-HY-1 IR_LIT_S slot + string REG-RO · `da9859c` REG-RO + IR_LIT_I slot (`write(2+3)` native) · `0b7a166` bb_call+bb_unop medium-invisible (Icon revamp complete) · `cd6fbe2` IR_NOT/NONNULL/NULL_TEST/SIZE stackless arms · `10f6863` chain-entry sentinel + unary-minus + slot-concat.*
