...
- **🔴 JTRAN-NAME-BUILTIN (2026-07-22, Claude Sonnet 4.6) — SCRIP `02eb13d4`. Icon 241/20/32. SOLE BLOCKER for jtran symbolic-pipeline self-host.**
  **WHAT WORKS:** SCRIP runs `oplexgen.icn`/`interfacegen.icn` (generates 611/415-line modules ✓). SCRIP `--compile` of all 17 jtran modules → 505,166 asm lines, zero bombs, links to 5,081,992-byte `jtran` binary ✓. Oracle `icont`-jtran (v9.5.25a) is built at `/home/claude/ojt/jtran`. Pipeline stages `preproc:yylex:parse:ast2ir` all succeed individually ✓.
  **THE BLOCKER:** `name()` builtin is declared in the arity table (`by_name_dispatch.c:4637`) but **has no handler** — falls through to "Error 5 Undefined function or operation". Every jtran IR output path hits it via `ir_dump → dump_simple/dump_verbose → dump_get_names → name(!x)`. Confirmed by 4-line probe: `name(v)` on a plain local variable errors; oracle returns `"v"`. `variable()` (inverse) has a real handler at `by_name_dispatch.c:5734` — use as template.
  **FIX LOCUS — two layers:** (1) **Lowerer** (`lower_icon.c`): `name()`'s argument must be passed *by reference* as a variable descriptor carrying the identifier (not dereferenced). `byref_mask` plumbing exists in `stage2.h:31`; look at how `bb_var_ref.cpp` passes lvalues. (2) **Runtime** (`by_name_dispatch.c` near `variable()` handler ~5734): implement `name(arg)` — extract the name string from the variable descriptor across all forms: local/param → identifier string; global → NV name; subscript `s[i]` → `"s[i]"`; record field → `"r.field"`. jtran only exercises simple-variable form (loop vars, record fields), so a scoped first cut covering those is enough to unblock the pipeline.
  **ALSO LANDED THIS SESSION:** `type()` DT_E fix (`02eb13d4`): any procedure/function value now returns `"procedure"` (canonical `fmisc.r:1174` — unconditional). DATATYPE path byte-unchanged. Cleared audit probe `89_proc_value` both modes; zero rung regression.
  **REMAINING NON-BLOCKING:** `88_swap_lv` line 3: `s[2:4] :=: s[5:6]` gives `aedebc` not `aedbcf`. Section-swap of unequal-length substrings. NOT exercised by jtran (all jtran `:=:` uses are string literals in operator tables). Fix separately.
  **NEXT SESSION START:** (1) `make scrip && make libscrip_rt` (both already `-O0` by default). (2) Build oracle: `cd /home/claude/work/icon-master/icon-master && make Configure name=linux && make` → `bin/icont`. (3) Stage jtran: `SCRIP=/home/claude/work/SCRIP CORPUS=/home/claude/work/corpus bash /home/claude/SCRIP/scripts/jcon_selfhost_build.sh` steps 1–6 only (skip [7] JDK/jcon.zip per s121 permanent note). (4) Implement `name()` in the two layers above. (5) Verify: `$JT/jtran preproc /tmp/hello_semi.icn : yylex : parse : ast2ir : symbolic_File : stdout` produces output byte-identical to oracle jtran.
