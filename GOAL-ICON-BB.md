...
**Runner:** `bash SCRIP/scripts/test_icon_bench_corpus.sh` (`SKIP_BUILD=1` after first build; `ICONM=` overrides the icon-master path, default `/home/claude/icon-master`). Runs all 10 through the iconx oracle + SCRIP m3/m4 with per-program args/stdin mirroring jcon-master/bmark/Makefile. SCRIP `link` is a NO-OP, so link deps ride as extra source args (`SCRIP_LINK_DEPS` table in the script — mechanism verified live 2026-07-18: cross-file proc calls resolve). Grades exit status only; no JCON arm yet. 2026-07-18 fresh: oracle 10/10 · m3 2/10 · m4 2/10 (micsum, version).
...
**Live blocker (found 2026-07-18; owns ≥6/10 m3 bench failures):** under the RSP ζ default, mode-3 never populates `main(args)` — `src/driver/scrip.c` ~L1343-4 gates the `rt_args_list_from` store on `mf != NULL`, but `mf` is only alloca'd when `ZC_FRAME != ZC_FRAME_RSP`; main's args param is left an UNINITIALIZED DESCR (garbage → "Error 5 Undefined function or operation" inside `options()`, then SEGV). Mode-4 forwards args via its own asm wrapper (scrip.c ~L1038/L1217) — its bench failures are a separate issue. Minimal repro: `procedure main(args); write(*args); end` + `--run f.icn -- -n8 foo` → prints 0 (Arizona prints 2).
