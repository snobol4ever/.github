...
- **🟡 JTRAN-SELF-HOST IN PROGRESS — FULL PIPELINE RUNS; ONE CONSTANT-POOL ORDERING DIVERGENCE REMAINS (2026-07-23, Claude Sonnet 4.6). Icon 239/22/32 (empirical fresh-container re-derive; 9 of 22 FAILs are cosmetic real-number formatting `2.0` vs `2.` — not code bugs; see Cluster A below). STALE-CURSOR CORRECTION: the prior LIVE CURSOR described a SEGFAULT on multi-token yylex that NO LONGER EXISTS at HEAD — the Bug E fix was deliberately never committed, so HEAD is the pre-fix baseline where `x;`/`write("hi")`/`procedure main()\nend` all lex cleanly. DO NOT follow the "tackle tab-β-slot-lifetime regression" instructions from the prior cursor — that regression only exists in the uncommitted WIP, not in the repo.**
