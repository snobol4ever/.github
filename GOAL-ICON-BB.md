...
- **MAIN-BOOT** (Opus 4.7, 2026-05-28, `80d0d5ee`): smoke_icon **5/5** · smoke_prolog 5/5 · broker 35 · rungs **161** · FACT 0. Three coordinated fixes: (1) main-bootstrap `SM_CALL_FN "main" nargs=0` at lower epilogue; (2) every-loop exit_pc moved `BB_INVOKE.a[0]` → `a[2]` (a[0] is α/β state flag — silent collision); (3) `lower_return` Icon arm aligned with Raku (no VOID_POP — leaves retval on stack for `sm_eval_subexpr`); (4) `every E` with no SM_BB_INVOKE in gen_expr AND no body lowers as one-shot (was infinite loop on non-generator E like `every write(sum_to(5))`; previously masked by `icn_bb_pump_proc_by_name`'s `icn_bb_oneshot` wrapper). Remaining 72 FAILs are mostly generator-proc rungs (rung03 suspend_gen family, etc.) — SM_CALL_FN → sm_call_proc → sm_eval_subexpr is a one-shot run with no `g_current_generator_state` so SM_SUSPEND silently fails. Restoring generator-proc semantics on the SM_CALL_FN path is a follow-up step (needs GeneratorState wiring on the SM call side).
