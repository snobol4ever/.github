...
`scrip --ir-run tree.sc + lower.sc + parser_snobol4.sc + sm_interp.sc + <prog>.sno`
...
- All five run modes of scrip can execute the lowered SM_Program: `--ir-run`
  (tree-walk reference), `--sm-run` (C SM dispatch loop, default),
  `--jit-run` (SM→x86 bytes), `--monitor` (in-process comparator).
  Our `.sc` interpreter `sm_interp.sc` runs inside `--ir-run` of a scrip
...
| SL-13b | pattern captures `. var` / `$ var` fix    | --ir-run TT_CAPT_COND_ASGN/IMMED_ASGN handle plain TT_VAR        | 2026-05-12 |
...
Target output (matches `scrip --sm-run --dump-sm trivial.sno`):
```
; SM_Program  count=6
...
- [ ] Confirm SM dump byte-identical to C `--sm-run --dump-sm` for trivial.sno
...
Convert `scrip --dump-ir <file>` text output into an AST-builder `.sc`.
Format example:
```
