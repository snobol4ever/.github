...
| **E-3** | `xa_flat.cpp` | **20** (all r11) | Icon's OWN generator-suspend continuation (`rt_gen_save_wires` / `rt_gen_get_{gamma,omega}_wire`), the `ARCH-LANGUAGES.md` BB_PUMP model — a separate live mechanism, not γ/ω debris. Re-state it in the contract; do not delete it. |
