...
- **Edge ROLE beats target KIND.** `build()`/`γ_to` auto-stamp β whenever the TARGET is generator-kind — trustworthy ONLY for backtrack edges. A producer's SUCCESS edge into a generator-kind junction (LIMIT gate, REPALT, …) is α (enter fresh); LOWER must α-restamp it (`lc_γ_to`). Enforced at 4 sites: statement spine (guarded — `every` re-aims its own γ), TT_REPALT's γ, TT_LIMIT count→entry, TT_SEQ/SEQ_EXPR.
...
- limit-of-RepAlt operand shape (`(|1) \ 3`) → `op=32 drive_unowned` (rung14 passes; only the wrapping shape is unhandled).
- `while (i := i+1) <= 3 do write(|1)` → nothing, rc=0, mode-3 — one unclassified data point; MONITOR-FIRST before theorizing.
- unary-test-op (`\` / `/`) failing-`if` else-routing — pre-existing, own rung.
...
## Watermark
**2026-07-01 — SCRIP `a3de01d2` (local, push pending): `22_revassign` LANDED — AUDIT **66/66 both modes, FAIL=0, the wholesale ladder is COMPLETE**; `bb_operand_aux_*` API deleted repo-wide (operands/n_operands is the single operand store); `IR_RASGN→IR_REV_ASSIGN` + `IR_SECTION→IR_SUBSCRIPT` renames.** Corpus 190/63/36 of 289 — per-program FAIL set byte-identical to `c0e74d52` (`comm` diff) · smoke 12/12×2 · no_stack 0 · one_reg 0 · semicolon prison green · mutation gate **HARD=4** (pre-existing baseline) · icont/iconx oracle live · bench-asm baseline 13/0/1/12. Prior rung same day: `c0e74d52` (`54_section_plus` — canonical icont desugar + `subscript_get2` canonical position swap).
