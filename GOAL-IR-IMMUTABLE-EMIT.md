...
- [ ] **RESERVED-SET RECONCILE — census DONE (2026-07-02, whole-src grep excluding decl/name-table), Lon decides delete-vs-implement per enum:**
  ...
  |---|---|---|---|
  | `IR_EXEC` | 0 on live path (1 in PARKED lower_snobol4.c — inert) | **NONE** | **strongest DELETE candidate** |
  | `IR_UNREACHABLE` | 0 | ir_Unreachable (gen_bc:524) | `x86_bomb` already serves the role — delete w/ doc note, or trivial implement |
  | `IR_SCAN_SWAP` | 0 | ir_ScanSwap (gen_bc:254) | save/restore lives INSIDE IR_SCAN_ENTER — delete w/ doc note, or split out |
  | `IR_DEREF` | 0 | ir_Deref (gen_bc:125) | deref lives in rt value paths — same choice |
  ...
  | `IR_RESUME_VALUE` | 0 | ir_ResumeValue (gen_bc:551 — JCON's ToBy RESUME uses it) | **KEEP-reserved**: the TO-SPLIT rung may want it |
