...
## ⛔ LVALUE-COLLAPSE (rung filed 2026-07-02, Lon design review) — SCRIP's ir_rval
**Finding (canonical, irgen.icn:459):** JCON has ZERO per-lvalue-kind translator arms — every assignment-family op is one generic opfn; the entire lvalue policy is `ir_rval`, a 10-line (operator, arg-position)→deref table with variable-transparent ops (`[]` `!` `?` `/` `\`) INHERITING rval-ness from parent context; ONE runtime assign (GeneralAsgn) switches on variable type. SCRIP diverged (a) deliberately — classify-by-name, no generic opfn (JCON-ALIGNMENT's one exception, KEEP), (b) historically — DT_V born only 2026-07-02, the arms predate it, (c) missing the ir_rval organ — rval-ness decided ad hoc per construct, so ASSIGN/AUGOP/REVASSIGN/SWAP/NULLTEST each re-implement the producer→DEREF→write ritual (5 copies, DUP FORM 2/4). **The rung:** make `lower_lvalue_var` TOTAL (TT_FIELD via a ~15-line `rt_field_var`, the rt_subscript_var record-arm sibling; keywords stay declined-loud, own rung) → collapse TT_ASSIGN to {TT_VAR fast path, generic lv→ASSIGN_VAR} and TT_AUGOP to {TT_VAR desugar, ONE rider} → deletes the LV-2/3a/3b copies AND the two arms added 2026-07-02. Increment 1 (ITERATE/RANDOM into the shared front) LANDED `84208eba`.
