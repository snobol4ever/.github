...
**NEXT (in order):** (1) ~~**`every x:=GEN do BODY` assign-wrapped-generator regression**~~ **[CLOSED-AS-FIXED 2026-07-01 (Claude Sonnet, wholesale audit): no longer reproduces at HEAD — `every x:=7 to 9 do write(x)` → `7 8 9` (the documented hang repro), `every x:=1 to 3 do write(x)` → `1 2 3`, `every x:=1 to 3 do write(x*2)` → `2 4 6`, all icont-oracle-confirmed both modes (audit probes 32/33/35, toby 34/35). No session logged an explicit fix; some intervening commit (RUNG 3-5 / label-variable span) repaired it as a side effect. The feab99c7 bisection below stands as accurate history of the era it describes.]** — bisected this
...
### FROM-SCRATCH RESULTS (this session's arc)
First run 49/63 → probe fixes (oracle-caught) 53/66 → **code fixes 60/66**. Icon smoke 12/12 BOTH modes; rung14 limit suite **0/5 → 5/5**; gates: no_stack / one_reg_frame / semicolon GREEN, mutation HARD=4 (the documented pre-existing baseline — every emitter change below is a read/staging change, zero new mutation sites).

### FIXES LANDED — 4 construct families, 9 root causes, each JCON-verified
1. **`*'abc'` → 0 (csetlit).** `walk_bb_node`'s `op_sval` re-derivation allowlist was missing `IR_LIT_CHARSET` (the opcode added 2026-06-30 to fix the PREVIOUS union clobber) — the preamble nulled what `emit_drive` staged; template sealed an EMPTY string. gdb-bracketed at `rt_size_d` (rsi → ""). One token in `emit.cpp`. Note `many('ab')` masked this: scan ops bake `op_name1` compile-time, bypassing the literal box — the CHARSET box had never actually executed.
2. **`&line` → 0.** `lc_key` hardcoded `ival=0` because **`tree_t` had NO line field at all**. Added `int line` (`ast.h`, calloc-zeroed for every language), parser stamps it on ident/&keyword leaves (`icon_parse.c`, token line was already tracked), `lc_key` now takes the tree and bakes the occurrence line — **JCON `ir_a_Key` verbatim: `ir_IntLit(target, p.coord.line)` (irgen.icn:1271)**.
3. **`&pos` stale inside scan.** `g_scan_regs_live` was declared, initialized 0, and **never assigned anywhere** — `bb_keyword`'s correct `r14+1` register arm was unreachable dead code; the fallback `rt_keyword_pos` returns stale state (registers ARE the truth per the stackless design). Now a depth counter: `++` at `IR_SCAN_ENTER` drive, `--` at `IR_SCAN` (leave) drive; nests correctly; `&subject` rides the same flag. All 5 scan probes stay green.
4. **`every write((1 to 5) \ 2)` — SIX stacked diseases on one construct** (mode-3 CRASH → emission abort → empty output → correct, in that order of unmasking):
   a. `emit_drive` IR_LIMIT resolved the generator slot via `bb_slot_get` — −1 when the generator sits LATER in nodes[] order (this probe's parenthesized shape) → `drive_unowned` FATAL. Fix: `drive_value_slot(gen)` — **the tmp doctrine: `IR_t.tmp` IS the slot, pre-assigned, order-independent.**
   b. Identical disease in IR_TO's `op_sa/op_sb/op_sc` (from/to/by literal boxes after TO in nodes[]) → `bb_to` "needs static operands" bomb. Same fix.
   c. **LOWER never wired JCON's `limit.success → expr.ir.start` (irgen.icn:137)** — count.γ pointed at the GATE, and the computed generator entry was literally discarded (`(void)ge`). Runtime entered the gate with no value. Fix: wire-later `γ_to(lr, ge)`, the TT_LCONCAT idiom.
   d. **`bb_limit_init` had ZERO call sites** — its own header comment promised "emitted once in the chain pre-pass"; that pre-pass emission never existed. Counter = frame garbage ≥ t → instant ω. Wired into `codegen_flat_chain_body` pre-spine, beside the suspend resume-slot-init precedent.
   e. **Slot-grant collision:** IR_LIMIT fell through to the produces-value k+=1 grant (16 bytes); the NEXT node's tmp landed exactly on the counter (+16) — the count literal's DT tag (6) overwrote it, gate read 6≥t. Fix: coordinated **k+=2** in `ir_drive_slot_assign` (the IR_ITERATE pattern).
   f. **`op_ival` clobber, same class as (1):** walk_bb_node's preamble re-derives `op_ival = IR_LIT(nd).ival` (0 for a LIMIT node), stomping the count `emit_drive` staged from operand[1]; emitted gate compared against 0. walk now reads operand[1]'s literal for IR_LIMIT.
   g. **β mis-stamp on a PRODUCE edge — the varying-α/β finding.** `build()`/`γ_to` auto-stamp **β whenever the TARGET is generator-kind**. IR_LIMIT is (correctly) generator-kind for consumer-backtrack routing — so TO's SUCCESS edge into the gate got stamped β: emitted `TO.γ → LIMIT.β → TO.β`, a silent produce-pump spin to exhaustion, empty output, rc=0. **The port lives on the EDGE's ROLE, not the target's kind**: producer-success into a generator-kind junction is α (enter the gate fresh); only consumer-resume is β. LOWER re-stamps α (`lc_γ_to(er, lim)`), per JCON's expr.success-riding-through-the-gate. **EXPORTABLE RULE for every future junction construct (Case-unbounded, RepAlt, and every other language's equivalents): after lowering a producer whose γ lands on a generator-kind junction node, LOWER must α-restamp that edge; the kind-based auto-stamp is only trustworthy for backtrack edges.**
...
### REMAINING FAILURES — 6, classified, the next rungs (repros are the probes themselves)
- **08_initial** — `initial` clause: FIRST call prints init+body, SECOND call prints NOTHING (want init/body/body). IR_ENTER_INIT done-flag routing sends the whole body to ω on re-entry, not just the initial clause.
- **22_revassign** — `x:=1; if (x <- 2)&(1=2) then … else write(x)`: silent, zero output. NOT generic else-routing (probe 64, plain relop-fail if/else, PASSES — oracle-confirmed) — `<-` itself.
- **25_if_value_else** — `write(if 2<1 then "a" else "b")` prints nothing/wrong: the documented value-convergence disease; fix-shape = the alt ig shared-cell precedent, already spelled out in the label-variable close.
- **38_repalt** — `every |1 do {…}` body never runs (counter stays 0).
- **51_lconcat** — `[1] ||| [2,3]` prints garbage (8, then s): the BINOP_CONCAT type-dispatch route claim in TT_LCONCAT's comment does not hold for lists.
- **54_section_plus** — `s[2+:3]`: honest `bb_section` bomb, `+:`/`-:` arms genuinely pending. (Plain `s[i:j]` PASSES — see correction below.)

### MD CORRECTIONS (Lon's standing verify-and-correct directive) — all verified against the live tree/oracle, not recalled
- **The "NEXT (1)" every-assign regression is FIXED at HEAD**: `every x:=3 to 5 do write(x)` → 3 4 5, `every x:=7 to 9 do write(x)` → 7 8 9 (the documented hang repro), `every x:=1 to 3 do write(x*2)` → 2 4 6 — all oracle-confirmed, both modes, probes 32/33/35. The feab99c7 bisection narrative below stands as history; the bug no longer reproduces. Struck in place in the punch list.
- **"corpus rung14 limit programs still pass" was STALE at HEAD**: `test_icon_ir_rung_14_limit.sh` was **0/5 at session start — stash-proven** (stash → rebuild pre-session tree → 0/5 → pop). It is 5/5 after this session's LIMIT ladder. Any prior-session claim resting on rung14 green needs re-dating.
- **`op=32` re-verified = IR_LIMIT at the CURRENT enum** (RETURN=31, LIMIT=32, REPALT=33) — the prior "compiler-verified ordinal" survived the two renumberings by coincidence; re-verify after any future enum edit, as that entry itself warns.
- **The punch-list "TT_SECTION/…(s[i:j]) → IR_FAIL-stubbed" claim is PARTIALLY STALE**: plain `s[i:j]` and string subscript `s[3]` PASS both modes (probes 53/63, oracle-confirmed). Only `+:`/`-:` remain pending (honest bomb).
- **Probe-suite lesson worth keeping:** SCRIP's decl grammar rejects `;` between declarations (matches standard Icon) — the semicolon-required rule is about STATEMENTS. Recorded so the next probe author doesn't re-pay it.

### Watermark
**2026-07-01 (Claude Sonnet, wholesale audit) — audit instrument landed (66 probes, icont-oracle 4-way); 60/66 OK both modes; csetlit/&line/&pos/LIMIT fixed (9 root causes); rung14 0/5→5/5; smoke 12/12×2; HARD=4 baseline; remaining ladder: initial, revassign, if_value_else, repalt, lconcat, section_plus.**

### Continuation (same session, same directive): initial + lconcat LANDED; repalt groundwork; revassign classified — 62/66
- **08_initial LANDED.** `bb_enter_init`'s cmp/mov used a 4-arg `x86("cmp","qword",FRQ,imm)` form the dispatcher **silently drops** — the emitted gate had NO flag test and NO flag store (asm-verified: only jne+jmp survived). Rewritten to house 3-arg forms (load→cmp / plain FRQ store, the bb_limit pattern). **SILENT-DROP AUDIT TARGET, same family as the clobber pattern: `x86()` returning empty for an unrecognized arg shape is how both this and the phantom machinery below stay invisible — a default-bomb in the dispatcher would have caught it, but retrofitting one at 147-template scale is its own carefully-gated rung, not a drive-by.** Honest limitation recorded: the done-flag lives in the ζ frame, and frames come from `g_rt_frames[depth]` (static, zero-init, reused per depth) — so `initial` is once-per-DEPTH, not once-per-procedure; recursion or varying call depth re-runs it. Canonical once-ever semantics need writable static storage (own rung; probe 08 is same-depth and correct).
- **51_lconcat LANDED.** `str_concat_d` had NO list arm — TT_LCONCAT's "runtime type-dispatch handles lists" comment was false; both operands were stringified (hence `8`/`s` garbage). Added the both-lists arm (new list, a's elements then b's, canonical olist.r lconcat; DATCON list layout reused from MAKELIST). Non-list operands unchanged. `string_ops.c`; both mediums via libscrip_rt rebuild; probe 50 list_ctor regression-guarded green.
- **38_repalt — groundwork landed, construct still open, phantom named.** (a) BFS discovery never enqueued IR_REPALT's operands → the repeated expression was NEVER emitted, the intercept's e_entry scan missed, e_lbl degraded to node_ω, instant fail — **fixed** (both discovery loops, the ml.operand precedent verbatim). (b) **`flat_drive_repalt` is a PHANTOM: three comments cite it (emit.cpp:679/797/1247) as owning e's four edges; zero definitions, zero calls** — same species as bb_limit_init's promised pre-pass. The yield-copy and exhausted-restart fragments EXIST in bb_repalt.cpp, orphaned; the ~1460 intercept emits only clear+jmp-e+β-pump. Building flat_drive_repalt per its own design note (emit.cpp:672-696, JCON ir_a_RepAlt :206-229) is the rung: e.success→yield(copy,flag:=1,goto consumer-γ); e.failure→exhausted-check(flag ? restart : ω); β→e.β.
- **22_revassign classified: TT_REVASSIGN's lowering is a bare IR_FAIL placeholder** (dump-verified: `IR_FAIL [s128,s144]` with orphan operands) — a MISSING conversion, not a mis-wire. JCON reversible-assignment machinery → own rung.
- **Closing sweep: harness 62/66 both modes; smoke 12/12×2; rung14 5/5 held; no_stack/one_reg/semicolon GREEN; mutation HARD=4 baseline.** Remaining ladder (4): if_value_else (fix-shape documented), repalt (design note located), revassign (missing conversion), section_plus (pending arms).
- Drive-by sighting, recorded not chased: emitted proc preamble contains `lea r10,[rip+Δ]` — r10 is documented RETIRED in the register contract; someone should reconcile the preamble with REGISTER-LAYOUT's table.

### Continuation (Claude Fable 5, same day): 38_repalt LANDED — 63/66; the phantom is real; three more β-mis-stamp sites; REPALT slot grant
- **38_repalt LANDED, both modes, oracle-verified.** `flat_drive_repalt` built as a real static function (emit.cpp, defined above `drive_unowned`) — the three citing comments now name something that exists. Four stubs per JCON ir_a_RepAlt (irgen.icn:206-229, decoded from the refs upload): α clear+jmp-e ≡ `MoveLabel(t,fail);Goto e.start` · yield (`ra_y`: copy e_sa→off, flag:=1, jmp-γ) ≡ `MoveLabel(t,start);Goto success` · exhausted-test (`ra_t`: flag==1 ? je-α-restart : jmp-ω) ≡ `IndirectGoto(t)` · β → e-β gated on `ir_is_generator_kind(e_root)`, else ra_t (a non-generator's resume IS its failure — the ml ival=0 rule; also avoids referencing a never-defined β label). e_root's γ/ω are redirected onto per-node `ra_y`/`ra_t` labels by the edge-resolution scan — emit-time READ of the graph, never a write; mutation gate held at HARD=4.
- **The LIMIT lesson's predicted RepAlt export came due, three sites:** build()/γ_to's kind-based auto-β poisoned PRODUCE edges at (1) the statement spine (`lower_proc_body` — the executed sequencer — plus `TT_SEQ`/`TT_SEQ_EXPR`; symptom: `every |1` printed 0, `assign.γ → REPALT.β` entered the pump with a cold flag), fixed with a **guarded** `lc_γ_to` restamp (`val->γ.node == succ` only — constructs like `every` re-aim their γ internally and must not be rewired); (2) `TT_REPALT`'s own γ — auto-β into a generator-kind consumer (LIMIT) sent the yield into LIMIT's resume pump → infinite spin on `|(1 to 0)\3`; (3) `TT_LIMIT`'s count→generator-entry edge, live only when the entry is the REPALT node itself (a TO's entry is a literal, which is why rung14 never tripped it). Role lives on the EDGE, not the target's kind — now enforced at four sites total.
- **`ir_drive_slot_assign`: IR_REPALT `k+=2` grant added** (16-byte value copy + int64 `yielded` flag at +16, the IR_LIMIT/ITERATE shape). It previously fell through and ALIASED onto e_root's tmp — with e=TO the flag landed exactly on TO's counter, so TO's own α write (counter:=from) set the flag, the test read yielded, and `|(1 to 0)` restarted forever. gdb-bracketed live per the two-step: rip parked in TO's retest, from=1 to=0 counter=1 stable, clear's [128] == TO's counter slot.
- **Closing sweep: audit 62→63/66 both modes, every row oracle `=exp`** (icont built from Lon's icon-master upload — plain `Configure name=linux`, no-X; `X-Configure` needs xpm and is the wrong target here — and `out/libscrip_rt.so` + libgc-dev were the fresh-sandbox NOASM causes, not code). **Corpus 185→190: exactly +5 FAIL→PASS (`rung13_repalt_{int,str,to,to3,var}`), ZERO PASS→FAIL** — stash/rebuild/per-program diff vs pristine 0225b85a; `rung13_repalt_empty` passed at baseline only because broken repalt happened to instant-fail, and passes now for the right reason. Smoke 12/12×2; no_stack/one_reg/semicolon green; bench-asm baseline (13/0/1/12). Remaining ladder (3): if_value_else (fix-shape documented), revassign (missing conversion), section_plus (pending arms).
