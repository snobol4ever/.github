...
## ⛔⛔ GC-6 BOEHM-EVICT — CHECKPOINT: the whole system on the scrip 3-stage GC, ZERO libgc linked (Lon directive, 2026-07-05 fourth session)
**Scope = SNOBOL4 + Icon ONLY — Prolog/Pascal/Raku are DESCOPED and rebuild later; their alloc sites PARK behind a compat shim (PARK-NEVER-DELETE).** Success = `ldd scrip` + `ldd out/libscrip_rt.so` show ZERO libgc; `grep -rn '<gc.h>|GC_[A-Z]' src/` = 0 outside `*-parked-*` files, the compat shim, and gc_heap.c's compile-gated `ZC_HEAP_LIBGC` fallback arm; full crosscheck + icon smoke + gc suite green at the 2026-07-05 fail sets. **The three live storage strategies after this rung: 3-STAGE regeneration (scrip heap, MARK/ADJUST/SLIDE), BUMP_LIFO (the ζ-stack, ZC_ALLOC default), BUMP_INFINITE (never-collect ζ axis) — Boehm is not one of them.**
**CENSUS (grep-exact this session):** `unification.c` ×25 + `rt_runtime.c` ×10 (`PL_CELL_ALLOC`/`Term**` — ALL Prolog, park) · `lower_prolog.c` ×3 + `emit.h:215` (`pl_gz_callee`, Prolog compile-side, park/malloc) · `scrip.c:1219,1317` **`m3_gva_arena = GC_MALLOC_UNCOLLECTABLE`** (VALUE-WORLD GVA slot array — plain calloc; already in the precise root walk, stays a root) · `pattern_match.c:533,536` capture u32 bufs (`GC_MALLOC_ATOMIC`) · `by_name_dispatch.c:2529` **Icon `collect()` builtin calls `GC_gcollect()` — wired to the WRONG collector** · `core/core.c` ×2 · `zeta_alloc.c` ×2 (`GC_add_roots` 8MB chunks — exists ONLY for libgc visibility of ζ DESCRs, dies with Boehm) · `zeta_storage.c` ×1 + `lower_common.c` ×2 (compile-side) · `gc_heap.c` ×2 (the gated fallback arm — KEEP) · `parser/prolog/test_pl_*.c` ×4 (park).
- [ ] **GC-6a COMPILER-SIDE → plain malloc/calloc** — emit.h, lower_common.c, zeta_storage.c, driver GC_INIT: compile-time allocations live for the process; no collector involvement. `m3_gva_arena` → calloc (root status unchanged — GVA is in core_gc_roots' precise walk).
- [ ] **GC-6b VALUE ROWS: AGGREGATES ONTO THE SCRIP HEAP, PINNED v1** — ARBLK/TBBLK/TBPAIR/DATINST/VCELL (+ any residual value cells in core.c) allocate via `rt_gcheap_alloc` with new `HB_*` type codes, born `HBF_PIN` (non-moving — exactly Boehm's contract, but OURS, so Boehm can go). `rt_gc_visit_descr` already traces INTO them; relocation of holder pointers deferred to GC-6e. The fill-window (this session's fix) is what makes a pin-heavy heap survivable — watch `[ZGC]` pin counts.
- [ ] **GC-6c RT BUFFERS** — pattern_match capture bufs → plain malloc + free at the gen-reset (or a scrip-heap RAW pinned row); audit rt.c residual allocs and the raw-malloc coexpr ctx.
- [ ] **GC-6d PARK PROLOG/PASCAL/RAKU** — `src/include/gc_compat.h` (`#define GC_MALLOC(n) calloc(1,(n))` family, leak-tolerant placeholder) included by unification.c / rt_runtime.c Term paths / lower_prolog.c / prolog parser tests. Build stays green; descoped languages keep compiling, rebuild later.
- [ ] **GC-6e UNPIN AGGREGATES (perf, optional for the checkpoint)** — record DT_A/DT_T/DT_DATA/DT_N holder cells during MARK; ADJUST relocates them; drop birth-PIN.
- [ ] **GC-6f ζ ROOT PRUNE** — delete zeta_alloc's `GC_add_roots`/chunk machinery and the `<gc.h>` includes on the SCRIP arms.
- [ ] **GC-6g LINK EVICT + GATE** — `-lgc` out of the Makefile (BOTH products) and out of every script link line (`test_crosscheck_snobol4.sh`, `test_gc_stress_suite.sh`, grep the rest); NEW `scripts/test_gate_no_boehm.sh`: ldd both products == 0 libgc, include-grep == 0 outside parked/compat/fallback.
- [ ] **GC-6h COLLECT()** — redirect Icon `collect()` (by_name_dispatch.c:2529) from `GC_gcollect` to `rt_gc_collect` (exported and waiting — the GC-7 note, folded in); wire SNOBOL4 `COLLECT()` the same way; ADD corpus tests that CALL it — **today's gc/ suite triggers only via SCRIP_GC_STRESS + exhaustion; NO test calls COLLECT() (Lon question 2026-07-05, answered).** Oracle note: SPITBOL's COLLECT() returns free-space count — pin the return-value semantics before wiring.
- [ ] **GC-6-FENCE** — full crosscheck both modes + icon smoke 12/12×2 + `test_gc_stress_suite.sh` ALL GREEN at the same fail sets as the 2026-07-05 baseline (m3 187/89, m4 187/3/86, DIVERGE=0); STRESS=1 full-corpus fail sets byte-identical; `test_gate_no_boehm.sh` PASS both build configs.

#### ZB-ALLOC — THE ζ ALLOCATION PLAN (2026-07-05, from the 4/28 .s + era C sources; refines ZB-2/ZB-3)

**⌚ GC-TEST WATERMARK 2026-07-05 (fourth session same day, Claude Fable 5 — GC TORTURE SUITE + EXHAUSTION TOP-PIN FIX + GC-6 RUNG AUTHORED).**
**(1) NEW SUITE `corpus/crosscheck/gc/`** — 15 oracle-pinned programs, one per collector root family: NV globals (201), ARBLK cells (202), TBBLK string keys looked up by FRESHLY REBUILT equal key post-slide (203 — content-hash proof), recursive DEFINE frames (204), DATINST linked list (205), scan+capture under churn (206), `$` computed names (207), one block aliased from global+array+table (208), 5KB block slide with POS/LEN + RTAB/REM head+tail reads (209), table→array→record→string chain (210), `.NAME` VCELL write-through (211), 3-fresh-string-arg in-flight window (212), plus anchor churn (200) and TWO EXHAUSTION TORTURES (213: 60MB churn / 214: 100 live strings vs 40MB churn). `scripts/test_gc_stress_suite.sh` = the matrix runner: both modes × {plain, S=25, S=7, S=1}, **120 cells ALL GREEN**; header documents the `ZCFLAGS="-DZC_HEAP_MB=2"` rebuild for exhaustion coverage.
**(2) BUG FOUND + FIXED — EXHAUSTION TOP-PIN ZERO-RECLAIM (gc_heap.c):** at allocation-site exhaustion the conservative scan near-always PINS the newest block — the allocating expression's own in-flight operand — at the heap TOP, so the top cannot retreat, ALL reclaimed space lands below it as HB_FILL, `reclaimed 0`, and the retry bombs (regenerate-then-retry was UNREACHABLE; proven: 213/214 at 2MB both bombed at regen #1, `bytes 2096992→2096992`). **FIX = THE FILL WINDOW:** the collector tracks the largest pin-forced fill gap and installs `[g_hp_win,g_hp_wend)`; `rt_gcheap_alloc` bumps main-top first, else carves the window's LOW end, rewriting the remainder's fill title in step so the title walk stays verify-green — and a 16B remainder is ABSORBED into the carved block (a header-only HB_FILL fails verify's min-32 rule; that degenerate title was a second bug the first fix version surfaced). Result: 213 = **30 regenerations cycling 60MB through 2MB**; 214 = 20 regens with the 100-string live set slide-adjusted each pass. `[ZGC]` telemetry now prints `win=`.
**(3) STRESS=1 FULL-CORPUS INVARIANCE (strictly stronger than the landing session's S=25):** the whole crosscheck under `SCRIP_GC_STRESS=1` — collect on EVERY string allocation — has fail sets BYTE-IDENTICAL to plain (m3 172/89, m4 172/3/86 {082,099,213}, DIVERGE=0, lists name-for-name, pre-gc/-install counts). Icon smoke 12/12×2 plain AND S=1; gates no_ir_mutation + no_lang PASS. 2MB-heap matrix: 15/15 × {plain, S=1} × {m3, m4}.
**(4) NEW BASELINE with gc/ installed: m3 187/89 · m4 187/3/86 {082,099,213} · DIVERGE=0** (+15 both modes, fail sets untouched). Codegen untouched (runtime-only session: gc_heap.c + scripts + corpus) — regen scripts N/A by RULES step 4's own condition.
**(5) COLLECT() NOT WIRED — Lon question answered:** no gc/ test calls it; triggers are STRESS env + exhaustion only. Census find: Icon `collect()` EXISTS at by_name_dispatch.c:2529 and calls **Boehm's `GC_gcollect()`** — the wrong collector. Folded into GC-6h.
**(6) BOEHM ANSWER (Lon: "we should not need Boehm anymore. Correct?"):** correct as END STATE, not yet fact — libgc still allocates ARBLK/TBBLK/DATINST/VCELL + GVA arena + capture bufs + all Prolog terms, and is linked into both products. The eviction path is now the **GC-6 BOEHM-EVICT rung above** (Lon-directed checkpoint: SNOBOL4+Icon only, ZERO libgc linked; Prolog/Pascal/Raku park via compat shim).
**NEXT = GC-6a..h in order; GC-6-FENCE is the checkpoint.**
...
**⌚ 2026-07-05 (fifth session same day, Claude Fable 5 — D15 EXECUTED + D13 SELF-LOAD HOOK LANDED, ZC-axis invariance sweep; SCRIP `261cbbcb`).**
**(1) D15 FLIP:** `zeta_choices.h` ZC_ALLOC default BUMP_INFINITE → BUMP_LIFO (one line, per the ruling's EXECUTE-FIRST order). Re-proven on landing: icon smoke 12/12×2 · crosscheck m3 168/93, m4 168/3/90, DIVERGE=0, FAIL sets comm-diff EMPTY vs the same-session INFINITE baseline. LIFO reclaim is real: recursive-fib telemetry allocs=205/releases=204/live=1 (INFINITE holds all 205).
**(2) ZC AXIS INVARIANCE SWEEP (§5j MODE-INVARIANCE GATE exercised, 3 rebuilds, zero source change):** INIT_NONE (0xAA on fresh) — FAIL sets byte-identical both modes ⇒ **NO ζ slot is read before written anywhere the passing corpus exercises** (no hidden zero-init reliance — a proven property, not a passing test). MALLOC — FAIL sets byte-identical; the **MAX_ROOT_SETS ceiling reproduced in THIS tree on demand** (icon `count(2000)` passes, `count(6000)` → "Too many root sets"/abort) while the SAME probe under LIFO runs clean BOTH modes (6000 ✓; hiwater 24,741,664B = 6001×4112, matching ZB-3's number exactly) — D3/GLOB-grain motivation made concrete.
**(3) D13 LANDED (emit.cpp, +9/−1 total with the flip):** the §5h hook — `zeta_choices.h` included; `zeta_selfload_mode()` (env `SCRIP_ZETA_SELFLOAD` overrides the ZC_SELFLOAD built-in, read at EMIT time ⇒ mode flips need NO rebuild — the quick lever as specified); `emit_zeta_selfload()` called at the chain body's **α-label site** (beside `emit_label_define_bb(lbls[i])`). ASSERT emits `test r12,r12; jnz 1f; ud2` — bytes nasm-verified `4D 85 E4 75 02 0F 0B` BEFORE implementation, BOTH mediums (text `1f` local + hand-encoded binary; encoder-decline footgun sidestepped by not going through `x86()` dispatch for a fixed 7-byte sequence). VERIFIED 3-way: **OFF = true no-op** (ud2 grep = 0 in .s; corpus FAIL sets byte-identical; smoke 12/12×2) · **ASSERT** = 27 guards in fib's .s, both modes print 0/1/8/55 (guard never fires ⇒ **rZ non-null at every α port — a live executable wiring invariant**) · **ASSERT full-corpus** = crosscheck FAIL sets byte-identical both modes + smoke 12/12×2 (bytes changed, behavior never — the gate's exact contract). REMAINDER: the **β-define site** is not yet hooked (β defines ride the template pair-loop define path, not a single central `emit_label_define_bb` call — hooking it is the second half of the ruling); PLANE_CELL/STATIC arms deliberately not built (no plane cell exists until ZB-5's COLLECTION owner work). ASSERT shelf is stocked for ARBNO bring-up per the ruling.
**(4) DESIGN NOTE (Lon, this session):** the LIFO bump arena is **GRANULARITY-INDEPENDENT of GLOB** — per-BB and per-GLOB activations share the ONE ζ region; the grain changes chunk SIZE, never push/pop ORDER, so GLOB-grain activation (the ZB-3 remainder) is purely a volume/perf/diagnostic knob (MALLOC's root-set ceiling recedes; the ASan instrument gets per-glob lifetimes) fully DECOUPLED from allocator correctness — LIFO soundness holds at any grain.
**(5) RULES step 4:** emit.cpp touched ⇒ all four regen scripts run — **zero artifacts changed** across benchmark/feature/demo/icon-bench (the mechanical proof of the OFF-default byte-identity; icon-bench compile-err set = the known pre-existing 9). Gates: no_ir_mutation/no_lang/no_slot_alloc all PASS pre-commit.
**NEXT = ZB-4** (struc overlays + no-.bss gate, printing from the clean zls maps) **or ZB-5** (COLLECTIONS/ARBNO on ζ at GLOB grain — ASSERT lever ready); the D13 β-define remainder rides whichever comes first.

**⌚ 2026-07-05 (fourth session, close) — D3/D13/D15 RULED (Lon): D3 = 1024 MB confirmed ("we'll see when it breaks"; telemetry stays ON to catch it) · D15 = ZC defaults confirmed + flip ZC_ALLOC→BUMP_LIFO · D13 = BUILD the α/β r12 self-load hook, OFF by default — SEQUENCED: D15 BEFORE D13. NEXT SESSION, in order: (1) D15 flip — one line in `zeta_choices.h` (ZC_ALLOC default → BUMP_LIFO; byte-identity already proven at ZB-3, re-verify smoke + crosscheck on landing); (2) D13 hook — the two central `x86()` α-label/β-define sites, `ZC_SELFLOAD`/env toggle, ASSERT mode shelf-ready for ZB-5; (3) ZB-4 struc overlays + no-.bss gate, printing from the now-clean zls maps. Full ruling annotations in ARCH-ZETA §8.**
