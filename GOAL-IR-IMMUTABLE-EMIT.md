...
## ⛔ ICNBENCH — GET `corpus/benchmarks/icon` RUNNING END-TO-END (fresh re-derive 2026-07-02, SCRIP `5c5697ab`, Claude Sonnet 5)
...
- [ ] **ICNBENCH-1 SHUFFLE-CALL** — unretested since ICNBENCH-0's reframe; `deal` now fails at the same `&regions`-family wall as concord/ipxref/queens (past shuffle's old wall). Re-verify once ICNBENCH-2c lands.
...
- [ ] **ICNBENCH-2c KEYWORD-INTROSPECTION FAMILY (`&host`, `&features`, `&regions`, `&storage`, `&collections`)** — **NEW, highest-leverage now — unblocks concord/deal/ipxref/queens (4 programs) at once.** `&features` silently yields nothing (should generate feature-name strings); `&regions`-driven list entries are malformed enough that later indexing SIGSEGVs. **Needs a design decision, not a mechanical fix**: these keywords describe classic Icon's interpreter memory model (static/string/block regions, GC-region byte counts), which has no literal referent in SCRIP's natively-compiled execution model. Two honest options: (a) wire to libgc's real stats where a plausible analog exists (has actual meaning), or (b) return fixed/plausible sentinel values purely for shape-compatibility (meaningless but non-crashing) — Lon call, not a session-default.
...
- [ ] **ICNBENCH-6 RSG-NV-CELL** — NEW: `bb_var_ref: variable has no addressable cell` — unexamined past the message itself. **[rsg]**
- [ ] **ICNBENCH-7 MICSUM-SCANTAB** — NEW: `bb_scan_tab: unhandled (needs literal positive n)` — a non-literal scan-count case; unexamined past the message itself. **[micsum]**
- [ ] **ICNBENCH-ARGS** — nothing today is actually blocked on this (every failure dies before touching `args`), but it will bite the moment ICNBENCH-2c lands: `scrip --run`/`--compile` don't forward CLI args into the Icon program's `args` list (`scrip --run f.icn foo` treats `foo` as another file to open). Needed for realistic reruns (`deal -h 1000`, `queens -n10`).
