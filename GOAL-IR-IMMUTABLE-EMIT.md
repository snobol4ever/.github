...
- **`emit_drive: IR op=N has no template` (8):** op=1 ×6 (lists, table, mindfa, record, string, substring) · op=3 ×1 (wordcnt) · op=50 ×1 (scan). The printed N looks like low IR_e ordinals (1=IR_ASSIGN, 3=IR_BINOP) that PLAINLY have templates — so the abort is firing on nodes reaching a walk context that shouldn't see them (mis-wired operand/edge, or a node minted with a stale op) — diagnose at the abort with the node, don't trust N.
