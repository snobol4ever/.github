...
## ⛔ PROBE RUN, MEASURED, REVERTED — THE PREDICATE FLIP ALONE DOES NOT CARRY IT, AND MY OWN PROPOSED GATE IS FALSIFIED
One-line gated probe (`SCRIP_RECFB_RSP=1`): `emit_rec_pin()` stops claiming a record rbp for `flat_pat`, so `emit_rec_fb()`/`_num()` select rsp in BOTH media. **Measured: `FB-DIVERGE TOTAL` 2 → 0, witness STILL rc=139.** Default 0 byte-identical (`e80364d34c2d`). **REVERTED, tree clean** (s38 precedent: a known-insufficient killswitch is M-5 debt, not a bisect instrument).
⛔ **THEREFORE: `FB-DIVERGE TOTAL=0` IS NOT A SUFFICIENT ACCEPTANCE GATE** — this FINDING proposed it as one; that is hereby corrected. Predicate agreement is NECESSARY, not sufficient: at α_body `rsp == hdr-40`, and across a γ-suspension rsp is not a stable base at ANY displacement — the record protocol needs a depth-immune base and post-DEL-T1 the ONLY one is `g_zctx[1]`. **Order is forced: convert the record SITES to g_zctx addressing first; the predicate then falls out as a consequence.** Do not spend a seat on the cheap predicate flip — it is measured and dead.
