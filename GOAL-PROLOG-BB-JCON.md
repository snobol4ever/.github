...
```
one4all: ef2f90e4 (PJ-9b)  corpus: 1fe096c
smoke_prolog: 5/5 ✅ (write_atom+unify+arith+clause+recursion all PASS)
crosscheck_prolog: 128/0 ✅ (124 corpus rungs + 4 inline; SKIP=4 timeout; ORACLE_MISS=11 frontend gaps)
Other smokes: snobol4 7/7, icon 5/5, snocone 5/5, rebus 4/4, raku 5/5
honest gates: prolog 124/0/0, icon 277/0/0 (no regression)
broker: 20/49 (was 19)
NOTE: PJ-9a regenerated snocone_parse.tab.h from snocone_parse.y to fix the
upstream PST-SC-4b (4aa8727b) build break. Tree builds clean from HEAD.
```
