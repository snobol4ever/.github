...
║                      to a BB object owned by language B (e.g. a bb_node_t with fn=icn_bb_dcg), ║
...
║                      Per-language internal semantics (Prolog unwinds the trail; Icon advances  ║
║                      a generator counter; SNOBOL4 backtracks anchors) are encapsulated inside  ║
...
║  as its operand.  Specifically, the handler for Prolog's `SM_BB_ONCE_PROC` is hardcoded to     ║
║  drive `dcg_table[i].ir_body` with Prolog's trail/choice-point semantics — it must not be      ║
║  passed `proc_table[i].ir_body` (Icon's generator DCG).  The handler doesn't know how to       ║
║  set up Icon's frame/scope, so cross-wiring corrupts state.  Cross-language calls go up to     ║
║  SM-level dispatch first via `g_user_call_hook`, never reach across at the bridge-handler      ║
║  level.                                                                                          ║
║                                                                                                  ║
