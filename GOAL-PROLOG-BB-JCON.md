...
| **PJ-9c** ✅ (partial) | **Mode 4 (`--sm-emit --target=x86`) dispatch wiring for `SM_BB_ONCE_PROC` — first Prolog primitive routed through emit pipeline.** Three coordinated changes: (1) **`src/runtime/rt/rt.c`**: added `rt_bb_once_proc(const char *name, int arity)` helper mirroring `case SM_BB_ONCE_PROC` in `sm_interp.c:671` — calls `pl_bb_once_proc_by_name`, on hit pushes `pl_bb_env`, drives `bb_broker(node, BB_ONCE, NULL, NULL)`, pops env. On miss prints the same `[NO-AST] SM_BB_ONCE_PROC stub` fingerprint as Modes 2/3 (PJ-9b consistency). Includes `pl_runtime.h` for types. `pl_runtime.c` already in `RT_PIC_SRCS` so link works. (2) **`src/emitter/emit_sm.c:562`**: changed `g_sm_templates[]` entry from `SM_TPL_ARITH / "rt_unhandled_sm"` to `SM_TPL_LBL_INT32 / "rt_bb_once_proc"` (same shape as `SM_CALL_FN` — string + int operands). (3) **`src/emitter/emit_sm.c`**: added `emit_sm_bb_once_proc_dispatch` (modeled on `emit_sm_call_dispatch`) and `case SM_BB_ONCE_PROC` in the master switch at line 2956. Result: `scrip --sm-emit --target=x86 hello.pl` now emits `BB_ONCE_PROC .S0, 0` (was `UNHANDLED 60`); assembles and links cleanly; produced binary runs and reaches `rt_bb_once_proc` correctly. **Open issue (PJ-9d):** the standalone binary's `g_dcg_table` is empty at runtime because the Mode-4 emit doesn't include a Prolog predicate-registry initialization (analog of the `rt_register_expressions` call already emitted for Snocone). So the runtime helper currently hits its miss-arm `[NO-AST]` print rather than executing the predicate body. Dispatch shape is correct; the registry-population emit is the next step. **All gates hold:** smoke_prolog 5/5, crosscheck_prolog 128/0, honest_prolog 124/0/0, honest_icon 277/0/0, unified_broker 20/49 — all unchanged from PJ-9b. |
| **PJ-9d** 📋 (design) | **Predicate-registry emit for Mode-4 Prolog binaries — design captured, implementation deferred to fresh session.** Goal: make `g_dcg_table` populated at startup of `scrip --sm-emit --target=x86 hello.pl` so `rt_bb_once_proc` finds the predicate and runs its body. **Why this is bigger than PJ-9c**: predicate bodies are stored as `IR_block_t*` (graphs of `IR_PL_*` nodes) built by `lower_pl_predicate` in the scrip process at lower-time. The standalone Mode-4 binary doesn't run `lower_pl_predicate`; it just calls `rt_bb_once_proc` and expects the registry already populated. **Three architectural options, evaluated:** (A) **Static IR serialization**: emit each `IR_block_t` as a serialized blob in `.rodata`; runtime startup deserializes once via `rt_register_predicates_pl`. Heavy: needs full serializer + pointer-rebasing deserializer (~300 LoC). (B) **Re-lower at runtime**: ship source/AST in binary, re-run `lower_pl_predicate` at startup. Defeats Mode 4's purpose. (C) **Per-predicate C builder fn emitted as x86**: each predicate's `IR_block_t` becomes a tiny "builder" function that constructs the IR graph on first call. Lowest infrastructure cost but still requires emitting code that creates `IR_t` nodes (via `IR_new_*`-style calls). **Recommendation: Option C, simplest scope.** The emitter walks each `(name, arity, ir_body)` in `g_dcg_table` at scrip-emit-time and emits assembly that, when called, calls `IR_new_*` constructors to rebuild the same graph. Then emits a startup table `(name, arity, builder_fn_ptr)` and a `rt_register_predicates_pl(tbl)` call in `main`. **Where to start:** (1) define `rt_register_predicates_pl` and matching `rt_predicate_entry_t` struct in `rt.h`/`rt.c`. (2) Add a Mode-4 emit pass `emit_pl_predicate_registry` analogous to `emit_expression_registry` in `emit_sm.c:1602`. (3) Walk `IR_t` graphs and emit one assembly function per `IR_block_t` whose body is `IR_new_*` calls. (4) Add a `rt_register_predicates_pl(.Lpl_registry)` PLT call in `main` prologue alongside the existing `rt_register_expressions`. **No code change this rung — design only. All gates hold.** |
...
```
one4all: 0ffffa2e (PJ-9c partial)  corpus: 1fe096c
smoke_prolog: 5/5 ✅
