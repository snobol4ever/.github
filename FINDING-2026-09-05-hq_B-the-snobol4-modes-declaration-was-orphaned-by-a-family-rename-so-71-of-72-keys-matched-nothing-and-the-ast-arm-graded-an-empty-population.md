...
- **pascal** — ⛔ **CORRECTED after hq_T caught the number, and the error is on-theme enough to keep**: I first wrote that stripping `__suffix` "resolves 70 of 75". It resolves **ZERO**. Re-measured on corpus `241579669`, splitting the two cases my first pass had merged: **already-matching=70, resolved-by-stripping=0, unmatched-either-way=5**. My counter tested `k in fams or k.split('__')[0] in fams` and reported the sum in the narrow question's words, so the 70 keys that never needed stripping were counted as if stripping had rescued them. ⭐ A disjunction answers a BROADER question than the one asked and reports it in the narrow question's words — the same narrow-instrument shape this finding is about, committed inside the measurement I used to route work to another lane. **So pascal is DECLARATION work, not a rename repair, and must not be budgeted as a repeat of the snobol4 job**: 5 orphans that do not strip, plus families **`ladder`** and **`parser`** wholly undeclared, covering all 40 entries.
