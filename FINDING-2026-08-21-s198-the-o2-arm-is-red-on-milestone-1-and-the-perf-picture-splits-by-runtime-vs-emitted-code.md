...
- ⛔⛔ **THE OBJDIR CARRIES RT_OPT IMPLICITLY, AND IT NEARLY CONVICTED AN INNOCENT COMMIT.** After the ablation, `out/rt_pic` still held 257 `-O2` objects. A seat then landed an Icon template change (`xa_flat.cpp`, `3b70bf67`); a plain `make` recompiled **only that one file at -O0** and relinked a **MIXED library**, which reproduced the wrong md5 `1c75f97d` — reading exactly like *"the seat's commit broke Milestone 1."* It did not: `rm -rf out/rt_pic` + a uniform `-O0` rebuild (verified `260 -O0 / 0 -O2` lines in the build log) restores the fixed point at that same HEAD. **A build whose objdir was populated under different flags is a THIRD binary, neither arm** — and HQ-27's per-checkout objdir does NOT protect against this, because the collision is across *time* in one checkout, not across trees. Wipe the objdir whenever RT_OPT changes, and **count the flag lines in the build log** before believing any verdict.
