...
```
one4all: d18b003e     (IJ-HELLO-3: SM_BB_PUMP_PROC wired via call .L<entry_pc>; icon hello PASS-wired)
...
.github: this commit  (IJ-HELLO-3 ✅; IJ-HELLO-4 (Prolog) is next)
test_smoke_compile_hello_all_langs: PASS=5 FAIL=1 ROWS_MATCH=6 ROWS_DRIFT=0  (icon flipped PASS-wired)
...
crosscheck_snobol4: 5/0   crosscheck_icon: 4/0   crosscheck_raku: 21/12   (snobol4 ≥ baseline 5/1)
...
broker: 22/27 (unverified this session — gate's csnobol4 Budne suite exceeds bash_tool wall-clock;
               change is mode-4 emitter-only, broker runs modes 2/3, no path of effect)
DAI-BOMB fires: 0 (the stubs no longer exist)

⚠ Modes 3 (--run / in-memory JIT) and 4 (--compile / GAS asm text) NOT
  validated for DAI-8 cluster 1. Next session MUST run the full all-modes
  ...
  matrix is green. (This session spot-checked Icon hello.icn under
  --interp and --run as well as --compile — all three modes print
  "Hello, World!" rc=0; deeper modes-3/4 corpus matrix is IJ-HELLO-5's scope.)

✅ IJ-HELLO-3 is COMPLETE. Icon hello-world is PASS-wired with no brokered
  imports.  The `lower_proc_skeletons()` + `sm_resolve_proc_entry_pcs()`
  pre-existing infrastructure proved sufficient; the new dispatch handler
  is a thin name→entry_pc lookup that emits a bare x86 `call .L<entry_pc>`.
  The spec's 3a `emit_icn_proc_flat_blob` IR_block_t walker proved unnecessary
  — the SM stream already contains the proc body as plain SM ops.  Next:
  IJ-HELLO-4 (Prolog wired hello — needs PUSH_EXPR macro fix first per
  IJ-HELLO-1 baseline-discrepancy findings), IJ-HELLO-5 (close).

Gate-suite wall-clock: ~120s (broker suite skipped due to bash_tool ceiling;
