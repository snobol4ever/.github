...
```
one4all: 5bc395d7     (IJ-HELLO-2b: SUB_TAG_ID match in lower_stmt; raku hello PASS-wired)
...
.github: this commit  (IJ-HELLO-2 ✅ both sub-rungs; IJ-HELLO-3 (Icon) is next)
test_smoke_compile_hello_all_langs: PASS=4 FAIL=2 ROWS_MATCH=6 ROWS_DRIFT=0  (raku flipped PASS-wired)
...
⚠ Modes 3 (--run / in-memory JIT) and 4 (--compile / GAS asm text) NOT
  ...
  matrix is green. (IJ-HELLO-2b ran the standard mode-2 + hello-mode-4
  gate matrix; deeper modes-3/4 corpus matrix is IJ-HELLO-5's scope.)

✅ IJ-HELLO-2 is COMPLETE. Both Bug 1 (icn-builtin fallback, 2a Opus 4.7
  996f03e0) and Bug 2 (proc-def wrapper trailing CALL_FN, 2b Sonnet 4.6
  this commit) are landed. Raku hello-world is PASS-wired with no
  brokered imports. Next sub-rungs: IJ-HELLO-3 (Icon wired hello),
  IJ-HELLO-4 (Prolog wired hello — needs PUSH_EXPR macro fix first
  per IJ-HELLO-1 baseline-discrepancy findings), IJ-HELLO-5 (close).
