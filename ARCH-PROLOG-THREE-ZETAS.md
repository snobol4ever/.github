...
| clause alternatives | the call box's β path: the clause cursor is a frame field; the last clause's β is the box's ω |
...
| the trail | β's undo: the mark is written into the FRAME at α, unwound at ω and on every β re-entry; frame-relative, never a fixed RSP slot (P10 folds in here) |
...
| the three live `setjmp` barriers, `g_plw_unwind_floor`, `g_plw_floor_bypass`, `rt_plw_floor_bypass_on`, `dop_call_nothrow` and its two wrappers | `by_name_dispatch.c:1486-1600, 4872-4875, 4911, 5064` | P7 |
...
**Program gate (the DONE-WHEN of the redesign):** rung suite **15/15 both modes** · van Roy **REFUSE 0** (21 kernels timed on the two-number basis) · `nm -D out/libscrip_rt.so` names none of the § 4 symbols · `grep -rcw Term src` = 0 and both headers absent · the `fact/2` witness emits rbp-relative frame addressing · C22/C23/C24/C26's witnesses green · master board not worse than 351/351.
