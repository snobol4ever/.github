...
**⭐ TRANSLATION PAGE (ceo 2026-09-02, on Lon's order *"analyze the Byrd Box literature on Prolog … how each Prolog construct maps into the Byrd Box for DEBUGGING and ANALYSIS, and just turn that into IMPLEMENTING using Todd Proebsting's Byrd Box translation technique"*):** `ARCH-PROLOG-BYRD-BOX-TRANSLATION.md` — the port correspondence, the one frame record every call box owns (`F.B F.B0 F.CUR F.RES F.HI F.LOG`, the trail = the `r12` C.A.S., B = `r13`, the standing root = `r14`, the ball = `r15` — Lon's 2026-09-02 grant of the string-scanning registers to Prolog; `r10`/`r11` stay the statement number and BB node id), all 18 constructs with their Proebsting shape and variable alignment, every census global mapped to a frame slot or deleted, 13 rungs with measured DONE-WHENs, five questions for Lon. DRAFT until HQ-COMPLETE (hq_C) has reviewed it against the code and Lon has answered § F.
