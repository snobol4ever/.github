...
that gates all subsequent emit-artifact work. Before any `--sm-emit`
...
peer ports today. But when emitted by `--sm-emit --target=x64`,
...
`--sm-emit` work) becomes the active rung in the parent
goal.
