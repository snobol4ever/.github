...
- [ ] LS-3.f.1 — Rewrite `snocone.l` rules to `return T_*` directly,
      drop `emit_simple()` accumulator, drop `ScTokenBuf` from
      production code.  Mirror `snobol4.l` `%option reentrant
      noyywrap` shape.  Token kind values come from `snocone.tab.h`
      (generated by Bison in LS-4.a), not from `snocone_lex2.h`.
- [ ] LS-3.f.2 — Move `test_snocone_lex2.c` (LS-1.b 31-test acceptance
      harness) from production source to `test/frontend/snocone/`.
      The harness can drive Flex via `yylex_init` + token-streaming
      and accumulate for assertions — that's a test concern, not a
      production architecture.
- [ ] LS-3.f.3 — `test_smoke_snocone.sh` 5/5 still PASS (the LS-3
      lexer regenerates as committed but no .y exists yet, so the
      old hand-written `snocone_lex.c` + `snocone_parse.c` etc. are
      still the active path for compile until LS-4 lands).
