...
**THE FIRST AND ONLY `DEFERRED` POPULATION TODAY: the 30 GNU Prolog finite-domain programs** in `packages/prolog/gnu_fd`. Ruled INTO the superset by Lon (CEO-572, *"I say the FD is superset"*) and ruled NOT-A-FAILURE the same sitting (CEO-579). What they wait on, measured: GNU's own FD is `EngineFD` 4,932 + `BipsFD` 8,636 = **13,568 lines, about a quarter of their entire Prolog system**; the 30 programs need 11 constraint operators (`#<=>` alone 96 times — reification, a second layer rather than more propagators) and 14 `fd_*` builtins; and **we hold no attributed-variable/suspension substrate at all** (zero hits in `src/` for `attr_var`, `put_attr`, `when/2`, coroutining, wake), which is the prerequisite nothing else can start before. ⛔ **SUPERSEDED FOR THE ANNOUNCEMENT AND THE HOME STRETCH (Lon 2026-09-23 14:4x CDT, in-chat to the ceo, verbatim: *"exclude the 30 GNU FD programs from the announcement and therefore the home stretch."*; CEO-1209; recorded here 2026-09-24, CEO-1231):** the gnu_fd SUITE TABLE row is removed, its `DEFERRED.tsv` row is WITHDRAWN (a deferral must lift a number the board prints -- ARM 2 of `test_gate_deferred_rows_name_a_ruling_and_the_work_they_wait_on.sh`, red on origin from the removal until the withdrawal), and the 30 programs are FUTURE WORK outside the home stretch, not a deferred population of the board; the measurement above stands as the record of what they wait on, and they re-enter `DEFERRED.tsv` only with a `SUITES.tsv` row.
...
⭐ **2026-09-23 (CEO-1221): THE BENCHMARKS ARE SUITE ROWS, AND EVERY BENCHMARK IS A TEST SO THAT IT CANNOT BE VACUOUS** — Lon's four messages and the rule are in RULES.md § THE KERNEL CONVENTION, 2026-09-23 addendum. **MEASURED STATE at corpus `18db81c9c` (ceo), sources / `.ref` / `.expected` / neither / `.in`:** snobol4 38/38/0/0/1 · icon 42/15/0/27/0 · prolog 123/0/23/100/0 · pascal 11/10/0/1/7 · raku 17/17/0/0/0 · snocone 16/16/0/0/0 · rebus 3/3/0/0/0; no `.out` anywhere (none is owed: OUT is REF, Lon 18:1x, CEO-1222); no benchmark row in SUITES.tsv; Icon has no benchmark triangulation script (only `bench_triangulate_demos_icon.sh`). Counted with `.expected` beside `.ref` on purpose (RULES.md CLAUSE 1 keeps the 4.7x over-count that forgetting it cost once).
