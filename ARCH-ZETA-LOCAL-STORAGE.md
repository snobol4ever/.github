...
- [x] **GC-1 MARK + GC-2 ADJUST + GC-3 SLIDE — LANDED 2026-07-05 (Fable, Lon-directed "mark, slide and adjust"; CSNOBOL4 cross-audit same session).** The SIL 3-stage storage
      ...
      confirmed against `silly/arena.c` GCLAD/GCLAM); adjust skips fwd==self (GCLAP `>= mvsgpt` analog). Coexprs: any created coexpr ⇒ collection REFUSES with an honest stderr note
...
**✅ RUNG ZB-FC-3a LANDED (SCRIP `a775820a`, s41 same session) — ALTERNATE LINEAR-ARM v1 onto the spine.** LOWER computes per-arm EXACT fp at construction time (granted leaves=16; SEQ/captures/wiring=0; nested ALT/ARBNO/ARB/DEFER → refuse whole ALT → stays flat). `fc_alt_{register,fpmax,fp}` side-table. ALT template fc arm: own 16B cell at α/ω, β reads alt_i at `[rsp+FPMAX+8]`, per-arm resume adds `(FPMAX−fp_j)` back (alt_resume_chain_fc), N pad-stub PAIRs at 2N+2+j. ALSO FIXED: `x86_pair_tgt` fallback — define-only pair slots now serve as jmp targets; previously `??`/skipped-patch → silent SEGV (the stubs' `jmp na_s` hit this). Census: **181 fc cells (89 ALT + 92 ZB-FC-1/2)**; watermark-exact both flavors; all prior probes clean.
...
## 14. SNOBOL4 IS ALL-STACK — NO HEAP-FLAVOR BB EXISTS FOR SNOBOL4 (2026-07-13 s50, Lon ruling, verbatim intent: "Ensure NO heap flavor BB for SNOBOL4. They all can fit on the stack... Icon is the only one with the problem, co-expressions and generator procedures.")
**THE LAW:** the heap flavor's client set is **ICON-ONLY** (co-expressions, generator procedures — IR_CREATE / IR_PROC_GEN / IR_SUSPEND). SNOBOL4 has ZERO escaping activations, so **every SNOBOL4 ζ rides rsp** and the statement bracket (ZB-FC-3d, S10e unwind) is the total lifetime authority. Classification is by **LIFETIME (escape), never by SIZE** — the s41 wholesale map's Tier D was a classification error of record: it routed constructs to the heap for having *variable-size* data, but variable size is a dynamic-extent problem and dynamic extent is what stacks are for.
**WHY SNOBOL4 HAS NO ESCAPES (derived, s50):** stored patterns are recipes/blobs = VALUES, not live activations; `*E` stores an unevaluated-expression VALUE; NRETURN returns a NAME (a value); DEFINE saves formals+locals on a pushdown stack and restores on return (manual Ch.8) = pure LIFO = exactly what call/ret nesting preserves (the s25 F1 proof); DEFER only ever looked like an escape because the C trampoline forced a return through C — post-NCB-2a the emitted β-resume exists and DEFER is a plain nested activation; EVAL/CODE ζ are nested activations (NCB-3's own plan: emitted `call rax`, rdi = rsp-bumped frame — the compiled ARTIFACT goes to Region 3 and result VALUES to Region 2, but the activation is stack). ARBNO iterations nest strictly LIFO inside the match inside the statement. ARB re-entrancy under recursion/ARBNO bodies is NESTING — each α is its own `sub rsp` cell, LIFO by construction (the BAL precedent: BAL is structurally ARB + a depth counter and landed s34 as a pure FORTH cell with NO zls2 grant).
**TIER D RECLASSIFIED (supersedes the §13 wholesale map's Tier D and AMENDS §13's law):** ARBNO variable per-iteration data = the **s32 LINKED FRAME CHAIN stack flavor on rsp** (header prev-link written at push, traversed at β/fail-glue, count in header; raw links safe because rsp frames never move — the s32 ruling promoted to law; the s29 per-extension static-offset design STAYS dead). ARB = plain FORTH cell (fc_geom grant, BAL/SPAN shape; zls2 grant DELETED). DEFER = plain nested activation on rsp (the 64KB rt_zls_alloc donation dies). EVAL/CODE ζ = nested activations at NCB-3. §13's "no other allocation on the spine" amends to: **the spine admits fixed cells AND the linked-frame-chain flavor for variable-population constructs; both are LIFO; both release under the statement bracket.** ZB-ACT-3 (the heap-flavor γ/ω alloc/free overloads) is an ICON rung.
**GC CONSEQUENCE:** no SNOBOL4 frame ever enters Region 2 ⇒ the collector's "escapee/suspended ζ" client family is Icon-only; GC-W-1's root-set fork is not a SNOBOL4 question (the s49 census already measured this: the chain-pins-dead-frames exhibits were all Prolog).

## ⭐⭐ THE FRAME-PLACEMENT CRITERION (Lon 2026-08-27, in-chat to CEO — supersedes any box-kind or language test above)
Verbatim in substance: *"the determining factor for whether to place a BB RESULT and/or BB LOCALS [in an
activation frame] is the UNBOUNDED — i.e. unknown at compile-time — stack growth between the time a BB box
leaves at GAMMA and is resumed at BETA. Or any time UNBOUNDED growth prevents an OPERAND from being loaded by
its OPERATOR with a fixed offset."* Per box: SPINE (RSP) iff every consumer reaches RESULT/LOCALS at a fixed
compile-time-known offset on every path; unbounded γ→β growth (γ-SUSPEND is the canonical instance) or an
operator→operand distance made unknowable by dynamic growth ⇒ ζ-ACTIVATION-FRAME (RBP); program-lifetime ⇒
ζ-STANDING/root. Behavioral, language-blind — the admission predicate for the rbp re-homing. Authority:
RULES.md § BB FRAME-PLACEMENT CRITERION.
