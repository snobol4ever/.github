...
**Working theory, not yet verified by instrumentation the way the base-mismatch theory above was killed by one `fprintf`:** the r12 dcap-pend-island and/or the capture-generation counter is not being fully unwound/reset by two ordinary (non-alternation) capture statements executing in between, so by the time a self-referential alternation-capture's DEFERRED RE-ENTRY fires on its next pass, it reads back state left by an unrelated, already-finished match rather than its own. **NEXT ACTOR: instrument `head.dcap_mark`/r12 and `capgen_save` the same way — print them at the alternation-capture's α and at its deferred re-entry, across a two-intervening-captures witness — before assuming this is the same code path as the base-mismatch/ZRESD-vs-ZOPD suspect above.** It may be the identical fix, or a sibling bug in the same neighborhood; both routes funnel through `rt_dcap_pump` and both corrupt a banked start cursor, so land whichever fix falls out first and re-run BOTH rows' DONE-WHENs against it.

Not cured. `snobol4-gimpel-diff-self-match-own-name-cluster` parked `BLOCKED-ON` this row rather than duplicating investigation into the same shared, high-blast-radius runtime code.
