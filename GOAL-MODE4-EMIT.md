...
**HEAD** one4all `1cc799bc` · Gates: smoke_icon 5/5, jit_emit 11/13.

**Next:** SF-12 — delete `emit_bb_rtcall_data` (now only used by ARBNO/CALLCAP/CAP_IMM/CAP_COND which have their own inline TEXT paths; confirm zero ICN_* callers). Also SF-8 — broad corpus ≥160/163. EM-ICN-FLAT series complete (IF-0..IF-6 all closed).
