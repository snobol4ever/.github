...
- [ ] **EM-SNOCONE-PREP** — Finish cleaning up emitter code for Snocone conversion. No behaviour changes; naming, comments, and dead-code only. Gates: smoke 7/7, template-byte-id 4/4, em8 5/5.
  ...
  **Scope** (in priority order):
  ...
  - [ ] **ESP-11 — Migrate `sm_codegen.c` binary dispatch to `emitter_sm.c`.** The per-opcode `emit_me*_blob()` functions in `sm_codegen.c` use a trampoline-based blob architecture (`g_trampoline_offset`, `emit_standard_blob`, `emit_jump_blob_skeleton`, etc.) that predates the unified emitter. Replace the dispatch switch so each case calls the corresponding `emit_sm_*()` function from `emitter_sm.c` with `bb_emit_mode = EMIT_BINARY_WIRED` set at entry to `sm_codegen()`. The trampoline, `g_blob_addrs`, SEG_CODE layout, `sm_jit_run` mechanics, and pass-2 jump patching are all preserved — only the per-opcode emission body changes. Special cases: `SM_JUMP` / `SM_JUMP_S` / `SM_JUMP_F` keep their pass-2 patch records; `SM_LABEL` keeps `emit_label_blob`; `SM_STNO` keeps `emit_standard_blob_no_stack`. Gates: smoke 7/7, template-byte-id 4/4, em8 5/5, `--jit-run` output identical to `--sm-run` on all six smoke suites.
