...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **EC-UNI-21 CLOSED 2026-05-20** — `scripts/test_gate_ec_uni_complete.sh` lands (5 gates + baseline md5 + M1 reconciliation), 9/9 PASS on HEAD.  Emergency build fix in same session (c)(7): `sm_call_expression(int)` runtime helper collided with new `sm_call_expression(void)` template; renamed runtime to `sm_eval_subexpr(int)` across 5 files; added `sm_expr_incr.c` to Makefile (was on disk but unbuilt; broke HEAD `c081758f`).  M1 oracle DRIFTED — `9cddff25...` (622 lines) vs baseline `abfd19a7...` (646 lines).  **NEXT: EC-UNI-15** (top-level shape — verbose 5-arm switches family-by-family) OR **EC-UNI-21-followup** (re-converge or retire M1).  See `GOAL-HEADQUARTERS.md` watermark. |
