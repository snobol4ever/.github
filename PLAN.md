...
| **SNOBOL4 BB** | `GOAL-SNOBOL4-BB.md` | **SBL-POS-RPOS-FLAG-FIX ✅** (Opus 4.7, 2026-05-29, one4all this commit). `bb_pat_pos.cpp:14` and `bb_pat_tab.cpp:14` used `int rpos = (pBB->ival != 0)` to distinguish RPOS from POS (and RTAB from TAB). Wrong — distinguished by `pBB->sval == "r"` per lowering (`lower_pat_dcg.c` TT_RPOS / TT_RTAB / XRPSI / XRTB branches). Heuristic misclassified `RPOS(0)` as `POS(0)` and `POS(N>0)` as `RPOS(N>0)`. Bug present in BOTH BINARY and TEXT arms (same `rpos` variable) so affected mode-3 native AND mode-4 compile. Fix: one-line each, `int rpos = (pBB->sval && pBB->sval[0] == 'r')`. **Gates: G1=13/13 default+native, G2=39, G3=184/280 (+6), G4=252/280 (+1), native=220/280 (+25), rungs M2=19/M4=17 (+2: 052 and 054), Prolog/Raku/Icon smokes 5/5/5, FACT=0, audit GATE OK. Zero regressions.** Newly-passing native clusters: anchored captures with `RPOS(0)` terminator — 052, 054, 061, 069, 075, 100/101/103/105 FENCE, 116, 120-127 calc+JSON parsers, 131, 142, 145/146, 152, W06_pos, W06_rpos, global_driver. **Pruned GOAL file 363→204 lines.** Handoff `HANDOFF-2026-05-29-OPUS-SBL-POS-RPOS-FLAG-FIX.md`. **NEXT:** (a) **046/047 TAB/RTAB SIGSEGV native** — quick check for sibling alignment bug in `bb_pat_tab.cpp` BINARY arm beyond the flag fix; (b) SBL-SPAN-2 / SBL-ARBNO-3 BINARY arms with `std::deque<int>` slot pattern from `bb_capture.cpp`; (c) flip default to native (remove getenv gate `scrip.c:449`). [Prior: **SBL-BOMB-STUB-ESCAPE-FIX ✅** (`c6abd06c`) prophylactic `\\x`→`\x` cleanup of 5 placeholder BOMB-stub sites; **SBL-SPAN-ARB-ESCAPE-FIX ✅** (`44766d91`) bb_pat_span/arb double-backslash bug; **SBL-POS-PATCH-OFFSET ✅** (`61ae501e`) bb_pat_pos rel32 sites realigned; **ARBNO MEDIUM_BINARY child-gate fix ✅** (`4471b80d`); **M3-NATIVE-4 combinator flat-wire LANDED ✅** (`10f97d29`); **SBL-EP-BINARY ✅** (`1bc53211`); **bb_capture BINARY arm FIXED ✅** (`e9a9d7f3`).] |
