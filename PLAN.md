...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-PER-KIND-DIFF ✅ LANDED + GEOMETRY-CORRECTED + GATE-CADENCE-REVISED (2026-05-21 evening, Opus 4.7, one4all `9b905d26`).**  Harness operational: `tools/emit_per_kind_audit.c` + `scripts/{freeze_per_kind_baseline,test_per_kind_diff,normalize_per_kind_cell}` + `baselines/per_kind/` (3.4 MB committed).  **5 backends** (x86, jvm, net, js, wasm) × sub-modes (x86 has text/binary/text_macro; JVM and .NET may grow binary later; JS and WASM text-only).  1059 cells per run (679 BB + 380 SM); current baseline PASS=399 STUB=660 FAIL=0 NEW=0 GONE=0.  Regression detection verified — inject canary → FAIL>0 exit=1; restore → FAIL=0 exit=0.  **Gate cadence change (Lon directive):** `test_per_kind_diff.sh` is now the primary per-slice invariant (~5–10s); matrix + beauty + smoke demoted to session-end / escalation-only.  **2026-05-21 session #2/#3 (Opus 4.7) — pat-keyword reachability story corrected per Lon directive:** REM/ARB/FENCE/FAIL/SUCCEED/ABORT/BAL are pre-bound PATTERN-typed VARIABLES in both SPITBOL and scrip (verified via DATATYPE — exact match).  Bare `'hello' REM` is canonical portable form, correctly handled via PAT_DEREF runtime path.  No grammar divergence.  3 real bugs tracked: HQ-BUG-RPOS-COMPILE-SEGFAULT, HQ-BUG-RTAB-COMPILE-SEGFAULT (both --compile-only crashes on reverse pos/tab variants), HQ-BUG-PROTECTED-PATTERN-VARS (scrip allows REM='x'; SPITBOL ERROR 042).  `KEYWORD()` parens form is real scrip-only extension (maps to TT_<kind> via `pat_prim_kind`).  EC-UNI-COVER-PAT path: portable file using POS/LEN/TAB/ANY/SPAN/BREAK/NOTANY/ARBNO (8 kinds, function-call constructors); REM/ARB/FENCE/FAIL/SUCCEED/ABORT/BAL reachable to BB-lower only via scrip extension.  **NEXT: EC-UNI-REFAITH** — coverage gate exists; re-lift any kinds whose live emit_bb_x* output differs from lifted template output when wiring shifts.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 stable. |
