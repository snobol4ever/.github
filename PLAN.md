...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **NEXT: EC-UNI-10** — `g_emit` single global struct + parameterless templates (three orthogonal commits: (a) add `emit_globals.{c,h}`; (b) migrate 7 ctx-bearing SM templates + drop `sm_ctx.h`; (c) migrate BB templates + remaining `FILE *out`-bearing SM templates).  Includes the old EC-UNI-3-beauty deliverable trivially.  EC-UNI is now a single 13-step ladder (EC-UNI-10 through EC-UNI-22) replacing the old EC-UNI-3-beauty/4/5/6/7/8.x and EC-PHASE-A ladders — design notes locked in HQ GOAL: three-layer cake (top-level templates verbose with `IS_<BE>` only, Layer-2 helpers extracted via REDUCE phase at EC-UNI-16 with strict justification rule, Layer-3 primitives `emit_text`/`emit_byte`).  Snocone is the bootstrap target.  IR-CONSOLIDATE-DCG ✅ COMPLETE (IR-CD-5/6/7 landed 2026-05-20).  IR-RN-3 ✅ (2026-05-20).  **ST2 rung ✅ COMPLETE** (2026-05-20) — ST2-1/1b/1c/2 all landed: stage2_t single struct, no shim macros, dynamic sidecars, token firewall gate (`27ad177b` → `b42b7979` → `b733dd13`).  Open siblings: ISO-4 (scrip_parse subprocess), IR-RN-4/5 (arch docs + close-out).  Watermark `b733dd13`. |
