...
| Snocone Beauty | `GOAL-SNOCONE-BEAUTY.md` | corpus+one4all | **SB-6.E.7-H runtime-rollback bisect** (see goal file — session 2026-05-02 #9 closed SB-6.E.7-D `~DIFFER`→`IDENT` sweep [41 replacements across 9 .sc files] and SB-6.E.7-G zero-space jam sweep [verified 0 across all 17 files]; new diagnostic shows globals roll back across `if (Src ? (POS(0) *Parse *Space RPOS(0)))` even on successful match — explains why all 269 oracle assignment-statements drop; PROGRAM TOP runs 3× per match-attempt with partial global preservation; SB-6.E.7-H bisects this; fingerprint unchanged `lines=89 parse_err=3` (oracle 646); three baseline gates green; corpus `f4d0099`) |
