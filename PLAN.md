...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: more `lower_icn_expr_node` coverage** (TT_REPEAT, TT_ALTERNATE, TT_AUGOP, nested user-proc TT_FNC). Watermark `c4de1e69`: smoke_icon 5/5, broker 16/49, ir-run 31/265, honest 278/0/0. IJ-19-to-dyn ✅ (non-literal TT_TO bounds). |
...
| **⚡ Prolog BB JCON triage** | `GOAL-PROLOG-BB-JCON.md` | one4all+corpus+.github | **NEXT: PJ-2** — create `src/lower/lower_pl.c` + `lower_pl.h`; placeholder `lower_pl_predicate(tree_t *pred)` returning NULL. Watermark `e6af028c`: smoke_prolog 0/5 (skeleton-only baseline). **PJ-1 ✅** — `pl_bb_dcg` bridge + `g_dcg_table[]` registry + `Pl_PredEntry_BB` (AST-free) landed; mirror of Icon `icn_bb_dcg`/`proc_table`. |
