...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-6.E.6 — confirm-with-Lon anti-rationalization pass (still open; needs Lon).  **SB-6.F LANDED** session 2026-05-01 — unary `!` (T_1BANG) lexer parity with SPITBOL operator inventory.  All five sub-rungs (.1..5) closed: T_1BANG added to ScKind enum, E_UN_BANG emit label + name-table entry, S_OP_BANG unary fallback fixed (`goto E_EXP` → `goto E_UN_BANG`), .y file extended with #define/#undef aliases + %token + OPSYN-slot grammar production + sc_kind_to_tok thunk case, parser regenerated.  Six gates green throughout (smoke snocone PASS=5, smoke snobol4 PASS=7, beauty all-modes PASS=42 SKIP=3, broker PASS=49, crosscheck snobol4 PASS=6, crosscheck snocone PASS=8); zero corpus uses of tight `!`; SPITBOL `-bf` cross-check matches on all six cases (`2 ! 3 → 8`, tight `2!3 → reject`, unary `!x → parse-then-runtime-undefined-operator-error`, plus three sanity).  **Process lesson recorded**: SB-6.F.1's "add to enum" directive was under-specified — adding only the enum entry shifts all downstream `ScKind` integer values, breaking the integer-keyed lexer↔parser bridge in `sc_kind_to_tok`; the full minimal change requires coordinated enum + .y aliases + %token + thunk case + parser regen + clean rebuild.  **Two structural follow-ons surfaced**: SB-6.G (collapse redundant E_UN_* lexer trampoline labels — 14 lines of pure boilerplate) and SB-6.H (collapse `sc_kind_to_tok` per-token dispatch to a one-line cast by aligning SC_T_/T_ integer values).  Neither blocks SB-6 self-host.  SB-6 itself remains open on the grammar/runtime axis. |
