...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#4 ABORTED in-session and SUPERSEDED by iter#5 (session 2026-05-04 cont. #7). Concept change: one-letter language prefix (`s_`/`S_` SNOBOL4, `c_`/`C_` Snocone, `r_`/`R_` Raku, `i_`/`I_` Icon, `p_`/`P_` Prolog, `b_`/`B_` Rebus, `x_`/`X_` SCRIP/scrip.sc) on every parser-private identifier — function names, globals, tag-string constants, pattern-builder companions — to avoid global-table collisions when all seven `.sc` files load simultaneously into one SCRIP process. Gates: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38 (all 100% at iter#3 baseline). Next: iter#5 Steps 0-10. |
