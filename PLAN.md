...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **Option-1 (`prev==SM_JUMP` predicate) ruled out sess 2026-05-10 (Claude later)**: dual-entry semantics (intra-function labels like `assign1` are BOTH cfn-callable AND fall-through-reachable) break the simple predicate.  Patch attempted, regressed PASS=4→2, reverted.  Next session: implement Option 1 = per-chunk thunks (registry points at `.Lthunk_<pc>: sub rsp,8; jmp .L<pc>`).  Baseline PASS=4 FAIL=13 unchanged. |
