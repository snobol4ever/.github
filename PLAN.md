...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-19-remaining** — ir-run 207. Sess 2026-05-15 (Claude Opus 4.7, one4all `f82de5d0`): FAIL-arg propagation fixed in two named-proc fast-paths (icn_value.c TT_FNC + icn_runtime.c icn_bb_build); rung36_jcon_lists ✅ byte-identical (final 2-line diff closed). Prior session's SM_STORE_VAR hypothesis misdiagnosed: y := x[-3+:6] with unset y yielding null is correct semantics. Remaining: fncs1/endetab/coerce/wordcnt; CH-17g-blocked: scan/string. |
