...
|---------|--------|------|----------------|
| **TINY backend** | `asm-backend` B-219 — M-EMITTER-NAMING ✅ done; next: M-ASM-RUNG8 | `5999162` B-219 | M-ASM-RUNG8 |
...
| **M-EMITTER-NAMING** | All four emitters in one file each (`emit_byrd_c.c`, `emit_byrd_asm.c`, `emit_byrd_jvm.c`, `emit_byrd_net.c`). All share canonical names: `vars[]`, `nvar`, `var_register()`, `named_pats[]`, `named_pat_count`, `named_pat_register()`, `next_uid()`, `out`, `emit_stmt()`, `emit_pat_node()`, `NamedPat`, `FnDef`, `DataType`, `collect_vars()`, `collect_fndefs()`, `escape_string()`. C backend merged from emit.c+emit_byrd.c; byrd_ prefix externs removed. | ✅ `5999162` B-219 |
