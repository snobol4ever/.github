...
| Goal | File | Step |
|------|------|------|
| **ICON-GROUND-ZERO** | `GOAL-ICON-GROUND-ZERO.md` | **GZ-0 — Carve ✅** (Opus 4.7, 2026-05-28). New goal: Icon as 100% Byrd Boxes, two-SM-op boot (`SM_BB_RUN_THE_DAMN_ICON` + `SM_HALT`), the whole program is one connected port-graph, modes 2+4 byte-identical from each rung onward. Supersedes legacy ICON-BB (frozen below at watermark 194 mode-2 rungs). Per ARCH-ICON.md *"Icon IS a Byrd Box graph"* and GOAL-ICON-BB-NATIVE.md *"ZERO C BYRD BOX FUNCTIONS"* — restart from zero. **NEXT: GZ-1** — `procedure main() write("hello") end` runs mode 2 via the new two-op boot: define `SM_BB_RUN_THE_DAMN_ICON`, four new BB kinds (`BB_ICN_PROGRAM`, `BB_ICN_PROC`, `BB_ICN_CALL`, `BB_ICN_SLIT`), `src/lower/lower_icn_gz.c`, mode-2 executors. Watermark: 0 programs (not rungs). |
| **ICON-BB (legacy, frozen)** | `GOAL-ICON-BB.md` | Frozen at GEN-BUILTIN watermark **194 PASS / 53 FAIL / 36 XFAIL / TOTAL 283** mode-2, one4all `8cc487e9`. The `SM_BB_INVOKE`-per-statement shape was the "convalescent" path — *"one month lost"* per GOAL-ICON-BB-NATIVE.md, now two and a half months. **Do not extend.** Smoke gates still required to pass throughout ground-zero work. Scheduled deletion after GZ-7 default-flip. |
