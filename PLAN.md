...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-8 translate sm_lower() main entry point.** SL-7 ✅ sess 2026-05-11 (Claude Sonnet 4.6): gate passes (6 SM instrs, diff clean). Two fixes: lower.sc:101 truncated pick= line restored; scrip label-counter-per-file bug fixed (static global_label_seq in sc_label_new — _Ltop/_Lend labels collided across --ir-run files). one4all `snocone_parse.y`+`.tab.c`, corpus `smoke_lower.ref`. |
