...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-13 steps 3-4 — wire parser→lower→sm_dump end-to-end, verify SM output matches C oracle. SL-13a ✅ sess 2026-05-12 (Claude Sonnet 4.6): SCRIP bug fixed — `subj ? pat = repl` in if/while condition not split into stmt fields; `sc_make_cond_fail_stmt` in snocone_parse.y+tab.c now unwraps TT_ASSIGN before sc_split_subject_pattern. qize.sc re-enabled in run_scrip_parser.sh. Gate: no hang, no Error 5, smoke 6/6, sm_lower_test 11/11.** |
