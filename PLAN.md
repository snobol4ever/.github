...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **LOCAL-PURGE whole-template 2026-05-25** — charset family COMPLETE: bb_lit + bb_pat_any/notany/span/break/arb ALL fully pure across X86/JVM/JS/NET/WASM (`82fc7560` + `c793cca8`). LP-5 prereq landed: `xa_bb_ptr_slot` body pure, side-effects lifted to wrapper (`760b3edc`). GATE-PK 504/0/625 NEW=0 GONE=0, AUDIT GREEN, prolog 124/0/0, smoke 188/190-71, all byte-identical. NEXT: **LOCAL-PURGE-5** — bb_arbno(1) + bb_capture(47) driver-lift (`rt_bb_arbno_new`/`bb_cap_new_call`→`g_emit.bb_rt_obj`, `child_cache_get_lbl`→`g_emit.bb_child_lbl`, `g_cap_fixup_cb` to driver). Only remaining template-body violators (+ benign xa_epilogue). ⛔ Beauty gate SUSPENDED. |
