...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | SC-6b-bug ✅ PASS=50 FAIL=0 (sess 5: `reduce_prim(E_Parse)` fix). SC-6b-bug-segfault open. Next: **SC-6c** full beauty.sc crosscheck (timeout/perf). |
...
| **PARSER-RAKU (pattern frontend)** | `GOAL-PARSER-RAKU.md` | corpus+one4all | PASS=105 FAIL=5 — 5 RK-21 gather/take fixtures landed (sess 2026-05-07). BUG-SCRIP-WS-1 RESOLVED engine-side. Next: PARSER-RK-WS2 (canonical White/Gray pivot — bisect 10-fixture regex/capture regression) THEN RK-21 grammar code. |
| **PARSER-REBUS (pattern frontend)** | `GOAL-PARSER-REBUS.md` | corpus+one4all | PASS=75/75 ✅ corpus@`9d80577` — RB-FW-5 LANDED (sess 2026-05-06): keyword refs (`&IDENT` → `E_KEYWORD`), real literals (`E_FLIT`), pattern conditional/immediate capture (`pat . var` / `pat $ var` → `E_CAPT_COND_ASGN`/`E_CAPT_IMMED_ASGN`); extended `Id` to permit embedded `.` (mirrors `rebus.l` IDENT regex). Next: operator-directed (compound `{ }` blocks, `+:` range substring, augmented assignment, `~`/`!` pattern unaries, or `@` cursor capture). |
