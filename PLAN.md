...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT: ME-14** — Full corpus pass (`--jit-run` ≥ `--sm-run` PASS counts for all six frontends; completion criterion that reopens `GOAL-MODE4-EMIT` `EM-MODE4-IS-MODE3-DUMP`). Closed: ME-1..7 + ME-9 (all sub-rungs) + ME-10 + ME-11 + ME-12 + **ME-13**. Sess 2026-05-11: **ME-13 ✅ `5cab388d`** — r10 audit (sm_codegen never writes r10; flat-glob α-preambles load it per ARCH-x86.md) + rsp-alignment fix in `emit_me6_define_entry_blob` (4-byte `sub rsp, 8` after `mov rbp, rsp` restores `rsp%16==8` dispatch-loop invariant violated by `push rbp`). Repaired 2 prior JIT-only crosscheck regressions (`100_roman_numeral`, `test_math`). Crosscheck three-mode parity: `--sm-run` 197 / `--jit-run` 197 (was 195), identical failure sets. Gates 7/49/3, isolation PASS, smoke_icon/prolog/raku 5/5/5. Beauty drivers: 3 pre-existing JIT-only fails (`Qize_driver`, `XDump_driver`, `omega_driver`) remain — different bug (r12 wild-pointer in `me11_exec_stmt`), out of ME-13 scope, candidate ME-14 work. |
...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: initial-block once-flag (+3 rung21/25), then scan context (+5 rung05).** Sess 2026-05-11 (Claude Sonnet 4.6): AST_IF condition-value leak ✅ (+3, one4all `2f3dbc65`); CH-17g-builtin-batch ✅ (+26, one4all `c95eb2bd`); CH-17g-case-swap-null ✅ (+7, one4all `7adfdc20`). Honest 177/12/6. Gates: smoke 5/7/5/5/5/4, broker 49/49, ir-run 185. Prior: smcall-proc ✅, augop-inline ✅, loop-stack ✅, scan ✅, A0-A4 ✅. |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **PAUSED** — reopens when ME-14 closes. Mode-4 = mode-3 + SEG_CODE serializer. Tripwire: test_gate_em_beauty_subsystems_mode4 PASS=4 FAIL=13 frozen. |

| **AST Rename** | `GOAL-AST-RENAME.md` | one4all+corpus+.github | AR-3 — prose "IR"→"AST" pass (AR-1+AR-2 ✅ 2026-05-09) |
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SR-4** — dispatcher infrastructure + cohort_literal. Closed: SR-1 ✅ `f209b8d3`, SR-2 ✅ `daf27aeb`, **SR-3 ✅ `20d2fb63`** — emit_goto + expression_scope_walk + kw_canonicalize → lower_ctx.c. |
