...
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** — 11 §⛔ violations remaining (R2/R10/R11/R14 closed this session). PRF-12-self/smatch/new/mcall/hof/capture/twigil/gather-splice/gather-hoist pending. one4all `efd30e36`, corpus `84e80db`. NEXT: PRF-12-self (R18) — move self-injection from raku.y to lower_class_decl. ⚠ Always regen `raku.tab.c` with `bison -d raku.y -o raku.tab.c` after any `raku.y` change. |
