...
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **DONE** PST-RB-5a-5d ✅; PST-RB-5e ✅ (parser_rebus.sc 696→266 lines, zero fns); SL-2 closed 2026-05-17 (use-after-free in `sc_finalize_if_else_pst` fixed); SL-3/4/5 opened — SQize hangs on no-apos input, pattern-matcher BREAK/ARB spuriously succeeds, parsers load but don't yet emit valid AST. See State `findings-5f`. |
