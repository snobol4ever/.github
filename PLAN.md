...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`27b4af72`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN. **RUNG THREE-SECTION-X86 (TSX)** — PURPOSE: DELETE every binary-byte-emitting function (insn_* ×52, bb_emit_byte sink, emit_seq_*, emit_call_sym_plt, emit_sym_lea_*). Raw bytes go INLINE as std::string data in each template's MEDIUM_BINARY arm. Method: DISCOVER → INLINE → DELETE → WIRE. **DISCOVER ✅ + TSX-INLINE ✅ COMPLETE.** TSX-INLINE BB-insn batch (`27b4af72`): bb_pat_rem/len/pos/tab fully cleared of binary-arm insn_*, bb_lit isolated site inlined; added `bb_sink_str()`. Zero capture_bytes + zero insn_* in any MEDIUM_BINARY block. **NEXT: TSX-DELETE** — ⚠ BLOCKED by prereq: text-arm insn_* (`sm_defines:69`, `sm_returns:54`, `bb_lit:43-46`) emit text mnemonics; convert to `s_*asm` builders (ER/STRING-CONCAT) before deleting the insn_* family. Then TSX-DELETE → TSX-WIRE (byte-diff oracle — snapshot insn_* output BEFORE delete). ⛔ PENDING Lon directive: TEMPLATE PURE-PROJECTION. STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
