...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1.c (LS-1.a + LS-1.b drafted, session #6 2026-04-30: lexer state-machine specification + 30-test token-stream corpus written into the goal file as a new top-level `## LS-1 Lexer specification` section. Token set inventoried (literals, operators with priorities, punctuation, keywords); previous-token state machine specified with PT_VALUE/PT_BINOP/PT_UNARY_PREFIX/PT_OPEN/PT_KW_BLOCK_OPENER/PT_KW_LOOPCTRL/PT_LABEL_COLON_PENDING; CONCAT emission rule formalized (prev=PT_VALUE + next can-start-expr + whitespace_seen>0 → emit CONCAT); special cases documented for `f(args)` zero-space (combined `IDENT_LPAREN_NOSP` token), `&IDENT` keyword-name combined token, `:` always label-suffix (no goto-field colon — Snocone has no goto-field), SNOBOL4-style strings (no backslash escapes), `+`/`.` column-1 line continuation, comments as transparent whitespace. **Today's lexer audit finding**: existing `snocone_lex.c` already has most of the token set including `::` `:!:` (under STR_IDENT/STR_DIFFER), `goto`, `break`, `continue` keywords; missing `switch` `case` `default` `until` keywords; emits a `NEWLINE` token that must go (newlines = pure whitespace). **Three open lexer-design issues identified for LS-1.d**: (1) dual-role `.`/`$`/`+`/`-`/`*`/`?` CONCAT-suppression rule — default placeholder: don't emit CONCAT before any token that could be binary, leave for grammar to interpret as binary or unary by Bison precedence; (2) `~`/`@` handling — same; (3) compound-assigns `+= -= *= /= ^= %=` — default placeholder: drop, align with Andrew (Andrew's .sc has none). Q13 break/continue form still pending Lon's pick — does not block LS-1 since lexer just emits the `KW_BREAK`/`KW_CONTINUE` token regardless of which form the grammar will accept. **Next**: Lon reviews the LS-1 spec, resolves the three §6 issues, then LS-2 grammar design (Bison productions + precedence declarations).) |
