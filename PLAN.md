...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **NEXT: IEP-5** — migrate every_table tree_t* to SM_Program; replace SM_EXEC_STMT IR_block_t* with integer index. IEP-1..3+vtable+include+IR.h ✅ `1e7a7f5e`. Three pointer sidecars to eliminate: SM_EXEC_STMT (IR_block_t*), SM_BB_EVAL (every_table tree_t*), SM_PUSH_EXPR (tree_t*). IEP-4 ✅ `d9dff43a`; IEP-5/6/7 open. |
