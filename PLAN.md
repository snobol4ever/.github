...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: -n SM nullary RT-calls (CONCAT/COERCE_NUM/PUSH_NULL etc.).** -m ✅ bb_xfarb `1fcb9437`. -l ✅ sm_arith `3b431771`. -k ✅ bb_xposi `30b19814`. -j ✅ sm_jump `cc21cf01`. |
...
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SI-6** — delete CODE_t/STMT_t/stmt_ast.c/helpers; remove code_to_ast fallback in sm_preamble. SI-5 ✅ `499948f3` (all five non-SNO frontends emit AST_PROGRAM directly; byte-identical 30/30). |
