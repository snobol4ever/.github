...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | EM-7-pre LANDED sess #71 (beauty.sno emits/assembles/links/starts running; halts at SM_PAT_CAPTURE_FN). Five SM ops baked: SM_CALL (full pseudo-call vocab + INVOKE_fn fallback), SM_CONCAT, SM_PUSH_NULL, SM_COERCE_NUM, all 8 conditional return variants, SM_PAT_CAPTURE_FN_ARGS, SM_PAT_USERCALL_ARGS. STRTAB_CAP 512->8192; VSTACK_CAP 256->65536. Tracked .s files: zero UNHANDLED_OP. Gate PASS=10. Next: EM-7-final (bake SM_PAT_CAPTURE_FN + user-fn dispatch table + beauty oracle). |
