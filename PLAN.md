...
| **SNOBOL4 BB** ⛔ FROZEN — SPLIT s23p into the two concurrent front rows below | `GOAL-SNOBOL4-BB.md` | Source of truth for THE MODEL / WHACK CONTRACT / LAWS only; work routes to ALPHA/OMEGA. |
| **SNOBOL4 BB — ALPHA front** ⬅ NEW s23p (Lon 2026-08-02) | `GOAL-SNOBOL4-BB-ALPHA.md` | Allocation/admission side (ZD ladder: ZD-4→7c→7→ledger→3→close→5b-proposal→6). Concurrent with OMEGA under the file-ownership contract in the goal file. HOW = `DESIGN-SN4-ZW-ZD-OPUS-PLAYBOOK.md`. Next: A-1. |
| **SNOBOL4 BB — OMEGA front** ⬅ NEW s23p (Lon 2026-08-02) | `GOAL-SNOBOL4-BB-OMEGA.md` | Release/frame side (ZW ladder + SHED: ZW-5s2→s3→1→2→3→6→ZD-5a/5c→SHED). Concurrent with ALPHA under the same contract. HOW = the playbook. Next: O-1. |
