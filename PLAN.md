...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **UNION-BB-STUBS COMPLETE. one4all HEAD=`0bd1bff7` (pushed).** 77 identical no-op BB stubs consolidated into 5 grouped fns per HQ Invariant #10: `bb_cset` (4), `bb_lit_scalar` (4), `bb_pl` (10), `bb_icn_stub` (22), `bb_stub` (42 generic, BB_FAIL excluded). GATE-PK 420/0/639, GATE-M 305/0. **NEXT:** (1) delete dead `fmt_*` format-mode layer in emit_core.c (`g_bb_emit_format` never set to 1, `emit_bb_is_format_mode` always false — guards in emit_jmp/emit_label_define/emit_text_jmp/emit_text_label + emit_bb.c:488 + bb_lit.c:32 + scrip.c --bb-format flag); (2) resume INLINE on remaining ~8 templates (sm_call_fn/suspend, sm_bb_once_proc/pump_proc, push_expression/call_expression, sm_stno). Pre-existing (not regressions): 2 annotation-only deltas (EXEC_STMT_VARIANT `.S_ERR`, PAT_CAPTURE_FN `.S7`); shared 3-mode pattern-fence failures. |
