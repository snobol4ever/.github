...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **SI-14 ✅ sess 2026-05-12 (Claude Sonnet 4.6): SM_JUMP_INDIR one-liner — `nm = sm_pop(); pc = g_labtab[nm]` — exploits the label table lower.sc already populates. Test :($L) with stored label name. Earlier SI-13 ✅: frame stack + SM_RETURN/FRETURN/NRETURN + SM_CALL_EXPRESSION. SI-12 ✅: SM_CALL_FN via APPLY(). Structural: sm_interp_step uses switch(opc) + 120-char lines. Gates: 9/10 PASS (si_07_pat_lit pre-existing FAIL). NEXT: SI-15 — Phase 2 closing gate: real corpus programs hosted-vs-native cross-check.** |
