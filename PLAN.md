...
| **Icon BB JCON triage (39 FAILs + rung37 tests)** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-9 PARTIAL** (arch established `5a71bf11`: coro_bb_scan_gen body-gen β-resume + coro_bb_fnc scan-builtin loop; blocker: image(every write(subj_gen?body)) dispatch path fires body only once — probe with fprintf next session) |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-UNIFY** — EM-9 ✅ sess 2026-05-13h (Claude Sonnet 4.6, one4all `cdd0f967`): ABI documented in rt.h; `make jit-emit-test` target added; GOAL-CHUNKS Step 8 marked `[x]`. |
