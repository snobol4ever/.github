...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-19-remaining** — TT_SUSPEND (user proc generators, blocked on CH-17g coroutine prereq); rung01 nested-to cross-product; rung02 proc-locals frame; rung28/30/32/36 builtins. ir-run 174→192 this sess (+18), honest 273, broker 23/49. Fixes: TT_SEQ filter, TT_SEQ_EXPR, TT_NONNULL, list/table/record/key iterate, icn_bb_idx_gen, icn_bb_cat, find/bal/key(T)/upto-scan/icn_bb_scan_gen, real to-by. |
...
| **⚡ Mode-4 Full Suite: SNOBOL4+Snocone** | `GOAL-MODE4-SN4-SNOCONE.md` | one4all+corpus+.github | **NEXT: M4SN-4b continued** — broad corpus 128/280 (+4). Fixed: SM_NEG missing from codegen switch; NRETURN mode-4 rt_do_nreturn(fname) reads NV[fname] directly; RETURN_VARIANT/NRETURN_VAR chunk-depth guard (skip frame restore inside call_native_chunk). Sess 2026-05-14d (`7eb1d935`). |
