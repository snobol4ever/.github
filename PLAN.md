...
| Icon Frontend Ladder | `GOAL-LANG-ICON.md` | one4all | IC-9 (session #26, 2026-05-01: rung36_jcon_subjpos CLOSED byte-identical (+1 PASS). rung_36 PASS=12→**13**/FAIL=33→32/XFAIL=30; full ladder PASS=200→**201**/FAIL=33→32/XFAIL=30. Smoke 5/0, broker 49/0, crosscheck 4/0/0, snobol4-smoke 7/0. Closes session #25's named highest-leverage pivot (`every <->` reversible swap). Three coordinated changes: (1) new `E_REVSWAP` IR kind in `ir.h` (separates `<->` from `:=:` — pre-fix parser routed both to E_SWAP, making `<->`'s revert-on-backtrack semantics unrepresentable); parser `TK_VALSWAP` now emits E_REVSWAP. (2) `icn_bb_revswap` Byrd box in `icn_runtime.c` (~95 lines) modeled on session #21's `icn_bb_revassign`: α writes lhs/rhs left-to-right halt-on-keyword-OOB, snapshots both originals; β reverts left-to-right with short-circuit-on-failure (so a body-induced subj-mutation that strands the lhs revert also strands the rhs revert — JCON-confirmed asymmetric behaviour visible in subjpos.expected lines 61/62: `every &pos <-> x do &subj:="A"` yields `&pos=1 x=3` because lhs revert OOBs first, while `every x <-> &pos do &subj:="A"` yields `&pos=1 x=2` because lhs reverts cleanly). (3) Found and fixed latent bug in scalar `:=:` (E_SWAP) keyword-OOB semantics: pre-fix called `icn_kw_assign` for keyword sides but ignored its return, leaving state half-swapped on OOB; rewrote E_SWAP for left-to-right halt-on-fail with proper return-value gating. The atomic-all-or-nothing model I tried first was wrong — burned 2 rounds before tracing subjpos.expected line by line; semantics is definitively L-to-R halt-on-fail (subjpos lines 57/58: `&pos:=3; x:=9; &pos:=:x` → `&pos=3 x=9` lhs OOB → halt; `x:=:&pos` → `&pos=3 x=3` lhs commits, rhs OOB → halt). Files (5): `src/ir/ir.h` (+2), `src/frontend/icon/icon_parse.c` (1-line route change), `src/driver/interp.c` (+~62: icn_kw_can_assign probe sibling, E_SWAP rewrite, E_REVSWAP standalone case, un-static icn_kw_assign), `src/runtime/interp/icn_runtime.h` (+7: function decls), `src/runtime/interp/icn_runtime.c` (+~124: icn_revswap_state_t, icn_revswap_write/read helpers, icn_bb_revswap, icn_is_gen entry, icn_eval_gen wire). Total 196+/9-. Build hazard re-verified: header decl-only changes (no struct layout shift) don't strictly require clean rebuild, but ran `find src -name '*.o' -delete && build` anyway as cheap insurance per session #16/#25 lessons. **Followup pivots ranked**: scan1 OOM diagnosis (likely tiny single-cast bug, low effort to investigate); procedure values + `image(p)` (DT_PROC tag, ~3 files, unlocks mathfunc/numeric/lists/var class); `100 -- 4` cset-diff on integers (one site in `--` op handler); `&random` keyword seeding.) |
| Prolog Frontend Ladder | `GOAL-LANG-PROLOG.md` | one4all+corpus | **PR-19a** (session 2026-05-01 #5 followup STRATEGY PIVOT: PL-12 monolithic-suite-line gate deprecated after 2 weeks bridge-neutral stalled at 43/57=75% with bridge held back. Replaced by ISO-aligned capability ladder PR-13–PR-24 (per ISO/IEC 13211-1 sections) with conformance suites as downstream gates PR-30+. The bridge becomes PR-19, broken into five sub-rungs PR-19a–e, each covering one builtin's goal-as-Var dispatch independently shippable. **PR-19a is the new active rung**: `corpus/programs/prolog/rung31_bridge_catch/` written this session with 5 tests targeting the catch/3 E_VAR case — 01_var_goal_fails (silent-success), 02_var_goal_unify (env-share), 03_var_goal_arith (Term→EXPR walker arith recursion), 04_var_goal_userpred (user-pred via pl_box_choice), 05_var_goal_throw (throw propagation through synth-EXPR). Currently 0/5 PASS — each fails with exact symptom bridge fixes. Driver script `scripts/test_prolog_rung31_bridge_catch.sh` written. **NEXT SESSION:** apply v3 bridge from `one4all/docs/PL-12-session-2026-05-01-bridge.diff` (session #6 of 2026-04-30, mechanically proven correct standalone), gate rung31_bridge_catch=5/5, smoke 5/5, broker 49/49. SWI suite (PR-30) will likely regress from 43/57 — expected and acceptable since PR-30 is now downstream not driver. Commit as `PR-19a LANDED`. Then PR-19b: `rung32_bridge_negation/` for `\+ Var`, `not(Var)`, `once(Var)`. The path forward replaces "two weeks of stalled bridge work" with definite per-builtin shippable wins.) |
...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-6.E.6 — confirm-with-Lon anti-rationalization pass (still open; needs Lon).  **SB-6.H LANDED** session 2026-05-01 (this session) — single-token-enum cleanup per Lon's "one set of names, no two things kept in sync" directive.  Deleted the `ScKind` enum from `snocone_lex.h`, deleted the 200-line `%code top` `#define T_X SC_T_X` / `#undef` alias dance from `snocone_parse.y`, deleted the `sc_kind_to_tok` per-token translation function (~110 lines) plus its declaration and comment.  Bison's `enum sc_tokentype` is now the sole source of T_* names; `snocone_lex.c` includes `snocone_parse.tab.h` once and uses those names directly; the yylex thunk returns `sc_lex_next`'s value with no translation.  Net -629 lines (-280 hand-written, rest is regenerated tab.{c,h} shrinkage).  All six gates green (PASS=5/7/42 SKIP=3/49/6/8); SB-6.F's `!` semantics preserved exactly (`2 ! 3 → 8`, `2!3 → reject`, `!x → E_OPSYN runtime-undefined-error`, `1 != 2 → true`).  Bug class from SB-6.F.1 (forgetting one of five coordinated edits silently shifts enum values) is now structurally impossible.  **SB-6.F LANDED** earlier this session — unary `!` (T_1BANG) lexer parity with SPITBOL.  **SB-6.G remains open** (E_UN_* lexer trampoline cleanup — 15 one-line goto labels in `snocone_lex.c` that each do nothing but `EMIT(T_1xxx);`; narrow ~14-line cleanup, not blocking).  SB-6 self-host itself remains open on grammar/runtime axis. |
