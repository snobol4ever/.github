...
| **Raku BB** | `GOAL-RAKU-BB.md` | RK-BB-1 ✅. RK-BB-2 ✅ (`d08237e0`). RK-BB-3.0+3d ✅ (`fcac4ab3`). RK-BB-3.0a ✅ (`ba481112`). RK-BB-3.0b ✅ (`7a60d30e`). RK-BB-3a CLOSED ✅ (`cc6c1a06`). RK-BB-3b/c scaffold ✅ (`42d2a367`). **RK-BB-3b/c-binding-fix ✅** (`af0df613`, Opus 4.7, 2026-05-27): `$_` binding bug closed. Root cause was NONE of the three handoff hypotheses — actual culprit was **sigil mismatch**. raku.y:46 `strip_sigil` rewrites every Raku VAR_SCALAR to its bare name pre-AST, so body's `$_ * 2` is `TT_BINOP(TT_VAR("_"), …)`. The for-loop binding in `lower_every` already passes the sigil-stripped sval, but `lower_raku_map_or_grep` hardcoded `"$_"` (with sigil) at L1684 and L1701 → store vs body's load targeted two different variables → null read coerced to 0. Two-line fix: `"$_"` → `"_"`. Diagnostic path: hand-desugared probe (passed, isolating bug to `lower_raku_map_or_grep`) + `--dump-sm` showed PC 28 `STORE_VAR s="$_"` vs PC 32 `PUSH_VAR s="_"` — the smoking gun in two adjacent instructions. Gates: GATE-RK mode-2 12→14 (+2), Mode-3 12→14 (+2), GATE-RK4 13→15 (+2). `rk_map_grep_sort24.raku` byte-exact in all three modes. New permanent probe `rk_for_array_underscore.raku`. Smokes/broker 198/FACT RULE 0 HOLD. ZERO new BB kinds, ZERO new runtime helpers, 100% template emission preserved. NEXT: **RK-BB-4** — junctions + infix `\|`/`&` → `BB_ALTERNATE` + Bool-collapse policy on ω/γ. REUSE `bb_gen_alt.cpp`/`bb_alt.cpp`. Add junction test. |
