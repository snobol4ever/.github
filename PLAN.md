...
| NET Beauty Self-Host | `GOAL-NET-BEAUTY-SELF.md` | snobol4dotnet | S-2-bridge-7-fullscan (session #63: wire-convention alignment landed — `ReplaceMatch` fire-point for destructive subject rebind, `ProgramDefinedFunctionStack` leak fix, RETURN-time VALUE emission removed, body-assign suppression removed + OutputChannel skip added; monitor watermark **1497 → 1617** in beauty self-host; **does NOT close S-2-bridge-7-fullscan** — beauty self-host still 28 stderr lines / Parse Error at `&FULLSCAN = 1`, unchanged from baseline; unit suite 2075p/14f exact match, beauty 17/17 PASS; new divergence at step 1617 is spl's `MWT_UNKNOWN` empty-bytes encoding of NameVar diverging from dot's `NAME(5)='dummy'` — same SN-26-bridge-coverage gap; **suggested next-session paths**: extend spl bridge type encoding OR add controller wildcard for UNKNOWN-with-empty-bytes to unblock forward motion; path (a) state-snapshot-replay still cheapest route to the underlying `&FULLSCAN` parse bug. Previously session #62: `$`-alternation-backtrack fix lands as sub-rung S-2-bridge-7-dollar-alt — Executive.Assign now bails when deferred Assignee FRETURNs, ImmediateVariableAssociation2.Scan returns Failure on Exec.Failure so outer alternation backtracks correctly; minimal repro `(SPAN \$ tx \$ *match(listA, lstPat) | SPAN \$ tx \$ *match(listB, lstPat))` now matches spl byte-for-byte; unit suite 1898/1898 + 295/295 patterns, 14 corpus failures pre-existing; **does NOT close S-2-bridge-7-fullscan** — beauty self-host stderr unchanged at 28 lines / same Parse Error; investigation found (a) icase('END') is semantically correct on dot end-to-end so step-1497 is a wire-only gap, (b) isolated repro of the FULLSCAN parse fails on BOTH spl and dot but spl succeeds during real beauty self-host, meaning the gating bug depends on state accumulated between startup and the FULLSCAN parse, not just patterns/OPSYN/keywords; suggested next-session paths: state-snapshot-replay in spl alone or parallel pattern-match state dump on spl and dot; previously session #58: stno-stable fix lands, monitor advances to step 1497 with real divergence at case.inc:22 destructive `. letter = ` match-and-replace, dot has missing-fire-point in pattern-match `=` rebinding, unit suite 2075p/14f) |
