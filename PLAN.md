...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | **M3-NATIVE-REPORTING — honest finding: native Prolog mode-3 NOT implemented** (Opus 4.8, 2026-05-29, one4all `855cbee2`). Per Lon directive, gate scripts now run mode-3 as genuinely native (`SCRIP_M3_NATIVE=1`→`sm_run_native`: x86 SM asm + flat-wired x86 BB, NOT sm_interp_run/bb_exec/brokers). `test_crosscheck_prolog.sh` drops 132/0→**22/132** under true native (the old 132/0 was silently the SM *interpreter* via the --run fallback). ZERO NATIVE-GAP fallbacks — `sm_run_native` runs but produces empty output. **Root cause:** MEDIUM_BINARY arm of `SM_BB_PL_INVOKE` (`sm_bb_switch.cpp:247`) is a no-op stub `\xE8\x00\x00\x00\x00`; the real env-push+walk_bb_flat entry exists ONLY in MEDIUM_TEXT. Scripts only, FACT 0. **NEXT (headline, multi-session): M3-PL-NOINTERP** — port the MEDIUM_TEXT flat Prolog entry into raw MEDIUM_BINARY bytes + populate BB_PL_* template MEDIUM_BINARY arms (mirrors Raku M3-RK-NOINTERP-1a..1d). **Mode-2 authoritative + unchanged:** GATE-1 5/5, GATE-3 m2 104/107, GATE-SWI 57/57. Prior stands: B3 `sumto(1e7)` O(1) heap (`0019cc7b`), WAM-CP-13 print/1 mode-4 corpus 55/107 (`2fae45ec`). Handoff `HANDOFF-2026-05-29-OPUS48-PROLOG-BB-WAM-CP-6-B3-TRAIL-RECLAMATION.md`. |
