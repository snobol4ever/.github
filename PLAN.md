...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **DAI-8 SWEEP DONE ✅ 2026-05-18 (Sonnet 4.6, one4all `f82a34c9`) — C8–C16 complete. All auditable dead fns deleted. Remaining GC-dead sections anchored live. NEXT: DAI-8 C17 or new goal per Lon.** |
...
| **⚡ PST: SNOBOL4** | `GOAL-PST-SNOBOL4.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 3` — 2 §⛔ violations. W1 ✅. **Active: W2** (snobol4.y:211 `goto_expr T_CONCAT goto_atom` always-fresh-wrap — canonical §⛔ example) and **W3** (expr15/expr17 g_cur mid-rule pattern → bison-local TAL counter discipline). |
| **⚡ PST: Parent (HQ coordination)** | `GOAL-PARSER-PURE-SYNTAX-TREE.md` | one4all+corpus+.github | **PST-LR-AUDIT-1 ✅ COMPLETE** 2026-05-19 (Opus 4.7). Three-facet block (F1/F2/F3) added across all 7 PST goal files 2026-05-19 (Opus 4.7 session 4). 51 §⛔ violations identified across all six parsers; every violation owned by a named step entry in its per-language goal file. Phase 1 ⏳ for all six parsers. Phase 2 SCRIP mirror BLOCKED. **Fanned out to six parallel sessions 2026-05-19.** Six per-language rows below. |
| **⚡ PST: Snocone** | `GOAL-PST-SNOCONE.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 1` — 10 §⛔ violations. Closed rungs 4a–4k ✅. Active: **PST-SC-4l** (`sc_split_subject_pattern` → lower.c); 4m/4n; PST-SC-FLATTEN, PST-SC-LABELS, PST-SC-RET-IN-FN, PST-SC-FOR-INIT (audit-promoted). |
| **⚡ PST: Icon** | `GOAL-PST-ICON.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 2`. Active: **PST-ICN-LR-1** (parse_proc uses `_id=nparams` as out-of-band separator — blocks PST-FIELD-2). Closed: PST-ICN-2a/2b ✅, PST-ICN-4a/4b ✅ (`parser_icon.sc` 0/0 sidecar), icon_helpers.sc DELETED 2026-05-18. PST-FIELD-1/2 cross-cutting with `GOAL-PST-RAKU.md`. |
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 4` (27 §⛔ violations). Active: PRF-12 family — 5 owned sub-rungs (gather ✅, sub, class, for, self, program), 18 new sub-rungs from audit (say, print, arr-hash-ops, try, unless, given, smatch, new, mcall, die, hof, capture, twigil, my-type, body-splice, gather-splice, gather-hoist). Phase A (helper elimination): ✅ COMPLETE — raku_stubs.sc DELETED 2026-05-18 (PRF-S7-1..6, corpus `1e8bba5`); `parser_raku.sc` 0 tree-building functions, only `dq_unescape` pure string processor. PRF-13 (SCRIP mirror for gather) pending Phase 2. |
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 5` — 5 §⛔ violations. RB-C-1 ✅, RB-C-2 ✅ (TT_UNLESS, 2026-05-19). Active: **RB-C-3/4/5** + **PST-RB-DECL-1..5** (eliminate `RDecl`/`RCase`/`RProgram` outer wrapper — all info passes inside tree, no side hustles, per Lon 2026-05-19). PST-RB-PRE-BEAUTY Bug #1 ✅ 2026-05-17; Bug #2 sketched, untested. |
| **⚡ PST: Prolog** | `GOAL-PST-PROLOG.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 6` — 4 §⛔ violations all owned by **PST-PL-6h** (move `pt_flatten_conj` + `pt_maybe_ifthenelse` + `pt_make_clause` body-wrap to `prolog_lower.c`). Plus **PST-PL-6f** (DCG → tree_t conversion, non-§⛔ scope). C rungs 6a–6g ✅ 2026-05-18. |
