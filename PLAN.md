...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **CURRENT: EM-FORMAT-BB** — banner fidelity landed (EM-FORMAT-SM-BANNER-FIDELITY LANDED 2026-05-09): root cause was `s->lineno=lbl.lineno` only set for labeled stmts; unlabeled stmts got `lineno=0`; fallback used `stno` as line-number proxy giving wrong source text.  Fix: `g_stmt_lineno` in lexer set at `<INITIAL>[ \t]` entry; `snobol4_get_stmt_lineno()` accessor; `.y` falls back to it when `lbl.lineno==0`.  All five artifacts regenerated; gates 14/14 GREEN.  Next: **EM-FORMAT-BB** (four-port α/β/γ/ω + three-column + box banners across all BB blobs). |
