...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | IJ-IRALLOC-OVERFLOW landed (`6ddb9584`): IR_block_t bounds-checked; lower_icn_proc_body 128→4096; IR_ICN_BINOP beta fix. honest 272→276 (+4), broker 17→19 (+2). Session note: only 1 construct landed (below 2-construct minimum). rung18 hang not fully resolved — overflow fix is in but hang persists via different path; root cause still open. Watermark: one4all `6ddb9584` · ir-run 126/265 · honest 276 · smoke 5/5 · broker 19/49. NEXT (must land ≥2): (1) Finish rung18 hang diagnosis — IR_EVERY infinite loop despite ir_body set and 4096 alloc; debug fprintf masked it. (2) IR_BINOP_GEN real-typed arithmetic for rung18/rung36. |
...
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **PST-RB-5i triage 🔄** 2026-05-17 (Opus 4.7, emergency handoff #2): all six parser_*.sc verified end-to-end against canonical samples — none produce a tree dump yet. parser_icon.sc head restored from commit 0ecae06 (lexer-prologue `white = ( SPAN(...)` was deleted along with `notmatch` extraction in commit 16b799c). Failure surface measured: snobol4/snocone/rebus segfault; icon silent timeout (new — was syntax-error); prolog/raku bb_emit_byte overflow at 262144. Diagnostic: prolog brokered call #43 (XFNCE wrapping XOR-of-10) runs away in a single emission because each of 10 XOR alts carries the same XOR-of-20 subtree (fp fe93d1376bd7d53c × 10) — tree-to-DAG materialization is replicating shared subtrees by value. Gates floor: smoke_rebus 4/0, smoke_scrip 2/0, crosscheck 4/2. PST-RB-5i-PRE ✅ (CSO-1..CSO-8) standing. NEXT: 5i triage queues (a) icon hang, (b) prolog/raku BB overflow, (c) three segfaulters. |
