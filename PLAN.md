...
| **Icon BB JCON triage (39 FAILs + rung37 tests)** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-7 remaining** (partial `2f54cd84`: scan-gen BB fixed, multi-gen proc args fixed, integer subscript fixed; outstanding: to-by JCON pure-real truncation, cset image, ?x random. ir-run 198, honest 268, broker 23) |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-7d** — beauty PASS=14/17. Infrastructure: bb_pool_reset + exec_stmt_pool_reset added (one4all `8476b3f4`). Blocked on mode-4 error-recovery wrapper (rt_init must setjmp(g_sno_err_jmp) + g_sno_err_active=1; model: sm_run_with_recovery). Tests 4–7 skip due to missing error recovery, NOT pool exhaustion alone. See GOAL-MODE4-EMIT.md watermark. stack/counter pre-existing mode-2 bugs. |
