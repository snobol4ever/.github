...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`0a0550c1`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN. **RUNG THREE-SECTION-X86 (TSX)** — DELETE every binary-byte-emitting function; raw bytes go INLINE as std::string data in each template's MEDIUM_BINARY arm. **TSX-DELETE propagation ladder:** DEL-0/1/2 ✅, W-LIT ✅ `a3d96ebd`, **W-PORT ✅ `0a0550c1`** (inlined bb_arbno+bb_capture port-call binary arms; emit_seq_port_call/_rip NOT deletable yet — second live caller in emit_bb.c driver + TEXT arms, deletion → TSX-WIRE). **NEXT: TSX-DEL-W-PL** (bb_pl_* PLT+lea, independent of port-call), then DEL-FINAL, then TSX-WIRE. ⛔ PENDING Lon directive: TEMPLATE PURE-PROJECTION. STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
