...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **NB-3 buffer-family removal started 2026-05-25** — NB-2 `bb_lit` BINARY→`bb_bin_t` ✅ (last static-site arm). NB-3a ✅ (`bb_fail`/`bb_eps`/`bb_pat_arb`→`bb_bin_t`), NB-3b `bb_lit` TEXT→`s_*` ✅; `bb_lit` fully off buffer family. Footprint 11→6 files. GATE-PK 504/0/625, AUDIT GREEN, prolog 124/0/0, smoke 188/190-71, byte-identical. NEXT: **NB-3c** `bb_capture`/`bb_arbno` port-loop driver-lift; **NB-3d** `bb_pl_arith`/`unify`/`builtin`/`seq` conditional-BINARY driver-lift (LP-5 intern prereq + fix text-in-binary baked baselines); **NB-3e** delete `emit_jmp`/`emit_label_define`/`emit_text_jmp`/`emit_text_label`/`bb_sink_str` + `bb_emit_buf`/`bb_emit_pos`/`bb_emit_patch_rel32`. ⛔ Beauty gate SUSPENDED. |
