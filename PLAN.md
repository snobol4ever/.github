...
| CSN FENCE Bug Fix | `GOAL-CSN-FENCE-FIX.md` | csnobol4 | F-2 Step 3a (session #50: PATBCL context-mismatch diagnosed via gdb + C-trap instrumentation — SCIN3-pushed slot[1] under inner-pattern PATBCL is later read by failure walker under outer PATBCL, SCIN3 fall-through dispatches with wrong pattern → SEGV; SPITBOL re-read shows `p_fnd` does `mov xs,wb; brn flpop` — rewinds ONE TRAP REGION DEEPER than CSNOBOL4 currently does; session #51 should implement rewind-one-deeper arithmetic before considering Plan B extended seal trap with PATBCL/PATICL save; seal slot[2]=PDLPTR-3*DESCR re-verified same outcome — do NOT re-test in isolation; full diagnosis in `csnobol4/docs/F-2-Step3a-findings.md` 'Session #50 update' section; working tree clean, beauty self-host 35 lines unchanged) |
