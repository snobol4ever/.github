...
| Goal | File | Repo | Step |
|------|------|------|------|
| **CHUNKS — Eliminate SM_PUSH_EXPR** | `GOAL-CHUNKS.md` | one4all+.github | CH-7 (Step 7 next — M1 milestone close: full gate set in --ir-run/--sm-run/--jit-run on smoke_snobol4, smoke_snocone, csnobol4 Budne, Snocone corpus) |
...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | SC-6b ✅ PASS=50 FAIL=0 (sess 3: `*if_cmd` recursion fix). SC-6b-bug refined sess 4: parser grammar bug (Compiland `(E_Parse & 'nTop()')` drops first stmt on large beauty.sc inputs); fix likely `reduce_prim('E_Parse')`. SC-6c blocked. |
