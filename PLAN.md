...
5. Open the Goal file. It names the repo. Open that repo's REPO file.
6. Run the Goal file's `## Session Setup` scripts (fallback: `REPO-one4all.md`).
7. Find the first incomplete Step (`- [ ]`). Do it.
...
| Goal | File | Repo | Step |
|------|------|------|------|
| **CHUNKS — Eliminate SM_PUSH_EXPR** | `GOAL-CHUNKS.md` | one4all+.github | **NEXT ACTIVE: CH-17g-irrun-execution** — the architectural unblock for the whole Icon/Prolog mode-2/3/4 isolation ladder.  Routes `--ir-run` non-SNO through `sm_preamble` + `sm_run_with_recovery` (same path as `--sm-run`), so modes 2 and 3 share the SM dispatch runtime.  Spec at GOAL-CHUNKS-STEP17.md §CH-17g-irrun-execution — 6 steps, file-disjoint from per-kind work, byte-identity gate against pre-rung `--ir-run` baseline.  After it lands the per-kind sequenced rungs (CH-17i-bang-concat 2/3/4, CH-17i-section, CH-17i-limit-random, CH-17i-prolog-initialization) gain real anchors.  Recently closed: CH-17i-bang-concat Phase 1 ✅ 2026-05-10 (one4all `a8a064a0`); Phases 2/3/4 ✅ SEQUENCED 2026-05-10 behind CH-17g-irrun-execution (706-program audit, doc `CHUNKS-step17i-bang-concat-phase234-audit.md`, one4all `f78d366c`).  Sequence to Mode 4 Icon/Prolog: CH-17g-irrun-execution → per-kind migrations → CH-17i-mode3-completeness → CH-17g-final → CH-17i-mode4-icon-prolog → CH-17i-final-isolation. |
| **CHUNKS Step 17** | `GOAL-CHUNKS-STEP17.md` | one4all+.github | **NEXT ACTIVE: CH-17g-irrun-execution** (same as above) |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **Thunk PoC + ABI-register-handling redesign carved sess 2026-05-10 (Claude latest)**: per-chunk thunks (PoC) moved gate PASS=4→5 confirming alignment hypothesis, then reverted on Lon's correct call-out that thunks are workaround.  Proper fix: `push rbp; mov rbp, rsp` chunk prologue + `pop rbp; ret` epilogue, gated by a DEFINE'd-flag emitted by sm_lower.c (filter registry to only DEFINE'd entries; internal :S(label) targets stay as plain jump labels).  Three-step plan in watermark.  Baseline PASS=4 FAIL=13 unchanged. |
