...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1 (LS-0 LANDED, sessions 2026-04-30 #1+#2: Lon's directive captured — new Snocone is **SPITBOL functional superset** with C/C++ control flow, juxtaposition concat (no `&&`), alternative-eval `(,,)` (no `||`), backtracking-expression conditions (SUBJ ? PAT = REPL legal in `if`/`while`/`do`-`while`/`do`-`until`/`for`-test/`case`-tag), full SNOBOL4 OPSYN, Flex+Bison implementation. SPITBOL Ch.15 precedence table extracted verbatim from manual pp.181–183; Koenig `bconv[]` table extracted from `snocone.sno` L600–627. **Session #2 clarifications**: (a) keep all 14 comparison-sugar operators — 6 numeric `== != < <= > >=` lowering to `EQ/NE/LT/LE/GT/GE`, 6 lexical `:==: :!=: :<: :<=: :>: :>=:` lowering to `LEQ/LNE/LLT/LLE/LGT/LGE`, NEW 2 identity `=== !==` lowering to `IDENT/DIFFER`; (b) `for(init;test;step)` C-style `;` separator; (c) `break;` C semantics — exits innermost loop or switch only, with pros/cons documented (familiar; composes; dead-simple lowering / con: nested-switch-in-loop trap with `goto` as escape valve); (d) `%` reserved as user OPSYN config symbol — no built-in modulo, use `REMDR()`; (e) bare expressions can fail silently (no :S/:F branches generated — failure falls through to next statement, matching SPITBOL default); (f) labels `name:` and `goto name;` (single keyword, tightening Koenig's two-word `go to`); (g) C semicolon-and-curly-brace conventions including dangling-else binds inner, empty `;` and `{}` legal; (h) no special newline processing — newlines are pure whitespace, `;` is the only statement terminator. Working name "Snocone" kept. Andrew Koenig 1978 source preserved as reference (uploaded `SNOCONE.zip`). **Next LS-1**: lexer state-machine specification + 50-line test corpus with expected token streams, no code yet — Lon reviews before LS-2 grammar design. Open Q: identity-op spelling — is `=== !==` correct or should it be `:===: :!==:` matching the lexical-comparison family?) |
