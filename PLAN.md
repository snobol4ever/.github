...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.c (session 2026-04-30 #4: **LS-4.b LANDED.** Comparison/identity tier `expr5` added between `expr0` (assignment) and `expr6` (add/sub) in `src/frontend/snocone/snocone.y`. All 14 operators — numeric `==`/`!=`/`<`/`>`/`<=`/`>=`, lexical `:==:`/`:!=:`/`:<:`/`:>:`/`:<=:`/`:>=:`, identity `::`/`:!:` — lower to `E_FNC` named calls (`a == b` → `E_FNC("EQ", a, b)`; `a :: b` → `E_FNC("IDENT", a, b)`; `a :!: b` → `E_FNC("DIFFER", a, b)`). Left-associative chaining via Bison left-recursion: `a == b == c` parses as `EQ(EQ(a,b), c)`. Per Andrew's `bconv[]` (`snocone.sc` lines 32–60) all comparisons sit at one priority BELOW arithmetic add/sub, so `a + b == c + d` correctly parses as `EQ(a+b, c+d)` — verified. **T_FUNCTION call-form pulled forward from LS-4.d's original scope into LS-4.b** because `OUTPUT = EQ(2+2, 4)` is the LS-4.b headline gate from the goal file: parses to `E_FNC("EQ", E_ADD(E_ILIT(2), E_ILIT(2)), E_ILIT(4))` via `expr17`'s new `T_FUNCTION T_LPAREN exprlist T_RPAREN` rule, mirroring `snobol4.y:197`. T_FUNCTION elevated from no-value catch-all to `%token <str> T_FUNCTION`; new helper non-terminals `exprlist`/`exprlist_ne` mirror `snobol4.y:187-193`. Ten comparison-tier `%token` declarations and `T_COMMA` moved to dedicated blocks (cosmetic). No bison conflicts. New gate `test_smoke_snocone_parse_b.sh` runs `test_snocone_parse_b.c` (10 test functions / **119/119 PASS** — every comparison op shape, the EQ(2+2,4) headline, precedence below `+`, zero-arg call `TIME()`, three-arg call `SUBSTR(S,1,3)`, nested call `SIZE(TRIM(S))`, left-assoc chaining, mixed var/literal args). **Production gates remain green:** smoke 5/5, beauty 42/0/3, broker 49/0; FSM lex test 31/31; LS-4.a parse-a 35/35; combined LS-4.{a,b} 35+119=154/154. LS-4.d's step text updated to remove the now-landed call-form clause. **Archive cleanup invariant:** four active-code references to `archive/snocone_lex.*` still present (unchanged from LS-4.a); LS-4.j removes them. **Next: LS-4.c** — concat (`T_CONCAT`) / alternation (`T_ALTERNATION`) / match (`T_MATCH`) / compound-assigns (`+=`/`-=`/`*=`/`/=`/`^=`). Parses `s = 'hello' ' world'`.) |
