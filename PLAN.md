...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-invariance-rule LANDED sess #84, 2026-05-07** — corrected invariance classification (only XVAR is variant, not the 17 kinds previously excluded); simulator propagates is_variant from arg rather than forcing 1; static .data ζ emission for all box kinds (no more emitter-heap pointers baked as imm64); XNME/XFNME use static cap_t + child sub-procs + scrip_rt_patch_cap_fn startup fixup; XARBN uses scrip_rt_init_arbno for heap arbno_t allocation. Bug existed in mode 3 too (--jit-run was missing flat-BB optimization). Tracked artifacts now contain real BB blobs (4 of 5; wordcount has none — its patterns use vars). Smoke ×6 + EM 12/12 + isolation + unified_broker 49 PASS. Honest deviation: deferred capture commit not wired (test `S POS(0) LEN(5) . W` matches but W stays empty — register_capture not called for static cap_t). Next: **EM-7c-greek-purge** (rule: the Latin words alpha/beta/gamma/omega must not appear anywhere in the source base in BB-port context; only single Greek letters α/β/γ/ω, matching what bb_boxes.c, the libscrip_rt symbol exports Σ/Δ/Σlen, and the historical .byrd-reference.s files already do; one external-linker-symbol exception permitted). Then **EM-7c-bb-three-column** (emit each BB box as a 4-port cluster in three-column LABEL/ACTION/GOTO form, all four α/β/γ/ω ports contiguous and labeled, wired by direct jmp; layout-only, no macros). Then EM-7c-bb-macros (replace PLT calls with inline macro expansions per snobol4_asm.mac). Then EM-7d (blocked on pre-existing beauty self-host regression). |
