...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **A0 — `SCRIP_NO_AST_WALK=1` runtime tripwire**.  Carved sess 2026-05-10.  Sister to CHUNKS-STEP17 and to MODE3-EMIT premier — focuses on the *closure property* "every Icon BB has a real SM lowering, mode 3 stands alone."  Lon's framing: "what we have today is mode 2 in a trench coat — `--sm-run` silently falls back to `coro_eval` for unmigrated kinds."  18 rungs (A0–A7, B1–B6, C1–C4) each gated by honest-mode-3 probes (`SCRIP_NO_AST_WALK=1` env var aborts SM dispatch on any reach into `coro_eval`/`interp_eval`/`call_user_function`/`execute_program`).  Stays active alongside MODE3-EMIT per that goal's row.  See file for full plan, probe helpers, and JCON `tran/irgen.icn` reference table. |
