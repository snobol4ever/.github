...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`1f034f2d`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN, Prolog BB honest 124/0/0, smoke jit parity 184 / --run 186/75. **TSX-WIRE: BB flat γ/ω epilogue extracted into `XA_FLAT_EPILOGUE` template** (symmetric to XA_FLAT_PROLOGUE) `1f034f2d`. Plus TSX-DEADDECL-SWEEP `1f89ed04` (47 dead bb_insn_*/emit_seq_* decls deleted). **NEXT:** (a) brokered-prologue 4-byte inline (`bb_build_brokered`) → XA template — last per-box frame emission outside a template; (b) emit_form_*/sigma-delta orphan only via PURE-PROJECTION (pending Lon); (c) NO-SNPRINTF (71 snprintf → emit_fmt); (d) ER-8 relocation rethink. ⛔ PENDING Lon: TEMPLATE PURE-PROJECTION. ⛔ Beauty gate SUSPENDED. |
