...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17g (Opus 4.7): **DAI-4 ✅** (`e3c803ea`): replaced the 3 stale `icn_bb_build` call sites in `interp_eval.c` (TT_EVERY-assign-rhs, TT_EVERY-gen, TT_AUGOP-suspend-rhs) with `NO_AST_WALK_GUARD + [DAI-BOMB] + exit(78)` and removed the dangling `extern bb_node_t icn_bb_build(...)` from `sm_jit_interp.c:263`. interp_eval.c −31 lines net. **ir-run 194/265 unchanged. Zero DAI-BOMs fired** across rung suite and all 5 frontend smokes. DAI-5 (delete `icn_value.c`/`icn_stmt.c` + `icn_bb_build` body + audit `pl_runtime.c`/`raku_builtins.c`/`interp_eval.c` `bb_eval_value` ternaries) and DAI-6 (docs) deferred. Prior sessions: DAI-1 ✅ (`b3e08fe8`) removed dead-on-vine `IR_ICN_EVERY`. DAI-2 ✅ (`3e1fc9cd`) bodies → `[DAI-BOMB]` stubs (−1641 LOC). DAI-3 ✅ (`a2365c5f`) disabled `ICN_BB_EVAL` shortcut; `--sm-run` 194/265 = `--ir-run` 194/265 at parity. |
