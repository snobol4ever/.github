...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT: ME-13** — Glob integration audit (r10 discipline end-to-end; full 706-program corpus byte-identical gate). Closed: ME-1..7 + ME-9 (all sub-rungs) + ME-10 + ME-11 + **ME-12**. Sess 2026-05-11: **ME-12 ✅ `3d5f698a`** — inline-native blob for `SM_BB_PUMP`/`SM_BB_ONCE`. Single 47B `emit_me12_bb_blob` parameterized by helper fn (`me12_bb_pump` or `me12_bb_once`). No r12↔sp sync needed (bb_broker doesn't touch STATE->stack; _usercall_hook uses separate nested SM_State; coro adapters don't touch SM stack). `SM_SUSPEND_VALUE` stays on `emit_standard_blob` fallback per ME-10 precedent (no-coro PUSH(v) path mutates STATE->stack, no win to inlining). Gates 7/49/3. Prolog 135 byte-identical (2 pre-existing flaky puzzles). icon/parser: 2 deterministic baseline fails (repeat_op, until_op) + ~15 flaky pre-existing — ME-12 introduces no new deterministic regressions. Like ME-9a, blobs present but unreached by current corpus (Prolog SM dumps show 1 SM_BB_ONCE/program but execution diverges before reaching it; bare opcodes cover sm_lower "unreachable in practice" fallback paths). |
...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: fix AST_IF condition-value leak (+3 rung18), then initial-block once-flag (+3 rung21/25), then scan context (+5 rung05).** Sess 2026-05-11 (Claude Sonnet 4.6): CH-17g-builtin-batch ✅ (+26, one4all `c95eb2bd`); CH-17g-case-swap-null ✅ (+7, one4all `7adfdc20`). Honest 174/12/6. Gates: smoke 5/7/5/5/5/4, broker 49/49, ir-run 185. Prior: smcall-proc ✅, augop-inline ✅, loop-stack ✅, scan ✅, A0-A4 ✅. |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **PAUSED** — reopens when ME-14 closes. Mode-4 = mode-3 + SEG_CODE serializer. Tripwire: test_gate_em_beauty_subsystems_mode4 PASS=4 FAIL=13 frozen. |

| **AST Rename** | `GOAL-AST-RENAME.md` | one4all+corpus+.github | AR-3 — prose "IR"→"AST" pass (AR-1+AR-2 ✅ 2026-05-09) |
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SR-2** — extract `labtab_*` family + LabelEntry/PatchEntry/LabelTable to `lower_ctx.{h,c}`, migrate to GC allocation. Closed: **SR-1 ✅ Sess 2026-05-11 one4all `f209b8d3`** — LowerCtx carved out, two file-scope globals removed, CH0/CH1/LOWER* macros + inline emit_push_expr in header, byte-identical harness baked (30 programs × 6 frontends), full smoke gate green. Rename followup: `lt` → `labtab` (lt reads as less-than to SNOBOL4 brains). 15 rungs in 4 phases: foundations (SR-1..3) → cohort carve-out (SR-4..11) → statement orchestration (SR-12..13) → polish (SR-14..15). Destination: ~16 cohort files ≤300 lines, `lower_expr` ≤80 lines as a kind→handler dispatch, no globals, no inline includes, no silent fallback. Gate: byte-identical bytecode across six frontends at every rung. |
