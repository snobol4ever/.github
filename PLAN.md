...
| SNOBOL4 Frontend Ladder | `GOAL-LANG-SNOBOL4.md` | one4all | SN-18 IN PROGRESS ⚠️: DESCR_t/spec_t ABI unification — root cause of Porter 54% found: bb_usercall declared spec_t but invoked via bb_box_fn (DESCR_t); point fix + extern spec_t→DESCR_t across stmt_exec.c/bb_build.c/bb_flat.c. Porter --ir-run 54%→83.46%, --sm-run 54%→60.64%. Smoke=7 Broker=49 HELD. BROAD REGRESSED 218→172: `3+2` fails under --sm-run ("Error 5: Undefined function or operation"); --ir-run of same test passes. NEXT: bisect three-file diff to isolate which sed-stripped cast broke SM path. HEAD caac661f (committed, unpushed) | ☐ |
