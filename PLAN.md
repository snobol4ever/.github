...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1 (LS-0 LANDED, sessions 2026-04-30 #1+#2+#3+#4+#5: framing crystallized as **"Andrew Koenig's `.sc` self-host vision plus Lon's restoration of SPITBOL space-as-concat"**. Andrew's `.sc` self-host source `bconv[]` table at lines 32–60 is the canonical operator set we adopt verbatim — including **`::` for `IDENT()` and `:!:` for `DIFFER()`** which were Andrew's choices (his bootstrap `.sno` source missed these but `.sc` self-host added them). Three deletions from Andrew's `.sc` for Lon's space restoration: drop `&&` (replaced by juxtaposition SPACE pri 4), drop `||` (replaced by SPITBOL alt-eval `(e1,e2,e3)`), drop `%` (reserved as user OPSYN slot — `REMDR()` for modulo). Plus surface tightenings: `for(init;test;step)` semicolon-separated, single-keyword `goto`. Plus three additions Andrew's `.sc` lacks: `switch/case/default`, `do/until`, `break;`/`continue;` (form pending Q13 — experimental). Conditions remain SPITBOL backtracking expressions exactly as Andrew intended. **Session #5 closures**: Q10 language name = **Snocone** (Andrew's name preserved); Q11 source-tree rename = N/A (repo/build/file-extension already match Andrew); Q12 identity-op spelling = `::` and `:!:` confirmed from Andrew `.sc` lines 45–46. **Only Q13 still open**: `break;`/`continue;` form (A both `break;` and `break label;` Java-style / B labeled-only / C no-break-just-goto — default placeholder A, experimental). All session #1–#4 commitments preserved. **Next**: Lon's word on Q13 → fold in → LS-1 lexer state-machine spec + 50-line test corpus, no code yet.) |
