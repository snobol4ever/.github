...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **NEXT: IEP-5** — eliminate every_table (tree_t* keyed by SM_BB_EVAL). IEP-4 ✅ `d9dff43a`. Three rules: IR_t→no tree_t ✅ clean; SM→no IR_block_t ✅ IEP-4; SM→no tree_t ⏳ IEP-5 every_table + IEP-6 SM_PUSH_EXPR. IEP-7 audit. |
