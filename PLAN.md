...
| Icon Frontend Ladder | `GOAL-LANG-ICON.md` | one4all | IC-9 (session #29, 2026-05-02: E_SCAN SNOBOL4/Icon dispatch guard fixed — was !icn_scan_depth, now !icn_frame_depth; icon scan body now correctly sets icn_scan_subj/icn_scan_pos. Root cause of E_SEQ assign persistence bug found: icn_scope_patch visits statement-1 x before x is in scope (from statement-2 E_SEQ), so ival=-1 → NV path instead of slot write. Fix: two-pass scope patch — collect E_GLOBAL decls first, then stamp slots. PASS=201 FAIL=32 XFAIL=30 unchanged (E_SCAN fix correct but E_SEQ bug unblocked requires scope-patch fix). one4all HEAD 833a95b7.) |
