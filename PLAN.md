...
| **Raku PAT BB** | `GOAL-RAKU-PAT-BB.md` | **RK-PAT-0 ✅ plan locked — ISOLATED `BB_NFA_*` family** (Opus 4.8 + Lon, 2026-05-29). Splits Raku regex/grammar off GOAL-RAKU-BB's deferred M3-RK-NOINTERP-1e. Thesis: regex retry = Prolog redo = Icon resume = SNOBOL4 backtrack = shared `β` port; move Raku regex off its bolted-on parallel-NFA interpreter onto the BB spine. **DECISION: build a NEW isolated `BB_NFA_*` family, NOT reuse SNOBOL4 `BB_PAT_*`** — (1) isolation removes the chief risk (shared templates would let a Raku bug regress SNOBOL4's hot path); (2) NFA kinds are the more-generic basis (PAT SPAN/BREAK/etc DERIVE from CLASS+); (3) captures genuinely diverge — SNOBOL4 `$`/`.` write GLOBALS, Raku `$0`/`$<n>` are scoped match-object captures, var model not unified; (4) mode-2 PAT executor (`snobol4_pattern.c`) is SNOBOL4-runtime-bound, no generic walker to reuse. Family 1:1 from `Nfa_kind`: CHAR/ANY/CLASS/SPLIT/EPS/BOL/EOL/CAP_OPEN/CAP_CLOSE/ACCEPT (P1); ASSERT/PRED/SUBCALL/LTM (P2). Driver `BB_PUMP`, β=next-state. **Convergence DEFERRED (RK-PAT-CONV):** later collapse primitive matchers into language-agnostic `BB_MATCH_*` where byte+semantics identical; SPLIT + captures stay separate. NFA = mode-2 oracle, not deleted. **NEXT: RK-PAT-1** (BB.h enum block + `raku_nfa_to_bb` state→node walk via dormant `bb_id` hook + isolated `bb_nfa_exec` mode-2 driver + L1–L3 oracle). Mode ladder 2→4→3. Test ladder L1..L15. Phase 2 (LTM `\|`, subrules, `:ratchet`, grammars) deferred. |
