...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: TC-UNSPLIT-3 — reverse over-eager file splits. TC-SPLIT-1/2/4/6/7/8 were valid; 3/5/9/10/11 left an inconsistent on-disk state (orphan bundle files with duplicate symbols, plus 3 splits that were marked `[x]` without ever being carried out). See TC-UNSPLIT rung in goal file for full audit table + per-rung procedure. Carry over: lower.c DEFINE_ENTRY labtab fix from sess 2026-05-12c handoff still pending re-apply. Baseline: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty-subsystems mode4 PASS=6 FAIL=11. one4all `c9b7428d`.** |
