...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.i.2 IN-PROGRESS (session 2026-05-01 #2: EMERGENCY HANDOFF — Q13 resolved to Option A by Lon ("Do both forms of break with and without label"); break/continue implementation LANDED in `snocone_parse.y` and gate-green, but side-channel test extension in `test_snocone_parse_i.c` deferred to session #3.  Implementation: grammar adds `T_BREAK [T_IDENT] T_SEMICOLON` / `T_CONTINUE [T_IDENT] T_SEMICOLON` alternatives to `simple_stmt`, plus a tiny `for_lead : T_FOR { sc_pending_to_stash(st); }` non-terminal that fires before init parses (so user labels survive init's sc_append_stmt clear).  Runtime: new `LoopFrame` linked-list stack rooted at `ScParseState.loop_top`; eager `cont_label`/`end_label` allocation in `sc_while_head_new`/`sc_do_head_new`/`sc_for_head_new` so break/continue stmts emitted DURING body parsing target the same names finalize lays down; `sc_finalize_*` reuses head's labels and pops frame.  For while loops `cont_label == _Ltop_NNNN` always emitted; for do/while and for, `cont_label = _Lcont_NNNN` is **lazy-emitted** only when frame's `cont_used` flag was set by `sc_append_continue` — preserves the LS-4.f/g lowering shape verbatim for code without continue, keeps all 95 parse-g tests green unmodified.  Stacked labels (`a: b: while(...)`) accumulate in `pending_user_labels` (consumed-and-cleared by head functions; cleared on any non-head stmt commit).  `sc_loop_find_by_user_label` walks innermost-first, skips switch frames when `want_loop=1` (LS-4.i.3 forward-compat).  Out-of-context `break;`/`continue;` and unresolved-label cases call `sc_error`; parse fails with `nerrors > 0`.  Five end-to-end smoke cases pass (saved as `docs/LS-4.i.2-session-2026-05-01-smoke-driver.c`): bare break/continue in while, labeled break/continue across nested whiles, error case (break-outside-loop returns NULL), lazy-emit ON for `for+continue`, lazy-emit OFF for `for-without-continue`.  All gates green: smoke 5/5, beauty 42/0/3, broker 49/0, parse-a..i 776/776 unchanged (LS-4.i.2 added ZERO new test functions to test_snocone_parse_i.c — that's the work for session #3).  Bison emitted zero shift/reduce and zero reduce/reduce conflicts.  Session #3 should: (1) verify baseline by building+running the smoke driver in `docs/`, (2) write ~12-15 ASSERT-based test functions in `test_snocone_parse_i.c` covering the smoke cases plus stacked-label, label-on-non-loop-disambiguation, and all error cases, (3) confirm gates remain 776+N/776+N then commit closing LS-4.i.2 and advance PLAN.md to LS-4.i.3 (switch/case/default).  one4all tree CLEAN at the LS-4.i.2 implementation commit; full architectural state in goal file's LS-4.i.2 step block.) |
