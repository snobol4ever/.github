...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **REOPENED — NEXT: Lon decision on -c mode-4 wiring (A/B/C); OR proceed with -d (bb_xchr) in parallel.**  Sub-rung -c PARTIAL LANDING sess 2026-05-11 (Claude Opus 4.7): mode-3 wired through `templates/sm_halt.c` via capture-and-flush adapter (`emit_halt_blob_via_template` in sm_codegen.c); byte-identity verified by both a unit test (literal `41 ff 45 14 c3` compare) and a behavioral gate (4/4 PASS across 4 SNOBOL4 programs).  Mode-4 wiring deferred: today's mode-4 emits `call rt_halt_tos@PLT` for SM_HALT while the template + design-doc spec emits `inc [r13+20]; ret` — legitimate environment-driven divergence that needs Lon's choice between Option A (mode-4 matches template), B (template body branches on backend — design doc forbids), or C (some opcodes exempt from cross-mode templating).  Sub-rung -b ✅ earlier sess 2026-05-11.  Sub-rung -a ✅ earlier sess 2026-05-11.  Gates: smoke 7/7+49/49+5/5; jit crosscheck three-mode parity 197/197 preserved; mode-4 beauty PASS=0 FAIL=17 (pre-existing link issue, unchanged). |
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: rung13/rung02/rung11/rung23/rung24 honest fails; conjunction fix landed (74faf1d0) fires only for directly-lowered exprs — every body still legacy.** Sess 2026-05-11 (Claude Sonnet 4.6): CH-17g-scan-subject ✅ (+5, `5f6d9d8b` — `NV_GET/SET_fn` wired to `scan_subj`/`scan_pos` for `&subject`/`&pos`; strcmp not strcasecmp per RULES.md ingress-at-lex rule); CH-17g-icon-conjunction ✅ (`74faf1d0` — `AST_SEQ` with `g_lang==LANG_ICN` lowers as goal-directed conjunction via SM_JUMP_F). Honest 185/7/1. Gates: smoke 5/7/5/5/5/4, broker 49/49, ir-run 185/48/30. Prior: CH-17g-initial-once ✅ (+3), AST_IF cond leak ✅, CH-17g-builtin-batch ✅ (+26), case/swap/null ✅ (+7), smcall-proc ✅, augop-inline ✅, loop-stack ✅, scan ✅, A0-A4 ✅. |

| **Collapse STMT_t+CODE_t into AST_t** | `GOAL-STMT-INTO-AST.md` | one4all+corpus+.github | SI-1 — add AST_PROGRAM/AST_STMT/AST_GOTO_* kinds to ast.h |
| **AST Rename** | `GOAL-AST-RENAME.md` | one4all+corpus+.github | AR-3 — prose "IR"→"AST" pass (AR-1+AR-2 ✅ 2026-05-09) |
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **✅ COMPLETE** — SR-1..SR-15b done. one4all `7bed0be3`. Unlocks GOAL-SNOCONE-SM-LOWER (SL-1). | — silent default fallback → lower_unhandled + explicit per-kind table. Closed: SR-1..SR-13 ✅. **rename ✅ `cc21aa5a`** — sm_lower→lower, cohort_*→lower_*. **SR-9 ✅ `907644e5`** — icn_relop+cset+unary (24 kinds). **SR-10 ✅ `a4545102`** — icn_ctrl+data+sect (19). **SR-11 ✅ `69e7dde8`** — lower_icn_gen (7) + lower_prolog (6); legacy switch now empty. Gate: PASS=30. |
