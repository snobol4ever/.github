...
| **SNOBOL4 BB** | `GOAL-SNOBOL4-BB.md` | **LANG-IGNORANT SM TEMPLATES ✅ (Opus 4.7, 2026-05-28 late, `08e05f68`):** Ripped 9 language-sniffing dispatch forks from SM/BB templates (sm_calls Raku/JVM/NET/WASM, sm_jumps Raku label, sm_returns NET ×2, sm_bb_calls Icon proc_table). Whacked `SM_BB_PUMP_PROC` (mode-2-only Icon top-level pump; was silent no-op in modes 3/4). Split `SM_BB_SWITCH` (tag-dispatched on `SM_BBSW_ICN_GEN`/`RK_GEN`/`PL_ENTRY`) into two clean opcodes: **`SM_BB_INVOKE`** (language-ignorant BB-graph jump-in/jump-out — Icon and Raku both use it; not a CALL, no stack push/pop, just control switching between SM-land and BB-land via γ/ω ports) + **`SM_BB_PL_INVOKE`** (Prolog program entry — kept distinct because its x86 is genuinely different: env push + sibling callee blocks; **mis-design recorded — collapsible after Prolog lower refactor: emit per-predicate as ordinary SM procs like Icon does, lower BB_PL_CALL→SM_CALL_FN, env push as ordinary SM opcodes**). 15 files changed; sm_bb_calls.cpp deleted. Gates: G1=13/13 (held), G2=31 (was 36; −5 Icon, expected), G3=175/280 (held), G4=238/280 (held), rungs M2=19/M4=15 (held), FACT=0. Per-language smokes: raku 5/5, prolog 5/5, **icon 0/5** (all 5 broke when SM_BB_PUMP_PROC whacked — expected, per Lon "Makes no difference what breaks"). **NOT-DONE for future sessions:** (a) restore Icon top-level pumping by lowering top-level `every` into normal SM_BB_INVOKE+SM_CALL_FN+SM_JUMP loop (no special opcode needed); (b) restore Raku user-sub correctness by emitting explicit frame-management SM opcodes in the Raku lower (or uniform frames in rt_call) — the deleted TEXT fast-path was fixing a real vstack underflow; (c) Prolog lower refactor to enable SM_BB_PL_INVOKE→SM_BB_INVOKE collapse. **M3-NATIVE-3 ✅** (Sonnet 4.6, 2026-05-28, `910d55c3`): BB call-out confirmed; ANY pattern fires BINARY arm natively; SM_CALL_FN rdi fix; 12/13 smoke pass native. (Sonnet 4.6, 2026-05-28, `d16c6780`): JUMP/JUMP_S/JUMP_F + RETURN/FRETURN/NRETURN BINARY arms; two-pass rel32 reloc; SM_PAT_LIT rdi fix. Control flow proven native. (Opus 4.7, 2026-05-28, `58c7cab9`): ANY/NOTANY/LEN/BREAK BINARY arms now VERIFIED-BY-EXECUTION via `--run` (mode-3 WIRED — the in-process JIT that emits+executes MEDIUM_BINARY+WIRED). Resolved prior "unverified" caveat. Found+fixed latent BREAK bug (no-terminator must FAIL per SPITBOL; oracle+TEXT+BINARY all wrongly succeeded). Validated vs SPITBOL on 6 cases across modes 2/3/4. NEXT: resume SPAN/ARBNO BINARY arms (need absolute-z_orig + GC-rooted scratch — same facility as SBL-CAP-2 saved_Δ), or SBL-BREAKX-2 BINARY arm (plain-BREAK layout now proven); validate each via `--run`. See HANDOFF-2026-05-28-OPUS-LANG-IGNORANT-SM-TEMPLATES.md. |
