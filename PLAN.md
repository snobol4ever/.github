...
| **BB Template Ladder — Icon + Prolog** | `GOAL-BB-TEMPLATE-LADDER.md` / `GOAL-ICON-BB.md` | one4all+corpus+.github | GOLDEN BB RULE: BB_t≡JCON ir_*; sval/ival/dval=payload. Phase H (Attribute Grammar). **Sess 2026-05-26e (Opus): BUILD RESTORED RED→GREEN** — scrip links. Fixed sm_jit_interp.c (undocumented blocker: missing CUR_INS macro, rt.h collision, fwd decls, SM_instr_t→SM_t, bb_exec_pat_fn→exec_stmt_blob), the documented emit_sm.c Prolog serializer (XA_PL_SUB_BUILDER → bb_pl_choice_state_t.bodies[], new xa_pl_sub_body_idx), and the coupled flat-pattern pair (emit_sm.c+emit_bb.c → new bb_pat_kids_state_t aux). UNCOMMITTED. ⛔ GATES RED: pre-existing bb_exec.c Phase H bugs (NOT touched) — BB_IF runs both branches (needs H-1 γ/ω threading), `every(1 to 3)` segfaults. smoke 3/5, broker 15, rungs 118. See HANDOFF-2026-05-26-OPUS-BUILD-GREEN.md. NEXT: fix the two bb_exec.c bugs → gates → commit. |
