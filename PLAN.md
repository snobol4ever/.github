...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **rsp-alignment in user-function chunks diagnosed sess 2026-05-10 (Claude)**: 13 mode-4 diffs all share root cause — chunks lack prologue, body inherits rsp%16==8 from cfn(), eventually crashes glibc's vsnprintf via `movaps -0xc0(%rbp)`. Naive sub/add patch breaks because some named SM_LABELs are dual-entry (fall-through + cfn). Next session: gate alignment on `prev_instr.op == SM_JUMP` (option 1 in watermark). Baseline PASS=4 FAIL=13 unchanged. |
