...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **NEXT: fill inline-x86 four-port bodies** in hollow `bb_pat_span/any/notany/break/capture/arbno.cpp` PLATFORM_X86 arms — broker pattern rungs climb from RED. FACT RULE fully clean `289d4aa1`. |
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | ⛔ **TOP: Prolog RUNG LADDER via proper LOWER (PJ-AG-WIRE) + proper EMITTER (Mode-4 x86).** **Sess 2026-05-26 (Opus 4.7): builtin coverage wave 2 LANDED — GATE-3 32→48 (+16), smoke_prolog 5/5, crosscheck_prolog 132/0, icon/snocone/raku 5/5, rebus 4/4, snobol4 7/6 (pre-existing).** `one4all cf17fd90`. NEW builtins on the PJ-AGW unify pattern: `char_type/2` (rung21 5/5: alpha/alnum/digit/space/upper/lower/punct/graph/csym + digit(W)/upper/lower/to_upper/to_lower/code extractors); string I/O (rung24 5/5: `atom_string/number_string/string_upper/lower/string_concat/string_length/string_chars/codes/string_to_atom/term_to_atom/atom_number`); `copy_term/2`, `atomic_list_concat/2,3`(+sep), `concat_atom/2` (rung26 5/5); term comparison `==/\==/@</@>/@=</@>=` via new `pl_term_compare` (standard order of terms). **PRIOR sess:** builtins+findall/3 GATE-3 20→32; PJ-AGW shared-var binding. ⛔ **NEXT: (A) main/0 auto-run when no `:- initialization` — DRIVER fix, STILL NEEDS LON DECISION; lights up rung16 (`@</@>=` — operators DONE, just unreachable), rung17 sort, rung20 numbervars, etc. (B) builtins: rung17 sort/msort, rung19 format, rung20 numbervars, rung22 writeq/write_canonical, rung27 aggregate. (C) rung25_arith term_to_atom needs operator-notation term writer (`1+2` not `+(1,2)`).** Then pushback DCG, AGW-9/10 Mode-4 emitter. **DEFERRED:** SNOBOL4 Mode-4/bench; SBL-* + PA-1/2/3 + AGW-1..8 all ✅. EMITTER bb_pl_* templates: cut FILLED; seq/call/choice/alt EMPTY (AGW-9 remaining). |
