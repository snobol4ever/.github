...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#2 LANDED session 2026-05-04 cont. #7 — Aligned token blocks with `$' '`/`$'  '` whitespace surrogates; all six parsers at column-aligned token sections, language-specific whitespace direction, no raw `' '` strings in token bodies. Gates: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38 (all 100%). Next: operator picks iter#3 concept. |
