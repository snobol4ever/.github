...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **ICN-XA-1 ✅ + ICN-M4 binop-gen ✅** (`7ff8fce8`, Opus 4.7 2026-05-27c). `walk_bb_node_str_c` (open_memstream sink swap) landed; sm_bb_switch ICN_GEN arm now pure. Generator-binop routed through ONE SM_BB_SWITCH (BB_BINOP_GEN graph) + real cross-product odometer in `bb_binop_gen.cpp`. **mode4_rung PASS=2→5** (lt/mult/compound byte-exact). Gates: smoke_icon 5/5, broker 23, rungs 198, smoke_prolog 5/5, FACT 0. NEXT: ICN-M4 follow-on (BB_VAR/KEYWORD/real single-shot operands; BINARY arm); or ICN-Z-2 ω→α port wire / ICN-Z-3 CONJ full-pass. |
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | ⛔ **TOP: Prolog RUNG LADDER + mode-4.** GATE-3 **88/107**, **GATE-4 1/4** (first green m4-seq `X is 1+2`→`3`, 2026-05-27 Opus). V-1+V-2 ✅. NEXT: (1) write(atom) mode-4 NULL-atom fix (bb_pl_ls channel loss on BB_BUILTIN path); (2) V-3/AGW-9B fill `bb_pl_call`→`choice`→`alt` (greens m4-call/choice/alt). Also pending: rung18 plus/3, rung25 term_to_atom ops, rung28 catch/throw, AGW-5 stateful ITE, runtime assertz-in-body. See HANDOFF-2026-05-27-OPUS-PROLOG-BB-GATE4-FIRST-GREEN.md. |
| **Raku BB** | `GOAL-RAKU-BB.md` | one4all+corpus+.github | **RK-BB-1** — move Raku's goal-directed ~20% from eager SM onto SHARED Icon generator kinds (no `BB_RK_*` family). Keystone = one lazy `Seq` pull box (`BB_SUSPEND`+`BB_EVERY`, `BB_PUMP`); every generative construct is producer/consumer of it. Add `SM_BBSW_RK_GEN`(0x524B474E)+`lower_raku_expr_top`; range→`BB_TO_BY`, gather/take→Seq box, lazy map/grep→`BB_ITERATE` consumers, junctions→`BB_ALTERNATE`. Reuses `bb_to_by/bb_upto/bb_iterate/bb_gen_alt.cpp` ✅. Scope (Lon): lazy map/grep IN, junctions IN, regex/grammar SPLIT→GOAL-RAKU-PAT-BB, sort/try/scalar STAY SM. GATE-RK4 (Mode-4 vs `test/raku` goldens); GATE-RK 13/0 holds. |
