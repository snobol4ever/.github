...
| **RUNTIME RENAME** (Lon 2026-06-02, split from SNOBOL4-BB) | `GOAL-RUNTIME-RENAME.md` | Language-independent DE-NAME — strip language tags from `src/emitter/**` + `src/runtime/**`, name by CS concept. ✅ Emitter+runtime de-name COMPLETE (6 gated byte-identical slices `a0478c4`…`85677cb` + LI-FENCE). Only **LI-CORE** (`src/runtime/core/` SNOBOL-lib naming) remains — a runtime-unification decision for Lon, coordinates with RUNTIME REORG. |
| **RUNTIME REORG** (Lon 2026-06-02, split from SNOBOL4-BB) | `GOAL-RUNTIME-REORG.md` | Dissolve the language silos in `src/runtime/**` — each FILE becomes a CS SUBSYSTEM, not a language. RS-1 CLUSTER done (562-fn inventory + 17-subsystem partition map in the RS-1 HANDOFF). RS-2 PARTITION in progress: `runtime_eval` `970dbf5` + `unification` `17e759e` landed gated byte-identical. Next: `runtime_init`/`io_format`/`arithmetic`/`pattern_match`, then the language-named files, then `core.c`, then RS-FENCE. |
