...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **DAI-8 SWEEP COMPLETE ✅ 2026-05-19 (Sonnet 4.6) — C17 ✅ snobol4_stmt_rt.c (−447 LOC, d48681fb). C18 audit: all remaining GC-dead symbols anchored live (internal chains + PLT + header call sites). DAI-8 done. NEXT: IR-RN-0 or EC-2 or new goal per Lon.** |
...
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** — 18 §⛔ violations remaining. PRF-12-say/print/sub/die/arr-hash-ops/class/program/for ✅. PRF-12-unless PARTIAL (parser 510ad7be done; lower pending). one4all `510ad7be`, corpus `e6f7504`. NEXT: finish PRF-12-unless (add lower_unless, regen unless_*.ref, gates). ⚠ Always regen `raku.tab.c` with `bison -d raku.y -o raku.tab.c` after any `raku.y` change. |
