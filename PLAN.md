...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17d (Opus 4.7): IJ-SWAP+IJ-SEQEXPR+IJ-INITIAL landed. one4all `1fb213db`. ir-run 185→191 (+6, zero regressions). rung15 swap_basic/swap_str ✅, rung20 seqexpr_basic/side ✅, rung21/rung25 initial_once ✅. NEXT: rung31 sort (3); rung15 lconcat (`|||` deferred 4th); rung13 alt_filter; rung06 upto; rung08 find_gen; rung30 misc_seq; rung36 jcon survey (32 remaining). |
...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-2 ✅** 2026-05-17 (Opus 4.7, continuation). Two surgical fixes: (1) `corpus/SCRIP/semantic.sc::qtag` — `REPLACE(t,"'","")` → `REPLACE(t,"'",'x')` for SPITBOL's equal-length 2nd/3rd arg rule; (2) `lower_sno.c::TT_VAR` — apply `label_sanitize()` to identifiers (was: only labels). SPITBOL sweep now: snobol4+snocone+rebus+icon CLEAN ✅; raku and prolog advance from runtime/syntax errors to "missing language-specific helpers" — same wall, owned by SCT-5/SCT-6. Smoke gates hold: snobol4 6/1 (baseline), snocone 5/0. SCRIP `--sm-run` still segfaults via rt_bb_arbno; owned by GOAL-PST-REBUS. **NEXT:** SCT-5 (delete `raku_helpers.sc`, re-express in 6 allowed primitives), SCT-6 (same for prolog), SCT-1f (needs SN-26-spl-bridge in x64). |
