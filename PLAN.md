...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-PER-KIND-DIFF ✅ LANDED + GEOMETRY-CORRECTED + GATE-CADENCE-REVISED (2026-05-21 evening, Opus 4.7, one4all `9b905d26`).**  Harness operational: `tools/emit_per_kind_audit.c` + `scripts/{freeze_per_kind_baseline,test_per_kind_diff,normalize_per_kind_cell}` + `baselines/per_kind/` (3.4 MB committed).  **5 backends** (x86, jvm, net, js, wasm) × sub-modes (x86 has text/binary/text_macro; JVM and .NET may grow binary later; JS and WASM text-only).  1059 cells per run (679 BB + 380 SM); current baseline PASS=399 STUB=660 FAIL=0 NEW=0 GONE=0.  Regression detection verified — inject canary → FAIL>0 exit=1; restore → FAIL=0 exit=0.  **Gate cadence change (Lon directive):** `test_per_kind_diff.sh` is now the primary per-slice invariant (~5–10s); matrix + beauty + smoke demoted to session-end / escalation-only.  **2026-05-21 session #2 (Opus 4.7) — pat-keyword reachability probe landed in HQ doc:** 10 of 12 BB pat-kinds reachable via `KEYWORD()` parens form (REM/ARB/FENCE/LEN/POS/TAB/ANY/SPAN/BREAK/NOTANY all emit `# BOX` banners + bake as invariant); RPOS(0) and RTAB(0) **segfault `--compile`** (two NEW bugs: HQ-BUG-RPOS-COMPILE-SEGFAULT, HQ-BUG-RTAB-COMPILE-SEGFAULT).  **Portability blocker:** SPITBOL rejects `REM()` with ERROR 022; `KEYWORD()` form is scrip-specific.  EC-UNI-COVER-PAT corpus authoring BLOCKED on Lon directive: (a) record as Permitted divergence + new `scrip_ext/` corpus subtree, or (b) align scrip's `pat_prim_kind` with SPITBOL and accept 10 kinds become unreachable from SNOBOL4 frontend.  **NEXT: EC-UNI-REFAITH** — coverage gate exists; re-lift any kinds whose live emit_bb_x* output differs from lifted template output when wiring shifts.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 stable. |
