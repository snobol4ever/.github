...
| **CLI 3-mode collapse + AST-interp delete** | `GOAL-CLI-3MODE.md` | one4all+.github | CLI-3M-1..7 ✅, CLI-3M-10 ✅ (2026-05-17j). Flags `--ast-run`/`--ir-run` etc. deleted; canonical names `--interp`/`--run`/`--compile`. **NEXT: CLI-3M-9** delete 9 `interp_*.c` files → CLI-3M-11 doc pass → CLI-3M-12 unblock AR-3. |
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **DAI-8 C7 ✅ 2026-05-18 (Sonnet 4.6, one4all `2b7081c5`) — icon_runtime.c (frontend) dead-fn sweep. −118 LOC.** 12 fns + 2 globals deleted. NEXT: DAI-8 cluster 8+ — remaining: rt.c ~3 truly-deletable (rest @PLT-rescued), snobol4.c ~25, icn_runtime.c (interp) ~10. |
...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **PRF-12 step 1 (gather) — plan validated, ready to execute.** Goal convergence clarified by Lon 2026-05-18 (Opus 4.7): C parser (raku.y) and SCRIP parser (parser_raku.sc + raku_stubs.sc) must produce BYTE-IDENTICAL tree_t for the same source — one-to-one with language constructs. PRF-12 IS the convergence work, not optional polish. Step 1 (gather) plan recorded in State block: add TT_GATHER to ast.h; raku.y gather rule emits pure TT_GATHER[stmts]; new raku_lower_hoist_gather_pass in raku.y (collect-then-bulk-prepend def stmts) called from raku_parse_string; regenerate raku.tab.c via bison; parser_raku.sc GatherBlock pure shift/reduce; raku_stubs.sc gathers helpers + hoist (with run_scrip_parser.sh loading caveat). Validated this session: C-side step compiled cleanly, all five gates held at floor exactly, REVERTED for clean handoff due to 88% context window + mid-session goal clarification. Prior: PRF-8/9/10 ✅ 2026-05-18 (Opus 4.7) — finish_given/finish_gather_body/push_interp_str eliminated; PRF-1..7 ✅ 2026-05-18. **parser_raku.sc function count: 1** (`dq_unescape` permitted). Gates baseline: smoke_raku 5/0, smoke_icon 5/0, scrip_all_modes 2/0, crosscheck_snobol4 5/1 (pre-existing), crosscheck_raku 21/12 unchanged. |
