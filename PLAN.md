...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | **GROUND-ZERO + ABORTs** (Opus 4.7, 2026-05-28, one4all `e572ecce`). Per Lon directive: deleted Icon legacy SM dispatch wiring (env-gate `SCRIP_ICN_BB`, `SM_CALL_FN "main"+SM_VOID_POP`, `icn_call_builtin` body — all replaced with abort). The BB-graph lowering itself is UNCHANGED so the prior session's corpus result holds: **mode 2 PASS=200 FAIL=47 XFAIL=36 TOTAL=283** via `bb_exec_once` C tree-walker (re-verified post-reset). What I added: ABORT at every unsupported mode-3 site so reality is visible. **Critical mode-3 correction this session:** prior mode_run Icon bypass called `bb_exec_once` — proven empirically by stderr trace inside the function that BOTH `--interp` and `--run` hit it. Mode 3 was the C interp masquerading as flat-wired x86. Driver mode_run now actually calls `bb_build_flat(bbg->entry)` → seal RX → invoke slab as `bb_box_fn`. **Mode-3 score on 5 canonical (hello/add/every_to/alt/full): 0/5 — every program aborts at a named gap** (hello → `bb_lit_scalar` MEDIUM_BINARY not verified ; add → `bb_call` write(BB_BINOP) unsupported ; every_to/alt/full → `walk_bb_flat` BB_EVERY needs `flat_drive_every`). MEDIUM_BINARY arms that lacked real bodies now ABORT (bb_call had embedded `(uint64_t)&rt_write_str_nl` movabs — violates "literal bytes + g_emit args only" rule; bb_seq n>0 passthrough stub; bb_every empty; bb_lit_scalar unverified pass-through). Mode-3 dispatch additions: `walk_bb_flat` routes BB_LIT_*/BB_CALL via FILL, BB_SEQ via new `flat_drive_seq` (γ-chain walk + per-child label mint, mirrors `flat_drive_pl_seq`), BB_EVERY aborts pending `flat_drive_every`. Mode 4 deferred per directive ("complete pass at very end"); one prior mode-4 commit `f387a7b9` had hello.icn end-to-end. Gates: smoke_icon 5/5 (mode 2), smoke_prolog 5/5, smoke_raku 5/5, FACT 0. Rung corpus mode-2 unchanged at 200/283. **NEXT:** `bb_lit_scalar` MEDIUM_BINARY (mirror `bb_pl_atom` leaf), then `bb_call` MEDIUM_BINARY (needs runtime-fn-addr/string-addr patch mechanism — call_rel32 to PLT or new bin relocation kinds), then `flat_drive_every` for the generator pump. Handoff `HANDOFF-2026-05-28-OPUS-IBB-GROUND-ZERO.md` (one4all `e572ecce`). |
