...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.i IN-PROGRESS (session 2026-04-30 #13: LS-4.h LANDED — `function`/`return`/`freturn`/`nreturn` parse and lower to SPITBOL idiom (DEFINE+goto+label+body+end_label), with `return E;` inside a function lowering to `fn=E :(RETURN)` and bare `freturn;`/`nreturn;` to `:(FRETURN)`/`:(NRETURN)`. New gate `test_smoke_snocone_parse_h.sh` 85/85 PASS across 15 tests; combined parse gates 668/668 (was 583/583). Bison zero conflicts. Two-pass token rename applied: first pass `T_KW_*` → `T_*` across 17 keyword tokens (snocone_lex.{h,c}, snocone_parse.y, test_snocone_lex.c). Second pass `T_FUNCTION` → `T_CALL` (the IDENT-LPAREN call-form token from the FSM) and `T_FUNCTION_KW` → `T_FUNCTION` (the `function` keyword takes the cleaner name now that the slot is free); FSM action label `E_FUNCTION` → `E_CALL` for symmetry. Smoke test `scripts/test_smoke_snocone.sh` updated `procedure` → `function`; legacy `archive/snocone_lex.c` got `"function"` as synonym for `SNOCONE_KW_PROCEDURE` so production path (still legacy parser until LS-4.j) accepts it. All production gates green: smoke 5/5, beauty 42/0/3, broker 49/0; FSM lex 31/31. Side-channel only — LS-4.j wires the new parser into the production driver. **Next: LS-4.i** — `switch`/`case`/`default`, `break`/`continue`, `goto`, `struct`, alt-eval `(a, b, c)` → `E_VLIST`. beauty.sc-class programs reachable after LS-4.i lands.) |
