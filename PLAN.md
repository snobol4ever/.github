...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **ICN-Z-0 ✅ + ICN-Z-1 ✅** (`821640c8`) zipper foundation: `icn_leaf` + bounded flag + `icn_tree_is_leaf`. ICN-Z-2 substantial. **ICN-Z-3 BLOCKED** (finding: BB_t FINAL, no slot for E1.β → needs the atomic bb_exec.c port-follower rewrite). Gate PASS=2. NEXT: atomic ICN-Z-3..9 + bb_exec.c port-follower pass (full session). mode-4 `lt`/`mult`/`compound` gated on that whole pass — NOT isolated rungs/templates (verified 3 ways). FACT RULE 0. |
