...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-stmt-banner-fidelity LANDED 2026-05-09** — three changes: (a) new `SM_TPL_NOOP` shape (empty-body marker macro, single three-column row consumes pending `.LpcN:`); (b) `SM_LABEL` and `SM_STNO` registered as NOOP-shape templates → `emit_sm_label`/`emit_sm_stno` emit real lines (`.LpcN:                  LABEL` / `STNO`), no naked labels; (c) `emit_sm_pat_baked` rewritten — blob-absorbed PCs render as `.Lpc7:                  # PUSH_INT       baked  _pat_inv_0 pc=7..12` (col 2 = macro name with `#` prefix so the line is a comment but still scans visually; col 3 = `baked` + back-reference).  Failure mode 3 (cross-banner drift) audited and not present on the five artifacts.  Files: `sm_emit_template.{c,h}`, `sm_codegen_x64_emit.c`.  Gates 10/10 GREEN: smoke ×6, isolation, unified_broker PASS=49, EM PASS=12, bb_flat_text PASS=18, sm_phase2_sim PASS=25.  Audit: zero naked `.LpcN:` lines; zero disembodied `(baked into ...)` comments.  All five tracked artifacts assemble cleanly.  **Next: EM-7c-s-file-beautify** — single uniform three-column grid across SM-side and BB-blob lines; remove `;` separators; fold in EM-7c-bb-three-column-data conversion of bb_flat.c's 12 remaining raw `EV_TEXT` blocks. |
