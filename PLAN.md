...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **NEXT: EC-UNI-14 proper** — delete the silo walkers and `dispatch_one_x86` wrapper now that every backend routes per-PC emission through `emit_sm_dispatch`.  Once `g_emit_use_unified_dispatch` is at floor under SCRIP_UNIFIED_DISPATCH=0 across the byte-identity matrix for one session, retire the flag.  Open follow-up: split `sm_push_null()` template into `sm_push_null()` and `sm_push_null_noflip()` so x86 doesn't need the exclusion list (other backends collapse the two; x86 distinguishes the emitted mnemonic).  **EC-UNI-14(b) ✅ COMPLETE 2026-05-21** — JVM + x86 silo walkers folded into `emit_sm_dispatch`.  `dc4e6a9d` extended dispatcher with SM_BB_ONCE_PROC/SM_BB_PUMP_PROC; `5dc52dd4` collapsed `emit_jvm_one_instr` (60→1 dispatch call + SM_EXEC_STMT override matching NET's pattern); `fe195613` reduced `dispatch_one_x86` from ~80 arms to two lines (sm_op_is_dispatched gate + emit_sm_dispatch) with explicit exclusions for SM_LABEL (x86 needs `emit_sm_label_dispatch` for LABEL annotations — 504 lines in beauty) and SM_PUSH_NULL_NOFLIP (template collapses with SM_PUSH_NULL; x86 needs distinct mnemonic).  Reframe: JVM's "structurally different method-split" applies only to outer `emit_jvm_from_sm`, NOT to inner `emit_jvm_one_instr`; the latter folded by the same WASM/JS/NET pattern.  Net LOC: −68 (emit_sm.c) + 12 (emit_core.c).  Byte-identity verified under both SCRIP_UNIFIED_DISPATCH={0,1}: beauty.s md5 `40df9e004c3e963c99af716c65f2c970`, beauty.o md5 `3adbb73f88edcc5416d38baade6faf97`, smoke 5/5/4/5/7/5 (icon/prolog/rebus/raku/snobol4/snocone), broker 23/26, icon rungs 194/36/35.  **EC-UNI-14(a) ✅ COMPLETE 2026-05-20** `66cf8506` — `emit_sm_dispatch(void)` shared across WASM/JS/NET silo walkers, ~163 case-arms collapsed.  **WASM/JVM SUSPEND_VALUE fix ✅ COMPLETE 2026-05-20** `9630b549` — pre-existing bug surfaced during 14(a) recon.  **EC-UNI-14-PREREQ ✅ COMPLETE 2026-05-20** `d6e5c8f1` — plumb prog+srclines through dispatch_one_x86.  **EC-UNI-13(a–e) ✅ COMPLETE 2026-05-20** — BB_templates/bb_pat.c (16→1 consolidation, `106f26a2`); SM_templates/sm_calls.c (`8514facf`); sm_defines.c (`ab2888bf`); sm_bb_calls.c (`498a2eed`); bb_pl.c (`8c01a32c`).  **EC-UNI-12 ✅ COMPLETE 2026-05-20** — `fprintf` → `emit_textf` sweep across 862 sites.  **EC-UNI-11 ✅ COMPLETE 2026-05-20** — emit_io scaffold + 6/6 self-test.  **EC-UNI-10 ✅ COMPLETE 2026-05-20** — `7835fb9d` g_emit scaffold; `5e607294` 7 ctx-bearing SM templates → parameterless; `3088dcba` BB templates + remaining SM templates parameterless. |
