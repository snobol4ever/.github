...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.i.3 (session 2026-05-01 #3 LANDED LS-4.i.2: wrote 16 ASSERT-based test functions (tests 16–31) in `test/frontend/snocone/test_snocone_parse_i.c`, advancing parse-i 108→158 assertions and combined parse-a..i 776→826/826.  Three small helpers added at top of LS-4.i.2 test block (`count_gotos_to`, `count_labels_with_prefix`, `find_label_with_prefix`) so assertions key off semantic invariants ("exactly one goto to outer's _Lend") rather than fragile stmt-index lookups.  Coverage: bare break/continue in while • break+continue in do/while (lazy-emit triggered) • break+continue in for (lazy-emit triggered) • for-no-continue lazy-emit suppressed • dowhile-no-continue lazy-emit suppressed • for-with-continue lazy-emit • dowhile-with-continue lazy-emit • labeled break across nested while loops (verifies break outer hits outer's _Lend_0002 not inner's _Lend_0004) • labeled continue across nested loops (verifies outer _Ltop_0001 receives 2 gotos = back-edge + continue, inner _Ltop_0003 receives 1) • stacked labels `a: b: while { break a; }` and `break b;` both targeting same loop frame • label-on-non-loop disambiguation `a: x = 1; while {...}` (a attaches to assignment, NOT loop) • `break a` to non-loop-tagged label is parse error • bare break/continue at top level is parse error.  Tests 29–31 trigger sc_error → fprintf(stderr) which is expected noise (matches parse-g do/until error tests).  No source touched — pure side-channel test extension.  Build clean under `-Wall -Wextra` for new test code.  All gates green: smoke 5/5, beauty 42/0/3, broker 49/0, parse-a..i 826/826, Bison 0/0 conflicts.  Previous LS-4.i.2 implementation context preserved in goal file's LS-4.i.2 step block — session #2 EMERGENCY HANDOFF narrative recorded the implementation (`snocone_parse.y` grammar adds `T_BREAK [T_IDENT] T_SEMICOLON` / `T_CONTINUE [T_IDENT] T_SEMICOLON` alternatives + `for_lead : T_FOR { sc_pending_to_stash(st); }` to preserve user labels across init parse; `LoopFrame` linked-list stack rooted at `ScParseState.loop_top`; eager cont/end label allocation in `sc_*_head_new`; lazy-emit of `_Lcont_NNNN` pad in `sc_finalize_for`/`sc_finalize_do_while` only when frame's `cont_used` flag set; while-loop reuses _Ltop as cont target so no separate _Lcont pad; `sc_loop_find_by_user_label` walks innermost-first and skips switch frames when `want_loop=1` for LS-4.i.3 forward-compat).  one4all advances to the LS-4.i.2-close commit; corpus/csnobol4/x64 unchanged.  Next: LS-4.i.3 — `switch (e) { case v1: S1; case v2: S2; default: SD; }` lowering to `tmp = e; IDENT(tmp,vN) :S(cN); :(cd); cN SN :(after); cd SD; after`.) |
