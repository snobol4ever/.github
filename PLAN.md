...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`2ef2926c`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN, Prolog BB honest 124/0/0. **RUNG THREE-SECTION-X86 (TSX)** — DELETE every binary-byte-emitting function; raw bytes go INLINE as std::string data in each template's MEDIUM_BINARY arm. **TSX-DELETE propagation ladder:** DEL-0/1/2 ✅, W-LIT ✅ `a3d96ebd`, **W-PORT ✅ `0a0550c1`**, **W-PL ✅ `2ef2926c`**. **Every template MEDIUM_BINARY arm now inline raw bytes.** KEY FINDING: no W-* rung orphaned its wrappers (emit_seq_port_call* live in emit_bb.c driver; emit_call_sym_plt/emit_sym_lea_rcx live in bb_lit+sigma helpers); insn_* frozen at 23 → bulk deletion concentrates at **TSX-WIRE/DEL-FINAL**, not the W-* rungs. **NEXT: TSX-WIRE** (multi-session — delete emit_bb.c binary driver + open the emit_sm.c forced-TEXT detour; prerequisite for DEL-FINAL). ⛔ PENDING Lon directive: TEMPLATE PURE-PROJECTION. STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
