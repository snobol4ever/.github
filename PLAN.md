...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **EC-3b ✅ COMPLETE 2026-05-19 (Sonnet 4.6, one4all `774c1e0e`) — sm_arith.c: 9 arithmetic fns unified. EC-3a ✅: sm_push_pop_lits.c 7 fns, SM_templates/ created, jvm_push_int2+jvm_emit_ldc_string promoted. 48 silo arms → one-liners. Gates floor 5/0·23/26·194/36. NEXT: EC-3c comparison+jump family (SM_ACOMP, SM_LCOMP, SM_JUMP/S/F → SM_templates/sm_control.c).** |
...
| **⚡ PST: Parent (HQ coordination)** | `GOAL-PARSER-PURE-SYNTAX-TREE.md` | one4all+corpus+.github | **PST-LR-AUDIT-2 ✅ COMPLETE 2026-05-19 (Opus 4.7).** All 51 violations confirmed closed (49 prior + R15 rescoped + R27 dead code deleted 2026-05-19 Sonnet 4.6). **1 gating item remains for Phase 2:** (1) **PST-SC-SWITCH-LABELS** in `GOAL-PST-SNOCONE.md` — switch finalizer still synthesizes `_Lend` label. **Once it lands:** Phase 2 ordering PRF-13 → PST-SN4-SC → PST-SC-SC → PST-RB-SC → PST-PL-SC → PST-ICN-SC, one language per session. |
| **⚡ PST: Snocone** | `GOAL-PST-SNOCONE.md` | one4all+corpus+.github | **⚠ Phase 1 C — V13-switch REOPENED by AUDIT-2.** 10 of 11 §⛔ violations confirmed closed. `sc_finalize_switch_pst` still pushes `TT_QLIT(_Lend_NNNN)` as last child of TT_CASE (snocone_parse.y:868–949). **NEXT: PST-SC-SWITCH-LABELS** — mirror what PST-SC-LABELS did for while/do/for. Plus optional PST-SC-DOC-CLEANUP. |
| **⚡ PST: Icon** | `GOAL-PST-ICON.md` | one4all+corpus+.github | **✅ AUDIT-2 VERIFIED 2026-05-19 (Opus 4.7)** — PST-ICN-LR-1 ✅ PST-FIELD-1 ✅ PST-FIELD-2 ✅. tree_t struct verified exactly {t,v,n,c} (src/include/ast.h:80-90). Phase 2 SCRIP mirror CLEARED. |
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **✅ Phase 1 C COMPLETE 2026-05-19 (Sonnet 4.6, one4all `50dee1c2`).** All 27 R-rows closed. PRF-12-R15-DISPOSITION ✅ (R15 rescoped as parser-local-scratch idiom). PRF-12-DEADCODE ✅ (dead make_for_range + raku_lower_hoist_gather_pass deleted). NEXT: PRF-13 (SCRIP mirror for PRF-12-gather) — Phase 2, gated on Snocone PST-SC-SWITCH-LABELS. |
| **⚡ PST: Prolog** | `GOAL-PST-PROLOG.md` | one4all+corpus+.github | **✅ AUDIT-2 VERIFIED 2026-05-19 (Opus 4.7)** — pt_flatten_conj+pt_maybe_ifthenelse+pt_make_clause moved to prolog_lower.c. Zero hits in prolog_parse.c. Phase 2 SCRIP mirror CLEARED. **NEXT: PST-PL-SC-1** (when Phase 2 gate opens). |
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **✅ AUDIT-2 VERIFIED 2026-05-19 (Opus 4.7)** — all 6 §⛔ violations closed; RDecl/RDKind/RProgram deleted; RCase parser-local scratch. Phase 2 SCRIP mirror CLEARED. **NEXT: PST-RB-SC-1** (when Phase 2 gate opens). |
