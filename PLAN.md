...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1.c (LS-1.a + LS-1.b drafted, sessions 2026-04-30 #6+#7: lexer state-machine specification + 30-test token-stream corpus written into the goal file as new top-level `## LS-1 Lexer specification` section. Token set inventoried (literals, operators with priorities, punctuation, keywords); previous-token state machine specified with PT_VALUE/PT_BINOP/PT_UNARY_PREFIX/PT_OPEN/PT_KW_BLOCK_OPENER/PT_KW_LOOPCTRL/PT_LABEL_COLON_PENDING; CONCAT emission rule formalized (prev=PT_VALUE + next can-start-expr + whitespace_seen>0 → emit CONCAT); special cases for `f(args)` zero-space combined token, `&IDENT` keyword-name combined token, `:` always label-suffix, SNOBOL4 strings (no escapes), `+`/`.` line continuation, comments transparent. **Today's lexer audit**: most token set already present including `::` `:!:` (under STR_IDENT/STR_DIFFER), `goto`, `break`, `continue`; missing `switch` `case` `default` `until`; emits a `NEWLINE` token that must go. **Three open §6 issues**: (1) dual-role `.`/`$`/`+`/`-`/`*`/`?` CONCAT-suppression — placeholder: don't emit CONCAT before any token that could be binary, leave to Bison precedence; (2) `~`/`@` same; (3) compound-assigns `+= -= *= /= ^= %=` — placeholder: drop, align with Andrew. **Session #7 amendment**: `procedure` keyword renamed to `function` since these forms return a value (Lon's directive). KW_TABLE `procedure` removed and `function` added; goal file's done-when criterion 6, comparison-of-tables row, control-flow keyword list, LS-2 corpus-coverage list, and §1.Keywords KW_TABLE-spec all updated. Q13 break/continue form still pending. **Next**: Lon reviews the LS-1 spec, resolves §6 issues, picks Q13 form → LS-2 grammar design.) |
