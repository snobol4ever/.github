...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-4b (session #67 closed SB-4b.2..SB-4b.7 — case/assign/match/stack audited as already faithful; counter.sc rewritten 6→16 procs adding canonical BegTag+EndTag families per counter.inc; tree.sc rewritten from MakeNode/MakeLeaf stubs to full canonical 9-proc ADT (Tree/Append/Prepend/Insert/Remove/Equal/Equiv/Find/Visit) verified by /tmp/tree_smoke.sc 7 tests pass. Snocone-port discovery: parser do_procedure does not accept leading-comma "(, locals)" form — silently corrupts parse and hangs at runtime; workaround is declaring locals as zero-defaulting params. Gates remain PASS=5/PASS=42 SKIP=3/PASS=49. Active rung: SB-4b.8 (ShiftReduce.sc) — audit found two gated semantic gaps documented in rung. SB-4b.14 (omega.sc) audit found doParseTree integer-vs-null mismatch + EVAL'd-source-text vs variable-substitution mismatch, both gated on xTrace>0. SB-5b beauty.sc binary &/~ rewrite remains the gating issue for end-to-end self-host. session #66 progress kept: SB-4b.1 global.sc, SB-4b.15 semantic.sc, SB-4b.16 trace.sc) |
