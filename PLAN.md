...
7. **If MODE3-EMIT or MODE4-EMIT, or ANY Byrd-box codegen / template / BB-LOCAL-STORAGE work (every `GOAL-*-BB` rung qualifies) — read the BB-CODEGEN DESIGN SET first, NON-NEGOTIABLE:**
   ...
   - `ARCH-SCRIP.md` — **STALE.** Describes a 3-mode system (mode 2 `--run`/SM-interp, mode 3 `--run`/SM-JIT, mode 4 `--compile`) built on `sm_lower`→`sm_interp_run`/`sm_jit_run`. Verified: `sm_interp_run` and `sm_jit_run` do not exist anywhere in `src/` (`grep -rn` empty); only `sm_lower` survives, as a bare failure-message reference in `scrip_sm.c`. Current reality (confirmed via `scrip.c`'s argv parser): exactly two modes, `--run` (native x86 binary in-process) and `--compile` (x86 text asm → gcc → exec) — see `GOAL-IR-IMMUTABLE-EMIT.md`'s ORIENTATION SYNOPSIS for the authoritative current description, which IS accurate (independently re-verified 2026-06-30, not just trusted).
   - **For routine Icon-only BB/codegen work, skip this whole numbered list and read `GOAL-IR-IMMUTABLE-EMIT.md`'s own "ORIENTATION SYNOPSIS" section instead** — it was written specifically to replace this stale six-doc detour and has been independently re-verified against the live repo (build, smoke, gates, corpus all reproduced exactly as documented, 2026-06-30).
