...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **JA-D-2** — excise Engine B (SB-LINEAR block) from sm_codegen.c (was sm_jit_interp.c). JA-D-0 pinned, JA-D-1 committed `c352bf4d`. Violator grep 192→0 target. JA-D-6 queued: total annihilation of "jit"/"JIT" naming after JA-D-5. |
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | ⛔ **TOP: Prolog RUNG LADDER via proper LOWER (PJ-AG-WIRE) + proper EMITTER (Mode-4 x86).** **Sess 2026-05-26 (Opus 4.7): DCG `phrase/2,3` lower-time rewrite (`0c547c01`) + compound-`write/1` (`f43aff1d`) — GATE-3 rung suite 16→19; smoke_prolog 5/5; honest 124/0/0; smoke_icon/snobol4 green.** phrase(NT,L[,R])→NT(L,R) in lower_pl_goal; BB_PL_VAR→DT_DATA for bound compounds + write DT_DATA arm via pl_write. rung30 basic_terminals/nonterminals/phrase3 PASS. **Sess 2026-05-26 (Opus 4.7): PJ-AGW shared-var unify binding LANDED — honest 123→128, GATE-3 19→20 (rung06 lists), ASAN clean.** bb_exec.c BB_PL_CALL: slot-copy writeback → WAM shared-var unification (control state in bb_snapshot, ALL binding in trail); `is` unifies result (not raw slot overwrite); resume does NOT pre-unwind (inner choice points own their trail; unwind to trail_mark only on exhaustion). Fixes append/3 nested-`[H|R]` tail (`[a,b]`), reverse/2, member terminates; prior attempt's rung05/rung10 regressions avoided. BONUS succ/2 builtin (correct in isolation; rung18 blocked by orthogonal main/0 auto-run gap). ⛔ **NEXT (see GOAL-PROLOG-BB.md top watermark): (A) main/0 auto-run when no `:- initialization` directive — DRIVER fix (RULES.md forbids corpus patching); lights up rung12–28 — NEEDS LON DECISION. (B) findall/3 live BB path (rung11_*+dcg_generate; reuse pl_copy_term, NOT the deleted AST pl_box_goal_from_ir). (C) builtins functor/arg/=.. (rung09), @</@>= (rung16), sort (rung17) via the unify pattern.** Then pushback DCG, AGW-9/10 Mode-4 emitter. **PRIOR (still valid):** SNOBOL4 Mode-4/bench DEFERRED; SBL-* + PA-1/2/3 + AGW-1..8 all ✅. EMITTER bb_pl_* templates: cut FILLED; seq/call/choice/alt EMPTY (AGW-9 remaining). |
