...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | **SB-6 self-host proper** is the active rung.  Goal file shrunk session 2026-05-01 #4 (this session) from 4171 → 369 lines (-91%, -3987 lines) — closed-rung narrative folded into one-line markers; commit history preserves the detail.  **End-to-end state (from session #3 emergency handoff, preserved verbatim in goal file):** `./scrip --ir-run $LIBS beauty.sc < beauty.sno` produces 1495 lines rc=0 but interleaves real input lines with 17 'Internal Error' + 417 'Parse Error' lines.  Tightest reproducer: `echo START` yields `Internal Error\nSTART\n\n` instead of oracle's `START\n`.  Smoking gun: on a successful outer parse, `Pop()` returns DATATYPE='PATTERN' instead of a tree node.  Two hypotheses to discriminate next session — **H1** (more likely): `epsilon . *Reduce(...)` deferred call inside `semantic.sc::reduce` not firing at match time; **H2**: `*Pat` deferred-pattern-reference being captured-by-value.  Four-step diagnostic plan at the bottom of the slim goal file.  Side issue **SC-MERGE-RESTART** tracked: prefix-of-beauty.sc + custom test produces repeated-output infinite loop, possibly label-name collision under merged-IR.  **SB-6.E.6** (confirm-with-Lon anti-rationalization pass) still requires Lon.  **SB-6.G** (E_UN_* lexer trampoline cleanup, ~14 lines) still open, narrow, not blocking.  All six gates green (PASS=5/7/42 SKIP=3/49/6/8). |
