...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.a (session 2026-04-30 #2: **LS-3.f COMPLETE — all three sub-steps landed.** Threaded-code FSM lexer at `src/frontend/snocone/snocone_fsm.c` (~700 lines), public API at `snocone_fsm.h`. Single function `sc_lex_next(ctx)` returns one token per call, label/[if]/action/goto column form, zero `switch/for/while/do` in body (verified post-preprocessor). Comments-as-whitespace handled via S_LCOMMENT and S_BCOMMENT/S_BC_STAR sub-FSMs. **Dead code deleted in LS-3.f.2** (one4all `55a30bcb`): `ScTokenBuf`, `ScToken2`, `LexerState`, `snocone_lex2()`, `sc_token_buf_free()` — all gone, ~470 lines net deletion. Test harness moved to `test/frontend/snocone/test_snocone_fsm.c`, drives `sc_lex_next` directly token-by-token, no buffer. Gates at handoff: LS-1.b 31/31, smoke 5/5, beauty 42/0/3, broker 49/0. Production link path still uses old hand-written `snocone_lex.c` until LS-4.j wires in `sc_lex_next`. **Next: LS-4.a** — Skeleton `snocone.y` Bison grammar with atoms (T_INT/T_REAL/T_STR/T_IDENT/T_KEYWORD) + arithmetic (`+`, `-`, `*`, `/`, `^`) + paren-grouping. Public entry `snocone_parse_program()`. The Bison parser will pull tokens by calling `sc_lex_next(ctx)` directly via a tiny `yylex` thunk. Token kind names already match snobol4.tab.h conventions so the grammar's `%token T_*` declarations transcribe directly from the FSM enum. First gate: parses and runs `OUTPUT = 2 + 3`.) |
