...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7d-prep LANDED sess #81, 2026-05-07** — SM_PAT_CAPTURE_FN/USERCALL (opcodes 51–54) emitters + runtime entries added; beauty.sno emits/assembles/links cleanly (exit 0, zero UNHANDLED_OP). Two deviations filed: (a) `_rt_usercall` returns FAILDESCR for user-defined functions (re-entrancy from BB match into SNOBOL4 call stack segfaults); (b) `--sm-run` itself broken for beauty self-host (10 lines, pre-existing). **Next: EM-7d-usercall-reentrant** (native chunk fn-ptr registry so _rt_usercall dispatches without interpreter re-entry), then diagnose --sm-run beauty regression, then EM-7d. |
