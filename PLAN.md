...
| **Icon BB JCON triage (39 FAILs + rung37 tests)** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-11** (IJ-10 ✅ `88db975a`/corpus `3df52f6`: ICN_KW_SWAP frame-slot write + lhs/rhs order; rung36_jcon_subjpos PASS; ir-run 199/36, honest 270. IJ-11 stash ready in one4all: icn_kw_read infra done; blockers: type() cset sentinel + image() null + generative keywords.) |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-BB-FORMAT** — EM-REORG-1..9 ✅ (sess 2026-05-13s): emit_bb_gen.c fully drained + deleted, 3 files renamed. one4all `71c31336`. Gates: smoke 7/7, byte-id 4/4, em8 5/5. **EM-REWRITE queued (HELD pending Lon sign-off).** |
