...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | **SWI-2-pre ✅** (Opus 4.7, 2026-05-28, one4all `cda40a70`): findall determinism guard — one-line `if (!bb_body_has_live_choice(fs->gcfg)) break;` in `bb_exec.c` findall loop, mirroring BB_PL_CALL discipline at line 3340. Closes handoff bug (C) — findall no longer re-fires deterministic bodies. Bare fact `findall(X,fact(X),L)` → `[only_one]`; non-det `findall(C,color(C),L)` → `[red,green,blue]`. All gates byte-identical to `86abe166`: G1=5/5, G2=132/0, G3 m2/m3=104/107, G4=4/4, m4 corpus=54/107, FACT=0. **NEXT: SWI-2a** (implement `clause/2` mode-2) then SWI-2b (plunit.pl rewrite using `clause(test(N,O),G)` enumeration + `test/1` normaliser). [Prior: **SWI-1a ✅** (Sonnet 4.6, `86abe166`) directive whitelist; **WAM-CP-1/2/3/4/5 ✅. WAM-CP-9 partial 🟡. WAM-CP-10 partial 🟡** (Opus 4.7, `5427e12e`).] |
