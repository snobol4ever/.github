...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **PPV-1 ✅ LANDED 2026-05-21 session #5 (Opus 4.7, one4all `44a5f9a5`).**  Runtime protection of REM/ARB/FENCE/FAIL/SUCCEED/ABORT/BAL — scrip ERROR 042 matches SPITBOL on user-program reassign.  Closes HQ-BUG-PROTECTED-PATTERN-VARS.  Helper `src/runtime/rt/rt_protected.{h,c}` with shared `{name→SM_op_t}` table (reused by PPV-2).  PK PASS=399 unchanged; SNOBOL4 smoke --run=186 unchanged.  PPV-0 inventory landed prior at .github `afa893a0`.  **NEXT: PPV-2** (lower.c:373 substitution — bare REM/ARB/etc. in pattern context emits `SM_PAT_<KIND>` directly).  Other open rungs: EC-UNI-REFAITH, EC-UNI-REWIRE-ALL (Path a/b Lon decision pending), EC-UNI-NAMEKEY-BIN, EC-UNI-21-followup, EC-UNI-18/19/20/22.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 + EC-UNI-21 9/9 stable. |
