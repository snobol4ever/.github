...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`302a1207`**. INLINE-6 complete: deleted `emit_sm_op/nullary/arith/int_arg/acomp/lcomp/pat_*` cluster, all orphaned dispatch decls, `emit_sm_op_*` family (no defs), stub files `sm_pat_control.c`/`sm_pat_position.c`. emit_sm.c: 1766→1414 (−352). GATE-PK 408/0/651. **NEXT: EC-UNI-22** (close docs) or Lon directive on emit_bb.c residue / pivot goal. |
