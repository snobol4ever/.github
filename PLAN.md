...
| **PARSER-PROLOG (pattern frontend)** | `GOAL-PARSER-PROLOG.md` | corpus+one4all | **GOAL PIVOTED 2026-05-07** -- 100% parse coverage on 677 .pl files (SWI+GNU+corpus). PR-17 PARTIAL: recovery+\+prefix+div/rdiv landed corpus@2a9d6a0. Next: single-char SY atoms in op/3, *-> soft cut, coverage measurement. ⛔ NO baseline gates at session start — .sc-only work, gates are too slow (~4s/file). |
| **PARSER-RAKU (pattern frontend)** | `GOAL-PARSER-RAKU.md` | corpus+one4all | **PIVOT 2026-05-07** old RK-0..RK-27 CLOSED at PASS=147 FAIL=0. New ladder grounded in OFFICIAL `Grammar.nqp` (759 productions). **RK-28-A LANDED** (coverage gate `test_parser_raku_coverage.sh`, one4all@7adcbdbd). **RK-28 LANDED** all 19 `statement_control:sym<...>` arms (corpus@6f1c61f), COV_PASS=13 FAIL=0. Next: **RK-29** (`statement_prefix:sym<...>` — 16+ phasers). |
