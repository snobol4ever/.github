...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`2e3b22b5`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN, Prolog BB honest 124/0/0. **RUNG THREE-SECTION-X86 (TSX)** — DELETE every binary-byte-emitting function; raw bytes go INLINE in each template's MEDIUM_BINARY arm. **TSX-DELETE ladder ✅** DEL-0/1/2, W-LIT, W-PORT, W-PL. **TSX-WIRE 🔄 IN PROGRESS — WIRE-1/2/3 done (`2e3b22b5`).** ⚡ SCOPE CORRECTION: the emit_bb.c driver was ALREADY template-routed (flat_fill_bin→emit_bb_node→templates); the only bypass was a DEAD ASSIGN_IMM/COND immediate branch (proven 0 hits) — deleted (WIRE-1), removing all 8 driver emit_seq_port_call sites + orphaned plain emit_seq_port_call. WIRE-2 deleted _rip's unreachable MEDIUM_BINARY arm (all callers are mode-4 MEDIUM_TEXT) + cascade-deleted 5 insn_*. WIRE-3 swept 2 zero-ref insn_*. **insn_* 23→16.** **NEXT: DEL-FINAL** — the 16 survivors fall after converting their callers: _rip text-arm insn_* → s_*asm string builders; bb_lit memcmp-PLT/rip-rel-lea/rel32 (ER-8 displacement question); emit_prologue/sigma insn_* bodies. mode-4 forced-TEXT stays. Oracle held: 3-mode parity 184, --run 186/75. ⛔ PENDING Lon: TEMPLATE PURE-PROJECTION. STRING-CONCAT-ALL + ER-8 deferred. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
