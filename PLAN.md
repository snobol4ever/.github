...
| SNO treebank-list | `GOAL-SNO-TREEBANK-LIST.md` | one4all | TL-2 IN PROGRESS: flush-time arg-name resolution for *fn(var) callcaps landed (pat_assign_callcap_named + NAM_push_callcap_named paths; SM path + IR-direct path + binary-box path all consistent); 10-line reproducer now matches oracle byte-for-byte; smoke=7, broker=49, broad corpus 172/228 unchanged (zero regression); full treebank-list still crashes with glibc heap corruption — separate downstream bug requiring AddressSanitizer/bisection | ☐ |
