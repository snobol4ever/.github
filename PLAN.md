...
| Goal | File | Repo | Step |
|------|------|------|------|
| **⛔ CASE-SENSITIVE-ONLY (command directive)** | `GOAL-CASE-SENSITIVE-ONLY.md` | one4all+.github | **CSO-1** — snapshot baseline. Lon command 2026-05-17 (PIVOT): SCRIP supports zero case-folding across all six languages (SNOBOL4, Snocone, Rebus, Icon, Prolog, Raku). Historical SNOBOL4 case-fold support to be replaced by a standalone preprocessor tool. Directive landed as ABSOLUTE RULE in `RULES.md` (a1558e7b+); code sweep pending. Default behavior is already case-sensitive since SN-31, so the sweep is structural — most fold calls are no-ops. Touches `snobol4.l`, `snobol4.c`, `eval_code.c`, `rt.c`, `interp_data.c`, `interp_hooks.c`, `scrip.c`. Must protect beauty.sno self-host byte-identity (Milestone 1). |
...
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | PST rewrite 5a–5h ✅. Subsystem suite **19/20** (Qize SKIP under SL-3 — acceptance met). **5i partial 🔄 (2026-05-17)**: SL-5 root cause fixed — `_builtin_DATA` (interp_data.c) and `_DATA_` (snobol4.c) split-registered DATA, only sc_dat_types[] populated, `_func_buckets[]` missed field accessors → `value()` unresolved from `IncCounter()` in parser_*.sc. Fix: `_builtin_DATA` chains through `sno_DATA_register`. Also: `--dump-ast` extended to all non-SNOBOL4 frontends (was lang_snocone only). BB caps: `FLAT_BUF_MAX` 16K→256K, `BB_POOL_SIZE` 4MB→64MB. `[apply-err5]` diagnostic via `SCRIP_DEBUG_APPLY=1`. Parsers now load past Error 5 but segfault in `rt_bb_arbno` (corrupted ζ) during `Src ? Compiland` — separate Byrd-box runtime issue. Per HQ RULES "casing at ingress layer", the `sno_fold_name(spec)` at `_builtin_DATA` ingress was removed. Next: triage rt_bb_arbno ζ corruption to finish 5i, then 5j (6-fn trace), 5k (Snocone INCLUDE). SL-2/5-cause closed; SL-3 open, SL-4 likely subsumed by 5h. |
