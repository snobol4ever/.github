...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT: ME-14** — Full corpus pass (`--jit-run` ≥ `--sm-run` PASS counts for all six frontends; completion criterion that reopens `GOAL-MODE4-EMIT` `EM-MODE4-IS-MODE3-DUMP`). Closed: ME-1..7 + ME-9 (all sub-rungs) + ME-10 + ME-11 + ME-12 + ME-13. Sess 2026-05-11b (Opus 4.7): ME-14 reconnaissance + `sm_jit_unwind_call_stack` helper landed (one4all `40ecae93`) as side improvement — handles longjmp recovery in JIT mode (real future hazard, harmless when not triggered). 3 pre-existing JIT-only beauty driver failures (`Qize_driver`, `XDump_driver`, `omega_driver`) **persist** — diagnosed as `STATE->sp=-3` corruption during Qize pattern build (3 extra r12 pops between SM_STNO and SM_EXEC_STMT for `*assign()` deletion at PC~1951). Hypothesis: stray SM_RETURN blob inside SM_PUSH_EXPRESSION body; not yet bisected. Gates: smoke 7/7, broker 49/49, me6 3/3, icon/prolog/raku 5/5/5, JIT crosscheck three-mode parity 133/197/197 unchanged. |
...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: scan context (+5 rung05), then 7 remaining honest fails in 01-35.** Sess 2026-05-11 (Claude Opus 4.7): CH-17g-initial-once ✅ (+3, one4all `b4d7ee18` — `expression_scope_walk` skips AST_INITIAL subtrees; proc-body setup densifies expression_sc to remove init-tracked names; sentinel-guarded inline lowering using per-AST `__initial_<ptr>__` NV var). Honest 180/12/6. Gates: smoke 5/7/5/5/5/4, broker 49/49, ir-run 185/48/30. Prior: AST_IF cond leak ✅, CH-17g-builtin-batch ✅ (+26), case/swap/null ✅ (+7), smcall-proc ✅, augop-inline ✅, loop-stack ✅, scan ✅, A0-A4 ✅. |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **PAUSED** — reopens when ME-14 closes. Mode-4 = mode-3 + SEG_CODE serializer. Tripwire: test_gate_em_beauty_subsystems_mode4 PASS=4 FAIL=13 frozen. |
