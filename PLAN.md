...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-pure-invariant LANDED sess #77, 2026-05-07** — fully-invariant patterns wired through .text-baked blobs via bb_build_flat_text + scrip_rt_match_blob; pattern_window_t pre-pass detects Phase-2 windows, emits one block per pattern with externally-visible _pat_inv_<id>_alpha/_beta/_gamma/_omega symbols; SM_EXEC_STMT for invariant patterns calls match_blob ABI; .s assembles + links cleanly; PASS=12. Honest deviation: linked binary segfaults — bb_flat.c bakes Σ/Δ/Σlen/lit/memcmp as process-address imm64s. **Next: EM-7c-symbolic** (replace with `lea reg, [rip + symbol]` + `call name@PLT` in TEXT mode). Then EM-7c-variant, then EM-7d (beauty oracle). |
