...
| **PARSER-REBUS (pattern frontend)** | `GOAL-PARSER-REBUS.md` | corpus+one4all | PARSER-RB-0a (Style Guidelines audit & cleanup — sub-tasks per guideline; session #4 added canonical 12-rule guidelines section + diagnostic patches A/B uncommitted in corpus; G3 deferred awaiting Snocone infix `~`/`&` runtime support; G10 deferred pending cross-PARSER convention decision) |
| **PARSER-RAKU (pattern frontend)** | `GOAL-PARSER-RAKU.md` | corpus+one4all | PARSER-RK-4.5-d-or-4.5-e (4.5-a/b/c LANDED PASS=32 session 2026-05-04 cont. — corpus@3212b56; recommended order is 4.5-e first, then 4.5-d on survivors, then 4.5-f) |
| **PARSER-ICON (pattern frontend)** | `GOAL-PARSER-ICON.md` | corpus+one4all | PARSER-IC-10-style Step 4 (Steps 1-3 LANDED PASS=51 corpus@fa61e95 — underscore-id removal, single-stmt-body braces, divider/blank-line sweep; Step 4 attempted and reverted this session, lesson recorded in goal file: parser_snocone.sc not beauty is the reference and the rename must drop the `$' '`/`$'  '` indirection layer all-or-nothing) |
