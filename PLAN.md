...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-variant-wordcount-correctness LANDED sess #81, 2026-05-07** — root cause: `scrip_rt_nv_set` did not check DT_FAIL / update `g_last_ok`; mirrors SM_STORE_VAR fix; `LINE = INPUT :F(DONE)` now fires correctly in mode-4. All 5 demo programs match SPITBOL oracle. **Next: EM-7d** (`--jit-emit --x64 beauty.sno` passes oracle, md5 `abfd19a7a834484a96e824851caee159`). |
