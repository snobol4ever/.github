...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: sub-rung -q (SM_RETURN family).**  -p ✅ sess 2026-05-11 (Claude Sonnet 4.6, one4all `3911a0df`): SM_CALL_FN template + t_lea_rdi_strtab_sym/t_mov_esi_imm32; MACRO_DEF source of truth; TEXT uses sm_emit_lbl_int32 (proven path).  -e ✅ sess 2026-05-11 (Claude Sonnet 4.6, one4all `157352d8`): charset-family template (XSPNC/XBRKC/XANYC/XNNYC) via `templates/bb_xspnc.c` + callback design. Byte-identical.  -c ✅ closed (Option C: SM_HALT is sanctioned terminative exception).  Gates: smoke 7/7+49/49+5/5; template-byte-id 4/4; claws5.s diff empty. |
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: rung13 conjunction-in-generator (Phase A4 AST_ALTERNATE + every body).** Sess 2026-05-11e (Claude Sonnet 4.6): g_lang scoped ✅ `3648dae5` (+4, ~231 honest). |
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **SL-2 next: emit_goto in Snocone. SL-1 ✅ labtab_* + smoke test (corpus sm_lower.sc).** |
