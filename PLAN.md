...
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SI-2** — shim helpers `stmt_to_ast`/`code_to_ast` in `src/driver/stmt_ast.c`. SR-1..SR-15c done, SI-1 ✅ one4all `fedf1e87` (ast.h gains AST_PROGRAM/STMT/GOTO_S/F/U). | — silent default fallback → lower_unhandled + explicit per-kind table. Closed: SR-1..SR-13 ✅. **rename ✅ `cc21aa5a`** — sm_lower→lower, cohort_*→lower_*. **SR-9 ✅ `907644e5`** — icn_relop+cset+unary (24 kinds). **SR-10 ✅ `a4545102`** — icn_ctrl+data+sect (19). **SR-11 ✅ `69e7dde8`** — lower_icn_gen (7) + lower_prolog (6); legacy switch now empty. Gate: PASS=30. |
