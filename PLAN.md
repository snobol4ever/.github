...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI LIFT Snocone-shape sweep in progress (2026-05-20)** — slices 1+2+3 done (one4all `1a9571fe` → `90235416` → `87d11afc`).  Snocone discipline: vars in template fns and their helpers are scalars/strings/collections of values in g_emit — NEVER pointer-to-struct.  C strings are the admitted exception.  g_emit.lbl_* are name strings, not bb_label_t pointers.  New name-taking primitives `emit_text_jmp` / `emit_text_label` in emit_core.c (single home for format-mode-aware label/jump emission).  8 of 14 emit_bb_x* fns lifted: xstar, xlnth, xposi+xrpsi, xfail, xfarb, xtb+xrtb, xchr.  Templates updated: bb_rem, bb_len, bb_pos, bb_fail, bb_arb, bb_tab, bb_lit.  Matrix gate 830/830, beauty byte-identical, smokes green throughout.  **NEXT:** continue with xcallcap, xfnce, xfnme, xnme, xdsar, xatp, emit_bb_charset; then dispatcher needs wiring to fill g_emit.lbl_* (currently shape-only); then SM-x86 fns from emit_sm.c into sm_*.c templates.  See GOAL-HEADQUARTERS.md watermark for full trail. |
