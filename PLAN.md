...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | **WAM-CP-6 B2 INDEXED-LCO ✅** (Opus 4.8, 2026-05-29, one4all `167f31cb`). Tail-call frame-reuse extended to indexable multi-clause callees — pairs WAM-CP-8 first-arg indexing with the WAM-CP-6 B1 redirect sentinel. `src/lower/bb_exec.c` ONLY, +95/−22, additive, NO emitter/template/FACT change. **Benchmark UNLOCKED:** `count(0). count(N):-N>0,N1 is N-1,count(N1).` to 1e6 runs in O(1) C stack — verified `count(1000000)` prints done at a **1MB** stack (O(N) recursion dies ~6k frames), `count(10000000)` at 8MB, `sumto(500000,0,R)=125000250000` at 1MB (accumulator binding propagation through flattened frames correct). **Two changes:** (1) BB_CHOICE index path runs the unique matching clause WITHOUT pl_cp_push when its body is cp-free-except-tail (gate extended from bb_body_single_solution), leaving g_pl_bfr unchanged so the body's tail call sees a clean CP spine — that live clause CP was exactly what defeated the LCO gate; (2) BB_PL_CALL B1 gate gains a B2 arm via new `pl_choice_unique_indexed_body()` that redirects into the index-resolved clause BODY of a BB_CHOICE callee (not the choice node, which would re-nest a frame per iteration). Safety: `count(0)` base cases take the unchanged CP-pushing scan (ncand==2 via wildcard clause) so the cut governs them; B2 fires only when exactly one clause matches AND its body is cp-free-except-tail. Gates byte-identical: GATE-1 5/5, GATE-2 132/0 (5 ORACLE_MISS), GATE-3 m2 104/107, GATE-4 4/4, GATE-SWI 57/57 (held — the WAM-CP-8 memberchk regression sentinel stays green), mode-4 corpus 54/107, FACT 0, sibling icon/raku 5/5/5, snobol4 13/13. Handoff `HANDOFF-2026-05-29-OPUS48-PROLOG-BB-WAM-CP-6-B2-INDEXED-LCO.md`. **NEXT — Phase B3: trail reclamation.** Heap is now the ceiling, not stack: `sumto(1e7)` OOMs because each iteration GC-allocates Term cells reachable through the trail until the top call returns (O(N) trail/live-term growth despite flat C stack). Truncate the trail to the pre-call mark on deterministic redirect after copying live bindings forward (the deferred `copyFrameArguments` analogue). Alternatives: WAM-CP-13 mode-4 corpus (54→~60, mechanical); WAM-CP-7 unify specialization. |
