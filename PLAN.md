...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **INLINE-4a-inline in progress.** Slices 19/20/21 landed: sm_pat_lit, sm_pat_refname, sm_pat_usercall, sm_pat_capture, sm_pat_capture_fn, sm_pat_capture_fn_args, sm_pat_usercall_args, sm_exec_stmt all inlined to bare `emit_textf("MNEMONIC args # anno\n", ...)` form. one4all `2e33f0fd`. GATE-PK PASS=420 FAIL=0 STUB=639 throughout. **NEXT: sm_returns family** (sm_return / sm_freturn / sm_nreturn — 9 opcodes total; sm_return handles g_in_define_body insn_pop_rbp before emitting "RETURN\n"; FRETURN/NRETURN variants emit bare "RETURN_VARIANT kind, cond, pc # opname\n"). After sm_returns: push_var/store_var/push_lit_s (LBL family) — need to promote or inline `render_str_preview` for the s-preview anno. See HQ INLINE-4 Targets section for detailed inline plans per template. ⚠ remaining ~14 templates still go through dispatchers in emit_sm.c; INLINE-4b sweep (delete dispatchers + renderer + sm_op_template_t table) blocked until all templates inlined. |
