...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.e (session 2026-04-30 #8: **LS-4.d LANDED.** Postfix subscript `a[i, j]` → `E_IDX(a, i, j)`. New `expr15` tier inserted between `expr11` (exponent) and `expr17` (atoms), mirroring `snobol4.y:183` exactly: `expr15 : expr15 T_LBRACK exprlist T_RBRACK | expr17` — left-recursive on the same tier so `a[i][j]` chains naturally to `E_IDX(E_IDX(a, i), j)` while `a[i, j]` produces a single n-ary `E_IDX(a, i, j)`. Reused existing `exprlist` non-terminal from LS-4.b. Empty subscript `a[]` permitted via empty-list arm yielding `E_IDX(a)`. **`expr11` redirect:** exponentiation now delegates to `expr15` instead of directly to `expr17`, so subscript binds tighter than `^`; other tiers above inherit the new tightness automatically. **Lexer unchanged:** `[` was already emitted unconditionally as `T_LBRACK` — `a[i]` and `a [i]` lex identically with no CONCAT injection. **Compound-assign LHS:** `a[i] += 1` lowers correctly to `E_ASSIGN(a[i], E_ADD(clone(a[i]), 1))` because `sc_clone_expr_simple` already handles E_IDX recursively (LS-4.c comment anticipated this). **Token block reorganization:** `T_LBRACK`/`T_RBRACK` promoted out of catch-all into labeled "LS-4.d" block. **Testing:** `test_snocone_parse_d.c` 15 functions / 79 assertions — headline gate `a[i,j]`, single index, empty subscript, two-deep & three-deep chaining, string keys, expression children `a[1+2, 3*4]`, call-then-subscript `f(x)[i]`, paren-then-subscript, compound-assign with subscript LHS (with deep clone-distinctness + no-shared-nodes verification), subscript-in-concat, subscript-in-arith, plain-assign with subscript LHS, nested `a[b[c]]`, subscript-below-exponent. **Bison:** zero shift/reduce conflicts. All gates green: parse-a 35/35, parse-b 119/119, parse-c 66/66, **parse-d 79/79**, smoke snocone 5/5, beauty 42/0/3, broker 49/0. **Combined parse gates 299/299** (was 220/220). Side-channel — production driver still uses legacy `archive/snocone_parse.c`; LS-4.j wires the new parser. **Forward note re: LS-4.f:** when `if`/`else` lands, use balanced `matched_stmt`/`unmatched_stmt` clause-phrase split (Pascal/Algol style, zero conflicts) rather than C's default-shift acceptance — goal file's hierarchical meticulousness points to the balanced grammar being the right fit. **Next: LS-4.e** — unaries `T_1PLUS T_1MINUS T_1STAR T_1DOLLAR T_1DOT T_1AT T_1TILDE T_1QUEST T_1AMP` (and OPSYN-slot `T_1PERCENT T_1SLASH T_1POUND T_1PIPE T_1EQUAL`); parses `*expr`, `.var`, `$x`.) |
