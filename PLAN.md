...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-13 step 3 BLOCKED — pre-existing SCRIP bug: Snocone pattern captures (`. var`) not updating variables in --ir-run; SM_PAT_CAPTURE (sm_interp.c:682) does not commit captures via NV_SET_fn. Fix SM_PAT_CAPTURE, then re-run run_scrip_parser.sh. SL-13a ✅ sess 2026-05-12 (Claude Sonnet 4.6): fixed `sc_make_cond_fail_stmt` TT_ASSIGN unwrap in snocone_parse.y+tab.c; qize.sc re-enabled. Gate: no hang, no Error 5, smoke 6/6, sm_lower_test 11/11.** |
