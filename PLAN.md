...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-5b (session #68: three gating bugs fixed — (1) snocone_lower.c unary `*` was lowering to E_INDIRECT instead of E_DEFER, breaking every `*Pat`; (2) scrip.c multi-file merge did not strip intermediate is_end sentinels, halting at first lib's END; (3) semantic.sc shift()/reduce() built malformed EVAL strings. All `&`/`~` sites in beauty.sc rewritten as explicit reduce()/shift() calls. ALL gotos and labels eliminated in beauty.sc per Lon — ss_unop restructured with `~EQ(n,1)` guard, refs proc rewritten with nested ifs, main loop rewritten as structured done/more/eof_inside flag machine. Mechanical rename `snocone_cf` → `snocone_control` landed (files + function + all references). beauty.sc now executes through pattern construction; three EVAL parse errors during pattern build remain unrooted. Snocone INPUT-on-EOF runtime bug discovered (returns previous value rather than failing) — separate rung needed. Gates remain PASS=5/PASS=42 SKIP=3/PASS=49 throughout.) |
