...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-16 CLOSED 2026-05-20 (superseded)** — REDUCE-via-helpers direction reverted in one4all `8b2f65e1` per Lon directive: "Leave the SM and BB templates without any helpers for now ... The goal now is to get the raw output into each template.  We will collapse once we know what all we have to work with from the entire body of source."  Slice 1 (`sm_call_or_suspend`) and slice 2 (`jvm_alpha/beta_method_hdr`) both reverted.  "One screen per fn" guideline also removed from RULES.md; audit script `test_gate_ec_uni_15_audit.{sh,py}` deleted.  **NEXT: EC-UNI-18** (table-driven dispatch where it earns its keep) OR **EC-UNI-21-followup** (M1 reconcile/retire) OR **EC-UNI-22** (close ARCH-IR/ARCH-SCRIP docs).  Matrix gate 450/450 still PASS.  EC-UNI-21 close gate 9/9 still PASS.  See `GOAL-HEADQUARTERS.md` watermark. |
