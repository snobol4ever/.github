...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **NEXT: SI-6 SM_EXP (exponentiation).** Phase-2 plan landed sess 2026-05-12 (Claude Sonnet 4.6) — 10 rungs SI-6..SI-15 derived from SPITBOL manual v3.7 (chapters 3/4/6/7/15/17/18/19); 41-opcode gap between `lower.sc` emits and `sm_interp.sc` handles, partitioned by SPITBOL semantic groups (arithmetic / patterns / comparisons / calls / returns / computed-goto / real-program cross-check). Icon/Prolog/generator opcodes deferred to Phase 3. |
