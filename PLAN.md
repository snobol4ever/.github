...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-19-remaining TT_SUSPEND via SM_SUSPEND + sm_yield_to_caller fix (no AST at runtime)** — TT_SUSPEND (blocked CH-17g); rung36 suite. ir-run 202, honest 273→275 (+2). Fix: icn_bb_scan_gen integer subject crash (DESCR_t union .s/.i aliasing — coerce DT_I/DT_R via descr_to_str_icn before pointer use). one4all e2d80506. |
...
| **⚡ Mode-4 Full Suite: SNOBOL4+Snocone** | `GOAL-MODE4-SN4-SNOCONE.md` | one4all+corpus+.github | **NEXT: M4SN-4b continued** — 229/280 target ≥250; corpus uppercase fix ✅ `c4a0cef`; --fold-case ✅ `8c06b7ab`. |
