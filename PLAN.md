...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Cat-3 fully landed. Fix #2 ✅ IJ-EVERY-SINGLESHOT (is_generator bit + recursive ir_is_single_shot classifier in IR_EVERY); `every write(fact(5))` now yields 120 once. ir-run 105→107. NEXT: augop-in-every-body accumulation bug (rung02_proc_locals 5≠15) or suspend-with-do-body (rung03_suspend_gen* 3 rungs empty). |
...
| **⚡ PST: Rebus + Prolog** | `GOAL-PST-REBUS-PROLOG.md` | one4all+corpus+.github | **PST-PL-6a** — verify Prolog kind-mapping; read prolog_parse.c + parser_prolog.sc. PST-RB-5a ✅ 5b ✅ 5c ✅ 5d ✅ 2026-05-16. |
