...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-three-column-non-bb LANDED 2026-05-09** — true three-column shape (label/opcode/args+comment, widths 24/16/N) extended to every non-BB line in emitted `.s` AND inside `sm_macros.s`; redundant opcode-name annotations stripped; C enums `SM_POP`→`SM_VOID_POP` and `SM_CALL`→`SM_CALL_FN` for symmetry with macro names.  Goal file condensed 4830→384 lines.  Gates 10/10 GREEN.  Next: **EM-7c-bb-three-column** — BB boxes as 4-port α/β/γ/ω clusters in `LABEL: ; ACTION ; GOTO` form. |
