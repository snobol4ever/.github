...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.i IN-PROGRESS (session 2026-04-30 #13: LS-4.h LANDED in working tree — `function`/`return`/`freturn`/`nreturn` parse and lower to SPITBOL idiom (DEFINE+goto+label+body+end_label), with `return E;` inside a function lowering to `fn=E :(RETURN)` and bare `freturn;`/`nreturn;` to `:(FRETURN)`/`:(NRETURN)`. New gate `test_smoke_snocone_parse_h.sh` 85/85 PASS across 15 tests; combined parse gates 668/668 (was 583/583). Bison zero conflicts. Token rename T_KW_* → T_* applied across snocone_lex.{h,c}, snocone_parse.y, test_snocone_lex.c — 17 tokens; `function` keyword token kept as `T_FUNCTION_KW` to avoid colliding with `T_FUNCTION` (the IDENT-LPAREN call-form token). Smoke test `scripts/test_smoke_snocone.sh` updated `procedure` → `function`; legacy `archive/snocone_lex.c` got `"function"` as synonym for `SNOCONE_KW_PROCEDURE` so production path (still legacy parser until LS-4.j) accepts it. All production gates green: smoke 5/5, beauty 42/0/3, broker 49/0; FSM lex 31/31. Side-channel only — LS-4.j wires the new parser into the production driver. **Next: LS-4.i** — `switch`/`case`/`default`, `break`/`continue`, `goto`, `struct`, alt-eval `(a, b, c)` → `E_VLIST`. beauty.sc-class programs reachable after LS-4.i lands.) |
