...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **IR-CD-1/2/3 ✅ COMPLETE 2026-05-20** (one4all `419fce29`). **IR-CD-RENAME ✅ COMPLETE 2026-05-20** (one4all `a97be4b0`). **IR-CD-4 ✅ COMPLETE 2026-05-20** (one4all `b97b267b`): 19 consumer read sites migrated across 5 files via strangler helpers; mode-4 carve-out still in fallback path. **NEXT: IR-CD-5** — delete `ir_body` field from `IcnProcEntry`/`Pl_PredEntry_BB`, delete fallback branch in strangler helpers, delete `ir_body = ...` assignments in `lower.c` and `pl_runtime.c`. Mode-4 standalone policy TBD (register stub `SM_sequence_t` at standalone startup, or accept carve-out permanently). Then IR-CD-6/7 (doc, close). IR-RN-2/3 emitter+runtime audits still owed before EC-UNI-3-beauty.
...
| **⚡ PST: Snocone** | `GOAL-PST-SNOCONE.md` | one4all+corpus+.github | Phase 1 C ✅. Phase 2 SC-SC ✅. **MIRROR-GAP-SC-SC-5 root cause found 2026-05-20**: 3 C bugs fixed (parse heap-corrupt + IR_lower_pat TT_FNC null + lower_scan DCG). Remaining: XDSAR in bb_build_brokered. one4all @ 3824560c, corpus @ b10933c. |
