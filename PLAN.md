...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all | EM-FORMAT-BB-LAW ✅ 2026-05-09 — BB-side now uses 4 margin tabs (LABEL/MNEMONIC/ARGS/GOTO at file cols 0/24/41/68); SM-side keeps its 3-tab layout. Bare label+jmp lines have `jmp` at vcol 68 (Tab 4); fused cond+uncond is `<cond_mn>` in col-2 with `<succ>;` padded to width 27 in col-3 and `jmp <fail>` in col-4 at vcol 68. Next: **EM-FORMAT-BB-LAW-TRIPLE-FUSION** (fuse preceding `test`/`cmp` action onto the cond-jmp's line) or **EM-FORMAT-BB-DATA-CONSOLIDATE** (consolidate scattered `.section .data` blocks), then beauty.sno crosscheck **EM-7d**. (TRAMPOLINE-ELIM is a future optimizer pass, not part of the EM-FORMAT-BB sequence.) |
