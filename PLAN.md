...
| **Icon BB JCON triage (39 FAILs + rung37 tests)** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: debug IJ-18..28** (IJ-18..28 ✅ `1fc11764`: all missing JCON BBs written — not/repalt/while/until/repeat/case/compound/field-gen; honest 276. IJ-15 ✅ `52db8b96`: cset literal/builtin/ops + coro_stmt default fallback; ir-run 201, honest 275. IJ-14 ✅ `00cec4c3`: fix coro_bb_iterate static-alias bug every put(L,!s); IJ-13c ✅ DT_FH. ir-run 200, honest 274.) |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: SF-2 (ARB flat template)** — SF-1 ✅ `3fcc90a7` emit_bb_xbal inline loop. Note: BAL frontend keyword wiring missing (PUSH_VAR not XBAL). beauty 10/17. |
...
| **Style: 200-col emitter reformat** | `GOAL-STYLE-200COL.md` | one4all+.github | **NEXT: S200-3** — `emit_core.c`. S200-2 ✅ `fe47f032`. Rules in RULES.md § "C code style". New rules: zero blank lines in C/H files; banner-only comments (no inline, no body comments). |
