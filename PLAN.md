...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: expand lower_icn_expr_node** (TT_BINOP → TT_IF → user-proc calls). Sess 2026-05-15 (Claude Opus 4.7, one4all `7fd70c00`): first AST→IR→BB wedge — proc bodies now lower to IR_block_t* on proc_table[pi].ir_body; icn_bb_pump_proc_by_name routes via icn_bb_dcg / bb_broker, bypassing SM_BB_EVAL entirely. New IR executor cases: IR_VAR, IR_ASSIGN, IR_CALL, IR_SEQ. lower_icn_proc_body covers TT_ILIT, TT_QLIT, TT_VAR, TT_ASSIGN(TT_VAR := expr), TT_FNC(builtin) — falls back to legacy SM path on unsupported kinds. Gates: smoke_icon 0/5→1/5 (write_str PASS); others unchanged. Two earlier wedge attempts this session put IR behind an SM lookup; reverted after Lon flagged the layering violation. Lesson recorded: SM_BB_EVAL stub message "needs fresh SM/BB lowering" misleads — the fix is to remove SM_BB_EVAL from the path, not to put IR behind it. Sess 2026-05-15g (Claude Opus 4.7, one4all `fb3c4153`): Lon directive — delete ALL AST walking from modes 2, 3, 4. 14 AST-walk sites stubbed across sm_interp.c (8) and sm_jit_interp.c (6); each prints `[NO-AST] <opcode> stub: needs fresh SM/BB lowering`. |
