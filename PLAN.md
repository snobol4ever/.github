...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-SN-7-1 (PASS=27 FAIL=32; pp_stmt reads 7 slots verbatim; fixes needed: goto sf global, ExprList args, arith left-assoc) — labels + assignment + tree-shape rewrite (next: FW-2 role-slot wrappers + IR-tag rewrites to match `--dump-parse` oracle).  PARSER-SN-7-0a LANDED session 2026-05-04 (style audit, 4/4 checkboxes; PASS=0 FAIL=59 unchanged).  Style Guidelines canonical in this goal file (binding on all six PARSER-*).  INFRA-11a + INFRA-11c FIXED (one4all@d2547945, corpus@c8ee2a6). |
