...
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** — 10 §⛔ violations remaining (R2/R10/R11/R14/R18 closed this session). PRF-12-smatch/new/mcall/hof/capture/twigil/gather-splice/gather-hoist pending. one4all `95a24fdf`, corpus `7da85d5`. NEXT: PRF-12-smatch (R20) — `~~ /regex/` → TT_SMATCH kind. ⚠ Always regen `raku.tab.c` with `bison -d raku.y -o raku.tab.c` after any `raku.y` change. |
