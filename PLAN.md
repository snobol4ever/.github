...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-REWIRE — blocked on Lon design choice + coverage gap (2026-05-21 evening)** — BB-side LIFT done through slice 7 (one4all `045baf4a`).  SM-side audit reclassifies all 36 fns as already-lifted-as-helper.  Methodical trace of `BB_PAT_REM` rewire found THREE design forks: (1) text-vs-binary in JIT — `g_emit.lbl_*` are name strings by Snocone-bootstrap design, binary mode needs name-keyed primitives not yet built; (2) **format drift** — templates use `emit_text_jmp`/`emit_text_label` while originals used `bb3c_format(out, "", "jmp", target)` directly, producing DIFFERENT text column layouts; (3) **coverage gap** — beauty.sno's --compile output contains only ONE `# BOX` banner (POS(0)) in 882901 bytes; the other 16 BB pat-level kinds are NOT exercised because most patterns lower to SM macros, not BB graphs.  The EC-UNI-21 gate has been structurally insensitive to 16 of 17 BB-side LIFT slices.  **Three rewire sub-paths:** (a.1) re-lift templates verbatim, (a.2) accept format drift + re-baseline gate, (b) name-keyed binary primitives first.  **Plus a prerequisite rung `EC-UNI-COVER-PAT`:** author a corpus program that forces invariant BB-lowering of REM/LEN/ARB/CAT/ARBNO/etc. before the rewire, so byte-identity claims mean something.  See GOAL-HEADQUARTERS.md EC-UNI-REWIRE section for full trace.  No code committed this session — HQ docs only.  Watermark `045baf4a` unchanged. |
