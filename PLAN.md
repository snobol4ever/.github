...
| Goal | File | Repo | Step |
|------|------|------|------|
| **CHUNKS — Eliminate SM_PUSH_EXPR** | `GOAL-CHUNKS.md` | one4all+.github | M1 CLOSED. **CH-14 LANDED sess #70** (generator infrastructure — SM_SUSPEND/SM_RESUME opcodes; SmGenState; bb_broker_drive_sm; hand-written 3-yield test PASS; named-FATAL JIT stubs). **Next inline: CH-15** (Icon generators per-kind — E_TO, E_TO_BY, E_EVERY, etc.; bb_broker_drive_sm as consumer). Step 8 (mode-4 x86 emitter) carved to `GOAL-MODE4-EMIT.md` — runs in PARALLEL with M4, file-disjoint. Don't confuse them: a session "doing GOAL-CHUNKS" defaults to its inline next rung (CH-15), NOT the carved sub-goal. |
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7-revert LANDED sess #72, 2026-05-07** — brokered Phase-3 torn out of the emitted-code path: 26 `scrip_rt_pat_*` functions + `scrip_rt_exec_stmt` deleted from libscrip_rt.so; `emit_bb_box` + EM-6 PLT-call helpers + `_ARGS` emitters deleted from emitter; `g_pat_stack[]` runtime descriptor-tree state removed; EM gate goes PASS=10 → PASS=9 (test 10 retired). EM-7-pre SM-side keepers retained (SM_CALL/CONCAT/PUSH_NULL/COERCE_NUM/return variants/STRTAB+VSTACK bumps). **Next inline: EM-7-default-bb-live** (flip BB-mode default `--bb-driver → --bb-live`), then **EM-7-default-jit-run** (flip exec default `--sm-run → --jit-run`) — daily testing should hit mode-3 + bb-live, which IS mode-4's existence proof. Then EM-7a (PATND_t bridge), EM-7b (bb_flat.c EMIT_TEXT parity), EM-7c (wire flat-or-runtime-build into emitter), EM-7d (beauty oracle gate). See GOAL-MODE4-EMIT.md "Design Discoveries" section for the corrected five-phase model + reuse map of proven BB infrastructure. |
...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | PARSER-SC-8 ✅ LANDED (sess 9, 2026-05-07) — break/continue with loop-label stacks. PASS=60 FAIL=0. Next: **SC-9** (struct). |
