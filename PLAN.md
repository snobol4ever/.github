...
| **PARSER-REBUS (pattern frontend)** | `GOAL-PARSER-REBUS.md` | corpus+one4all | PARSER-RB-0 (REWRITE — beauty.sc-style refactor landed corpus 9f6b32f: build-time RB_*() helpers, $'op' wrappers, direct E_VAR/RB_ASSIGN constants. Parser still hangs; suspect doubled-FENCE in mul/add_expr — next session bisects per parser_snocone.sc shape.) |
