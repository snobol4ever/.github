...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-TEMPLATE-PURITY -u (rung close: beauty-subsystems gate; delete legacy emitter files). one4all `ebb338d9`, corpus `dcce732`.** BB template files split one-per-box sess 2026-05-11 (Claude Sonnet 4.6, one4all `5fa770d6`): bb_xfarb→xeps+xfail+xfarb, bb_xlnth→xtb+xrtb+xlnth, bb_xposi→xposi+xrpsi. SM families stay together (Lon decision). Gates: build clean, template-byte-id 4/4. |
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: rung14 limit-in-generator (Phase A3 TT_LIMIT).** Sess 2026-05-11g (Claude Sonnet 4.6): rung13 conjunction-in-generator ✅ `fa8bd48f`. SM_GEN_TICK opcode + bb_broker_drive_sm_one + IcnFrame.every_gen[]. lower_every hoists TT_ALTERNATE as inner SM coroutine; outer loop drives via SM_GEN_TICK. Honest SM 208→211. |
=======
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-7 lower_stmt full port (SL-5+SL-6 ✅ sess 2026-05-12 Claude Opus 4.7).** lower.sc 526 → 1025 L; added control flow (if/while/until/repeat/loop_break/loop_next/return/proc_fail), comparisons (acomp/lcomp), Icon unary ops (nonnull/null/not/size/identical/random), augop fast path, seq_expr, vlist, opsyn, idx, scan, swap, case (Icon pair), makelist/record/field/global/initial, section family, generator stubs (to/to_by/every/limit/suspend/bb_pump_ast); emit_lhs_store augmented (INDIRECT/FNC/FIELD); lower_fnc Icon-callee path; lower_stmt LANG_PL + LANG_ICN-skip + TT_VAR-return special-case; lower_proc_skeletons stub; lower() main calls skeletons + skips LANG_ICN + emits BB_PUMP_PROC tail. Gap vs lower.c (~210L): emit_range_coroutine, emit_thunk, proc_skeletons body, deep lower_pat_expr, scope_walk — all Ph2/Ph3. **PLUS** AST_/TT_ unification across all 9 SCRIP/*.sc: 799 AST_→TT_ renames, 244 TT_-variable decls removed, 135 bare TT_-refs in lower.sc inlined as 'TT_X', parser_snobol4/raku preserved as "'TT_X'". 0 AST_, 0 TT_-decls, 0 bare TT_-refs verified. Error 3 in smoke_lower still open from prior session — not addressed. Build/test gate not run this session (context budget). |
