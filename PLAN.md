...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#1 LANDED session 2026-05-04 cont. #6 — White-encodes-comments cross-pollination; all six parsers at canonical `White`/`Gray`/`$' '`/`$'  '` shape, comments folded into per-language White, SC empty-stmt for C-style `;;`. Gates: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38 (all 100%). Next: operator picks iter#2 concept. |
