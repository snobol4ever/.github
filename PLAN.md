...
| Goal | File | Repo | Step |
|------|------|------|------|
| **CHUNKS — Eliminate SM_PUSH_EXPR** | `GOAL-CHUNKS.md` | one4all+.github | M1 CLOSED. **CH-14b LANDED sess #71** (gen-local slot infrastructure — SM_LOAD_GLOCAL/SM_STORE_GLOCAL opcodes; locals[8] on SmGenState surviving SUSPEND/RESUME; SM_BB_PUMP_SM consumer entry point for migrated generator kinds; CH-14 gate 18/18). **Next inline: CH-15a** (E_TO + E_TO_BY producer migration — first kinds to use the new infrastructure; bb_broker_drive_sm as consumer). Step 8 (mode-4 x86 emitter) carved to `GOAL-MODE4-EMIT.md` — runs in PARALLEL with M4, file-disjoint. Don't confuse them: a session "doing GOAL-CHUNKS" defaults to its inline next rung (CH-15a), NOT the carved sub-goal. |
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7-emit-determinism LANDED sess #74, 2026-05-07** — `sm_lower.c:1287` bare field-assign replaced with `sm_emit_si` (strdups subject name; fixes dangling IR pointer after `code_free`). All 5 demo .sno byte-stable across runs. Gates: smoke ×6 PASS, EM PASS=9, isolation PASS. Next inline: EM-7a (PATND_t bridge from SM Phase-2 + sub-tree partition; extend `flat_is_eligible` to recursive; decide path 1 vs path 2). | — both default flips done; EM-7-default-bb-live-investigate root-caused + fixed in same rung. ABI mismatch between `spec_t` (rax=σ, rdx=δ) and `DESCR_t` (rax=v|slen<<32, rdx=s) at 4 binary-emitter sites in bb_build.c (LIT/EPS/POS/RPOS) + bb_flat.c top-level epilogue meant `IS_FAIL_fn(spec_empty)` returned false → every failed match looked like success-at-position-0, producing "Xabc" for `S 'b' = 'X'` instead of oracle "aXc". Fix: `emit_descr_success_from_stack` + `emit_descr_fail` helpers, replace all 4+1 γ/ω sequences. Gates: smoke ×6 PASS under both bb-modes, EM PASS=9, isolation PASS, pattern test = "aXc". **Next inline: EM-7-emit-determinism** (NEW rung, root-caused this session) — `--jit-emit --x64 *.s` is byte-nondeterministic across runs; sm_lower.c:1287 stores a pointer into the IR (`s->subject->sval`) without strdup, then `code_free(prog)` in scrip_sm.c:60 dangles it. Then EM-7a (PATND_t bridge), EM-7b (bb_flat.c EMIT_TEXT), EM-7c (wire), EM-7d (beauty oracle gate). See "Design Discoveries" + watermark in GOAL-MODE4-EMIT.md. |
...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | 🏁 COMPLETE — SC-10 ✅ PASS=67 FAIL=0 (sess 11, 2026-05-07). All rungs done. |
| **PARSER-REBUS (pattern frontend)** | `GOAL-PARSER-REBUS.md` | corpus+one4all | RB-FW-10 LANDED (2026-05-07, PASS=91 FAIL=0: multi-arg subscript a[i,j]; global var fix; loop idiom fix. corpus@e532680) |
...
| **PARSER-RAKU (pattern frontend)** | `GOAL-PARSER-RAKU.md` | corpus+one4all | RK-24 LANDED PASS=132 FAIL=0 corpus@78bdcb9 — class_and_main fixed (SayFhStmt stray push_var; FENCE guard). Next: RK-25. |
