...
| **Icon BB JCON triage (39 FAILs + rung37 tests)** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-10 cont.** (IJ-10 partial `259e1aec`: SM_PUSH_VAR/SM_STORE_VAR &pos/&subject fixed — reads from scan state, writes via kw_assign normalization. ICN_KW_SWAP for atomic swap. Remaining: coro_value.c TT_SWAP keyword case for swap-inside-scan-body.) |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-REORG-1** — call-graph-verified 10-step plan: emit.c+emit_bb_gen.c mutual dep resolved; 16→15 files by true L0-L5 depth. one4all `1fc4490c`. Gates: smoke 7/7, byte-id 4/4, em8 5/5. |
