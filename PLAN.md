...
| **⭐ DYNAMIC BYRD BOX** | DYN-47 | one4all `c116c2c` · corpus `2f2bbe3` | **DYN-48**: MONITOR diff scrip-interp vs SPITBOL on regressed tests → fix broad 126p/52f → 169p/9f. Then M-PARSE-1: parse.y. |
...
| **SNOBOL4 JS** | SJ-10 | one4all `402750f` · .github `249eb15` | **SJ-11**: fix DATATYPE in sno-interp.js _call() (shadows sno_runtime.js), CONVERT REAL→_real_result, NRETURN throw/catch, OPSYN. |
...
| **⭐ one4all-SNOBOL4-NET** | D-170 | one4all `cb1b00c` | **M-NET-INTERP-A01b complete + pattern/capture fixes — 118/178 broad**. Next: fix goto_failure, builtin IDENT/GT, &ALPHABET/&ANCHOR, arith exponent, ALT commit, ARRAY → ≥ 140/178. See SESSION-snobol4-net.md + MILESTONE-NET-SNOBOL4.md. |
