...
| **CSN FENCE Bug Fix** | `GOAL-CSN-FENCE-FIX.md` | csnobol4 | F-2 Step 3a (session #51: rewind-one-deeper arithmetic implemented and tested — same outcome as #49/#50 (min_crash clean, beauty regresses 35→33+SEGV), reverted not committed. **Pivoted to building 32-test fence_suite/** — tests 100-131 in corpus, Makefile in csnobol4. Baseline: 26 OK / 4 FAIL / 2 CRASH. Maps 4 distinct bug classes: outer-alt fallthrough after FENCE-via-`*var` (114, 124), conditional-assign-`.`-not-committed inside FENCE (127), SEGV on triple-indirection ARBNO+*var+FENCE (119, 129 — beauty class), concat of two *var-FENCE (130). corpus @ b794c7c, csnobol4 @ 48d99a3. Session #52 should pivot off seal arithmetic — 3 sessions of validation suggest fix is elsewhere; investigate bug 2 (test 127 — non-crashing) first to expose the recursive-SCIN-return mechanism likely shared with bugs 1, 3, 4.) |
...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-5b (session #67 closed full SB-4b sweep — all 14 sub-rungs done. case/assign/match/stack/Gen/Qize/ReadWrite/TDump/XDump audited as faithful (no change). counter.sc rewritten 6→16 procs adding BegTag+EndTag per counter.inc. tree.sc rewritten from MakeNode/MakeLeaf stubs to full canonical 9-proc ADT (Tree/Append/Prepend/Insert/Remove/Equal/Equiv/Find/Visit). ShiftReduce.sc rewritten to restore canonical leading-whitespace strip. omega.sc rewritten to fix TV/TW/TX `omega = 'pat'` literal-source-text contract. Three Snocone-runtime discoveries documented: (1) parser do_procedure rejects leading-comma `(, locals)` form — workaround: declare locals as zero-defaulting params; (2) `if (assignment)` doesn't propagate EVAL failure to test arm — proxy via `if (~DIFFER(lhs))`; (3) `expr && @var . *fn(...)` collapses to STRING instead of PATTERN — open follow-on, no current regression. Gates remain PASS=5/PASS=42 SKIP=3/PASS=49 throughout. Active rung: SB-5b — rewriting beauty.sc's binary &/~ sites as explicit reduce/shift function calls. This is the gating issue for end-to-end SB-5.) |
