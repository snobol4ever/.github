...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: sub-rung -g (BB-axis: bb_xlnth+xtb+xrtb integer-cursor family).**  -f ✅ sess 2026-05-11 (Claude Sonnet 4.6, one4all `33b3c7ba`): sm_push_lit_i template; emit_sm_halt/push_lit_i renamed in mode-4 emitter. Byte-identical.  -e ✅ charset-family template. -c ✅ Option C decision. |
...
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SI-4** — SNOBOL4 frontend emits AST_STMT directly. SI-1 ✅ `9d23cf8c`, SI-2+SI-3 ✅ `9e9e1f8f` (pure AST_t {t,v,n,c}; tagged-attr STMT; lower() takes AST_PROGRAM). |
