...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **LOCAL-PURGE whole-template 2026-05-25** — NB-3f/3g ✅ (`7088c76a`, all xa_flat + macro_library bodies pure). NEW Lon ruling: purge ALL locals in ALL arms (not just X86). bb_lit ✅ + bb_pat_any ✅ (`82fc7560`): zero locals across X86/JVM/JS/NET/WASM. Recipe proven: `gas_escape_str` helper + side-effects lifted to wrapper (`g_emit.bb_cs_id`/`bb_cs_zeta`, new runtime `rt_cs_new`) + `emit_for` + inline scalars. GATE-PK 504/0/625, AUDIT GREEN, prolog 124/0/0, smoke 188/190-71, BB_PAT_ANY byte-identical. NEXT: same recipe → bb_pat_notany/span (identical), break/arb (divergent BINARY); then xa_bb_ptr_slot (LP-5). ⛔ Beauty gate SUSPENDED. |
