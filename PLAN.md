...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: port `sm_jump.c` (needs `t_emit_jmp`, `t_test_rax_rax`), then `sm_arith.c`, then BB templates.** Sess 2026-05-11 Opus 4.7 (one4all `39e25002`): three-way template pattern landed per Lon's spec (plain C helpers consult `bb_emit_mode`; no vtable inside templates). `bb_emit_mode_t` now 3-value (TEXT/BINARY/MACRO_DEF). Helpers added: `t_comment`, `t_bb_box_banner`, `t_inc_mem_r13_disp8`, `t_ret`, `t_pad_to_blob_size`, `t_mov_rdi_imm64`, `t_call_sym_plt`, `t_macro_begin/end`. SM templates ported: sm_halt, sm_push_lit_i, sm_void_pop. Gates: smoke 6/7 broker 45/49 (upstream SI-6 regression, not mine); template-byte-id 4/4 (mine, green). |
