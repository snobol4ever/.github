...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI LIFT BB-side COMPLETE; SM-side audit reclassifies — rewire NEXT (2026-05-21)** — BB-side LIFT done through slice 7 (one4all `045baf4a`): all 17 pat-level `emit_bb_x*` fns physically in `BB_templates/`.  SM-side: 2026-05-21 audit of 36 `emit_sm_*_(dispatch|line|template)` fns shows ALL are already-lifted-as-helper — they depend on `emit_sm.c`'s file-private macro-rendering machinery (`sm_op_template_t`, `render_call_line`, `emit_sm_args_t`), which is the correct abstraction boundary the templates call through.  No SM-side body lifting work remains.  Lon directive 2026-05-21 stands down "slap-it-in" recipe in favor of methodical lift respecting abstraction boundaries.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) and matrix 855/855 stable.  **NEXT:** the long-deferred rewire — wire `emit_bb_node` to fill `g_emit` per BB node kind and dispatch to templates; verify byte-identity; delete the originals in `emit_bb.c`.  This is live-path work, full gates apply.  See GOAL-HEADQUARTERS.md LIFT PATTERN + watermark for the trail. |
