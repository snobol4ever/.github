...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`5bd1fdc6`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. **RUNG THREE-SECTION-X86 (TSX)** — PURPOSE: DELETE every binary-byte-emitting function (insn_* ×52, bb_emit_byte sink, emit_seq_*, emit_call_sym_plt, emit_sym_lea_*). Raw bytes go INLINE as std::string data in each template's MEDIUM_BINARY arm. Method: DISCOVER → INLINE → DELETE → WIRE. **DISCOVER phase COMPLETE this session** — TSX-SM-4 (`c7d47736`), TSX-SM-5 (`39f95975`, all 14 SM templates OK), TSX-B1 (`f222b7a3`, BB stragglers + charset-helper exemption), TSX-X1 (`5bd1fdc6`, 7 XA templates). **TSX-AUDIT ✅ `util_three_section_audit.sh` GREEN** — every compiled SM/BB/XA x86 block has all three medium sections. **NEXT: TSX-INLINE** (inline determinable bytes from capture_bytes/insn_* sites; flagged-empty PLT/rip-rel/rel32 arms wait for TSX-WIRE). Then TSX-DELETE → TSX-WIRE. ⛔ PENDING Lon directive: TEMPLATE PURE-PROJECTION (templates drive only from g_emit; no traversal/recursion — see GOAL-HEADQUARTERS session state). STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
