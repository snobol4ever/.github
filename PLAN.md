...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **🎉 PHASE 2 COMPLETE; PHASE 3 OPCODES STUBBED ✅ sess 2026-05-12 (Claude Sonnet 4.6). SI-16 one-shot: all 87 SM_* opcodes now have at least a stub — conditional returns (`_S`/`_F`), `SM_INCR`/`DECR`/`ICMP_GT`/`LT`, `PUSH_NULL_NOFLIP`, `SM_DEFINE`/`DEFINE_ENTRY` no-ops, `SM_LOAD/STORE_FRAME` + `_GLOCAL` (stub w/ TERMINAL note), pattern callbacks (`PAT_CAPTURE_FN`/`_ARGS`, `PAT_USERCALL`/`_ARGS`) stubbed, generator coroutines (`SUSPEND`/`SUSPEND_VALUE`/`RESUME`/`GEN_TICK`) stubbed, BB_* broker opcodes stubbed. Stubs report to TERMINAL but don't crash. Test `si_16_cond_return.sc` exercises `SM_RETURN_S` via `:S(RETURN)`. Earlier this session: SI-12 (SM_CALL_FN/APPLY), SI-13 (frame stack + SM_CALL_EXPRESSION + SM_RETURN family), SI-14 (SM_JUMP_INDIR), SI-15 (real-program cross-checks) all closed. Also fixed long-standing si_07_pat_lit FAIL via SM_EXEC_STMT named-subject write-back. Cross-check gates: self_host 14/14, smoke (snobol4/snocone/icon/prolog/rebus/raku/scrip_all_modes) 33/33, snobol4/snocone crosschecks 11/3 FAIL pre-existing. **Zero regressions across the broader matrix.** NEXT: Phase 3 wiring — real BB scheduler, real SUSPEND/RESUME, SM_PAT_CAPTURE_FN with host pattern-engine hook.** |
