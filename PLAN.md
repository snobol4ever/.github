...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | PJ-10a/10b ✅. **NEXT: PJ-11a** — ban `pBB` dereference / raw `(uintptr_t)pBB` in BB templates for modes 3+4; replace with `_.node->*` + `bb_ptr_slot_lbl` / `bb_rt_obj` pattern. Then PJ-11b..d (full fix), PJ-12 (free SM+BB after emit in modes 3+4). Test modes 2+3+4 after every sub-step. |
