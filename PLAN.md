...
| **⚡ Parser-SC Transpile** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **NEXT: SCT-SN4-ERR041** — ERROR 041 on multi-stmt fixtures under -CASE 0. Root cause: -CASE 0 folds `c` identifier, corrupting DATA('tree(t,v,n,c)') field accessor. Fix: rename `c` field to `ch` in corpus SCRIP/tree.sc (and all callers). one4all `4decab7d`, corpus `9dfe203`. SN4 PASS=49 PARSE_ERROR=24 OTHER_FAIL=15 / 88. |
