...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **ICN-G-1 ✅** mode-4 gate built; **gate PASS=2** (`to5`, `to_by`). Fixed: every-loop ω-exhaustion (SWITCH a[0].i→ω jmp .L<exit>), ACOMP macro arg (`mov edi,\n`), bb_to_by TEXT yield (rt_push_int not raw r12). NEXT: `lt`/`mult`/`compound` filter/cross-product cases need the BB-port-graph zipper (operand re-eval across back-edge). FACT RULE 0. |
