...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **NEXT: EC-UNI-14 proper** — delete silo walkers + `dispatch_one_x86` wrapper, retire `SCRIP_UNIFIED_DISPATCH` flag once one session passes byte-identity matrix at floor.  Open follow-up: split `sm_push_null()` template so x86 doesn't need the SM_PUSH_NULL_NOFLIP exclusion.  **Closed ladder:** EC-UNI-10 → 13(a–e) → 14-PREREQ → 14(a) → 14(b).  See `GOAL-HEADQUARTERS.md` watermark for full hash trail. |
