...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | EM-6 LANDED sess #70 (pattern matcher integration; gate PASS=10). ScripRtVal/ScripRtTag removed — DESCR_t throughout. libscrip_rt.so links full SNOBOL4 runtime; real nv_get/set; all scrip_rt_pat_*() + scrip_rt_exec_stmt(); string rodata + RIP-relative LEA; SM_STNO no-op; SM_HALT uses halt_tos. Next: EM-7 (beauty.sno oracle). |
...
| **PARSER-REBUS (pattern frontend)** | `GOAL-PARSER-REBUS.md` | corpus+one4all | RB-FW-7 LANDED (2026-05-06, PASS=82 FAIL=0: compound_stmt { s; s; }; BUG-COMPOUND fixed in rebus_lower.c. corpus@6267a27) |
