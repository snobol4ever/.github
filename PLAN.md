...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-19-remaining** — ir-run 206. Done: s[i]:=val in every loop (BB frame scope_get); s[i:j]:=val standalone (SM ICN_SECTION_RANGE_SET). Broken: "if s[i:j]:=val" BB path (icn_value.c TT_SECTION→TT_IDX mismatch); substring J (s[i]:=t[i] in every). CH-17g-blocked: scan/string/wordcnt. caa71037. |
