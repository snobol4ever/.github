...
| **Prolog BB JCON triage** | `GOAL-PROLOG-BB-JCON.md` | one4all+corpus+.github | **PJ-8** — delete `pl_runtime.c` AST-walking from modes 2/3/4 (stub `_usercall_hook` prolog branch in `interp_hooks.c` and pl_broker.c per RULES.md `[NO-AST]` template). PJ-7 ✅ (2026-05-16e): backtracking pump landed — IR_PL_CHOICE stateful, IR_PL_CALL resumable, IR_PL_SEQ with backtrack cursor. smoke_prolog 5/5 (was 4/5), broker 20/49 (was 19), honest_prolog 124/0/0, no regressions. |
...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **DONE** PST-RAKU-3b ✅ 2026-05-16. All violations fixed; gates green. |
