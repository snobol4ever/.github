...
| **CLI 3-mode collapse + AST-interp delete** | `GOAL-CLI-3MODE.md` | one4all+.github | **CLI-3M-10 ✅ partial** (2026-05-17j, Opus 4.7). Deprecated alias surface eliminated: scripts/ + docs/ swept across one4all+corpus+.github (commits `b65882ea` scripts/docs, `730da38e` scrip.c). Old `--ast-run`/`--ir-run`/`--sm-run`/`--jit-run`/`--jit-emit`/`--sm-emit`/`--bb-driver`/`--bb-live`/`--dump-ir`/`--dump-ir-bison`/`--x64` all deleted from argv parser + usage text + DEPRECATE() machinery purged. scrip.c internal vars renamed to match mode names: `mode_sm_run`→`mode_interp`, `mode_jit_run`→`mode_run`, `opt_jit_emit`+`opt_emit_x64`+`mode_jit_emit_x64`→`mode_compile`/`mode_compile_x86`, `dump_ir`→`dump_ast`. Three scripts also acted on: `test_icon_ir_all_rungs.sh`→`test_icon_all_rungs.sh` (renamed); `test_smoke_jit_emit_x64.sh`→`test_smoke_compile.sh` (renamed; harness build target broken pre-existing); `test_icon_sm_no_ast_walk.sh` (deleted — tautology after AST walker amputation; flaky-gate row also removed from RULES.md). NEXT: CLI-3M-7 (`--monitor` fate, Lon call) → CLI-3M-9 delete 9 `interp_*.c` files (big rip) → CLI-3M-11 final doc pass → CLI-3M-12 unblock AR-3. Prior: CLI-3M-1..6 ✅ 2026-05-17 (Opus 4.7); canonical flag names `--interp`/`--run`/`--compile`/`--bb={brokered,wired}` introduced as live aliases; BB axis enforced only under `--interp`; tty-only deprecation warnings on legacy flags. one4all `730da38e`, .github `TBD`. |
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17j (Opus 4.7): **DAI-5c ✅** + **DAI-6 ✅**. **IJ-DEL-ICN-AST is complete.** DAI-5c-trans (commit `1823e90b`) added a `--mode` parameter to the rung script to measure both legacy `--ir-run` (mode 1) and `--sm-run` (mode 2): both 194/265, byte-identical PASS/FAIL sets (diff = 0 lines). Then CLI-3M-10 collapsed the script to a hardcoded `--interp` canonical (commit `b65882ea`), renamed it `test_icon_all_rungs.sh`, and deleted the redundant `test_icon_sm_no_ast_walk.sh` honest gate. DAI-6 docs: `ARCH-ICON.md` Active-Goal pointer refreshed (NATIVE → JCON) + IJ-DEL-ICN-AST + CLI-3M-10 post-amputation note; `RULES.md` mode-1 paragraph rewritten + Icon-specific addendum + flaky-gate row removed; this PLAN.md row updated; GOAL-ICON-BB-JCON watermark refreshed; GATE-3/GATE-4 of the goal collapsed to a single `test_icon_all_rungs.sh --interp` row. New floor: Icon `--interp` 194/265. Smoke ×6 unchanged (smoke_snobol4 +1, smoke_unified_broker +1 — both predicted by CLI-3M-6); crosscheck_prolog 128/0/4SKIP/11ORACLE_MISS unchanged; zero DAI-BOMB fires. Prior: DAI-5b-2 ✅ (`ed957bfe`) deleted `icn_value.c`/`icn_stmt.c`/`icn_stmt.h` (−294 LOC), scrubbed public AST-walker decls; DAI-5b-1 ✅ (`28005157`) helper relocation; DAI-5a ✅ (`f7c9cfeb`); DAI-4 ✅ (`e3c803ea`); DAI-3 ✅ (`a2365c5f`) ICN_BB_EVAL no-op; DAI-2 ✅ (`3e1fc9cd`) bodies → DAI-BOMB (−1641 LOC); DAI-1 ✅ (`b3e08fe8`) dead IR_ICN_EVERY. |
...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-pivot 2026-05-17 (Sonnet 4.6):** icon ✅ raku ✅ (with helpers+stubs) snobol4 ✅ snocone ✅. Prolog: 1 bug — Compiland nInc fires at build time not match time; fix: move nInc to AFTER first committed token in directive/clause/dcg_rule (e.g. `$':-' nInc() ...`). Rebus: not started. SCT-9g-snobol4 ✅: Expr3/Expr4/Expr11 n-ary, exponent right-assoc, OPSYN #/% left-assoc. corpus `b349680` one4all `fef30354`. See HANDOFF_2026-05-17f.md. |
