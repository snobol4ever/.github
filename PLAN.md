...
| **BB Template Ladder — Icon + Prolog** | `GOAL-BB-TEMPLATE-LADDER.md` | one4all+corpus+.github | ICN-T-5 ✅ `a2cbd6d8` (BB_UPTO inline), ICN-T-8 ✅ (BB_ITERATE inline). GATE-PK 513/0/602. Invariant 8: no RT calls, all inline x86. **NEXT: ICN-T-6** (BB_GEN_ALT), **ICN-T-7** (BB_LIMIT), **ICN-T-9** (BB_GEN_SCAN), **ICN-T-10** (BB_KEYWORD), **PL-T-4** (BB_CALL). |
| **Prolog BB** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | PJ-12a/b ✅ `d073acf9` — tree_t freed after lower; SM+BB freed after emitter; ast_tree_free added to ast.h. **NEXT: PJ-11a** — ban raw `(uintptr_t)pBB` in BB templates (use `_.node->*` + bb_ptr_slot/bb_rt_obj). Then PJ-12c ASAN verify. |
