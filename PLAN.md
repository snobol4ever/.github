...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-sm-three-column IN PROGRESS sess #89, 2026-05-09** — three-column SM dispatch format (`.LpcN:                  SM_OPCODE args                       # comment`) PLUS macro-library externalised to standalone `sm_macros.s` (`.include "sm_macros.s"` in the emitted .s).  Roman.s shrinks 474→215 lines; sm_macros.s is 229 lines and regenerated alongside each emit run.  ⛔ INCOMPLETE — gate-script regex assertions in `scripts/test_smoke_jit_emit_x64.sh` still expect the old `\\tSM_PUSH_INT 42` shape (leading tab) but the new shape is `.LpcN:                  SM_PUSH_INT 42` (leading label).  Smoke ×6 / isolation / unified_broker / tracked-artifact regen NOT yet run.  Next: fix gate regexes, run all gates, regen 5 tracked .s, then commit clean.  See watermark + per-step body. |
