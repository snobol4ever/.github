...
| **SNOBOL4 Pattern BB** | `GOAL-SNOBOL4-BB.md` | one4all+corpus+.github | SBL-ANY/NOTANY/SPAN/BREAK/CAPTURE/**ARBNO** TEXT arms ✅ + lower_flat_invariant ASSIGN_IMM/COND inclusion ✅ + PAT_LIT/REFNAME/USERCALL GAS macro-annotation fix ✅. Rung M4=8, SKIP=1 (was 6), broad corpus 137→**183** (+46). **NEXT: 051 alt-child duplicate-symbol bug** (pre-existing) or SBL-BREAKX or BINARY arms. Mode-2 unchanged 7/7; GATE-PK still stale. |
...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | ⛔ **NEXT: RUNG ICN-Z-ATOMIC — wholesale conversion of operand-bearing composites (BINOP/CALL/IF/CONJ/ALT/ASSIGN/EVERY/TO/TO_BY) to 4-port threaded form, ONE pass.** Fresh context required (~80%+). See "START HERE FIRST RUNG FIRST STEP" block at top of goal file: structural insight (operands are PEERS not children, mirroring irgen), 5 numbered steps, 7 construct families in order, full gate set, acceptance criteria. Litmus: multi-write `write("a"); write("b")` must print `a\nb` (today prints only `a`). PRIOR: ICN-Z-2b ✅ (leaf+SEQ explicit ω-advance wire, bb_exec port-follower). Gates: smoke_icon 5/5, broker 23, rungs 198, smoke_prolog 5/5, FACT 0. |
