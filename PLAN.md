...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **LOCAL-PURGE-6 ✅** (`07708564`) — pl_atom/arith/unify/builtin driver-lifted via `bb_prepare_pl`; fixed shared-buffer aliasing (`emit_intern_str` `g_intern_str_buf` → distinct `g_emit.bb_pl_{ls,rs,op_lbl}`). All four bodies zero `emit_intern_str` calls. Purity audit cleaned (`f5fafb68`, xa_epilogue false-positive removed) → 45 violations all in bb_capture JVM/JS arms. GATE-PK 504/0/625, AUDIT GREEN, prolog 124/0/0, smoke 188/190-71. NEXT: **LP-7-NONX86** (convert bb_capture JVM/JS arms to pure-return — BLOCKED on x86-only rule, awaits Lon) or next HQ rung. ⛔ Beauty gate SUSPENDED. |
