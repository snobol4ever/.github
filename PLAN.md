...
| **Raku BB** | `GOAL-RAKU-BB.md` | **RK-EXCEPTIONS ✅** (Claude Sonnet 4.6, 2026-05-28, one4all `ed6fec27`): try/CATCH/die mode-4. Added `raku_exc_clear/check/get` to `raku_try_call_builtin_by_name` (were referenced in lower.c but never implemented in rt path). Fixed `raku_die` SSE-safe. Fixed `raku_try_hash_builtin` type guard (was segfaulting `say(int)` inside sub with `my`-decl via VARVAL_fn on INTVAL). GATE-RK4 22→23. GATE-RK 20→21. 10 FAILs remain: REGEX/NFA (6, deferred), I/O (2), JUNCTIONS (1, blocked Q9-Q12), CLASS (1). NEXT: I/O. |
