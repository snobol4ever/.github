...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-TEMPLATE-PURITY-2 (add t_bb_port_alpha/beta + low-level t_* helpers to bb_emit.h).** BB template files split one-per-box sess 2026-05-11 (Claude Sonnet 4.6, one4all `5fa770d6`): bb_xfarb→xeps+xfail+xfarb, bb_xlnth→xtb+xrtb+xlnth, bb_xposi→xposi+xrpsi. SM families stay together (Lon decision). Gates: build clean, template-byte-id 4/4. |
