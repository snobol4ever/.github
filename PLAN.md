...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-5b (session #71: identified and fixed scrip multi-file label-collision bug. Per-file Snocone CfState reset `label_ctr` to 0, so each .sc produced its own L.1, L.2, … and after scrip's IR merge, `label_table_build` registered first occurrence — subsequent files' :goS/:goF refs landed in the wrong file. Symptom: `./scrip --ir-run global.sc minimal_match.sc` silently terminated mid-statement (if-arm jumped into global's UTF loop); reverse order looped infinitely. Fix: replaced `st->label_ctr` with file-scope `static int g_snocone_label_ctr` in `src/frontend/snocone/snocone_control.c::newlab` (+17/-1). All three baseline gates remain green: PASS=5 / PASS=42 SKIP=3 / PASS=49. The 14×3 beauty subsystem suite remains 42 PASS + 3 SKIP. Disconfirmed session #70's claim that the 9-line "proc with if-body" reproducer hangs — it does not, in any mode. Disconfirmed session #69's ARBNO+&FULLSCAN hypothesis (FULLSCAN=0 doesn't help). Tightest residual reproducer: `echo START | ./scrip --ir-run $LIBS beauty.sc` hangs 5 s; SPITBOL prints START in 13 ms. With `&STLIMIT=500000` prepended, scrip emits the 7-line comment header byte-identically (md5 matches SPITBOL on first 7 lines), then Error 22 on bare-label line. Trivial `v?POS(0)` after libs works ✅; `Src?POS(0)` inside beauty.sc still hangs ❌ — the difference is something beauty.sc does at top level before the match. Next session: bisect beauty.sc top-level by appending chunks to minimal_match until OK→hang transition isolates the trigger.) |
