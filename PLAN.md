...
| SNOBOL4 Frontend Ladder | `GOAL-LANG-SNOBOL4.md` | one4all | SN-3 IN PROGRESS: shadow table implemented, not yet firing — debug intercept next session; HEAD 31b916ea | ☐ |
...
| Cross-Lang Verify | `GOAL-CROSS-LANG-VERIFY.md` | one4all | S-1 (prerequisite: Prolog Phase 1C) | ☐ || Snocone Beauty | `GOAL-SNOCONE-BEAUTY.md` | one4all | SB-1 next (diagnose lowerer underflow sites in beauty.sc); blockers: $'...' lex + *deref replacement | ☐ |
...
| NET Beauty Self-Host | `GOAL-NET-BEAUTY-SELF.md` | snobol4dotnet | S-2 next: $'@S' empty after ARBNO(*Command) match — Shift never fires; investigate (A) semantic side-effects in child scanner, (B) FRETURN propagation in nested fn-call args; beauty 17/17; no code changes this session | ☐ |
