...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#7 LANDED (session 2026-05-05): parser_snobol4.sc only — `rw_tag` rewrite layer removed; grammar emits canonical IR (E_*) tags directly via `&` OPSYN. String capture switched to inner-body + `Push_qlit` worker (pair-shape from iter#5). `rw_expr` simplified: dropped tag-rename, dropped quote-strip branch; left-rotation predicate becomes `is_rotatable(t)` over the seven binary-arith / capt-asgn IR tags. Iter#5 pair-shape @ corpus@5a602ff; iter#6 token-variables/Snocone ops @ corpus@7d05e04. Gates all 100%: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38. Next: operator picks iter#8 concept. |
