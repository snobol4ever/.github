...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | iter#8 LANDED (corpus@ab89236): FENCE around all the right alternation tails. **Next session opens at iter#9** (left-associative arith assumption: re-examine — convert grammar to left-recursive `ARBNO(tail)` shape and remove `is_rotatable` + rotation block from `rw_expr`) **and iter#10** (FENCE sweep second pass: low-level lexical token classifiers + `nl_opt`/`semi_opt`/etc.). Operator picks order. Gates all 100%: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38. |
