...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: sub-rung -f (SM-axis: sm_push_lit_i, now unblocked by Option C decision).**  -e ✅ sess 2026-05-11 (Claude Sonnet 4.6, one4all `157352d8`): charset-family template (XSPNC/XBRKC/XANYC/XNNYC) via `templates/bb_xspnc.c` + callback design. Byte-identical.  -c ✅ closed (Option C: SM_HALT is sanctioned terminative exception).  Gates: smoke 7/7+49/49+5/5; template-byte-id 4/4; claws5.s diff empty. |
