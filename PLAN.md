...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all | EM-FORMAT-BB-LAW ✅ 2026-05-09 — every BB jxx (jmp/je/jne/jl/jge/jg) now in col-3 (file vcol 41); col-2 reserved for non-jump action mnemonics; one space between adjacent jxx, no padding. Bare label+jmp lines have empty col-2 with `jmp <target>` in col-3; fused cond+uncond is `<cond_mn> <succ>; jmp <fail>` in col-3. Next: **EM-FORMAT-BB-LAW-TRIPLE-FUSION** (defer preceding action onto the cond-jmp's line) or **EM-FORMAT-BB-DATA-CONSOLIDATE** (consolidate scattered per-blob `.section .data` blocks), then beauty.sno crosscheck **EM-7d**. (TRAMPOLINE-ELIM is a future optimizer pass, not part of the EM-FORMAT-BB sequence.) |
