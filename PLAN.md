...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-PER-KIND-DIFF NEXT (2026-05-21 evening)** — Audit landed (one4all `d2b6dac3`): only 4 of 17 BB pat-kinds (LIT, SPAN, POS, ANY) flow through `emit_bb_xstar`/etc. in current corpus; 13 are dormant.  **Lon directive evening:** completeness > format-faithfulness; end state is one template per kind for ALL kinds.  Path (c) deletion ruled out — dormant code stays.  Verification problem: existing gates exercise only 4 of 17 kinds.  **Solution:** per-kind unit regression — for each SM opcode and BB kind, synthesize an instance, emit via legacy AND template paths, diff.  Author `tools/emit_per_kind_audit.c` + `scripts/test_per_kind_diff.sh`.  Per-kind PASS/FAIL table replaces "hope corpus triggers kind" coverage.  Failing kinds get re-lifted byte-faithfully (`EC-UNI-REFAITH`); only when per-kind diff is 100% PASS does `EC-UNI-REWIRE-ALL` proceed.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 stable. |
