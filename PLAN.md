...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **✅ CLOSED** sess 2026-05-11c (Claude Opus 4.7, one4all `ff9100cb`). ME-1..14 complete. Reopened GOAL-MODE4-EMIT `EM-MODE4-IS-MODE3-DUMP`. |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: sub-rung -i (BB-axis: bb_xbrkx).** -h ✅ sm_void_pop `87f59f43`. -g ✅ bb_xlnth/xtb/xrtb `9e2ea80e`. -f ✅ sm_push_lit_i. -e ✅ charset. -c ✅ Option C. |
...
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SI-5** — icon/prolog/raku/rebus/snocone emit AST_STMT/AST_END directly. SI-4 ✅ `9c21656d` (sno_parse_ast single-pass dual-build; sm_preamble(prog, ast_prog); byte-identical 30/30). |
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **PAUSED — awaits SI-5..SI-8.** Translation is a transcription exercise once `lower.c` is pristine. SL-0 ✅ folder + goal created. |
