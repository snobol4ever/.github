...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | **WAM-CP-13 print/1 mode-4 ✅ (mode-4 corpus 54→55) + WAM-CP-6 B3 TRAIL-RECLAMATION ✅ (LCO COMPLETE)** (Opus 4.8, 2026-05-29, one4all `2fae45ec`). **WAM-CP-13 print/1:** one-line widening of the MEDIUM_TEXT `write` arm in `bb_builtin.cpp` to also match `print` — write-equivalent for atoms/ints/compounds; existing arm routes BB_ATOM/BB_PL_VAR/BB_LIT_I/BB_PL_STRUCT correctly, no nl-suffix for print. rung22_print PASS, FACT 0/12 (widened strcmp, no new byte producer). NEXT mechanical: writeq/write_canonical (quoting+operator writer via pl_writeq/pl_write_canonical effect helpers after rt_pl_node_to_term), char_type/2, numbervars/3. **WAM-CP-6 B3 (`0019cc7b`):** closes the heap ceiling B2 left — `sumto(10000000,0,R)=50000005000000` runs in O(1) trail/heap (was OOM-Killed). `bb_exec.c` only, +45 lines, mode-2. On a deterministic redirect (caller frame provably dead) the freshly-bound callee-arg trail entries are slid down onto a fixed per-chain baseline (`g_pl_b3_call_mark`) and `top` reset, discarding the dead slab; slid args stay BOUND (not unwound), reclaimed vars GC'd, accumulator survives via `at->ref`. Probe: trail top pinned at 13, GC heap flat 3MB across 10M iters. Gates byte-identical: GATE-1 5/5, GATE-2 132/0 (5 ORACLE_MISS), GATE-3 m2 104/107, GATE-4 4/4, GATE-SWI 57/57 (memberchk sentinel held), mode-4 corpus 55/107, FACT 0/12, sibling icon/raku 5/5/5, snobol4 13/13. Handoff `HANDOFF-2026-05-29-OPUS48-PROLOG-BB-WAM-CP-6-B3-TRAIL-RECLAMATION.md`. **NEXT — LCO/CP stack-and-heap track CLOSED.** Substantive: continue WAM-CP-13 mode-4 corpus (55→~60, mechanical, CAT-D — writeq/write_canonical/char_type/numbervars). Alternatives: WAM-CP-7 unify specialization; PL-RT-ASSERTZ. |
