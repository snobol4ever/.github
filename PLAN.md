...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | LFJ-0..LFJ-1b ✅. LFJ-1c ✅ (`5cd9003d`). **LFJ-2..LFJ-7 ✅** (`0b3b331d`, Opus 4.7, 2026-05-27): 28 AST kinds flipped to `lower_icn_new.c` in one session. NoOp/Intlit/Reallit/Stringlit/Csetlit/Global/Binop (20 kinds)/If/ToBy all transcribed from ir_a_* procedures in irgen.icn. Gates green throughout (5/5 · 198 · 24 · 5/5). Three legacy helpers de-static'd for cross-file use (lower_icn_legacy_NULL, lower_icn_expr_node, icn_fold_signed_lit); all legacy fns remain linkable per LFJ-1b one-line-revert contract. Next: **LFJ-8** — transcribe `ir_a_Every`, flip TT_EVERY. Intersects with AG-pure step 8.1 BB_EVERY logic (lower_icn.c:444 flat-wire gate, marker `nd->ival=1`) and TO/TO_BY 8.2 dispatch — needs careful attention. |
