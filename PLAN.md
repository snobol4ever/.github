...
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** — 8 §⛔ violations remaining. PRF-12-mcall/hof/capture/twigil/gather-splice/gather-hoist pending. one4all `9700c0c3`, corpus `2e6e6bd`. NEXT: PRF-12-mcall (R22) — `obj.method(args)` → TT_METHCALL kind. ⚠ Always regen `raku.tab.c` with `bison -d raku.y -o raku.tab.c` after any `raku.y` change. |
