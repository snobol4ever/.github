...
| Icon Frontend Ladder | `GOAL-LANG-ICON.md` | one4all | IC-9 (session #30, 2026-05-02: two bugs fixed — parser & precedence (wrong: tighter than :=, correct: looser) and icon-frame E_SEQ missing from frame switch. x:=5 & write(x) now correctly parses as (x:=5)&write(x). rung36 14/31/30, ladder 202/31/30. one4all HEAD 18331036.) |
| **Prolog Frontend Ladder** | `GOAL-LANG-PROLOG.md` | one4all+corpus | **PR-17** (PR-16 LANDED — atom builtins rung39 5/5. Next: string builtins rung40) |
...
| **Snocone-in-Snocone** | `GOAL-SNOCONE-IN-SNOCONE.md`   | one4all+corpus | SS-2 (SS-0 audit DONE — interp.c not a clean model; rewrite proposal written. interp.sc to be written fresh ~200 lines clean, not ported from 6201-line interp.c. parse.sc recursive-descent until beauty.sc lands. ~20 of 90 EKind values needed. Next: SS-2 value.sc Val struct.) |
| Snocone Beauty (ON HOLD) | `GOAL-SNOCONE-BEAUTY.md`     | corpus+one4all | **ON HOLD session 2026-05-02 #19** — SB-6.E.7-H runtime bug remains the only unblocking lever for SB-6, but no methodology was found to localise it. Three sessions of bisection produced contradictory framings (rollback / leak / failure-walker / abandoned-seal) without converging on a fix. Per Lon (this session): "you will never find it then. You have no methodology to find it." Goal preserved for later resumption when a new diagnostic angle surfaces. SB-6 fingerprint at hold: lines=98 stderr=4488 parse_err=12 internal_err=0 rc=124, 19 hunks vs oracle. Three baseline gates green throughout. Last active step before hold: **SB-6.E.7-J pass #3 COMPLETE on .sc side; pivot to SB-6.E.7-H** (session 2026-05-02 #18: goto-based mechanical port of beauty.sno main00..main05 LANDED. Each `:F(LBL)` → `if (~(EXPR)) { goto LBL; }`, each `:S(LBL)` → `if (EXPR) { goto LBL; }`, each `:(LBL)` → `goto LBL;`. Body parts (Line=INPUT, Src=Src Line nl, Src ? *Parse *Space RPOS(0), DIFFER(sno=Pop()), pp(sno), the OUTPUTs) preserved verbatim with .sno line-comments inline. Per Lon: "use the goto based main loop. It just works without thinking. It is byte identical to SNOBOL4 except a colon character and the if/else. Straight mechanical." Fingerprint movement: lines 603→98 (down — old shape's invented flags input_done/have_line/cont_more inadvertently dampened SB-6.E.7-H rollback bug, RULES.md violation), parse_err 170→12, internal_err 4→0, hunks vs oracle 91→19 (5× closer). Two alternate envelopes tested (structured nested-while + break LABEL; IDENT-guard hybrid) — both produced identical fingerprint, proving runtime is insensitive to envelope; goto chosen as mechanical translation. Three baseline gates green. Spot-checked all 16 lib .sc files (assign/match/stack/case/counter/ShiftReduce/omega) confirmed body-part-faithful to .inc — consistent with session-#15 audit. SB-6.E.7-J COMPLETE on .sc side; **only lever that moves SB-6 forward is now SB-6.E.7-H** runtime rollback fix. Findings: `.github/SB-6.E.7-J-pass3-session18-main-loop-findings.md`. Diff vs old shape preserved at `corpus/programs/snocone/demo/beauty/docs/SB-6.E.7-J-pass3-session18-main-loop-goto-attempt.diff` (126 lines). NEW SB-6 fingerprint: lines=98 stderr=4488 parse_err=12 internal_err=0 rc=124, 19 hunks.) (session 2026-05-02 #17: pp/ss invented-helper removal + per-type body-part dispatch LANDED. Removed ppLeaf/ppList/ppStmt/ss_leaf (4 invented helpers, none in beauty.sno); inlined 10 leaf + 6 goto-clause + 7-component pp_Stmt + per-type pp_,/pp_|/pp_../pp_[]/pp_()/pp_Call/pp_ExprList body parts; added 15+15 explicit pp_OP/ss_OP cases per beauty.sno 296-323/502-529; added ssUnOp/ssBinOp helpers symmetric to ppUnOp/ppBinOp.) SB-6.E.7-A closed. SB-6.E.7-H is the active blocker. |
