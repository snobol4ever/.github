...
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **PST-RB-5i-PRE ✅** 2026-05-17 (Opus 4.7, emergency handoff): CSO-1..CSO-8 landed as combined commit `d432ed6f`. Zero fold infra / strcasecmp / kw_case remain. &CASE read-only returning 0 (folding-OFF per SPITBOL Ch.16 spec); writes Error 10. CMPILE.[ch] + util_compare_cmpile_vs_bison_ir.sh archived. Gates at baseline: subsystem 19/1 (Qize bb_pool exhaustion diagnosed → PST-RB-NEXT-BB-CACHE). Milestone 1 unchanged from pre-CSO state (scrip+beauty.sno = 0 lines, pre-existing per GOAL-LANG-SNOBOL4.md 270-285). NEXT: PST-RB-5i-PRE-CORPUS (corpus case-canonicalization) → PST-RB-NEXT-BB-CACHE (pattern EXEC cache + 4-datatype short-circuit: *Var/*(expr)/EXPRESSION/CODE need only DATA, never EXEC re-emit per Lon directive 2026-05-17) → PST-RB-NEXT-LABTAB → PST-RB-5i finish (rt_bb_arbno ζ) → 5j → 5k. |
