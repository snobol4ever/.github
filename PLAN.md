...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **NEXT: SI-6 SM_EXP (exponentiation).** Phase-2 plan landed sess 2026-05-12 (Claude Sonnet 4.6) — 10 rungs SI-6..SI-15 derived from SPITBOL manual v3.7 (chapters 3/4/6/7/15/17/18/19). C-parallel rename also applied: `struct SM_State { stack, sp, stack_cap, last_ok, pc }` mirroring `sm_interp.c` field-for-field; `sm_push`/`sm_pop`/`sm_interp_run` match C signatures with `st` elided as module-global singleton. Lower.sc still uses `g_count`/`g_instr_tbl`; mechanical rename to `struct SM_Program { instrs, count }` deferred as an SL- follow-up. |
