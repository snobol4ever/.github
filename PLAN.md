...
| **⚡ PST: SNOBOL4** | `GOAL-PST-SNOBOL4.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** per `PST-LR-AUDIT.md § Scan 3` re-grade. W1/W2/W3 (orig scope) ✅; **W2-AUDIT** (goto_expr always-fresh-wrap) and **W3-AUDIT** (g_cur → bison-local TAL) added 2026-05-19. |
| **⚡ PST: Parent (HQ coordination)** | `GOAL-PARSER-PURE-SYNTAX-TREE.md` | one4all+corpus+.github | **PST-LR-AUDIT-1 ✅ COMPLETE** 2026-05-19 (Opus 4.7). All sub-steps 1a–1j done. 51 §⛔ violations identified across all six parsers (11 Snocone, 1 Icon, 2 SNOBOL4, 27 Raku, 6 Rebus, 4 Prolog); every violation owned by a named step entry in its per-language goal file. Phase 1 ⏳ for all six parsers (no per-language rung is yet closed). Phase 2 SCRIP mirror work remains blocked. Six per-language rows below. |
