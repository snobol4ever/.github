...
| **Prolog BB JCON triage** | `GOAL-PROLOG-BB-JCON.md` | one4all+corpus+.github | **PJ-9a ✅** Mode-3 JIT `h_bb_once_proc` wired through `pl_bb_once_proc_by_name`+`bb_broker` (was `[NO-AST]` stub). `test_crosscheck_prolog.sh` 0/4 → **4/4** (hello+backtrack+arith+recursion agree across `--ir-run`/`--sm-run`/`--jit-run`). All other gates unchanged. Also regenerated stale `snocone_parse.tab.h` to unblock builds since `4aa8727b`. NEXT phase per goal-file Done-When: inline x86 emitters for Prolog primitives (mode 4) — open PJ-9b when starting. |
...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **REOPENED** PST-ICN-4a — SCRIP mirrors not brought to shift/reduce-only. parser_icon.sc: 13 helpers; parser_raku.sc: ~80 helpers (flatten_* = child-order violation). C infra needed: TT_MATCH_UNARY, TT_FIELD child fix. |
