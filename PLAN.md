...
| **SNOBOL6 Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1 (LS-0 LANDED, sessions 2026-04-30 #1+#2+#3: Lon's directive captured — new language name is **SNOBOL6** (session #3 — funny because SNOBOL4 was the line-by-line one missing structured control, SNOBOL5 already exists as Gimpel's never-shipped successor, SNOBOL6 is the structured-control reboot picking up where 5 left off; source tree keeps `snocone` paths through LS-6, rename sweep deferred to `GOAL-LANG-SNOBOL6-RENAME`). Language is **SPITBOL functional superset** with C/C++ control flow plus 14 comparison-sugar ops, juxtaposition concat (no `&&`), alternative-eval `(,,)` (no `||`), backtracking-expression conditions (SUBJ ? PAT = REPL legal), full SNOBOL4 OPSYN, Flex+Bison implementation. **Three open questions pending Lon's clarification before LS-1**: Q12 identity-op spelling (`::` `:!:` vs `:===:` `:!==:` vs `===` `!==` — Lon wrote ambiguous "`::` `:!:`", default placeholder Option A); Q13 break/continue form (A both `break;` and `break label;` Java-style / B labeled-only / C no-break-just-goto — default placeholder A); Q11 rename-sweep timing (default deferred to its own goal after LS-6). All session #1+#2 commitments preserved: 14 comparison ops kept, % is OPSYN-only, bare expressions fail silently, labels + goto, no newline magic, dangling-else binds inner. **Next**: Lon's word on Q11/Q12/Q13 → fold into goal → LS-1 lexer state-machine spec + 50-line test corpus, no code yet.) |
