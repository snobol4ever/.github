...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-BB-FORMAT-ARCH** — port-context accumulator in bb_emit.c for 4-col `;`-fused FORMAT mode (no `if` in templates; pure x86 output; col1=label col2=instr col3=operands col4=jmp). Then EM-BB-FORMAT-1 (XPOSI/XRPSI). Then ESA-1/2 (.s artifacts to corpus). New steps added: EM-SPEC-T-ERADICATE (remove spec_t for cross-language BB), EM-XVAL-DESCR (DESCR_t protocol SNO/PL/ICN), EM-S-ARTIFACTS-COMMIT. Baseline: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty-subsystems PASS=10. one4all `d96d5520`. |
