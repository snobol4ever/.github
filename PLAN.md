...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-3.d (session 2026-04-30 #9: LS-3 REDESIGNED with W{OP}W envelope pattern matching snobol4.l lines 235-315 exactly. Token names aligned with snobol4.tab.h for every concept equivalence (Lon's "same names everywhere" directive): T_IDENT, T_FUNCTION, T_INT, T_REAL, T_STR, T_KEYWORD, T_CONCAT, T_ASSIGNMENT, T_MATCH, T_ALTERNATION, T_ADDITION, T_SUBTRACTION, T_MULTIPLICATION, T_DIVISION, T_EXPONENTIATION, T_IMMEDIATE_ASSIGN, T_COND_ASSIGN, T_AMPERSAND, T_AT_SIGN, T_POUND, T_PERCENT, T_TILDE, T_LPAREN, T_RPAREN, T_LBRACK, T_RBRACK, T_COMMA, all T_UN_*. Snocone-only: T_LEQ/T_LNE/T_LLT/T_LGT/T_LLE/T_LGE for Andrew's :==:, T_IDENT_OP (::), T_DIFFER (:!:), T_LBRACE/T_RBRACE/T_SEMICOLON/T_COLON, T_KW_*. Space envelops the operator via {W}OP{W}; whitespace alone -> T_CONCAT; bare operators -> T_UN_*. f(x) now lexes as T_FUNCTION T_LPAREN T_IDENT T_RPAREN matching snobol4.y rule line 197 verbatim. one4all `02db637d`. All 31 LS-1.b tests PASS. Next: LS-3.d remove old snocone_lex.c + LS-4 Bison grammar — grammar will share T_* names with snobol4.y.) |
