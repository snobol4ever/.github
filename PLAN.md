...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **BB LAYER TOTAL 2026-05-20** — one4all `99630c7e` added 76 stub template files so BB layer is total over `BB_op_t` (97/97 kinds dispatched).  Matrix gate now reports 110 files, 166 fns, **830 cells PASS, 0 misses** (was 34/90/450).  EC-UNI-16 closed-by-supersession; EC-UNI-15 closed.  **NEXT: copy-paste per-arm output into the 76 stub templates from `src/lower/ir_exec.c` and `src/lower/lower_*.c`** — one kind at a time, all slots already exist.  Then **EC-UNI-22** (close ARCH-IR/ARCH-SCRIP docs) and/or **EC-UNI-21-followup** (M1 reconcile/retire).  See `GOAL-HEADQUARTERS.md` watermark. |
