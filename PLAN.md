...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.l (EMERGENCY HANDOFF LS-4.k one4all `eec7fd0f`: snocone_lower.c/h + snocone_control.c/h moved to archive/; FRONTEND_SNOCONE now contains only snocone_lex.c + snocone_parse.tab.c + snocone_driver.c; scrip.c updated to call snocone_compile() not snocone_control_compile(); archive/snocone_lex.c synonym reverted; snocone_lex.c: 'procedure' synonym for T_FUNCTION added (corpus uses old keyword); arithmetic binary ops +/-/*/^// fixed: when last_value=1 always binary per SPITBOL manual Ch.15; smoke 5/5 green; broker 49/0 green; parse side-channel 965/965 green. BREAKAGE: beauty PASS=6 FAIL=36 SKIP=3 — corpus beauty tests use && (old Snocone concat-and) which new Bison parser rejects. LS-4.l gate of 42/0/3 blocked pending LS-5 corpus migration (util_migrate_snocone_to_lang_space.py). Next: LS-5.a — write migration script to replace && with space, || with (,), then re-run beauty gate.) |
