...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-7 fix `(COND val, alt)` → if/else throughout lower.sc** (sess 2026-05-11 Claude Sonnet 4.6: root cause of Error 3 confirmed — Snocone comma-in-parens is pattern ALT not conditional value; ~24 sites in lower.sc; see GOAL file Critical Syntax section). |
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-7 mass-rename `'TT_*'`→`"TT_*"` in lower.sc** (sess 2026-05-11 Claude Sonnet 4.6: goto_u_expr added, parse-error fixed, root cause of Error 3 found — single-quoted TT_ literals are NAME not STRING in Snocone; Session Setup + Gate updated with correct --ir-run invocation). lower.sc 1028 L. |
