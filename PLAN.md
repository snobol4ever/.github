...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **DAI-8 C5 ✅ 2026-05-18 (Opus 4.7, one4all `af744aaa`) — snobol4_pattern.c dead-fn sweep. −197 LOC.** 16 functions + 1 typedef + 9 header decls deleted. 9 dead public wrappers (`pat_ref_val`, `array_create`, `MAKE_TREE_fn`, `push_val`, `pop_val`, `top_val`, `define_spec`, `apply_val`, `pat_call` — each refs=2 = decl + def only) + 7 static helpers forming a closed dead expression-evaluator sub-graph (`_ev_skip`/`_ev_ident`/`_ev_strlit`/`_ev_args`/`_ev_val`/`_ev_term`/`_ev_expr` + `SnoEvalCtx` typedef). 9 matching decls pruned from `snobol4.h`. Kept LIVE: `register_fn` (heavy use by rt.c + scrip.c for DEFINE_fn dispatch-table population). Methodology #7 (internal-caller-chain analysis, landed in C4) handles the `_ev_*` sub-graph cleanly. snobol4_pattern.c 955→767, snobol4.h 514→505. All gates held floor: smoke ×6 (5/0,5/0,5/0,4/0,5/0,7/0); hello matrix 6/6; Icon `--interp` 194/36/35/265; broker 23/26; crosscheck_prolog 128/0/4SKIP/11ORACLE_MISS; scrip_all_modes 2/0. **Combined day deliverable (C3a+C3b+C4+C5): −511 LOC across 6 files, ~50 fns deleted.** Prior: DAI-8 C4 ✅ (one4all `de6e7b77`); C3a+C3b ✅ (one4all `c3af9e23` + `a7259b9b`). **NEXT: DAI-8 cluster 6+** — remaining candidates: rt.c ~22 (needs methodology #4 closure pass), icon_runtime.c (frontend) 25, emit_wasm.c 22, prolog_builtin.c 20, icn_runtime.c (interp) 18. OR IJ-HELLO-MODE3-RAKU. |
