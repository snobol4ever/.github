...
| **SN4 CELL MACHINE** ⬅ NEW (Lon 2026-07-28 "get this going") | `GOAL-SN4-CELL-MACHINE.md` | ζ-CELL MACHINE: per-BB self-allocation × WAM/FORTH cells; contract `DESIGN-SN4-CELL-MACHINE.md`. Supersedes SNOBOL4-BB s200 rungs (b)/(c), absorbs (a) as CELL-7. Next: CELL-0. |
| **SNOBOL4 BB** | `GOAL-SNOBOL4-BB.md` | ⛔ **READ `THE MODEL` AT THE TOP OF THE GOAL FILE FIRST** — there is NO graph frame; every BB allocates its own storage at α, releases at γ/ω, and jumps. The whole-graph carve is LEGACY DEBT (1057 reader sites / 108 templates), not infrastructure, and reasoning about "which constructs need it" is the named failure mode. Then the `LIVE CURSOR`. Current rung: CARVE-ERAD. |
