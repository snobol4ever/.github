...
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | PST rewrite 5a–5h ✅. Subsystem suite **19/20** (Qize SKIP under SL-3 — acceptance met). 5h flipped 4 buckets (match, Gen, semantic, trace) from one root cause: `lower_scan` dispatched Icon scan-env for all `TT_SCAN`; Snocone `?` is SNOBOL4 pattern match per SPITBOL Ch. 18. Plus AST_*→TT_* sweep in `.y` and AST_*→LX_* in `.c` lex. Active: **5i** (parser AST dumps match `--dump-ast` C ref), **5j** (6-fn trace), **5k** (Snocone INCLUDE). SL-2 closed; SL-3/5 open, SL-4 likely subsumed by 5h. |
