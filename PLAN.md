...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **SI-12 ✅ sess 2026-05-12 (Claude Sonnet 4.6): SM_CALL_FN via host APPLY(); arity dispatch 0..5; pseudo-calls stubbed. Structural: sm_interp_step rewritten from if-chain to switch(opc) O(1) dispatch + +X coercion. Gates: 7/8 PASS (si_07_pat_lit pre-existing FAIL). NEXT: SI-13 — SM_RETURN / SM_FRETURN / SM_NRETURN + frame stack.** |
