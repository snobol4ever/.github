...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: fix downstream match_driver bb_pool crash + case/omega U+2192 `→`→`->` in sm_lower.c. Sess 2026-05-12b (Claude Opus 4.7): C-ABI stack alignment fix for mode-4 DEFINE_ENTRY landed in `sm_emit_template.c::render_macro_body` — `push rbp; mov rbp,rsp` at entry; `mov rsp,rbp; pop rbp` before ret. Beauty-subsystems PASS=6/17 (was 5; Gen_driver unblocked). Gates: smoke 7/7, template-byte-id 4/4. Architectural finding logged: `templates/*.c` are NOT the production macro source — `render_macro_body` in `sm_emit_template.c` is.** |
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | ⛔ SUPERSEDED by GOAL-ICON-BB-NATIVE. SM-coroutine approach wrong. One month lost. |
| **⚡ Icon BB Native (flat BB templates)** | `GOAL-ICON-BB-NATIVE.md` | one4all+.github | **NEXT: IB-1 — emit_bb_icn_to (integer range). IB-0 ✅ `c8032b0c`: scaffolding + N0=212. Boxes are x86 blobs via t_* template functions, one emit_bb_icn_* per construct. 10 rungs IB-0..IB-10, 7 gates each. SM_BB_PUMP at stmt level already correct — do not touch. Zeta allocated at runtime not emit time. |
| **Prolog BB Complete (honest mode 3)** | `GOAL-PROLOG-BB-COMPLETE.md` | one4all+.github | **NEXT: PB-6 TT_CHOICE full inline (OR-box SM coroutine).** PB-0..PB-5 ✅ `695aed00` (Claude Sonnet 4.6): honest dial 17→**106**/294. 14 remain (rung10 puzzles; need proper head-unif env threading). |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: fix downstream match_driver bb_pool crash + case/omega U+2192 `→`→`->` in sm_lower.c. Sess 2026-05-12b (Claude Opus 4.7): C-ABI stack alignment fix for mode-4 DEFINE_ENTRY landed in `sm_emit_template.c::render_macro_body` — `push rbp; mov rbp,rsp` at entry; `mov rsp,rbp; pop rbp` before ret. Beauty-subsystems PASS=6/17 (was 5; Gen_driver unblocked). Gates: smoke 7/7, template-byte-id 4/4. Architectural finding logged: `templates/*.c` are NOT the production macro source — `render_macro_body` in `sm_emit_template.c` is.** |
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-13 step 4 — Verify SM output matches C `--sm-run --dump-sm` for same input.** SL-13b ✅ sess 2026-05-12 (Claude Opus 4.7, one4all `fff3a307`): fixed `--ir-run` pattern captures (`. var` / `$ var`) not writing back to variables. Root cause was NOT in SM_PAT_CAPTURE (sm_interp.c:682) as previously hypothesized — `--ir-run` uses IR tree-walk, not SM. Real bug was in `eval_code.c` `TT_CAPT_COND_ASGN` / `TT_CAPT_IMMED_ASGN` handlers: capture-target resolution fell through to `eval_node(tgt)` which returns the variable's value, not a NAME, for plain `TT_VAR` / `TT_KEYWORD`. Fix mirrors `SM_PAT_CAPTURE` and the existing `TT_NAME` case: short-circuit with `NAME_fn(tgt->v.sval)`. Diff +24/−4 in `src/runtime/x86/eval_code.c`. Authority: SPITBOL Manual Ch. 18 pp. 62, 87. SL-13 step 3 also unblocked: `run_scrip_parser.sh snobol4 trivial.sno` now runs end-to-end (empty SM is separate parser-driver work). Gate: smoke_lower 6/6, sm_lower_test 11/11, smoke_snocone 5/5, smoke_snobol4 7/7, smoke_icon 5/5, smoke_rebus 4/4, scrip_all_modes 2/2; pre-existing baselines unchanged. SL-13a ✅ sess 2026-05-12 (Claude Sonnet 4.6). |
