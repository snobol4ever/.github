...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-INLINE-1 NEXT** (inline body of `emit_sm_<op>_dispatch` shims into `SM_templates/*.c` IS_X86 arms; then delete the shim with `emit_sm_op`/`g_sm_nullary`). INLINE-2..7 follow; INLINE-8 orphan sweep runs LAST. PPV-9 ✅. one4all `794b9435`. GATE-PK PASS=420 FAIL=0 STUB=639. EC-UNI-22 (close/docs) sequenced AFTER INLINE-ALL. |
