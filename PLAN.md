...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-TEMPLATE-PURITY on bb_xchr/xspnc/xbrkx/xlnth — Lon must pick Option A (narrow is_text for zeta storage split) vs Option B (bb_zeta_* vtable abstraction). See watermark.** Sess 2026-05-11 Opus 4.7 (one4all `57d272e0`): emitter vtable extended with `data_long`, `bb_zeta_rdi`, `bb_dispatch_jne_jmp` slots (both backends implemented + wired); bb_xchr.c rewrite attempted and reverted (binary data_string writes bytes into SEG_CODE corrupting code). Gates: smoke 7/7, broker 49/49, template-byte-id 4/4. Closed: -d through -m + EM-TEMPLATE-PURITY partial (xposi/xfarb already pure). |
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: rung06 scan/any bug; rung13 conjunction-in-generator.** Sess 2026-05-11c (Claude Sonnet 4.6): loop_next ✅ `cf389ad7` (+19), assign-cat ✅ `f32e690e` (+2). Honest 226/37. |
