...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **BLOCKED on GOAL-CHUNKS + new GOAL** — IEP-5/6 owned by CHUNKS; IEP-7 also blocked on proc_table[].proc and g_pl_pred_table holding live tree_t* (not covered by CHUNKS — new GOAL needed to lower Icon procs + Prolog clauses into dcg_table). IEP-4 ✅ `d9dff43a`. IEP-8 (sm_prog_free frees dcg_table) can proceed. |
 | **⚡ SN4 JVM Emitter (IR_t-based, beauty self-host)** | `GOAL-SN4-JVM-EMIT.md` | one4all+.github | **SJ4-JVM-1 ✅ 2404f429 — all 19 IR_PAT_* BB template emitters; SJ4-JVM-2 ✅ f41fa90f — SnoRt.j + SnoRtMatchState.j; SJ4-JVM-3 ✅ sess 2026-05-15c — smoke 7/7 (Makefile + --sm-emit); SJ4-JVM-4 🔄 sess 2026-05-15f (Claude Sonnet 4.5, one4all `f40c034c`) — SM control flow (SM_JUMP/JUMP_S/JUMP_F via goto_w), comparison builtins (LE/LT/GE/GT/EQ/NE via builtin_numcmp), SUBSTR 2-arg + bounds, unknown-fn nargs pop, pop_obj empty-stack safety. Smoke 5/7 PASS (output, concat, arith, goto_s, arith_sm; FAIL: pattern, define). Loops work (LE-driven 1,2,3 correct). C smoke 7/7 no regression. NEXT: SM_PAT_* opcodes + IR generator dispatch (for beauty patterns), SM_DEFINE/SM_CALL_FN user function dispatch (for define test).** |
 ...
 | **SN4 .NET Emitter (IR_t-based, beauty self-host)** | `GOAL-SN4-NET-EMIT.md` | one4all+.github | **SN4-NET-1..4 ✅** — SnoRt.il, 19 BB emitters, SM walker, smoke 7/7. **SN4-NET-5 ⏳ partial** (sess 2026-05-15, Claude Opus 4.7, one4all `eb74b9a5`): beauty.il assembles + mono runs to completion exit 0; 58407-line .il, 628 stdout lines. Emitter fixes: MSIL `;`→`//` comment syntax; SM_JUMP/JUMP_S/JUMP_F now emit conditional br to NET_L{target} (were no-ops); SM_HALT branches to NET_DONE; has_continue tracking; added safe-default cases for SM_CALL_FN/RETURN family, SM_PAT_*, SM_BB_*, SM_DEFINE_ENTRY/DEFINE, SM_EXEC_STMT, frame/glocal ops. Runtime fixes: coerce_num/_obj_to_dbl use Double.TryParse (was Parse → crash); do_return sets last_ok per kind; sno_call dispatches 8 built-ins (SIZE/TRIM/DUPL/SUBSTR/IDENT/DIFFER/INTEGER/DATATYPE). Smoke 7/7 maintained. Beauty produces 1 non-blank line (oracle 622) — user-defined function dispatch + SM_PAT_* wiring still pending. NEXT: SM_DEFINE_ENTRY→name/pc table; SM_CALL_FN user-function lookup; call stack for return addresses; SM_PAT_* wiring to emitted pat_*_* classes. |
 | **SN4 WASM Emitter (IR_t-based, beauty self-host)** | `GOAL-SN4-WASM-EMIT.md` | one4all+.github | **NEXT: SN4-WASM-1** — sno_runtime.wat scalar runtime. |
