...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-PER-KIND-DIFF ✅ LANDED + GEOMETRY-CORRECTED (2026-05-21 evening, Opus 4.7, one4all `9b905d26`).**  Harness operational: `tools/emit_per_kind_audit.c` + `scripts/{freeze_per_kind_baseline,test_per_kind_diff,normalize_per_kind_cell}` + `baselines/per_kind/` (3.4 MB committed).  **5 backends** (x86, jvm, net, js, wasm) × sub-modes (x86 has text/binary/text_macro; JVM and .NET may grow binary later; JS and WASM text-only).  1059 cells per run (679 BB + 380 SM); current baseline PASS=399 STUB=660 FAIL=0 NEW=0 GONE=0.  Regression detection verified — inject canary → FAIL>0 exit=1; restore → FAIL=0 exit=0.  Filter-then-diff via `normalize_per_kind_cell.py`; also captures `as`-assembled .o md5s for x86/text cells.  x86/binary uses structural (byte-count) comparison since ASLR randomizes baked-in addresses every run.  **NEXT: EC-UNI-REFAITH** — coverage gate now exists; re-lift any kinds whose live emit_bb_x* output differs from lifted template output when wiring shifts.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 stable. |
