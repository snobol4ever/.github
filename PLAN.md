...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | LFJ-0..LFJ-1a-vi ✅. **LFJ-1b** ✅ (`e5eb34b0`: `lower_kind_table[TT_KIND_COUNT]` of `BB_t*(*)(BB_graph_t*,tree_t*)` introduced; `lower_kind_table_init()` populates 74 slots with `lower_icn_legacy_<KIND>` fns; dispatcher = init-on-first-use + bounds check + table lookup + indirect call; 60-line switch replaced. Legacy fns static + table-reachable → one-line revert. ZERO logic change). Next: **LFJ-1c** — create empty `lower_icn_new.c` + `lower_icn_new.h`, wire into build (no new fns yet). Then LFJ-2 transcribes `ir_a_NoOp` → flip first table slot. Gates: smoke 5/5, broker 24, rungs 198, smoke_prolog 5/5. |
