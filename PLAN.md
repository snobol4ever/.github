...
| **SNOBOL4 BB** | `GOAL-SNOBOL4-BB.md` | SBL-MODE3-REACTIVATE ✅. SBL-MODE-PURITY-1..5 ✅ (`e7e7bd63`). SBL-ANY-2-CORRECTNESS ✅ (`3eb09ba0`). **SBL-ANY-2 + SBL-ANY-2-DISPATCH-TRACE ✅** (Opus 4.7, 2026-05-27, continued 15, one4all `3b78f297`): 104-byte BINARY-arm fill in `bb_pat_any.cpp` mirroring TEXT arm byte-for-byte; pattern after `bb_lit.cpp` (`TEMPLATE_ADDR_SIGMA/SIGLEN`, `bytes()+u32le()+u64le()`, `bb_bin_t` 5-site list). Dispatch trace complete: BINARY arms exercised ONLY by mode-4 *runtime* path (compiled binary → `rt_match_variant` → `exec_stmt` → `bb_build_brokered` → `walk_bb_flat` → templates). Mode-2/-3 `--interp`/`--run` use C oracle (`bb_exec.c`); mode-4 emit phase uses TEXT arms only. Verified via `probe7.sno` runtime-PATND `PAT = ANY('cab'); S PAT :F(NOMATCH)` — mode-4 pre-fill prints `no match` (stub `jmp ω`), post-fill prints `matched` byte-identical to SPITBOL oracle. All five gates HOLD at watermark: GATE-1 13/13, GATE-2 28, GATE-3 175/280, GATE-4 238/280, M2=19 M4=15. probe1 (`S PAT . M` capture form) reveals SBL-XNME-XLATE — XNME wrapper falls through legacy `(BB_t *)pp` cast (XNME=23 collides with BB_ALT=23, NOT BB_PAT_ALT; `walk_bb_flat default:` emits fail-stub). Separate bug. NEXT: SBL-XNME-XLATE (extend translator gate to XNME/XFNME, carve probe1→rung 058), then SBL-NOTANY-2/SBL-SPAN-2/SBL-BREAK-2/SBL-ARBNO-3/SBL-CAP-2 BINARY arms (scope clarified to runtime-PATND mode-4 only). |
