...
| Prolog Frontend Ladder | `GOAL-LANG-PROLOG.md` | one4all+corpus | **PR-19a LANDED** → PR-19b (one4all `a4d03638`: v3 Term→EXPR bridge applied to pl_runtime.c catch/3 E_VAR arm, +192 lines. Gates: smoke 5/5, broker 41/49 preserved (Icon-failing 8 are pre-existing, bridge-neutral on Prolog axis), rung31_bridge_catch 0/5→**5/5**. SWI suite 43/57→17/57 expected and accepted — 43 was false-positive ceiling on plunit silent-success, 17 is honest baseline. Pre-existing throw-payload unify bug discovered separate from bridge — parked for PR-24. **Next: PR-19b** — rung32_bridge_negation/ for `\+ Var`, `once(Var)`, `not(Var)`; extend bridge ~10 lines per builtin mirroring catch/3 arm. Then PR-19c call/N, PR-19d setof/bagof/findall, PR-19e setup_call_cleanup. Ladder strategy proven: 2 weeks of stalled bridge work landed in 1 session by changing the gate definition.) |
...
| Snocone Beauty | `GOAL-SNOCONE-BEAUTY.md` | corpus+one4all | **SB-6 self-host proper** (see goal file — session 2026-05-02 #8 fixed pp/ss_leaf identical-condition dispatch in beauty.sc and recharacterized the bottleneck: upstream parse-reduction (`reduce('Stmt', 7)` not firing on label-only Stmt) + Gen-buffer never-flush, not pp dispatch; fingerprint unchanged `lines=89 parse_err=3` (oracle 646); three baseline gates green) |
...
| NET Beauty Self-Host | `GOAL-NET-BEAUTY-SELF.md` | snobol4dotnet | S-2-bridge-7-fullscan (see goal file — session #78 landed PM fire-points + Graft cache; first divergence at sync-step #2837 in snoProtKwd→snoUnprotKwd alternation; HEADs x64 dd66e14, snobol4dotnet 12bd3fa, one4all 1072fc61) |
