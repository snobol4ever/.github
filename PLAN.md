...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **REOPENED — NEXT: sub-rung -e (BB-axis: bb_xspnc; OR SM-axis: sm_push_lit_i after Lon A/B/C decision).**  Sub-rung -d ✅ sess 2026-05-11 (Claude Opus 4.7, one4all `f74db5bf`): first BB-box template — `templates/bb_xchr.c`, byte-for-byte lift of `flat_emit_lit` + inline banner from `bb_flat.c`'s XCHR case.  Key finding: BB boxes call same runtime symbols in mode-3 and mode-4, so BB-axis sub-rungs are NOT blocked on the A/B/C decision; SM-axis sub-rungs still block.  Byte-identity proven: `claws5.s` git stash/pop diff empty; --sm-run 197/64 == --jit-run 197/64.  Sub-rung -c PARTIAL (mode-3 wired; mode-4 blocked on A/B/C — see Goal file watermark).  Sub-rungs -b ✅ and -a ✅ earlier sess 2026-05-11.  Gates: smoke 7/7+49/49+5/5; jit crosscheck 197/197 preserved; template byte-identity 4/4 + literal-byte unit test PASS.  Mode-4 beauty PASS=0 FAIL=17 (pre-existing libscrip_rt link issue, unchanged across all rungs this session). |
