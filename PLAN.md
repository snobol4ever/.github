...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-5c.1 (session #73: SB-5c.0 LANDED — `&TRIM` keyword on INPUT now strips trailing spaces+tabs in `src/runtime/x86/snobol4.c::input_read()` and the channel-bound branch in `NV_GET_fn`, matching SPITBOL. Three baseline gates green: PASS=5, PASS=42 SKIP=3, PASS=49. Two more bugs identified, tracked as runtime fixes per Lon's "no SC workarounds" rule: SB-5c.1 — `ARBNO(ARBNO(*X) Y)` matcher bug (8-case bisection isolates the failing shape; SPITBOL handles it, scrip doesn't, all three modes). SB-5c.2/SB-5d.2 — `treebank-array.sc` Error 5 traces to bare-juxtaposition concat (`LT(i, n-1) i + 1`) — Snocone lacks SNOBOL4 space-concat; gates on `GOAL-SNOCONE-LANG-SPACE.md` (LS-0). claws5 also has independent input-file alignment issue (corpus `claws5.input` doesn't yield `claws5.ref` under SPITBOL — likely .ref derived from `CLAWS5inTASA.dat`). Active rung: SB-5c.1 — fix `ARBNO(ARBNO(*X) Y)` in `src/runtime/x86/snobol4_pattern.c`.) |
