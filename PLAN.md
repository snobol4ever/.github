...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | IJ-CSET-COMPL + IJ-CSET-BINOPS landed: TT_CSET_COMPL → IR_CSET_COMPL; TT_CSET_UNION/DIFF/INTER → IR_CSET_UNION/DIFF/INTER (shared case). Eliminates `[NO-AST] SM_BB_EVAL` stub for `~cset`/`++`/`--`/`**` in `--ir-run`. ir-run 111/265 unchanged (downstream scan_builtins gating on `\0`-safe lookups). Gates: smoke_icon 5/5, broker 20/49, honest 277/0/0, cross-lang smokes all green. NEXT: `\0`-safe scan builtins (memchr-based upto/many/any) — likely unlock for cset-complement jcon rungs. |
...
| **Prolog BB JCON triage** | `GOAL-PROLOG-BB-JCON.md` | one4all+corpus+.github | **PJ-9d design 📋** captured. Mode-4 Prolog predicate-registry emit needed so standalone binary's `g_dcg_table` is populated. Three options evaluated; **recommended Option C**: emit per-predicate "builder" assembly functions that reconstruct `IR_block_t` graphs via `IR_new_*` calls at startup, plus a `rt_register_predicates_pl(tbl)` call in `main` prologue. Starting points listed in goal-file PJ-9d row. **Implementation deferred to fresh session.** PJ-9c partial ✅ (one4all 0ffffa2e) wired Mode-4 dispatch for SM_BB_ONCE_PROC; PJ-9b ✅ (ef2f90e4) aligned stub fingerprints + crosscheck 4→128; PJ-9a ✅ (fc6fa0a8) wired Mode-3 JIT dispatch. All gates hold: smoke 5/5, crosscheck 128/0, honest_prolog 124/0/0, honest_icon 277/0/0, broker 20/49. |
...
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **DONE** PST-RB-5a-5d ✅. Rebus rewrite complete. |
| **⚡ PST: Prolog** | `GOAL-PST-PROLOG.md` | one4all+corpus+.github | **PST-PL-6e** — promote TRSlotMap; remove VarScope.next_slot; delete scope_get slot assignment. PST-PL-6a-6d ✅ 2026-05-16. |
