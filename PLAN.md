...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **GOAL COMPLETE** sess 2026-05-15 (Claude Sonnet 4.6). IEP-1..3 + vtable redesign + src/include/ layout + IR.h rename. one4all `1e7a7f5e`. One `emit_ir_block` entry all 6 backends; vtable dispatch inside single `ir_walk`; `src/include/` holds 6 stage-boundary headers (descr, ast, IR, sm_prog, bb_box, emit_ir); `scrip_ir.h` → `IR.h`; `src/backend/` removed. |
