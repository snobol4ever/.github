...
| **Prolog Frontend Ladder** | `GOAL-LANG-PROLOG.md` | one4all+corpus | **PR-13** (PR-19 bridge FULLY CLOSED this session — PR-19e setup_call_cleanup/3 LANDED. rung31–35 all 5/5. Next: rung36_arith_edge ISO §8) |
...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | **SB-6.E.7-J pass #3 — beauty.sc full rewrite remains the active blocker** (session 2026-05-02 #16: SB-6.E.7-M LANDED — sno prefix stripped from beauty.sno (67 names, 319 occurrences). Found one collision: snoSorF parser pattern vs SorF runtime variable; fixed via pre-rename .SorF → .sf. New oracle md5 `9cddff2534472b822438801d8db58a99` (622 lines, down from 646 because shorter identifiers fit on single lines). Three baseline gates green. SB-6 fingerprint: lines=603 stderr=4389 parse_err=170 internal_err=4 rc=124, 91 hunks (vs session #15 600/88, within noise — .sc port already used unprefixed names). Old md5 `abfd19a7a834484a96e824851caee159` retained in PLAN.md Milestone 1 historical record; other goal files (NET-BEAUTY-SELF, SCRIP-BOOTSTRAP, LANG-SNOBOL4, CORPUS-LAYOUT) still reference old md5 in their gate spec, will rebaseline when next exercised. corpus @ `5cc1baa`, .github @ `537945f`, one4all clean.) (session 2026-05-02 #15: pass #3 walks all 17 files line-by-line under body-part correspondence. Edits: stack.sc Push assignment-as-test, case.sc brace cleanup, counter.sc PushBegTag/PushEndTag assignment-as-test, ShiftReduce.sc Shift assignment-as-test, ReadWrite.sc Read inner-loop polarity bug fix (latent — would truncate >131K-byte lines), TDump.sc TValue :S(TValue3) routing bug fix (TValue on null-v(x) was returning instead of looping over children), global.sc full faithful rewrite using `&ALPHABET ? (POS(p) LEN(n) . name)` form removing all invented intermediates (`_alphabet_run`, `_utf_n`, `_nm`, `define_alphabet_run`). beauty.sc PARTIAL: bVisit→visit and Refs→snoRefs only. **Three baseline gates green throughout.** **Fingerprint moved big**: `lines=1495 stderr=8448 parse_err=418 internal_err=18` → `lines=600 stderr=4389 parse_err=169 internal_err=4` (~60-78% reduction); diff hunks vs oracle 260 → 88. rc=124 timeout supersedes rc=0 because runtime is no longer self-truncating via the prior rollback symptom — it's now doing real deep work. **Two new rungs surfaced**: **SB-6.E.7-L** (locals-as-parameters semantic bug — caller `fn(a,b,1,2,3)` against `function fn(x,y,p,q,r)` corrupts locals; need Andrew's `procedure name(args) locals { body }` syntax per /tmp/snocone_andrew/SNOCONE/snocone.sc:977; lower to `DEFINE('name(args)locals')`); **SB-6.E.7-M** (drop `sno` prefix in beauty.sno itself, not just .sc — Lon directive reverses pass #2 plan) — **SB-6.E.7-M closed session #16**. **Outstanding: beauty.sc full rewrite** — un-collapse pp_TYPE/ss_TYPE giant if-cascades back to per-type body parts, remove invented ppLeaf/ppList/ppStmt helpers (keep ppUnOp/ppBinOp), restore canonical main00..main05 body parts in main loop. ~150 lines structural change deferred. (sno-prefix strip from beauty.sc was part of the deferred work but is now mooted at the .sno side by SB-6.E.7-M; the .sc port already uses unprefixed names.) Active blocker for SB-6.E.7-J pass #3 completion.) SB-6.E.7-A closed. SB-6.E.7-H still downstream blocking after beauty.sc completes. |
