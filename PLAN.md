...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-1d ✅** 2026-05-17 (Opus 4.7). SCT-1/1b/1c/1d landed: lower_sno.c handles all six parsers with zero `?TT_` placeholders, line-continuation under SPITBOL's 1024-char cap, multi-file --dump-sno mode, `-CASE 0` prelude, dedup tail `:(RETURN)`, wrapper passes runtime prelude. `scrip --dump-sno <7 runtime files> parser_snobol4.sc > p.sno` (885 lines) parses **cleanly end-to-end on SPITBOL**, single runtime ERROR 041 at line 873 (downstream). All gates green: smoke 5/5, crosscheck 8/0, all-modes 2/0. one4all `f097651a`, corpus `c97eeb0`. **NEXT: SCT-1e** — triage the single ERROR 041 (`n(ptree)` on non-tree value); then SCT-1f drives 2-way sync-monitor. |
