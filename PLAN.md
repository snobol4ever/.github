...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.j (session 2026-05-01 #6 LANDED LS-4.i.5 at one4all `4a026191`: `struct NAME { f1, f2, ... }` → `DATA('NAME(f1,f2,...)')`.  Andrew's `.sc` line 162 record-decl form, lowered onto SPITBOL's built-in `DATA()` primitive which simultaneously installs the constructor and per-field accessors (each accessor doubling as L-value via SPITBOL's standard `fk(x) = newval` idiom).  Two new `matched_stmt` rules — fields and empty-fields forms — plus a left-recursive `struct_field_list` helper non-terminal mirroring `func_arglist_ne` shape verbatim (left-to-right comma-joined string accumulation `"f1,f2,f3"` in a single malloc'd buffer per reduction).  No `opt_head_sep` between T_RPAREN-equivalent and T_LBRACE — same as `func_head`/`switch_head`, FSM does NOT synthesize T_CONCAT before `{`.  No trailing semicolon — matches Andrew's `.sc` surface and every corpus user (`include-sc/stack.sc`, `tree.sc`, `counter.sc`; `snocone/demo/beauty/{stack,counter,tree}.sc`).  Adjacent structs without separators (`struct a{...} struct b{...}`) lex+parse cleanly because Bison's LR(1) lookahead at T_STRUCT after a closed matched_stmt unambiguously starts a new struct decl.  **Lowering: `sc_emit_struct(st, name, fields)` builds the QLIT spec `"NAME(f1,f2,...)"` (or `"NAME()"` for empty), constructs `E_FNC("DATA", E_QLIT(spec))`, appends as bare-expression statement.  Mirrors `sc_func_head_new`'s DEFINE-emission idiom but simpler — no goto, no label, no cur-context save/restore (struct is exactly one stmt, no body to splice).  End-to-end SPITBOL semantic check verified: `DATA('link(next,value)')` (LS-4.i.5 lowering target for `struct link { next, value }`) installs `link()`, `next()`, `value()` as functions; chained `link(...)` calls building a 3-element list traversed via `next(a)` walks correctly under `sbl -bf`, output `10\n20\n30\n` exactly.  Confirms `DATA()` is the right lowering target — the SPITBOL primitive Andrew's `struct` keyword is sugar for, not an invented translation.  Tests added (59–69, +38 assertions): headline 2-fields stack.sc shape, single-field, empty-fields zero-arity record, four-fields tree.sc verbatim, splice-in-sequence (struct between assignments — 3 stmts in order, splice-clean), back-to-back counter.sc shape (two adjacent struct decls without separators), underscore-in-name, mixed-case-bytes-preserved (RULES.md case-sensitive name space), struct-followed-by-constructor-use, explicit-trailing-semi regression guard (T_SEMICOLON-alone produces no stmt entry — total stays 2 stmts not 3), struct-no-name syntax error.  Combined parse-a..i delta from LS-4.i.5 close: **927/927 → 965/965** (+38).  All gates green: smoke snocone 5/5, beauty 3-mode 42/0/3, broker 49/0, parse-a..i 965/965.  Bison: 0 shift/reduce, 0 reduce/reduce.  Side-channel only — LS-4.j wires the new parser into scrip's production driver.  **This rung closes LS-4.i — every Andrew `.sc` surface construct (goto/label, break/continue, switch/case/default, alt-eval `(,,)`, struct) now has a Bison rule and an IR lowering.**  Next: LS-4.j — wire `snocone_parse_program()` into `snocone_compile()` as the production parse path.) |
