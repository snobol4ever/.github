...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`50a08f8b`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN. **RUNG THREE-SECTION-X86 (TSX)** — PURPOSE: DELETE every binary-byte-emitting function (insn_* family, bb_emit_byte sink, emit_seq_*, emit_call_sym_plt, emit_sym_lea_*). Raw bytes go INLINE as std::string data in each template's MEDIUM_BINARY arm. Method: DISCOVER → INLINE → DELETE → WIRE. **DISCOVER ✅ + TSX-INLINE ✅ COMPLETE.** **TSX-DELETE re-scoped into a propagation ladder (single-shot deletion proven impossible — bulk insn_* reachable only via TSX-WIRE-deferred reloc/PLT wrappers still live in templates).** **TSX-DEL-0 ✅ `bf915876`** (5 text-arm insn_* → s_*asm/emit_2asm; zero insn_* in any template). **TSX-DEL-1 ✅ `bf915876`** (deleted 9 orphaned insn_*, 34→25 defs). **TSX-DEL-2 ✅ `50a08f8b`** (3 dead macros + bb_emit_patch_rel8 + 6 ghost decls; layer drained). **NEXT: per-family rewire+delete **TSX-DEL-W-LIT → W-PORT → W-PL → DEL-FINAL** (incremental TSX-WIRE + byte-snapshot oracle each). ⛔ PENDING Lon directive: TEMPLATE PURE-PROJECTION. STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
