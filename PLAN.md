...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#5 LANDED (session 2026-05-05): pair-shape semantic instrumentation + bare names (no language prefix). Old one-letter-prefix iter#5 SUPERSEDED — Lon directive: SCRIP will support multi-program/modules so each `parser_*.sc` has its own global dictionary, no namespace-collision defense in `.sc` files. Canonical: every in-pattern semantic action is a lowercase-worker + Capitalized-companion pair (Form 1: `Push_qlit = epsilon . *push_qlit();` / Form 2: `function Push_var(varname) { Push_var = EVAL("epsilon . thx . *push_var('" varname "')"); return; }`). All `sc_`/`SC_`/`rb_`/`RB_`/`ic_`/`Rk_`/`rk_` prefixes stripped; rebus surface tags `RB_FUNC_DECL` → `FUNC_DECL` (wire-protocol `'rb_'` label literal stays); raku `*_done` → action verbs. Gates all 100%: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38. Next: operator picks iter#6 concept. |
