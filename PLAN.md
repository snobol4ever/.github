...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | LFJ ✅. Steps 9, 10a-1..10, **10b ✅ COMPLETE**, **10a-4 ✅ COMPLETE** (Opus 4.7, 2026-05-28). AG-pure migration functionally done: all 4 acceptance greps 0 (`bb_operand_aux_set`=0, `bb_exec_node(nd->[αβ])`=0, `icn_kind_owns_omega_operand` removed, rungs 198). 10b ring-peek landed for BB_ASSIGN (`d209c93e`) + BB_CALL deep-arg (`9fb1dbb8`); omega-operand predicate removed (`b55bc261`). 10a-4: streaming-gen `every` flat-wires via ival=2 (`09353f25`); **BB_SEQ_EXPR block bodies now flat-wire via a BB_SUCCEED reset-shim (ival=7), break/next honored (`295108a4`)** — also fixed a pre-existing break-in-body loss on the single-node path. ONLY remaining Every_ag exclusion: generator-bodies (`lic_is_gen_node(body)`). Gates: smoke_icon 5/5 · broker 36 · rungs 198 · prolog 5/5 · FACT 0. Verified modes 2+3. NEXT: generator-body Every_ag, or MODE3 (--run) BB_CALL/EVERY native parity, or another goal. |
