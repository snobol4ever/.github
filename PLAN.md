...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-3.f.2 (session 2026-04-30 #2: **LS-3.f.1 LANDED via threaded-code FSM, not flex.** Two new Lon directives during the LS-3.f.1 attempt forced an architectural pivot away from Flex+Bison entirely. Directive (a): comments `/* */` and `//...\n` must be embedded in `{W}` so `x  /* hi */ + y` lexes `IDENT ADDITION IDENT` (not `IDENT CONCAT ADDITION IDENT`). Directive (b): no `switch`/`for`/`while`/`do/until` anywhere in the lex body, pure FSM in label/[if]/action/goto column form. Folding comments into `{W}` proved infeasible — flex 2.6.4 hangs at 0% CPU / 2.7 MB RSS for >180s on the resulting `{W}OP{W}` × ~30 rules NFA→DFA pass; verified across `--full`, `-Cf`, `-CF`, `-Ca`, `-Cem`, `-Ce`, `-Cm` (none help). Considered alternatives: CMPILE-style 256-byte tables, free-form C, threaded-code FSM. Picked threaded-code FSM. `snocone.l` and `snocone.lex.c` deleted; `snocone_fsm.c` added (~740 lines). Single function `sc_lex_next(ctx)` returns one token per call. Body has zero `switch/for/while/do` (verified post-preprocessor — only matches are keyword-table strings `"while"/"do"/"for"/"switch"`, which are data not control). Three-column visual layout, separators between major states. Comments-as-whitespace handled by S_LCOMMENT and S_BCOMMENT/S_BC_STAR sub-FSMs that return to S_WS. LS-1.b 31-test acceptance harness 31/31 PASS. Production gates unchanged: `test_smoke_snocone.sh` 5/5, `test_beauty_snocone_all_modes.sh` 42/0/3, `test_smoke_unified_broker.sh` 49/0 (production link path still uses old hand-written `snocone_lex.c` until LS-4 wires in the new front-end). LS-3.f.3 also closed (gates green throughout). LS-3.f.2 next: move `test_snocone_lex2.c` from `src/frontend/snocone/` to `test/frontend/snocone/` and migrate the `snocone_lex2()` back-compat wrapper out of `snocone_fsm.c` into the harness. After LS-3.f.2 we proceed to LS-4: Bison grammar still planned for the parser, but it consumes `sc_lex_next` directly — no flex.) |
