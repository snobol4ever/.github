...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-bb-three-column PARTIAL 2026-05-09** — central paths converted: `emitter_text.c`, `bb_emit.c bb_insn_*`.  Three-column shape live for everything through these.  Per-op arithmetic macros (ADD_NUM/SUB_NUM/MUL_NUM/DIV_NUM/MOD_NUM); `.byte` hex paths replaced by `abort()`; blank lines eliminated.  Gates 10/10 GREEN.  **Next: EM-7c-stmt-banner-fidelity CARVED 2026-05-09** — every `# stmt N` banner must be followed by code that lowers from EXACTLY that stmt, in source order, with no naked `.LpcN:` ghost lines and no disembodied `# (baked into _pat_inv_<id> ...)` comments.  Audit found three failure modes in current artifacts: (1) blob-absorbed PCs leave comment-only lines under banner; (2) `.LpcN:` with empty col 2 for SM_LABEL/SM_STNO; (3) cross-banner code drift on deferred returns.  Done when: every line between two banners maps to the active stmt via `pc → stno` map; every `.LpcN:` has non-empty col 2; blob-absorbed lines emit `PAT_<op> baked  # _pat_inv_<id> line <L>` form; runtime check still output='abXc'; 10/10 gates GREEN.  Lands BEFORE EM-7c-s-file-beautify (carved earlier; correctness before cosmetics). |
