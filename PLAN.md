...
| SNOBOL4 Frontend Ladder | `GOAL-LANG-SNOBOL4.md` | one4all | SN-6 IN PROGRESS: PASS=218/228; Bug #1d RE-DIAGNOSED — prior bb_seq-rollback theory tested and falsified for 1+2*3 (rollback fires, output unchanged); real root cause: one4all invokes `*fn()` at match-time, SPITBOL defers to commit-time (manual p.133–134); stk side-effects of Push() leak past backtrack; no one4all commits this session, HEAD 6e98862f; smoke PASS=7 broker PASS=49 | ☐ |
| SNO treebank-array | `GOAL-SNO-TREEBANK-ARRAY.md` | one4all | TA-1 DONE (landed via C5 session HEAD 6ee09b7f); TA-2 next | ☐ |
| SNO treebank-list | `GOAL-SNO-TREEBANK-LIST.md` | one4all | TL-2 IN PROGRESS: case-sensitivity refactor landed (11 strcasecmp→strcmp in snobol4.c function registry — 6e98862f); gates green (smoke=7, broker=49); treebank-list before→after: empty → ('ROOT') (1/24 lines); remaining bug isolated to SM lowering (sm_lower.c:282/302 drops args; h_pat_capture_fn passes NULL) — *fn(var) callcap args need flush-time resolution to match oracle. Next: extend SM_PAT_CAPTURE_FN opcode to carry arg names. | ☐ |
...
| Snocone claws5.sc | `GOAL-SNOCONE-CLAWS5.md` | one4all+corpus | CL-2 next: prior CL-1 invalidated; two bugs uncovered: (A) SC-26 still open — `(PAT.var).*fn(var)` doesn't fire fn and doesn't commit var either; (B) Snocone `==` lowers to SNOBOL4 `EQ` (numeric) at snocone_lower.c:242, so strings both coerce to 0 and compare TRUE — fix B first, then re-diagnose A with reliable tests | ☐ |
