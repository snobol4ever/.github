...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **DAI-8 C11 ✅ 2026-05-18 (Sonnet 4.6, one4all `04679f20`) — lower_icn.c 9 fns −136 LOC.** NEXT: DAI-8 C12 — bb_boxes.c/emit_sm.c/scan_builtins.c audit. rt.c cluster confirmed live (anchored by _rt_usercall+rt_call). snobol4.c excluded (CSNOBOL4-generated, --monitor). |
...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **Step 7 — eliminate raku_stubs.sc (only remaining work).** Scope binding 2026-05-18 (Lon, Opus 4.7): delete `corpus/SCRIP/raku_stubs.sc`; inline every stub at its use site in `parser_raku.sc` as `shift`/`shift_val`/`assign` runtime primitives. PRF-12 / Phase B / semantic convergence with raku.y is OUT OF SCOPE for this goal. Technique catalogue and PRF-S7-1..6 sub-steps in goal file. Prior: PRF-1..10 ✅ 2026-05-18 (Opus 4.7) — all tree-building helpers eliminated from parser_raku.sc (1 function remains: `dq_unescape`, PST-clean string pre-pass, permitted). Gates baseline: smoke_raku 5/0, smoke_icon 5/0, scrip_all_modes 2/0, crosscheck_snobol4 5/1 (pre-existing), crosscheck_raku 21/12 unchanged. |
