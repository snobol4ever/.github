...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17 part 2: IJ-IDX + IJ-SECTION + IJ-LIMIT landed. one4all `d51fc518`. ir-run 157/265 (was 148). rung14 5/5 ✅ rung16 5/5 ✅ rung20 section 3/3 ✅ (+1 bonus rung23_insert_delete). broker 19→20. honest 280/281. NEXT: rung13/23/35 table cluster (10 fails); rung22 lists (5); rung24 records (5); rung31 sort (5); then rung36 jcon survey. |
...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-1 PARTIAL** 🔄 2026-05-17 (Opus 4.7). SCT-0a ✅ (reading + SPITBOL manual map). SCT-1 partial: `src/lower/lower_sno.{c,h}` + `scrip --dump-sno` + `scripts/run_parser_sync_monitor.sh` landed. `parser_snobol4.sc` transpiles to 126 lines of SNOBOL4 that parse cleanly and begin executing; fails at stmt 63 (Error 5) on unhandled TT_IF/TT_WHILE placeholders. NEXT: SCT-1b (statement-position TT_IF/TT_WHILE/TT_RETURN lowering, ~30 lines each per ARCH-SNOCONE.md "Lowering map"). Constraint: only six runtime primitives allowed in parsers (`shift`, `reduce`, `nPush`, `nInc`, `nTop`, `nPop`); no helpers_*.sc. |
