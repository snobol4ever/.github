...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **⛔ INLINE-4a-LABELFIX BLOCKS ALL — fix first.** Slices 19-24 inlined to bare emit_textf (pat_lit/refname/usercall/capture*/exec_stmt/return/freturn/nreturn @ slice 19-22 `2187693e`; PUSH_STR/PUSH_VAR/STORE_VAR @ slice 23 `ef06a3e8`; DEFINE/DEFINE_ENTRY @ slice 24 `65c5713f`). HEAD=`65c5713f`, GATE-PK 420/0/639. **BUT slices 23/24 (likely 19-22) carry a jump-target LABEL-DISPLACEMENT bug GATE-PK cannot see** — bare emit_textf doesn't flush bb3c's buffered `.L<pc>:` label, sliding it onto the wrong instruction. Diagnosis COMPLETE + golden-diff harness ready (see HQ Watermark CRITICAL BUG block). Fix = emit_pc_label_flush() at top of every inlined X86 arm; verify via /tmp/semdiff.py vs slice-18 `8aa204c2` until 0/153. **DO NOT inline more templates until 0/153 + committed as INLINE-4a-LABELFIX.** Then resume remaining ~8 (sm_call_fn/suspend done in-progress but reverted; sm_bb_once_proc/pump_proc, push_expression/call_expression, sm_stno). |
...
| **⚡ Parser-SC Transpile** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-SN4-ERR041 ✅ CLOSED** (2026-05-21d, Opus 4.7) — deleted 2 stray `nInc()` before `*StmtRepl` in parser_snobol4.sc `Stmt`; multi-stmt fixtures now parse. **PASS=49→64/88.** smoke 7/0, crosscheck 5/1 (beauty_omega pre-existing). 27-fact assumption grid verified vs SPITBOL Manual v3.7 + added to PRIMER. **NEXT: SCT-SN4-IMPLICIT-MATCH** — remaining 24 PARSE errors are implicit pattern-match-by-juxtaposition (`S 'a' \| 'b'` with no `?`); add an implicit-match branch to `Stmt`'s inner FENCE. Also still open: SCT-BEAUTY-SC-PARSE (beauty.sc `shift()` EVAL-global-scope bug — Option A/B awaiting Lon). |
