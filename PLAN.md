...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`d0ab64e2`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. **RUNG THREE-SECTION-X86 (TSX)** — PURPOSE: DELETE every binary-byte-emitting function (insn_* ×52, bb_emit_byte sink, emit_seq_*, emit_call_sym_plt, emit_sym_lea_*). Raw bytes go INLINE as std::string data in each template's MEDIUM_BINARY arm (bytes = data, like text chars in TEXT arm). Binary is currently DETOURED around templates (emit_sm.c forces TEXT_MODE; bytes from emit_bb.c+insn_*). Method: DISCOVER (capture_bytes scaffold) → INLINE raw bytes → DELETE functions → WIRE detour open. TSX-0 ✅ audit script. TSX-SM-1 ✅ (`734031de`+`d0ab64e2`): capture_bytes infra + sm_halt/calls/pat_anchors/pat_nullary three-section. **NEXT: TSX-SM-2** (sm_exec_bb). STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
