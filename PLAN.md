...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-4-pre — Register layout study + design (DRAFT COMPLETE — REVISED through Lon Q&A, pending Lon sign-off).**  Sess 2026-05-10 (Claude latest): `REGISTER-LAYOUT.md` rewritten at `.github` HEAD with refined locked decisions — **r12 = SM value-stack TOS pointer (FORTH-style single register)**, not SM_State*; r10 = current BB DATA-block pointer (constant inside a BLOB); rbp = DEFINE'd function frame when active; rbx/r13/r14/r15 = FREE (no pre-reservation).  SM_State accessed via RIP-relative addressing.  Stack push: `mov [r12], rax ; mov [r12+8], rdx ; add r12, 16`.  Pop: `sub r12, 16 ; mov rax, [r12] ; mov rdx, [r12+8]`.  Multi-pop: `sub r12, N*16` (N compile-time-known per opcode).  SM_STNO blob: `mov r12, [rel sm_state_stack_base]` — single instruction reset of TOS, handles realloc.  **Pattern history stack does not exist in SCRIP** — Byrd-boxes are stackless; per-invocation DATA blocks ARE the alternatives; backtracking is DATA-tree traversal.  Two stacks total (SM value + native rsp) + heap-resident BB DATA-block tree + heap-resident SmCallFrame array.  `*P` (P=pattern) re-entry: `rt_alloc_blob_data` allocates fresh DATA tree per match; CODE reused.  `*(expr)` DEFER box: SM_CALL_EXPRESSION distinguishes BB-land entry (saves `caller_r10` in SmCallFrame) from SM-blob-land entry (no r10).  ME-4-pre closes on Lon's sign-off line in REGISTER-LAYOUT.md's watermark; until then ME-4-post stays blocked.  Companion ARCH-x86 updates (HEAD `a49e099`, `ae5a069`): "Boxes are stackless" (CODE shared, DATA per-invocation, sibling DATA chained via save-list); flat-BB ABI (no esi, no port discriminator) vs dispatched-BB ABI (legacy bb_boxes.s with esi=port, preserved for `--bb-brokered`); intra-/extra-BLOB jump rules (intra and extra-tail need no r10 save; only call-style extra-BLOB needs source-BLOB `push r10`/`pop r10`).  Code state: one4all `e7ac6f77` — variable-size blobs + ME-4 inline-native emitters + rt.c pluggable backend landed; gates clean (smoke 7/7, unified_broker 49/49; 100k counter loop ~32% faster than `--sm-run`); 🐛 multi-statement arith reproducer aborts on `realloc(): invalid next size` (fixed by ME-4-post's SM_STNO TOS-reset blob).  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`. |
...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **A2 — CH-17i-section** (`AST_SECTION` + `AST_SECTION_MINUS` + `AST_SECTION_PLUS`). A0 ✅ 2026-05-10 (one4all `adb289b9`): `SCRIP_NO_AST_WALK=1` tripwire landed. A1+A4-pulled-forward ✅ 2026-05-10 (one4all `75b57a32`): `SM_BB_PUMP_AST` unified opcode + `g_ast_pump_active` re-entrant suppression counter; `AST_BANG_BINARY`, `AST_LCONCAT` (gen path), `AST_ITERATE` all migrated off legacy fallthrough. `every write(!s)` passes honestly under `SCRIP_NO_AST_WALK=1`. Gates: smoke 5/7/5/5/5/4, broker 49/49, isolation PASS, ir-run 177/56/30. |
