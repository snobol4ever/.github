...
| NET Beauty Self-Host | `GOAL-NET-BEAUTY-SELF.md` | snobol4dotnet | S-2-bridge-7-betastack-failure-leak / S-2-bridge-7-fullscan (session #71, 2026-04-30 — `Executive.PatternMatch` (the `?` operator) leaked `Failure=true` from deferred-assignment side effects in the BetaStack commit loop. Bug found via C# function tracing between last-agreed and first-diverged events: `[QPM] MR.Outcome=SUCCESS BetaStack.Count=14` followed by `BetaStack[13] Assign returned, Failure=True` and `EXIT Failure=True` — the match succeeded but Failure leaked, statement engine took :F(mainErr1). Fix: `Failure = false;` after the BetaStack iteration. snobol4dotnet `92b79be`. Beauty self-host: 28→47 stderr lines (line 26 `&FULLSCAN = 1` → line 48 `snoDQ = '"' BREAK('"' nl) '"'`); 23 more lines of beauty.sno now parse. Unit suite 2385p/0f/2s (ex-Csnobol4); Beauty 17/17 corpus PASS. Diagnostic patches (MonitorIpc `_standaloneCount`/`StandaloneInitEnvVars`, Scanner/ScannerState `[PM]`/`[M]`/`[ALT]` traces) reverted before commit per RULES.md.) |
