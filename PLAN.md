...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-4 (PARTIAL — emergency handoff sess 2026-05-10).**  Inline-native blobs for SM_ADD/SUB/MUL/DIV/MOD/CONCAT/COERCE_NUM/PUSH_NULL/PUSH_VAR/STORE_VAR landed.  Variable-size blob architecture (g_blob_addrs[] dispatch, no fixed stride) landed.  rt.c pluggable vstack/last_ok backend landed.  Gates clean: smoke 7/7, unified_broker 49/49; 100k counter loop ~32% faster than --sm-run.  **🐛 BUG live:** multi-statement arith programs abort with `realloc(): invalid next size` — mode-3 missing inter-statement sp reset that mode-2 enforces (sm_interp.c:295); ME-4's inline PUSH_NULL/PUSH_VAR have no bounds check.  Three fix candidates listed in Goal-file addendum (Option (a) emit per-statement sp-reset at SM_STNO is the architecturally honest one).  **Also a regret note: register-layout redesign was skipped this session — a "ME-4b" rung is recommended to lock r13=stack-base, r14/r15=locals/heap-frame before further inline blobs land.**  ME-1 ✅ 2026-05-10 one4all `cc3cd475` (pat-stack unification).  ME-2 ✅ 2026-05-10 one4all `babf76be` (`r12 = SM_State*` reservation).  ME-3 ✅ 2026-05-10 one4all `aca47e6c` (per-instruction native blob emitter).  ME-4 in flight — see GOAL-MODE3-EMIT.md addendum + this session's commit. |
