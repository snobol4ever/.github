...
| Prolog Frontend Ladder | `GOAL-LANG-PROLOG.md` | one4all | PL-12 (session 2026-04-30 #2: 75% baseline preserved; pl_invoke_term attempt regressed 43→10 due to Term→EXPR arith bridge gap, NOT committed; saved docs + revised next-session plan; previous fix #1 hypothesis (anon-var post-assertz) confirmed wrong via DBG_ASSERTZ trace — lowering already produces distinct slots; new latent directive-binding bug discovered (`:- assertz(test_g(hello)), test_g(G), write(G)` prints `_G0`); next session ordered tasks: (1) fix #3 plunit throw-vs-succeed (corpus-only, +1), (2) directive-binding bug in interp.c:4445 LANG_PL STMT handler (+1+), (3) pl_invoke_term with full Term→EXPR bridge for builtin arith (+3-4)) |
...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SC-INPUT-EOF (session #76: SB-5c.4 LANDED — `claws5.sc::pp_mem` re-ported 1-for-1 against canonical `claws5.sno::pp_mem` with zero gotos. All 19 canonical DEFINE() locals present (`ssk, si, sentno, wsk, wi, wkey, wq, wrd, tsk, ti, tag, tv, tline, pfx, pad, next_wkey, last_sent, lline, ns`); each canonical SNOBOL4 label annotated as comment at corresponding .sc location. Translation pattern: `:F(label)` exit at iteration top → while-condition peek-ahead `while (DIFFER(arr[i + 1, 1]))`; `:F(label)` skip-block → if/else; `:S(label)` loop-back at iteration bottom → implicit; conditional-value-assign predicate (`pfx = EQ(si, 1) ...`, `last_sent = IDENT(si, ns) 1`) → `if (pred) lhs = rhs;`. Statement-form match-and-replace `wrd ? ARB "'" = ''` works inside `if (...)` in Snocone (verified: performs replacement AND yields success/failure for the test). All canonical string literals byte-identical including trailing-space scrutiny (`': {'`, `': '`, `', '`, `"': "`, `'}}'`, `'},'`, `'_CRD :_PUN'`). `claws5.sc` goto count = 0 (was 6). **Byte-identical output to SPITBOL canonical pp_mem on a controlled fixture** — md5 `557d5df30ba52a698d1a5f3ca569d9b4` for both, on a 2-sentence fixture exercising `'`-containing words (double-quote branch), single/multi-tag words, sentence-1-prefix-`{1: {` vs sentence-2-prefix-` 2: {`, last-sentence terminator `}}`, mid-sentence terminator `},`, mid-word terminator `,`. Three baseline gates green (PASS=5, PASS=42 SKIP=3, PASS=49); crosscheck snobol4 PASS=6, snocone PASS=8 unchanged. **&TRIM verified** — both runtimes default `&TRIM=1` and strip trailing space+tab on INPUT byte-equivalently (session #73 SB-5c.0 fix live and correct). **Pre-existing slurp-loop hang documented** — `./scrip --ir-run claws5.sc < claws5.input` still hangs because Snocone's INPUT-after-EOF returns the previous line forever, so `while (DIFFER(line))` never terminates. Reproduced cleanly with a 7-line standalone test on a 3-line input (after EOF, INPUT forever returns last line). Same SC-INPUT-EOF bug session #68's SB-5b worked around in beauty.sc via `IDENT(Line, PrevLine)` no-advance detection. Independent of pp_mem; new active rung **SC-INPUT-EOF** opened. Fix surface: `src/runtime/x86/snobol4.c`'s `input_read()` / NV_GET_fn channel path — INPUT-after-EOF must return `''` and signal stream exhausted so `DIFFER(line)` becomes false. Complements session #73 SB-5c.0 &TRIM fix on the same code path. SB-5c.2 (claws5 input alignment, corpus-curation), SB-5d.2 (treebank-array.sc blocked on LS-0), SB-5d.4 (zero-goto invariant) still open. Files touched: `corpus/programs/snobol4/demo/claws5.sc` only (+99/-30 net). No runtime/frontend source changes.) |
