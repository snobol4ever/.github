...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-INLINE-GROUP active** (Lon directive session #N+2: grouped templates — one fn per opcode-GROUP, opcode via `g_emit.instr->op` switch; supersedes pure one-fn-per-opcode INLINE-ALL). 3 grouped templates landed: `sm_arith` (5), `sm_compare` (2), `sm_pat_nullary` (22). 29 opcodes in 3 fns. one4all `6a806546`. GATE-PK PASS=420 FAIL=0 STUB=639 throughout. NEXT: continue grouping (sm_jump_group, sm_misc_nullary, sm_incr_decr ready; sm_pat_string_arg + sm_pat_capture blocked on INLINE-4 machinery promote). |
