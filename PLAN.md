...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all | EM-FORMAT-BB-FUSED-GOTOS ✅ 2026-05-09 — adjacent cond+uncond jmp pairs fuse to `<cond>  <succ> ; jmp <fail>` (24 fusions across tracked corpora). Next: **EM-FORMAT-BB-LAW** — realize the original three-column LAW per `archive/BB-GEN-X86-TEXT.md` (col-3 fixed for goto; bare label+jmp pushed to col-3; triple fusion of action+cond+uncond). Then **EM-FORMAT-BB-DATA-CONSOLIDATE**, then beauty.sno crosscheck **EM-7d**. (TRAMPOLINE-ELIM is a future optimizer pass, not part of the EM-FORMAT-BB sequence.) |
