...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | **MAIN-BOOT ✅** (Opus 4.7, 2026-05-28, `80d0d5ee`): recovered Icon from 0/5 smoke + 1/269 rungs (post-`08e05f68` SNOBOL4 whack of SM_BB_PUMP_PROC) to **5/5 smoke + 161/269 rungs**, zero regressions. Four fixes: (a) main bootstrap as ordinary `SM_CALL_FN "main"`+VOID_POP at lower epilogue (per `08e05f68` handoff guidance); (b) every-loop `exit_pc` moved off `SM_BB_INVOKE.a[0]` (which is the α/β state flag — silent slot collision masked for years by PUMP_PROC bypass) onto `a[2]`, with matching FAIL-jump in interp + template; (c) `lower_return` Icon arm aligned with Raku (no VOID_POP — retval reaches `sm_eval_subexpr`'s nested-stack read); (d) `every E` with no SM_BB_INVOKE in gen_expr AND no body lowers as one-shot (was infinite-looping on non-gen E like `every write(sum_to(5))`; previously masked by `icn_bb_oneshot`). Goal file pruned 183→122 lines. Gates: smoke_icon 5/5 · smoke_prolog 5/5 · broker 35 · rungs 161 · FACT 0. **NOT-DONE for future sessions:** the 72 remaining rung FAILs are mostly generator-proc semantics on the SM_CALL_FN path (rung03 suspend_gen family etc.) — `sm_eval_subexpr` is a one-shot run with no `g_current_generator_state` so SM_SUSPEND silently fails. Needs GeneratorState wiring on the SM call side, or a distinct route for generator procs. Pre-MAIN-BOOT watermark `64ca51b7` had 199 rungs; 38 of those gen-proc rungs depended on `icn_bb_pump_proc_by_name`'s GeneratorState path, which PUMP_PROC has removed. |
