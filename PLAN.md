...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: re-apply lower.c DEFINE_ENTRY labtab fix (sess 2026-05-12d) + diagnose BB-blob R10 corruption at bb_broker:44. TC-UNSPLIT-1/2/4/6/7/8 + CLOSE re-amend ✅ sess 2026-05-12g (Claude Opus 4.7): extended unsplit to remaining 6 split groups Lon corrected as wrong verdicts in original audit. All 11 TC-SPLITs now superseded. 43 per-op files deleted; bundles sm_arith.c, sm_rtcall.c, sm_jump.c, sm_return.c, sm_exec_stmt.c, sm_pat_rtcall.c are source of truth. File count 89→46. Baseline preserved (smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty-subsystems mode4 PASS=6 FAIL=11). one4all `802c88ee`.** |
