...
| **⚡ Icon BB Native (flat BB templates)** | `GOAL-ICON-BB-NATIVE.md` | one4all+.github | **NEXT: IB-10 part 2 (is_suspendable() flip — deferred, needs design call). IB-10 part 1 ✅ sess 2026-05-12 (Claude Opus 4.7, one4all `1b13cc6d`): SM coroutine emission purged from `lower_every` / `lower_limit_every`; `lower_alternate_gen` deleted; -164 lines from lower.c; all Icon `every`/`every (E\N)` route through SM_BB_PUMP_EVERY now. Gates: smoke_icon 5/5, broker 22/49 (unchanged post-PB-8), ir_all 180 (unchanged), honest 210 PASS / 2 segfault (+2/-2 vs 7be3c8e0). BB tally: 8 of 43 JCON templates landed (ToBy, iterate, Alt, Every, Limitation, bang-Binop, lconcat, Mutual-seq); 35 remain on SM_BB_PUMP_AST. Also: added `.github/jcon_irgen.icn` (canonical reference).** |
