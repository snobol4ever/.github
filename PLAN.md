...
| Goal | File | Repo | Step |
|------|------|------|------|
| SNOBOL4 Frontend Ladder | `GOAL-LANG-SNOBOL4.md` | one4all | SN-32b-beauty-ipc (session #59 IPC harness on `beauty.sno < beauty.sno` advances from step 2023 → 2552 under both `--sm-run` and `--jit-run` after fixing `lower_pat_expr E_DEFER` bare-`*fn(args)` lowering: the all-E_VAR fast path was packing names into `ins->a[2].s` but `SM_PAT_USERCALL` handler was discarding them via `pat_user_call(fname, NULL, 0)`, so `*upr(tx)` was invoked with zero args at match time — fix: always take args-on-stack path when `nchildren>0`, defer non-E_QLIT args via `SM_PUSH_EXPR`/DT_E (mirrors IR-side -t fix at interp.c:4188-4208); also extended the same defer-non-E_QLIT rule to `E_CAPT_COND_ASGN` and `E_CAPT_IMMED_ASGN` args-on-stack paths for parity; new divergence at step 2552 is `VALUE sno = UNKNOWN` (spl) vs `VALUE sno = DATA` (scr) — pattern/data block type discrimination, same shape as latent SN-26-bridge-coverage-r) |
| CSN FENCE Bug Fix | `GOAL-CSN-FENCE-FIX.md` | csnobol4 | F-2 Step 3a (session #47: cpypat v=4 + FNCA PDL-rewind landed; SIGSEGV→Error 17, fence_function 10/10 preserved; remaining bug = inner P corrupted during FNCA's recursive SCIN, surfaces as PATBRA fall-through to INTR13) |
