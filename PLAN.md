...
| **⚡ Icon BB Native (flat BB templates)** | `GOAL-ICON-BB-NATIVE.md` | one4all+.github | **✅ GOAL DONE on IB ladder. IB-10 fully resolved sess 2026-05-12 (Claude Opus 4.7). Part 1 ✅ `1b13cc6d`: SM coroutine emission purged from lower_every/lower_limit_every; lower_alternate_gen deleted; -164 lines. Part 2 ✅ no-op: is_suspendable() audit found the as-written invariant unsound (the 8 migrated kinds remain semantic generators regardless of backend; the function is asked the semantic question by 12 sites in coro_runtime.c that all still need YES). BB tally: 8 of 43 JCON templates landed; 35 remain on SM_BB_PUMP_AST. Gates: smoke_icon 5/5, broker 22/49, ir_all 180, honest 210 PASS / 2 segfault (+2/-2 vs 7be3c8e0). NEXT: investigate the 2 GATE-4 segfaults (rung24_records_two_types, rung36_jcon_gener) OR open new IB ladder for next BB cluster.** |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: ⛔ Lon directive sess 2026-05-12h (handoff): finish ALL SM and BB C templates (EM-TEMPLATE-COMPLETE rung) BEFORE attempting any beauty-suite or beauty self-host work. Walk the SM_t and XKIND_t enums; grep for `emit_sm_<x>`/`emit_bb_<x>` in sm_templates.c/bb_templates.c; add missing ones; register in g_sm_templates[]. AFTER that closes, return to the 9 mode-4 diff-fails + XDump link-fail. Sess 2026-05-12h (Claude Opus 4.7, one4all `baa29424`): (1) BB-blob R10 corruption FIXED — flat_emit_box_call (bb_flat.c) + flat_box_call + flat_box_call_slot + t_bb_port_call (bb_emit.c) all now push/pop r10 around runtime calls; new BB_INSN_PUSH_R10/POP_R10 opcodes added to emitter enum + binary + text emitters. case_driver now passes mode-4. PASS 6→7. (2) lower.c DEFINE_ENTRY labtab fix re-applied (internal :(fname) gotos skip prologue). (3) Template consolidation: 78 per-opcode template files → 2 files (sm_templates.c, bb_templates.c), templates/ subdir deleted, sm_helpers.h deleted (static-in-file forward decls). Gates: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty-subsystems mode4 PASS=7 FAIL=10.** |
...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **SI-11 ✅ sess 2026-05-12c (Claude Opus 4.7): hand-built AST exercises SM_ACOMP TT_LT + SM_LCOMP TT_LGT with :S(label) forward jumps. Cross-check 6/7 byte-identical (si_07_pat_lit FAIL pre-existing). Unbundled fix in lower.sc (`SL-13c-tilde-fix`): `~LT(res, 0)` and `~IDENT(...)` are NOT boolean negation in Snocone — `~` builds a PATTERN value, always truthy in `if`. Two of three sites fixed to `GE(...)`; line 1054 occurrence left as benign (redundant trailing SM_HALT). Forward-reference jumps via .sc-side `lower()` now patch correctly. NEXT: SI-12 — SM_CALL_FN via host APPLY().** Earlier: SI-7..SI-10 ✅ sess 2026-05-12 (Claude Opus 4.7). |
