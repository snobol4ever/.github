...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | PRF-1..7 ✅. **NEXT: PRF-8** (`finish_given`), then PRF-9 (`finish_gather_body`). |
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | Bug #1 ✅ (`bad0fffd`). **NEXT: Bug #2** — mode-1 interp_exec missing SUBJ-PAT split; beauty hangs in icase. ~30 lines sketched. |
