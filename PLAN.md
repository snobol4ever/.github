...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17f (Opus 4.7): **PIVOT — IJ-DEL-ICN-AST surgery.** **DAI-1 ✅** (`b3e08fe8`): removed dead-on-vine `IR_ICN_EVERY` + `lower_icn_every`. **DAI-2 ✅** (`3e1fc9cd`): `bb_eval_value`/`bb_exec_stmt`/`icn_bb_build` bodies → `[DAI-BOMB]` stubs (−1641 LOC). **DAI-3 ✅** (`a2365c5f`): disabled `ICN_BB_EVAL` shortcut in lower.c; Icon expressions now lower through standard SM. **`--sm-run` measured 194/265 = `--ir-run` 194/265 — mode-2 at parity with mode-1.** All smoke gates hold floor; the 36 remaining fails are real IR/BB/SM gaps, not mode-2 immaturity. DAI-4..6 deferred (cosmetic). |
