...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | **iter#10 LANDED** (corpus@408fbe0, session 2026-05-04 cont.): arith Expr-tier grammar made iterative left-recursive (Expr_n = *Expr_{n+1} ARBNO(Expr_n_tail) with new shared `foldop()`/`FoldOp()` flatten-or-binary worker in semantic.sc + ShiftReduce.sc).  Removes the post-parse flatten layer in rw_expr; brings parser_snobol4.sc into shape-uniformity with the C frontends' expr_binary_flatten().  Fixes latent same-tier mixed-op bug (1+2-3); 3 new fixtures lock it in. Gates: SN=62/62 ✅ IC=88 PR=54 RK=31/32 (1 fail: arith_chain — iter#9 RK Phase D pending) SC=46 RB=38/38 ✅ (RB iter#9 Phase D closed by sibling session 09d7f80). Iter#9 itself: Phases A+B+C committed at one4all@4393ce1e; SN regression fix at corpus@61d825d; iter#10 supersedes Phase D for SN. **Next: cross-pollinate iter#10 foldop pattern to parser_icon.sc / parser_prolog.sc / parser_raku.sc / parser_snocone.sc (all share the latent same-tier-mixed-op bug today, gate corpora don't exercise it).  Then iter#9 RK Phase D arith_chain closes RK to 32/32.  Then iter#9 Phase E FENCE sweep second pass.  Cleanup CR-1..CR-5 (Rebus RExpr → EXPR_t double layer) is now lower priority — RB iter#9 Phase D closed without it.** |
