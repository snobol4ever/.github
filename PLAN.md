...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **TEMPLATES FULLY PURE 2026-05-25** — NB-3f ✅ + NB-3g ✅ (`7088c76a`): `xa_flat` + `xa_bb_macro_library` bodies now return only `(std::string, bb_bin_t)` with ZERO side effects; `g_emit_pos`/data-buffer flush+reset lifted to driver `codegen_flat_body`; macro-lib file-write moved to wrapper. BRIDGE RESOLVED: `bb_emit_asm_result`+`bb_emit_buf`/`pos`/`size` are permanent relocation infra (rel32 back-patch), NOT buffer-family — they STAY. GATE-PK 504/0/625, AUDIT GREEN, prolog 124/0/0, smoke 188/190-71, flat text byte-identical. NEXT: **pure-body sweep** — `bb_lit` `g_emit_pos+=7` lift; charset `g_flat_node_id++` (LP-4); `xa_bb_ptr_slot` (LP-5). ⛔ Beauty gate SUSPENDED. |
