...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-9 partial 🔄** 2026-05-17 (Sonnet 4.6). SPITBOL lazy-ARBNO root cause diagnosed + fixed: added POS(0)/RPOS(0) sentinels to all 6 Compilands. snobol4+snocone+rebus produce correct AST from SPITBOL. 224 parser .ref files renamed AST_*→TT_*. corpus `b64f6f1`. icon/raku/prolog blocked on SCT-4/5/6 helper deps. Smoke gates: snobol4 6/1, snocone 5/0. **NEXT:** SCT-9 remainder (wire test script, cross-validate vs --dump-ast), SCT-4 (icon_helpers.sc), SCT-1f (SN-26-spl-bridge). |
