...
| **BB Template Ladder — Icon + Prolog** | `GOAL-BB-TEMPLATE-LADDER.md` / `GOAL-ICON-BB.md` | one4all+corpus+.github | GOLDEN BB RULE: BB_t≡JCON ir_*; sval/ival/dval=payload. Phase H (Attribute Grammar): γ/ω inherited, α/β synthesized. **BUILD RED** at bb_exec.c BB_LIMIT (one4all `309274c2`). bb_exec.c c[] 294→130, n→37. H-2 done (BB_SEQ γ/ω-chain); 21 executor kinds port-migrated (SEQ/SEQ_EXPR/CALL/ASSIGN/SWAP/INITIAL/RETURN/IF/EVERY/WHILE/UNTIL/REPEAT/NOT/NONNULL/IDENTICAL/NULL_TEST/RANDOM/NEG/POS/CSET_COMPL). Fixed latent CALL arg-drop bug (args 3+). **NEXT: generator cluster** (BB_LIMIT first) needs per-node GC aux-state struct (opaque/ival2 collide w/ counter) — see GOAL-ICON-BB watermark option (b). Then emit_bb.c(20)/emit_sm.c(17). NO GATES RUN (build red). BB_IF else-on-γ is leaf/last-stmt only — full fix needs H-1 inherited-γ/ω threading. |
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | PJ ✅ COMPLETE. SB-LINEAR ✅ landed 916d61a5 — sm_emit_linear iterates SM, emits linear x86 blobs, falls through, relocates, calls. SM+BB freed before run. **BLOCKED: G-1 bb_exec.c must compile to link.** NEXT: fix bb_exec.c G-1, run smoke_snobol4 --run gate. |
