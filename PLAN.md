...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | **IBB-1 ✅ closed** (Sonnet 4.6, 2026-05-28, one4all `9ccf95e1`). Two-op boot landed: `SCRIP_ICN_BB=1 ./scrip --dump-sm /tmp/hello.icn` shows EXACTLY `SM_BB_INVOKE` + `SM_HALT` (the bogus `SM_BB_RUN_THE_DAMN_ICON` opcode was removed; `SM_BB_INVOKE` is the canonical right opcode). Hook in `lower_proc_skeletons` builds the BB graph via `lower_icn_proc_body` and registers it, then emits ZERO SM ops for the proc skeleton; top-level `has_icn` block emits `SM_BB_INVOKE <main_bb_idx>` + `SM_HALT`. **GOAL RULE added**: only `SM_BB_INVOKE` and `SM_HALT` may appear in an Icon SM stream. Gates: smoke_icon 5/5, smoke_prolog 5/5, broker 36/17, FACT 0. **NEXT: IBB-3** — `write(1 + 2)` mode 2; needs `TT_ADD`/`TT_INTLIT` in the BB lower path. |
