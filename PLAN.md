...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **NEXT: EC-UNI-12** — mechanical sweep of `fprintf(g_emit.out, ...)` → `emit_textf(...)` across all current SM/BB templates; `fputc`/`fwrite` in binary-x86 arms → `emit_byte`/`emit_bytes`; drop all `g_emit.out` reads inside templates.  Gate: byte-identical beauty.sno after final flush.  **EC-UNI-11 ✅ COMPLETE 2026-05-20** — Layer-3 string-builder primitives scaffold: `src/emitter/emit_io.{c,h}` (4 primitives + flush + inspection + save/restore); flush hook wired into both `emit_program()` exit points (no-op today; load-bearing at EC-UNI-12); 6/6 self-test PASS.  Gate floor unchanged.  **EC-UNI-10 ✅ COMPLETE 2026-05-20** — three orthogonal commits: `7835fb9d` (a: g_emit scaffold), `5e607294` (b: 7 ctx-bearing SM templates → parameterless), `3088dcba` (c: BB templates + remaining SM templates → parameterless).  All template fns now `void <name>(void)` (or `int` return); 16 BB + ~50 SM templates migrated; sm_ctx.h deleted; ~113 call sites rewritten across 3 dispatcher files; EC-UNI-3-beauty deliverable structurally absorbed.  Snocone is the bootstrap target.  IR-CONSOLIDATE-DCG ✅ COMPLETE (IR-CD-5/6/7 landed 2026-05-20).  IR-RN-3 ✅ (2026-05-20).  **ST2 rung ✅ COMPLETE** (2026-05-20).  Open siblings: ISO-4 (scrip_parse subprocess), IR-RN-4/5 (arch docs + close-out).  Watermark advances this commit. |
