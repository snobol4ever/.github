...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **PRF-12 next** — separation-of-concerns cleanup queue. Principle (binding 2026-05-18): parser transcribes source to pure `tree_t`; lower handles semantics; emitter walks SM. PRF-8/9/10 ✅ 2026-05-18 (Opus 4.7): `finish_given` eliminated (TT_CASE shape changed to 2-per-arm, cmpkind moved to lower.c LANG_RAKU branch); `finish_gather_body` eliminated (transitional — mirrors raku.y violation, queued under PRF-12 for proper TT_GATHER + lower-side hoist); `push_interp_str` eliminated (new leaf-only `push_interp_leaves` stub + `reduce('TT_CAT', r_nTop)` wrap). **parser_raku.sc function count: 1** (`dq_unescape` — PST-clean pre-pass string processor, permitted). PRF-12 queue: gather/sub/class/program/method-self/for-range — each is a parser→lower migration session. Prior: PRF-1..7 ✅ 2026-05-18. Gates: smoke_raku 5/0, smoke_icon 5/0, scrip_all_modes 2/0, crosscheck_snobol4 5/1 (pre-existing), crosscheck_raku 21/12 unchanged (rk_given/given18/interp/strings all stable). |
