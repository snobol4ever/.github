...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI LIFT BB-side CORRAL slice 5 2026-05-21** — slices 1+2+3+4 (one4all `1a9571fe` → `90235416` → `87d11afc` → `44e41588`) + slice 5 (`3e2d982f`).  All 14 pat-level emit_bb_x* function bodies physically inside BB_templates/ files (xstar, xlnth, xposi+xrpsi, xfail, xfarb, xtb+xrtb, xchr, xfnce, xcallcap, xfnme, xnme, xdsar, xatp, charset) + xarbn (slice 5 → bb_arbno.c IS_X86 arm).  Templates: bb_rem, bb_len, bb_pos, bb_fail, bb_arb, bb_tab, bb_lit, bb_fence, bb_capture + new bb_charset_helper, bb_dsar, bb_atp_template, bb_arbno (IS_X86 arm filled).  Live path UNCHANGED — emit_bb_node does not yet fill g_emit, so emit_bb.c originals run.  Beauty byte-identical (md5 `0c192b2f26fd1288e19c21614af95218` --interp / `40df9e004c3e963c99af716c65f2c970` --compile) and matrix gate 845/845 throughout.  **NEXT:** wire emit_bb_node to fill g_emit fields (child_fn, op_name1/2, op_kind, lbl_*) per BB node kind and call the template fns; once template path emits byte-identical to legacy, delete originals.  Then mirror the sweep on SM side (~70 SM fns from emit_sm.c into sm_*.c templates).  Optional smaller next slice: lift emit_bb_xeps into bb_eps.c (one-liner).  See GOAL-HEADQUARTERS.md watermark for full trail. |
