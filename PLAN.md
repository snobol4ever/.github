...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-4-post-r12-tos — full re-emit of 10 ME-4 inline-native blobs against the r12=TOS-pointer locked convention (REGISTER-LAYOUT.md Lon-signed sess 2026-05-10).**  Sess 2026-05-10 (Claude latest, third session): ME-4-pre ✅ closed on Lon's sign-off line in REGISTER-LAYOUT.md ("I think your register layout has a chance.  So sign off.").  ME-4-post split: (1) realloc-bug fix ✅ landed this session — `sm_jit_run` pre-grows `state->stack` to 4096 entries; `h_stno` C handler does `STATE->sp = 0` mirroring mode-2's contract at `sm_interp.c:329`; 1 file +48/-3 lines; smoke 7/7, unified_broker 49/49, SN-7 26/25 baseline-unchanged, realloc reproducer (`X = X + 1 ; Y = Y + X` loop) now byte-identical `--sm-run` vs `--jit-run`, 100k loop perf preserved ~80ms (vs ~110ms sm-run).  (2) Full r12=TOS-pointer re-emit ⏳ deferred to next session — mechanical rewrite of trampoline pc-read, emit_standard_blob's pc-bump, and each of the 10 emit_me4_* blobs' push/pop sequences; would introduce r13 = &SM_State as the first justified claim of a previously-free register per REGISTER-LAYOUT.md.  Sequencing rationale captured in GOAL-MODE3-EMIT.md addendum (sess 2026-05-10 Claude latest, third session).  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`. |
...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **A3 — CH-17i-limit-random** (`AST_LIMIT` + `AST_RANDOM`). A0 ✅ 2026-05-10 (one4all `adb289b9`): `SCRIP_NO_AST_WALK=1` tripwire landed. A1+A4-pulled-forward ✅ 2026-05-10 (one4all `75b57a32`): `SM_BB_PUMP_AST` unified opcode; `AST_BANG_BINARY`, `AST_LCONCAT` (gen path), `AST_ITERATE` migrated. A2 ✅ 2026-05-10 (one4all `242edf06`): `AST_SECTION/SECTION_PLUS/SECTION_MINUS` — scalar SM lowering via `ICN_SECTION_RANGE/PLUS/MINUS` SM_CALL_FN handlers; honest mode-3 PASS 114→118, ABORT 8→3. Gates: smoke 5/7/5/5/5/4, broker 49/49, isolation PASS, ir-run 177/56/30. |
