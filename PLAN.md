...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **NEXT: SI-6 Phase-2 begin (patterns / EXEC_STMT / calls).** SI-1..SI-5 ✅ sess 2026-05-12 (Claude Sonnet 4.6): self-hosting loop closes — `sm_interp.sc` (~70 L) executes SM produced by `lower.sc`; smoke_interp prints `hi`, sm_interp_test computes `(2+3)*4=20`. SI-5 cross-check script PASS=2/2 (hosted == native byte-identical). Runtime is trivial as predicted — opcode arms are one-liners riding on host operators. |
