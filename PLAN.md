...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **ICN-Z-0 ✅** (`ba46ac5c`) zipper foundation: `icn_leaf` + bounded flag, exercised via proc-body bounded=1. Gate PASS=2 (`to5`,`to_by`). NEXT: ICN-Z-1 rewire leaves, then Z-2..9. `lt`/`mult`/`compound` need full zipper (gen.ω→outer.β odometer); oracle = bb_exec.c BB_BINOP_GEN. FACT RULE 0. |
