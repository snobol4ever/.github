...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **BLOCKED on GOAL-CHUNKS** — IEP-5/6 (every_table + SM_PUSH_EXPR) owned by CHUNKS; IEP-7 (delete tree_t after lower) gates on CHUNKS completion. IEP-4 ✅ `d9dff43a`. IEP-8/9 can proceed independently. |
