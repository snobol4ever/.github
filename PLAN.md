...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-MODE4-HELLO-ALL-LANGS — Hello-world `--compile` end-to-end for all six languages, wired-only.** (Lon directive 2026-05-18: *"Ensure compiled Icon/Prolog use flat/wired BBs. Do not use brokered BBs in `--compile` mode. Get all six languages to say hello world before continuing mass dead code removal."*) **DAI-8 PAUSED** behind this step. Baseline matrix captured 2026-05-18 (Opus 4.7) at HEAD `a4fe1c21`: snobol4/snocone/rebus run cleanly wired-mode-4 ✓; prolog runs but via brokered `rt_bb_once_proc` → `bb_broker` (architecture violation); icon aborts on `[NO-AST] SM_BB_PUMP_PROC stub`; raku runs silently (no output). Sub-rungs IJ-HELLO-1..5 in goal file. **Prior:** DAI-8 cluster-1 retroactive mode-3/4 validation ✅ (this session, Opus 4.7) — byte-identical pre/post `a4fe1c21` across `test_smoke_snobol4_jit` (--interp 196/65, --run 195/66), `test_crosscheck_{snobol4,icon,raku,rebus,snocone}` (5/1, 4/0, 21/12, 4/0, 8/0), `test_mode4_broad_corpus_snobol4` (202/75/3 SKIP). The mode-3 lag on snobol4_jit (1 PASS-point) is pre-existing pre-DAI-8 regression debt. Cluster 1 is regression-clean across modes 2/3/4. **DAI-7 ✅** 2026-05-17 (Opus 4.7) — IJ-DEL-ICN-AST arc CLOSED. DAI-7a (`254dedb9`) deleted 14 empirically-dead Icon C-BB zeta-fns + 10 self-contained state structs from `icn_runtime.c` (−560 LOC). DAI-7b (`ff3d100a`) deleted all 3 [DAI-BOMB] stubs + `icn_lazy_box` + `find_leaf_suspendable`. **Combined DAI-7 net: −608 LOC `icn_runtime.c` (1259→651).** Prior session 2026-05-17j: DAI-5c ✅ + DAI-6 ✅. |
