...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **SI-13 ✅ sess 2026-05-12 (Claude Sonnet 4.6): SM_CALL_EXPRESSION + SM_RETURN/FRETURN/NRETURN + frame stack (si_call_stack, si_csp, si_push_frame, si_pop_frame). Also SM_PUSH_EXPRESSION as DT_E-style TABLE descriptor. Test EVAL(*X), EVAL(*(3+4)), EVAL(*PROC_FAIL):F(...) all byte-identical. Earlier SI-12 ✅: SM_CALL_FN via APPLY() arity 0..5; pseudo-calls stubbed. Structural: sm_interp_step uses switch(opc) + 120-char lines. Gates: 8/9 PASS (si_07_pat_lit pre-existing FAIL). NEXT: SI-14 — SM_JUMP_INDIR (computed goto via g_labtab).** |
