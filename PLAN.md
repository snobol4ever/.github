...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-9c+9d+9e+9f ✅** 2026-05-17 (Opus 4.7). C `scrip --dump-ast` now matches SPITBOL TDump byte-for-byte: ast_print.c uses 140-char inline threshold (`--dump-width N` configurable); snocone_parse.y flattens n-ary ADD/SUB/MUL/DIV/SEQ/ALT via `sc_flatten_arith`; qize.sc dead `LEQ` purged; 60 snocone .ref files regenerated. Fixture gate PASS=24 FAIL=2 SKIP=41. one4all `96d39c05`, corpus `1b24df4`. **NEXT:** SCT-9g (precedence/assoc audit vs SPITBOL Ch.15 — Lon directive), SCT-9h (token-slot mangling — unblocks 40 fixtures), SCT-9i (do-while PST alignment), SCT-9j (wire test_parser_sc_transpile.sh), SCT-9k (snobol4 parser tree-shape), SCT-4 (icon_helpers.sc), SCT-1f (SN-26-spl-bridge). |
