...
| **CLI 3-mode collapse + AST-interp delete** | `GOAL-CLI-3MODE.md` | one4all+.github | **CLI-3M-7** (Lon decision) — `--monitor` fate: salvage as sm-vs-jit comparator, delete, or replace. CLI-3M-1..6 ✅ 2026-05-17 (Opus 4.7). New flags `--interp` / `--run` / `--compile` / `--bb={brokered,wired}` live as aliases; 68 scripts/docs renamed (Group A); BB axis enforced only under `--interp`; tty-only deprecation warnings on legacy flags. Empirical audit: mode-1 deletion is **net +1 passing test** across smoke tier (SNOBOL4 6/1→7/0, unified_broker 21/28→22/27, sn26_label_flow loses 1 via test-edit not regression). All 5 frontends + Snocone convert cleanly. After CLI-3M-7: CLI-3M-8 delete `--ast-run`/`--ir-run` → CLI-3M-9 delete 9 `interp_*.c` files (big rip) → CLI-3M-10 delete legacy aliases → CLI-3M-11 doc updates → CLI-3M-12 unblock AR-3. one4all `4aadcf1e`, .github `814ecea4`. |
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17i (Opus 4.7): **DAI-5b-1 ✅** (`28005157`) relocated 3 surviving DESCR_t helpers (`icn_str_concat_d`/`icn_lconcat_d`/`icn_proc_as_value`) from `icn_value.c` to `icn_runtime.c`. **DAI-5b-2 ✅** (`ed957bfe`) deleted `icn_value.c` (−267), `icn_stmt.c` (−20), `icn_stmt.h` (−7); scrubbed public AST-walker decls from 3 headers + Makefile; relocated `bb_icn_rnd_seed`; added 3 file-local `static` [DAI-BOMB] stubs in `icn_runtime.c` for ~25 residual internal callers in surviving Icon zeta-fn bodies (empirically unreachable per DAI-5a trace). **Outside `icn_runtime.c`, zero real callers of `bb_eval_value`/`bb_exec_stmt`/`icn_bb_build` remain in `src/`.** Gates: ir-run 194/265, smoke ×6, crosscheck_prolog 128/0 — all unchanged. Net −294 LOC across deleted files. DAI-5c (sm-run re-baseline) and DAI-6 (docs) deferred. Prior: DAI-5a ✅ (`f7c9cfeb`) caller swap; DAI-4 ✅ (`e3c803ea`) icn_bb_build callers; DAI-3 ✅ (`a2365c5f`) ICN_BB_EVAL no-op; DAI-2 ✅ (`3e1fc9cd`) bodies → DAI-BOMB (−1641 LOC); DAI-1 ✅ (`b3e08fe8`) dead IR_ICN_EVERY. |
