...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: continue old-system deletion + resume BB DCG work.** Sess 2026-05-15c (Claude Opus 4.7, one4all `54304353`): Lon directive — old AST-walker system being deleted; pipeline is AST → IR → BB/SM only. Deleted polyglot_execute (57 lines): scrip.c dispatch arm + polyglot.c definition + polyglot.h decl. Polyglot .scrip/.md files now route through SM via has_non_sno fallthrough; were previously silently broken (proc_table_call returned FAILDESCR with no g_current_sm_prog set). Gates unchanged 5/5, 23/49, 207, 275. BB tally: 10/51 DCGs done. NEXT: continue deletion (interp_eval.c 4268 lines, interp_exec.c AST paths, _usercall_hook AST fallbacks), then highest-yield BB: TT_ITERATE list/table (template state struct already located). All CH-17g references removed from goal file per directive (.github `489b9e12`). |
...
| **⚡ Mode-4 Full Suite: SNOBOL4+Snocone** | `GOAL-MODE4-SN4-SNOCONE.md` | one4all+corpus+.github | **NEXT: M4SN-4b continued** — 240/280 target ≥250; sess 2026-05-15e (Opus 4.7) `3cfbfae0`: rt_set_stno→comm_stno wiring (error-reporting fix; gates unchanged). Diagnostics in goal watermark for 1016_eval (PUSH_EXPRESSION fnptr scheme) and 1114_item (`_usercall_hook` needs `rt_chunk_lookup` fallback for mode-4). |
...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **BLOCKED on GOAL-CHUNKS + new GOAL** — IEP-5/6 owned by CHUNKS; IEP-7 also blocked on proc_table[].proc and g_pl_pred_table holding live tree_t* (not covered by CHUNKS — new GOAL needed to lower Icon procs + Prolog clauses into dcg_table). IEP-4 ✅ `d9dff43a`. IEP-8 (sm_prog_free frees dcg_table) can proceed. |
 | **⚡ SN4 JVM Emitter (IR_t-based, beauty self-host)** | `GOAL-SN4-JVM-EMIT.md` | one4all+.github | **SJ4-JVM-1/2/3 ✅; SJ4-JVM-4 🔄 sess 2026-05-15f (Claude Sonnet 4.5, one4all `c6f60145`) — SM control flow (SM_JUMP/JUMP_S/JUMP_F via goto_w), comparison builtins (LE/LT/GE/GT/EQ/NE), SUBSTR 2-arg + bounds, SM_SUSPEND_VALUE = call(), unknown-fn nargs pop. JVM smoke 7/7 PASS (honest gate: scalar+arith+control flow only). C smoke 7/7 (no regression). Snocone 5/5, Icon 5/5. NEXT: SM_PAT_* opcodes + IR generator dispatch (for beauty patterns), SM_DEFINE_ENTRY/SM_CALL_FN user function dispatch.** |
 ...
 | **SN4 .NET Emitter (IR_t-based, beauty self-host)** | `GOAL-SN4-NET-EMIT.md` | one4all+.github | **SN4-NET-1..4 ✅** — SnoRt.il, 19 BB emitters, SM walker, smoke 7/7. **SN4-NET-5 ⏳ partial** (sess 2026-05-15, Claude Opus 4.7, one4all `bb77ac29`): two rounds landed. R1 `eb74b9a5`: beauty.il assembles + mono runs to completion exit 0; fixed `;`→`//` comment, SM_JUMP family now emits br, SM_HALT→NET_DONE, has_continue tracking, safe-defaults for unimplemented opcodes; runtime coerce_num/_obj_to_dbl use TryParse, do_return sets last_ok per kind, sno_call dispatches 8 built-ins. R2 `bb77ac29`: user-function dispatch — pre-scan builds (name→entry_pc) table from SM_LABEL define_entry=1; SM_SUSPEND_VALUE/SM_CALL_FN for known funcs emit push_ret_pc/_pc=entry_pc/br DISPATCH; SM_RETURN family pops ret_pc. SnoRt: _ret/_frames static fields + push_ret_pc/pop_ret_pc. Test baseline confirmed clean: smoke 7/7 maintained; all SNOBOL4/Snocone shared gates green; 31 scripts run, all failures pre-existing (REPORT.md). Beauty: simple DEFINE'd functions reach body and return; parameter binding pending. NEXT: per-parameter store_var emit at function entries (mirror h_call's NV_SET_fn loop); save/restore name globals via _frames; parse param list from SUSPEND_VALUE DEFINE literal. After: SM_PAT_* → pat_*_* Alpha/Beta wiring. |
 | **SN4 WASM Emitter (IR_t-based, corpus ladder)** | `GOAL-SN4-WASM-EMIT.md` | one4all+.github | **NEXT: SN4-WASM-5d** — diagnose ladder emission segfault. Sess 2026-05-15 (Claude Opus 4.7, one4all `204e4fb8`): updated stale watermark — 1..4 were ✅ in prior commits `468910b0`+`685183c1` but never marked. 5a (scrip.c `--target=wasm` dispatch), 5b (memory-layout collisions), 5c (LT/GT/EQ/NE/GE/LE/IDENT/DIFFER in `$sno_call`), 5-LADDER (`test_sn4_wasm_ladder_safe.sh` baseline PASS=11/129). **Goal pivot:** closing gate is ladder PASS ≥ 100 over csnobol4-suite+demo+feat, not beauty self-host (deferred). Smoke 7/7 holds. |
