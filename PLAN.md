...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **✅ PIVOT COMPLETE — formatting layer DELETED. HEAD=`1bd95155` (local, see ⚠ push note).** LABELFIX no longer needed: the label-displacement was a *formatting* artifact (renderer co-located/suppressed pc-labels). Per Lon directive (session 2026-05-22): templates carry ZERO formatting. Killed bb3c entirely — `bb3c_format`/`bb3c_emit_jmp` gutted to single-spaced straight prints, `bb3c_write_line`/`pad_to_width`/`visual_width`/`is_cond_jmp` + pending-state globals deleted, flush fns now no-ops; `t3`/`tf`/`T3C`/`tj` redirect to `fprintf`; `emit_three_column_line` straight print; dispatch loop emits `.L<pc>:` standalone via `fprintf` (loop owns label placement, not templates); `je;jmp` un-fused into 2 plain lines; `data_buf_three_col`+pend-label helpers de-padded. **Comparison is now filtered (`/tmp/semdiff8.py`): labels bind to next real instr, whitespace collapsed, SM pseudos zero-byte — NOT byte-identity.** Verified: 60/60 run-parity vs pre-pivot `c44c3db6`; GATE-PK 420/0/639 (re-frozen); GATE-S 184 three-mode parity; GATE-J 4/0. **NEXT:** (1) push `1bd95155`; (2) delete dead `fmt_*` format-mode layer in emit_core.c (`g_bb_emit_format` never set to 1); (3) resume INLINE on remaining ~8 templates (sm_call_fn/suspend, sm_bb_once_proc/pump_proc, push_expression/call_expression, sm_stno) — now formatting-free by construction. Pre-existing (not regressions): 2 annotation-only deltas (EXEC_STMT_VARIANT `.S_ERR`, PAT_CAPTURE_FN `.S7`); shared 3-mode pattern-fence failures. |
