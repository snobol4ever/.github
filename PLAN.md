...
| **⚡ Icon BB Native (flat BB templates)** | `GOAL-ICON-BB-NATIVE.md` | one4all+.github | **NEXT: IB-9 — purge SM_BB_PUMP_AST from Icon path. IB-0..8 ✅ `655f9866`: all templates done (to/iterate/alt/every/limit/bang/lconcat/seq), lower_to+lower_iterate→SM_BB_PUMP_AST, N~214. Progress: bash scripts/util_icon_bb_progress.sh |
| **Prolog BB Complete (honest mode 3)** | `GOAL-PROLOG-BB-COMPLETE.md` | one4all+.github | **⛔ BLOCKED on CH-17g-irrun-execution.** Phase A+B complete. Remaining 18 FAILs (13×rung10 puzzle backtrack + 5×rung30 DCG) all require SM backtracking through proc bodies. one4all `7d8dbea8`, honest 96/294. |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: Fix DEFINE_ENTRY loop-reentry bug — lower.c must emit inner body label after SM_DEFINE_ENTRY so :(fname) gotos bypass prologue → case_driver+omega_driver PASS. Then match_driver+XDump_driver. Target PASS≥10/17. one4all `25be767c`, template-byte-id 4/4, beauty-subsystems PASS=6/17.** |
