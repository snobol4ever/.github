...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | EM-4 LANDED sess #68 (SM_JUMP/S/F + readability standard; gate PASS=7). Generated .s now reads as annotated disassembly: per-stmt page-break banners w/ verbatim source, inline annotations on every opaque pointer-immediate. Emitter signature now (prog, out, src_path). 5 tracked .s files regen'd. Next: EM-5 (SM_PUSH_CHUNK/CALL_CHUNK/RETURN; gate: two chunks calling each other). |
