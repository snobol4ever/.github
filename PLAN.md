...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | CAT-D-1..9b ✅, CAT-D-6 ✅, CAT-D-10 ✅, CAT-D-12-S1 ✅, CAT-D-12-S2 ✅, CAT-B ✅, CAT-C ✅. **CAT-D-11 ✅** (Opus 4.7, 2026-05-27): `sort/2` + `msort/2` mode-4 emission via two-path effect helper mirroring CAT-D-6 (atom_chars). Two linkage entries: scalar `rt_pl_sort_msort` 7-scalar / compound-literal `rt_pl_sort_msort_term` 1 ptr + 3 scalars. Shared static `sort_msort_common(do_msort, t0, t1)` — direct port of bb_exec.c BB_BUILTIN sort/msort arm (walk cons list, insertion-sort by pl_term_compare, optional dedup, build reverse cons-list, unify into t1 under trail mark). Template dispatcher in `bb_builtin.cpp` between CAT-D-10 and CAT-D-9b; Path B (BB_PL_STRUCT a0) checked first via `emit_build_compound_term`, Path A (scalar) otherwise. Net diff: +56 lines bb_exec.c, +5 lines bb_exec.h, +50 lines bb_builtin.cpp. Mode-4 corpus 24/107 → **28/107 (+4: rung17_sort_sort_basic + rung17_sort_sort_already_sorted + rung17_sort_msort_basic + rung17_sort_msort_dupes; rung17_sort_sort_empty already passing)**. `rt_pl_sort_msort_term@PLT` confirmed in emitted `.s` (no stub-comment, no `unknown 'sort'` fallthrough). Downstream list-pattern unify (`S = [A,B,C]` after sort) confirmed working in mode-4 via pre-commit probe. Gates: GATE-1 5/5, GATE-2 132/0, GATE-3 mode-2/3 89/107 ea., GATE-4 4/4, FACT RULE 0, sibling smokes hold. NEXT: **CAT-A-3** (BB_PL_CALL + BB_CHOICE β-resume — needs Lon directive on inline-on-demand vs resumable-call design choice; est. +15-25 PASS); then investigate the surfaced rung07 `bb_emit_byte: non-BINARY-mode reach` issue. |
