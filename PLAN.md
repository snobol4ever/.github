...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7d-usercall-reentrant LANDED sess #82, 2026-05-07** — native chunk registry (rt_chunk_entry + scrip_rt_register_chunks); emitter walks SM_LABEL a[0].s to build .data table; call_native_chunk binds formals via FUNC_PARAM_fn+NV_SET_fn. Honest deviation: SNOBOL4 RETURN not yet SM_RETURN — bodies fall through to halt (blocked on CH-17c). Next: diagnose --sm-run beauty self-host regression, then EM-7d. |
