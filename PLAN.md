...
| **Prolog BB Complete (honest mode 3)** | `GOAL-PROLOG-BB-COMPLETE.md` | one4all+.github | **NEXT: PB-0 tripwire + test_prolog_bb_honest.sh.** Carved 2026-05-12 (Claude Sonnet 4.6). Honest baseline: 31/294. TT_CHOICE named already SM_BB_ONCE_PROC. Remaining: TT_UNIFY, TT_CUT, TT_TRAIL_*, TT_CLAUSE, TT_CHOICE unnamed. |
=======
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-13a — fix qize.sc Append hang (then SL-13 steps 3-4). SL-13 steps 1+2 ✅ sess 2026-05-12 (Claude Sonnet 4.6): T_2AMP/T_2TILDE fixed (& ~ → reduce/shift in all parser_*.sc), TDump→Lower_collect+Lower_run wired, run_scrip_parser.sh added.** |
