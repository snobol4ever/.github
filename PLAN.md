...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#8 LANDED (session 2026-05-05): FENCE around all the right alternation tails — using SNOBOL4 Expr gauntlet as the model. Snocone, icon, prolog, raku, rebus all gain `FENCE($'op' rhs (...) \| epsilon)` so once the operator commits we don't backtrack. SNOBOL4 was already canonical. iter#7 corpus@fa4a14e. Gates all 100%: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38. Next: operator picks iter#9 concept. |
