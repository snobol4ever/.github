...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7a-sim-test-fix LANDED sess #85, 2026-05-07** — `sm_phase2_sim_test.c` had stale assertions left over from before EM-7c-invariance-rule (sess #84); rebuilt unit test exposed 6 failures (Test A `BREAK("=") . LHS` has_variant; Test C `*PAT` XDSAR variant; Test E XDSAR/XBRKC variant). Inverted all 6 assertions to match corrected EM-7c semantics (only XVAR is variant). Smoke ×6 + EM 12/12 + isolation PASS. Pure test-expectation fix; no source change to bb_flat.c / sm_codegen / etc. **EM-7c-greek-purge ATTEMPTED + ABANDONED sess #85** — initial purge applied across 7 files (bb_flat.c / .h, sm_codegen_x64_emit.c partial, stmt_exec.c, snobol4_pattern.c, scrip_rt.h, bb_flat_text_test.c, scripts/test_smoke_jit_emit_x64.sh) and gates were green, BUT a `git stash` mishap dropped the entire purge before commit; only the sim-test fix survived. Lessons documented in goal file watermark: (a) the audit grep `\\balpha\\b\\|...` MISSES `_alpha`/etc. in format strings because `_` is a word char — need to add `_alpha\\|_beta\\|_gamma\\|_omega` to the audit; (b) `bb_flat_text_test.c` and `scripts/test_smoke_jit_emit_x64.sh` are part of the purge surface; (c) DO NOT use `git stash` mid-purge. Next: re-attempt **EM-7c-greek-purge** with corrected audit (~50–60 hits expected). Then **EM-7c-bb-three-column** (emit each BB box as a 4-port cluster in three-column LABEL/ACTION/GOTO form). Then EM-7c-bb-macros. Then EM-7d (blocked on pre-existing beauty self-host regression). |
