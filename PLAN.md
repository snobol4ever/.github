...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI LIFT pattern established 2026-05-20** — one4all `71bd8b6f` lifted `emit_bb_xstar` and `emit_bb_xlnth` from `emit_bb.c` into `bb_rem.c` and `bb_len.c` IS_X86 arms.  `g_emit` gained three label fields (`lbl_succ`, `lbl_fail`, `lbl_back`).  Pattern: copy-paste the fn body verbatim into the matching template's `IS_X86 { ... }` arm; parameters come from `g_emit`; helpers stay where they are (90% slated for deletion in the next pass).  Two paths coexist until rewire/consolidation.  Matrix gate 830/830 PASS; beauty md5 byte-identical.  **NEXT: continue the lift sweep** — remaining BB-x86 fns in `emit_bb.c` (`emit_bb_xchr`, `emit_bb_xfarb`, `emit_bb_charset`, `emit_bb_xposi`/`xrpsi`, `emit_bb_xtb`/`xrtb`, `emit_bb_xfail`, `emit_bb_xeps`, etc.) into their matching `bb_*.c` templates; then SM-x86 fns from `emit_sm.c` into `sm_*.c` templates.  Then rewire dispatchers and delete the originals.  See `GOAL-HEADQUARTERS.md` watermark. |
