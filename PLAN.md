...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`87492642`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN, Prolog BB honest 124/0/0, smoke jit 184/186. **RUNG THREE-SECTION-X86 (TSX) — DELETE phase COMPLETE.** TSX-DELETE ladder ✅ (DEL-0/1/2, W-LIT/PORT/PL). TSX-WIRE-1/2/3 ✅. **TSX-DEL-FINAL ✅ `872c01d2` — insn_* family FULLY ELIMINATED 16→0** + capture_bytes scaffolding gone. **TSX-DEADCODE-SWEEP ✅ `87492642` — 28 dead emitter fns / 195 lines deleted** (Group A 0-ref: emit_bb_box_banner/bb_ptr_slot/io_is_buffered/macro_begin/macro_end/mov_rdi_imm64/seq_noop_macro/store_delta; Group B emit_flat_* family + macros; Group C 7 legacy void(FILE*) JVM/NET wrappers superseded by _str; Group D s_L3asm). KEPT (live): emit_seq_port_call_rip (text-asm, 6 callers), emit_form_*/sigma-delta chain (live via emit_bb.c flat epilogue). **NEW RUNG: NO-SNPRINTF** — remove all 71 snprintf in BB/SM/XA templates, replace with `X + emit_fmt(a,"%d") + Y` concat (NS-0..NS-6; emit_fmt already exists). **NEXT:** NS-0/NS-1 (start NO-SNPRINTF, bb_capture has 19); or (a) trivial sweep of pre-existing dead emit_seq_frame_*/mov_edx_i32 + 44-decl bb_insn_* legacy block; (b) move emit_bb.c flat epilogue + brokered prologue into a template (XA frame-wrapper) → orphans emit_form_*/sigma-delta for deletion; (c) ER-8 relocation rethink (bb_lit memcmp-PLT/rip-rel-lea/rel32). ⛔ PENDING Lon: TEMPLATE PURE-PROJECTION. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
