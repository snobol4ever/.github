...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-16 slice 2 LANDED 2026-05-20** — `jvm_alpha_method_hdr(stack, locals)` and `jvm_beta_method_hdr(stack, locals)` helpers in `emit_core.c`, 30 sites across 15 BB templates substituted.  JVM emission byte-identical (md5 unchanged for test fixture).  Matches existing `net_alpha_hdr`/`net_beta_hdr` precedent.  Slice 1 (one4all `56b5afb6`) was `sm_calls.c` duplicate collapse via `sm_call_or_suspend(int)`.  **NEXT: EC-UNI-16 slice 3** — per-fn in-template helpers for the 6 BB pattern templates ≥60 lines (look for shared multi-line patterns within JVM/NET arms; `aload_0; getfield ...` single-insn wrapping is NOT justified) OR **EC-UNI-21-followup** (M1 reconcile/retire).  See `GOAL-HEADQUARTERS.md` watermark. |
