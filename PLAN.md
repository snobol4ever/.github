...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-4-pre — Register layout study + design.**  Lon-directed sess 2026-05-10: ME-4 inline-native blobs were emitted without a coherent register convention (each blob reloads `[r12]`-stack-base into a scratch; `rbx`/`r13`/`r14`/`r15` unclaimed) and the live realloc bug surfaces in the gap.  ME-4-pre reads CSNOBOL4 (`v311.sil` → `snobol4.c`) + SPITBOL (`sbl.min` → bootstrap asm) + `bb_boxes.s` callee-save discipline, then writes `REGISTER-LAYOUT.md` for Lon sign-off.  ME-4 stays PARTIAL until ME-4-post fixes the realloc bug + re-emits the ten blobs against the locked convention.  Code state: one4all `e7ac6f77` — variable-size blobs + ME-4 inline-native emitters + rt.c pluggable backend landed; gates clean (smoke 7/7, unified_broker 49/49; 100k counter loop ~32% faster than `--sm-run`); 🐛 multi-statement arith reproducer aborts on `realloc(): invalid next size` (mode-3 missing inter-statement sp reset, ME-4 push blobs have no bounds check).  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`. |
