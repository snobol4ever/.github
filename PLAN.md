...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17f (Opus 4.7): **PIVOT — IJ-DEL-ICN-AST surgery.** Lon directive: stop the per-construct hunt; amputate the Icon-specific tree_t* AST walker (it had become the maintenance treadmill). **DAI-1 ✅** (`9d4da318`): removed dead-on-vine `IR_ICN_EVERY` + `lower_icn_every`. **DAI-2 ✅** (`41b6ef24`): bodies of `bb_eval_value`, `bb_exec_stmt`, `icn_bb_build` replaced with `[DAI-BOMB]` stubs (stderr + exit 78). **Net −1641 LOC across 3 files.** Icon `--ir-run` 194/265 unchanged, zero bomb fires across 265 rungs — proves the Icon walker was already silently dead for the rung suite; mode-1 Icon runs via `interp_eval`+`ir_exec.c` (IR_block_t walker). All smoke gates hold floor. DAI-3..6 deferred (cosmetic caller cleanup; bombs make latent caller discovery automatic). |
