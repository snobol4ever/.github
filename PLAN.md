...
| **⭐ DYNAMIC BYRD BOX** | DYN-64 | one4all `ad17595` · corpus `8d5cc6a` | **DYN-65**: 169p/9f · fix 1012_func_locals + 1013_func_nreturn + 1015_opsyn → ≥172p → M-DYN-S1 |
...
| **SNOBOL4 JS** | SJ-23 | one4all `e4354a5` · .github this | **SJ-24**: fix -include/_skip_to_eol/_prev_ws length bug → +4 → E_NAME(E_IDX) nameref → ≥176p → M-SJ-INTERP+2 |
...
| **⭐ one4all-SNOBOL4-NET** | D-179 | one4all `07e9030` · .github this | **165p/13f** · stdin EOF, arg-fail propagation, ARG/LOCAL, $.var\<idx\>, -include done · **D-180**: fix FRETURN (test_*/lib), triplet blank-line, word1/cross → ≥170p → OPSYN+NRETURN → ≥175p |
