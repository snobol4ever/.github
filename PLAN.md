...
| **BB Template Ladder — Icon + Prolog** | `GOAL-BB-TEMPLATE-LADDER.md` | one4all+corpus+.github | GOLDEN BB RULE: BB_t≡JCON ir_*; sval/ival/dval=payload. G-1 partial `e099fdae`: 15/16 files clean, bb_exec.c remains (294 hits, build fails bb_exec.c:57). STRUCTURAL GAP: γ/ω continuations not threaded through lowerer — must pass succ/fail BB_t* down recursively. **NEXT: G-1 finish** — bb_exec.c then continuation threading. .github `db503a3e`. Rungs 153. |
