...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | AG-PURE Steps 1-7 ✅ merged to main (`f81e1d51`). Step 8.1 ✅ (BB_EVERY passthrough for literal-bound gens, `f81e1d51`). Next: 8.2 BB_TO/BB_TO_BY dynamic-bound AG-pure — see HANDOFF-2026-05-27-OPUS-ICON-BB-AG-STEP-8-1.md for the architectural question about lower_icn_expr_threaded_b routing through TT_EVERY's gen. Gates: smoke 5/5, broker 24, rungs 198. |
...
| **SNOBOL4 BB** | `GOAL-SNOBOL4-BB.md` | SBL-SPLITTER-1 ✅ (grammar restructured: `opt_subject: expr14 T_CONCAT expr2 → TT_SCAN`). Multi-element patterns now parse with structural subject/pattern boundary. Rung suite M2=14 M4=15 (+5,+6). Broad corpus 174 (net regression of ~10 from downstream gaps in TT_VAR/TT_DEFER pattern handling — newly exposed by clean AST). Next: SBL-DCG-DEFER (handle TT_VAR/TT_DEFER in lower_pat_dcg). |
