...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-DOPPELGANGER-PURGE EDP-3 — sm_emit_template.c legacy table purge.** ⛔ Lon directive sess 2026-05-12h: with sm_templates.c + bb_templates.c established as single source of truth (91 SM + 35 BB emitters covering all SM_t and XKIND_t entries), eradicate every parallel SM/BB emission path. New EM-DOPPELGANGER-PURGE rung with 12 sub-rungs (EDP-1..EDP-12) added to goal file. Beauty-suite / beauty self-host work BLOCKED until this rung closes. Sess 2026-05-12h (one4all `baa29424`): (1) BB-blob R10 corruption FIXED — push/pop r10 around runtime calls; new BB_INSN_PUSH_R10/POP_R10. case_driver passes mode-4. PASS 6→7. (2) lower.c DEFINE_ENTRY labtab fix re-applied. (3) Template consolidation: 78 per-opcode files → 2 files (sm_templates.c, bb_templates.c). Gates: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty-subsystems mode4 PASS=7 FAIL=10.** |
| **Snocone SM (M2 path: lower.sc + sm_interp.sc, self-host)** | `GOAL-SNOCONE-SM.md` | corpus+one4all+.github | **MERGED sess 2026-05-12 (Claude Sonnet 4.6, handoff #3) from former GOAL-SNOCONE-SM-LOWER.md + GOAL-SNOCONE-SM-INTERP.md. Old files were 1635 lines total of which 90% was historical detail of completed rungs (SL-0..SL-13c, SI-1..SI-17b). Merged file is 312 lines: completed rungs collapsed into a status table, only open work and Phase 4 deferred items in full. Current state: SL ladder ✅ through SL-13c-tilde-fix; SL-13d open (parser_snobol4.sc driver populating Lower_collect). SI ladder ✅ through SI-17b; SI-17c/d/f/h open as Phase 3 corpus mirrors. All 87 SM_* opcodes have handlers (real or silent stubs for Icon/Prolog complexity). Gates: self-host 16/16 PASS, smoke matrix 33/33, zero regressions. **NEXT: SL-13d parser-driver work (true self-hosting) OR continue SI-17 ladder (Track A corpus mirrors).** |
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-DOPPELGANGER-PURGE EDP-3 — sm_emit_template.c legacy table purge.** ⛔ Lon directive sess 2026-05-12h: with sm_templates.c + bb_templates.c established as single source of truth (91 SM + 35 BB emitters covering all SM_t and XKIND_t entries), eradicate every parallel SM/BB emission path. New EM-DOPPELGANGER-PURGE rung with 12 sub-rungs (EDP-1..EDP-12) added to goal file. Beauty-suite / beauty self-host work BLOCKED until this rung closes. Sess 2026-05-12h (one4all `baa29424`): (1) BB-blob R10 corruption FIXED — push/pop r10 around runtime calls; new BB_INSN_PUSH_R10/POP_R10. case_driver passes mode-4. PASS 6→7. (2) lower.c DEFINE_ENTRY labtab fix re-applied. (3) Template consolidation: 78 per-opcode files → 2 files (sm_templates.c, bb_templates.c). Gates: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty-subsystems mode4 PASS=7 FAIL=10.** |
