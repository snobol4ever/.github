...
| **⭐ DYNAMIC BYRD BOX** | DYN-58 | one4all `eebc09d` · corpus `2f2bbe3` | **DYN-59**: remove `pat_ctx`/`_expr_is_pat` from `interp_eval E_SEQ`; add `in_pat_ctx` flag so pattern exprs always use `pat_cat`; fix lexer grammar regression (restore ≥155p); then 310/311/312 |
