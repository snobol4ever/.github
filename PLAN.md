...
| **Prolog Frontend Ladder** | `GOAL-LANG-PROLOG.md` | one4all+corpus | **PR-15** (PR-15 rung38_iso_errors IN PROGRESS — 4/5 pass, test 03 existence_error FAIL. Root cause: pl_throw_existence_error_procedure must fire in interp.c E_CLAUSE/pl_box_choice empty-clause path, NOT interp_exec_pl_builtin E_FNC fallthrough. Diff saved as one4all/docs/PR-15-session-2026-05-02-attempt.diff. Apply diff, hook empty-clause path in interp.c, gate 5/5.) |
...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | **SB-6.E.7-J pass #3 — main00..main05 main-loop rewrite is the SOLE remaining piece** (session 2026-05-02 #17: pp/ss invented-helper removal + per-type body-part dispatch LANDED. Removed ppLeaf/ppList/ppStmt/ss_leaf (4 invented helpers, none in beauty.sno); inlined 10 leaf + 6 goto-clause + 7-component pp_Stmt + per-type pp_,/pp_|/pp_../pp_[]/pp_()/pp_Call/pp_ExprList body parts; added 15+15 explicit pp_OP/ss_OP cases per beauty.sno 296-323/502-529; added ssUnOp/ssBinOp helpers symmetric to ppUnOp/ppBinOp (all four ARE in beauty.sno as inline labels). beauty.sc 498→588 lines. Three baseline gates green throughout. SB-6 fingerprint UNCHANGED: lines=603 stderr=4389 parse_err=170 internal_err=4 rc=124, 91 hunks — structural rewrite is observably-equivalent on SB-6 input; behavioral closure remains downstream of SB-6.E.7-H. main00..main05 rewrite ATTEMPTED but collapsed lines=603→0 (NOT committed per RULES.md regression-in-error-class); diagnosis suggests `if (~(Line = INPUT))` interaction with failure-rollback or a near-miss in main02's Src=Src Line nl ordering. Session #18 should try (a) `while (Line = INPUT)` outer + tagged-break inner with `break LABEL`, or (b) wrap main into `function main()` so `return;` works, or (c) bisect by changing one body part at a time. corpus @ `5c468a3`, .github @ `88755be`, one4all clean.) (session 2026-05-02 #16: SB-6.E.7-M LANDED — sno prefix stripped from beauty.sno (67 names, 319 occurrences). Found one collision: snoSorF parser pattern vs SorF runtime variable; fixed via pre-rename .SorF → .sf. New oracle md5 `9cddff2534472b822438801d8db58a99` (622 lines, down from 646 because shorter identifiers fit on single lines). Three baseline gates green. SB-6 fingerprint: lines=603 stderr=4389 parse_err=170 internal_err=4 rc=124, 91 hunks. corpus @ `5cc1baa`, .github @ `537945f`, one4all clean.) SB-6.E.7-A closed. SB-6.E.7-H still downstream blocking after beauty.sc main-loop completes. |
...
| NET Beauty Self-Host | `GOAL-NET-BEAUTY-SELF.md` | snobol4dotnet | S-2-bridge-7-fullscan-slot-fix (session #80, 2026-05-02 — S-2-bridge-7-byrd-pattern LANDED (x64 5035571, one4all 872b5a3c). Root cause of line-48 Parse Error isolated via C# function trace: *match(snoUnprotKwds,snoTxInList) deferred code pushes snoFunctions slot (455 chars) instead of snoUnprotKwds (125 chars). Fix: slot-index collision in BuilderResolve. Next session: dump VariableSlotIndex to confirm collision, fix ResolveVariable, rerun beauty self-host gate. | snoUnprotKwd | ...)` alternation, so dot never advances to the snoUnprotKwd arm where FULLSCAN actually lives.  HEADs: x64 `dd66e14`, snobol4dotnet `12bd3fa`, one4all `1072fc61`.) |
