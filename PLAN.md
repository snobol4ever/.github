...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-TEMPLATE-PURITY-1 (audit BB templates for is_text/callback violations).** BB template files split one-per-box sess 2026-05-11 (Claude Sonnet 4.6, one4all `5fa770d6`): bb_xfarb→xeps+xfail+xfarb, bb_xlnth→xtb+xrtb+xlnth, bb_xposi→xposi+xrpsi. SM families stay together (Lon decision). Gates: build clean, template-byte-id 4/4. |
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **SL-2 ✅ skeleton: corpus `SCRIP/lower.sc` (633 L) + `sm_lower_driver.sc` + `smoke_lower.sc`. emit_goto table-driven; dispatcher + literals + vars + arithmetic + concat + calls + assigns + pattern primitives + STMT walker + sm_dump formatter. Verified: Lower(STMT :lbl END :end) yields 3 instrs (SM_LABEL,SM_STNO,SM_HALT). Sess 2026-05-12 (Claude Opus 4.7). lower.c further squeezed −4 L (1241→1237) — `lower_alt`/`lower_cset_op` inlined, `emit_goto` table, 3 section_* wrappers folded; broad gate byte-identical (PASS=206/FAIL=74). Two open issues for next session: (1) load-time Error 3 (likely emit_goto's REPLACE-on-null); (2) qize.sc load-order breaks Append-thru-param. Tag mismatch: lower.sc compares canonical `TT_VAR` (matching C `--dump-ir`); parser_*.sc still emit `'AST_VAR'` (pre-second-rename). Constants are isolated at top of lower.sc — one-line fix when .sc parsers are retagged.** |
