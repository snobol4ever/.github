...
| **M-MONITOR-IPC-5WAY** | all 5 participants (CSNOBOL4+SPITBOL+ASM+JVM+NET) write trace to per-participant FIFO; `run_monitor.sh` parallel launch + collector; hello PASS all 5; zero stderr/stdout blending | snobol4x | ⚠️ `064bb59` B-236 — **IMPLEMENTED WRONG**: async parallel logger, not sync-step barrier; participants run to completion independently; post-hoc diff only; does NOT stop at first divergence as designed |
