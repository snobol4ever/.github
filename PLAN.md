...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-16 slice 1 LANDED 2026-05-20** — `sm_calls.c` 188→112 lines via `sm_call_or_suspend(int suspend)` helper.  `sm_call_fn` and `sm_suspend_value` were byte-near-duplicates differing in one JS line; helper absorbs the body, two thin wrappers remain.  Matrix gate extended to follow same-file static-helper delegation; 450/450 still PASS.  EC-UNI-21 close gate 9/9 PASS.  **NEXT: EC-UNI-16 slice 2** (pick one of the 6 remaining ≥60-line BB templates: `bb_arbno` 111, `bb_lit` 98, `bb_cat` 87, `bb_tab` 80, `bb_alt` 75, `bb_capture` 66) OR **EC-UNI-21-followup** (re-converge or retire M1).  See `GOAL-HEADQUARTERS.md` watermark. |
