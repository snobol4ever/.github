...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **ICN-Z-0 ✅ + ICN-Z-1 ✅** (`821640c8`) zipper foundation: `icn_leaf` + bounded flag + `icn_tree_is_leaf` (leaves hardened bounded=1). ICN-Z-2 substantial. Gate PASS=2 (`to5`,`to_by`). NEXT: ICN-Z-3 (BB_CONJ), then Z-4/Z-7/Z-8 (gate movement). `lt`/`mult`/`compound` need generator nesting (gen.ω→outer.β odometer); oracle = bb_exec.c BB_BINOP_GEN. FACT RULE 0. |
