...
| Icon Frontend Ladder | `GOAL-LANG-ICON.md` | one4all | IC-9 (session #21, 2026-04-30, one4all HEAD `8ddcbc89`: rung36_jcon_table **PASS** — empty diff against expected.  First PASS-count advance in IC-8 since session #17.  Six fixes landed: (1) `every !x := V` — bang-iterate as lvalue, E_ASSIGN with E_ITERATE LHS in icon-frame `case E_ASSIGN` walks every cell of table/list in one pass; (2) `every x[key(x)] := V` — gen-index in subscript lvalue, drives the gen via `icn_eval_gen` and `subscript_set` per yielded key; (3) `every !x OP:= V` — augmented-bang-iterate, E_AUGOP with E_ITERATE LHS, RHS evaluated once with per-cell AUGOP_CELL macro; (4) `copy()` proper shallow copy for tables (fresh `TBBLK_t` + bucket walk + `table_set_descr` per entry preserving key descriptors) and lists (fresh DESCR_t[] + memcpy + `DATCON_fn("icnlist",...)`); was a no-op alias before — fixes the `every !y +:= 40` mutating x bug; (5) `delete()` and `member()` arity — 1-arg form treats &null as the key (Icon null-arg padding), extra args ignored; (6) `<-` reversible assign — new IR kind `E_REVASSIGN` (was previously parsed as E_ASSIGN, indistinguishable in IR); parser routes to E_REVASSIGN; standalone (non-every) handler in interp.c acts like := (no driver to backtrack against); `icn_bb_revassign` Byrd box snapshots prior value via `subscript_get` (preserves table-default for missing keys), writes new value via cell pointer (table_ptr/array_ptr) or `subscript_set` fallback, on β reverts via the same channel; net effect under `every x[3] <- 19` is entry-created with default-value preserved (because subscript_set on revert writes the saved default into the now-existing cell).  No header struct changes (no clean rebuild needed).  Files touched: `src/ir/ir.h` (+2 enum/name), `src/frontend/icon/icon_parse.c` (1 line: E_ASSIGN → E_REVASSIGN), `src/driver/interp.c` (~150), `src/runtime/interp/icn_runtime.c` (~110).  Gates: smoke 5/0, broker 49/0, crosscheck 4/0, **all-rungs PASS=189 FAIL=44 XFAIL=30 TOTAL=263 (+1)**, **rung36 PASS=6 FAIL=39 XFAIL=30 TOTAL=75 (+1)**.  Working tree clean post-session (one stray `foo.baz` removed per RULES.md).  **Next pivot:** `!s := "."` — bang-iterate as lvalue on a string, mirror of work just landed.  String-rebuild semantics (walk chars, concatenate prefix + new + suffix per tick, write back to source var) would close `rung36_jcon_string`.) |
| Prolog Frontend Ladder | `GOAL-LANG-PROLOG.md` | one4all | PL-12 (session 2026-05-01: 5 commits landed bisectable+gate-neutral. corpus `dfc26da` Step A plunit stdlib enrichment (~25 stubs), `80ce2f2` Step A-patch numbervars/4 direction fix. one4all `1de19342` Step B.1 copy_term_rec slot fix (1<<20 + nmap so bind() trails fresh vars), `018bfdef` Step B.2 findall snapshots use pl_copy_term (preserves var sharing), **`3bc1573d` Step C — NEW — arith INT_MIN/-1 SIGFPE guard** (pre-existing latent bug; on x86 IDIV INT_MIN/-1 raises hardware FPE; baseline never tripped because plunit's silent-success default arm registered PASS without running test bodies; session #7 hit it via test_arith.pl line 174 `:- begin_tests(gdiv).`; fix guards 4 arith sites E_DIV/E_MOD/E_FNC mod/E_FNC rem; <limits.h> added). **B.3 bridge HELD BACK** as committed doc `docs/PL-12-session-2026-05-01-bridge.diff` (211 lines) — applied on top moves smoke/broker green and SWI 7→14/57 (Step C unblocks test_arith load) but still below 43/57 metric because the harness scores at suite-line granularity (block PASSes only if ALL sub-tests PASS). Bridge IS mechanically correct — verified via 5 decisive repros (session #7) + reconfirmed (this session). The 43/57 baseline is a false-positive ceiling: silent-success counts as MISS-PASS in the matcher. With bridge + A-patch, test_term:numbervars sub-tests jump 0/11 → 5/11 PASSes (single, single_s, shared, shared_s, twice_singleton). **REVISED next-session plan:** Step D harness scoring decision (per-test vs per-block); Step E apply held-back B.3 bridge; Step F sub-test stdlib gap inventory v2; Step G runtime semantic fixes (numbervars negative-start, =@=, compound/1 edges). Saved 2 docs: bridge.diff + findings.md. Smoke 5/5, broker 49/49, SWI 43/57 unchanged at session-end.) |
...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.i.4 (session 2026-05-01 #4 LANDED LS-4.i.3 at one4all `c3a56a6a`: switch/case/default per Q6 (modern no-fall-through, implicit break at end of each case body except the last).  Grammar: `switch_head : T_SWITCH T_LPAREN expr0 T_RPAREN`, `switch_body : case_clause | switch_body case_clause`, `case_clause : case_or_default_label | case_clause stmt`, `case_or_default_label : T_CASE expr0 T_COLON | T_DEFAULT T_COLON`.  Two new `matched_stmt` arms wire it in (with empty-body alt).  Zero shift/reduce or reduce/reduce conflicts.  Lowering: `tmp = e; IDENT(tmp,vN) :S(_Lcase_NNNN); :(_Ldefault_NNNN-or-_Lend); _Lcase A :(_Lend); _Lcase B :(_Lend); _Ldefault D; _Lend`.  No fall-through — implicit `:(_Lend)` goto emitted at the start of each new case label firing (suppressed for stacked `case 1: case 2: stmts;` via `last_case_label_tail == code->tail` check — both pads chain forward to shared body via SNOBOL4 label-pad chaining).  No goto for the LAST clause (falls through to `_Lend` naturally).  Implementation: `struct CaseEntry { case_label, value }`; `struct SwitchHead` (disc, tmp_name, end_label, default_label, has_default, after_tmp_assign, cases[], last_case_label_tail, prev_switch, lineno); `ScParseState.cur_switch` for case actions to find their owner; `sc_switch_head_new` (eagerly emits tmp-assign, allocates 3 control labels, snapshots splice anchor, pushes LoopFrame is_loop=0, sets cur_switch); `sc_switch_case_label`/`sc_switch_default_label` (emit implicit-break, emit pad, record entry); `sc_finalize_switch` (build dispatch chain, splice after tmp-assign anchor, append _Lend pad, pop frame, restore cur_switch).  LS-4.i.2 forward-compat fully consumed: break inside switch finds the switch's `is_loop=0` frame; continue rejects via `want_loop=1` skip; `break LOOP_LABEL` from inside switch-in-loop walks past switch frame to find the labeled loop frame (test 42 verifies).  Tests added (32–45, +45 assertions): empty switch, single-case-no-default, two-cases-no-default, two-cases-with-default (catch-all → `_Ldefault`), default-first (allowed), stacked-case-labels-suppress-implicit-break, explicit-break-adds-extra-goto, continue-in-switch-error, multiple-defaults-error, nested-switches-independent-labels, `break LABEL` out of switch-in-loop targets loop's `_Lend` (LS-4.i.2 forward-compat headline gate), switch-in-if-then-arm, switch-with-user-label (label on tmp-assign per LS-4.i.2 disambiguation), bare-`case 1:`-at-top-level-error.  All gates green: smoke 5/5, beauty 42/0/3, broker 49/0, **parse-a..i 871/871** (35+119+66+79+71+118+95+85+203, was 826/826).  Note: goal-file LS-4.i.3 step prose says "falls through to the next case implicitly; explicit `break;` for non-fall-through" but this contradicts Q6 ("break is implicit at end of each case (no fall-through, matching modern usage)") and the lowering shape itself — implementation follows Q6 verbatim.  Side-channel only — LS-4.j wires the new parser into scrip's production driver.  Next: LS-4.i.4 — alt-eval `(a, b, c)` → `E_VLIST` (pure expression-tier change at expr0 / paren-grouping).) |
