...
| **TEXTF: literal emit_textf templates** | `GOAL-TEXTF-TEMPLATES.md` | one4all+.github | **NEW** drafted 2026-05-21 session #6. Convert every IS_<BE> template arm to one emit_textf(LITERAL, vars) of captured baseline; delete bb3c_format/emit_text_*/emit_bb_x* legacy helpers. 14 groups across 4 phases. **NEXT: G1** (x86/text simple pat: REM/ARB/ABORT/EPS/FENCE). Beauty gate stays SUSPENDED. |
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **PPV-7 ✅ LANDED 2026-05-21 session #6 (Sonnet 4.6, one4all `f6e4968a`).  RPOS/RTAB --compile segfault fixed: nd->c guard in emit_flat_invariant + pre_build_children_text + pre_build_children.  All gates unchanged.  NEXT: PPV-8 WASM arms for bb_rem/bb_arb/bb_fence/bb_abort; beauty gate SUSPENDED per Lon 2026-05-21. |
