...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **SCT-0a** (reading-only session start: SPITBOL manual + parser_snocone.sc + ARCH-SNOCONE.md). Opened 2026-05-17 (Opus 4.7). Eureka path: transpile parser_*.sc → portable .sno → run side-by-side in SCRIP + SPITBOL with existing sync-step IPC monitor. Constraint: only six runtime primitives allowed in parsers (`shift`, `reduce`, `nPush`, `nInc`, `nTop`, `nPop`); no helpers_*.sc. |
