...
| **⚡ Icon BB Native (flat BB templates)** | `GOAL-ICON-BB-NATIVE.md` | one4all+.github | **NEXT: IB-10 — REOPENED. Prior `[x]` did not match code. lower.c still emits SM_RESUME (lines 921/929/1007), SM_STORE_GLOCAL (932/946), SM_SUSPEND_VALUE (1036) on Icon paths (lower_limit_every, lower_every_gen, lower_suspend); is_suspendable() returns 1 for all 8 migrated TT_ kinds. Doc work this session: added `.github/jcon_irgen.icn` (JCON 43 ir_a_* canonical reference), updated ARCH-ICON.md and Goal required-reading. BB tally: 8 of 43 templates landed (ToBy, iterate, Alt, Every, Limitation, bang-Binop, lconcat, Mutual-seq); 35 remain on SM_BB_PUMP_AST path. IB-9 ✅ `3f1c5c87`: SM_BB_PUMP_AST opcode fully deleted.** |
