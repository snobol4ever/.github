...
| **⭐ DYNAMIC BYRD BOX** | DYN-61 | one4all `264da8a` · corpus `2f2bbe3` | **DYN-62**: fix `wordcount` (`&TRIM`/match-replace) + `cross` (INPUT/`&TRIM`) → ≥167p; then `1012_func_locals`/`1013_func_nreturn` |
...
| **⭐ TINY JVM** | J-232 | one4all `7146a6d` | **J-233**: EVAL + APPLY + 1011_func_redefine → ≥165p |
| **SNOBOL4 JS** | SJ-19 | one4all `126ae3e` · .github this | **SJ-20**: fix 1011 (ne(f,1) S=PR split) + cross (NH capture) + 212 ($.var<N>) → ≥170p · M-SJ-INTERP |
...
| **⭐ one4all-SNOBOL4-NET** | D-176 | one4all `a4d40cb` · .github this | **~99p/79f** · static boxes.dll abandoned (Roslyn CS0012 unfixable) · **D-177**: `BoxFactory.cs` via `Reflection.Emit` — MSIL boxes in-memory, no DLL ref → ≥ 151p → word1–4/cross + 1012 → ≥ 160p |
