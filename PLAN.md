...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **REOPENED — NEXT: EM-MODE4-IS-MODE3-DUMP-c** (first template, SM_HALT).  Sub-rung -b ✅ sess 2026-05-11 (Claude Opus 4.7): vtable skeleton landed — `emitter.h` extended with SM-template surface (structural markers, BB-port primitives, formatting, macro hooks); `emitter_text.c` + `emitter_binary.c` implementations; new `emitter_macro_def.c` (thin wrapper, TEXT_MODE_DEFINITION); `src/runtime/x86/templates/` + README; Makefile wired; narrow-vtable shape retained per surface-design tension flag.  Sub-rung -a ✅ earlier sess 2026-05-11.  Gates: smoke 7/7+49/49+5/5; tripwire test_gate_em_beauty_subsystems_mode4 PASS=0 FAIL=17 (all link) — pre-existing libscrip_rt link issue (not a one-liner; 10+ transitive symbols), carved as future housekeeping rung. |
...
| **⚡ sm_lower.c refactor (prereq for SL)** | `GOAL-SM-LOWER-REFACTOR.md` | one4all+.github | **NEXT: SR-15** — doc pass + rename AST param `e`→`t` (tree) throughout lower.c/lower_ctx.h | — silent default fallback → lower_unhandled + explicit per-kind table. Closed: SR-1..SR-13 ✅. **rename ✅ `cc21aa5a`** — sm_lower→lower, cohort_*→lower_*. **SR-9 ✅ `907644e5`** — icn_relop+cset+unary (24 kinds). **SR-10 ✅ `a4545102`** — icn_ctrl+data+sect (19). **SR-11 ✅ `69e7dde8`** — lower_icn_gen (7) + lower_prolog (6); legacy switch now empty. Gate: PASS=30. |
