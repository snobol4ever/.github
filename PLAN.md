...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | Session 2026-05-17h (Opus 4.7): **DAI-5a ✅** (`f7c9cfeb`): swapped all stale `bb_eval_value(tree_t*)` callers to `interp_eval(tree_t*)` (universal AST walker) — 3 in `pl_runtime.c`, 46 in `raku_builtins.c`, 7 ternary collapses in `interp_eval.c::icn_string_section_assign`. Empirical trace pass confirmed zero fires across all 56 sites under smoke + Icon rungs + 3 crosschecks → swapped. **Zero non-stub callers of `bb_eval_value` remain in src/.** Gates: ir-run 194/265, smoke ×6 frontends, crosscheck_prolog 128/0 — all unchanged. DAI-5b (file deletion + helper relocation `icn_str_concat_d`/`icn_lconcat_d`/`icn_proc_as_value` to `icn_runtime.c`), DAI-5c (sm-run re-baseline), DAI-6 (docs) deferred. Prior: DAI-4 ✅ (`e3c803ea`) icn_bb_build callers; DAI-3 ✅ (`a2365c5f`) ICN_BB_EVAL no-op; DAI-2 ✅ (`3e1fc9cd`) bodies → DAI-BOMB stubs (−1641 LOC); DAI-1 ✅ (`b3e08fe8`) dead-on-vine IR_ICN_EVERY. `--sm-run` 194/265 = `--ir-run` 194/265 at parity since DAI-3. |
