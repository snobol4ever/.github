...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **DAI-8 C2 ✅ 2026-05-18 (Opus 4.7, one4all `895ab323`) — emit_core.c + emit_sm.c byte-emit family sweep.** Linker-GC (`--gc-sections --print-gc-sections`) + @PLT-aware filter nominated 472 truly-dead candidates after generated-file exclusion. The two largest clusters (emit_core.o 143, emit_sm.o 73) form a single coherent dead unit — emit_sm.c byte-emit family (`emit_sm_push_lit_s`, `emit_sm_call_fn`, `emit_sm_halt`, `emit_sm_jump_*`, `emit_sm_pat_*`, `emit_sm_arith*`, etc. — the EMIT_BINARY mode path) is dead because `--compile` switched to text-asm output via `emit_sm_*_dispatch` functions; the byte-emit fns are the only callers of most x86-instruction primitives in emit_core.c (`bb_insn_*`, `insn_*`, `bb3c_*`, `emit_seq_*`). Cross-call audit (each candidate's callers tracked through GC-dead unions) confirmed 132 emit_core fns had zero callers outside emit_core; 10 had only-GC-dead emit_sm.c callers; 1 (`emitter_init_macro_def`) preserved because demo_template_productions.c standalone demo uses it. Deletion via /home/claude/delete_dead.py (type-led regex; multi-decl-per-line in emit_core.h left for cluster 3). **Net: −877 LOC across 3 files (emit_core.c 1787→1198, emit_core.h 217→159, emit_sm.c 3315→3085), ~198 functions deleted.** All gates held floor: hello-world matrix mode 4 6/6 PASS-wired ROWS_DRIFT=0; modes 2/3 hello 6/5 (mode-3 raku preexisting from IJ-HELLO-2 era, see latent IJ-HELLO-MODE3-RAKU); smoke ×6 (5/0,5/0,5/0,4/0,5/0,7/0); Icon --interp 194/36/35/265; crosscheck_prolog 128/0/4SKIP/11ORACLE_MISS; scrip_all_modes 2/0 (NET skipped, no ilasm). **+1 bonus**: unified_broker 22→23. **PLAN unchanged: DAI-8 cluster 1's modes-3/4 corpus-wide validation IMPLICITLY CLEARED** as a side-effect of C2's full gate matrix — every mode passed at HEAD `895ab323`. Prior: IJ-HELLO-5 ✅ + IJ-HELLO-4 ✅ 2026-05-18 (four commits `fc04134a`/`411d041c`/`796d688e`/`8a6a5204`) — 6/6 wired hello-world matrix sealed; bb_broker / rt_bb_once_proc / rt_bb_pump_proc structurally impossible in --compile binaries. **NEXT: DAI-8 cluster 3** — see watermark for candidate tally (rt.c 31, icon_runtime.c frontend 25, emit_wasm.c 22, prolog_builtin.c 20, icn_runtime.c interp 18, snobol4_pattern.c 16, stmt_exec.c 15, plus emit_form.h static-inline triplet and emit_core.c::ef_greek_port that DAI-8 C2's regex missed) OR IJ-HELLO-MODE3-RAKU (raku --run broken; mode-4 rt_call fallback ladder needs mirroring in mode-3's sm_codegen_x64_emit path; uncovered during C2 emitter-wiring audit). **Emitter-wiring audit confirmed all 9 emitter source files** (emit_bb.c, emit_core.c, emit_ir.c, emit_ir_targets.c, emit_js.c, emit_jvm.c, emit_net.c, emit_sm.c, emit_wasm.c, sm_codegen_x64_emit.c) are reachable from CLI dispatch; 6-langs × 5-emitters matrix all green. |
