...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`0a906d83`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0. THREE-MEDIUM (TM-1→12) ✅ — but scoped to BB pattern/charset/pl only. **NEW rung THREE-SECTION-X86 (TSX)**: binary is DETOURED around the templates (emit_sm.c forces TEXT_MODE; bytes come from emit_bb.c+insn_*). All 14 SM templates + most XA lack a MEDIUM_BINARY section. TSX gives every PLATFORM_X86 arm three sections (MACRO_DEF/BINARY/TEXT, one return each) with real bytes in BINARY, then TSX-WIRE opens the detour so binary flows through templates. TSX-0 ✅ (audit script `scripts/util_three_section_audit.sh` + root-cause). **NEXT: TSX-SM-1** (sm_halt/calls/pat_anchors/pat_nullary three-section). STRING-CONCAT-ALL (SC-1+) and ER-8 deferred behind TSX. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
