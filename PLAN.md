...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-5 — Control flow** (`SM_JUMP_S`, `SM_JUMP_F`, `SM_LABEL`, `SM_STNO` inline-native lowering + optional `emit_standard_blob_no_stack()` variant for handlers that don't touch the value stack).  Sess 2026-05-10 (Claude latest, fourth session): **ME-4-post-r12-tos ✅ CLOSED, ME-4 closed.**  Full r12=TOS-pointer FORTH-style re-emit landed: sm_jit_run loads `r13 = &SM_State` + `r12 = st->stack`; trampoline reads PC from `[r13+20]`; standard_blob adds r12↔sp sync protocol around C-handler call (28→57B); 10 me4_* blobs shrunk 8-22% (arith 84→69, concat 76→63, coerce_num 56→49, push_null 53→42, push_var 63→52, store_var 76→59); halt 6→5B; jump_skeleton 14→13B.  Audit checklist (REGISTER-LAYOUT.md) verified.  Gates: smoke 7/7, unified_broker 49/49, sn7_beauty 26/25 unchanged baseline, mode-4 tripwire 0/17 unchanged, realloc repro (`X=X+1;Y=Y+X`) byte-identical sm-run vs jit-run, 30-program corpus parser sweep 0 divergences.  Perf: 100k counter loop sm-run 143ms → jit-run 95ms (~34% faster).  one4all `ae7f325a`.  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`, ME-4-post bug-fix ✅ `06b8f503`. |
