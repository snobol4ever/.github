...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-three-column-non-bb ACTIVE sess #91, 2026-05-09** — extend the SM dispatch's three-column format (`label / opcode+args / # comment`, whitespace-aligned, 24/36/N widths) to every non-BB line in the emitted `.s`: `.section`/`.string`/`.byte`/`.long`/`.quad` data lines, static `.data` ζ blocks, `.Lchunk_registry` entries, `_pat_inv_<id>_α/β/γ/ω` blob entry labels, cap/arbno child sub-procs, `main()` epilogue, the `.include "sm_macros.s"` line, and the contents of `sm_macros.s` itself.  Banner lines (`# ====...`, `# stmt N (line L): ...`) preserved full-width.  BB-blob *body* instructions stay out of scope — the next rung `EM-7c-bb-three-column` reformats those to the *different* three-column shape `LABEL: ; ACTION ; GOTO` with literal `;` separators.  Prior rung **EM-7c-sm-three-column LANDED sess #91, 2026-05-09** (one4all `589e56b5`, corpus `cee54dd`, .github `de1ff9a` + this rung's authorship). |
