...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-HELLO-3 — Icon hello-world via mode 4, wired (no `bb_broker`).** IJ-HELLO-2b ✅ 2026-05-18 (Sonnet 4.6): `lower_stmt` in `src/lower/lower.c` now matches `lang == LANG_RAKU && subject->t == TT_FNC && subject->_id == SUB_TAG_ID` BEFORE the general pattern/subject path, lowering body kids `c[nparams+1..]` directly via `lower_expr` + `SM_VOID_POP` and skipping the spurious wrapping `SM_CALL_FN <name>` emit. SUB_TAG_ID hoisted from a local `#define` in `raku.y` to `frontend/raku/raku_driver.h` for cross-file visibility; raku.y line 201 `e->_id = 0;` removed (was clearing the tag); synthetic-main wrapper at raku.y line 213 now gets `mf->_id = SUB_TAG_ID;` for symmetry. raku.tab.c regenerated via `regenerate_parser_and_lexer_from_sources.sh`. **Hello-world matrix flipped:** raku row PASS-wired (was FAIL-run/Error 5); `HW_EXPECTED_PASS=4 HW_EXPECTED_FAIL=2`. Mode-2 (`--interp`) bonus: rc=1+Error 5 → rc=0 clean. +18 net LOC hand-edited. All gates held floor: Icon `--interp` 194/265; smoke ×6 5/0,5/0,5/0,4/0,5/0,7/0; broker 22/27; crosscheck snobol4 5/1, icon 4/0, raku 21/12, rebus 4/0, snocone 8/0. **IJ-HELLO-2 COMPLETE** (both 2a Opus 4.7 + 2b Sonnet 4.6). IJ-HELLO-2a ✅ 2026-05-18 (Opus 4.7): `rt_call` in `src/runtime/rt/rt.c` chains `icn_try_call_builtin_by_name` before `INVOKE_fn`. IJ-HELLO-1 ✅ 2026-05-18 (Opus 4.7): `test_smoke_compile_hello_all_langs.sh` landed. (Lon directive 2026-05-18: *"Ensure compiled Icon/Prolog use flat/wired BBs. Do not use brokered BBs in `--compile` mode. Get all six languages to say hello world before continuing mass dead code removal."*) **DAI-8 PAUSED** behind this step. **Prior:** DAI-8 cluster-1 retroactive mode-3/4 validation ✅ (Opus 4.7) — byte-identical pre/post `a4fe1c21`. **DAI-7 ✅** 2026-05-17 (Opus 4.7) — IJ-DEL-ICN-AST arc CLOSED. **Combined DAI-7 net: −608 LOC `icn_runtime.c` (1259→651).** |
...
| **⚡ Parser-SC Transpile (2-way harness via Snocone→SNOBOL4)** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **NEXT: SCT-9-zero-output** — diagnose why transpiled parser_snocone.sno runs cleanly under SPITBOL on arith_add.sc but produces no TDump output. 2026-05-18 (Opus 4.7): notmatch-dedup ✅ — deleted redundant `notmatch` from parser_snocone.sc and icon_helpers.sc (canonical home is match.sc). Cross-language SPITBOL sweep moved from 3-of-6 to **4-of-6 clean** (+ icon). Fixture-gate rebaselined: actual PASS=60 FAIL=0 SEGV=7 (prior watermark of 24/2/41 was stale). SCT-9h downgraded to verify-or-close — `($'(') = pat` bug not reproducible against current SPITBOL. raku/prolog still hit SCT-5/6 helper-elimination walls. Smoke gates hold: snobol4 7/0, snocone 5/0. one4all `996f03e0` (no touch), corpus this session. Prior: SCT-9g-snocone audit + n-ary arith fix 2026-05-17. |
