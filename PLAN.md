...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1 (LS-0 LANDED, session 2026-04-30: Lon's directive captured — new Snocone is a **full SPITBOL superset** with C/C++ control flow, juxtaposition concat (no `&&`), alternative-eval `(,,)` (no `||`), backtracking-expression conditions (SUBJ ? PAT = REPL legal in `if`/`while`/`do`-`while`/`do`-`until`/`for`-test/`case`-tag), full SNOBOL4 OPSYN, Flex+Bison implementation. SPITBOL Ch.15 precedence table extracted verbatim from manual pp.181–183; Koenig `bconv[]` table extracted from `snocone.sno` L600–627; six disagreements identified and resolved (drop `&&`/`||`/`==`/`!=`/`<`/`<=`/`>`/`>=`/`%`/`:==:` family — use SPITBOL primitives instead). 10 open design questions Q1–Q10 resolved with default decisions (strict `f(` zero-space rule; alt-eval `(,)` replaces `||`; juxtaposition replaces `&&` in conditions; `for(init;test;step)` C-style; `do/while`+`do/until`; `switch` with implicit break; `;` mandatory; `//` and `/* */` comments; no compatibility flag — atomic flip; name "Snocone" kept through LS-7). Working name "Snocone" (rename TBD). Andrew Koenig 1978 source preserved as reference (uploaded `SNOCONE.zip`); directive overrides Koenig where they conflict. **Next LS-1**: lexer state-machine specification + 50-line test corpus with expected token streams, no code yet — Lon reviews before LS-2 grammar design.) |
