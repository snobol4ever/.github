...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-INLINE-GROUP active** (Lon directive session #N+2: grouped templates — one fn per opcode-GROUP, opcode via `g_emit.instr->op` switch; supersedes pure one-fn-per-opcode INLINE-ALL). 6 grouped templates landed: `sm_arith` (5), `sm_compare` (2), `sm_pat_nullary` (22), `sm_jump_group` (3), `sm_misc_nullary` (7), `sm_incr_decr` (2). 41 opcodes in 6 fns. one4all `8aa204c2`. GATE-PK PASS=420 FAIL=0 STUB=639 throughout. All HQ-listed "ready to group" entries cleared. **NEXT: INLINE-4a-inline** — per Lon end-state (zero template helpers, sprintf-on-globals in each template TU), inline `emit_sm_<op>_dispatch` bodies into the ~16 still-individual templates' IS_X86 arms via `bb3c_format` + per-shape sprintf. Renderer + `sm_op_template_t` table to be DELETED, not promoted. Targets in shape order: LBLOPT (LIT/REFNAME/USERCALL) first, then LBLOPT_INT32 (CAPTURE/USERCALL_ARGS), then LBL/LBL_INT32 family (push_var/store_var/push_lit_s/call_fn/bb_once/bb_pump), then NOOP (DEFINE family), then EXEC_VAR (exec_stmt), then PCREF (push_expression/call_expression). After ~16 templates inlined, sweep emit_sm.c (delete renderer + table + dispatchers). Pre-req: tiny `emit_strtab.h` exposing `strtab_label` accessor (Layer-3, mirrors `wasm_intern_str` pattern), or inline the table lookup. Then INLINE-3-GROUP for BB-side. |
...
| **⚡ Parser-SC Transpile** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **NEXT: SCT-BEAUTY-SC-PARSE** — beauty.sc transpiles cleanly (2158 lines, 0 placeholders) and SPITBOL `-bf` runs the .sno with zero errors, but parser reports "Parse Error" on simplest input. Trace shows `Push(Label) Shift(Label, )` then Parse Error, where oracle next produces `Push(Id) Shift(Id, x)`. Stmt's outer FENCE `*White *Expr14` branch silently fails. Carry-over `epsilon . ''` → `shift(epsilon, '')`, `EVAL('upr(tx)')` → `*upr(tx)`, removed bogus `DEFINE('LEQ')`/`DEFINE('Ucvt')` from Qize.sc landed corpus `f3b8afb`. lower_sno.c TT_FOR PST 4-child + TT_IDX n-ary/indirect-base fixes landed one4all `01577f1a`. SCT-SN4-ERR041 still open (parser_snobol4.sc Stmt nInc overcount). |
