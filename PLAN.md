...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **IR-RN-1 ✅ COMPLETE 2026-05-20** (lower.c audit clean — no old-name leakage). **IR-CONSOLIDATE-DCG steps 1-3 ✅ COMPLETE 2026-05-20** (one4all `419fce29`): + `int dcg_idx` field on `IcnProcEntry`/`Pl_PredEntry_BB`, populated during lowering, strangler inline helpers `bb_graph_of_proc`/`bb_graph_of_pred` in headers. Mode-4 standalone-binary carve-out documented. All gates baseline. **NEXT: IR-CD-RENAME** — survey complete, Category A (119 occurrences of `dcg_*` misnaming the BB-graph table) rename to `bb_*` BEFORE IR-CD-4 (consumer migration). Then IR-CD-4/5/6/7 (migrate, delete `ir_body`, doc, close). IR-RN-2/3 emitter+runtime audits still owed before EC-UNI-3-beauty.
