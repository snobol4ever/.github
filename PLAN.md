...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-6.c (session 2026-05-01 #7: LS-7 documentation pass LANDED + LS-5.d corpus syntax-conversion LANDED. LS-5.d: 61 files migrated `procedure` → `function` (297 replacements via new `util_migrate_snocone_procedure_to_function.py`); 6 hand-fixes in `programs/snocone/corpus/` (removed `then` from sc4/sc9, removed erroneous `do` from `while` in sc5/sc6/sc10, rewrote alt-eval `(a,b)` to juxtaposition concat in sc8, corrected stale `.ref` for sc9 vs SPITBOL oracle); `snocone_lex.c` KW_TABLE `procedure` synonym removed — bare `procedure` now a syntax error. snocone/corpus 5/10 → 10/10 PASS. Production gates remain green: smoke snocone 5/0, beauty 42/0/3, broker 49/0, smoke snobol4 7/0. LS-6.c REMAINS OPEN, GATED on GOAL-SNOCONE-BEAUTY SB-6.D `EVAL(EXPRESSION)` double-wrap fix at beauty.sc Reduce call site. After LS-6.c lands, GOAL closes and `GOAL-LANG-SNOCONE` advances D-1 → D-2.) |
