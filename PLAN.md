...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-symbolic-runtime-correctness LANDED sess #79, 2026-05-07** — fixed blob α entry label placement in `flat_emit_body_v`: external `_pat_inv_N_alpha` was placed after the r10-setup preamble so calling it directly skipped preamble (r10=garbage, match always failed, output=`abc`). Fix: external alpha label moved before preamble; internal `alpha_body` label for dispatch target. Test 13 extended with runtime sub-test (binary output=`abXc` matches --jit-run). EM gate PASS=12, all smokes PASS. **Next: EM-7c-variant** (variant pattern nodes via Phase-2 SM ops + bb_pool), then EM-7d (beauty oracle). |
