...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **EC-UNI-15 CLOSED 2026-05-20** — top-level shape audit: 450/450 matrix cells, 90 fns, 0 misses; new `scripts/test_gate_ec_uni_15_audit.sh` records fn-size distribution and queues EC-UNI-16 candidate list (8 fns ≥60 lines: `bb_arbno`, `bb_lit`, `sm_suspend_value`, `bb_cat`, `sm_call_fn`, `bb_tab`, `bb_alt`, `bb_capture`).  **NEXT: EC-UNI-16** (Layer-2 helper extraction — start with the sm_calls.c byte-near-duplicate pair) OR **EC-UNI-21-followup** (re-converge or retire M1 oracle md5).  See `GOAL-HEADQUARTERS.md` watermark. |
