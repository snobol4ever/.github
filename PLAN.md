...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`48497c58`**. Formatter deletion complete: bb3c_format, emit_three_column_line, fmt_body/flush cluster, emit_bb_is_format_mode all deleted. IS_MACRO_DEF added to all 27 SM templates; IS_X86 narrowed. emit_sm_macro_library wired to template dispatch. GATE-PK 420/0/639. **NEXT: delete render_macro_body + g_sm_templates[] + sm_template_lookup + sm_op_template_t + emit_sm_template + related machinery (INLINE-6/E3). Then delete emit_macro_begin/end + EMIT_MACRO_DEF/EMIT_TEXT_INLINE.** |
