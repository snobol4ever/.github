...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-REWIRE — blocked on Lon design choice (2026-05-21 PM)** — BB-side LIFT done through slice 7 (one4all `045baf4a`).  SM-side audit reclassifies all 36 fns as already-lifted-as-helper.  Methodical trace of the rewire (route x86 through `emit_bb_node`) found TWO design forks: (1) text-vs-binary in JIT — `g_emit.lbl_*` are name strings by Snocone-bootstrap design, binary mode needs name-keyed primitives not yet built; (2) **format drift** — templates use `emit_text_jmp`/`emit_text_label` while originals used `bb3c_format(out, "", "jmp", target)` directly, producing DIFFERENT text column layouts.  Three sub-paths: (a.1) re-lift templates verbatim to byte-faithful, (a.2) accept format drift + re-baseline EC-UNI-21 md5, (b) build name-keyed binary primitives first.  **Recommendation:** Path (a.2) — the assembled `.o` md5 (semantic) stays identical; only the text presentation changes; re-baseline once, move forward.  See GOAL-HEADQUARTERS.md EC-UNI-REWIRE section for full trace.  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + assembled `.o` `3adbb73f88edcc5416d38baade6faf97` + matrix 855/855 stable on `045baf4a`.  No code committed this session — HQ docs only. |
