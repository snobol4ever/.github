...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **GOAL CORRECTIONS sess #86, 2026-05-09** — Lon corrections recorded in goal watermark + step bodies: (1) next rung is **EM-7c-sm-macros** (SM-side asm macros, parallel to historical `snobol4_asm.mac`; SM opcodes emit as flat macro calls; macros are SM-only — BB stays as straight x86) NOT EM-7c-bb-three-column; (2) BB-port labels in generated code are Greek-only — no `bb`/`BB` prefix or suffix — `α3`/`β3`/`γ3`/`ω3` (NOT `bb3_α` etc.), with `_pat_inv_<id>_` as the only cross-blob namespacing prefix; (3) three-column form uses literal `;` as the column separator (one logical asm line per BB-port emission: `LABEL: ; ACTION ; GOTO`). EM-7c-bb-three-column rung body rewritten to encode all three rules; new EM-7c-sm-macros rung inserted before it in the step list with full body. **EM-7c-greek-purge LANDED sess #86, 2026-05-09** — re-attempt of the purge abandoned in sess #85, with corrected audit grep including `_alpha\|_beta\|_gamma\|_omega` substrings (orig grep missed those because `_` is a word char). 160 hits across 12 source files + 9 in scripts; final audit zero hits. C identifiers (`lbl_β`, `α_lbl`, `child_α_label`, `blob_α`, `save_Σ/Ω/Δ`) and emitted asm labels (`_pat_inv_<id>_α/_β/_γ/_ω`, `_cap<n>_child_α`, `_arbno<n>_child_α`) all use Greek. gcc 13 + GAS accepted UTF-8 identifiers and labels; emit→assemble→link→run pipeline produces correct runtime output (Test 13 `abXc` matches `--jit-run` oracle). Smoke ×6 + EM 12/12 + isolation + bb_flat_text 18/18 + sim 25/25 + unified_broker 49 PASS. 4 of 5 tracked .s artifacts changed (label renames only; wordcount.s unchanged — no invariant blob). NO `git stash` used. one4all @ ca704e95, corpus @ 4404b2f. **EM-7c-sm-macros LANDED sess #88, 2026-05-09** — sess #87's WIP architectural work brought to landing in sess #88 with the gate-script update + tracked-artifact regen + commit under LCherryholmes that sess #87 ran out of context to do. Architecture per LCherryholmes directive ("absolute guarantees that no drifting is even possible"): `src/runtime/x86/sm_emit_template.{h,c}` introduce `sm_op_template_t` + `g_sm_templates[]` table (~55 entries; 17 arg-shape kinds) driving paired `render_macro_body()` / `render_call_line()` switches — ONE source of truth for the GAS macro library AND the per-call dispatcher; drift impossible by construction. Every `emit_sm_*` function in `sm_codegen_x64_emit.c` refactored to use template wrappers; old `emit_pat_call_*` helpers deleted; `sm_emit_macro_library()` called at top of every emitted .s. No `sm_macros.s` file on disk — macro library generated live from the table. **Byte-identical .o invariant verified** across all 5 demo programs vs parent ca704e95. Gates 10/10 GREEN: smoke ×6 (7/7+5/5+5/5+5/5+5/5+4/4), isolation PASS, EM 12/12, sm_phase2_sim 25/25, bb_flat_text 18/18, unified_broker FLOOR_FAIL=0. Sess #88 closing work: 5 textual assertions in `scripts/test_smoke_jit_emit_x64.sh` updated to match the new macro shape (was looking for inline x86 like `movabs rdi, 42`, `jmp .Lpc`, `jz .Lpc`, `jnz .Lpc1`, `call .LpcN` in the dispatch loop body — those instructions now live inside .macro bodies, and call sites read `SM_PUSH_INT 42`, `SM_JUMP .LpcN`, etc.). Tracked artifact line counts: roman+200, wordcount+195 (small programs grow due to fixed ~200-line macro library at top), claws5-204, treebank-list-270, treebank-array-390 (large programs shrink ~10-15% from macro reuse); net -469 lines. Watermark prediction "~10× shrink" was overstated — the win shows at scale, not on tiny programs. one4all @ 92b922ca, corpus @ 5a0b181. WIP branch `wip/em7c-sm-macros-sess87` deleted post-push. Order from here: EM-7c-greek-purge ✅ → EM-7c-sm-macros ✅ → **EM-7c-bb-three-column** → EM-7c-bb-macros → EM-7d. |
| CHUNKS Step 17 (proc/pred → entry_pcs) | `GOAL-CHUNKS-STEP17.md` | one4all+.github | **CH-17a LANDED sess #75, 2026-05-07** — scaffolding: entry_pc fields added to IcnProcEntry + Pl_PredEntry (init -1); sm_resolve_proc_entry_pcs walks both tables after sm_lower; gates byte-identical. **CH-17b LANDED sess #75, 2026-05-07** — sm_lower emits named-chunk SKELETONS for every Icon/Raku proc (forward-jumped, empty body). Resolver now finds non-(-1) entry_pcs end-to-end. Body lowering split into CH-17b'. **CH-17b' LANDED sess #78, 2026-05-07** — proc-body lowering. Per-proc emission loop in sm_lower.c walks `proc->children[1+nparams..]` and calls `lower_expr` + `SM_POP` per body child; trailing `SM_RETURN`. Chunks now contain real lowered SM ops (palindrome.icn: chunk 1–52 holds full palindrome body; Raku rk_given: 3 procs at 1/81/161 with substantial bodies). Chunks remain unreachable (coro_call still walks IR; forward-jumps skip every chunk). File-static `g_chunk_body_lowering` flag silences lower_expr's "unhandled expr kind" stderr warning during proc-body emission only. Frame-slot resolution stays at runtime (E_VAR → SM_PUSH_VAR by name). Gates byte-identical: smoke ×6, isolation, Budne PASS=61, Icon corpus 186/47/30 of 263, unified_broker PASS=49. Documented in `docs/CHUNKS-step17b-prime-validation.md`. Next: **CH-17c** (flip coro_call consumers via entry_pc; add sm_call_proc). **CH-17c LANDED sess #82, 2026-05-07** — `sm_call_proc` added; `proc_trampoline`/`gather_trampoline` dispatch via `sm_call_proc` when `entry_pc >= 0`; `nparams` added to `IcnProcEntry`; `gather_entry_pc`/`gather_nparams` to `coro_t`; E_FNC lowering fixed for Icon-style nodes. Gates byte-identical: smoke ×6, isolation, Budne PASS=61, unified_broker PASS=49, Icon corpus 186/47/30 TOTAL=263. **CH-17d LANDED sess #83, 2026-05-07** — `sm_lower.c` emits named-chunk skeletons for every Prolog predicate in `g_pl_pred_table` (SM_JUMP+SM_LABEL+SM_RETURN+SM_LABEL, forward-jumped over); `pl_runtime.h` include added. `SCRIP_PROC_ENTRY_PCS=1` confirms non-(-1) entry_pcs: palindrome/2@1, main/0@5. Consumer dormant. Gates byte-identical: smoke ×5 PASS, isolation PASS, unified_broker PASS=49, Budne PASS=61, Icon corpus 186/47/30 TOTAL=263. **CH-17e LANDED sess #84, 2026-05-07** — `pl_box_choice_pc` added to `pl_broker.h/c`; `pl_pred_entry_lookup` added to `pl_runtime.h/c`; 5 consumer sites flipped (interp_eval.c x2, pl_runtime.c x2, interp_hooks.c, polyglot.c). Chunks skeleton-only (CH-17d SM_RETURN); routing established, no crash. Gates byte-identical: smoke x5 PASS, isolation PASS, unified_broker PASS=49, Budne PASS=61, Icon PASS=186/47/30. Next: **CH-17f** (fill Prolog chunk bodies; --sm-run Prolog end-to-end). **CH-17f LANDED sess #85, 2026-05-07** — `SM_BB_ONCE_PROC` opcode added; `lower_stmt` LANG_PL + `lower_expr` E_CHOICE emit it instead of `emit_push_expr + SM_BB_ONCE`; `--sm-run` Prolog no longer FATALs (hello.pl, roman.pl produce correct output). Predicate chunk bodies remain skeleton-only; chunk body fill deferred. Gates: smoke ×6 PASS, isolation PASS, Budne PASS=61, unified_broker PASS=49, Icon 186/47/30 TOTAL=263. one4all @ `a2c6c089`. Next: **CH-17g** (drop `EXPR_t *proc` from `IcnProcEntry`; lift `code_free` gate). Sub-goal owns rungs CH-17a..CH-17h; M4 cleanup converges here. |
