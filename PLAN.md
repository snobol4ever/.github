...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.i.5 (session 2026-05-01 #5 LANDED LS-4.i.4 at one4all `441731f3`: alt-eval `(a, b, c)` → `E_VLIST`.  Three rule additions in `expr17` atoms tier of `snocone_parse.y`, mirroring `snobol4.y:194-196` byte-for-byte: (1) `T_LPAREN expr0 T_COMMA exprlist_ne T_RPAREN` constructs n-ary E_VLIST, draining children from the exprlist_ne E_NUL container; (2) `T_LPAREN T_RPAREN` → E_NUL (empty-parens canonical form); (3) pre-existing `T_LPAREN expr0 T_RPAREN` → `$2` single-expr grouping stays intact — Bison's LR(1) lookahead at the token after expr0 (T_RPAREN for grouping vs T_COMMA for VLIST) disambiguates without conflict.  **Zero IR / runtime / lowering changes** — `E_VLIST` was already defined at `src/ir/ir.h:83-87` ("Goal-directed value-context disjunction. N-ary. SPITBOL `(a, b, c)` paren-list and Snocone `||`. Distinct from E_ALT") and three downstream consumers (`sm_lower.c:631`, `interp.c:3454`, `interp.c:4531`) already handle it.  No new tokens — `snocone_parse.tab.h` byte-identical pre/post regen, only `.tab.c` table sizes shifted.  Tests added (46–58, +56 assertions): headline 3-arity, minimum 2-arity, single-expr grouping regression guard, empty parens, SPITBOL Manual Ch.15 footnote example with concat sub-expressions verified through to E_SEQ child shape, nested VLIST (proves it's NOT a left-recursive fold), VLIST as if-cond and while-cond (LS-4.f emit-and-splice carries through), VLIST as binop operand, VLIST distinct from E_ALT (`(a | b, c)` outer is E_VLIST(2) with E_ALT child), call (E_FNC) NOT VLIST regression guard, VLIST as bare-expr stmt, all-literal VLIST.  Combined parse-a..i delta from LS-4.i.4 close: **871/871 → 927/927** (+56).  All gates green: smoke 5/5, beauty 42/0/3, broker 49/0, parse-a..i 927/927.  Bison: 0 shift/reduce, 0 reduce/reduce.  Side-channel only — LS-4.j wires the new parser into scrip's production driver.  End-to-end smoke verified through 7 realistic shapes including Andrew's Q2-canonical `if ((DIFFER(x), DIFFER(y))) z = 1;` or-cond replacement.  Next: LS-4.i.5 — `struct NAME { field, field, ... };` per Andrew `.sc` line 162.) |
