...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **PRF-13** — rewrite GatherBlock in parser_raku.sc to pure reduce('TT_GATHER','nTop()'); create raku_helpers.sc with gather_hoist_pass(). Then PRF-12-sub/class/for/self (lower.c discrete sessions). Clarification 2026-05-18: grammar actions in raku.y are correct CFG behavior — PRF-12 items are lower.c interface work (lower reads _id fifth-field; TT_FOR/TT_SUB_DECL/TT_CLASS_DECL missing). crosscheck_raku 23/10 (was 21/12). |
