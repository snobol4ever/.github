...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | PARSER-FAMILY-LOOP iter#6 LANDED (session 2026-05-05): use `$'X'` token variables in productions + full Snocone operator-token table (Step 3f from `GOAL-PARSER-SNOCONE.md`). Rule for dual-use operators: binary form is canonical (`$'X' = $' ' 'X' $' '`); at unary sites, sprinkle just `$' '` before raw `'X'`. Iter#5 (pair-shape + bare names) landed @ corpus@5a602ff. Old one-letter-prefix iter#5 SUPERSEDED. Gates all 100%: SN=59 IC=51 PR=54 RK=32 SC=21 RB=38. Next: operator picks iter#7 concept. |
