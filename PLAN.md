...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **NEXT: EC-PA-1** — globals-based ctx (drop `const sm_ctx_t *ctx` parameter; templates read `g_emit_pc`/`g_jvm_in_body`/`g_net_pc_to_fn` etc. directly; producer sites set globals before each call).  Phase A is 8 rungs ending in emit-to-strings (EC-PA-7) and Phase B handoff (EC-PA-8).  Snocone is the bootstrap target — design choice ratified 2026-05-20.  IR-CONSOLIDATE-DCG ✅ COMPLETE (IR-CD-5/6/7 landed 2026-05-20).  IR-RN-3 ✅ (2026-05-20).  **ST2-1b ✅ COMPLETE** (2026-05-20) — all six shim macros retired (final cluster: proc_table/proc_count, 127 sites across 10 files, `27ad177b`).  Open siblings: ST2-1c (dynamic arrays), ST2-2 (link-time isolation gate), ISO-4 (scrip_parse subprocess), IR-RN-4/5 (arch docs + close-out).  Watermark `27ad177b`. |
