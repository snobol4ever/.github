...
| **⚡ Parser-SC Transpile** | `GOAL-PARSER-SC-TRANSPILE.md` | one4all+corpus+.github | **NEXT: SCT-SN4-ERR041 (Stmt nInc overcount)** — ERROR 041 on multi-stmt fixtures. Root cause: `nInc()` before `*StmtRepl` in Stmt's inner FENCE overcounts because `reduce('TT_EQ',2)` inside StmtRepl already consumed the Expr14 child (item 2). Fix: study beauty.sno Stmt pattern (line ~229) and transplant correct nInc placement. Command fix (nInc inside FENCE alternates) already landed in corpus but NOT committed. Run beauty gate first. one4all `4decab7d`, corpus uncommitted diff on parser_snobol4.sc. SN4 PASS=49 PARSE_ERROR=24 OTHER_FAIL=15 / 88. |
