...
| SNO treebank-list | `GOAL-SNO-TREEBANK-LIST.md` | one4all | TL-2 IN PROGRESS: ROOT CAUSE FULLY DIAGNOSED — `bb_arbno` missing NAM_mark/NAM_rollback_to (sibling of SN-6 Bug #1d); fixture is treebank.input (not VBGinTASA.dat — oracle errors on it); trial fix in bb_boxes.c makes treebank-list PASS byte-clean but regresses demo_claws5 PASS→crash; fix reverted; next session: refine ARBNO rollback to not wipe legitimate empty-success iteration effects; HEAD 9a43cddd | ☐ |
