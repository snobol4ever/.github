...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`3529f907`**. Session ~12: IS_X86-STRUCTURE sweep — fixed if/else spine in 3 BB + 11 SM templates (IS_MACRO_DEF/IS_BIN now always subordinate to IS_X86). Added GOAL steps 4–8 (DEAD-EMIT-AUDIT, BIN-INTO-TEMPLATES, IS_X86-STRUCTURE ✅, STYLE-ONE-SPACE, STYLE-NO-LOCAL-SHADOWS). GATE-PK 407/0/647. **NEXT: STYLE-ONE-SPACE (step 7) or STYLE-NO-LOCAL-SHADOWS (step 8) or EC-UNI-23 or BREAKX-2.** Uncommitted IS_X86-STRUCTURE changes in one4all — commit at next session start. |
