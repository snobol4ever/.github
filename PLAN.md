...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: -s (BB templates — fix all six BB templates to pure t_* calls).**  -r ✅ sess 2026-05-11 (Claude Sonnet 4.6, one4all `d8fec035`): SM_PAT_* all opcodes (sm_pat_nullary.c, sm_pat_lbl.c, sm_pat_capture.c, sm_pat_capture_fn.c). All SM templates now complete. |
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **SL-2 next: emit_goto. SL-1 ✅ `3de3f44`. Sess 2026-05-11: lower.c full refactor pass `aac9cb32` (−65 lines): p-alias elimination, emit_lhs_store, emit_pat_nary, emit_prolog_call, emit_proc_stub, emit_thunk unification. lower.c ready to serve as sm_lower.sc reference.** |
