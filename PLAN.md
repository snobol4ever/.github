...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all | **EM-FORMAT-BB-LAW — handed off for Lon's visual review (3rd pass, sess 2026-05-09).**  This session: rule re-read with Lon, no code change.  Rule clarified: cond-branches (`je`/`jne`/...) are opcode+args in cols 2+3, only uncond `jmp` is the col-4 jmp.  A line is "by itself" only when one column has content.  Audit against current tree (one4all `ff416eee`, corpus `7d92fed`): 0 lone labels, 0 lone jmps, 0 jmp-paired-only-with-jmp across all 5 artifacts.  Awaiting Lon's "looks nice" sign-off. |
