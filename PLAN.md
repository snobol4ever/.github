...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-PER-KIND-DIFF ✅ LANDED + GEOMETRY-CORRECTED + GATE-CADENCE-REVISED (2026-05-21 evening, Opus 4.7, one4all `9b905d26`).**  Harness operational: `tools/emit_per_kind_audit.c` + `scripts/{freeze_per_kind_baseline,test_per_kind_diff,normalize_per_kind_cell}` + `baselines/per_kind/` (3.4 MB committed).  **5 backends** (x86, jvm, net, js, wasm) × sub-modes (x86 has text/binary/text_macro; JVM and .NET may grow binary later; JS and WASM text-only).  1059 cells per run (679 BB + 380 SM); current baseline PASS=399 STUB=660 FAIL=0 NEW=0 GONE=0.  Regression detection verified — inject canary → FAIL>0 exit=1; restore → FAIL=0 exit=0.  **Gate cadence change (Lon directive):** `test_per_kind_diff.sh` is now the primary per-slice invariant (~5–10s); matrix + beauty + smoke demoted to session-end / escalation-only.  **2026-05-21 session #2/#3 (Opus 4.7) — pat-keyword reachability story corrected:** REM/ARB/FENCE/FAIL/SUCCEED/ABORT/BAL are PATTERN-typed VARIABLES (verified via DATATYPE — SPITBOL and scrip match).  Bare `'hello' REM` is canonical portable form (PAT_DEREF runtime).  3 real bugs filed: HQ-BUG-RPOS-COMPILE-SEGFAULT, HQ-BUG-RTAB-COMPILE-SEGFAULT, HQ-BUG-PROTECTED-PATTERN-VARS.  **2026-05-21 session #3 handoff: NEW RUNG EC-UNI-PROTECTED-PAT-VARS landed (PPV-0..PPV-7).**  Recognize the 7 pat-names as protected (SPITBOL ERROR 042) and substitute SM_PAT_<kind> at lower-time for bare names in pattern context — closes HQ-BUG-PROTECTED-PATTERN-VARS AND unlocks BB-lower coverage 4→8 active pat-kinds from portable corpus.  **NEXT CONCRETE: PPV-1** (runtime protection in rt_nv_set; PPV-0 inventory ✅ landed 2026-05-21 session #4, Opus 4.7 — deliverable `PPV-0-INVENTORY.md`; substitution site confirmed `lower.c:373`, protection site confirmed `rt.c:467`, error primitive `sno_runtime_error(42, NULL)`).  Each a separate commit; gates on test_per_kind_diff.sh + matrix + beauty (md5 may change at PPV-5, Lon decision required there).  Other openrungs: EC-UNI-REFAITH (independent), EC-UNI-REWIRE-ALL (Path a.1/a.2/b choice pending).  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 stable. |
