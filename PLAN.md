...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: fix Error 3 in smoke_lower then SL-5.** SL-2/3/4 ✅ corpus `c06defc`. Housekeeping sess 2026-05-11 (Claude Sonnet 4.6): `sm_lower_driver.sc`→`lower_driver.sc`; all SCRIP/*.sc comments stripped; blank lines removed; `/* ==== */` separator between functions; function names aligned to C source (emit_pat_nary, stmt_attr_*, attr_int_of, attr_expr_of, lower). Error 3 in smoke_lower run (stmts 36,40,44) not yet fixed — fires inside lower(), suspected c(t)[i] on childless tree node. Long lines in parser_rebus.sc predate this session (confirmed via git log). |
