...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all | **EM-FORMAT-BB-LAW — STILL OPEN.**  Two attempts in sess 2026-05-09 did not satisfy Lon: (a) all-jxx-in-col-3 (one4all `4ada754d`) — wrong, no col-4; (b) 4-tab BB layout with cond@vcol24, jmp@vcol68 (one4all `ff416eee`, in tree) — also wrong per Lon's handoff review.  See goal file rung block "⛔ NEXT AGENT" section for hypothesis list and re-orientation steps.  **Ask Lon for clarification before a third attempt.** |
