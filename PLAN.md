...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | EM-7-pre LANDED sess #71 with WRONG Phase-3 architecture (brokered/driven BB inside libscrip_rt.so via scrip_rt_pat_*@PLT + bb_broker — Lon course-corrected post-handoff). EM-7-pre's SM-side keepers survive: SM_CALL, SM_CONCAT, SM_PUSH_NULL, SM_COERCE_NUM, all 8 conditional return variants, STRTAB 8192/VSTACK 65536 caps. **Next: EM-7-revert** (tear out brokered Phase-3 from emitted-code path), then EM-7a (PATND_t bridge from SM Phase-2), EM-7b (bb_flat.c EMIT_TEXT parity), EM-7c (wire flat-or-runtime-build into emitter), EM-7d (beauty oracle gate). See GOAL-MODE4-EMIT.md "Design Discoveries" section for the corrected five-phase model + reuse map of proven BB infrastructure. |
...
| **PARSER-RAKU (pattern frontend)** | `GOAL-PARSER-RAKU.md` | corpus+one4all | RK-23 LANDED PASS=126 FAIL=0 corpus@838304e — full raku.y gap closed (elsif fix, repeat, for-noarrow, typed decl, arr/hash set, field write, raku_new, say_fh, LitFloat). Next: RK-24 |
