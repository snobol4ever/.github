...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: DAI-8 — whole-codebase dead-code elimination across `src/`** (Lon directive 2026-05-17: "everything not reachable from SCRIP's main is unreachable and therefore we want deleted; I'll be analyzing the code base and do not want to waste my time on code that does not matter"). Six methods (linker `--gc-sections` authoritative; cppcheck, cflow, nm, DAI-BOMB stubs, grep cross-check), DAI-BOMB before delete, orthogonal-cluster commits, ledger in goal file. CLI-3M-9 territory (`interp_*.c` big rip) is the largest expected cluster. **DAI-7 ✅** 2026-05-17 (Opus 4.7) — IJ-DEL-ICN-AST arc CLOSED. DAI-7a (`254dedb9`) deleted 14 empirically-dead Icon C-BB zeta-fns + 10 self-contained state structs from `icn_runtime.c` (−560 LOC); these violated the ZERO-C-BYRD-BOX absolute rule and were preserved by DAI-2 as "infrastructure" but were silently dead post-amputation. DAI-7b (`ff3d100a`) deleted all 3 [DAI-BOMB] stubs (`bb_eval_value`, `icn_bb_build`, `bb_exec_stmt`) + `icn_lazy_box` + `find_leaf_suspendable`. Per Lon directive ("delete anyway if truly dead"), `icn_lazy_box` was the surviving caller of `bb_eval_value` and a RULES-named permitted shim — but had zero external callers, so the named slot was reserving space for a non-inhabitant. RULES.md ABSOLUTE RULE + GOAL Invariant 2 updated: only `icn_bb_dcg` remains as a permitted C-BB infrastructure-shim survivor. **Combined DAI-7 net: −608 LOC `icn_runtime.c` (1259→651), −10 `icon_gen.h`, −8 `emit_bb.c`, −2 `icn_runtime.h`.** All gates held floor: Icon `--interp` 194/265, smoke ×6, broker 22/27, crosscheck_prolog 128/0/4SKIP/11ORACLE_MISS, zero DAI-BOMB fires (stubs no longer exist). Prior session 2026-05-17j: DAI-5c ✅ + DAI-6 ✅. |
