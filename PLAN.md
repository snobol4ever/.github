...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-5b (session #70: hang root cause is NOT grammar/ARBNO/&FULLSCAN — it is a Snocone runtime bug. Minimal reproducer (9 lines, no beauty.sc): `procedure foo(x) { if (x) { x = 0; } return; } v = 'X'; if (v ? POS(0)) ...` HANGS in the C runtime indefinitely. Defining a procedure whose body contains `if` or `while` (which lower to `:goS L.N :goF L.N` conditional-jump labels in the IR) corrupts state such that any subsequent pattern match — even POS(0) — spins. Pure-context (no proc) Id match works in 5 stmts. Bisected: global.sc line 34 `procedure define_alphabet_run` is the very first proc-with-if/while in the lib chain; that's why hangs only appear once global.sc loads. Session #69 hypothesis (ARBNO+FULLSCAN backtracking) is WRONG. The 3 fixes from #69 (semantic.sc EVAL double-quote wrapping for tag names containing single-quotes, if(~done) → if(DIFFER(done)) for non-pattern boolean check, integer-0 → ''-as-false flags for done/cont/more/eof_inside) remain correctly landed. Made NO source changes this session — investigation only; repos clean. Next session: (1) confirm whether `--sm-run` and `--jit-run` exhibit same hang or only `--ir-run`; (2) write SNOBOL4 equivalent (`DEFINE('foo(x)'); foo bar = differ(x) :s(L1)f(L2); L1 x = 0; L2 :(return); foo_end; v 'X' pos(0) :s(ok)f(no)`) and run under SPITBOL `-bf` to confirm scrip-Snocone-specific (not a SNOBOL4-runtime issue); (3) bisect `src/driver/interp.c` (--ir-run tree-walker) for handling of conditional jumps inside DEFINE'd procedures — look for PDL, &STCOUNT, or pattern-matcher state left dirty after stepping `:goS/:goF` labels. Gates remain green PASS=5/PASS=42 SKIP=3/PASS=49.) |
