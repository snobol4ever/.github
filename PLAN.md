...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: SF-8** (corpus gate). `6bcc9837`: --bb-inline-limit=N; BB_OVER_LIMIT guards; emit_bb_stateful→emit_bb_rtcall rename. SF-11/10/9 ✅. SF-12 blocked on IF-0..5. Architecture note: --bb-inline-limit falls back to whole brokered-path C BB, not true hybrid. |
