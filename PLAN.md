...
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **NEXT: SI-1 skeleton + dispatch loop.** Goal opened sess 2026-05-12 (Claude Sonnet 4.6) per Lon's directive: self-hosting closes the lower+interp loop — ultimate test for `lower.sc` is to run its SM output through a Snocone interpreter under scrip. Rungs SI-1..SI-5 retrace SL-1..SL-9 on the interp side. |
