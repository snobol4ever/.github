...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **EC-UNI-14 proper CLOSED 2026-05-20** via six-commit ladder (one4all `c081758f`): SCRIP_UNIFIED_DISPATCH flag flipped ON then deleted, sm_push_null split, sm_label() added, last 5 opcodes covered, legacy switch + dispatch_one_x86 deleted, bb_pl_* wired into emit_bb_node.  Net LOC roughly −130 across the rung.  **NEXT: EC-UNI-21** — write `scripts/test_gate_ec_uni_complete.sh` (five gates + baseline md5 + M1 oracle md5 reconciliation; M1 currently drifted at `abfd19a7...`).  See `GOAL-HEADQUARTERS.md` watermark for full hash trail. |
