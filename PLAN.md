...
| Goal | File | Step |
|------|------|------|
| **ICON-BB** | `GOAL-ICON-BB.md` | LFJ-0 ✅ (`bc7dae2a`). LFJ-1a-i ✅ (`f79ea9ba`: 9 leaf cases). LFJ-1a-ii ✅ (`013703ff`: 6 cases). **LFJ-1a-iii** ✅ (`b252409f`: 8 loop/driver cases extracted — TT_IF/TT_TO/TT_TO_BY/TT_EVERY/TT_WHILE+TT_UNTIL/TT_REPEAT/TT_LIMIT → lower_icn_legacy_<KIND> static fns; TT_WHILE/TT_UNTIL share one helper; ZERO logic change). Next: **LFJ-1a-iv** — binop group (TT_ADD..TT_NE, TT_CAT, TT_LCONCAT, TT_LLT..TT_LNE), TT_NOT, TT_ALTERNATE, TT_AUGOP. Gates: smoke 5/5, broker 24, rungs 198, smoke_prolog 5/5. |
...
| **Raku BB** | `GOAL-RAKU-BB.md` | RK-BB-1 ✅. RK-BB-2 steps 1-5 ✅ + step 6 PARTIAL `6ea3637d` (bb_suspend.cpp + bb_seq.cpp + emit_core peels + lower_fnc gather-call routing). BLOCKER: `lower_icn_proc_body` returns NULL for every Raku TT_SUB_DECL → bb_idx=-1 → routing falls through → bb_seq/bb_suspend templates unreached. NEXT: author `lower_raku_proc_body()` parallel to `lower_pl_predicate` for the strict gather-body shape (BB_SUSPEND chain). See HANDOFF-2026-05-27-OPUS-RK-BB-2-STEPS-4-6-PARTIAL.md. |
