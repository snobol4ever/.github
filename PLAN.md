...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **CURRENT: EM-FORMAT-BB** — banner fidelity landed (EM-FORMAT-SM-BANNER-FIDELITY LANDED 2026-05-09): root cause was `s->lineno=lbl.lineno` only set for labeled stmts; unlabeled stmts got `lineno=0`; fallback used `stno` as line-number proxy giving wrong source text.  Fix: `g_stmt_lineno` in lexer set at `<INITIAL>[ \t]` entry; `snobol4_get_stmt_lineno()` accessor; `.y` falls back to it when `lbl.lineno==0`.  Also landed same session: `.Lstr_N:` + `.string` merged to one line; all internal implementation comments removed from emitted `.s` (header block, blob banners, `subj_name=`, `has_repl=`, `expression:` annotations). All five artifacts regenerated; gates 14/14 GREEN. one4all `203426c6`, corpus `95589c1`. Next: **EM-FORMAT-BB** (four-port α/β/γ/ω + three-column + box banners across all BB blobs). |
