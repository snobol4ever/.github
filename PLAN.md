...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-8 — SM_PUSH_EXPRESSION / SM_CALL_EXPRESSION consumer** (lands after GOAL-CHUNKS-STEP17 closes). ME-6a ✅ 2026-05-11 (one4all `accafb5f`): SM_DEFINE_ENTRY opcode + jit_in_call guard solves re-entry-into-entry-label hazard; icase-style self-goto loops no longer corrupt native stack; me6_reentry_hazard gate 3/3. Prior closed: ME-1 ✅, ME-2 ✅, ME-3 ✅, ME-4 ✅, ME-5 ✅, ME-7 ✅, ME-6a ✅.  Sess 2026-05-10 (Claude latest, seventh session): **ME-6 Path A scaffolding landed; routing held — re-entry-into-entry-label hazard discovered.**  Built `SM_State.jit_epilogue_pending` (offset 24), `me6_return_dispatch` C helper, `emit_me6_define_entry_blob` (15B), `emit_me6_return_blob` (72B).  Initial routing of SM_LABEL `a[2].i==1` and the nine return variants regressed sn7_beauty 26→20: case_driver segfaulted with `rip == rsp` because `icase` in case.sno does SM_JUMP back to its own define-entry SM_LABEL — each loop iter re-executes `push rbp` without paired pop, polluting native stack; SM_HALT's `ret` fetches a stale rbp.  Routing reverted; emit functions kept as `__attribute__((unused))` scaffolding.  Recommended retry: ME-6a — introduce explicit `SM_DEFINE_ENTRY` opcode emitted at every DEFINE body head + `SmCallFrame.jit_prologue_active` flag set by `h_call`; SM_DEFINE_ENTRY blob does push, SM_RETURN variant blob does gated pop.  ME-6b — `test_gate_me6_reentry_hazard.sh` locks the fix.  Gates: smoke 7/7, unified_broker 49/49, sn7_beauty 26/25, recursive fib byte-identical sm/jit/ir.  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`, ME-4 ✅ `06b8f503`+`ae7f325a`, ME-5 ✅ `880adc36`, ME-7 ✅ `3d88cee7`. |
