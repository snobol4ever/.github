...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: fix rt_match_variant vstack underflow, then -u rung close.** Sess 2026-05-12 (Claude Sonnet 4.6): beauty-subsystems gate PASS=2 FAIL=15 (regressed from PASS=4). Root cause: rt_match_variant pops subj from vstack but mode-4 passes it as C arg; fix = `NV_GET_fn(subj_name)` instead of vstack_pop(). No code change this session — fix next. one4all `554aa38f`, corpus `dcce732`. |
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **NEXT: rung15 AST_ITERATE (Phase A4 !E).** Sess 2026-05-11h (Claude Sonnet 4.6): rung14 limit-in-generator ✅ `554aa38f`. lower_limit_every: slot_inner+slot_limit; GLOCAL[0]=count; nested SM_GEN_TICK; SM_DECR 1. Honest SM 212→213. |
=======
...
| **Snocone sm_lower (M2 path)** | `GOAL-SNOCONE-SM-LOWER.md` | corpus+one4all+.github | **NEXT: SL-10 lower_pat_expr Ph2.** SL-9 ✅ sess 2026-05-12 (Claude Sonnet 4.6): `sm_lower_test.sc` + `sm_lower_test.ref` baked — 11 SM instrs across assignment / pattern-match / END archetypes. Both gates clean (smoke 6 instrs, sm_lower_test 11 instrs). |
| **Snocone sm_interp (M2 path, self-host)** | `GOAL-SNOCONE-SM-INTERP.md` | corpus+one4all+.github | **NEXT: SI-6 SM_EXP (exponentiation).** Phase-2 plan landed sess 2026-05-12 (Claude Sonnet 4.6) — 10 rungs SI-6..SI-15 derived from SPITBOL manual v3.7 (chapters 3/4/6/7/15/17/18/19). C-parallel rename applied: state names `stack`/`sp`/`stack_cap`/`last_ok`/`pc` mirror `sm_interp.c` `SM_State` fields directly (struct dropped — single interpreter, no use for the indirection). `sm_push`/`sm_pop`/`sm_interp_run` match C signatures with no state-param (singleton). Lower.sc still uses `g_count`/`g_instr_tbl`; mechanical rename to `instrs`/`count` deferred as an SL- follow-up. |
