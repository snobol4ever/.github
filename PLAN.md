...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4 + LS-5 DONE; LS-6/LS-7 pending (session 2026-05-01 #6 — LS-4.l LANDED. Two LS-4.l fixes in `src/frontend/snocone/snocone_parse.y`: (Bug A) added binary `.` and `$` (E_CAPT_COND_ASGN / E_CAPT_IMMED_ASGN) at new `expr12` tier between `expr11` exponent and `expr15` subscript, mirroring snobol4.y:159-161 byte-for-byte — fixed parse errors in `fence`/`semantic`. (Bug B) added `sc_split_subject_pattern(EXPR_t **subj_io, EXPR_t **pat_io)` helper splitting `E_SCAN(subj, pat)` and `E_SEQ(name, rest...)` out of `s->subject` into separate `s->subject`/`s->pattern` slots, called from `sc_append_stmt`, `sc_make_cond_fail_stmt`, `sc_make_cond_succ_stmt`. Mirrors snobol4.y:248-270. Without the split, runtime evaluates E_SCAN as a value (always succeeds), breaking `if (subj ? pat)` everywhere — fixed runtime FAILs in `match`/`semantic`/`trace`. Decisive isolated repro: `'xyz' ? ANY('aeiou')` was succeeding in Snocone but correctly failing in SPITBOL oracle and in scrip's snobol4 mode. **Beauty 3-mode 30/12/3 → 42/0/3** — full LS-4.l acceptance MET. NEW gate `test_smoke_snocone_parse_j.sh` runs `test_snocone_parse_j.c` (22 functions / **90/90 PASS**) covering both fixes. All gates green: smoke snocone 5/5, smoke snobol4 7/7 (no regression), broker 49/0, beauty 42/0/3, parse-a..i 965/965 (no regression), parse-j 90/90 (NEW). Combined parse-a..j **1055/1055**. Bison conflicts 0/0. Files touched: `snocone_parse.y` (~100 lines added), regenerated `snocone_parse.tab.{c,h}`, NEW `test/frontend/snocone/test_snocone_parse_j.c`, NEW `scripts/test_smoke_snocone_parse_j.sh`. **LS-4 closes — the new Snocone front-end is the only Snocone front-end**, byte-equivalent to SPITBOL on the full 14-subsystem beauty suite across all 3 modes. Next active step: LS-6.a/b/c (largely confirmation; atomic landing already effectively happened across LS-4.j → LS-5.c → LS-4.l). LS-7 is documentation.) |
