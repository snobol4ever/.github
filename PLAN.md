...
| **BB Template Ladder — Icon + Prolog** | `GOAL-BB-TEMPLATE-LADDER.md` / `GOAL-ICON-BB.md` | one4all+corpus+.github | GOLDEN BB RULE: BB_t≡JCON ir_*; sval/ival/dval=payload. Phase H (Attribute Grammar). **Sess 2026-05-26d (Opus): INTERP path fully c[]/n/opaque/ival2/sval2-free and compiling** — all Icon + all Prolog executors migrated (bb_exec.c), both Prolog lowerers (lower_pl.c) + runtime builder (rt.c) + lower_pat_dcg.c + lower_icn.c TT_AUGOP. BB_t struct is FINAL (no new fields): runtime=value/counter/state + option-(b) GC aux structs (ptr in counter) for multi-slot generators — bb_arbno/pl_seq/pl_choice/pl_call_state_t in BB.h. ⛔ BUILD RED at **emit_sm.c ONLY** (Mode-4 Prolog serializer): helpers added (pl_node_kids/ival2/choice_body0) but 4 read sites not wired — needs XA_PL_SUB_BUILDER loop reshaped to iterate bb_pl_choice_state_t.bodies[] (was per-BB_SUCCEED opaque). one4all `97b92f26`. NEXT: finish emit_sm.c → build → smoke 5/5, broker ≥17, rungs ≥153 → commit. Then emit_bb.c(20). |
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | PJ ✅ COMPLETE. SB-LINEAR ✅ landed 916d61a5 — sm_emit_linear iterates SM, emits linear x86 blobs, falls through, relocates, calls. SM+BB freed before run. Mode 4 factorial working (PJ-12c ASAN clean). **WAS BLOCKED on G-1 bb_exec.c compile — interp-path migration landed 2026-05-26d (`97b92f26`); now blocked ONLY by emit_sm.c (see BB Ladder row).** NEXT: finish emit_sm.c, then run smoke_snobol4 --run gate. |
