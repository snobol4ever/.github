...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **IR-RN-1 ✅ COMPLETE 2026-05-20** (lower.c audit clean — no old-name leakage). **IR-CONSOLIDATE-DCG steps 1-3 ✅ COMPLETE 2026-05-20** (one4all `419fce29`). **IR-CD-RENAME ✅ COMPLETE 2026-05-20** (one4all `a97be4b0`): Category A dcg_*→bb_* across 15 files; pl_broker.c side-finding fixed; idempotent drive script committed. Category C deferred (RULES.md coordination). **NEXT: IR-CD-4** — migrate ~56 consumer call sites from `entry->ir_body` to `bb_graph_of_*(entry)`. Order: `icn_runtime.c` → `pl_runtime.c` → `ir_exec.c` → `emit_sm.c` → `rt.c`. Floor after each engine. Then IR-CD-5/6/7 (delete `ir_body`, doc, close). IR-RN-2/3 emitter+runtime audits still owed before EC-UNI-3-beauty.
