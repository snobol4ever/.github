...
## ⛔ PROGRESS REPORTING — every session, every step, no exceptions

While working, Claude reports progress as it goes. The format is concise and consistent so Lon can see **what is complete** and **what is in flight** at a glance:

```
[STEP X.Y]  <one-line description>     STATUS: <DONE | IN FLIGHT | BLOCKED>
```

After each meaningful unit of work (a code change, a build, a gate run, a commit), emit a status line of this shape. At minimum:

- **Before** starting a sub-step: `[STEP X.Y] <what>     STATUS: IN FLIGHT`
- **After** the sub-step lands or fails: `[STEP X.Y] <what>     STATUS: DONE` (or `BLOCKED: <one-line reason>`)
- **Gate runs** report their numbers inline: `[GATE] rung31 — PASS=5 FAIL=0 ✓` or `[GATE] SWI — 17/57`

At session-end (handoff), the Goal file's `Current state` section lists every step touched this session and its final status. The PLAN.md table's Step column gets updated to reflect the next active sub-step. This is mandatory: no silent work, no buried failures.
...
| **CSN FENCE Bug Fix** | `GOAL-CSN-FENCE-FIX.md` | csnobol4 | F-2 Step 3a (see goal file — last touched session #66; fence_suite 46/7/0 of 53; next: option (A2) leak-region zeroing at L_STARP2 success when FNCDCL present) |
| Icon Frontend Ladder | `GOAL-LANG-ICON.md` | one4all | IC-9 (see goal file — session #25 closed rung36_jcon_record; ladder PASS=200/FAIL=33/XFAIL=30; IC-10b queued for value-typed Icon scan-frame Byrd boxes) |
| Prolog Frontend Ladder | `GOAL-LANG-PROLOG.md` | one4all+corpus | **PR-19b LANDED** → PR-19c (2026-05-02 session: bridge extended to `\+/1`, `not/1`, `once/1` in pl_runtime.c — ~20 lines mirroring PR-19a's catch/3 arm. New gate `rung32_bridge_negation` PASS=5/5. All preserved: smoke 5/5, broker 49/49, rung31 5/5. SWI suite 17/57 unchanged (suite-line metric is not the gate). **Next: PR-19c** — `rung33_bridge_callN/` for `call/1`, `call/N` with goal-as-Var; bridge must reconstruct goal Term with extra args. Then PR-19d setof/bagof/findall, PR-19e setup_call_cleanup. Each independently shippable.) |
