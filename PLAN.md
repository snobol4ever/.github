...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-1 (LS-0 LANDED, sessions 2026-04-30 #1+#2+#3+#4: framing crystallized as **"Andrew Koenig's `.sc` self-host vision plus Lon's restoration of SPITBOL space-as-concat"**. Andrew's `.sc` self-host source `bconv[]` table at lines 32–60 is the canonical operator set we adopt verbatim — including **`::` for `IDENT()` and `:!:` for `DIFFER()` which were Andrew's choices** (his bootstrap `.sno` source missed these but `.sc` self-host added them). Three deletions from Andrew's `.sc` for Lon's space restoration: drop `&&` (line 38, replaced by juxtaposition SPACE pri 4), drop `||` (line 37, replaced by SPITBOL alt-eval `(e1,e2,e3)` Manual Ch.15 footnote), drop `%` (line 57, reserved as user OPSYN slot — `REMDR()` for modulo). Plus surface tightenings: `for(init;test;step)` semicolon-separated (frees `,` for alt-eval), single-keyword `goto`. Plus three additions Andrew's `.sc` lacks: `switch/case/default`, `do/until` (failure-driven loop companion to Andrew's `do/while`), `break;`/`continue;` (form pending Q13 — both/labeled-only/none — experimental). Conditions remain SPITBOL backtracking expressions exactly as Andrew intended (his `*exp` production goes inside `if(...)` parens directly). All session #2 commitments preserved (14 comparison ops, bare-expressions-fail-silently, labels, no newline magic, dangling-else binds inner). Q10 language name re-opened by Lon session #4 ("not now SNOBOL6 nor will it likely be — just ideas") — working repo/build name remains `snocone` indefinitely; rename Q11 moot until Q10 lands. Q12 identity-op spelling RESOLVED to `::` and `:!:` from Andrew's `.sc` lines 45–46. Q13 break/continue still pending. **Next**: Lon's word on Q13 → fold in → LS-1 lexer state-machine spec + 50-line test corpus, no code yet.) |
