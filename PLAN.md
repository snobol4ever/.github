...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-6.F.1 — add `T_1BANG` to `snocone_lex.h` ScKind enum.  **New rung opened session 2026-05-02** after the SPITBOL Manual Chapter 15 (p. 181-183) cross-check identified that Snocone's token inventory has full coverage of every defined and OPSYN-reserved SPITBOL operator with one narrow gap: unary `!` (T_1BANG) is missing.  Two concrete defects in `snocone_lex.{h,c}`: (1) `T_1BANG` not declared in ScKind enum; (2) `S_OP_BANG` unary fallback emits `T_2CARET` (binary-exponent token) for unary-position `!x`, collapsing two distinct SPITBOL operator roles (binary-exponent alternate spelling vs. unary-undefined OPSYN-available) into one token.  Five sub-rungs: SB-6.F.1 add token to enum; .2 add E_UN_BANG emit label + name-table entry; .3 fix S_OP_BANG unary fallback; .4 SPITBOL -bf cross-check (`2 ! 3` → 8, `2!3` → reject, `!x` → unary-bang token); .5 full gate sweep + corpus tight-bang scan.  Narrow scope: lexer-only.  Does NOT include OPSYN grammar dispatch (separate architectural decision).  Does NOT gate SB-6 self-host (beauty doesn't use `!`).  SB-6.E.6 (confirm-with-Lon) remains open separately.  SB-6.E.1..5 closed in commits corpus@1042e6b, one4all@9aed8555, .github@9c8ef5d. |
