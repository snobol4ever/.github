...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **EC-UNI-PER-KIND-DIFF ✅ LANDED (2026-05-21 evening, Opus 4.7).**  Harness operational: `tools/emit_per_kind_audit.c` + `scripts/{freeze_per_kind_baseline,test_per_kind_diff,normalize_per_kind_cell}` + `baselines/per_kind/` (3.3 MB committed).  962 cells per run (582 BB + 380 SM); current baseline PASS=384 STUB=578 FAIL=0 NEW=0 GONE=0.  Regression detection verified empirically — inject canary → FAIL>0 exit=1; restore → FAIL=0 exit=0.  Filter-then-diff via `normalize_per_kind_cell.py`; also captures `as`-assembled .o md5s for x86_text cells.  x86_bin uses structural (byte-count) comparison since ASLR randomizes baked-in addresses every run.  **NEXT: EC-UNI-REFAITH** — coverage gate now exists; re-lift any kinds whose live emit_bb_x* output differs from lifted template output (the harness will tell us which when wiring shifts).  Beauty md5 `40df9e004c3e963c99af716c65f2c970` (--compile) + matrix 855/855 stable. |
