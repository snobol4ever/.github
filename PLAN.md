...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-HELLO-2 — Raku hello-world via mode 4** (`say` builtin lookup at runtime — Error 5 today). IJ-HELLO-1 ✅ 2026-05-18 (Opus 4.7): `test_smoke_compile_hello_all_langs.sh` landed, baseline locked `PASS=3 FAIL=3 ROWS_MATCH=6 ROWS_DRIFT=0`. Three baseline-discrepancy rows audited vs goal-file's documented matrix (Icon `unhandled SM opcode 61` not `[NO-AST]` stub; Prolog **assemble-fails** on missing `.macro PUSH_EXPR` in directive form, not "runs brokered"; Raku errors out with Error 5, not silent). Root causes documented in GOAL-ICON-BB-JCON.md → "Baseline-discrepancy findings"; IJ-HELLO-2/3/4 scope unchanged. (Lon directive 2026-05-18: *"Ensure compiled Icon/Prolog use flat/wired BBs. Do not use brokered BBs in `--compile` mode. Get all six languages to say hello world before continuing mass dead code removal."*) **DAI-8 PAUSED** behind this step. **Prior:** DAI-8 cluster-1 retroactive mode-3/4 validation ✅ (Opus 4.7) — byte-identical pre/post `a4fe1c21` across `test_smoke_snobol4_jit` (--interp 196/65, --run 195/66), `test_crosscheck_{snobol4,icon,raku,rebus,snocone}` (5/1, 4/0, 21/12, 4/0, 8/0), `test_mode4_broad_corpus_snobol4` (202/75/3 SKIP). **DAI-7 ✅** 2026-05-17 (Opus 4.7) — IJ-DEL-ICN-AST arc CLOSED. DAI-7a (`254dedb9`) deleted 14 empirically-dead Icon C-BB zeta-fns + 10 self-contained state structs from `icn_runtime.c` (−560 LOC). DAI-7b (`ff3d100a`) deleted all 3 [DAI-BOMB] stubs + `icn_lazy_box` + `find_leaf_suspendable`. **Combined DAI-7 net: −608 LOC `icn_runtime.c` (1259→651).** |
