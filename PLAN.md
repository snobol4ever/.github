...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | **iter#9 IN PROGRESS** (n-ary trees everywhere, source-order children). Phases A+B+C DONE on local working copy (NOT COMMITTED): C frontends in snobol4.y / snocone_parse.y / raku.y / icon_parse.c / rebus_lower.c flatten same-kind chains via new `expr_binary_flatten` / `expr_binary_flatten_right` helpers in `scrip_cc.h`; runtime n-ary lowering in sm_lower.c (LOWER_NARY_LFOLD/RFOLD), interp_eval.c, eval_code.c. All three runtime paths execute n-ary chains correctly; SNOBOL4 smoke 7/7 + scrip(.sc) green. Phase D IN PROGRESS: parser_snobol4.sc rewritten (is_rotatable split into is_flatten_op + is_rotate_op, flatten path splices same-tag children) — NOT YET TESTED. Current gates: SN=57/59 (2 fail: arith_lassoc, arith_lassoc_div), IC=51, PR=54, RK=31/32 (1 fail: arith_chain), SC=21, RB=35/38 (3 fail: alt_*_three). **Next session: FIRST land Cleanup CR-1..CR-5 (delete Rebus RExpr→EXPR_t double layer per Lon directive) BEFORE finishing iter#9 Phase D in parser_raku.sc + parser_rebus.sc; then Phase E FENCE sweep second pass.** All Phase A/B/C work is local-only — uncommitted. |
...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | PARSER-SC-INFRA-3 Step 3d-bug (Steps 3a+3b+3c+3d DONE; corpus@a82b43c; Step 3d-bug DIAGNOSED session #66 cont. — root cause: SM_PAT_DEREF and interp_eval_pat both lack DT_E branch for `$indirect` in pattern context; SM-side fix LANDED one4all parser@a6d8b379; IR-side companion still pending; Step 3e BLOCKED until IR-side lands) |
| **PARSER-REBUS (pattern frontend)** | `GOAL-PARSER-REBUS.md` | corpus+one4all | PARSER-RB-2 (RB-0/RB-0a/RB-1 LANDED session 2026-05-04 cont.#6, PASS=18 FAIL=20: END-leak fixed via parser_icon.sc Procbody tail-recursive idiom; MAIN-only call emission; RB-1 assignment via Expr0 idiom. Next: if_stmt/while_stmt with deferred label-generation lowering walk.) |
| **PARSER-ICON (pattern frontend)** | `GOAL-PARSER-ICON.md` | corpus+one4all | PARSER-IC-10 (IC-10-style Steps 1-6 ALL LANDED PASS=51 corpus@1734c42 — underscore-id removal, single-stmt-body braces, divider/blank-line sweep, Gray/White rename, pseudo-token name cleanup, horizontal-density audit; next is feature coverage: augop expansion, more unary fixtures, Expr2 to..by, Expr5 concat) |
