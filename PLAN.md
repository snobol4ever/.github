...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-19-remaining** — TT_SUSPEND (blocked CH-17g); rung32; rung36 suite. ir-run 192→200 (+8), honest 273, broker 23/49. Fixes: IR_ICN_TO_NESTED; trim; seq; icn_drive_node accumulation; icn_bb_mutual (A&B); TT_NONNULL empty-str; loop_next/break in IR_ICN_EVERY; ?record lhs (ICN_RANDOM_SET); every !container lhs (icn_bb_assign_lhs_iter). OPEN: !str := val string frame-local writeback. one4all e116b9fd. |
...
| **⚡ Mode-4 Full Suite: SNOBOL4+Snocone** | `GOAL-MODE4-SN4-SNOCONE.md` | one4all+corpus+.github | **NEXT: M4SN-4b continued** — broad corpus 182/280 (+26 this sess), beauty 13/17. Fixed: stack misalignment in emit_seq_port_call{,_rip} (sub/add rsp,8 around push r10 / call). Fixes FENCE-via-*var 108–113,115–119. one4all `ad5cb86d`. Next: 059_capture_dollar_deferred, 106_pat_fence_with_capture, test_case. |
