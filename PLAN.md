...
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | **EM-7c-s-file-beautify LANDED 2026-05-09** — emitted `.s` artifacts are now one uniform three-column document end-to-end.  Three changes: (a) `;` separators removed from `bb_emit.c::bb3c_format` and `emitter_v.h::ev3c` (format string `%-24s ; %-16s ; %s\n` → `%-24s%-16s %s\n`); (b) 12 raw `EV_TEXT` blocks in `bb_flat.c` converted via new `flat3c_*` / `flat_data_*` / `flat_box_call*` helper family (charsets, XLNTH, XTB, XRTB, XFNCE, XFARB, XSTAR, XBRKX, XATP, XDSAR, XARBN, XNME/XFNME — zero raw `EV_TEXT` remain); (c) `emit_sm_exec_stmt_blob` rerouted through `emit_three_column_line` to drop the last 3 tab-indented stragglers per artifact.  **`EM-7c-bb-three-column-data` SUBSUMED + LANDED here.**  Files: `bb_emit.c`, `emitter_v.h`, `bb_flat.c` (+127 / 12 conversions), `sm_codegen_x64_emit.c`.  Gates 10/10 GREEN: smoke ×6, isolation, unified_broker PASS=49, EM PASS=12, bb_flat_text PASS=18, sm_phase2_sim PASS=25.  Audit per artifact: zero blank, zero tabs, zero `;`.  All five tracked artifacts assemble cleanly (line counts: roman 208, wordcount 158, claws5 1114, treebank-list 1395, treebank-array 1578, sm_macros 249).  **Next: EM-7c-sm-three-column-verify** — audit SM three-column shape end-to-end; close remaining template gaps (SM_PUSH_LIT_F, SM_NEG, SM_EXP, SM_NEXT_PUSH etc.); then **EM-7c-bb-macros** — BB-side macro library; col-2 BB content becomes single macro names like `LEN_α`, `RPOS_β`. |
