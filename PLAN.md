...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **NEXT: IEP-5** — eliminate every_table. IEP-4 ✅ `d9dff43a`. Three rules enforced by DELETION: tree_t freed after lower() (IEP-7); IR_block_t freed after dcg_table registration (IEP-8). IEP-5 every_table + IEP-6 SM_PUSH_EXPR must complete before IEP-7. |
