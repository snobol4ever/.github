...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **DAI-8 C17 ✅ 2026-05-19 (Sonnet 4.6, one4all `d48681fb`) — snobol4_stmt_rt.c deleted (43 fns, −447 LOC). All gates floor held. Session baseline established: SNOBOL4 7/0, Icon 194/265, Prolog 5/0, Snocone 2/3, Rebus 4/0, Raku 5/0, JVM 7/6, JS 4/2, NET 0/9 (no ilasm), WASM SKIP. NEXT: DAI-8 C18 (icn_runtime static_get/static_set, rt_set_last_ok, address-of audit for emit_core inlines).** |
...
| **⚡ PST: Raku** | `GOAL-PST-RAKU.md` | one4all+corpus+.github | **⏳ Phase 1 NOT clean** — 18 §⛔ violations remaining. PRF-12-say/print/sub/die/arr-hash-ops/class/program/for ✅. one4all `e645ab4b`, corpus `e6f7504`. NEXT: PRF-12-try (R10) or PRF-12-unless (R11). ⚠ Always regen `raku.tab.c` with `bison -d raku.y -o raku.tab.c` after any `raku.y` change. |
