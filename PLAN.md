...
| **⚡ Prolog BB JCON triage** | `GOAL-PROLOG-BB-JCON.md` | one4all+corpus+.github | **NEW (parallel to GOAL-ICON-BB-JCON).** NEXT: PJ-1 — add `pl_bb_dcg` + `g_dcg_table[]` skeleton. Watermark one4all `b974e111`: smoke_prolog 0/5 (all five stub on SM_BB_ONCE_PROC NO-AST). Architecture mirrors GOAL-ICON-BB-JCON byte-for-byte with Prolog port semantics substituted (β = retry next clause + trail unwind, instead of Icon's β = advance generator counter). Cross-language SM↔SM via `g_user_call_hook` / BB↔BB via four-port contract — both allowed; banner tightened followup #3. Four-port-hard-wired absolute rule applies. Sister Goal: GOAL-ICON-BB-JCON at one4all `b974e111` (smoke_icon 5/5, ir-run 31). |
