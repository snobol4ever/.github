...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-4-pre — Register layout study + design (DRAFT COMPLETE, pending Lon sign-off).**  Sess 2026-05-10 (Claude latest): `REGISTER-LAYOUT.md` committed at `.github` HEAD with locked decisions — r12=SM_State*, r13=stack base, r14=sp (hot), r15 reserved (ME-8), rbx reserved (ME-11), rbp=chunk frame, r10=BB-glob LOCAL ptr (constant inside a BLOB).  Intra-BLOB jumps don't touch r10; extra-BLOB tail-jumps don't touch r10 (destination's α-preamble loads its own); only call-style extra-BLOB jumps need source-BLOB `push r10`/`pop r10` framing.  Three always-live SM-blob regs + one always-live BB-glob reg + 14 free in the scratch+reserve budget.  Stack push: `mov [r13+r14*16], rax ; mov [r13+r14*16+8], rdx ; inc r14d`.  SM_STNO blob: `xor r14d, r14d ; mov r13, [r12+STACK_OFFSET]` — replicates `sm_interp.c:295` and handles realloc.  ME-4-pre closes on Lon's sign-off line in REGISTER-LAYOUT.md's watermark; until then ME-4-post stays blocked.  Companion ARCH-x86 update lands intra-/extra-BLOB jump rules and splits flat-BB ABI from dispatched-BB ABI (BBs are stackless; CODE shared, DATA per-invocation, sibling DATA blocks chained for the appearance of stack-like depth; flat BBs have no `esi`, dispatched-BBs do).  Code state: one4all `e7ac6f77` — variable-size blobs + ME-4 inline-native emitters + rt.c pluggable backend landed; gates clean (smoke 7/7, unified_broker 49/49; 100k counter loop ~32% faster than `--sm-run`); 🐛 multi-statement arith reproducer aborts on `realloc(): invalid next size` (fixed by ME-4-post's SM_STNO sp-reset blob).  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`. |
