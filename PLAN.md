...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-BB-FORMAT-ARCH** — port-context accumulator in bb_emit.c (t_label_define saves label; t_emit_jmp flushes 4-col ;-fused line; no if in templates). Then EM-BB-FORMAT-1 (XPOSI/XRPSI verify). Then ESA-1/2 (.s artifacts). New steps: EM-SPEC-T-ERADICATE, EM-XVAL-DESCR, EM-S-ARTIFACTS-COMMIT. bb_templates.c 477→281, sm_templates.c 729→415 (reformatted). Baseline: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty PASS=10. one4all `710d9088`. |
