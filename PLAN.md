...
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | ⛔ **TOP: Prolog RUNG LADDER via proper LOWER (PJ-AG-WIRE) + proper EMITTER (Mode-4 x86).** **Sess 2026-05-26 (Opus 4.7): main/0 auto-run LANDED — GATE-3 48→57 (+9).** Driver-only fix in `lower()`: Prolog file defining `main/0` with NO `:- initialization` directive auto-runs `main/0` (corpus convention; resolves the long-pending path-(A) "NEEDS LON DECISION" in favor of the DRIVER, not corpus patching). Guarded on LANG_PL; suppressed when any init directive present (no double-run). Unblocked rung16 term-compare x5 + rung18 succ x2. smoke_prolog 5/5, crosscheck_prolog 132/0, icon/snocone/raku 5/5, rebus 4/4, snobol4 7/6 (pre-existing). `one4all 62ce5788`. **PRIOR:** builtin wave 2 GATE-3 32→48. ⛔ **NEXT: (B) builtins — rung13/14/15 assertz/retract/abolish dynamic-pred directives (15 tests, currently `[NO-AST] … skipped`), rung17 sort/msort, rung18 plus/3, rung19 format, rung20 numbervars, rung22 writeq, rung27 aggregate, rung28 catch/throw. (C) rung25 term_to_atom operator-notation writer.** Then EMITTER AGW-9 (`flat_drive_pl_seq`). **DEFERRED:** SNOBOL4 Mode-4/bench; SBL-* + PA-1/2/3 + AGW-1..8 all ✅. EMITTER bb_pl_* templates: cut FILLED; seq/call/choice/alt EMPTY (AGW-9 remaining). |
