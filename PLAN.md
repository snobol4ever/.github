...
| **Icon BB JCON triage** | `GOAL-HEADQUARTERS.md` | one4all+corpus+.github | **EC-UNI-0..2d ✅; EC-UNI-3 partial 🔄 (42908963) 2026-05-19 (Opus 4.7).** Unified dispatch wired into `emit_walk_codegen` behind `SCRIP_UNIFIED_DISPATCH` env var; mode-set invariant surfaced & fixed. Flag-off and flag-on both pass smoke + broker + icon rungs at floor; `hello.sno` diff is GAS-comment-only. **NEXT: EC-UNI-3-beauty** — gate script that md5-compares beauty.sno output flag-off vs flag-on. Then EC-UNI-4 deletes `emit_walk_codegen`.
