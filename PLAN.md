...
| **Icon BB JCON triage (39 FAILs + rung37 tests)** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-12 debug** (IJ-11 ✅ `9681072c`. IJ-12 PARTIAL `36fc9d2f`: coro_bb_mutual + lhs-gen revassign + string subscript_set + use_rhs_gen chained <-. Queens still FAIL — inner q() finds no valid rows; suspect coro_bb_binop beta reads wrong frame for `c`. rung37_mutual pending.) |
...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-REORG-7** — EM-REORG-1..6 ✅ (sess 2026-05-13r): 6 new files, emit_bb_gen.c drained by 2/3. one4all `47c2f30c`. Gates: smoke 7/7, byte-id 4/4, em8 5/5. |
