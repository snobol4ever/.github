...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.f (session 2026-04-30 #9: **LS-4.e LANDED.** All remaining SPITBOL unary operators added to Snocone Bison grammar: `*` E_DEFER, `.` E_NAME, `$` E_INDIRECT, `@` E_CAPT_CURSOR, `~` E_NOT, `?` E_INTERROGATE, `&` E_OPSYN("&"), plus OPSYN-slot unaries `%` `/` `#` `|` `=`. Applied at expr17 (atom tier) — highest priority. Chains nest right-to-left: `~.x` → E_NOT(E_NAME(x)). Zero Bison conflicts. parse-e 71/71. Combined parse gates 370/370. All production gates green. one4all @ `a929d72b`. Next: LS-4.f — control flow if/else/while, using matched_stmt/unmatched_stmt balanced grammar per goal file forward note.) |
