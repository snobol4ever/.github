...
| **PARSER-SNOBOL4 (pattern frontend)** | `GOAL-PARSER-SNOBOL4.md` | corpus+one4all | **iter#9 + iter#10 + Phase E LANDED**.  Family at 100%: SN=62/62 ✅ IC=88 ✅ PR=60 ✅ RK=37 ✅ SC=46 ✅ RB=38 ✅.  Cross-pollination of iter#10's `foldop()`/`FoldOp()` arrived in PARSER-RK-5 (sibling) and PARSER-PR-8b (sibling).  Phase E FENCE sweep partial-landed for parser_snobol4.sc at corpus@c6bb327 (3 iter#10 ARBNO tail rules wrapped in FENCE).  parser_snobol4.sc structural work essentially complete; surviving `is_rotate_op` block in `rw_expr` waits on runtime `E_CAPT_*_ASGN` going n-ary (iter#10b, runtime work, out of scope).  **Next: PARSER-SN-7-2 (keyword recognition fixture cluster) — pure ladder-extension work.  Or iter#10b runtime work elsewhere.** |
