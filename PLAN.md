...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-HELLO-2b — Raku trailing CALL_FN "main" on proc-def wrapper TT_FNC**. IJ-HELLO-2a ✅ 2026-05-18 (Opus 4.7): `rt_call` in `src/runtime/rt/rt.c` now chains `icn_try_call_builtin_by_name` before `INVOKE_fn`, mirroring `sm_interp.c`'s SM_CALL_FN handler. `Hello, World!` now reaches stdout under Raku `--compile` (was: silent + Error 5; now: prints + rc=1 from Bug 2). +26 LOC single file. Baseline regression check held everywhere (gate PASS=3 FAIL=3, smoke ×6 unchanged, crosscheck snobol4 5/1 icon 4/0 raku 21/12 rebus 4/0 snocone 8/0). Bug 2 diagnosis: Raku top-level `sub main(){...}` arrives at lower_stmt as TT_FNC{sval=NULL (union overwrite by ival=nparams), c[0]=TT_VAR(name), c[nparams+1..]=body}; lower_fnc treats it as a call → emits `SM_CALL_FN "main"` with no mode-4 target. AST-shape discrimination fails (regular `write(x)` calls have identical shape). Path (B) for next session: preserve `_id=SUB_TAG_ID` through `add_proc` (currently cleared at `raku.tab.c:1614`, mirror raku.y:201); match in lower_stmt's LANG_RAKU branch; lower body children directly via `lower_expr`+`SM_VOID_POP`. Safe: `_id` post-parse is read only by Icon-specific paths. After fix, edit `test_smoke_compile_hello_all_langs.sh:182` `raku` row to `PASS-wired` and bump `HW_EXPECTED_PASS=4 HW_EXPECTED_FAIL=2` in same commit. IJ-HELLO-1 ✅ 2026-05-18 (Opus 4.7): `test_smoke_compile_hello_all_langs.sh` landed. (Lon directive 2026-05-18: *"Ensure compiled Icon/Prolog use flat/wired BBs. Do not use brokered BBs in `--compile` mode. Get all six languages to say hello world before continuing mass dead code removal."*) **DAI-8 PAUSED** behind this step. **Prior:** DAI-8 cluster-1 retroactive mode-3/4 validation ✅ (Opus 4.7) — byte-identical pre/post `a4fe1c21`. **DAI-7 ✅** 2026-05-17 (Opus 4.7) — IJ-DEL-ICN-AST arc CLOSED. **Combined DAI-7 net: −608 LOC `icn_runtime.c` (1259→651).** |
