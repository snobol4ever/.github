...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: IJ-19-remaining** — ir-run 206. Sess 2026-05-15 (Claude Opus 4.7, one4all `5198d619`): ICN_RANDOM_SET list case fixed (?x:=val); rung36_jcon_lists diff shrunk 5→1 region. Next: y := x[-3+:6] (wraparound slice) returns &null instead of fail — bb_section returns FAIL but var store yields &null; trace SM_STORE_VAR / emit_var_store path. CH-17g-blocked: scan/string/wordcnt. |
