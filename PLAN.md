...
| **CLI 3-mode collapse + AST-interp delete** | `GOAL-CLI-3MODE.md` | one4all+.github | CLI-3M-1..7 ✅, CLI-3M-10 ✅ (2026-05-17j). **CLI-3M-9 ✅ 2026-05-18 (`8c799b2e`)** — `interp_eval.c` deleted; contents moved to proper homes; mode 1 gone from binary and codebase. NEXT: CLI-3M-11 doc pass → CLI-3M-12. |
...
| **⚡ PST: SNOBOL4** | `GOAL-PST-SNOBOL4.md` | one4all+corpus+.github | **PST-SN4-W1** — remove `->t==TT_QLIT` inspection from `sno4_stmt_commit_go`. 1a–1d ✅. Phase 2 SCRIP mirror pending. |
| **⚡ PST: SNOBOL4 + Snocone** | `GOAL-PARSER-PURE-SYNTAX-TREE.md` | one4all+corpus+.github | **PST-SC-4k** next. SN4: owned by GOAL-PST-SNOBOL4. SC: 4a…4j ✅ 4k→4n pending. |
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **PRF-S7 ✅ 2026-05-18 (Sonnet 4.6, corpus `481851e`)** — raku_stubs.sc deleted. All 94 stubs inlined: 88 class-a as shift_val/assign; push_interp_leaves moved to parser_raku.sc; emit_to_sub_list/push_stmt_subj eliminated via nPop+nInc; Compiland restructured to reduce('TT_PROGRAM','nTop()'). parser_raku.sc: 2 PST-clean functions (dq_unescape, push_interp_leaves). Zero tree-building functions. **GOAL COMPLETE** corpus `1e8bba5`. parser_raku.sc: 0 functions, pure shift/reduce. NEXT: debug Compiland empty-output in SCRIP-hosted path (separate session). |
| **⚡ PST: Rebus** | `GOAL-PST-REBUS.md` | one4all+corpus+.github | **RB-C-1 ✅** 2026-05-18 (Sonnet 4.6, one4all `0da9ec20`): stmt_list_ne always-wrap fix in rebus.y. NEXT: PST-RB-PRE-BEAUTY Bug #2 (~30 lines sketched): mode-1 interp_exec SUBJ-PAT split. Then 5i triage queues. |
