...
| **BB Template Ladder — Icon + Prolog** | `GOAL-ICON-BB.md` | one4all+.github | **ICN-Z-0 ✅ + ICN-Z-1 ✅** (`ce649c14`) zipper foundation. ICN-Z-2 substantial. ICN-Z-3 BLOCKED (CONJ needs full driver pass). **⚡ CORRECTION:** mode-4 `lt`/`mult`/`compound` are TEMPLATE-ONLY (BINOP_GEN α/β = operand gen boxes, reachable at emit time) — prereq is just **ICN-XA-1 `walk_bb_node_str`**, then fill `bb_binop_gen.cpp` odometer → PASS=2→5. Gate PASS=2. FACT RULE 0. |
