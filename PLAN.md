...
| **Prolog BB / SB-LINEAR** | `GOAL-PROLOG-BB.md` | one4all+corpus+.github | ⛔ **TOP: Prolog RUNG LADDER via proper LOWER (PJ-AG-WIRE) + proper EMITTER (Mode-4 x86).** **Sess 2026-05-26 (Opus 4.7): builtins + findall/3 LANDED — GATE-3 20→32 (+12), honest 128/0/0, smoke_prolog 5/5, icon 5/5, snobol4 7/6 (pre-existing).** rung09 (`functor/3`,`arg/3`,`=../2`,11 type-tests), rung12 ×5 (`atom_length/concat/chars/codes`,`up/downcase_atom`), rung11 ×5 + rung30 (`findall/3`: Goal→self-contained BB sub-graph, run to exhaustion, `bb_copy_term` template per solution, unify into arg3). Meaty fix: **determinacy guard in BB_PL_CALL resume** — body with no live inner choice point reports exhaustion instead of re-firing ∞ (killed findall-over-fact loop + fixed DCG generate); multi-solution recursion (append-split) verified intact. Also arith functors in TERM position reconstruct as compounds (findall `K-V`). All on PJ-AGW unify pattern. See `HANDOFF-2026-05-26-OPUS-PROLOG-BUILTINS-FINDALL.md`. **PRIOR sessions (still valid):** DCG phrase rewrite (`0c547c01`)+compound write (`f43aff1d`) GATE-3 16→19; PJ-AGW shared-var binding honest 123→128, GATE-3 19→20+succ/2. ⛔ **NEXT: (A) main/0 auto-run when no `:- initialization` — DRIVER fix (RULES.md forbids corpus patching); lights up ~30 directiveless rungs incl. rung16 — STILL NEEDS LON DECISION. (B) builtins rung16 `@</@>=`, rung17 sort, rung19 format, rung21 char_type, rung24 string_io — each a BB_BUILTIN arm, unify outputs as functor/arg/atom_* do.** Then pushback DCG, AGW-9/10 Mode-4 emitter. **DEFERRED:** SNOBOL4 Mode-4/bench; SBL-* + PA-1/2/3 + AGW-1..8 all ✅. EMITTER bb_pl_* templates: cut FILLED; seq/call/choice/alt EMPTY (AGW-9 remaining). |
