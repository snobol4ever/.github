...
| **⚡ PREMIER: Mode-3 x86 Emitter** | `GOAL-MODE3-EMIT.md` | one4all+corpus+.github | **NEXT ACTIVE: ME-6 — Function calls and returns** (PARTIAL — see GOAL-MODE3-EMIT.md handoff addendum sess 6 of 2026-05-10).  Sess 2026-05-10 (Claude latest, sixth session): **ME-7 ✅ CLOSED + two diagnostic prereqs landed.**  ME-7: SM_LABEL DEFINE-entry flag landed at `a[2].i = 1` (not `a[1].i` per original spec — that slot already holds the label's PC).  New runtime API `FUNC_IS_ENTRY_LABEL(label)` in `snobol4.c`/`.h` scans every FNCBLK_t bucket for any user-fn entry matching `label`; `lower_stmt` consults after each `sm_label_named` and tags hits.  `sm_prog.c` printer extended: SM_LABEL now displays `s="<name>" define_entry=1` in `--dump-sm`.  Diagnostic prereqs: (a) `opnames[]` shift bug fixed — was missing `"SM_BB_PUMP_AST"`, displayed every opcode from SM_CALL_FN onward shifted up one slot; (b) bare-statement RETURN/FRETURN/NRETURN lowering fixed — was emitting `SM_PUSH_VAR s="RETURN"; SM_VOID_POP` instead of `SM_RETURN`, breaking `define` smoke under `--sm-run`/`--jit-run` (produced empty output where IR produced `42`).  Post-fix: `define` smoke produces `42` byte-identical across IR/SM/JIT.  ME-6 deferred-scope notes: dispatch model has no native return-from-function hook (rsp threads through entire chain); 9 conditional return variants need handler→blob unwind signal (e.g. new `STATE->jit_epilogue_pending` field); recursive fib already works correctly all three modes today proving rbp accounting isn't needed for correctness — prologue/epilogue's real purpose is the latent vsnprintf alignment in `bb_pool` byrd-box code (different code path than SM-blob land); `roman.sno` gate unsatisfiable in this env (bb_alloc pool exhausted under all three modes including `--ir-run`).  Recommended next-session path A (build the epilogue signal mechanism) or path B (find and fix the actual vsnprintf alignment failure as a real test).  Gates: smoke 7/7, unified_broker 49/49, sn7_beauty 26/25 unchanged baseline, mode-4 tripwire unchanged, 30-program parser sweep 0 divergences, recursive fib byte-identical sm/jit/ir, 100k loop perf unchanged (~80ms jit).  Prior closed rungs: ME-1 ✅ `cc3cd475`, ME-2 ✅ `babf76be`, ME-3 ✅ `aca47e6c`, ME-4 ✅ ME-4-post bug-fix `06b8f503` + r12-tos `ae7f325a`, ME-5 ✅ `880adc36`. |
...
| **Icon BB Complete (honest mode 3)** | `GOAL-ICON-BB-COMPLETE.md` | one4all+.github | **A4 — CH-17i-alternate** (`AST_ALTERNATE` — `E1 \| E2` alternation) + **A3-seed-fix** (unify ICN_RANDOM seed with coro_value.c `_rnd_seed` so ir-run and sm-run match for random programs). A0 ✅ 2026-05-10 (`adb289b9`). A1+A4-pulled-forward ✅ 2026-05-10 (`75b57a32`). A2 ✅ 2026-05-10 (`242edf06`): SECTION scalar SM. A3 ✅ 2026-05-10 (`cb4f88ed`): RANDOM scalar SM — stack underflow eliminated, ABORT 8→2; honest PASS at 116 (2 programs diverge due to seed mismatch vs ir-run; seed unification = A3-seed-fix follow-on). AST_LIMIT left on legacy (corpus programs all inside `every`, already honest). Gates: smoke 5/7/5/5/5/4, broker 49/49, isolation PASS, ir-run 177/56/30. |
