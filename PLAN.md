...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.b (session 2026-04-30 #3: **LS-4.a LANDED.** Skeleton `snocone.y` Bison grammar (~600 lines) at `src/frontend/snocone/snocone.y`, generated `snocone.tab.c`/`.tab.h` committed. Atoms (T_INT/T_REAL/T_STR/T_IDENT/T_KEYWORD), arithmetic (`+ - * / ^` with right-assoc exponent), paren-grouping, unary `+`/`-`, and `;`-terminated statements; `T_ASSIGNMENT` splits top-level E_ASSIGN into subject+replacement matching snobol4.y's stmt_commit_go pattern. Public entry `snocone_parse_program(src, filename) -> Program*`. **Token-namespace decoupling** resolved cleanly: the FSM enum `ScKind` and Bison enum `sc_tokentype` share enumerator names (T_INT etc.) but different values (1..N vs 258+M); Bison's `%code top` block aliases the FSM names to `SC_T_*` via `#define`/`#undef` around the `#include "snocone_lex.h"`, so a single translation table `sc_kind_to_tok()` is the only boundary. yylex thunk calls `sc_lex_next(ctx)` directly — single-pass per LS-4.j convention. **File reorganisation:** `snocone_fsm.c`/`.h` renamed to `snocone_lex.c`/`.h` (the standard frontend lexer name); old hand-written `snocone_lex.c`/`.h` moved to `one4all/archive/` with Makefile and 2 callsite includes (`snocone_control.c`, `snocone_parse.h`) redirected to the archive path; production path unchanged. Test `test_snocone_fsm.c` renamed to `test_snocone_lex.c` (still 31/31 PASS). **New gate** `test_smoke_snocone_parse_a.sh` runs `test_snocone_parse_a.c` (9 cases / **35/35 PASS** — assignment IR shape, precedence, parens override, exponent right-assoc, unary minus, bare-expr stmt subject-only, multiple statements, string and real literals). **Production gates remain green:** smoke 5/5, beauty 42/0/3, broker 49/0. Regen script `regenerate_parser_and_lexer_from_sources.sh` extended with snocone block. **Archive cleanup invariant:** four active-code references to `archive/snocone_lex.*` exist after LS-4.a (Makefile line 27, `snocone_control.c:53`, `snocone_parse.h:41`, doc-comment in `snocone_lex.h:15`); LS-4.j must end with zero such references — verify `grep -rn 'archive/snocone' src/ scripts/ test/` returns empty. **Next: LS-4.b** — comparison/identity operators (T_EQ/T_NE/T_LT/T_GT/T_LE/T_GE/T_LEQ/T_LNE/T_LLT/T_LGT/T_LLE/T_LGE/T_IDENT_OP/T_DIFFER) all lower to E_FNC named calls. Parses `OUTPUT = EQ(2+2, 4)`.) |
