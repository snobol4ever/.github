...
| Snocone Beauty         | `GOAL-SNOCONE-BEAUTY.md`       | corpus+one4all | SB-6 (session #77: SC-INPUT-EOF CLOSED as a corpus fix, NOT a runtime fix. Sessions #68/#76 had diagnosed an `input_read()` bug; session #77 verified `input_read` already returns FAILDESCR on EOF correctly (SNOBOL4 frontend agrees with SPITBOL byte-for-byte). The actual issue was the slurp idiom in the .sc sources: `line = INPUT; while (DIFFER(line)) { ...; line = INPUT; }` is structurally broken — failing assignment leaves LHS unchanged so DIFFER stays true forever. Canonical Snocone idiom (per `programs/snocone/report.md` line 1240): `while (line = INPUT) { ... }` — assignment-as-predicate FAILs on EOF, while-test exits cleanly. Fixed two .sc files: (1) `claws5.sc` slurp loop replaced (5 lines) — was hanging rc=124 on `claws5.input`, now exits cleanly with `Pattern match failed` in all three modes, matching SPITBOL byte-for-byte; (2) `beauty.sc` main loop wholesale rewrite (~60 lines net) — removed session #68 SB-5b workaround flags (`done`, `cont`, `more`, `eof_inside`, `PrevLine`, `IDENT(Line, PrevLine)` no-advance detection), replaced with clean look-ahead structure using `have_line` flag plus `Line = INPUT` predicate; structure mirrors canonical `beauty.sno::main00..main05` exactly; goto count remains 0. Verification: 50KB `beauty_oracle.sno` input now terminates cleanly in all three modes (1449 lines emitted, header passthrough working). Three baseline gates green (PASS=5, PASS=42 SKIP=3, PASS=49); crosscheck snobol4=6, snocone=8 unchanged; broad interp suite PASS=222 FAIL=52 unchanged from session #75 baseline. No runtime/frontend source changes. Open downstream of SC-INPUT-EOF: SB-6 (beauty self-host — pp() trips Error 5 on END node and three startup pattern-construction errors remain — pre-existing beauty-grammar issues unrelated to slurp loop), SB-5c.2 (input-curation), SB-5d.2 (LS-0 dependency). Files touched: `corpus/programs/snobol4/demo/claws5.sc` (~5 lines), `corpus/programs/snocone/demo/beauty/beauty.sc` (~60 lines main loop). No `one4all`, `csnobol4`, `x64` changes.) |
