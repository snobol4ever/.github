...
| **Mode-4 x86 Emitter** | `GOAL-MODE4-EMIT.md` | one4all+corpus+.github | **NEXT: EM-BB-FORMAT-4** — charset family (XSPNC/XBRKC/XANYC/XNNYC): FORMAT branch in `emit_bb_port_call` appends `call fn@PLT` to body; flush on final jmp. Closed: EM-BB-FORMAT-ARCH ✅ + EM-BB-FORMAT-1/2/3 ✅ (`cd0b5964`). Baseline: smoke 7/7, template-byte-id 4/4, snocone 5/5, beauty PASS=9. one4all `cd0b5964`. |
