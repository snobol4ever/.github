...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **NEXT: more lower_icn_expr_node coverage** (non-literal TT_TO bounds, TT_REPEAT, nested user-proc TT_FNC, TT_ALTERNATE, TT_AUGOP). Sess 2026-05-15 followup #3 (Claude Opus 4.7, one4all `b974e111`): TT_WHILE/TT_UNTIL/TT_SEQ_EXPR/TT_GLOBAL added. ir-run 20→31 (+11), honest 277→278 (+1), no regressions. Cross-language banner tightened: SM(A)↔SM(B) via `g_user_call_hook` and BB(A)↔BB(B) via universal four-port contract are BOTH allowed; only forbidden case is invoking language-A SM-bridge handler with language-B BB object. Retracted "drop per-construct IR opcodes" + "generator state as IR register" over-reach; kept compile-time-label-thinking as a lowering style technique. Sess 2026-05-15 followup #2 (Claude Opus 4.7, one4all `927e0296`): four ports hard-wired with direct pointers — IR_node_alloc bakes α=β=nd self-loop + γ=ω=NULL terminator into every IR_t at allocation time. Invariant-preserving across all gates. Sess 2026-05-15 (Claude Opus 4.7, one4all `c2c20d1a`): **smoke_icon 3/5 → 5/5**, broker 15→16, ir-run 15→20. Closed IJ-AST-IR-BB-if-every-to: added IR_IF + IR_EVERY (statement consumer) executors; extended lower_icn_expr_node with TT_IF/TT_EVERY/TT_TO. Both smoke programs compile to the target two-SM-instruction shape (`SM_BB_PUMP_PROC main` + `SM_HALT`). HQ doc updated with jcon side-by-side comparison and "many SM ↔ many BB" granularity table. |
...
| **⚡ Prolog BB JCON triage** | `GOAL-PROLOG-BB-JCON.md` | one4all+corpus+.github | **NEXT: PJ-2** — create `src/lower/lower_pl.c` + `lower_pl.h`; placeholder `lower_pl_predicate(tree_t *pred)` returning NULL; wire `lower()` to populate `g_dcg_table[i].ir_body = NULL` per Prolog predicate. Sess 2026-05-15 followup #4 (Claude Opus 4.7): **PJ-1 ✅** — added `pl_bb_dcg` infrastructure bridge + `g_dcg_table[PL_DCG_TABLE_MAX]` registry with `pl_dcg_lookup/register` helpers in `pl_runtime.{c,h}`; `pl_dcg_state_t { IR_block_t *cfg; int first; }` mirrors `icn_dcg_state_t` byte-for-byte; `Pl_PredEntry_BB` mirrors `IcnProcEntry` minus AST fields (BB-land registry is AST-free per NO-AST-WALK). Build clean. Smokes unchanged: prolog 0/5 (skeleton-only; consumers wired in PJ-3+), icon 5/5, snobol4 7/7, snocone 5/5, rebus 4/4, raku 5/5. Broker 16/49 unchanged. Architecture mirrors GOAL-ICON-BB-JCON byte-for-byte with Prolog port semantics substituted (β = retry next clause + trail unwind, instead of Icon's β = advance generator counter). Cross-language SM↔SM via `g_user_call_hook` / BB↔BB via four-port contract — both allowed; banner tightened followup #3. Four-port-hard-wired absolute rule applies. Sister Goal: GOAL-ICON-BB-JCON at one4all `b974e111` (smoke_icon 5/5, ir-run 31). |
| **⚡ Mode-4 Full Suite: SNOBOL4+Snocone** | `GOAL-MODE4-SN4-SNOCONE.md` | one4all+corpus+.github | **M4SN-4b ✅ target ≥250 HIT. NEXT: M4SN-4c.** one4all `1ef85cdc`. Sess 2026-05-15i (Opus 4.7): **+8 total (242→250/280)** — two iterations. Iter 1 (+3) closed numeric coercion: `shared_arith` SM_EXP int^int → INTVAL for SNOBOL4 via fixed gate; `rt_exp` delegates to `shared_arith`; `SM_COERCE_NUM`/`rt_coerce_num`/`h_coerce_num` (3-mode parity) scan `.eEdD` for INT-vs-REAL. Fixed `027`, `410`, `literals`. Iter 2 (+5) closed REM binary emission: `emit_bb_xstar` had asm only in `IS_TEXT`; binary mode emitted nothing so brokered REM was a no-op (Δ never updated). Added binary fallthrough mirroring XLNTH/XTB. Fixed `048`, `060`, `Qize_driver`, `test_stack`, `word4`. Gates all green; smoke_icon 3/5→5/5 (concurrent session pulled in). **Next (M4SN-4c)**: check `emit_bb_xatp` for same `IS_TEXT`-only emit-gap (+2 potential: `074`, `W07_capt_cur`); then DATA-type accessor cluster (+6 potential). |
...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **BLOCKED on GOAL-CHUNKS + new GOAL** — IEP-5/6 owned by CHUNKS; IEP-7 also blocked on proc_table[].proc and g_pl_pred_table holding live tree_t* (not covered by CHUNKS — new GOAL needed to lower Icon procs + Prolog clauses into dcg_table). IEP-4 ✅ `d9dff43a`. IEP-8 (sm_prog_free frees dcg_table) can proceed. |
 ...
 | **⚡ SN4 JS Emitter (IR_t-based, ladder climbing per PIVOT)** | `GOAL-SN4-JS-EMIT.md` | one4all+.github | **SJ4-JS-1..4c ✅; SJ4-JS-4d 🔄 in progress.** Sess 2026-05-15 (Claude Opus 4.7, one4all `e795385d`): four commits, ladder 31→44 PASS (handoff baseline was 10). Closed sub-rungs: INPUT EOF/blank distinction + `_FAIL` propagation + `_last_ok` on call (`a6c9b0e8`); arith fail-instead-of-throw (`04d01f7d`); pat-globals helper + real CONVERT (`284310c8`); pattern globals out of `_kw_store` — the BIG one, unlocked any/bal/case2/conv2/factor/len/loop/repl/space/trim0/trim1/words/f14_dyn_opt/longrec (`e795385d`). NEXT: pat-refname (`*H` deferred eval, blocks uneval/uneval2). Then: pattern-backtrack for `pat $ output FAIL`, then EVAL/CODE, then TABLE/SORT.
 | **SN4 .NET Emitter (IR_t-based, beauty self-host)** | `GOAL-SN4-NET-EMIT.md` | one4all+.github | **SN4-NET-1..4 ✅; 5a/5b/5c ✅ — recursive DEFINEs work** (sess 2026-05-15, Claude Opus 4.7, one4all `b3c0527d`). 5c step 3 frame save/restore landed: `frame_push` + `frame_save(name)` at SM_LABEL define_entry; `frame_exit()` at all RETURN/NRETURN/FRETURN sites AND on null-name `SM_CALL_FN` implicit return. `Fact(5)=120` ✅; `Greet('World')='Hello, World'` ✅. Gates: smoke_snobol4_net 9/9 (+define_recursive), smoke_snobol4/snocone/icon/prolog/rebus all unchanged, unified_broker 23/49 (unchanged). **NEXT: SN4-NET-5d** — SM_PAT_* opcode wiring to pat_*_* BB classes. Pattern matching is needed for beauty.sno. |
