...
| **Prolog BB** | `GOAL-PROLOG-BB.md` | CAT-D-1..9 ✅ (Opus 4.7, one4all `b1a37351`). CAT-D-9 (`b1a37351`, this session): all 12 mode-4 comparison ops (==, \==, @<, @>, @=<, @>=, =:=, =\=, <, >, <=, >=) — previously all fell through to bb_pl_builtin's "unknown stub" and silently succeeded. Two new effect helpers (rt_pl_term_cmp, rt_pl_arith_cmp) + single template arm via 7-scalar System V (op + 2×(k,i,s)). GATE-1 5/5, GATE-3 89/107, GATE-4 4/4, **GATE-2 15→19/107 (+4)**, mode-4 rung suite 15→19 (+4). +5 genuine PASSes (rung04 arith + 4× rung16_atop standard-order @-comparisons), -1 spurious (rung26 copy_term was relying on the bug). FACT RULE 0. Sibling smokes held. NEXT: CAT-A-3 (BB_PL_CALL + BB_CHOICE β-resume — needs Lon directive), CAT-D-6 (atom_chars/atom_codes — list cons-cell construction), CAT-D-9b (compound-term `==` — needs emit-time term walk like CAT-D-7's emit_write_term). |
