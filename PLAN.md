...
| Goal | File | Repo | Step |
|------|------|------|------|
| **CHUNKS — Eliminate SM_PUSH_EXPR** | `GOAL-CHUNKS.md` | one4all+.github | M1 CLOSED. **CH-13 LANDED sess #67** (Raku CASE: `SM_BB_PUMP_CASE` opcode replaces synthesised `emit_push_expr + SM_BB_PUMP` wrapper at sm_lower.c:1062; topic + per-arm (cmp_kind, val, body) chunks + optional default chunk; audit-clean on rk_given.raku). **Next inline: CH-14** (generator infrastructure — SM_SUSPEND/SM_RESUME). Step 8 (mode-4 x86 emitter) carved to `GOAL-MODE4-EMIT.md` — runs in PARALLEL with M4, file-disjoint. Don't confuse them: a session "doing GOAL-CHUNKS" defaults to its inline next rung (CH-14), NOT the carved sub-goal. |
| Mode-4 x86 Emitter (CHUNKS Steps 8 + 19) | `GOAL-MODE4-EMIT.md` | one4all | EM-5 LANDED sess #69 (chunk discipline; gate PASS=9). Three SM opcodes baked: SM_PUSH_CHUNK (descriptor push via scrip_rt_push_chunk_descr@PLT), SM_CALL_CHUNK (baked direct `call .Lpc<entry_pc>`), SM_RETURN (native ret). Two-chunks-calling-each-other gate green (rc=13). 5 tracked .s files regen'd. Honest deviation: emitter uses shared-stack call/ret vs. interpreter's snapshot-and-restore — byte-correct for clean chunk bodies; snapshot moves to EM-10. Next: EM-6 (pattern matcher integration; libscrip_rt v1; SPAN/BREAK/LEN/*expr/$ capture). |
...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | PARSER-SC-6c-bug ⏳ PASS=50 FAIL=0 (sess 7). Bug isolated to ~20-line repro `/tmp/redux.sc`; root cause architectural in else-if-recursion branch line 625 (`nPush() *if_cmd Save_nbody nPop()`) — STMT pushed at depth≥2 instead of depth 1 when inner if has nested if-else-if. Two-stack instrumentation confirms 1147 STMT pushes land at depth=1 vs 1148 expected. Next: per-construct firing trace `(construct, depth, frame_id)`. |
