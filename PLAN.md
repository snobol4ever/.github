...
| **SCRIP Head Quarters** | `GOAL-HEADQUARTERS.md` | one4all+corpus+(.github) | **one4all HEAD=`872c01d2`** ✅ GATE-PK 442/0/612 NEW=0 GONE=0, three-section audit GREEN, Prolog BB honest 124/0/0, smoke jit 184/186. **RUNG THREE-SECTION-X86 (TSX)** — DELETE every binary-byte-emitting function; raw bytes go INLINE in each template's MEDIUM_BINARY arm. **TSX-DELETE ladder ✅** DEL-0/1/2, W-LIT, W-PORT, W-PL. **TSX-WIRE-1/2/3 ✅** (`2e3b22b5`, insn_* 23→16). **TSX-DEL-FINAL ✅ `872c01d2` — insn_* family FULLY ELIMINATED 16→0.** FINAL-1 inlined brokered prologue bytes at call site; FINAL-2 inlined bb_lit text-arm sigma/lea as emit_2asm builders (cascade 11 insn_*); FINAL-3 converted emit_seq_port_call_rip text arm to plain fprintf (last 3 insn_* + B/U32/U64/T3C macros + t3/tf/tj statics gone); FINAL-4 deleted capture_bytes scaffolding + <functional>. Zero insn_* defs/decls. bb_emit_* sink STAYS (live via bb_sink_str); emit_seq_port_call_rip STAYS (text-asm, 6 mode-4 callers). **NEXT:** trivial sweep of pre-existing dead emit_seq_frame_*/mov_edx_i32/noop_macro + bb_insn_* legacy block; then ER-8 relocation rethink (bb_lit memcmp-PLT/rip-rel-lea/rel32). ⛔ PENDING Lon: TEMPLATE PURE-PROJECTION. ⚠️ pre-existing: sm_codegen_x64_emit_test harness SM.h build break. ⛔ Beauty gate SUSPENDED. |
