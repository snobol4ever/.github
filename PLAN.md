...
| **⚡ IR_t Emitter Foundation (prereq for JVM+JS)** | `GOAL-IR-EMITTER-PREREQ.md` | one4all+.github | **NEXT: IEP-4** — eliminate `IR_block_t*` pointer from `SM_EXEC_STMT`; replace with DCG roster index. IEP-1..3+vtable+include+rename ✅ one4all `1e7a7f5e`. IEP-4/5/6 added: remove all `tree_t*` and `IR_block_t*` from `SM_Program` for mode-4 correctness. |
