...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-3.f.1 (session 2026-04-30 #1: LS-4 attempted as monolithic Bison grammar; **architectural correction required** per Lon directive — "use clean emission technique just like SNOBOL4, use one pass when ever possible." Today's `snocone.l` (one4all `02db637d`) accumulates tokens into a `ScTokenBuf` via `emit_simple()`, a two-pass design left over from LS-3.c standalone token-stream tests. one4all convention (snobol4, raku, rebus) is single-pass: each Flex rule `return T_*;` directly, Bison pulls via `yylex`. New plan inserts **LS-3.f** (lexer single-pass refactor) before LS-4, and splits LS-4 into 11 finer sub-steps (LS-4.a–k): a/atoms+arith, b/comparisons+identity, c/concat+alt+match+assign, d/subscript+call, e/unary, f/if-while, g/do-for, h/function-return, i/switch+break+goto+struct, j/wire-in+old-file-removal, k/gates. ~870-line WIP grammar saved as `docs/LS-4-session-2026-04-30-1-attempt.snocone.y` — IR-construction semantic actions correct (port unchanged), only lex/parse plumbing was wrong. one4all working tree clean at `02db637d` after this commit (only docs/ added). Smoke gate 5/5 PASS verified at handoff. Findings: `docs/LS-4-session-2026-04-30-1-findings.md`. Naming convention check: SNOBOL4 uses `snobol4.l`→`snobol4.lex.c` and `snobol4.y`→`snobol4.tab.c`+`.tab.h`; Snocone matches: `snocone.l`→`snocone.lex.c` already, `snocone.y`→`snocone.tab.c`+`.tab.h` will follow. Naming aligned. Next session: implement LS-3.f.1 — rewrite `snocone.l` rules to `return T_*;` directly, drop accumulator, drop `ScTokenBuf` from production. The 31-test LS-1.b acceptance harness moves from `src/frontend/snocone/test_snocone_lex2.c` into `test/frontend/snocone/` and drives Flex via `yylex` token-streaming — that's a test concern, not production architecture.) |
