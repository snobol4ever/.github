...
| **Icon BB JCON triage** | `GOAL-ICON-BB-JCON.md` | one4all+corpus+.github | **DAI-8 C4 ✅ 2026-05-18 (Opus 4.7, one4all `de6e7b77`) — stmt_exec.c dead-fn sweep + ABSOLUTE RULE violation removal. −248 LOC.** Single-construct cluster targeting `src/runtime/snobol4/stmt_exec.c`: 10 functions + 1 typedef + supporting statics deleted (`patnd_is_invariant`, `cache_get_fresh`, `cache_stats`, **`bb_usercall`** (C BB violation!), `usercall_t`, `cache_test_run`, `nv_set_str`, `nv_get`, `deferred_var_test`, `anchor_test`, `exec_stmt_args`+fwd-decl). Methodology: Method 1 linker-GC nominated 17 dead `.text` sections; Method 6 caller-audit + **methodology refinement #7 (internal-caller chain analysis)** refined to 10 truly-deletable. Kept: `descr_match_span`/`descr_match` (static-inline-header artifacts, not deletable), `rt_in_native_chunk` (weak/strong override, methodology #6 deferred), `exec_stmt_pool_reset` (called from rt.c:697), `exec_stmt_blob` (called from rt.c:511), `xkind_name` (duplicate at snobol4_pattern.c:903, conservative keep). **Bonus structural fix:** `bb_usercall` was a `DESCR_t fn(void *zeta, int entry)` static — the exact C-BB signature ABSOLUTE-FORBIDDEN by RULES.md ("ZERO C BYRD BOX FUNCTIONS — NO EXCEPTIONS"). DAI-7 amputated the Icon C-BB family; C4 removes the last C-BB violation outside the permitted `icn_bb_dcg` shim. stmt_exec.c 680→432 LOC. All gates held floor: smoke ×6 (7/0,5/0,5/0,5/0,4/0,5/0); hello matrix 6/6 ROWS_DRIFT=0; Icon `--interp` 194/36/35/265; broker 23/26; crosscheck_prolog 128/0/4SKIP/11ORACLE_MISS; scrip_all_modes 2/0; SM dump round-trip stable. Prior: DAI-8 C3a + C3b ✅ 2026-05-18 (one4all `c3af9e23` + `a7259b9b`). **NEXT: DAI-8 cluster 5+** — remaining candidates: rt.c ~22 (needs transitive-rescue methodology #4 closure pass), icon_runtime.c (frontend) 25, emit_wasm.c 22, prolog_builtin.c 20, icn_runtime.c (interp) 18, snobol4_pattern.c 16. OR IJ-HELLO-MODE3-RAKU (mode-3 raku --run prints nothing; rt_call ladder needs mirroring in `sm_codegen_x64_emit.c`). |
...
| **⚡ PST: Icon + Raku** | `GOAL-PST-ICN-RAKU.md` | one4all+corpus+.github | **PRF-8 next** — PRF-1..7 ✅ 2026-05-18: finish_call_body/new_body/mcall_body/sub_body/method_body/class_body/main_body all eliminated from parser_raku.sc via inline shift/reduce. Technique: prefix-node-in-counter, nTop()+1 for below-frame obj, Emit_to_sub_list for sub_list emitters, lower_record updated for convention-2. 4 functions remain: push_interp_str+dq_unescape (PST-clean, leave), finish_given (PRF-8), finish_gather_body (PRF-9). |
