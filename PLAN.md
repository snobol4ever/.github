...
| Goal | File | Repo | Step |
|------|------|------|------|
| **CHUNKS — Eliminate SM_PUSH_EXPR** | `GOAL-CHUNKS.md` | one4all+.github | CH-0a (audit not yet started) — supersedes the carrier-fiction in RS-20; mode-4 prerequisite |
...
| **PARSER-SNOCONE (pattern frontend)** | `GOAL-PARSER-SNOCONE.md` | corpus+one4all | PARSER-SC-6b ⏳ PASS=46 FAIL=0. Lon rewrote parser_snocone.sc (new White/Gray/token design); 3 bugs fixed (missing `;` on White, `*Ident . captured_X` whitespace capture, Expr14 unary). PRIMER updated with `.` vs `$` execution model. Next: SC-6b beauty.sc crosscheck. |
