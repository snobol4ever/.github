...
| **Snocone Language: Space-Concat** | `GOAL-SNOCONE-LANG-SPACE.md` | one4all+corpus | LS-4.l partial (session 2026-05-01 #1: LS-4.k cleanup-fix landed — emergency handoff `eec7fd0f` left a stale `#include "frontend/snocone/snocone_control.h"` in driver/interp.c:26 (vestigial — no symbol from that header was used); build broke at driver/interp.o; one-line removal restores clean build. Gate state on the fix: smoke snocone **5/5 ✅** (new Bison parser as production, no legacy fallback), broker **49/0 ✅**, beauty **6/36/3 ⚠ expected** — corpus `.sc` files still use `&&` rejected by new parser, gate blocked on LS-5.a. LS-4.l now partial-verified: structural LS-4 close confirmed (parser is production), beauty 42/0/3 acceptance pending LS-5.a corpus migration. Next: LS-5.a — write migration script.) |
...
| NET Beauty Self-Host | `GOAL-NET-BEAUTY-SELF.md` | snobol4dotnet | S-2-bridge-7-fullscan (session #74, 2026-05-01 — diagnostic-only, no commit. Re-applied session #71's `[QPM]` C# function tracing in PatternMatch (Question Mark).cs + `_emitCount`-ticks-standalone patch in MonitorIpc.cs (.orig-backed, reverted). Standalone full-run beauty self-host trace = 1112 QPM events. Findings: line 47 `snoInteger = SPAN(digits)` SUCCESS at ec=12113 BetaStack=22; line 48 `snoDQ = '"' BREAK('"' nl) '"'` FAILS at ec=11265, again ec=12499, final ec=37775 BetaStack=31 — ~25k events of catastrophic backtracking between line-47 success and line-48 final failure. Trace shape is repeating triplet (snoFunctions FAIL → snoBuiltinVars FAIL → snoSpecialNms FAIL) at successive cursor positions; snoProtKwds/snoUnprotKwds NOT in failure set (no `&` keyword). Outcome shape is real FAILURE, NOT session #71's Failure-leak. Minimum repro isolated: single-line stdin `        x = '"' BREAK('"' nl) '"'` to beauty.sno reproduces dot Parse Error / spl clean. Confirms session #73 diagnosis: same alternation-graph-skip shape as line 26 (which session #71 fixed) — dot's snoExpr17 alternation falls through to keyword-list arms without trying `*snoString` at snoExpr17@line 187. Bug is structural in PatternAlternation (Pipe).cs or AbstractSyntaxTree.Build, NOT in scanner. Session #74 confirmed empirically what session #73 predicted: with current wire kinds (CALL/RETURN/VALUE/LABEL only), wire's last-agreed/first-diverged pair sits ~25k emit-events upstream of the structural divergence — the user's "adjacent C# trace reveals bug" strategy needs the structural event ON the wire to deliver. Two paths for session #75: (A) land S-2-bridge-coverage-pattern-traversal — new MWK_PM_ENTER/EXIT/NODE/BACKTRACK/MARK/SEAL + MWK_BETA_PUSH/COMMIT + MWK_FAILURE_FLIP wire kinds in monitor_wire.h, dot-side emit in MonitorIpc.cs first (smallest blast radius), spl-side later, controller recognition in monitor_sync_bin.py; (B) direct AST dump under DOT_DUMP_AST=1 — dump snoExpr17 alt-arm chain on dot, diff against beauty.sno grammar lines 167–189, fingerprint wrong Alternate link directly. Path B faster, narrower. Path A closes harness-trust gap. Recommend Path B first to unblock goal, Path A as separate rung. Real baseline at a629a15: 2073p/14f-Csnobol4-only/0s; Beauty 17/17 PASS; self-host UNCHANGED at 47 lines / Parse Error at snoDQ line 48. snobol4dotnet HEAD unchanged.) |
